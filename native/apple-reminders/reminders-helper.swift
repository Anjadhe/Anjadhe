...
if command == "events" {
    ...
    let rows: [[String: Any]] = events.compactMap { e in
        ...
        return [
            ...
            // Everything else EventKit knows about the event (2026-09-22),
            // so the app never shows less than Calendar.app does.
            ...
            "organizer": e.organizer.map { participantRow($0) } ?? NSNull(),
            "attendees": (e.attendees ?? []).map { participantRow($0) },
            "alarms": (e.alarms ?? []).map { alarmRow($0, iso) },
            "geo": geoRow(e.structuredLocation),
            "created": e.creationDate.map { iso.string(from: $0) } ?? "",
            "updated": e.lastModifiedDate.map { iso.string(from: $0) } ?? "",
        ]
    }
    emit(["calendars": calRows, "events": rows])
}

// ── Event field vocabulary (2026-09-22) ─────────────────────────────────
// Words, not raw enum numbers, and in Google's vocabulary where the two
// overlap (accepted/declined/tentative/needsAction), so the app reads both
// sources through one shape.
...
func participantRow(_ p: EKParticipant) -> [String: Any] {
    var email = ""
    let u = p.url.absoluteString
    if u.lowercased().hasPrefix("mailto:") { email = String(u.dropFirst(7)) }
    let status: String
    ...
    return ["name": p.name ?? "", "email": email, "status": status,
            "role": role, "type": type, "self": p.isCurrentUser]
}

func alarmRow(_ a: EKAlarm, _ iso: ISO8601DateFormatter) -> [String: Any] {
    if let d = a.absoluteDate { return ["at": iso.string(from: d)] }
    // relativeOffset is seconds, negative = before the start.
    return ["minutes": Int((-a.relativeOffset / 60).rounded())]
}

func geoRow(_ loc: EKStructuredLocation?) -> Any {
    guard let loc = loc, let g = loc.geoLocation else { return NSNull() }
    return ["title": loc.title ?? "", "lat": g.coordinate.latitude, "lng": g.coordinate.longitude]
}
