...
export function startRelay(port = DEFAULT_PORT) {
  ...
  function handleHello(ws, msg) {
    ...
    if (msg.role === 'host') {
      // Host continuity (2026-08-05, mirrors the hosted relay in
      // anjadhe-connect): a live host that presented a hostKey is only
      // replaced by a reconnect presenting the same key, so a stranger who
      // learns the routing id cannot evict the user's Mac. Keyless hellos
      // keep last-writer-wins against a keyless host; an empty slot may
      // always be claimed.
