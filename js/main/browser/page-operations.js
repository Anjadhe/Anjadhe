...
function look(epoch, options = {}, manualFields = '') {
    ...
    // The extension's own "nenva is working here" pill is not part of the page.
...
function dismissConsent(target) {
    ...
    if (target && (target.ref || (Number.isFinite(target.x) && Number.isFinite(target.y)))) {
        ...
        if (el.closest && el.closest('[data-anjadhe-shield]')) return { ok: false, outcome: 'refused', url: location.href, reason: 'That is nenva\u2019s own notice, not part of the page. Nothing was clicked.' };
