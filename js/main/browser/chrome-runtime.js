...
class ChromeRuntime {
    ...
    async _prepare() {
        ...
        bridge.on('state', state => {
            ...
            this.onChange({ manual: state.reason === 'user' });
        });
        bridge.on('connected', () => { this.target = {}; this.retryAfter = 0; this.settings.set('workroom-chrome-paired', true); this.onChange(); });
        bridge.on('disconnected', () => { this.target = {}; this.controlEpoch++; this.onChange(); });
        ...
        try {
            ...
            fs.writeFileSync(path.join(hosts, hostName + '.json'), JSON.stringify({
                name: hostName, description: 'Anjadhe browser connection', path: launcher, type: 'stdio',
    ...
    async view() {
        ...
        try { return await this.connection.request('view', {}, 8000); }
            ...
            return { status: 'idle', message: /Unsupported browser operation/.test(error.message)
                ? 'Reload the Anjadhe extension in Chrome (chrome://extensions) to see the browser here.' : error.message };
        }
    }
