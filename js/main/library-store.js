 ...
 *  V5  The read path is arithmetic: FTS5 BM25 + cosine over unit vectors,
 *      fused with reciprocal-rank fusion. No model judgment anywhere in
 *      retrieval (the query embedding is an encoding, not a judgment).
...
const LibraryStore = {
    ...
    _scanTimer: null,
    _draining: false,
    _matrixCache: null,   // { ids: number[], dims, data: Float32Array } — rebuilt lazily
    ...
    dir() {
        return process.env.ANJADHE_LIBRARY_DIR || path.join(os.homedir(), 'Anjadhe', 'library');
    },
    ...
    _deleteDocTx(docId) {
        ...
        for (const r of rows) this.db.prepare('DELETE FROM library_fts WHERE rowid = ?').run(r.id);
    ...
    async _indexDoc(doc) {
        ...
        try {
            ...
            const tx = this.db.transaction(() => {
                ...
                for (const r of old) this.db.prepare('DELETE FROM library_fts WHERE rowid = ?').run(r.id);
    ...
    async search(query, { k = 8, collection } = {}) {
        ...
        if (this.embed && this.embed.isInstalled && this.embed.isInstalled()) {
            try {
                ...
                vecRank = this._vectorRank(qvec, this.CANDIDATES);
            } catch (e) {
                console.warn('[library] vector search unavailable:', e.message);
            }
        }
        const ftsRank = this._ftsRank(q, this.CANDIDATES);
