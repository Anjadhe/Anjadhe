 ...
 *  V1  Originals are FILES at ~/nenva/library/ (ANJADHE_LIBRARY_DIR
...
const LibraryStore = {
    ...
    dir() {
        ...
        // The content root (~/nenva, or ~/nenva where it could not move)
        // honours ANJADHE_DATA_ROOT itself — the DATA_ROOT law: a test
        // instance must never open the real library.
        return path.join(require('./content-root').resolveRoot(), 'library');
    },
    ...
    tidySet(docId, { text, coveredChars, model, method, pages, pagesDone } = {}) {
        ...
        try {
            ...
            const front = [
                ...
                rec.method === 'vision'
                    ? 'generated: nenva transcribed the page images of the source file above with a vision model; the source is the document.'
                    : 'generated: nenva tidied this from the source file above for reading; the source is the document.',
                '---',
                ''
            ].join('\n');
