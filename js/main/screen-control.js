...
function register({ ipcMain, BrowserWindow }) {
    ...
    ipcMain.handle('screen-capture', async (event) => {
        ...
        if (!win) return { error: 'Only the Anjadhe window can be captured.' };
        if (win.isMinimized() || !win.isVisible()) {
            return { error: 'The Anjadhe window is minimized or hidden, so there is nothing to see. Ask the user to bring it back on screen.' };
        }
        try {
    ...
    ipcMain.handle('screen-input', async (event, req) => {
        const win = ownWindow(event);
        if (!win) return { error: 'Only the Anjadhe window can be operated.' };
        const wc = event.sender;
        const z = (typeof wc.getZoomFactor === 'function' && wc.getZoomFactor()) || 1;
