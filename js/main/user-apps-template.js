/**
 * Templates written into ~/nenva/apps/ when the user enables app building
...
const AGENT_DOCS = `# Building nenva Apps

This folder holds user-built apps for nenva. Each subfolder is one app:
...
Apps load when nenva starts and **hot-reload automatically** when you save
changes while nenva is running — no restart needed. If an app fails to
load or throws at runtime, nenva appends the error to \`<app-id>/.errors.log\`.
...
Apps can build on the user's existing data (journal, schedule, projects,
...
Apps run inside nenva's window, so \`fetch()\` works for public,
