 ...
 *    enabled in Settings (off by default) AND the renderer has asked for
 *    it (`ensure()`, called from AppManager.init only when the `terminal`
 *    feature flag is on — since 2026-09-09 the feature is flag-gated, and
 *    main must not listen on a build where the flag is off even if the
 *    setting was left on earlier).
...
// Overridable so a blank-slate test instance (ANJADHE_DATA_ROOT) can enable
// terminal access while the real app holds the default port.
const CLI_PORT = parseInt(process.env.ANJADHE_CLI_PORT, 10) || 18789;
...
const CLIServer = {
    ...
    init(settingsStore, getWindow, appVersion) {
        ...
        // Deliberately no auto-enable here: see ensure().
    },

    /**
     * Start listening if the user turned Terminal access on — called by the
     * renderer ('cli-ensure') once per launch, and only when the `terminal`
     * feature flag is on. The flag lives in the renderer (localStorage
     * override + FEATURE_DEFAULTS), so this is the one place main learns it.
     */
    async ensure() {
        if (this._server) return { enabled: true, port: CLI_PORT };
        if (this._store.get('cliEnabled', false) !== true) return { enabled: false };
        try {
            return await this.enable();
        } catch (e) {
            console.error('[cli] enable at startup failed:', e.message);
    ...
    /**
     * Stop listening WITHOUT forgetting the user's choice: the renderer
     * calls this when the `terminal` flag is off, so a flag flipped off
     * mid-session (Settings › Developer, then reload) takes the server
     * down now rather than at the next launch; flipping it back on
     * re-enables through ensure() with no setting to redo.
     */
    suspend() {
        if (!this._server) return { enabled: false };
        for (const [, p] of this._pending) { try { p.res.end(); } catch {} }
        this._pending.clear();
        try { this._server.close(); } catch {}
        this._server = null;
        try { fs.unlinkSync(CLI_CONFIG_PATH); } catch {}
        console.log('[cli] terminal access suspended (feature flag off)');
    ...
    installCommand() {
        ...
        const launcher = '#!/bin/sh\n'
            + '# anjadhe — generated by nenva (Settings → AI Assistant → Terminal Access)\n'
            + `APP_BIN=${shq(process.execPath)}\n`
            + `CLI_JS=${shq(script)}\n`
            + 'if [ ! -x "$APP_BIN" ] || [ ! -f "$CLI_JS" ]; then\n'
            + '    echo "The nenva app this command belonged to is no longer installed - removing the command." >&2\n'
    ...
    async _handle(req, res) {
        ...
        try {
            ...
            if (!win || win.isDestroyed()) {
                this._json(res, 503, { error: 'The nenva window is not open — open the app and try again.' });
                return;
            }
            ...
            if (req.method === 'POST' && url === '/v1/command') {
                // Generic command channel (model list/switch, new conversation,
                // …): same relay as ask/task, but the renderer answers with a
                // single `done` event carrying structured `data` instead of a
                // streamed turn.
                const body = await this._readBody(req);
                const command = String(body.command || '').trim();
                if (!command) { this._json(res, 400, { error: 'command required' }); return; }
                const requestId = `cli_${Date.now().toString(36)}_${++this._seq}`;
                res.writeHead(200, { 'Content-Type': 'application/x-ndjson', 'Cache-Control': 'no-cache' });
                this._pending.set(requestId, { res });
                req.on('close', () => { this._pending.delete(requestId); });
                win.webContents.send('cli-request', {
                    requestId,
                    kind: 'command',
                    command,
