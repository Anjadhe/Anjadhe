 ...
 * must never operate, and shows the floating "Anjadhe is using your Mac ·
 ...
 *       Anjadhe itself (screen_act owns that, with its own no-touch rules).
...
const BLOCKED_NAMES = /^(keychain access|passwords|system settings|system preferences|terminal|iterm2?|warp|ghostty|kitty|alacritty|1password.*|bitwarden|lastpass|dashlane|keepassxc|anjadhe|electron)(\.app)?$/i;
...
const blockedMessage = (label) => `Anjadhe never operates ${label || 'that app'} — password managers, Keychain, System Settings, terminals and Anjadhe itself are off limits (use screen_act for Anjadhe's own window, run_command for shell work). Ask the user to do that part.`;
...
function register({ app, ipcMain, BrowserWindow, screen }) {
    ...
    const BAR_HTML = `<!doctype html><html><head><meta charset="utf-8"><style>
        ...
        </style></head><body><div class="bar"><span class="dot"></span><span id="l">Anjadhe is using your Mac</span><a href="#stop">Stop</a></div></body></html>`;
    ...
    function showBar(label) {
        ...
        const set = () => bar.webContents.executeJavaScript(
            `document.getElementById('l').textContent = ${JSON.stringify(String(label || 'Anjadhe is using your Mac').slice(0, 80))}`
        ).catch(() => {});
    ...
    ipcMain.handle('mac-snapshot', async (_e, opts = {}) => {
        ...
        if (res && !res.error) {
            ...
            showBar(`Anjadhe is looking at ${res.app}`);
        }
        return res;
    });
    ...
    ipcMain.handle('mac-act', async (_e, step) => {
        ...
        showBar(norm.kind === 'open'
            ? `Anjadhe is opening ${norm.app || 'your browser'}`
            : `Anjadhe is using ${target.app}`);
    ...
    // A consent ask while another app is in front: bring Anjadhe forward so
