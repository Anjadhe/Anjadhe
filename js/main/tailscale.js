...
// The standalone CLI first: it is a CLI whoever launches it. The app
// bundle's binary is last, and is always run with TAILSCALE_BE_CLI=1 (see run).
...
function run(args, timeout = 8000) {
    ...
    // The app bundle's binary decides between CLI and GUI by how it was
    // launched: from a terminal it is the CLI, from another app (nenva's main
    // process, no TTY) it tries to start the Tailscale GUI, prints "The
    // Tailscale GUI failed to start: …" and exits 0 — which read as "could not
    // read the Tailscale status" in Settings (2026-09-26). TAILSCALE_BE_CLI=1
    // is Tailscale's own switch for CLI mode; the standalone CLI ignores it.
    const r = spawnSync(cli, args, { encoding: 'utf8', timeout, env: { ...process.env, TAILSCALE_BE_CLI: '1' } });
...
function selfInfo() {
    ...
    try {
        ...
        // Say what Tailscale actually printed; a bare "could not read" hid
        // the GUI-mode reply above for a day.
        const said = String(r.out || '').trim().split('\n')[0].slice(0, 160);
        return { ok: false, error: said ? `could not read the Tailscale status (Tailscale said: ${said})` : 'could not read the Tailscale status' };
    }
}
