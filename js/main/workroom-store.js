...
class WorkroomStore {
    constructor(dir, { legacyFile = null, onEvent = () => {} } = {}) {
        ...
        for (const room of this.rooms) if (['running', 'queued'].includes(room.status) || room.approval) {
            ...
            if (wasRunning) this.append(room, { type: 'status', state: 'paused', text: 'Work stopped when nenva restarted. Continue to pick it up from here.' }, { quiet: true });
        }
        this.saveIndex();
    }
    ...
    command(action, input = {}, sender) {
        ...
        if (action === 'approval') {
            ...
            if (event.type === 'task_end') Object.assign(clean, { id: this.text(event.id, 60), agent: this.text(event.agent, 40),
                status: ['done', 'partial'].includes(event.status) ? event.status : 'blocked', text: this.text(event.text, 60000),
                ...(event.note ? { note: this.text(event.note, 600) } : {}),
