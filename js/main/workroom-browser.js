...
class WorkroomBrowser {
    ...
    async execute(sender, { action, args = {}, run, image = false, tainted = false } = {}) {
        ...
        if (!this.extensionReady()) throw new Error('The Anjadhe extension loaded in Chrome is older than this app. Open chrome://extensions, reload the Anjadhe extension, then Continue.');
        let waitedMs = 0;
        if (action === 'act') {
