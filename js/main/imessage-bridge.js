 ...
 *
 * Since 2026-09-10 the same script also carries the assistant's
 * `send_text` tool (`sendTo`): any handle the user names, consent asked
 * per send in the renderer (PermissionManager; a standing grant covers
 * the user's own handle only). Main validates the handle's shape and
 * sends — it never decides whether a send is wanted.
 *
...
function createIMessageBridge({ settingsStore, runScript }) {
     ...
     * (js/main/notify-format.js) — iMessage has no bold, so the kind on
     * the first line ("Reminder") does the whole job of saying what this
     * is and why your own number is texting you.
     */
    ...
    function sendTest() {
        return send(notificationText('Test from nenva', "Notifications from this Mac will arrive here.", '').text);
    }

    return { status, setNotify, setHandle, send, sendTo, notifyToLinked, sendTest, SEND_SCRIPT };
}
