...
const MCPManager = {
    ...
    async callTool(name, toolName, args) {
        try {
            ...
            // Boundary adjustments (js/main/mcp-args.js) — e.g. a screenshot
            // must come back as an image, not a link to a file on disk.
            const res = await conn.callTool(toolName, prepareToolArgs(toolName, args));
...
const { prepareToolArgs } = require('./mcp-args');
