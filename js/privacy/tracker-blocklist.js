...
// Gatekeepers: bot-protection and CAPTCHA vendors. EasyPrivacy lists
// several of these as trackers — which they are — but blocking them does
// not protect the user, it locks them out. The site has already decided
// nobody gets in until this script runs, so a blocked script reads to the
// WAF as "JavaScript disabled or not working" and the page never loads;
// worse, when a challenge does appear ("Press & Hold to confirm you are a
// human"), its own widget is the thing we blocked, so the user cannot even
// solve it. Verified 2026-08-02: zillow.com 403'd with client.px-cloud.net
// blocked, and datadome.co / client.perimeterx.net were both on the live
// EasyPrivacy list.
//
// The line: a host is a gatekeeper only if a site uses it to decide
// whether to serve the page at all. Pure analytics and fingerprinting-for-
// advertising vendors (fingerprint.com, castle.io, go-mpulse.net) stay
// blocked — a page still renders without them.
const GATEKEEPER_HOSTS = new Set([
    // DataDome
    'datadome.co', 'captcha-delivery.com',
    // PerimeterX / HUMAN
    'perimeterx.net', 'px-cdn.net', 'px-cloud.net', 'px-client.net', 'pxchk.net',
    // Akamai Bot Manager sensor delivery
    'aksb-a.akamaihd.net', 'ds-aksb-a.akamaihd.net',
    // Imperva / Incapsula
    'incapsula.com', 'imperva.com',
    // Kasada
    'kasada.io', 'kasadapartners.com',
    // Cloudflare Turnstile / challenge
    'challenges.cloudflare.com',
    // CAPTCHA widgets
    'hcaptcha.com', 'recaptcha.net', 'arkoselabs.com', 'funcaptcha.com',
    // Virtual waiting rooms — blocking these strands the user in the queue
    'queue-it.net'
]);
 ...
 *   on the exception list, and not a gatekeeper).
 */
function isBlocked(url) {
    try {
        ...
        if (_hostMatches(host, GATEKEEPER_HOSTS)) return false;
        if (_hostMatches(host, _state.exceptionHosts)) return false;
        return _hostMatches(host, _state.blockedHosts);
    } catch { return false; }
}

/** @returns {boolean} true if the URL is a bot-defense / CAPTCHA gatekeeper. */
function isGatekeeper(url) {
    try {
        return _hostMatches(new URL(url).hostname.toLowerCase(), GATEKEEPER_HOSTS);
    } catch { return false; }
}
...
module.exports = { init, isBlocked, isGatekeeper, parseList, stats };
