...
(function (factory) {
  ...
  function createStore(post, getSync, onHydrated) {
    ...
    // Keys THIS phone changed since the Mac last took them (2026-09-25), as
    // { key: modifiedAt of our write }. The manifest carries the list so the
    // Mac can MERGE a record-merged key it holds a newer copy of, instead of
    // sending its copy down over ours (main.js handleSyncManifest `dirty`).
    // An entry clears only when the Mac acks the exact write it names; a
    // later write, or a Mac copy replacing ours, settles it too. Device-local
    // (the channel prefix): persisted, never advertised or uploaded.
    const DIRTY_KEY = 'anjadhe:channel:dirty';
    function dirtyMap() {
      const e = cache.get(DIRTY_KEY);
      return (e && !e.deleted && e.value && typeof e.value === 'object') ? e.value : {};
    }
    function writeDirty(map) {
      const entry = { value: map, modifiedAt: new Date().toISOString() };
      cache.set(DIRTY_KEY, entry); persist(DIRTY_KEY, entry);
    }
    function markDirty(key, at) {
      if (isLocalOnly(key)) return;
      const m = Object.assign({}, dirtyMap()); m[key] = at; writeDirty(m);
    }
    function settleDirty(key) {
      const m = dirtyMap();
      if (!(key in m)) return;
      const next = Object.assign({}, m); delete next[key]; writeDirty(next);
    }
    ...
    const electronStore = {
      ...
      set: function (key, value) {
        ...
        cache.set(key, entry); persist(key, entry); if (!isLocalOnly(key)) { markDirty(key, entry.modifiedAt); scheduleLocalPush(); } return true;
      },
      delete: function (key) {
        const entry = { deleted: true, tombAt: new Date().toISOString() };
        cache.set(key, entry); persist(key, entry); if (!isLocalOnly(key)) { markDirty(key, entry.tombAt); scheduleLocalPush(); } return true;
      },
    ...
    const anjadheStore = {
      ...
      applyRemote: function (key, value, modifiedAt) {
        ...
        cache.set(key, entry); persist(key, entry); settleDirty(key); return true;
      },
      ...
      /** Keys changed on this phone and not yet taken by the Mac. */
      exportDirty: function () { return Object.keys(dirtyMap()); },
      /** The Mac acked an upload: `{key: modifiedAt}` as sent. A key written
       *  again since keeps its mark. */
      clearDirty: function (sent) {
        const m = Object.assign({}, dirtyMap());
        let changed = false;
        Object.keys(sent || {}).forEach(function (k) { if (m[k] !== undefined && m[k] === sent[k]) { delete m[k]; changed = true; } });
        if (changed) writeDirty(m);
      },
    ...
    const bridge = {
      ...
      applyLocalWrite: function (key, value, modifiedAt) {
        ...
        cache.set(key, { value: value, modifiedAt: at });
        markDirty(key, at);
        scheduleLocalPush();
      },
      applyLocalDelete: function (key, tombAt) {
        const at = tombAt || new Date().toISOString();
        cache.set(key, { deleted: true, tombAt: at });
        markDirty(key, at);
        scheduleLocalPush();
      },
      // "Re-download from Mac": forget every synced key in the mirror (the
      // device-local `keepPrefix` keys — pairing, identity — stay) so the next
      // manifest is empty and the Mac's copy wins outright. Native purges its
      // own store in the same step; nothing is persisted or uploaded here.
      reset: function (keepPrefix) {
        Array.from(cache.keys()).forEach(function (k) {
          if (!(keepPrefix && k.indexOf(keepPrefix) === 0)) cache.delete(k);
        });
        // Nothing of ours is left to merge.
        writeDirty({});
      },
      _cache: cache, // tests
    };
