...
(function () {
  ...
  async function connect() {
    ...
    if (!winner && !closed) {
      ...
      // When the relay fails too, the Mac itself was unreachable (quit,
      // asleep, restarting) — penalising the LAN for two minutes then kept
      // the phone off the direct path after the Mac came back.
      if (winner && lanTried) noteLanFailure();
    }
