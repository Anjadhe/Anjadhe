...
(function () {
  ...
  async function acceptOfferText(text) {
    ...
    if (!offer || offer.v !== 1 || !offer.relayUrl || !offer.routingId) {
      return { ok: false, error: 'That is not a nenva pairing code.' };
    }
    try {
      ...
      const lanUrls = Array.isArray(offer.lanUrls)
        ...
        : [];
      // A wss url is the Mac's relay behind Tailscale Funnel (public https
      // name): reachable from any network, slower to shake hands than a LAN.
      const rungs = lanUrls.map((url) => ({ url, timeoutMs: /^wss:\/\//.test(url) ? 10000 : 4000 }))
        ...
        try {
          result = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(
              () => reject(new Error('Timed out. Is your Mac on the same Wi-Fi with nenva open?')),
              rung.timeoutMs,
            )),
          ]);
          pc.close();
