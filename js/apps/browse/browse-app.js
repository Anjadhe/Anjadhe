...
const BrowseApp = {
    ...
    initialized: false,
    // True while the browser view is off screen; every tab is muted then.
    _hidden: false,
    ...
    /**
     * Navigated away from the browser (AppManager calls this).
     *
     * Hiding the view is display:none, which a <webview> ignores for audio —
     * a playing page kept making noise from behind the home feed. Mute every
     * tab on the way out and unmute in render() on the way back in.
     *
     * Muting rather than stopping is deliberate: tabs are meant to survive
     * you leaving the app, so a video has to still be where you left it when
     * you return. Silence is what the user is missing, not playback.
     */
    onHide() {
        this._setAllTabsMuted(true);
    },

    _setAllTabsMuted(muted) {
        this._hidden = muted;
        for (const tab of this.tabs) {
            // setAudioMuted throws until the guest page is attached, and a
            // tab sitting on the blank state never attaches at all.
            try { tab.webview?.setAudioMuted(muted); } catch (e) { /* not ready */ }
        }
    },
    ...
    // The store belongs to the Bookmarks package; Browse acts through the
    // API it exposes (Anjadhe.use — docs/PLATFORM.md "App packages"). With
    // no such package installed the star is inert and the list is empty.
    _bookmarksApi() {
        return (typeof Anjadhe !== 'undefined') ? Anjadhe.use('bookmarks') : null;
    },


    _loadBookmarks() {
        const api = this._bookmarksApi();
        // Most-recent first — the API's order, matching the Bookmarks app.
        this.bookmarks = api ? api.list() : [];
    },
    ...
    _toggleBookmark() {
        ...
        const api = this._bookmarksApi();
        if (!api) { this._toast('The Bookmarks app is not available.', 'error'); return; }
        if (api.has(url)) {
            api.removeByUrl(url);
            this._toast('Bookmark removed');
        } else {
            api.add({ title: (tab && tab.title) || url, url, group: null });
    ...
    _wireQuickLinks() {
        ...
        if (this.promptsManageBtn) {
            this.promptsManageBtn.addEventListener('click', () => {
                // Saved prompts live in Notes; the Routines page (app id
                // 'prompts') only holds scheduled ones.
    ...
    _goHome() {
        // Reset the active tab back to the blank/home state without closing
        // it. The user can submit a new URL or pick a bookmark to navigate
        // again.
        this._closeHistoryView();
        if (this.readerOpen) this._closeReader();
        const tab = this._activeTab();
        ...
        if (tab.webview) {
            ...
            // ...and unload the page that already loaded. stop() only cancels
            // a load in flight; hiding the webview is display:none, which a
            // guest ignores. Without this the page kept running behind the
            // blank state — audibly so, if it was playing media.
            //
            // Navigating to about:blank fires did-navigate asynchronously,
            // which would repopulate tab.url and undo the blank state, so
            // flag the tab and let the handlers skip that one event.
            tab.blanking = true;
            try { tab.webview.src = 'about:blank'; } catch (e) { /* not ready */ }
        }
        tab.url = '';
        tab.title = 'New tab';
    ...
    _createTab({ url = null, activate = false, isPrivate = false } = {}) {
        ...
        // No `useragent` attribute on purpose. The UA is set once on the
        // session in main.js (`_configureBrowseSession`), derived from the
        // Chromium the app is actually running. A literal typed here would
        // override that and go stale on the next Electron upgrade, which is
        // exactly the mismatch bot protection blocks on.
    ...
    _wireWebviewEvents(tab) {
        ...
        // A guest can attach while the browser is off screen — a background
        // page opening a window, or a tab still loading when the user left.
        // setAudioMuted only sticks once the guest exists, so re-apply the
        // current state here or that tab comes up audible.
        wv.addEventListener('dom-ready', () => {
            try { wv.setAudioMuted(!!this._hidden); } catch (e) { /* not ready */ }
        });
        ...
        wv.addEventListener('did-stop-loading', () => {
            ...
            // One-shot auto reader (openInBrowse readerMode — article links
            // from News/Discover). Google News URLs are redirector hops,
            // not the article: hold the flag until the destination loads
            // (did-navigate closes any reader, so no stale view survives).
            if (tab.autoReader && tab.id === this.activeTabId && tab.url) {
                let host = '';
                try { host = new URL(tab.url).hostname; } catch {}
                if (!/(^|\.)news\.google\.com$/i.test(host)) {
                    tab.autoReader = false;
                    if (!this.readerOpen) this._openReader(true);
                }
            }
        });
        wv.addEventListener('did-navigate', (e) => {
            const url = e.url || wv.getURL?.() || '';
            // The about:blank that _goHome navigated to in order to unload
            // the page. Swallow it so the tab stays in its blank state.
            // Consume the flag either way, so a stale one can never eat a
            // real navigation.
            if (tab.blanking) {
                tab.blanking = false;
        ...
        wv.addEventListener('page-title-updated', (e) => {
            // A tab sitting on the blank state has no page — ignore the title
            // the unloading about:blank reports, or the tab strip relabels
            // itself "about:blank" instead of "New tab".
            if (!tab.url) return;
            const title = e.title || '';
            tab.title = title;
    ...
    // auto=true (auto reader from News/Discover links): a page with no
    // extractable article closes the reader silently instead of parking an
    // error overlay on a page the user never asked to strip.
    async _openReader(auto = false) {
        const tab = this._activeTab();
        if (!tab || !tab.url || !tab.webview) {
            if (!auto) this._toast('Open a page first', 'error');
            return;
        }
        this.readerOpen = true;
        this.readerBtn?.setAttribute('aria-pressed', 'true');
        ...
        try {
            ...
            if (!result || !result.html) {
                if (auto) { this._closeReader(); return; }
            ...
            if (auto) { this._closeReader(); return; }
            this._renderReaderEmpty('Reader mode failed on this page.');
        }
    },
    ...
    async _renderReader(result) {
        ...
        this.readerArticle.innerHTML = `${this._readerHeadHtml(result)}<div class="browse-reader-content">${safe}</div>`;
    ...
    /**
     * The article header: title, dek, then one meta line — author · site ·
     * published date (· updated) · reading time — and the lead image when
     * the body carries none. Everything here comes from the page's own
     * metadata (JSON-LD, Open Graph, <time>); nothing is model-written.
     */
    _readerHeadHtml(result) {
        const esc = (s) => this._readerEscape(s);
        const parts = [];
        if (result.byline) parts.push(`<span class="browse-reader-author">${esc(result.byline)}</span>`);
        if (result.siteName) parts.push(esc(result.siteName));
        const pub = this._readerDate(result.datePublished);
        const mod = this._readerDate(result.dateModified);
        if (pub) parts.push(`<time datetime="${esc(pub.iso)}" title="${esc(pub.full)}">${esc(pub.label)}</time>`);
        if (mod && (!pub || mod.day !== pub.day)) parts.push(`<time datetime="${esc(mod.iso)}" title="${esc(mod.full)}">Updated ${esc(mod.label)}</time>`);
        if (result.wordCount) {
            const mins = Math.max(1, Math.round(result.wordCount / 220));
            parts.push(`${mins} min read`);
        }
        const title = result.title ? `<h1 class="browse-reader-title">${esc(result.title)}</h1>` : '';
        const dek = result.description ? `<p class="browse-reader-dek">${esc(result.description)}</p>` : '';
        const byline = parts.length ? `<div class="browse-reader-byline">${parts.join('<span class="browse-reader-sep">&middot;</span>')}</div>` : '';
        const img = result.leadImage && /^https?:/i.test(result.leadImage)
            ? `<figure class="browse-reader-lead"><img src="${esc(result.leadImage)}" alt="" loading="lazy" referrerpolicy="no-referrer"></figure>`
            : '';
        return `<header class="browse-reader-head">${title}${dek}${byline}</header>${img}`;
    },

    /** ISO/loose date string → {iso, label, full, day}; null when unparseable. */
    _readerDate(raw) {
        if (!raw) return null;
        let s = String(raw).trim();
        // "2026-08-27" alone parses as UTC midnight and can show yesterday
        // locally; treat a bare date as a local calendar day.
        const bare = /^\d{4}-\d{2}-\d{2}$/.test(s);
        const d = bare ? new Date(s + 'T12:00:00') : new Date(s);
        if (Number.isNaN(d.getTime())) return null;
        const hasTime = !bare && /\d{1,2}:\d{2}/.test(s);
        const sameYear = d.getFullYear() === new Date().getFullYear();
        const label = d.toLocaleDateString(undefined, { month: 'short', day: 'numeric', ...(sameYear ? {} : { year: 'numeric' }) })
            + (hasTime ? ', ' + d.toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit' }) : '');
        const full = d.toLocaleString(undefined, { dateStyle: 'full', ...(hasTime ? { timeStyle: 'short' } : {}) });
        return { iso: d.toISOString(), label, full, day: d.toDateString() };
    },
    ...
    async _improveReaderWithAI() {
        ...
        const head = this.readerArticle.querySelector('.browse-reader-head');
        ...
        if (head) this.readerArticle.appendChild(head);
        ...
        const systemPrompt = [
            ...
            'The title, author and date are already shown above your output — never repeat them. Use ## for section headers, blank lines between paragraphs, and "-" bullet lists. No code fences. No commentary before or after the article. Output only the cleaned markdown.',
        ...
        const params = {
            ...
            // repeat_penalty + repeat_last_n are sampler options that bias
    ...
    _renderPromptsSection(escape) {
        ...
        // Saved prompts are notes with the 'prompt' template — read them via
        // the shared NotePrompts helper. Routines (scheduled prompts) run in
        // the background and post to the feed; only the run-by-hand saved
        // prompts belong on a shelf whose action is "run this in the browser".
        let prompts = (typeof NotePrompts !== 'undefined')
            ? NotePrompts.list().filter(p => !NotePrompts.config(p).offline) : [];
        if (prompts.length === 0) {
            this.promptsList.innerHTML =
                '<p class="browse-pane-empty">No saved prompts yet. Create one in Notes with the Saved prompt template.</p>';
            return;
        }
 ...
 *
 * Visible text is not always enough. DataDome's interstitial ships a body
 * of one line ("Please enable JS and disable any ad blocker") and builds
 * the rest from script, so at did-stop-loading `innerText` is empty and
 * every text rule above misses — that is why mensfitness.com showed a
 * bare block page with no banner over it (2026-08-02). When the visible
 * text is that thin we fall back to the markup, which still names the
 * vendor in the script tag it just inserted.
 */
...
(function() {
    try {
        ...
        // Note the plain ampersand: this used to read "press &amp; hold",
        // which is how the page's HTML spells it but never how innerText
        // does, so the PerimeterX rule could not fire.
        if (/perimeterx|press\\s*&\\s*hold to confirm you are a human|please verify you are a real person/i.test(body)) {
        ...
        // Nothing readable on the page. Now — and only now — look for the
        // vendor's own interstitial. Every site behind one of these ships
        // its script on ordinary pages too, so we match the markers that
        // exist ONLY on the block/challenge shell, and require a page with
        // no links: a real page that is still hydrating has navigation,
        // a block page has nowhere to go.
        const linkless = !document.querySelector('a[href]');
        if (linkless && body.trim().length < 400) {
            const html = (document.documentElement && document.documentElement.innerHTML || '').slice(0, 30000);
            // DataDome's shell declares a global \`dd\` naming its captcha host.
            if ((window.dd && window.dd.host) || /geo\\.captcha-delivery\\.com/i.test(html)) {
                return { blocked: true, reason: 'datadome' };
            }
            if (document.getElementById('px-captcha') || /\\/px\\/captcha|px-cloud\\.net\\/captcha/i.test(html)) {
                return { blocked: true, reason: 'perimeterx' };
            }
            if (document.getElementById('challenge-form') || document.getElementById('cf-error-details')
                || /cdn-cgi\\/challenge/i.test(html)) {
                return { blocked: true, reason: 'cloudflare' };
            }
            if (/_incapsula_resource/i.test(html)) return { blocked: true, reason: 'imperva' };
            if (/enable js and disable any ad ?block/i.test(html)) return { blocked: true, reason: 'unknown' };
        }
        ...
        if (tinyBody && /access denied|forbidden|blocked|verify you are|access (is )?(temporarily )?restricted/i.test(title)) {
...
(function() {
    ...
    // ---- Article metadata, read from the ORIGINAL document (the clone
    // below strips <header>/<time>, which is where sites keep the date —
    // that stripping is why the reader used to show no date at all).
    // Sources in order of trust: JSON-LD (NewsArticle/Article/BlogPosting,
    // also inside @graph), Open Graph / standard <meta>, then the first
    // <time datetime> in or near the article.
    function ld() {
        var out = {};
        var scripts = document.querySelectorAll('script[type="application/ld+json"]');
        var types = /^(News|Blog|Report|Scholarly|Tech|Analysis|Opinion|Review|Live|Satirical)?(Article|Posting|BlogPosting|WebPage)$/i;
        function visit(o, depth) {
            if (!o || typeof o !== 'object' || depth > 4) return false;
            if (Array.isArray(o)) { for (var i = 0; i < o.length; i++) if (visit(o[i], depth + 1)) return true; return false; }
            var t = o['@type'];
            if (Array.isArray(t)) t = t.join(' ');
            if (t && types.test(String(t).split(' ')[0]) && (o.datePublished || o.headline || o.author)) {
                out.datePublished = o.datePublished || '';
                out.dateModified = o.dateModified || '';
                out.headline = o.headline || '';
                out.description = o.description || '';
                var a = o.author;
                var names = [];
                (Array.isArray(a) ? a : (a ? [a] : [])).forEach(function(x) {
                    if (typeof x === 'string') names.push(x);
                    else if (x && x.name) names.push(String(x.name));
                });
                out.author = names.join(', ');
                return true;
            }
            if (o['@graph']) return visit(o['@graph'], depth + 1);
            return false;
        }
        for (var s = 0; s < scripts.length; s++) {
            try { if (visit(JSON.parse(scripts[s].textContent || ''), 0)) break; } catch (e) { /* malformed block */ }
        }
        return out;
    }
    function metaAny(names) {
        for (var i = 0; i < names.length; i++) {
            var v = meta(names[i]);
            if (!v) { var ip = document.querySelector('[itemprop="' + names[i] + '"]'); v = ip ? (ip.getAttribute('content') || ip.getAttribute('datetime') || '').trim() : ''; }
            if (v) return v;
        }
        return '';
    }
    var root = pickRoot();
    var info = ld();
    var timeEl = (root && root.querySelector('time[datetime]')) || document.querySelector('article time[datetime], header time[datetime], time[datetime]');
    var datePublished = info.datePublished
        || metaAny(['article:published_time', 'og:article:published_time', 'datePublished', 'pubdate', 'publishdate', 'publish-date', 'date', 'DC.date.issued', 'dc.date', 'parsely-pub-date', 'sailthru.date', 'article.published'])
        || (timeEl ? timeEl.getAttribute('datetime') : '');
    var dateModified = info.dateModified
        || metaAny(['article:modified_time', 'og:updated_time', 'dateModified', 'last-modified', 'lastmod']);
    var authorEl = document.querySelector('[rel="author"], [itemprop="author"] [itemprop="name"], [itemprop="author"], .byline a, .author-name, .byline');
    var byline = meta('article:author') || info.author || meta('author') || meta('parsely-author') || meta('sailthru.author')
        || (authorEl ? (authorEl.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 120) : '');
    if (/^https?:/i.test(byline)) byline = info.author || meta('author') || '';
    byline = byline.replace(/^by\s+/i, '');
    var description = meta('og:description') || meta('description') || info.description || '';
    var leadImage = meta('og:image') || meta('twitter:image') || '';
    var titleText = meta('og:title') || info.headline || document.title || '';

    if (!root) return { title: titleText, url: location.href, html: '', wordCount: 0, byline: byline, siteName: meta('og:site_name') || '', datePublished: datePublished, dateModified: dateModified };
    ...
    var strip = 'script, style, noscript, iframe, object, embed, link, meta, ' +
                ...
                '[role="contentinfo"], [role="dialog"], [role="search"], ' +
    ...
    // The clone is detached, so innerText (layout-dependent) is unreliable —
    // use textContent everywhere below.
    function textLen(el) { return ((el.textContent || '').trim()).length; }
    var rootLen = textLen(clone) || 1;

    // Cruft by class/id SUBSTRING pattern (Readability-style): exact-class
    // selectors above miss the real world's 'social-share-buttons',
    // 'nav-menu', 'related-posts'. Guarded so a wrapper that happens to
    // hold most of the article can never be removed.
    var CRUFT = /(^|[-_ ])(share|sharing|share-?bar|social\\w*|related|recommend\\w*|promo\\w*|newsletter|subscribe|signup|sign-?up|comment\\w*|disqus|breadcrumbs?|menu|navbar|nav|sidebar|side-?bar|widget|popup|pop-?up|modal|overlay|banner|masthead|toolbar|paywall|outbrain|taboola|sponsor\\w*|pagination|pager|tag-?list|tags|footer|header|skip-?link|dropdown|search|login|cta|cookie|consent|print-?only|read-?next|more-?stories|trending|most-?popular|author-?box|bio-?box)([-_ ]|$)/i;
    var maybe = clone.querySelectorAll('div, section, ul, ol, table, span, p, a, figure');
    for (var m = 0; m < maybe.length; m++) {
        var cand = maybe[m];
        // The clone is detached, so isConnected is false everywhere — an
        // earlier pass's removals are detected via clone.contains instead.
        if (cand !== clone && !clone.contains(cand)) continue;
        var idcls = ((cand.getAttribute && (cand.getAttribute('class') || '')) || '') + ' ' + (cand.id || '');
        if (!idcls.trim()) continue;
        if (CRUFT.test(idcls) && textLen(cand) / rootLen < 0.6) cand.remove();
    }

    // Link-density pruning: menus, tag rows, share strips, and "read more"
    // lists are mostly link text. A block that is >60% links (or a short
    // block >40% links with 3+ links) is chrome, not prose. Same guard.
    function linkLen(el) {
        var links = el.querySelectorAll('a');
        var t = 0;
        for (var i = 0; i < links.length; i++) t += ((links[i].textContent || '').trim()).length;
        return t;
    }
    var blocks = clone.querySelectorAll('div, section, ul, ol, table');
    for (var b = 0; b < blocks.length; b++) {
        var blk = blocks[b];
        if (blk !== clone && !clone.contains(blk)) continue;
        var len = textLen(blk);
        if (!len || len / rootLen >= 0.6) continue;
        var density = linkLen(blk) / len;
        var linkCount = blk.querySelectorAll('a').length;
        if (density > 0.6 || (len < 200 && density > 0.4 && linkCount >= 3)) blk.remove();
    }

    // Empty-shell cleanup: containers left with no text and no media.
    var shells = clone.querySelectorAll('div, section, ul, ol, p, figure, table');
    for (var s = shells.length - 1; s >= 0; s--) {
        var sh = shells[s];
        if (sh !== clone && !clone.contains(sh)) continue;
        if (!textLen(sh) && !sh.querySelector('img, video, picture, svg')) sh.remove();
    }
    ...
    // A duplicate of the headline at the top of the body (most CMSs put the
    // h1 inside the article) would print twice under our own title.
    function norm(s) { return String(s || '').replace(/\\s+/g, ' ').trim().toLowerCase(); }
    var heads = clone.querySelectorAll('h1, h2');
    for (var h = 0; h < heads.length && h < 3; h++) {
        if (norm(heads[h].textContent) && norm(heads[h].textContent) === norm(titleText)) { heads[h].remove(); break; }
    }
    // Loose byline/date lines at the top of the body ("By Jane Doe",
    // "Published 3 hours ago") repeat what the header now states.
    var tops = clone.querySelectorAll('p, div, span');
    for (var q = 0; q < tops.length && q < 12; q++) {
        var tt = (tops[q].textContent || '').replace(/\\s+/g, ' ').trim();
        if (tt.length && tt.length < 120 && /^(by\\s|published|updated|posted|last updated|\\d{1,2}\\s+(min|hour|day)s?\\s+ago)/i.test(tt) && !tops[q].querySelector('img')) tops[q].remove();
    }
    var hasImage = !!clone.querySelector('img[src], picture');
    var text = (clone.textContent || '').replace(/\\s+/g, ' ').trim();
    var words = text ? text.split(/\\s+/).filter(Boolean).length : 0;
    // The dek (standfirst) is shown under the title only when it is not
    // simply the article's first sentence repeated.
    var firstPara = norm(text).slice(0, 160);
    var dek = description && description.length <= 400 && firstPara.indexOf(norm(description).slice(0, 60)) === -1 ? description : '';
    return {
        title: titleText,
        byline: byline,
