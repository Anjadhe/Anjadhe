...
const SettingsApp = {
    ...
    async loadSettings() {
        ...
        // Discover pane (home right-hand links column)
        const discoverToggle = document.getElementById('settings-discover-pane');
        if (discoverToggle && typeof DiscoverPane !== 'undefined') {
            discoverToggle.checked = DiscoverPane.settings().enabled;
        }
    ...
    _setupNavigator() {
        ...
        if (list) {
            list.addEventListener('click', (e) => {
                ...
                // card goes straight to its page — no intermediate "AI" page).
    ...
    setupEventListeners() {
        ...
        // Discover pane on/off
        this._bindChange('settings-discover-pane', (on) => {
            if (typeof DiscoverPane !== 'undefined') DiscoverPane.setEnabled(on);
        }, true);
    ...
    async _loadLLMSummary() {
        ...
        const engineLabels = { llamacpp: 'llama.cpp', server: 'Your server', openai: 'OpenAI', anthropic: 'Anthropic' };
    ...
    async openLLMSettings() {
        ...
        this._bindAssistantName();

        // Tool servers + permission grants live on this page now (the old
        // intermediate "AI" category is gone) — refresh them on entry so
        // changes made since the Settings render show up.
        this._loadMCPServers();
        this._loadAgentPermissions();
    ...
    _bindAssistantName() {
        ...
        const save = () => {
            ...
            else if (before) UIUtils.showToast('Name removed', 'success');
        };
        document.getElementById('settings-assistant-name-save')?.addEventListener('click', save);
    ...
    _openAddModelModal() {
        ...
        // Local models ARE the product default — open straight into the
        // catalog instead of an engine quiz; server/API live behind
        // "More options" at the bottom.
        this._renderAddLocalStep(body, modal, 'llamacpp', { first: true });
    },
    ...
    _renderAddEngineStep(body, modal) {
        ...
        back.textContent = '‹ Models for this Mac';
        ...
        intro.textContent = 'Run a model somewhere else. What you use these models for is sent to the server you pick.';
        ...
        const options = [
            {
                engine: 'server',
                name: 'Your server',
                hint: 'A computer you own that runs an OpenAI-compatible server (llama-server, vLLM, LM Studio).'
            },
            {
                engine: 'openai',
                name: 'OpenAI API',
                hint: 'GPT models with your own OpenAI key, billed to your OpenAI account.'
            },
            {
                engine: 'anthropic',
                name: 'Anthropic API',
                hint: 'Claude models with your own Anthropic key, billed to your Anthropic account.'
            }
        ];
        ...
        for (const opt of options) {
            ...
            btn.addEventListener('click', () => {
                ...
                else this._renderAddCloudStep(body, modal, opt.engine);
            });
    ...
    _renderAddLocalStep(body, modal, engine, opts = {}) {
        body.innerHTML = '';
        if (!opts.first) body.appendChild(this._addModalBackLink(body, modal));
        ...
        desc.textContent = opts.first
            ...
            : `Models recommended for your Mac (${totalRam} GB RAM), running on ${this._engineLabel(engine)}.`;
        body.appendChild(desc);
        ...
        if (opts.first) {
            ...
            more.href = '#';
            more.className = 'settings-add-model-more';
            more.textContent = 'More options: your own server, or OpenAI / Anthropic with your key ›';
    ...
    async _renderSearchProviders() {
        ...
        // Master switch: provider cards (and the key hint) only show when
        // web search is on — off means no feature sends queries anywhere,
        // so there is nothing to configure. `.onchange` (not addEventListener)
        // because this render runs repeatedly.
    ...
    _buildSearchProviderCard(id, info, isActive) {
        ...
        statusEl.textContent = info.builtin
            ? 'Included with Anjadhe'
            : (info.hasKey ? 'API key saved' : 'No API key yet');
    ...
    _renderConnectManage(body, id, info) {
        ...
        const hint = document.createElement('p');
        hint.className = 'settings-hint';
        hint.textContent = 'Included with Anjadhe — no signup, no key. Searches route through Anjadhe’s server (api.anjadhe.com), which forwards them to a search provider without your identity and doesn’t log what you search. Prefer to keep Anjadhe out of the loop? Use your own key with one of the providers below.';
        body.appendChild(hint);
        ...
        const refreshUsage = async () => {
            ...
            if (u.notProvisioned) { status.textContent = 'Not set up yet — select it as your search provider, or press Test'; return; }
