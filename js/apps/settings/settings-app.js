...
const SettingsApp = {
    ...
    async loadSettings() {
        if (typeof BackgroundRuntime !== 'undefined') BackgroundRuntime.renderSettings();
        ...
        // The shell (Settings › Appearance › Simple home). A preference, not
        // a flag, since 2026-09-20 — see SimpleExperience.PREF_KEY.
        this._renderSimpleExperience();
        ...
        // Brokerage accounts linked through nenva Cloud (Plaid)
        this._renderBrokerages();
        ...
        // iMessage notification lane (outbound only, through Messages.app)
        this._renderIMessage();
    ...
    showRoot() {
        ...
        // the Documents settings govern nothing — hide the door. The Writing
        // voice card stays: the voice is core (js/core/voice-store.js).
    ...
    openCategory(cat) {
        ...
        // The writing voice page: source counts, document status and the
        // study state all change underneath it.
        if (cat === 'voice' && typeof SettingsVoice !== 'undefined') SettingsVoice.render();
        ...
        // The license card is state-driven (claim / enter key / facts) and
        // main owns the state, so it is read fresh on every open.
        if (cat === 'license') this._renderLicense();
    },

    // ── Settings › License ──────────────────────────────────────────────
    // Everything the card says comes from LicenseStore.status() in main
    // (docs/BUSINESS_MODEL.md "License"); this only lays it out. Server
    // reachability decides whether the alpha claim is OFFERED (the server is
    // what would honour it); offline, the card falls back to local policy
    // and the claim simply reports the failure.
    async _renderLicense() {
        const host = document.getElementById('license-card');
        ...
        if (!status) return;
        // The server's word counts only when it can mint at all: a Connect
        // with licensing disabled says alphaOpen:false, which is "cannot
        // issue right now", not "the alpha closed" — local policy decides.
        const alphaOpen = (remote && remote.enabled) ? remote.alphaOpen : status.alphaOpen;
        const human = (iso) => {
            if (!iso) return '';
        ...
        let html = '';
        if (status.licensed) {
            const isAlpha = status.class === 'alpha';
            html += `<div class="license-status">${isAlpha ? 'Alpha license' : 'Paid license'}
                <span class="settings-badge">${isAlpha ? 'Free for good' : (status.updatesActive ? 'Updates active' : 'Updates ended')}</span></div>`;
            html += facts([
                ['Issued to', status.email ? esc(status.email) : 'this Mac'],
                ['Issued', esc(human(status.issuedAt))],
                ['Updates', isAlpha ? 'Every release, forever'
                    : (status.updatesActive ? `Until ${esc(human(status.updatesUntil))}` : `Ended ${esc(human(status.updatesUntil))}`),
                    (!isAlpha && !status.updatesActive) ? 'license-row-warn' : ''],
                ['License id', `<code>${esc(status.id)}</code>`]
            ]);
            if (!isAlpha && !status.updatesActive) {
                html += `<p class="settings-card-hint">Builds released after that date are not offered on this Mac; everything installed keeps working. A renewal is not for sale yet &mdash; <a href="#" id="license-contact-link">contact the founder</a> if you want one.</p>`;
            }
            html += `<p class="settings-card-hint">${isAlpha
                ? 'Keep this with the email address you claimed it with: on another Mac, or after a reinstall, claiming again with the same address returns this same key.'
                : 'Keep the key somewhere safe; it works on every Mac you use.'}
            ...
            if (alphaOpen) {
                html += `<div class="license-status">Closed alpha <span class="settings-badge">Free for good</span></div>
                    <p class="settings-card-hint">Everyone who installs during the alpha keeps nenva free for good. Claim your alpha license now so that is on record: it is a small signed key saved on this Mac, and it keeps this Mac on every future release once the alpha closes.</p>
                    <p class="settings-card-hint">Enter an email address to issue it to. That address is the only thing sent to nenva, and it is how you get the same key back on another Mac or after a reinstall.</p>
                    ...
                    <div class="feedback-actions">
                        ...
                        <button id="license-claim-btn" class="primary-btn">Claim alpha license</button>
                    </div>`;
                html += keyEntry('Already have a key? Paste it here, or open the file it came in.');
            } else {
                const t = status.trial;
                if (!t) {
                    // Server says closed, local policy has not caught up (no
                    // trial clock yet): say what is true, never "trial ended".
                    html += `<div class="license-status">Alpha closed <span class="settings-badge">No key on this Mac</span></div>
                        <p class="settings-card-hint">New alpha licenses are no longer issued. Everything keeps working; if you claimed one, enter it below.</p>`;
                } else if (!t.ended) {
                    html += `<div class="license-status">Trial <span class="settings-badge">${t.daysLeft} day${t.daysLeft === 1 ? '' : 's'} left</span></div>
                        <p class="settings-card-hint">nenva keeps working after the trial; what stops is updates. Updates continue until ${esc(human(t.endsAt))}, then this Mac stays on the build it has until a license is entered.</p>`;
                } else {
                    html += `<div class="license-status">Trial ended <span class="settings-badge">Updates paused</span></div>
                        <p class="settings-card-hint">Everything you have keeps working; newer releases are not offered on this Mac until a license is entered.</p>`;
                }
                html += `<p class="settings-card-hint">${status.price ? `A one-time license costs ${esc(status.price)} and brings` : 'A one-time license is planned; the price is not set yet. It will bring'} a year of updates, with no account. Buying is not open yet &mdash; <a href="#" id="license-contact-link">contact the founder</a> to get one${status.alphaClosesAt ? `, or to recover an alpha license claimed before ${esc(human(status.alphaClosesAt))}` : ''}.</p>`;
                html += keyEntry('Have a key? Paste it here, or open the file it came in.');
            }
        }
        host.innerHTML = html + footer;
        ...
        document.getElementById('license-contact-link')?.addEventListener('click', (e) => {
            e.preventDefault();
            window.electronAuth?.openExternal?.(WhatsNew.FOUNDER_MAILTO.replace(/subject=[^&]*/, 'subject=nenva%20license'));
        });
        document.getElementById('license-show-key')?.addEventListener('click', async (e) => {
            e.preventDefault();
            const p = document.getElementById('license-key-show');
            if (!p) return;
        ...
        document.getElementById('license-remove-link')?.addEventListener('click', async (e) => {
            e.preventDefault();
        ...
        document.getElementById('license-claim-btn')?.addEventListener('click', async () => {
            const btn = document.getElementById('license-claim-btn');
            const email = document.getElementById('license-claim-email')?.value || '';
            btn.disabled = true;
            line('license-claim-status', 'Claiming…');
            ...
            UIUtils.showToast(r.created ? 'Alpha license claimed. Free for good.' : 'Welcome back — your alpha license is restored.', 'success');
            this._renderLicense();
            this._updateRootHints();
            if (typeof LicenseClaim !== 'undefined') LicenseClaim.refresh();
        });
        const apply = async (promise) => {
            ...
            UIUtils.showToast(`${r.class === 'alpha' ? 'Alpha' : 'Paid'} license applied.`, 'success');
            this._renderLicense();
            this._updateRootHints();
            if (typeof LicenseClaim !== 'undefined') LicenseClaim.refresh();
        };
        document.getElementById('license-apply-btn')?.addEventListener('click', () => {
            const text = document.getElementById('license-key-input')?.value || '';
            if (!text.trim()) { line('license-status-line', 'Paste a key first.', true); return; }
            apply(window.electronLicense.set(text));
        });
        document.getElementById('license-open-file-btn')?.addEventListener('click', () => {
            apply(window.electronLicense.openFile());
        });
    },

    // Fill the disclosure block with the values a send would carry.
    // collectDiagnostics() is BOTH what is shown and what is sent, so the
    // card cannot promise one thing and post another. `details` is a
    // multi-line string (one labelled fact per line) and is rendered as-is.
    async _renderFeedbackDisclosure() {
        ...
        this._syncFeedbackDiscloseState();
    },

    // An unticked box dims its own lines — what is being withheld stays
    // readable, which is the point of showing it at all.
    _syncFeedbackDiscloseState() {
        const box = document.getElementById('feedback-include-details');
        const block = document.getElementById('feedback-details-block');
        if (!box || !block) return;
        block.classList.toggle('is-off', !box.checked);
        if (!box.dataset.discloseBound) {
            box.dataset.discloseBound = '1';
            box.addEventListener('change', () => this._syncFeedbackDiscloseState());
        }
    },
    ...
    _updateRootHints() {
        ...
        // Writing voice: off / on but unstudied / studied.
        try {
            if (typeof VoiceStore !== 'undefined') {
                const v = VoiceStore.selfVoice();
                set('settings-root-hint-voice', !v ? 'Let the assistant learn how you write'
                    : ((v.body || '').trim() ? 'On — the assistant can draft in your voice' : 'On — not studied yet'));
            }
        } catch {}
        ...
        try {
            ...
            if (def) {
                const where = def.engine === 'server' ? 'Your server'
                    ...
                    : def.engine === 'anjadhe' ? 'nenva Cloud'
                    : 'This Mac';
                // nenva Cloud entries name themselves by catalog label —
                // "nenva Cloud · anjadhe-cloud" would be a stutter.
        ...
        // License: async, from main. The row reads its state, or the ask.
        try {
            window.electronLicense?.get?.().then((s) => {
                if (!s) return;
                const text = s.state === 'alpha' ? 'Alpha license · free for good'
                    : s.state === 'paid' ? (s.updatesActive ? `Paid · updates until ${s.updatesUntil}` : 'Paid · updates ended')
                    : s.state === 'trial' ? `Trial · ${s.trial?.daysLeft ?? ''} days left`
                    : s.state === 'trial-ended' ? 'Trial ended · updates paused'
                    : 'Claim your free alpha license';
                set('settings-root-hint-license', text);
            }).catch(() => {});
        } catch {}
    },
    ...
    setupEventListeners() {
        ...
        document.getElementById('feedback-founder-link')?.addEventListener('click', (e) => {
            e.preventDefault();
            window.electronAuth?.openExternal?.(WhatsNew.FOUNDER_PAGE);
        });
    ...
    /**
     * Linked institutions card (Settings › Accounts): brokerages (Portfolio)
    ...
    async _renderBrokerages() {
        ...
        const title = 'Linked institutions';
        const desc = 'Connect brokerages (Fidelity, Robinhood, Schwab…) so Portfolio pulls positions, cash and trades itself, and banks or credit cards so Spending shows every transaction by month, category and merchant. You sign in on the institution’s own page through Plaid; no password ever reaches nenva. Holdings and transactions travel through nenva’s server (nenva Cloud) on their way to this Mac and are not stored or logged there; the only thing kept on the server is Plaid’s access token for each link, encrypted, deleted when you unlink. Nothing here is required — accounts can always be tracked by hand or from confirmation emails.';
        ...
        if (!bk && !sp) {
            const why = (typeof InstitutionLink === 'undefined' || !InstitutionLink.available())
                ...
                : 'Install the Portfolio or Spending app (More apps) to link an institution.';
    ...
    /**
     * iMessage card (Settings › Accounts). Outbound only: this Mac's
     * notifications go to the user's OWN iMessage handle by scripting
     * Messages.app (js/main/imessage-bridge.js) — Apple end-to-end, no
     * bot, no server of ours. Honest copy: it still leaves the Mac (over
     * Apple's servers), Messages must be signed in here, and macOS asks
     * once for Automation consent — which is what "Send a test" is for,
     * so the prompt appears while the user is looking.
     */
    async _renderIMessage() {
        const container = document.getElementById('settings-imessage');
        if (!container) return;
        if (!window.electronIMessage) { container.innerHTML = ''; return; }
        let st;
        try { st = await window.electronIMessage.getStatus(); } catch { return; }
        if (!st.available) { container.innerHTML = ''; return; }
        if (typeof Notify !== 'undefined') Notify.refreshIMessage(st);

        let tone = 'off', text = 'Not set up — enter the number or email you use for iMessage';
        if (st.lastError) { tone = 'err'; text = st.lastError; }
        else if (this._imessageTestSent) { tone = 'ok'; text = 'Test sent — check your phone'; }
        else if (st.notify && st.handle) { tone = 'ok'; text = 'On — reminders and alerts are sent to your iMessage'; }
        else if (st.handle) { tone = 'off'; text = 'Off — nothing is sent'; }
        this._imessageTestSent = false;

        container.innerHTML = this._acct({
            title: 'iMessage',
            status: this._esc(text),
            tone,
            statusId: 'imessage-status-line',
            control: this._acctSwitch('imessage-notify', st.notify, st.handle ? '' : 'disabled'),
            desc: 'Send this Mac’s reminders and alerts to your own iMessage through the Messages app on this Mac — a text from you to you, so it reaches your iPhone and Watch with Apple’s end-to-end encryption and no bot or server in between. Messages here must be signed in to iMessage; the first send asks macOS for permission to control Messages. This switch only sends; reading your texts is the card below, and the assistant never answers a text.',
            form: `<input type="text" id="imessage-handle-input" class="settings-input" value="${this._esc(st.handle)}" placeholder="Your iMessage number or email" autocomplete="off">`
                + this._acctBtn('imessage-save-btn', 'Save')
                + this._acctBtn('imessage-test-btn', 'Send a test', { disabled: !st.handle })
        });

        container.insertAdjacentHTML('beforeend', this._imessageInsightsCardHtml());
        this._bindIMessageInsights();

        const saveBtn = document.getElementById('imessage-save-btn');
        if (saveBtn) saveBtn.addEventListener('click', async () => {
            const input = document.getElementById('imessage-handle-input');
            const res = await window.electronIMessage.setHandle(input ? input.value : '');
            if (res && res.error) {
                this._acctStatusSet(document.getElementById('imessage-status-line'), res.error, 'err');
                return;
            }
            this._renderIMessage();
        });
        const notifyInput = document.getElementById('imessage-notify');
        if (notifyInput) notifyInput.addEventListener('change', async () => {
            await window.electronIMessage.setNotify(notifyInput.checked);
            this._renderIMessage();
        });
        const testBtn = document.getElementById('imessage-test-btn');
        if (testBtn) testBtn.addEventListener('click', async () => {
            testBtn.disabled = true;
            this._acctStatusSet(document.getElementById('imessage-status-line'), 'Sending a test…', 'busy');
            const res = await window.electronIMessage.sendTest();
            this._imessageTestSent = !!(res && res.ok);
            this._renderIMessage();
        });
    },

    /**
     * Texts as an insight source (2026-09-10) — the second card under
     * iMessage. Per-Mac switch (IMessageSource, the AppleImport mould):
     * ON reads NEW conversations from this Mac's Messages database and
     * hands them to the same engine that reads mail; the first read is
     * what surfaces a Full Disk Access denial, so enabling awaits the
     * probe and the status line says exactly what macOS wants. Honest
     * copy: the permission covers the whole Messages database, the
     * texts go only where the chosen brain runs (the `messages` cloud
     * privacy class, off by default), and one Mac should do this.
     */
    _imessageInsightsCardHtml() {
        if (typeof IMessageSource === 'undefined') return '';
        const st = IMessageSource.state();
        let tone = 'off', text = 'Off — your texts are not read';
        if (st.enabled) {
            if (st.lastError) { tone = 'err'; text = st.lastError; }
            else if (st.lastAt) {
                tone = 'ok';
                text = `On — last checked ${UIUtils.formatDateTime(st.lastAt)} · ${st.conversations} conversation${st.conversations === 1 ? '' : 's'} read`;
            } else { tone = 'busy'; text = 'On — checking for new texts'; }
        }
        const fdaHint = st.enabled && st.lastErrorReason === 'fda'
            ? 'How to allow it: 1. Open System Settings › Privacy & Security › Full Disk Access. 2. Click the + button under the list (nenva is not listed until you add it). 3. Choose nenva from your Applications folder (it is still called Anjadhe there if this Mac updated from it) and make sure its switch is on. 4. Quit nenva, open it again, then press Check now.' : '';
        return this._acct({
            title: 'Insights from your texts',
            status: this._esc(text),
            tone,
            statusId: 'imessage-insights-status-line',
            control: this._acctSwitch('imessage-insights-enabled', st.enabled),
            desc: 'Read the texts that arrive in Messages on this Mac the way Insights reads your mail: each conversation is read once it goes quiet, and appointment reminders, deliveries, bills, codes and plans made over text land in the Insights app’s folders — anything that needs doing becomes a task. The first time it reads the last 30 days of texts, grouped by conversation (one conversation is one item), so the count is smaller than your Messages list; after that it picks up new texts as they arrive. Needs Full Disk Access, which you give nenva by hand: System Settings › Privacy & Security › Full Disk Access, the + button, then nenva from Applications (it is still called Anjadhe there if this Mac updated from it) (macOS does not add it for you). That permission covers the whole Messages database, which is why this is off until you turn it on. Your texts go only to the model you chose — with a cloud brain they stay on this Mac unless Settings › AI › Cloud privacy allows Messages. Turn this on for ONE Mac. Turning it off forgets the conversations it read; tasks you got from them stay.',
            foot: this._esc(fdaHint),
            footActions: st.enabled ? this._acctBtn('imessage-insights-check', 'Check now') : ''
        });
    },

    _bindIMessageInsights() {
        if (typeof IMessageSource === 'undefined') return;
        const sw = document.getElementById('imessage-insights-enabled');
        if (sw) sw.addEventListener('change', async () => {
            const on = sw.checked;
            if (on) {
                this._acctStatusSet(document.getElementById('imessage-insights-status-line'), 'Checking that this Mac can read Messages…', 'busy');
                const probe = await IMessageSource.setEnabled(true);
                if (probe && !probe.ok) {
                    UIUtils.showToast(probe.reason === 'fda'
                        ? 'Full Disk Access is needed — see the card for the path'
                        : (probe.error || 'Could not read Messages'), 'error');
                }
            } else {
                const n = await IMessageSource.setEnabled(false);
                UIUtils.showToast('Stopped reading texts — conversations forgotten, tasks kept', 'info');
                void n;
            }
            this._renderIMessage();
        });
        const check = document.getElementById('imessage-insights-check');
        if (check) check.addEventListener('click', async () => {
            check.disabled = true;
            this._acctStatusSet(document.getElementById('imessage-insights-status-line'), 'Checking…', 'busy');
    ...
    _renderAppleImport() {
        ...
        container.innerHTML = this._acct({
            ...
            desc: 'Import this Mac’s iCloud reminders into Tasks. Read-only — nothing is written back, and turning this off removes the imported tasks (they stay in Reminders). Pick which lists come across below.',
            rows: remRows,
            ...
            desc: 'Import this Mac’s Apple Notes into Notes, tagged “apple-notes” plus the name of their Apple Notes folder. Read-only; locked notes are skipped; turning this off removes the imported copies. Turn this on for ONE Mac only — imported notes reach your other Macs through nenva’s own sync.',
    ...
    LLM_SECTIONS: {
        ...
        'llm-sec-privacy': { label: 'Cloud Privacy' },
    ...
    async openLLMSection(secId) {
        ...
        switch (secId) {
            ...
            case 'llm-sec-privacy': this._renderCloudPrivacy(); break;
    ...
    async _renderLlmRootHints() {
        ...
        try {
            ...
            const def = AgentService.getDefaultEntry?.();
            // The hint names the brain, the list size, and — since the
            // page absorbed the assignment table — how many parts of the
            // app were deliberately sent elsewhere.
            let routed = 0;
            try { routed = Object.keys(AgentService.getRouteAssignments?.() || {}).length; } catch { /* none */ }
            set('llm-hint-models', def
                ? `${def.model}${entries.length > 1 ? ` · ${entries.length} models` : ''}`
                    + (routed ? ` · ${routed} part${routed === 1 ? '' : 's'} elsewhere` : '')
                : 'No model yet · add one');
        } catch {}
        try {
            // Behind the `terminal` feature flag: the card is hidden when
            // it is off, so don't wake the IPC for a hint nobody sees.
            const gated = typeof FEATURES !== 'undefined' && !FEATURES.isEnabled('terminal');
            const r = (!gated && window.electronCLI) ? await window.electronCLI.status() : null;
            set('llm-hint-cli', r ? (r.enabled ? 'On · this Mac only' : 'Off') : '');
        } catch {}
        try {
            if (typeof CloudPrivacy !== 'undefined') {
                const held = CloudPrivacy.CLASSES.filter(c => !CloudPrivacy.isEnabled(c.id)).map(c => c.label);
                // anyLeaves, not brainLeaves: a part of the app pointed at
                // a model off this Mac makes "nothing leaves" false even
                // under a local brain (model-routing.js R4).
                set('llm-hint-privacy', !CloudPrivacy.anyLeaves()
                    ? 'Every model runs on this Mac · nothing leaves'
                    : held.length ? `Kept on this Mac: ${held.join(', ')}` : 'Ambient AI may send everything');
            }
        } catch {}
        try {
            const s = await window.electronSearch?.getStatus?.();
            const NAMES = { anjadhe: 'nenva', tavily: 'Tavily', brave: 'Brave' };
    ...
    async _renderModelLibrary() {
        ...
        if (!entries.length) {
            ...
            this._renderModelRoutes();   // hides its block; nothing to assign
            this._renderPhoneAI();
            return;
        }
        ...
        // The page's second half: which model answers what. Same render, so
        // adding, removing or defaulting a model can never leave the
        // assignment table showing a model that is gone.
        this._renderModelRoutes();
        this._renderPhoneAI();
    },

    /**
     * The phone's second model (docs/MOBILE_NATIVE.md "M5"): what the paired
     * phone may answer with by itself when it cannot reach this Mac. The
     * synced `phone-ai` key, so the phone's own Settings shows and changes
     * the same choice. Absent = off: the phone waits for this Mac, as it
     * always has (POSITIONING.md — never a default, never a fallback the
     * user did not pick). Only nenva Cloud today: it is the one choice the
     * phone can reach without a credential of its own.
     */
    _renderPhoneAI() {
        const host = document.getElementById('phone-ai-list');
        if (!host) return;
        const esc = (v) => UIUtils.escapeHtml(String(v == null ? '' : v));
        const cur = (StorageManager.get('phone-ai') || {}).fallback || null;
        const paint = (models) => {
            const opts = [`<option value=""${cur ? '' : ' selected'}>Off — wait for this Mac</option>`];
            const list = models.slice();
            if (cur && cur.engine === 'anjadhe' && !list.some(m => m.id === cur.model)) {
                list.push({ id: cur.model, label: cur.label || cur.model });
            }
            for (const m of list) {
                const sel = cur && cur.engine === 'anjadhe' && cur.model === m.id;
                const name = /nenva cloud/i.test(m.label) ? m.label : `nenva Cloud · ${m.label}`;
                opts.push(`<option value="${esc(m.id)}"${sel ? ' selected' : ''}>${esc(name)}</option>`);
            }
            host.innerHTML = `
            <div class="settings-card-row settings-experimental-row">
                <div>
                    <div class="settings-card-title">Second model</div>
                    <p class="settings-card-hint">${cur ? 'Your phone answers with this when this Mac is away, and says so under each answer.' : 'Your phone waits for this Mac.'}</p>
                </div>
                <select class="settings-input settings-routing-select" id="phone-ai-select">${opts.join('')}</select>
            </div>`;
            host.querySelector('#phone-ai-select').addEventListener('change', (e) => {
                const id = e.target.value;
                const m = list.find(x => x.id === id);
                const fallback = id ? { engine: 'anjadhe', model: id, label: m ? m.label : id } : null;
                StorageManager.set('phone-ai', { fallback, updatedAt: new Date().toISOString() });
                UIUtils.showToast(fallback
                    ? `Your phone answers with ${fallback.label} when this Mac is away`
                    : 'Your phone waits for this Mac', 'success');
                this._renderPhoneAI();
            });
        };
        paint([]);
        window.electronLLM?.anjadheModels?.().then((res) => {
            if (res && Array.isArray(res.models)) paint(res.models);
        }).catch(() => { /* the saved choice still shows */ });
    },
    ...
    _buildModelCard(entry, isDefault) {
        ...
        // nenva Cloud entries show their catalog label, never the raw
        // public id — and skip the engine badge when the label already says
        // "nenva Cloud" (the pair would read as a stutter).
        ...
        if (entry.engine !== 'anjadhe' || AgentService.anjadheEntryLabel(entry) !== 'nenva Cloud') {
    ...
    _renderAnjadheManage(body, entry) {
        ...
        const desc = document.createElement('p');
        desc.className = 'settings-section-desc';
        desc.textContent = 'An open-weight model served by nenva Connect (api.anjadhe.com). '
    ...
    _renderAddAnjadheStep(body, ctx) {
        ...
        const desc = document.createElement('p');
        desc.className = 'settings-section-desc';
        desc.textContent = 'Open-weight models served by nenva Connect (api.anjadhe.com). '
        ...
        window.electronLLM?.anjadheModels?.().then((res) => {
            const models = (res && Array.isArray(res.models) && res.models.length)
                ? res.models
                : [{ id: this.ANJADHE_CLOUD_MODEL, label: 'nenva Cloud' }];
            ...
            render([{ id: this.ANJADHE_CLOUD_MODEL, label: 'nenva Cloud' }]);
        });
    },
    ...
    async _renderCliCard() {
        ...
        if (installHint && installBtn) {
            if (st.installedAt) {
                ...
                installHint.textContent = 'One click puts the anjadhe command on your PATH — no other software needed. (If you ever delete nenva itself, the command notices and removes itself on its next run.)';
    ...
    _renderConnectManage(body, id, info) {
        ...
        hint.textContent = 'Included with nenva — no signup, no key. Searches route through nenva’s server (api.anjadhe.com), which forwards them to a search provider without your identity and doesn’t log what you search. Prefer to keep nenva out of the loop? Use your own key with one of the other providers.';
        body.appendChild(hint);
    ...
    _renderPrivacySettings() {
        ...
        // The id in the form nenva STORES — the only one that appears on the
        // service side, so it is the one the admin console shows and the one
        // worth printing here. The raw form is a wire detail.
        const installIdEl = document.getElementById('settings-analytics-install-id');
        if (installIdEl) {
            window.electronSearch?.installId?.()
                .then((r) => { installIdEl.textContent = r?.idHash || AnalyticsManager.getInstallId(); })
                .catch(() => { installIdEl.textContent = AnalyticsManager.getInstallId(); });
        }
    ...
    SB_SECTIONS: {
        ...
        'sb-sec-files': { label: 'Notes and Journal as Files' },
    ...
    async openStorageBackup() {
        ...
        if (!this._storageBackupBound) {
            ...
            // Merge on demand. nenva merges on launch and on refresh and
    ...
    async _renderContentFilesToggle() {
        ...
        if (pathEl) pathEl.textContent = info.root ? `${info.root}/Notes and ${info.root}/Journal` : '';
    ...
    async _renderSyncToggle() {
        ...
        if (st.enabled) {
            hint.textContent = peers.length
                ? `Syncing with: ${names}`
                : 'No other Macs yet — install nenva on another Mac and turn sync on there.';
        } else {
            hint.textContent = peers.length
                ? `nenva data from another Mac is in your iCloud Drive (${names}) — turn sync on to merge with it.`
                : '';
        }
    },
    ...
    _launchIdForApp(app) {
        const map = {
            ...
            Journal: 'journal', Projects: 'goals', Bookmarks: 'bookmarks',
    ...
    _logMatchesQuery(log, query) {
        const haystack = [
            log.source, log.model, log.provider, log.destination,
        ...
        return `
            <tr class="llm-log-row ${status}" data-log-index="${index}">
                ...
                <td class="log-where-cell${log.left ? ' is-left' : ''}" title="${log.left ? 'This request left this Mac' : log.left === false ? 'Ran on this Mac' : 'Logged before destinations were recorded'}">${
                    log.left ? '&#8599; ' + this._esc(log.destination || 'Left this Mac')
        ...
        content.innerHTML = `
            ...
            <div class="log-detail-section">
                ...
                <p class="log-detail-text">${
                    log.left ? `&#8599; ${this._esc(log.destination || 'Left this Mac')} — every message below left this Mac exactly as shown.`
    ...
    EXPERIMENTAL_COPY: {
        aisuggestions: {
            title: 'Suggested prompts written for you',
            hint: 'Let your personal AI write the "Try asking…" prompts each hour from what is actually on your plate, instead of the standard list. Runs on this Mac only, and only on data you allow to leave it.'
        },
        workrooms: {
            title: 'Specialist workrooms · Preview',
            hint: 'Delegate research and planning to a visible team of specialists. Workrooms stay on this Mac. Specialists research and draft. Connect Browser (Playwright) for interactive websites; browser interactions ask for approval in the workroom.'
        },
        library: {
            title: 'Library',
        ...
        noticing: {
            title: 'Noticing',
            hint: 'Let the assistant read finished journal entries on this Mac and note what a friend would follow up on. Nothing is shown anywhere yet — this phase only records what it would have said, for review.'
        },
        maccontrol: {
            title: 'Use other apps',
            hint: 'Let the assistant open and operate other apps on this Mac — your own browser, Mail, Finder and more — one step at a time. It asks before working on a website new to it, and every time before buying, confirming an order, sending, deleting or signing in. Needs Accessibility permission (and Screen Recording to see screenshots); a Stop bar shows while it works.'
        },
        terminal: {
            title: 'Terminal access',
            hint: 'Use the assistant from a terminal with the anjadhe command. Adds a Terminal Access page under AI Assistant; nothing listens until you turn it on there.'
        }
    },
     ...
     */
    /**
     * Which model answers what (js/agent/model-routing.js) — the second
     * half of the Models page, not a page of its own: picking a model and
     * saying what it answers are one thought, and the list above is what
     * these rows choose from. One row per surface, each a select of every
     * saved model entry with "Follows the default model" first.
     *
     * The AI Assistant is NOT a row here: it runs on the default model,
     * and the default model is chosen by the radio in the list above —
     * on this same page, a few hundred pixels up. A second picker for it
     * would be the same setting twice (R2). So the table is exactly the
     * surfaces that can be sent elsewhere, and "Follows the default
     * model" points at that radio.
     *
     * A second LOCAL entry is refused, not silently coerced (R3): this Mac
     * holds one local model at a time, so the option is disabled with the
     * reason on the row rather than accepted and quietly ignored.
     */
    async _renderModelRoutes() {
        const host = document.getElementById('llm-routing-list');
        const desc = document.getElementById('llm-routing-desc');
        const block = document.getElementById('llm-routing-block');
        if (!host || typeof ModelRouting === 'undefined' || typeof AgentService === 'undefined') return;
        const list = AgentService.getModelList() || [];
        // Nothing to assign with one model — the table would be six rows
        // all saying the same thing. It appears when a second model does.
        if (block) block.hidden = list.length < 2;
        if (list.length < 2) { host.innerHTML = ''; return; }
        const def = AgentService.getDefaultEntry?.() || null;
        const assignments = AgentService.getRouteAssignments?.() || {};
        const localOne = ModelRouting.localInPlay(assignments, list, def);
        const esc = (v) => UIUtils.escapeHtml(String(v == null ? '' : v));

        if (desc) {
            desc.textContent = 'Chat and every other part of the app use the model marked Default above. Point any of these at a different model instead — a cheap fast model for filing, your own key for email. Work sent to a model off this Mac also runs beside your chat instead of waiting in line behind it.';
        }

        const label = (e) => AgentService.entryLabel(e);
        // Where it runs, appended to the model's name — dropped when the
        // name already says it ("nenva Cloud · nenva Cloud").
        const where = (e) => {
            if (!e) return '';
            const place = ModelRouting.isLocal(e) ? 'this Mac'
                : e.engine === 'anjadhe' ? 'nenva Cloud'
                    : e.engine === 'openai' ? 'OpenAI'
                        : e.engine === 'anthropic' ? 'Anthropic' : 'your server';
            return label(e).toLowerCase().includes(place.toLowerCase()) ? '' : place;
        };

        const rows = ModelRouting.assignable().map(sf => {
            const cur = assignments[sf.id] || '';
            const opts = [`<option value=""${cur ? '' : ' selected'}>Follows the default model</option>`];
            for (const e of list) {
                if (def && e.id === def.id) continue;                  // that IS "follow your model"
                const clash = ModelRouting.isLocal(e) && localOne && localOne.id !== e.id;
                const place = where(e);
                const suffix = clash ? ' — one local model at a time' : (place ? ` · ${place}` : '');
                opts.push(`<option value="${esc(e.id)}"${cur === e.id ? ' selected' : ''}${clash ? ' disabled' : ''}>${esc(label(e))}${esc(suffix)}</option>`);
            }
            return `
            <div class="settings-card-row settings-experimental-row">
                <div>
                    <div class="settings-card-title">${esc(sf.label)}</div>
                    <p class="settings-card-hint">${esc(sf.desc)}</p>
                </div>
                <select class="settings-input settings-routing-select" data-route-surface="${esc(sf.id)}">${opts.join('')}</select>
            </div>`;
        });
        host.innerHTML = rows.join('');

        host.querySelectorAll('select[data-route-surface]').forEach(sel => {
            sel.addEventListener('change', () => {
                const surfaceId = sel.dataset.routeSurface;
                const res = AgentService.setRouteAssignment(surfaceId, sel.value || null);
                if (!res.ok) {
                    UIUtils.showToast(res.reason || 'That model cannot answer this part', 'error');
                    this._renderModelRoutes();
                    return;
                }
                const sf = ModelRouting.surface(surfaceId);
                const entry = sel.value ? AgentService.getEntry(sel.value) : null;
                UIUtils.showToast(entry
                    ? `${sf.label} now runs on ${label(entry)}`
                    : `${sf.label} follows the default model again`, 'success');
                // A new destination can change what ambient work may send,
                // and what the once-per-destination disclosure should say.
                try { CloudPrivacy._resetNotes(); CloudPrivacy.discloseIfNeeded?.(); } catch { /* best-effort */ }
                this._renderModelRoutes();
                this._renderLlmRootHints?.();
            });
        });
    ...
    /**
     * Cloud Privacy page: one switch per data class that AMBIENT AI work may
     * send off this Mac (js/core/cloud-privacy.js; docs/CLOUD_PRIVACY.md).
     * Rendered whatever the brain is — the choices persist and apply the
     * moment a cloud model is picked — with the header saying which case
     * the user is in right now.
     */
    _renderCloudPrivacy() {
        const list = document.getElementById('llm-privacy-list');
        const desc = document.getElementById('llm-privacy-desc');
        if (!list || typeof CloudPrivacy === 'undefined') return;
        // The header states the case the user is actually in, and a part
        // of the app routed elsewhere (Models › Which model answers what) counts:
        // saying "nothing leaves" while email runs on a cloud model would
        // be the exact lie this page exists to prevent.
        const brainLeaves = CloudPrivacy.brainLeaves();
        const anyLeaves = CloudPrivacy.anyLeaves();
        const dest = CloudPrivacy.brainDestination();
        if (desc) {
            desc.textContent = brainLeaves
                ? `Your AI model runs on ${dest || 'a server off this Mac'}. nenva does some work without being asked — email insights, routines, reviews — and each kind of data below may or may not go along.`
                : anyLeaves
                    ? 'Chat runs on this Mac, but you have pointed part of the app at a model that runs elsewhere (Models › Which model answers what). These switches govern what that work may send; each one is asked of the model doing it.'
                    : 'Your AI model runs on this Mac, so nothing leaves it. These choices take effect the moment you pick a model that runs elsewhere (nenva Cloud, your own key or server).';
        }
        list.innerHTML = CloudPrivacy.CLASSES.map(c => `
            <div class="settings-card-row settings-experimental-row">
                <div>
                    <div class="settings-card-title">${UIUtils.escapeHtml(c.label)}</div>
                    <p class="settings-card-hint">${UIUtils.escapeHtml(c.desc)}</p>
                </div>
                <label class="settings-switch">
                    <input type="checkbox" data-cloud-class="${UIUtils.escapeHtml(c.id)}"${CloudPrivacy.isEnabled(c.id) ? ' checked' : ''}>
                    <span class="settings-switch-track"></span>
                </label>
            </div>
        `).join('');
        list.querySelectorAll('input[data-cloud-class]').forEach(input => {
            input.addEventListener('change', () => {
                const cls = input.dataset.cloudClass;
                CloudPrivacy.setEnabled(cls, input.checked);
                CloudPrivacy._resetNotes();
                const label = CloudPrivacy.CLASSES.find(c => c.id === cls)?.label || cls;
                UIUtils.showToast(input.checked
                    ? `${label} may now leave this Mac for ambient AI work`
                    : `${label} stays on this Mac`, 'success');
                // Work that was skipped while the class was off can resume.
                if (cls === 'email' && input.checked) {
                    try { EmailApp.drainAnalysisQueue?.(); } catch { /* not loaded */ }
                }
                this._renderLlmRootHints?.();
            });
        });
    ...
    /**
     * Settings › Appearance › Simple home — the default shell's off switch.
     * It reloads for the same reason an experimental flag does: the shell is
     * built once at startup (SimpleExperience.init has no teardown), so a live
     * flip would leave the window half-rebuilt. The reload goes through
     * AppManager.requestReload, which asks before dropping work in flight.
     */
    _renderSimpleExperience() {
        const input = document.getElementById('settings-simple-experience');
        if (!input || typeof SimpleExperience === 'undefined') return;
        input.checked = SimpleExperience.isOn();
        input.onchange = () => {
            SimpleExperience.setOn(input.checked);
            UIUtils.showToast(
                `Simple home ${input.checked ? 'on' : 'off'} — reloading to apply`,
                'success'
            );
    ...
    /**
     * Settings › Portfolio. One switch over both of that app's daily
     * model-written surfaces (PortfolioProfile and PortfolioBrief), and a
     * second, default-off one over CNBC's current headlines
     * (PortfolioHeadlines) — PortfolioApp holds both preferences, and each
     * writer asks at its own ensure()/render seams.
     */
