...
const SettingsApp = {
    ...
    _buildModelCard(entry, isDefault) {
        ...
        // Anjadhe Cloud's model id IS the product name — showing both the
        // id and the engine badge reads as a stutter, so the card carries
        // the label once and skips the badge.
        name.textContent = entry.engine === 'anjadhe' ? 'Anjadhe Cloud' : entry.model;
    ...
    // The public model name Anjadhe Connect serves on /v1/llm. One fixed
    // model on purpose: the server maps this name to whatever open weights
    // it currently runs, so a better model needs no app release.
    ANJADHE_CLOUD_MODEL: 'anjadhe-cloud',

    /**
     * Add step for Anjadhe Cloud: no key, no model picker — just the
     * consent copy and an Add button. The Connect key mints itself on the
     * first request (clicking Add is the opt-in; nothing reaches the
     * service until the model is actually used).
     */
    _renderAddAnjadheStep(body, ctx) {
        ...
        quota.textContent = 'Anjadhe Cloud is in preview: the free plan includes 1,000 AI requests a '
            + 'month. When it runs out you can wait for the next month, run a model on this Mac, or '
            + 'add your own key or server.';
        body.appendChild(quota);

        const footer = document.createElement('div');
        footer.className = 'settings-input-row settings-add-model-footer';
        // One fixed model, so "already added" is knowable here — unlike the
        // key/server engines, where a second entry is legitimate. Offer the
        // way back to the list instead of an Add that could only toast.
        const existing = AgentService.getModelList().find(e => e.engine === 'anjadhe');
        if (existing) {
            const done = document.createElement('p');
            done.className = 'settings-key-status';
            const def = AgentService.getDefaultEntry?.();
            done.textContent = def && def.id === existing.id
                ? '✓ Already in your list — it’s your default model.'
                : '✓ Already in your list.';
            body.insertBefore(done, quota);
            ...
            addBtn.textContent = 'Add Anjadhe Cloud';
            addBtn.addEventListener('click', () => {
                this._finishAddModel(ctx, { engine: 'anjadhe', model: this.ANJADHE_CLOUD_MODEL });
            });
    ...
    async _finishAddModel(ctx, { engine, model, baseUrl, key }) {
