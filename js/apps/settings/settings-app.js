...
const SettingsApp = {
    ...
    async loadSettings() {
        ...
        // Brokerage accounts linked through Anjadhe Cloud (Plaid)
    ...
    async _renderLicense() {
        ...
        if (status.licensed) {
            ...
            if (alphaOpen) {
                html += `<div class="license-status">Closed alpha <span class="settings-badge">Free for good</span></div>
                    <p class="settings-card-hint">Everyone who installs during the alpha keeps Anjadhe free for good. Claim your alpha license now so that is on record: it is a small signed key saved on this Mac, and it keeps this Mac on every future release once the alpha closes.</p>
                    <p class="settings-card-hint">Enter an email address to issue it to. That address is the only thing sent to Anjadhe, and it is how you get the same key back on another Mac or after a reinstall.</p>
                ...
                if (!t) {
                    ...
                    html += `<div class="license-status">Trial <span class="settings-badge">${t.daysLeft} day${t.daysLeft === 1 ? '' : 's'} left</span></div>
                        <p class="settings-card-hint">Anjadhe keeps working after the trial; what stops is updates. Updates continue until ${esc(human(t.endsAt))}, then this Mac stays on the build it has until a license is entered.</p>`;
                } else {
        ...
        document.getElementById('license-contact-link')?.addEventListener('click', (e) => {
            e.preventDefault();
            window.electronAuth?.openExternal?.(WhatsNew.FOUNDER_MAILTO.replace(/subject=[^&]*/, 'subject=Anjadhe%20license'));
        });
    ...
    _renderConnectedAccounts() {
        ...
        if (accounts.length === 0) {
            html += this._acct({
                ...
                desc: 'One sign-in links Gmail and Calendar. Mail syncs from Google straight to this Mac and your own model reads it here; nothing passes through a server of Anjadhe’s. Optional, and you can disconnect at any time.'
            });
        } else {
     ...
     * pass through Anjadhe's server and that Plaid is in the loop — this is
    ...
    async _renderBrokerages() {
        ...
        const cards = [...byItem.values()].map(it => {
            ...
            else if (it.status === 'missing') { tone = 'warn'; status = 'No longer linked on Anjadhe Cloud — unlink to clear'; }
            ...
            actions.push(this._acctBtn('', 'Unlink', { cls: 'brokerage-unlink-btn', data, title: 'Remove the link at Anjadhe Cloud and Plaid, and its accounts from Portfolio and Spending' }));
            return this._acct({
        ...
        if (fetchErr?.code === 'disabled') {
            head = this._acct({ title, status: 'Not enabled on Anjadhe Cloud yet', tone: 'off', desc });
        } else if (fetchErr) {
            head = this._acct({
                title, status: 'Anjadhe Cloud could not be reached — showing what this Mac knows', tone: 'warn', desc,
        ...
        if (orphanBtn) orphanBtn.addEventListener('click', async () => {
            ...
            const ok = await UIUtils.confirm('Remove leftover accounts',
                `Delete ${n === 1 ? 'this account' : `these ${n} accounts`} from Portfolio, with the positions and trades ${n === 1 ? 'it' : 'they'} brought in? Nothing changes at Anjadhe Cloud or Plaid.`,
                '❓', { confirmText: 'Remove' });
            if (!ok) return;
        ...
        container.querySelectorAll('.brokerage-unlink-btn').forEach(btn => btn.addEventListener('click', async () => {
            ...
            const ok = await UIUtils.confirm('Unlink institution',
                `Disconnect ${it.institution || 'this institution'} from Anjadhe Cloud? The link is removed at Plaid too${gone.length ? `, and ${gone.join(', and ')} ${nPf + nSp === 1 ? 'is' : 'are'} removed from the app` : ''}.`,
                '❓', { confirmText: 'Unlink' });
            if (!ok) return;
            btn.disabled = true;
            try {
     ...
     * Anjadhe users paired by invite code over the peer channel
    ...
    async _renderContacts() {
        ...
        const desc = 'Another Anjadhe user you have paired with by invite code, so you can share a Portfolio account with them read-only (and they with you). Everything travels end to end encrypted between your Macs through Anjadhe’s relay, which sees only ciphertext and keeps no record of who shares with whom. For now both Macs have to be open and online for a share or an update to arrive; a shared copy that hears nothing for 30 days removes itself.';
    ...
    _imessageInsightsCardHtml() {
        ...
        const fdaHint = st.enabled && st.lastErrorReason === 'fda'
            ? 'How to allow it: 1. Open System Settings › Privacy & Security › Full Disk Access. 2. Click the + button under the list (Anjadhe is not listed until you add it). 3. Choose Anjadhe from your Applications folder and make sure its switch is on. 4. Quit Anjadhe, open it again, then press Check now.' : '';
        return this._acct({
    ...
    _renderAppleImport() {
        ...
        container.innerHTML = this._acct({
            ...
            desc: 'Import this Mac’s Apple Notes into Notes, tagged “apple-notes” plus the name of their Apple Notes folder. Read-only; locked notes are skipped; turning this off removes the imported copies. Turn this on for ONE Mac only — imported notes reach your other Macs through Anjadhe’s own sync.',
    ...
    async _renderRemoteAccess(pending) {
        ...
        if (!st.installed) {
            ...
            text = 'Off — away from home, your phone goes through the Anjadhe Connect relay';
            control = this._acctSwitch('remote-access-toggle', false, !st.relayPort ? 'disabled' : '');
        }
    ...
    _renderAnjadheManage(body, entry) {
        ...
        for (const line of [
            ...
            'Work Anjadhe starts on its own sends only the kinds of data you allow in Cloud Privacy; chat sends what you typed.',
        ...
        ledgerBtn.textContent = 'See everything this Mac has sent to Anjadhe Cloud';
    ...
    _renderAddLocalStep(body, ctx, engine) {
        ...
        desc.textContent = belowFloor
            ? `Running a model on the Mac itself takes more memory than this Mac has (${totalRam} GB), so none is offered here. Anjadhe Cloud, a server you own, or your own API key (in the list on the left) are the ways to run the AI on a Mac this size.`
     ...
     * Anjadhe Cloud card opens 'left': the Sent-to-Cloud ledger IS this
    ...
    _renderPrivacySettings() {
        ...
        // The id in the form Anjadhe STORES — the only one that appears on the
    ...
    async _renderContentFilesToggle() {
        ...
        if (rowHint) rowHint.textContent = info.enabled ? 'On · Markdown in ~/Anjadhe' : 'Off';
    ...
    async _renderModelRoutes() {
        ...
        // name already says it ("Anjadhe Cloud · Anjadhe Cloud").
        const where = (e) => {
            if (!e) return '';
            const place = ModelRouting.isLocal(e) ? 'this Mac'
                : e.engine === 'anjadhe' ? 'Anjadhe Cloud'
    ...
    _renderCloudPrivacy() {
        ...
        if (desc) {
            desc.textContent = brainLeaves
                ? `Your AI model runs on ${dest || 'a server off this Mac'}. Anjadhe does some work without being asked — email insights, routines, reviews — and each kind of data below may or may not go along.`
                : anyLeaves
                    ...
                    : 'Your AI model runs on this Mac, so nothing leaves it. These choices take effect the moment you pick a model that runs elsewhere (Anjadhe Cloud, your own key or server).';
        }
