...
const SettingsApp = {
    ...
    showRoot() {
        // Reader/Writing Voices ship as a package (js/apps/reader/); with it
        // uninstalled the Library settings govern nothing — hide the door.
        const libCard = document.querySelector('.settings-card-nav[data-cat="library"]');
        if (libCard) libCard.hidden = (typeof LibraryApp === 'undefined');
        this._mode = 'root';
        ...
        if (!st.configured) {
            ...
            body = `
                <div class="connected-account-row">
                    <div class="connected-account-header">
                        <div class="connected-account-info">
                            <div class="connected-account-email">Chat from your phone</div>
                            <div class="connected-account-subtitle">Message the assistant on this Mac from the Telegram app.
                                Create a bot with Telegram's @BotFather (send it /newbot), then paste the token it gives you.
                                Messages and replies travel through Telegram's servers; the token is stored encrypted on this Mac.
                                Set this up on ONE Mac &mdash; Telegram allows a single listening app per bot.</div>
                        </div>
                    </div>
                    <div class="connected-account-actions-row">
                        <input type="password" id="telegram-token-input" class="settings-input"
                               placeholder="Bot token from @BotFather" style="flex:1; min-width: 200px;">
                        <button id="telegram-connect-btn" class="secondary-btn">Connect</button>
                    </div>
                    <div class="connected-account-subtitle" id="telegram-setup-error" style="color:#dc2626"></div>
                </div>
            `;
        } else {
            const botLabel = st.bot
                ? `${this._esc(st.bot.name || 'Bot')}${st.bot.username ? ' &middot; @' + this._esc(st.bot.username) : ''}`
                : 'Bot';
            let linkBody = '';
            if (st.chat) {
                linkBody = `
                    <div class="connected-account-actions-row">
                        <span class="connected-account-subtitle">Linked to ${this._esc(st.chat.name || 'a chat')} &mdash; only that chat is answered.</span>
                        <button id="telegram-unlink-btn" class="secondary-btn">Unlink</button>
                    </div>
                    <div class="connected-account-actions-row">
                        <span class="connected-account-subtitle">Send this Mac's notifications there too &mdash; reminders and alerts
                            arrive as messages in the linked chat (through Telegram's servers).</span>
                        <label class="settings-switch">
                            <input type="checkbox" id="telegram-notify" ${st.notify ? 'checked' : ''}>
                            <span class="settings-switch-track"></span>
                        </label>
                    </div>
                `;
                ...
                linkBody = `
                    <div class="connected-account-actions-row">
                        <span class="connected-account-subtitle">In Telegram, send this code to
                            ${st.bot && st.bot.username ? '@' + this._esc(st.bot.username) : 'your bot'}:
                            <strong style="font-size: 1.1em; letter-spacing: 0.1em;">${this._esc(this._telegramLinkCode)}</strong>
                            &mdash; the chat that sends it becomes the one this Mac answers.</span>
                        <button id="telegram-cancel-link-btn" class="secondary-btn">Cancel</button>
                    </div>
                `;
            } else {
                linkBody = `
                    <div class="connected-account-actions-row">
                        <span class="connected-account-subtitle">Not linked to a chat yet &mdash; nothing is answered until you link yours.</span>
                        <button id="telegram-link-btn" class="secondary-btn">Link your Telegram</button>
                    </div>
                `;
            }
            let statusLine = '';
            if (st.lastError) statusLine = `<span style="color:#dc2626">${this._esc(st.lastError)}</span>`;
            else if (st.enabled && st.running) statusLine = 'Listening for messages.';
            else if (!st.enabled) statusLine = 'Off &mdash; messages are not received.';
            body = `
                <div class="connected-account-row">
                    <div class="connected-account-header">
                        <div class="connected-account-info">
                            <div class="connected-account-email">${botLabel}</div>
                            <div class="connected-account-subtitle">The assistant answers your Telegram messages using its tools
                                (it can read your data and add tasks or notes; anything needing confirmation is declined and has
                                to be done here). Messages and replies travel through Telegram's servers.</div>
                        </div>
                        <div class="connected-account-row-actions">
                            <label class="settings-switch">
                                <input type="checkbox" id="telegram-enabled" ${st.enabled ? 'checked' : ''}>
                                <span class="settings-switch-track"></span>
                            </label>
                        </div>
                    </div>
                    ${linkBody}
                    <div class="connected-account-actions-row">
                        <span class="connected-account-subtitle" id="telegram-status-line">${statusLine}</span>
                        <button id="telegram-disconnect-btn" class="secondary-btn">Disconnect</button>
                    </div>
                </div>
            `;
        }
    ...
    async _renderIMessage() {
        ...
        let statusLine = '';
        if (st.lastError) statusLine = `<span style="color:#dc2626">${this._esc(st.lastError)}</span>`;
        else if (this._imessageTestSent) statusLine = 'Test sent &mdash; check your phone.';
        else if (st.notify && st.handle) statusLine = 'On &mdash; reminders and alerts are sent to your iMessage.';
        else if (st.handle) statusLine = 'Off &mdash; nothing is sent.';
        ...
        container.innerHTML = `
            <div class="connected-account-row">
                <div class="connected-account-header">
                    <div class="connected-account-info">
                        <div class="connected-account-email">Notifications on your phone</div>
                        <div class="connected-account-subtitle">Send this Mac's reminders and alerts to your own iMessage, through
                            the Messages app on this Mac &mdash; a text from you to you, so it reaches your iPhone and Watch.
                            Messages here must be signed in to iMessage; the first send asks macOS for permission to control
                            Messages. Outbound only: the assistant does not read your iMessages.</div>
                    </div>
                    <div class="connected-account-row-actions">
                        <label class="settings-switch">
                            <input type="checkbox" id="imessage-notify" ${st.notify ? 'checked' : ''} ${st.handle ? '' : 'disabled'}>
                            <span class="settings-switch-track"></span>
                        </label>
                    </div>
                </div>
                <div class="connected-account-actions-row">
                    <input type="text" id="imessage-handle-input" class="settings-input" value="${this._esc(st.handle)}"
                           placeholder="Your iMessage number or email" autocomplete="off" style="flex:1; min-width: 200px;">
                    <button id="imessage-save-btn" class="secondary-btn">Save</button>
                    <button id="imessage-test-btn" class="secondary-btn" ${st.handle ? '' : 'disabled'}>Send a test</button>
                </div>
                <div class="connected-account-actions-row">
                    <span class="connected-account-subtitle" id="imessage-status-line">${statusLine}</span>
                </div>
            </div>
        `;
        ...
        if (saveBtn) saveBtn.addEventListener('click', async () => {
            ...
            if (res && res.error) {
                const line = document.getElementById('imessage-status-line');
                if (line) line.innerHTML = `<span style="color:#dc2626">${this._esc(res.error)}</span>`;
                return;
            }
            this._renderIMessage();
        });
