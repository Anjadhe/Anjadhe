...
const SettingsApp = {
    ...
    async _loadAgentPermissions() {
        ...
        const row = (g) => {
            ...
            const since = created.toLocaleDateString([], { month: 'short', day: 'numeric' });
            return `
            <div class="settings-card agent-perm-row" data-grant="${esc(g.id)}">
                <div class="agent-perm-main">
                    <div class="agent-perm-title">${esc(p.title)}</div>
                    ${p.detail ? `<div class="agent-perm-detail" title="${esc(p.detail)}">${esc(p.detail)}</div>` : ''}
                    ${b ? `<div class="agent-perm-bounds">${esc(b)}</div>` : ''}
                </div>
                <span class="agent-perm-since" title="Granted ${esc(created.toLocaleDateString())}">${esc(since)}</span>
    ...
    LLM_SECTIONS: {
        ...
        'settings-search-section': { label: 'Web Search' },
