...
(function registerSpendingTools() {
    ...
    reg({
        ...
        if (!SpendingApp.accounts().length) return { linked: false, hint: 'No bank or card is linked. In Spending, click "+ Link a bank or card" to connect one through nenva Cloud.' };
    ...
    if (typeof SpendingSync !== 'undefined' && SpendingSync.available()) reg({
        name: 'sync_spending',
        description: 'Pull the latest transactions and balances from the user\'s LINKED banks and cards (connected through nenva Cloud). Reports what is linked, how stale each is, and what arrived. Use dryRun=true to only report link status. Linking a new bank is not a tool action: tell the user to click "+ Link a bank or card" in Spending.',
        parameters: { type: 'object', properties: {
        ...
        if (!links.length) return { linked: [], hint: 'No bank or card is linked. In Spending, click "+ Link a bank or card" to connect one through nenva Cloud.' };
        ...
        ask: true, blockUntrusted: true,
        describe: (args) => args.dryRun ? 'Report which banks and cards are linked (no pull)' : 'Pull the latest transactions from your linked banks and cards through nenva Cloud'
    });
