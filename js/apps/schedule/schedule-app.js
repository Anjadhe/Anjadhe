...
const ScheduleApp = {
    ...
    setupEventListeners() {
        // The standalone Tasks list page is retired (markup removed) — its
        // controls may not exist. Only the editor bindings below are load-
        // bearing; list-page bindings are all guarded.
        ...
        // Auto-save: existing tasks commit on every field edit — debounced
        // for typing, and 'change' catches selects/dates/checkboxes.
        // Delegated at the DOCUMENT and keyed on the editor's content
        // wrapper: embedEditor moves the editor view's CHILDREN into the
        // Actions/Plan host, so a listener on #schedule-editor-view itself
        // would go deaf the moment the editor is embedded.
        if (!document._scheduleAutoSaveWired) {
            document._scheduleAutoSaveWired = true;
            // Skips when a leave-flush already wrote (nothing dirty).
            this._autoSaveDebounced = UIUtils.debounce(() => { if (this._editorDirty) this.saveCurrentItem({ auto: true }); }, 600);
            const onEdit = (e) => {
                if (!e.target.closest || !e.target.closest('.schedule-editor-main')) return;
                // Dirty from the first keystroke — the head's indicator says
                // "Unsaved" until a write lands (SaveStatus). A NEW task is
                // dirty too: it has no auto-save, so leaving it is what the
                // flush-on-leave alert is for.
                this._editorDirty = true;
                this._markSave('dirty');
        ...
        // Date / time clear buttons (the × beside Date, Start and End).
        // A native date input has no reliable way to empty itself — the
        // picker offers no "none" and Backspace only blanks one segment —
        // so the editor carries its own. Delegated at the document for the
        // same reason as auto-save above (the editor DOM gets embedded).
        if (!document._scheduleFieldClearWired) {
            document._scheduleFieldClearWired = true;
            document.addEventListener('click', (e) => {
                const btn = e.target.closest && e.target.closest('.schedule-field-clear');
                if (!btn || !btn.closest('.schedule-editor-main')) return;
                const input = document.getElementById(btn.dataset.clearFor);
                if (!input) return;
                input.value = '';
                // 'change' is what the auto-save listener above hears for
                // date/time fields; a new task just waits for Save.
                input.dispatchEvent(new Event('change', { bubbles: true }));
                this._syncFieldClearButtons();
            }, true);
            const onFieldEdit = (e) => {
                if (e.target.closest && e.target.closest('.schedule-field-value')) this._syncFieldClearButtons();
            };
        ...
        // Repeat type change. The clone-to-rewire trick loses a <select>'s
        // LIVE value (a clone carries the markup's `selected` option, i.e.
        // "None"), and init() runs on every embed — so with a recurring
        // task open, opening another task or hopping to Projects flushed
        // the still-current task with Repeat reset to none (2026-09-10;
        // the record silently became one-time). Carry the value across.
        const repeatSelect = document.getElementById('schedule-repeat-select');
        const newRepeatSelect = repeatSelect.cloneNode(true);
        newRepeatSelect.value = repeatSelect.value;
        ...
        if (focusBtn) {
            ...
            newFocusBtn.addEventListener('click', () => {
                ...
                if (this.currentItemId && focus) focus.startForTask(this.currentItemId);
            });
        }
    ...
    updateRepeatOptions(repeatType) {
        ...
        // A recurring task can't be undated: its date is the anchor every
        // occurrence is computed from, so the clear button steps aside.
        const dateValue = document.getElementById('schedule-date-value');
        if (dateValue) dateValue.classList.toggle('no-clear', !isOneTime);
        // ...and an undated task turning recurring gets today as that
        // anchor — shown in the field, since that is what save will store.
        const dateInput = document.getElementById('schedule-date-input');
        if (!isOneTime && dateInput && !dateInput.value) dateInput.value = this.getLocalToday();
    },

    /** Show a field's × only while it holds a value. */
    _syncFieldClearButtons() {
        document.querySelectorAll('.schedule-editor-main .schedule-field-value').forEach(wrap => {
     ...
     * @param {string} type - 'all' | 'unassigned' | 'goal'
     * @param {string|null} id - goal id when type is 'goal'
     */
    setFilter(type, id = null) {
        this.activeFilter = { type, id: id || null };
        // The sidebar picks WHICH tasks, the Agenda/List toggle picks HOW
        // they're grouped — deliberately independent.
        this.render();
    },
    ...
    toggleComplete(id) {
        ...
        if (!done) {
            // Every caller of this funnel is a user gesture (Actions checkbox,
            // detail button, row menus, widget, Pomodoro) — the agent's
            // update_task writes completion directly and stays quiet.
    ...
    openEditor(itemId = null, opts = {}) {
        // Switching tasks: flush the previous task's still-debounced edits
        // BEFORE the fields repopulate, or the last keystrokes are lost.
        ...
        this._registerSaveStatus();
        // An existing task starts as quietly "Saved" (the answer to "is
        // it saved?" is always on the page); a new one says nothing until
        // typed into.
        this._markSave(itemId ? 'saved' : 'idle', '', { quiet: true });
        ...
        this._renderEditorAskPills(item);
        ...
        if (itemId) {
            ...
            if (item) {
                ...
                this._renderEditorTagPills(item.tags || []);
                ...
                // Show the message source if applicable: an email, or since
                // 2026-09-10 an iMessage conversation (same ledger key; the
                // banner names which and the door lands on the insight).
                const sourceKind = this.messageSourceKind(item);
                if (emailBanner && sourceKind) {
                    const isText = sourceKind === 'imessage';
                    emailBanner.style.display = '';
                    emailBanner.style.cursor = 'pointer';
                    emailBanner.classList.toggle('is-text', isText);
                    document.getElementById('schedule-email-source-from').textContent = `${isText ? 'Text from' : 'From'}: ${item.sourceEmailFrom || 'Unknown'}`;
                ...
                // Existing tasks auto-save — no explicit Save to press.
                const saveBtn = document.getElementById('schedule-editor-save-btn');
            ...
            // Creation is the one explicit save left.
            const saveBtn = document.getElementById('schedule-editor-save-btn');
        ...
        // Updates posted on this task — the running log, above the links
        // because it is what changes; the check-in door hands the task to
        // the assistant and asks it to post its read back here.
        const updatesEl = document.getElementById('schedule-updates-section');
        if (updatesEl && typeof UpdatesUI !== 'undefined') {
            if (itemId && item) {
                UpdatesUI.mount(updatesEl, `task:${itemId}`, {
                    compact: true,
                    since: item.createdAt,
                    checkIn: `Check in on my task "${item.title}": read its notes and updates, say plainly where it stands and what is at risk, and post that as an update on the task with post_update.`
                });
            } else updatesEl.innerHTML = '';
        }

        // Decisions saved about this task (assistant- or user-written)
        const decisionsEl = document.getElementById('schedule-decisions-section');
        if (decisionsEl && typeof DecisionsUI !== 'undefined') {
            if (itemId) DecisionsUI.mount(decisionsEl, `task:${itemId}`);
    ...
    renderEditorLinkedItems(itemId) {
        ...
        // Pending assistant filing suggestion (same chip the Actions list
        // shows) — accepting links the goal.
        const editItem = this.scheduleItems.find(i => i.id === itemId);
        const suggestMeta = (!linkedGoal && editItem
            && editItem.suggestionState === 'pending' && editItem.suggestedGoalId)
            ? LinkManager.getItemMeta('goals', editItem.suggestedGoalId) : null;

        html += `<div class="schedule-goal-picker">
            <label class="schedule-field-label">Project</label>
            <div class="schedule-goal-search-wrap${linkedGoal ? ' is-linked' : ''}">
                ${linkedGoal
                    ? `<button class="schedule-goal-link" id="schedule-goal-link" title="Open this project">${UIUtils.escapeHtml(linkedGoal.title)}</button>`
                    : `<input type="text" id="schedule-goal-search" class="schedule-goal-search" placeholder="Search projects..." value="">`}
                ${linkedGoal ? `<button class="schedule-goal-clear" id="schedule-goal-clear" title="Remove or change this project">&times;</button>` : ''}
                <div id="schedule-goal-dropdown" class="schedule-goal-dropdown" style="display: none;"></div>
            </div>
            ${suggestMeta ? `<div class="schedule-goal-suggest">
                <span class="actions-suggest-chip" title="Assistant suggestion: file under ${UIUtils.escapeHtml(suggestMeta.title)}">
                    <span class="schedule-suggest-text"><span class="actions-suggest-label">Suggested:</span> ${UIUtils.escapeHtml(suggestMeta.title)}?</span>
                    <button class="actions-suggest-act" id="schedule-suggest-accept" title="File under this project">&#10003;</button>
                    <button class="actions-suggest-act" id="schedule-suggest-dismiss" title="Don't file">&#10005;</button>
                </span>
            </div>` : ''}
        </div>`;
        ...
        html += `<button class="goals-linked-toggle schedule-completed-toggle" data-toggle="schedule-linked-notes" aria-expanded="${!!expanded}">
        ...
        // Suggestion accept/dismiss — one recipe with the Actions list chips
        // (ActionsApp._handleSuggestAction links goal + focus, saves, toasts).
        const suggestWire = (btnId, act) => {
            const btn = document.getElementById(btnId);
            if (!btn) return;
            btn.addEventListener('click', () => {
                if (typeof ActionsApp !== 'undefined') {
                    ActionsApp._handleSuggestAction(act, itemId);
                }
                this.renderEditorLinkedItems(itemId);
            });
        };
        suggestWire('schedule-suggest-accept', 'goal-yes');
        suggestWire('schedule-suggest-dismiss', 'goal-no');
        ...
        const renderDropdown = (query) => {
            ...
            if (filtered.length === 0) {
                ...
                dropdown.innerHTML = filtered.map(g => `
                    <div class="schedule-goal-option" data-goal-id="${g.id}">
            ...
            dropdown.querySelectorAll('.schedule-goal-option[data-goal-id]').forEach(opt => {
                opt.addEventListener('mousedown', (e) => {
                    e.preventDefault();
                    // The dropdown only exists while nothing is linked (the
                    // linked state is a title + ×), so there is no prior link
                    // to clear here.
        ...
        // A linked goal is a LINK, not a field: the title opens the goal (the
        // task detail's only way through to it), and the × is how you remove
        // or change it — it unlinks and hands the search field straight back,
        // focused, so "change" stays one gesture.
        const linkBtn = document.getElementById('schedule-goal-link');
        if (linkBtn && linkedGoal) {
            linkBtn.addEventListener('click', () => {
                LinkedItemsUI.navigateToItem('goals', linkedGoal.itemId);
            });
        }
        ...
        if (clearBtn) {
            clearBtn.addEventListener('click', () => {
                if (linkedGoal) {
                    ...
                    this.renderEditorLinkedItems(itemId);
                    document.getElementById('schedule-goal-search')?.focus();
                }
            });
        }
    ...
    closeEditor() {
        // Flush any edit still inside the debounce window — leaving must
        // never discard typed changes. What cannot be saved (a new task
        // that was never Saved, a blank title) is announced, never dropped
        // in silence.
        this._leaveEditor({ announce: true });
        ...
        // Embedded in the Goals page's detail pane: the view is already
        // active, so just hand the pane back (restores the pre-task selection).
        if (origin === 'plan') {
            GoalsPage.closeTaskDetail();
            return;
        }

        // The FYI tab's insight detail — it holds the open insight in its own
        // state, so re-opening the page lands back on it.
        if (origin === 'fyi') {
            AppManager.openApp('fyi');
            return;
        }

        // Opened from the analysis panel under a message (its action items are
        // doors to their tasks). LinkedItemsUI has no 'email' case — mail is
        // not a linkable item — so route it here, or the object branch below
        // would swallow the origin and strand the user in Tasks.
        if (origin && typeof origin === 'object' && origin.app === 'email' && origin.itemId) {
            EmailApp.openMessageFrom(origin.itemId);
            return;
        }
        ...
        // No origin: land on the Actions hub — the standalone Tasks list
        // page this used to reactivate is retired.
        AppManager.openApp('actions');
    },
    ...
    _renderEditorTagPills(tags = []) {
        ...
        const addBtn = document.getElementById('schedule-tag-add-btn');
        if (!container || !addBtn) return;
        container.querySelectorAll('.tag-item').forEach(t => t.remove());
        tags.forEach(tag => container.insertBefore(this._buildEditorTagPill(tag), addBtn));
    },

    _buildEditorTagPill(tag) {
        ...
        el.querySelector('.tag-remove').addEventListener('click', () => {
            el.remove();
            // Existing tasks autosave field edits; a new task's pills just
            // wait for the explicit Save like every other field.
            if (this.currentItemId) this.saveCurrentItem({ auto: true });
        });
        return el;
    },

    _getEditorTags() {
        return Array.from(document.querySelectorAll('#schedule-tags-container .tag-item'))
            .map(el => el.dataset.tagName)
            .filter(Boolean);
    },

    /** Every tag name in use on any task — the picker's suggestion list. */
    _allTagNames() {
        const names = new Set();
        for (const it of this.scheduleItems) {
    ...
    _setupEditorTagPicker() {
        const addBtn = document.getElementById('schedule-tag-add-btn');
        if (!addBtn) return;
        // Re-clone per open so stale listeners never stack (the notes recipe).
        ...
        newBtn.addEventListener('click', (e) => {
            ...
            TagPicker.open({
                anchor: newBtn,
                suggestions: this._allTagNames(),
                selected: this._getEditorTags(),
                placeholder: 'Search or create…',
                onAdd: (name) => {
                    const trimmed = String(name || '').trim();
                    if (!trimmed || this._getEditorTags().some(t => t.toLowerCase() === trimmed.toLowerCase())) return;
                    const container = document.getElementById('schedule-tags-container');
                    container.insertBefore(this._buildEditorTagPill(trimmed), newBtn);
                    if (this.currentItemId) this.saveCurrentItem({ auto: true });
                },
            });
        });
    ...
    // auto:true = the auto-save path for EXISTING tasks: silent (no toasts),
    // stays open, never creates. Validation failures just skip the commit —
    // the last valid state stands until the fields are valid again.
    // Creation stays behind the explicit Save button so half-typed tasks
    // don't spawn on the first keystroke.
    saveCurrentItem({ auto = false, quiet = false } = {}) {
        // auto never creates; quiet creates without closing (the
        // flush-on-leave toast's Save action — the fields are still in the
        // DOM, the user is already elsewhere).
        if (auto && !this.currentItemId) return { ok: false, reason: 'press Save to add it' };
        ...
        // Tags: names on the record (the notes-app shape, no registry for
        // tasks) — read from the editor's pill strip.
        const tags = this._getEditorTags();

        if (!title) {
            if (auto) this._markSave('error', 'needs a title');
            else UIUtils.showToast('Please enter a task name', 'error');
        ...
        if (repeat === 'weekly') {
            ...
            if (repeatDays.length === 0) {
                if (auto) this._markSave('error', 'pick a repeat day');
                else UIUtils.showToast('Please select at least one day', 'error');
                return { ok: false, reason: 'its repeat needs a day' };
            }
        }
        ...
        // An empty date is a real state for a one-time task — a "someday"
        // to-do that lives in the No date bucket (the same null the
        // reschedule menu's "No date" writes). A recurring task keeps
        // needing its anchor, so an empty field there still means today.
        const isOneTime = !repeat || repeat === 'none';
        const dateToSave = scheduledDate || (isOneTime ? null : this.getLocalToday());

        if (this.currentItemId) {
            const item = this.scheduleItems.find(i => i.id === this.currentItemId);
            if (!item) { this._editorDirty = false; return { ok: true }; } // deleted under us (auto-flush after delete)
            {
                item.title = title;
                ...
                // tasks (null = undated) and the START/anchor date for
                // recurring ones.
                item.scheduledDate = dateToSave;
                item.reminderDaysBefore = isOneTime ? reminderDaysBefore : [];
                item.modifiedAt = new Date().toISOString();
            }
        } else {
            ...
            this.scheduleItems.push({
                ...
                reminderDaysBefore: isOneTime ? reminderDaysBefore : [],
        ...
        this.saveData();
        this._editorDirty = false;
        this._markSave('saved');
        if (auto) return { ok: true }; // stay open; the editor keeps reflecting the fields
        if (quiet) { UIUtils.showToast('Task added', 'success'); return { ok: true }; }
    ...
    _markSave(state, reason = '', { quiet = false } = {}) {
        if (typeof SaveStatus === 'undefined') return;
        const el = document.getElementById('schedule-editor-save-status');
        if (!el) return;
        SaveStatus.mark(el, state, reason, { quiet });
    },

    /**
     * Write the open editor's edits now. { ok:true } when nothing is dirty
     * or the write landed; otherwise why not, plus the toast action that
     * fits: a titled NEW task offers Save (creates it in place — the
     * fields are still in the DOM), an invalid one offers a way back.
     */
    flushEditor() {
        if (!this._editorDirty) return { ok: true };
        const res = this.saveCurrentItem({ auto: true }) || { ok: true };
        if (res.ok) return res;
        if (!this.currentItemId) {
            const titled = !!document.getElementById('schedule-title-input')?.value.trim();
            if (titled) return { ok: false, reason: '', actionLabel: 'Save', onAction: () => this.saveCurrentItem({ quiet: true }) };
            return { ok: false, reason: 'it needs a title', actionLabel: 'Go back', onAction: () => this._reopenAsIs() };
        }
        return { ok: false, reason: res.reason, actionLabel: 'Go back', onAction: this._reopenCurrent(this.currentItemId, this._editorOrigin) };
    },

    /** How to get back to task `id` through the door it was opened from. */
    _reopenCurrent(id, origin) {
        const host = this._embedHost;
        if (typeof host?._reopenTask === 'function') return () => host._reopenTask(id);
        if (origin === 'plan') return () => { AppManager.openApp('goals'); setTimeout(() => GoalsPage.selectNode('task', id), 0); };
        return () => { AppManager.openApp('actions'); if (typeof ActionsApp !== 'undefined') ActionsApp._openTaskEditor(id); };
    },

    /** Re-show the standalone editor with whatever is still typed in it. */
    _reopenAsIs() {
        this.restoreEditorHome();
        document.querySelectorAll('.view.active').forEach(v => v.classList.remove('active'));
        document.getElementById('schedule-editor-view').classList.add('active');
    ...
    _registerSaveStatus() {
        if (typeof SaveStatus === 'undefined') return;
        SaveStatus.register('task', {
            isDirty: () => !!this._editorDirty,
            flush: () => this.flushEditor(),
            label: () => {
                const t = document.getElementById('schedule-title-input')?.value.trim();
                return this.currentItemId ? (t ? `Task "${t}"` : 'This task') : 'New task';
            }
        });
    },

    /**
     * Leave the editor: flush, announce what could not be saved, drop the
     * registration. The shared exit for closeEditor and for hosts that
     * take the DOM back without closing (detachEditor).
     */
    _leaveEditor({ announce = false } = {}) {
        const res = this.flushEditor();
        if (!res.ok && announce && typeof SaveStatus !== 'undefined') {
            SaveStatus.announce({ label: this.currentItemId ? 'This task' : 'New task', ...res });
        }
        this._editorDirty = false;
        if (typeof SaveStatus !== 'undefined') SaveStatus.unregister('task');
        return res;
    },

    /**
     * A host page is taking its pane back (Tasks nav click, Projects
     * re-render): flush first, then hand the DOM home. The old path went
     * straight to restoreEditorHome and left currentItemId set — the
     * debounce fired later against a detached editor, and the list the
     * user was already looking at showed the stale title (2026-09-10).
     */
    detachEditor() {
        if (this.currentItemId || this._editorDirty) this._leaveEditor({ announce: true });
        this.currentItemId = null;
        this.restoreEditorHome();
    },
    ...
    async deleteCurrentItem() {
        ...
        if (confirmed) {
            this.deleteTask(this.currentItemId);
            this.closeEditor();
    ...
    navigateToSourceEmail(emailId) {
        if (!emailId) return;
        // Captured before the editor closes: coming back means reopening THIS
        // task, with the origin it was opened with, so the chain of back
        // links stays intact all the way to the list the user started on.
        const taskId = this.currentItemId;
        const editorOrigin = this._editorOrigin;
        // The task detail is USUALLY embedded in a page (Actions › Tasks,
        // Goals), not the standalone editor view — so "back to the task"
        // means back to that page with the task open in it, which only the
        // host knows how to do (it stamps _reopenTask when it embeds).
        // Reopening the editor here instead would land the user on a bare
        // editor view they never visited.
        const reopen = typeof this._embedHost?._reopenTask === 'function'
            ? this._embedHost._reopenTask
            : null;
        document.getElementById('schedule-editor-view').classList.remove('active');
        // openMessageFrom, NOT a deferred openViewer: EmailApp.init is async
        // and decides its own opening view at the end of a chain of awaits, so
        // a setTimeout(0) painted the message and then had the list painted
        // straight over it (the card looked broken: a flash of the email, then
        // the inbox). Honoured at the point of truth inside _initBody.
        EmailApp.openMessageFrom(emailId, {
            label: 'Task',
            onBack: () => {
                if (reopen) { reopen(taskId); return; }
                // Standalone editor (opened from the Calendar, a link): the
                // editor view IS where the user was.
                this.init();
                this.openEditor(taskId, { origin: editorOrigin });
            },
        });
    },
    ...
    render() {
        // The standalone Tasks list page is retired — nothing to paint.
        // (Data writes still happen; Actions/Goals render their own lists.)
        if (!document.getElementById('schedule-view')) return;
    ...
    _renderBreadcrumb() {
        ...
        const crumbs = [{ label: 'Tasks', action: () => AppManager.openApp('actions') }];
        if (f.type === 'all') {
            ...
            if (f.type === 'unassigned') {
                ...
                const meta = LinkManager.getItemMeta('goals', f.id);
                if (!meta) {
                    this.activeFilter = { type: 'all', id: null };
    ...
    _buildTaskCrumbs(taskId, taskTitle) {
        ...
        // Same for the Tasks page — route back where the user was. Embedded
        // in the Tasks pane (2026-09-10) the crumb IS the way back, so it
        // names the selection ("Tomorrow", "Learning · This Week") the way
        // the project sheet's crumb names its group.
        if (this._editorOrigin === 'actions') {
            const label = (typeof ActionsApp !== 'undefined' && ActionsApp._selLabel)
                ? ActionsApp._selLabel() : 'Tasks';
            crumbs.push({ label, action: () => this.closeEditor() });
            crumbs.push({ label: taskId ? 'Task' : 'New Task' });
            return crumbs;
        }

        if (this._editorOrigin === 'fyi') {
            crumbs.push({ label: 'Insights', action: () => this.closeEditor() });
            crumbs.push({ label: taskId ? 'Task' : 'New Task' });
            return crumbs;
        }
        ...
        // Embedded in the Projects pane: "<project> › Task", the project
        // being the way back (GoalsPage.closeTaskDetail restores it).
        if (this._editorOrigin === 'plan' && goalCrumb) {
            crumbs.push({ label: goalCrumb.title, action: () => this.closeEditor() });
            crumbs.push({ label: taskId ? 'Task' : 'New Task' });
            return crumbs;
        }

        if (goalCrumb) {
            crumbs.push({ label: 'Projects', action: () => AppManager.openApp('goals') });
            crumbs.push({ label: goalCrumb.title, action: () => { AppManager.openApp('goals'); setTimeout(() => GoalsPage.selectNode('goal', goalCrumb.itemId), 0); } });
        } else {
    ...
    // The sheet's foot band: the task's quoted prompt pills + the open
    // composer pill (ActionsApp owns the wording). Empty for a new task
    // or without the assistant, and the band hides itself when empty.
    _renderEditorAskPills(item) {
        const row = document.getElementById('schedule-task-ask');
        if (!row) return;
        const canAsk = typeof AgentUI !== 'undefined' && AgentUI.askWithPrompt
            && typeof ActionsApp !== 'undefined' && ActionsApp.taskAskPillsHtml;
        row.innerHTML = (item && canAsk) ? ActionsApp.taskAskPillsHtml(item.title, item.id) : '';
        row.querySelectorAll('[data-ask]').forEach(b =>
            b.addEventListener('click', () => AgentUI.askWithPrompt(b.dataset.ask, { newChat: true })));
        // Empty composer with the task as context: the CURRENT TASK provider
        // reads currentItemId / the host page's selection, nothing to stamp.
...
if (typeof Anjadhe !== 'undefined') {
    Anjadhe.expose('schedule', {
        ...
        openTask(id) {
            if (!id) return;
            AppManager.openApp('actions');
            if (typeof ActionsApp !== 'undefined' && ActionsApp._openTaskEditor) setTimeout(() => ActionsApp._openTaskEditor(id), 0);
        }
    });
}
...
if (typeof AgentContext !== 'undefined') {
    AgentContext.register('schedule', () => {
        ...
        const abandoned = isOneTime
            ? !!ScheduleApp.lastAbandonedDate(item)
            : ScheduleApp.isAbandonedToday(item);
        // Abandoned = deliberately not done — resolved like completed, the
        // user isn't going to work on it anymore.
        const status = done ? 'completed' : abandoned ? 'abandoned (deliberately not done — treat as resolved)' : 'open';
