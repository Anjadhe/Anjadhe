...
const ScheduleApp = {
    ...
    setupEventListeners() {
        ...
        if (!document._scheduleAutoSaveWired) {
            ...
            this._autoSaveDebounced = UIUtils.debounce(() => this.saveCurrentItem({ auto: true }), 600);
            const onEdit = (e) => {
    ...
    openEditor(itemId = null, opts = {}) {
        ...
        if (this.currentItemId && this.currentItemId !== itemId) {
            this.saveCurrentItem({ auto: true });
        }
    ...
    closeEditor() {
        ...
        // never discard typed changes. No-ops when creating (no id), when
        // the fields are invalid, or when the item was just deleted.
        this.saveCurrentItem({ auto: true });
    ...
    saveCurrentItem({ auto = false } = {}) {
        if (auto && !this.currentItemId) return;
        ...
        if (!title) {
            if (!auto) UIUtils.showToast('Please enter a task name', 'error');
            return;
        }
        ...
        if (repeat === 'weekly') {
            ...
            if (repeatDays.length === 0) {
                if (!auto) UIUtils.showToast('Please select at least one day', 'error');
                return;
            }
        }
        ...
        if (this.currentItemId) {
            ...
            if (!item) return; // deleted under us (e.g. auto-flush after delete)
            {
                item.title = title;
        ...
        this.saveData();
        if (auto) return; // stay open; the editor keeps reflecting the fields
    ...
    _buildTaskCrumbs(taskId, taskTitle) {
        ...
        if (this._editorOrigin === 'fyi') {
            crumbs.push({ label: 'Email AI', action: () => this.closeEditor() });
            crumbs.push({ label: taskId ? 'Task' : 'New Task' });
            return crumbs;
        }
