...
const HelpApp = {
    ...
    TOPICS: [
        ...
        {
            id: 'fyi', group: 'Your day', title: 'Insights',
            blurb: 'Everything the assistant found in your email and texts, grouped by kind.',
            body: `
                <p>Not everything worth knowing is something to do. A flight confirmation, a renewal date, a statement closing, a package arriving: none of those is a task, but you still want them somewhere you can find them. <strong>Insights</strong> is that somewhere.</p>
        ...
        // ── The assistant ────────────────────────────────────────────
        {
            ...
            body: `
                ...
                <ul class="help-app-list">
                    ...
                    <li><strong>Do things for you</strong> — create tasks and notes, file action items, search the web, read pages, and (if enabled in Settings &rsaquo; AI) work with files.</li>
        ...
        },
        {
            ...
            body: `
                ...
                <p>nenva ships with a few starter routines — <strong>Daily Briefing</strong> (your tasks and calendar each morning), <strong>News Digest</strong>, <strong>Daily Motivation</strong>, and <strong>Weekly Reflection</strong>. Edit, reschedule, or delete them freely.</p>
        ...
        // ── Connected accounts ───────────────────────────────────────
        {
            id: 'email', group: 'Connected accounts', title: 'Inbox and Insights',
            ...
            body: `
                ...
                <ul class="help-app-list">
                    ...
                    <li><strong>All of it</strong> is collected in <strong>Insights</strong>, its own app beside the Inbox, grouped by kind: bookings, bills, deliveries, renewals &mdash; including the ones that became tasks.</li>
                </ul>
        ...
        },
        {
            ...
            body: `
                ...
                <p>Connect Google Calendar in Settings &rsaquo; Accounts. Today's events also appear at the bottom of the Tasks Today page for time context.</p>
                <ul>
                    <li><strong>Tasks on the same days</strong> &mdash; anything you schedule in Tasks shows on the calendar too, so the grid works without any account at all. The <em>Tasks</em> checkbox in the header hides them when you only want events.</li>
                    <li><strong>Google round-trips</strong> &mdash; new events, edits, deletes and recurring events all write back to Google; editing one occurrence of a series asks whether the change is for this event or all of them. Apple calendars are a read-only mirror.</li>
                    <li><strong>Ask about any event</strong> &mdash; open an event and use &ldquo;Ask about this event&hellip;&rdquo; to hand it to the assistant; the conversation already knows which event you mean.</li>
                </ul>`
        },
        ...
        },
        {
            ...
            body: `
                <p>Save links with a title and tags; browse them as a grid or a list. Clicking a bookmark opens the page in your own web browser; the small <em>i</em> button on a card opens its details (description, tags, edit, delete). The assistant can save bookmarks for you and can pull them into research when you ask.</p>`
        },
        {
            ...
            body: `
                ...
                <ul class="help-app-list">
                    ...
                    <li><strong>AI reviews</strong>: turn on a <strong>Wellness Review</strong> (Sunday mornings, an honest read of your week) or a <strong>Daily Check-in</strong> (a short morning note with one intention). Both post to your Home feed and use the computed trends.</li>
        ...
        },
        {
            ...
            body: `
                ...
                <ul class="help-app-list">
                    <li><strong>Topics</strong> — the Topics button opens the Topics page (or Edit topics in the rail). Pick from the suggestions or add your own; an optional location adds local news.</li>
                    ...
                    <li><strong>Show fewer like this</strong> — the &times; on any story teaches ranking; restore hidden stories from the Topics page. Boosted stories carry a "For you" label with the reason.</li>
                </ul>
        ...
        {
            id: 'reader', group: 'Apps', title: 'Documents',
            blurb: 'All your digital documents in one place — tagged, indexed on this Mac, and readable with the assistant.',
            body: `
                <p><strong>Documents</strong> is a repository for the paperwork of your life: PDFs, scans and photos of receipts and letters, statements, contracts, spreadsheets, Word and PowerPoint files, web pages, notes. Bring them in, keep them organized with tags, and find or research any of them with the assistant. Everything is parsed and indexed <em>on this Mac</em>.</p>
                <h4 class="help-heading">Getting documents in</h4>
                <p>Click <strong>Import files&hellip;</strong>, drag files anywhere onto the page, or drop them into the folder in Finder (<strong>Open folder</strong> shows it). Supported: <code>.pdf</code> (scans included), images (<code>.png</code>, <code>.jpg</code>, <code>.heic</code> &hellip; read with the Mac&rsquo;s own OCR), <code>.docx</code>, <code>.doc</code>, <code>.rtf</code>, <code>.odt</code>, <code>.xlsx</code>, <code>.csv</code>, <code>.pptx</code>, <code>.md</code>, <code>.txt</code>, <code>.html</code>. Files stay ordinary files in one folder you own.</p>
                <h4 class="help-heading">Tags, not folders</h4>
                <p>Hover a document and click <strong>+ tag</strong>, or use the tag strip while reading one. A slash makes a level: tag something <em>Finance/Taxes/2025</em> and it shows under <em>Finance</em>, under <em>Taxes</em> and under <em>2025</em> in the tree on the left. Click a tag chip to see everything carrying it, &times; to remove it, <strong>Rename tag</strong> to rename it everywhere. To make a child tag, hover a tag in the tree and click its <strong>+</strong> (or <strong>+ New</strong> at the top for a new top-level tag), name it, and it is selected and ready: <strong>drop files on it</strong> (or anywhere while it is selected) and they import already tagged, <strong>drag a document row onto it</strong>, or hover a row and click <strong>+ tag</strong> — the picker already starts with the selected tag&rsquo;s path. <em>Untagged</em> collects what still needs filing.</p>
                <h4 class="help-heading">Find, read, ask</h4>
                <p>The search box finds passages across everything (or inside the selected tag), grouped by document. Open one and it reads right there. Extracted text from a PDF or a scan is rarely pretty, so the reader <strong>tidies it with AI</strong> once — paragraphs rejoined, headings and tables restored, nothing summarized or added — and saves the result beside your files (a Markdown copy in the folder&rsquo;s hidden <code>.anjadhe/tidy</code>), so a document is tidied once, even on another Mac sharing the folder; the banner above the text says which view you are on, <strong>Show extracted text</strong> flips back (that view is where search terms are marked), and <strong>Open original</strong> opens the file itself. If your AI model can <em>see</em> (a local model with its vision file, or a vision-capable cloud model), a PDF or image is read from its page images instead — far more accurate than the text layer — and that transcription becomes the document&rsquo;s text for search and the assistant too; the first 25 pages read automatically, the rest on a click. A document that was parsed or OCR&rsquo;d before you had a vision model shows <strong>Re-read with vision</strong> in the same banner — one click reprocesses it from its pages. The tidy or read runs by itself when your AI model is on this Mac; with a model elsewhere it waits for you to click <strong>Tidy with AI</strong> unless Files may leave this Mac in Cloud privacy. Also here: <strong>Show in Finder</strong> and <strong>Ask about this document&hellip;</strong>, which opens the assistant already knowing what you&rsquo;re reading. In any chat: <em>"what does my lease say about pets?"</em>, <em>"find the receipt for the dishwasher"</em>, <em>"tag the last three statements Finance/Bank"</em>. Semantic (by-meaning) search needs a one-time ~330&nbsp;MB model download in <strong>Settings &rsaquo; Documents</strong>; until then it works by keywords. Cmd+K finds documents by title or tag.</p>
                <h4 class="help-heading">Private by default</h4>
                <p>Parsing, OCR and indexing always run locally, whatever AI brain you chose for chat. Only the passages the assistant retrieves for a question travel with that chat turn. Deleting a document moves the file to the Trash — recoverable, never a hard delete. Tags sync between your Macs; the index is rebuilt on each.</p>`
        },
        {
            id: 'voice', group: 'How it works', title: 'Your writing voice',
            blurb: 'Teach the assistant how you write, then ask for drafts in your voice.',
            body: `
                <p>nenva can learn how <em>you</em> write and draft in your voice when you ask. It lives in <strong>Settings &rsaquo; Writing voice</strong>: turn it on, pick what it may learn from, press <strong>Study</strong>. There is no separate app and no other people's voices — just yours.</p>
                <h4 class="help-heading">What it learns from</h4>
                <p>Each source is a switch: <strong>Documents</strong> you add (posts, essays, letters — <strong>Add documents&hellip;</strong> or drag files onto the page; <code>.md</code>, <code>.txt</code>, <code>.pdf</code>, <code>.docx</code>, <code>.html</code>), your <strong>Notes</strong>, your <strong>Journal</strong>, and your <strong>sent emails</strong> — only the parts you wrote; quoted replies, forwarded messages and signatures are stripped before anything is studied. Nothing is read until you press Study.</p>
                <h4 class="help-heading">Study</h4>
                <p>Study reads a spread sample of your selected sources on the AI model you chose and writes a short <em>style guide</em> — tone, rhythm, phrases, moves — plus real passages of your writing it will imitate. The guide is yours: edit it and your edits stick through re-studies; pin the best passages, remove the rest. <strong>Study depth</strong> on the same page sets how much one study reads.</p>
                <h4 class="help-heading">Drafting in it</h4>
                <p>Ask the assistant in any chat: <em>"write this in my voice"</em>, <em>"make it sound like me"</em>, <em>"draft a reply the way I would"</em>. It fetches your guide and passages first, then writes. <strong>Draft in my voice&hellip;</strong> on the settings page opens a chat already primed. Routines and other automatic writing use the assistant's own voice.</p>
                <h4 class="help-heading">Private by default</h4>
                <p>Your documents are indexed <em>on this Mac</em> by a small local model, whatever AI brain you use for chat. What travels with a draft is the style guide and a few passages — they go to the model you chose, and nowhere else. <strong>Turn off</strong> removes the guide and passages; your documents, notes, journal and emails all stay where they are.</p>`
        },
// ── The platform ─────────────────────────────────────────────
        {
            ...
            body: `
                <p>nenva is private by default. There is no remote database and no account. Your data is stored on this Mac at:</p>
        ...
        },
        {
            ...
            body: `
                <p>You can build your own nenva apps in a terminal with a coding agent — like Claude Code. They live in a plain folder on your Mac and nenva loads them automatically.</p>

                <h4 class="help-heading">Where apps live</h4>
                <p>Turn on <strong>Build Apps</strong> in <strong>Settings</strong>. nenva creates a folder in your home directory:</p>
                <ul class="help-app-list">
                    <li><code>~/nenva/apps/</code> — one subfolder per app. Each has a manifest, the app's code, and its own saved data.</li>
                    <li><code>~/nenva/apps/CLAUDE.md</code> and <code>AGENTS.md</code> — the full contract for building an app: the manifest format, the <code>nenva</code> SDK (storage, navigation, tools), and worked examples. Both files hold the same instructions; a coding agent reads whichever one it looks for.</li>
                    <li><code>~/nenva/apps/.anjadhe-schemas.json</code> — the shape of the built-in data (notes, goals, schedule, and so on) so an app can read it.</li>
                </ul>

                <h4 class="help-heading">Point a coding agent at it</h4>
                <p>Open the folder in your terminal and start your agent there:</p>
                <ul class="help-app-list">
                    <li><code>cd ~/nenva/apps</code></li>
                ...
                <p><em>"Build a reading tracker app for nenva. I want to add books with a title, author, and status (want to read / reading / finished), see them grouped by status, and mark one finished. Follow the manifest and SDK in CLAUDE.md, and save data with the nenva storage API."</em></p>

                <h4 class="help-heading">It updates live</h4>
                <p>nenva watches the apps folder. When the agent writes or changes a file, your app reloads on its own — no restart. A new app shows up as its own launcher tile next to the built-in ones.</p>
                <p>If an app has a problem, nenva writes the error to <code>.errors.log</code> inside that app's folder. Point your agent at that file and it can read the error and fix itself.</p>
