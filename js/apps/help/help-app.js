...
const HelpApp = {
    ...
    TOPICS: [
        // ── The framework ────────────────────────────────────────────
        {
            ...
            body: `
                <p>Actions has two tabs. <strong>Tasks</strong> is what to do now, and <strong>Email AI</strong> is everything the assistant found in your mail. Planning lives behind the <strong>Plan</strong> button on the Tasks page, which opens <strong>Goals</strong>. Tasks opens to <strong>Today</strong> — tasks due today, anything overdue, repeating tasks, action items pulled from email, and today's calendar events. When everything is checked off, it says so — "done for today" is a real, reachable state.</p>
                <h4 class="help-heading">Filter along two dimensions</h4>
                <p>The nav beside the list combines <strong>dates</strong> (Today, Tomorrow, This Week, This Month, Later, All time) with your <strong>goal groups</strong>. Pick one of each — say Today + Health — and the list narrows to tasks linked to that group's goals. Pills above the list show the active filters; the &times; on a pill clears just that one. Every view ends with a collapsed "N done" section of completed tasks.</p>
        ...
        },
        {
            ...
            blurb: 'Come up with goals, break them into tasks, and plan.',
            body: `
                <p>A goal is something you can finish: ship the release, run the marathon, hit the savings number. Each goal has what "done" looks like, a target date, a <strong>group</strong> label, and a list of linked tasks. A goal is either completed or it isn't — the progress lives in its tasks, and the n/m count on every goal shows it.</p>
                <h4 class="help-heading">Set a goal by talking</h4>
                <p>The <em>"Help me set a new goal"</em> button opens the assistant, which walks you through it one question at a time — what you want to accomplish, what done looks like, by when — then proposes the first steps as tasks with dates for you to approve. Stop mid-conversation and the goal is saved as a <strong>draft</strong> you can finish later. You can also add and edit goals by hand — every field on a goal's page saves as you change it.</p>
                <h4 class="help-heading">Groups</h4>
                <p>Goals are shown grouped by a plain <strong>Group</strong> field — Work, Health, Family, whatever you type. There is nothing to manage: a group exists because a goal carries its name. Reuse an existing name (the field suggests them), edit the field to move a goal, or drag a goal onto a group in the sidebar.</p>
                <h4 class="help-heading">Staying honest</h4>
                <p><strong>Review it weekly</strong> on a goal's page schedules an AI review: each week it reads the goal's linked tasks and posts a short honest read to your Home feed — moving, stalled, or done in all but name. Feeling stuck is a conversation, not a label: the <em>"I'm stuck"</em> button hands the goal to the assistant. On the Actions Today page, tasks show their goal as a small chip — click it to jump to the goal.</p>`
        },
        {
            ...
            body: `
                ...
                <p>Reminders before the due time, a work timer per task (the Pomodoro button in the header runs focus sessions against a task), a search box, and filters by goal.</p>`
        },
        ...
        {
            id: 'wellness', group: 'Everyday apps', title: 'Wellness',
            blurb: 'Track vitals, sleep, meals, water, workouts, and mood in one timeline.',
            body: `
                <p>Wellness is one place to log everything about your health: blood pressure and pulse, weight, blood glucose, SpO2, temperature, meals, water, activities with heart rate, sleep, mood, medications, and symptoms. Use the <strong>+ Log</strong> button (or ask the assistant: "log my blood pressure, 122 over 79").</p>
                <ul class="help-app-list">
        ...
        {
            id: 'browse', group: 'Everyday apps', title: 'Browse',
        ...
        {
            id: 'news', group: 'Everyday apps', title: 'News',
        ...
        {
            id: 'library', group: 'How it works', title: 'Writing Voices',
            blurb: 'Writing styles learned from documents — yours or anyone\u2019s — for drafts that sound right.',
            body: `
                <p>A <strong>writing voice</strong> is a way of writing, learned from documents: <em>My voice</em> from your own posts and essays, <em>Barack Obama</em> from his books and speeches. The app opens on your voices; each holds its own documents, an editable description of how it writes, and real passages it imitates.</p>
                <h4 class="help-heading">Create one</h4>
                <p>Name it in the box at the top. It gets its own folder on this Mac, and everything else happens on its page: <strong>Add documents&hellip;</strong> (or drag files onto the page) to teach it, then <strong>Study</strong> — the assistant reads a sample and writes a short <em>style guide</em> (tone, rhythm, phrases, moves) plus passages that carry the voice. The guide is yours: edit it and your edits stick through re-studies; pin the best passages. Supported files: <code>.md</code>, <code>.txt</code>, <code>.pdf</code>, <code>.docx</code>, <code>.html</code>.</p>
