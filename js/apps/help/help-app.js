...
const HelpApp = {
    ...
    TOPICS: [
        // ── The framework ────────────────────────────────────────────
        {
            ...
            body: `
                <p>Actions has two tabs. <strong>Tasks</strong> is what to do now; <strong>Plan</strong> is where you organize. Tasks opens to <strong>Today</strong> — tasks due today, anything overdue, repeating tasks, action items pulled from email, and today's calendar events. When everything is checked off, it says so — "done for today" is a real, reachable state.</p>
                <h4 class="help-heading">Filter along two dimensions</h4>
                <p>The nav beside the list combines <strong>dates</strong> (Today, Tomorrow, This Week, This Month, Later, All time) with <strong>focus areas</strong>. Pick one of each — say Today + Health — and the list narrows to both. Pills above the list show the active filters; the &times; on a pill clears just that one. Every view ends with a collapsed "N done" section of completed tasks.</p>
        ...
        },
        {
            ...
            body: `
                ...
                <p>Background runs post to the <strong>feed</strong> on your home page — each result shows when it ran and which model wrote it. Open a post to read it full-page; from there, <strong>Discuss with Assistant</strong> starts a chat that already has the result in context, so you can ask follow-ups immediately. Unread <strong>email insights</strong> (renewals, bills, appointments) appear in the same feed; the &times; on one marks it read.</p>
                <p>Anjadhe ships with a few starters already scheduled — <strong>Daily Briefing</strong> (your tasks and calendar each morning), <strong>News Digest</strong>, <strong>Daily Motivation</strong>, and <strong>Weekly Reflection</strong>. They're ordinary prompts: edit, reschedule, or delete them freely.</p>
...

        {
            ...
            body: `
                ...
                <ul class="help-app-list">
                    <li><strong>Topics</strong> — the Topics button opens the Discover topics dialog (shared with the Discover pane on Home). Pick from the suggestions or add your own; an optional location adds local news and puts today's weather on the Discover pane.</li>
                    ...
                    <li><strong>Show fewer like this</strong> — the &times; on any story teaches ranking; restore hidden stories from the Discover topics dialog. Boosted stories carry a "For you" label with the reason.</li>
                </ul>
                <p>News needs the Discover pane turned on (<strong>Settings → Appearance</strong>) and web search enabled (<strong>Settings → AI Assistant → Web Search</strong>). Clicks, hides, and ranking stay on this Mac.</p>`
        },
