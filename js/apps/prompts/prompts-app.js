...
const PromptsApp = {
    ...
    /** Studied Writing Voices, when the feature is available at all. */
    _voices() {
        if (typeof VoiceStore === 'undefined') return [];
        try { return VoiceStore.pages().filter(v => (v.body || '').trim()); } catch { return []; }
    },

    /** The configured voice's display name, or null (assistant's own voice). */
    _voiceName(cfg) {
        if (!cfg || !cfg.voiceId || typeof VoiceStore === 'undefined') return null;
        const v = VoiceStore.byId(cfg.voiceId);
        // A deleted voice degrades to the default at run time; say so here
        // rather than showing a dead name.
        return v ? v.name : null;
    },

    /**
     * The form's Voice row (digest only — a task run produces a log, not
     * prose). Rendered only when there is something to choose: no studied
     * voices and nothing configured means no row, not a dead dropdown.
     * "Assistant's own voice" is the default and what null means.
     */
    _voiceRowHtml(cfg) {
        const voices = this._voices();
        if (!voices.length && !cfg.voiceId) return '';
        const esc = UIUtils.escapeHtml;
        const known = voices.some(v => v.id === cfg.voiceId);
        return `
            <div class="routines-prop routines-voice-row" ${cfg.runMode === 'task' ? 'hidden' : ''}>
                <span class="routines-prop-label">Voice</span>
                <select id="prompt-mgr-voice" class="routines-voice-select" title="Which Writing Voice the posts are written in">
                    <option value="">Assistant's own voice</option>
                    ${voices.map(v => `<option value="${esc(v.id)}" ${v.id === cfg.voiceId ? 'selected' : ''}>${esc(v.name)}</option>`).join('')}
                    ${cfg.voiceId && !known ? `<option value="${esc(cfg.voiceId)}" selected>(deleted voice — runs use the assistant's own)</option>` : ''}
                </select>
            </div>`;
    },
    ...
    _renderDetail(container, note) {
        ...
        // What the routine draws on, as one quiet value line. A digest's
        // Writing Voice belongs here: it is part of what shapes the output.
        const voiceName = this._voiceName(cfg);
        const uses = [cfg.web ? 'Web search' : '', cfg.useContext ? 'My context' : '',
            voiceName ? `Voice: ${esc(voiceName)}` : '']
    ...
    _renderForm(container, note) {
        ...
        modeBtns.forEach(b => b.addEventListener('click', () => {
            ...
            // A voice styles written output; a task run produces a log.
            const voiceRow = container.querySelector('.routines-voice-row');
            if (voiceRow) voiceRow.hidden = isTask;
        ...
        container.querySelector('.prompt-mgr-save')?.addEventListener('click', () => {
            ...
            const config = {
                ...
                voiceId: runMode === 'digest'
                    ? (container.querySelector('#prompt-mgr-voice')?.value || null) : null,
