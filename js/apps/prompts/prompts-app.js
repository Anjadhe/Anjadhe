...
// AgentContext provider — the routine open in the detail view. Routines had
// no provider at all before the Decisions feature (2026-08-06), which meant
// no CURRENT ROUTINE block and no per-record conversation reattach; the
// recordKey ('prompts:<noteId>') is also what routes a routine's saved
// decisions into the ambient context (DecisionStore.fromRecordKey).
if (typeof AgentContext !== 'undefined') {
    AgentContext.register('prompts', () => {
        if (PromptsApp._view !== 'detail' || !PromptsApp._openId) return null;
        const note = PromptsApp._note(PromptsApp._openId);
        if (!note) return null;
        const cfg = NotePrompts.config(note);
        const body = NotePrompts.bodyText(note) || '';
        return {
            ...
            body: `The user is viewing this routine (id: ${note.id}):\n` +
