...
const PromptFeed = {
     ...
     * results, email insights, and whether read items (prompt results and
     * insights alike) stay hidden.
    ...
    openPrefs() {
        ...
        const modal = Modal.create({
            ...
            content:
                ...
                + row('hideRead', 'Hide read items', 'Hide prompt results and email insights you&rsquo;ve already read.', p.hideRead),
    ...
    _filteredItems() {
        ...
        // Read prompt posts leave the feed when the preference says so —
        // error cards are exempt (they can't be opened, so they can never
        // be read; only × removes them).
        if (prefs.hideRead) cards = cards.filter(c => !c.read);
        return [...this._publishedModels(), ...insights, ...cards]
            .sort((a, b) =>
    ...
    _renderCard(it) {
        ...
        return `
            <article class="feed-card${it.published ? '' : ' feed-card--prompt'}${it.read ? ' feed-card--read' : ''}" data-feed-open="${it.id}">${head}
