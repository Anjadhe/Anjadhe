...
const PromptFeed = {
    ...
    async _generateWithWeb(prompt, model) {
        ...
        for (let i = 0; i < MAX_ITERS; i++) {
            if (this._current && this._current.cancelled) {
                return { content: '', error: `Stopped by you after ${Math.round((Date.now() - startedAt) / 60000)} min`, stoppedByUser: true, model: lastModel };
            }
            // On the final allowed iteration — or once the run's clock is
            // spent — drop tools so the model is forced to produce a textual
            // answer instead of another call.
            const allowTools = i < MAX_ITERS - 1 && (Date.now() - startedAt) < this.RUN_BUDGET_MS;
            const res = await LLMLogger.call('prompt-feed', {
                model,
                activitySubject: prompt.title || null,
                messages,
     ...
     * each post published by Anjadhe. Newest edition first within a series;
    ...
    /* ── Colour as data (2026-09-09, by request): one stable hue per
       ...
       before its title is read. Published Anjadhe posts stay neutral. ── */
    ...
    // What kind of routine wrote an item: 'published' (Anjadhe's own
    ...
    KIND_LABELS: { report: 'Routine', task: 'Routine \u00b7 takes actions', published: 'From Anjadhe' },
    ...
    _briefClauses(items) {
        ...
        for (const it of items) {
            if (it.published) continue;                       // Anjadhe's own posts are not "what my routines did"
    ...
    // name, the path is noise on a feed card. Anjadhe Cloud ids get their
    ...
    _displayModel(model) {
        ...
        if (/^anjadhe-cloud/.test(model)) {
            try { return AgentService.anjadheEntryLabel(model); } catch { return 'Anjadhe Cloud'; }
        }
