...
const PromptFeed = {
    ...
    // next scheduled run, which is the expected behaviour. Returns what the
    // run returned (the posted feed note on a digest that posted), or
    // nothing when the click was queued or there was nothing to run.
    ...
    _ensureOverlay() {
            ...
            <article class="feed-post-main">
                <header class="feed-post-header">
                    ...
                    <div class="feed-post-meta">
                        ...
                        <button class="feed-post-prompt-link feed-post-run" type="button" hidden
                                title="Run this routine again now">Run now</button>
                    </div>
        ...
        // Back is one rung at a time: with the routine open over the post it
        // returns to the result, the same ladder Escape climbs.
        ov.querySelector('.feed-post-back').addEventListener('click', () => {
            if (this.isRoutinePanelOpen()) this.closeRoutinePanel();
            else this.closePost();
        });
        ...
        ov.querySelector('.feed-post-run').addEventListener('click', () => this._runFromPost());
    ...
    openPost(id, { back = 'Feed' } = {}) {
        ...
        const runBtn = ov.querySelector('.feed-post-run');
        runBtn.hidden = !promptNote || !NotePrompts.bodyText(promptNote).trim();
        this._paintRunButton();
    ...
    /**
     * "Run now" from the result itself — the same two paths the Routines
     * page's Run now takes (PromptsApp._runNow): an offline routine runs
     * here and, when it posts, the page turns to the new result; any other
     * routine hands off to the assistant or the browser, so the post closes
     * first rather than sit over the answer.
     */
    async _runFromPost() {
        const ov = this._overlay;
        const pid = ov && ov._promptId;
        const note = pid ? NotePrompts.list().find(p => p.id === pid) : null;
        if (!note || this._runningFromPost) return;
        if (!NotePrompts.config(note).offline) {
            this.closePost();
            NotePrompts.runDefault(note);
            return;
        }
        const back = ov.querySelector('.feed-post-back span').textContent;
        this._runningFromPost = pid;
        this._paintRunButton();
        let res;
        try { res = await this.runNow(pid); }
        finally { this._runningFromPost = null; }
        if (typeof PromptsApp !== 'undefined' && PromptsApp.render) PromptsApp.render();
        // Still reading this routine's result? Show the one just written.
        if (!ov.hidden && ov._promptId === pid && res && res.id && this._item(res.id)) {
            this.openPost(res.id, { back });
        } else {
            this._paintRunButton();
        }
    },
    ...
    _paintRunButton() {
        const btn = this._overlay && this._overlay.querySelector('.feed-post-run');
        if (!btn) return;
        const running = !!this._runningFromPost && this._runningFromPost === this._overlay._promptId;
    ...
    openRoutinePanel(promptId) {
        ...
        // The routine REPLACES the result on screen (2026-09-23). Stacked
        // under the post it read as a second page glued to the bottom of the
        // first — you opened the routine and landed below the response. The
        // post is hidden, not torn down, so "Back to the result" puts it
        // back exactly where the reader left it.
        if (!ov.classList.contains('is-routine-open')) ov._postScrollTop = ov.scrollTop;
        panel.hidden = false;
    ...
    closeRoutinePanel(opts = {}) {
        ...
        const wasOpen = ov.classList.contains('is-routine-open');
        ov.classList.remove('is-routine-open');
        if (wasOpen && !ov.hidden) ov.scrollTop = ov._postScrollTop || 0;
