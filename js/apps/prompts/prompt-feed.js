...
const PromptFeed = {
    ...
    async _runPrompt(prompt, context = null) {
        ...
        // Engine-driven run + transient failure + attempts left → hand the
        // item back to the queue instead of posting an error card the user
        // has to read. The routine's "Last problem" line carries the story
        // while it waits; a success clears it (clearError above).
        if (error && this._isTransient(error)
            && context && context.attempts != null && context.attempts < this.RETRY_MAX) {
    ...
    _isTransient(error) {
        return /empty response|socket hang up|ECONNREFUSED|ECONNRESET|ETIMEDOUT|EPIPE|timed out|timeout|premature close|fetch failed|network error|temporarily unavailable|overloaded|too many requests|\b(429|502|503|504)\b/i
            .test(String(error || ''));
    },
    ...
    _friendlyError(error) {
        ...
        return `${reason} It will try again at its next scheduled time — or run it now from the Routines page.`;
    },
