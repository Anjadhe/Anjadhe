 ...
 * stored content (posts stay out of the Notes lists unless pinned/on-home).
 *
...
const PromptFeed = {
    ...
    init() {
        ...
        this.startScheduler();
        // Feed-header entry points route to the Prompts page (the standalone
        // home of prompt notes; the old in-place modal manager is retired).
        // The dashboard markup is static, so wiring once here is safe.
        document.getElementById('prompt-feed-new')
            ?.addEventListener('click', () => PromptsApp.open({ create: true }));
    ...
    async _ensureEmailData() {
        ...
        if (!EmailApp.accounts.length && !EmailApp.emails.length) {
    ...
    _insightModels() {
        ...
        return rows.slice(0, this.INSIGHT_MAX).map(({ email, a }) => ({
            ...
            html: this._insightHtml(email, a),
            error: null,
            model: null,
    ...
    _insightHtml(email, a) {
        const esc = UIUtils.escapeHtml;
        const from = (typeof EmailUI !== 'undefined' && EmailUI.extractName)
            ? EmailUI.extractName(email.from)
            : (email.from || '');
        let html = `<p class="feed-insight-from">${esc(from)}</p>`;
        if (a.summary) html += `<p>${esc(a.summary)}</p>`;
        const items = Array.isArray(a.actionItems) ? a.actionItems.filter(it => it && it.text) : [];
        if (items.length) {
            html += `<ul>${items.slice(0, 3).map(it =>
                `<li>${esc(it.text)}${it.dueDate ? ' &mdash; ' + esc(it.dueDate) : ''}</li>`).join('')}</ul>`;
        }
        return html;
    },
    ...
    _renderCard(it) {
        ...
        const delTitle = it.published ? 'Dismiss' : it.insight ? 'Mark read' : 'Remove';
        // A read insight (visible when "hide read" is off) has nothing left
        // to dismiss — no × for it.
        const delBtn = (it.insight && it.read) ? ''
            : `<button class="feed-card-del" type="button" data-feed-del="${it.id}" title="${delTitle}">&times;</button>`;
        const head = `
                <header class="feed-card-head">
                    ${it.published ? '<span class="feed-card-source">Anjadhe</span>' : ''}
                    ${it.insight ? '<span class="feed-card-source">Email</span>' : ''}
        ...
        return `
            <article class="feed-card${it.insight && it.read ? ' feed-card--read' : ''}" data-feed-open="${it.id}">${head}
