...
const FyiPage = {
    ...
    init() {
        ...
        NavResizer.attach({
            ...
            defaultW: 340,
            min: 240,
            // Raised from 620 (2026-08-21): the reading pane's content caps
            // at 62ch, so a wide monitor can afford a much wider list —
            // the CSS default is responsive now (clamp to ~38vw) and the
            // drag ceiling should not undercut it.
            max: 900,
            // Until the user drags, the stylesheet's clamp() default sizes
            // the pane (NavResizer writes nothing); a first drag starts
            // from the pane's real rendered width.
            responsiveDefault: true,
            measureSel: '#fyi-view .fyi-list-pane',
        });
    },
    ...
    // The Unread/All choice is a PREFERENCE, not view state (2026-09-22):
    // kept per-Mac in localStorage so it survives quitting the app, the way
    // the Inbox keeps its own unread-only toggle per machine. It used to
    // ride the sessionStorage state above and reset on every launch.
    _unreadPrefKey: 'fyi-unread-only',

    _restoreUnreadPref() {
        try { this._unreadOnly = window.localStorage.getItem(this._unreadPrefKey) === '1'; } catch (_) {}
    },

    _saveUnreadPref() {
        try { window.localStorage.setItem(this._unreadPrefKey, this._unreadOnly ? '1' : '0'); } catch (_) {}
    },

    _restoreViewState() {
        this._restoreUnreadPref();
        try {
    ...
    /**
     * The Accounts nav's scopes: every connected Gmail account plus every
     * source that is on, each keyed by what its records carry in
     * `email.account` ('imessage' for texts). One list so the nav, the
     * scope filter and the stale-scope reset cannot disagree.
     */
    _scopes() {
        const accounts = (typeof EmailApp !== 'undefined' && EmailApp.getAccounts)
            ? EmailApp.getAccounts() : [];
        return [
            ...accounts.map(a => ({ key: a.email, label: a.email })),
            ...this._sourcesOn().map(src => ({ key: src.account, label: src.label || src.account })),
        ];
    },
    ...
    _busyEmptyMessage() {
        ...
        const explain = 'Bills, receipts, renewals, bookings and deliveries land here as each one is read; anything that needs doing also shows up under Tasks.';
        let queued = 0;
        try { queued = EmailApp.analysisBacklogStatus().queued; } catch { /* blob unreadable */ }
        if (queued || EmailApp.isAnalyzing) {
            return `Your AI is reading your email now${queued ? ` &mdash; ${queued} message${queued === 1 ? '' : 's'} in the queue` : ''}. ${explain}`;
        }
        if (EmailApp.isSyncing) {
            return `Pulling your latest mail from Google (the newest 50 per account to start)&hellip; your AI reads each message as it arrives. ${explain}`;
        }
        return null;
    },
    ...
    // docked by default on a big monitor (off by default on a laptop-sized
    // display — UIUtils.readerPaneDefault, 2026-09-02); turning it off
    // gives the rows the full width, and
    ...
    _readerPref() {
        ...
        try {
            const v = localStorage.getItem('fyi-reader-pane');
            if (v === '0') return false;
            if (v === '1') return true;
        } catch { /* private mode */ }
    ...
    render() {
        ...
        // A scope whose account was disconnected (or whose source was
        // switched off) would show an empty page with no way to tell why.
        if (this._account && !this._scopes().some(sc => sc.key === this._account)) {
            this._account = null;
        ...
        if (!groups.size) {
            // Work in flight (initial sync, the analysis backlog) means the
            // honest blank state is a status line, not "nothing here".
            ...
            if (this._account) {
                ...
                this._renderNav(nav, groups, rows);
                list.innerHTML = '';
                main.innerHTML = `<div class="fyi-empty">${busy || `Nothing from ${UIUtils.escapeHtml(this._account)} yet — insights land here as your AI reads its mail.`}</div>`;
                return;
            }
            empty(busy || `Nothing here yet. Bills, receipts, renewals, bookings and deliveries land here as your AI reads new email; anything that needs doing also shows up under Tasks.`);
            return;
        }
    ...
    _renderOverview(main, groups) {
        ...
        const rowHtml = (x) => {
            ...
            return `
                <div class="fyi-ov-row${x.r.a.readAt ? '' : ' is-unread'}" role="button" tabindex="0" data-fyi-goto="${esc(x.r.id)}" data-ins-type="${esc(x.type)}">
                    ...
                    <span class="fyi-ov-tag" data-ins-type="${esc(x.type)}">${esc(this._label(x.type))}</span>
        ...
        if (!upcoming.length && !fresh.length) {
            html += `<div class="fyi-empty">All caught up — nothing coming up and nothing unread. The folders on the left hold everything your AI has found in your mail.</div>`;
        }
        html += '</div>';
        main.innerHTML = html;
    },
    ...
    _renderTripsIndex(main) {
        ...
        if (!all.length) {
            html += `<div class="fyi-empty">No trips yet. Book a flight, hotel or table and the confirmations will gather here as your AI reads your email.</div>`;
        }
        html += '</div>';
        main.innerHTML = html;
    },
    ...
    _gateMessage() {
        ...
        if (!this._accountsExist()) {
            ...
            return UIUtils.appWelcome({
                ...
                lede: 'Connect Gmail — or turn on iMessage under Settings › Accounts — and your AI reads every incoming email and text, files what matters, and turns what needs doing into tasks — so the mail you have to keep in your head gets close to zero.',
                cta: '<button type="button" class="primary-btn" data-fyi-manage-accounts>Connect an account</button>',
                rows: [
                    ['Folders that file themselves',
                     'Bills, receipts, renewals, appointments, reservations, deliveries, deadlines, sign-in codes — each finding lands in one predictable folder, and marketing never gets in.'],
                    ['What needs doing becomes a task',
                     'Action items found in your mail land on your Tasks list with a link back to the message; the insight stays here, so what your mail said is always findable.'],
                    ['Trips assemble themselves',
                     'Flight, hotel and car reservations cluster into trips — one card with every leg, built as the bookings arrive.'],
                    ['A briefing, not a feed',
                     'The overview shows what is coming up, by each event&rsquo;s own date, and what is new since you last looked. Everything read files quietly into its folder.']
                ]
            });
        }
        ...
        for (const type of this._typeOrder()) {
            ...
            html += `
                <button type="button" class="actions-nav-item${type === this._type ? ' is-active' : ''}${list.length ? '' : ' is-quiet'}"
                        data-fyi-type="${UIUtils.escapeHtml(type)}" data-ins-type="${UIUtils.escapeHtml(type)}">
                    <span class="fyi-nav-dot" aria-hidden="true"></span>
    ...
    _accountsNavHtml(rows) {
        ...
        if (scopes.length === 1) {
            html += `<div class="fyi-account-static" title="${esc(scopes[0].label)}">${esc(scopes[0].label)}</div>`;
        } else {
            ...
            html += rowHtml('', 'All accounts') + scopes.map(sc => rowHtml(sc.key, sc.label)).join('');
        }
        html += '</div>';
    ...
    _renderList(pane, list) {
        ...
        let html = `
            <div class="fyi-head" data-ins-type="${UIUtils.escapeHtml(this._type)}">
                <h3 class="fyi-title"><span class="fyi-nav-dot" aria-hidden="true"></span>${UIUtils.escapeHtml(this._label(this._type))}</h3>
            </div>
    ...
    _rowHtml(r, opts = {}) {
        ...
        const typeKey = this._typeOrder().includes(r.a.type) ? r.a.type : 'general';
        const tag = opts.tag ? this._label(typeKey) : '';
        // A text wears its source on the metadata line: the same folder
        // holds mail and conversations, and a phone number under a summary
        // does not say which.
        const srcTag = this._isSourceRow(r) ? 'iMessage' : '';
        return `
            <div class="fyi-row${r.a.readAt ? '' : ' is-unread'}${cancelled ? ' is-cancelled' : ''}${r.id === this._openId ? ' is-selected' : ''}"
                    role="button" tabindex="0" data-ins-type="${UIUtils.escapeHtml(typeKey)}"
                ...
                <span class="fyi-row-body">
                    ...
                    <span class="fyi-row-sub">
                        ...
                        ${srcTag ? `<span class="fyi-row-tag fyi-row-tag-src">${srcTag}</span>` : ''}
     ...
     *
     * The email itself renders below the insight — the insight organizes
     * what was found, and the source is right there to check it against
     * (same sanitized-iframe treatment as the Inbox viewer, via
     * EmailUI.renderEmailBodyInto). "Open email" still goes to the Inbox,
     * where reply/archive/labels live.
     */
    _renderDetail(main, row) {
        ...
        const isText = this._isSourceRow(row);
        push(isText ? 'Chat' : 'From', isText ? this._chatLabel(email) : EmailUI.extractName(email.from));
        if (received) push('Received', received.toLocaleString([], { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }));
        ...
        // trust it. The shared .back-link (components.css) — the one control
        // the Inbox viewer and the News reader also use.
        ...
        const back = showBack
            ? `<button type="button" class="back-link fyi-detail-back" data-fyi-back>&#8592; ${esc(backLabel)}</button>`
            : '';

        const typeKey = this._typeOrder().includes(a.type) ? a.type : 'general';
        let html = `
            <div class="fyi-detail" data-ins-type="${esc(typeKey)}">
                ${back}
                <article class="fyi-detail-page">
                    <header class="fyi-detail-head">
                        <span class="fyi-detail-kind" aria-hidden="true">${EmailUI.insightIcon(a, 20)}</span>
                        <div class="fyi-detail-headtext">
        ...
        html += `
                    ...
                    <div class="fyi-detail-footer">
                        <button type="button" class="fyi-detail-btn" data-fyi-open-email="${esc(row.id)}">${isText ? 'Open in Messages' : 'Open email'} &#8594;</button>
                    ...
                    <div class="fyi-detail-email${isText ? ' is-conversation' : ''}">
                        <div class="fyi-detail-email-head">
                            <div class="fyi-detail-section-label">${isText ? 'Conversation' : 'Email'}</div>
                            <span class="fyi-detail-email-meta"${email.from ? ` title="${esc(email.from)}"` : ''}><strong>${esc(isText ? this._chatLabel(email) : (EmailUI.extractName(email.from) || ''))}</strong>${received ? ` &middot; ${esc(received.toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' }))}` : ''}</span>
                        </div>
                        <div class="fyi-detail-email-body" data-fyi-email-body>${isText
                            ? (typeof IMessageSource !== 'undefined' ? IMessageSource.transcriptHtml(email) : esc(email.bodyText || ''))
                            : esc(email.snippet || 'Loading…')}</div>
                    </div>
                </article>
            </div>`;
    ...
    _wireDetail(main, row) {
        ...
        // The message itself, below the insight. Bodies live in a separate
        // table and are fetched on demand; the snippet stands in until the
        // fetch resolves. Guard on _openRow — selecting another insight
        // mid-fetch must not paint the old body into the new detail.
        const bodyEl = main.querySelector('[data-fyi-email-body]');
        // A conversation is rendered in place above (its lines ARE the
        // record); only mail has a body to fetch.
        if (bodyEl && !this._isSourceRow(row)) {
            EmailApp._ensureBody(row.email).then(() => {
                if (this._openRow !== row || !bodyEl.isConnected) return;
                EmailUI.renderEmailBodyInto(bodyEl, row.email);
            });
        }
    ...
    _rowMenu(id, at) {
        ...
        const items = [
            ...
            { label: EmailApp.sourceRecordById?.(id) ? 'Open in Messages' : 'Open email', act: () => this._openEmail(id) },
        ];
