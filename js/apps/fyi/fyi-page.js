...
const FyiPage = {
    ...
    _renderOverview(main, groups) {
        ...
        const rowHtml = (x) => {
            ...
            return `
                <button type="button" class="fyi-ov-row${x.r.a.readAt ? '' : ' is-unread'}" data-fyi-goto="${esc(x.r.id)}">
    ...
    _renderDetail(main, row) {
        ...
        const showBack = searching || this._type ? !this._readerPref() : true;
        const backLabel = searching ? 'Results' : this._type ? this._label(this._type) : 'Overview';
        const back = showBack
    ...
    _wire() {
        ...
        view.addEventListener('click', (e) => {
            ...
            if (typeBtn) {
                ...
                // navigation, search is a mode: choosing a destination ends
                // the search rather than silently filtering inside it.
                this._clearSearch();
                this._openId = null;
        ...
        // Escape walks back out: detail → search → folder → overview. Same
        // shape as the Tasks tab's inline task, with the page's home as the
        // floor. (Esc INSIDE the search input is handled on the input.)
