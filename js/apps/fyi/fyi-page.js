...
const FyiPage = {
    ...
    _gateMessage() {
        ...
        if (!this._accountsExist()) {
            ...
            return UIUtils.appWelcome({
                ...
                lede: 'Connect Gmail and your AI reads every incoming email, files what matters, and turns what needs doing into tasks — so the mail you have to keep in your head gets close to zero.',
    ...
    _renderDetail(main, row) {
        ...
        html += `
                    ...
                    <div class="fyi-detail-email">
                        <div class="fyi-detail-email-head">
                            <div class="fyi-detail-section-label">Email</div>
                            <span class="fyi-detail-email-meta"${email.from ? ` title="${esc(email.from)}"` : ''}><strong>${esc(EmailUI.extractName(email.from) || '')}</strong>${received ? ` &middot; ${esc(received.toLocaleDateString([], { month: 'short', day: 'numeric', year: 'numeric' }))}` : ''}</span>
                        </div>
                        <div class="fyi-detail-email-body" data-fyi-email-body>${esc(email.snippet || 'Loading…')}</div>
                    </div>
                </article>
            </div>`;
