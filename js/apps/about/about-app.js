    ...
    showAlphaInfo() {
        const modal = Modal.create({
            ...
            content: `
                ...
                <p>
                    <strong>What it costs.</strong> Free during alpha. The plan is a one-time license later, with a year
                    of updates and no account; the price is not set yet &mdash; and everyone who installs during the alpha
                    keeps the app free for good. Hosted extras like web search and Anjadhe Cloud models keep a free monthly allowance;
                    only usage beyond it would ever be paid, and that stays optional.
                </p>
                <p>
                    <strong>Put it on record.</strong> <a href="#" id="about-alpha-license-link">Claim your alpha license</a>
                    in Settings &rsaquo; License: a small signed key, saved on this Mac, that keeps you on every future release
                    once the alpha closes. It asks for an email address only so the same key can be issued to you again.
                </p>`,
        });
        modal?.body?.querySelector('#about-alpha-license-link')?.addEventListener('click', (e) => {
            e.preventDefault();
            modal.close?.();
            // Unclaimed while the alpha is open → the claim modal itself;
            // otherwise the Settings page that shows the facts.
            if (typeof LicenseClaim !== 'undefined' && LicenseClaim.stepApplies()) LicenseClaim.openModal({ source: 'about' });
            else if (typeof HelpActions !== 'undefined') HelpActions.open('license');
            else { AppManager.openApp('settings'); SettingsApp?.openCategory?.('license'); }
        });
    }
};
