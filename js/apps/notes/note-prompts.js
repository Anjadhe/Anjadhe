...
const NotePrompts = {
    DEFAULTS: {
        ...
        // Writing Voice for digest output (2026-08-08). See config().
        voiceId: null
    },
    ...
    config(note) {
        ...
        return {
            ...
            // Which Writing Voice a DIGEST's post is written in (a VoiceStore
            // record id — ids, never names: renames must not orphan). Null =
            // the assistant's own voice, the default. Style only; task-mode
            // runs produce logs, not prose, and ignore it.
            voiceId: typeof p.voiceId === 'string' && p.voiceId ? p.voiceId : null
        };
    },
