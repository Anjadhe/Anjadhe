...
const NotesUI = {
    ...
    renderFilters(notes, currentFilter) {
        ...
        // Prompt notes and feed posts live on the Prompts page / Home feed,
        // not in the notes lists — keep them out of the All count.
        const hiddenCount = notes.filter(n => ['prompt', 'feed'].includes(NoteTemplates.resolve(n))).length;
    ...
    renderTagFilters(notes, tags, currentFilter) {
        ...
        // Prompt notes and feed posts never show in the tag lists (see
        // NotesApp.getFilteredNotes), so keep them out of every count.
        const visible = notes.filter(n => !['prompt', 'feed'].includes(NoteTemplates.resolve(n)));
        const tagCounts = {};
        ...
        if (searchEl && !searchEl.dataset.bound) {
            ...
            searchEl.addEventListener('input', () => {
                this.renderTagFilters(
                    ProfileManager.filterByActiveProfile(NotesApp.notes),
