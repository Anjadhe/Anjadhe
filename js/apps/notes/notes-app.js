...
const NotesApp = {
    ...
    // The sidebar filter when it is an actual tag (not All/Pinned/On Home/
    ...
    _activeTagFilter() {
        ...
        if (!f || ['all', 'pinned', 'on-home', 'assistant', 'untagged'].includes(f)) return null;
    ...
    getFilteredNotes() {
        ...
        // buckets both would only bury real notes. Only explicit user flags
        // (pinned / on-home) surface them here.
        if (!['pinned', 'on-home'].includes(this.currentFilter)) {
