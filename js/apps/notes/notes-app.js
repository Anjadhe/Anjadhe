...
const NotesApp = {
    ...
    // Individually collapsible panes (2026-08-09). The stored preference is
    // per-Mac (localStorage, the side-nav precedent); a deep-linked open
    // (opts.focus on openEditor — goals page rows, assistant pills, record
    // links) collapses both TRANSIENTLY, without touching the preference,
    // so the next normal visit to Notes comes back as the user left it.
    PANES: {
        nav: { cls: 'notes-nav-collapsed', btn: 'notes-toggle-nav-btn', store: 'notes-nav-collapsed', label: 'filters' },
        list: { cls: 'notes-list-collapsed', btn: 'notes-toggle-list-btn', store: 'notes-list-collapsed', label: 'note list' }
    },

    setPaneCollapsed(pane, collapsed, opts = {}) {
        const def = this.PANES[pane];
        if (!def) return;
        document.getElementById('notes-view')?.classList.toggle(def.cls, !!collapsed);
        if (!opts.transient) {
            try { localStorage.setItem(def.store, collapsed ? '1' : '0'); } catch { /* private mode */ }
        }
        this._syncPaneToggles();
        // The list column's "← All notes" chip exists exactly when the
        // filters sidebar is hidden — collapsing/expanding changes that.
        if (typeof NotesUI !== 'undefined') NotesUI.renderFilterCrumb();
    },

    _paneCollapsed(pane) {
        const def = this.PANES[pane];
        return !!def && !!document.getElementById('notes-view')?.classList.contains(def.cls);
    },

    /** Re-assert the per-Mac preference (clears any transient deep-link fold). */
    _applyStoredPaneState() {
        for (const pane of Object.keys(this.PANES)) {
            let stored = null;
            try { stored = localStorage.getItem(this.PANES[pane].store); } catch { /* private mode */ }
            this.setPaneCollapsed(pane, stored === '1', { transient: true });
        }
    },
    ...
    _syncPaneToggles() {
        for (const pane of Object.keys(this.PANES)) {
            ...
            if (!btn) continue;
            const collapsed = this._paneCollapsed(pane);
            btn.classList.toggle('is-active', collapsed);
            btn.setAttribute('aria-pressed', String(collapsed));
            btn.title = `${collapsed ? 'Show' : 'Hide'} ${def.label}`;
        }
    },

    // ── Reading pane preference ──────────────────────────────────────────
    //
    // The Inbox / Email AI control, carried to Notes (2026-08-10): the note
    // pane is docked beside the list by default on a big monitor (off by
    // default on a laptop-sized display — UIUtils.readerPaneDefault,
    // 2026-09-02); turning it off gives the
    // list the full width, and opening a note then swaps to a full-width
    // note with a way back above it. Per-Mac localStorage like the pane
    // collapse state — pane layout depends on the monitor, so it must not
    // ride the sync journal. Under the 981px stacked layout the preference
    // is moot (the CSS is scoped to the wide frame) and the toggle hides.

    READER_KEY: 'notes-reader-pane',

    _readerPref() {
        // Unset means "the display's default" (UIUtils.readerPaneDefault:
        // off on a laptop-sized screen); '0'/'1' is the user's own choice.
        try {
            const v = localStorage.getItem(this.READER_KEY);
            if (v === '0') return false;
            if (v === '1') return true;
        } catch { /* private mode */ }
        return UIUtils.readerPaneDefault();
    },

    toggleReaderPane() {
        const turningOff = this._readerPref();
        try { localStorage.setItem(this.READER_KEY, turningOff ? '0' : '1'); } catch { /* private mode */ }
        // The FYI guard, carried over: hiding the pane while a note is open
        // would answer "no pane" with a full-width note — drop back to the
        // list instead.
        if (turningOff && this._paneSessionActive) {
            this.flushSave();
            this._editorOrigin = null;
            this._clearSelection();
        }
        this._syncReaderState();
    },

    /** Display name of the list the current filter shows — what the
     *  full-width note's back chip promises to return to. */
    _listLabel() {
        const f = this.currentFilter;
        if (f === 'all') return 'All notes';
        if (f === 'pinned') return 'Pinned';
        if (f === 'assistant') return 'AI Assistant';
        if (f === 'untagged') return 'Untagged';
        return f; // a tag name
    },

    /** Reflect the preference and the open-note state on the view + toggle.
     *  `notes-reader-off` widens the list; `notes-reading` (pane off AND a
     *  note session live — a brand-new draft counts) swaps to the note. */
    _syncReaderState() {
        const view = document.getElementById('notes-view');
        if (!view) return;
        const off = !this._readerPref();
        view.classList.toggle('notes-reader-off', off);
        view.classList.toggle('notes-reading', off && !!this._paneSessionActive);
        const btn = document.getElementById('notes-reader-toggle');
        if (btn) {
            // Same convention as the pane toggles beside it: pressed while
            // the pane is hidden.
            btn.classList.toggle('is-active', off);
            btn.setAttribute('aria-pressed', String(off));
            btn.title = `${off ? 'Show' : 'Hide'} reading pane`;
        }
        // The back chip names the list it returns to — "← Pinned",
        // "← work" — not a fixed "All notes" (the FYI back row's rule).
        const back = document.getElementById('notes-reader-back');
        if (back) back.innerHTML = `&#8592; ${UIUtils.escapeHtml(this._listLabel())}`;
    },
    ...
    init() {
        ...
        // Entering the app proper restores the user's own pane preference —
        // which also clears any transient deep-link fold from a prior visit.
        this._applyStoredPaneState();
        this._syncReaderState();
    ...
    // The sidebar filter when it is an actual tag (not All/Pinned/
    // AI Assistant/Untagged). Notes created inside a tag-filtered list
    // inherit that tag — otherwise the new note wouldn't match the very
    // list it was created from and would "vanish" on save.
    _activeTagFilter() {
        const f = this.currentFilter;
        if (!f || ['all', 'pinned', 'assistant', 'untagged'].includes(f)) return null;
        return this.activeProfileTags().some(t => t.name === f) ? f : null;
    },
    ...
    migratePromptsToNotes() {
        ...
        oldTags.forEach(t => {
            ...
            this.tags.push({ id: UIUtils.generateId(), name: t.name, createdAt: t.createdAt || new Date().toISOString() });
     ...
     */
    saveNotes(tombstones = null) {
        // tombstones: {id: deletedAtISO} for notes REMOVED by this save —
        // sync merges notes per record, so an undeclared removal resurrects
        // from the other Mac's copy (see NotePrompts._writeNotes).
        StorageManager.set('notes', tombstones ? { notes: this.notes, tombstones } : { notes: this.notes });
    ...
    createNote(title = '', body = '') {
        ...
        const inheritedTag = this._activeTagFilter();
        const newNote = {
            id: UIUtils.generateId(),
            title: title || 'Untitled',
            content,
            tags: inheritedTag ? [inheritedTag] : [],
            template: 'blank',
            bookNumbered: true,
            bookLayout: 'scroll',
            pinned: false,
     ...
     */
    setupEventListeners() {
        // Add note button — opens a blank note; the editor's "Template: …"
        // control switches the template in place when a different one is
        // wanted.
        ...
        // Pane collapse toggles — filters sidebar and note list, individually.
        for (const pane of Object.keys(this.PANES)) {
            const btn = document.getElementById(this.PANES[pane].btn);
            if (!btn) continue;
            const newBtn = btn.cloneNode(true);
            btn.parentNode.replaceChild(newBtn, btn);
            newBtn.addEventListener('click', () => {
                this.setPaneCollapsed(pane, !this._paneCollapsed(pane));
            });
        }
        ...
        // Pane header buttons: pin, PDF export, delete.
        ...
        // Reading-pane toggle + the full-width note's way back. The back
        // chip deliberately ignores a deep-link origin — its label promises
        // the list, so it always lands there.
        replaceBtn('notes-reader-toggle', () => this.toggleReaderPane());
        replaceBtn('notes-reader-back', () => {
            this.flushSave();
            this._editorOrigin = null;
            this._clearSelection();
        });
            ...
            const relatedRow = (n) => `
                <div class="note-viewer-link-item" data-note-id="${n.id}">
                    ...
                    <span class="note-conn-tags">${(n._sharedTags || []).map(t =>
                        `<span class="note-conn-tag" data-tag="${UIUtils.escapeHtml(t)}" title="Show notes tagged &quot;${UIUtils.escapeHtml(t)}&quot;">#${UIUtils.escapeHtml(t)}</span>`).join(' ')}</span>
                    <span class="note-conn-date">${UIUtils.formatDate(n.modifiedAt)}</span>
                </div>`;
            head += `<div class="note-viewer-link-section">
    ...
    _renderCrossAppConnections(container, noteId, head) {
        ...
        const sections = [
            { app: 'goals', label: 'Project' },
            { app: 'schedule', label: 'Task' },
            // App packages that take links (LinkManager.registerApp) follow.
            ...Object.keys(LinkManager._apps || {}).map(app => ({ app, label: LinkManager._apps[app].label || app }))
        ];
        ...
        container.querySelectorAll('.note-viewer-link-item, .note-prompt-run').forEach(el => {
            el.addEventListener('click', (e) => {
                const tagEl = e.target.closest('.note-conn-tag');
                if (tagEl) {
    ...
    openEditor(noteId = null, opts = {}) {
        if (typeof AppManager !== 'undefined' && AppManager.currentApp !== 'notes') {
            ...
            // A jump from another app straight to a note is about THAT note —
            // arrive with the filters + list folded (transient; opts.focus:
            // false opts out). Doors that switch apps before calling here
            // (record links, the assistant's record pills, linked-item rows)
            // pass focus: true themselves.
            if (noteId && opts.focus === undefined) opts = { ...opts, focus: true };
        }
    ...
    _loadNoteIntoPane(noteId = null, opts = {}) {
        ...
        // Deep-linked open: the note is the destination, so it takes the
        // whole frame — both panes fold, transiently (the user's stored
        // preference is untouched and comes back on the next normal visit).
        if (opts.focus && noteId) {
            this.setPaneCollapsed('nav', true, { transient: true });
            this.setPaneCollapsed('list', true, { transient: true });
        }
        ...
        // The indicator flips to Unsaved on the first keystroke; the
        // callback (RichEditor's own 1 s debounce) marks + schedules.
        const bodyEl = document.getElementById('note-content-editor');
        if (bodyEl && !bodyEl._dirtyWired) {
            bodyEl._dirtyWired = true;
            bodyEl.addEventListener('input', () => { if (this._paneSessionActive) this.updateSaveStatus('unsaved'); });
        }
        ...
        newTitleInput.addEventListener('input', () => {
            autosizeTitle();
            this.markAsUnsaved();
            // The title used to only MARK the note dirty; the write waited
            // for a body edit or a note switch (2026-09-10).
            this.autoSave();
        });
        ...
        if (noteId && note) {
            ...
            // Pre-select the active tag filter as a pill — visible and one
            // click to remove if it's not what the user wanted.
            const inheritedTag = this._activeTagFilter();
            this._renderEditorTagPills(inheritedTag ? [inheritedTag] : []);
        }
        autosizeTitle();
        ...
        this.updateSaveStatus('saved', { quiet: true });
        this._renderEditorDates(note);
    ...
    _teardownEditorSession() {
        ...
        if (typeof SaveStatus !== 'undefined') SaveStatus.unregister('notes');
        RichEditor.destroy();
    ...
    _clearSelection() {
        // No note open + both panes folded would be an empty page — leaving
        // the note restores the user's own pane preference.
        this._applyStoredPaneState();
        this._teardownEditorSession();
    ...
    /**
     * Save indicator (the shared SaveStatus shape). Accepts the legacy
     * vocabulary: 'unsaved' → dirty, 'saving', 'saved'.
     */
    updateSaveStatus(status, { quiet = false } = {}) {
        const state = status === 'unsaved' ? 'dirty' : status;
        if (typeof SaveStatus !== 'undefined') SaveStatus.mark('note-save-status', state, '', { quiet });
        if (status === 'saved') this.hasUnsavedChanges = false;
    },

    /**
     * SaveStatus registration: while a pane session is live, the app can
     * ask "is Notes dirty?" and "flush it" from anywhere (navigation,
     * window hide, the unload guard) — see js/components/save-status.js.
     */
    _registerSaveStatus() {
        if (typeof SaveStatus === 'undefined') return;
        SaveStatus.register('notes', {
            isDirty: () => this._paneSessionActive && (this.hasUnsavedChanges
                || (RichEditor.editor?.id === 'note-content-editor' && !!RichEditor.autoSaveTimeout)),
            flush: () => this.flushSave(),
            label: () => {
                const t = document.getElementById('note-title-input')?.value.trim();
                return t ? `Note "${t}"` : 'This note';
            },
            reopen: () => {
                AppManager.openApp('notes');
                if (this.currentNoteId) this.openEditor(this.currentNoteId);
            }
        });
    },
    ...
    _buildTagPill(tag) {
        ...
        el.title = `Show notes tagged "${tag}"`;
        el.innerHTML = `
    ...
    /**
     * Switch the list column to a tag filter — what clicking the tag in
     * the sidebar does, reachable from any rendered tag pill.
     */
    filterByTag(name) {
        if (!name) return;
        this.setFilter(name);
    ...
    /** One door for every filter/tag pick (sidebar rows, tag pills).
     *  Picking a list while a note has the full width (reading pane off)
     *  means "show me that list" — the note closes; with the pane on, the
     *  list is already beside the note and the selection stays. */
    setFilter(name) {
        if (!name) return;
        this.currentFilter = name;
        if (!this._readerPref() && this._paneSessionActive) {
            this.flushSave();
            this._editorOrigin = null;
            this._clearSelection();
            return;
    ...
    _attachTagToCurrentNote(name) {
        ...
        if (!existing) {
            this.tags.push({ id: UIUtils.generateId(), name: tagName, createdAt: new Date().toISOString() });
            this.saveTags();
        }
     ...
     */
    autoSave() {
        // Nothing to write for an empty, untitled draft.
        const titled = !!document.getElementById('note-title-input')?.value.trim();
        if (!(this.currentNoteId || titled || RichEditor.getText().trim().length > 0)) return;
        this.updateSaveStatus('unsaved');
        // ONE stored, cancellable timer — the old code scheduled an
        // independent, unstoppable setTimeout per call and showed a
        // 1 s "Saving…" for a write that is synchronous.
        if (this._autoSaveTimer) clearTimeout(this._autoSaveTimer);
        this._autoSaveTimer = setTimeout(() => { this._autoSaveTimer = null; this.flushSave(); }, 1000);
    },

    /**
     * Write now: fire RichEditor's pending input callback (which is what
     * sets hasUnsavedChanges), cancel the timer, save. Called by the
     * timer, by every note switch / close, and by SaveStatus when the
     * user leaves. Returns SaveStatus's { ok } shape.
     */
    flushSave() {
        if (!this._paneSessionActive) return { ok: true };
        if (typeof RichEditor !== 'undefined' && RichEditor.editor?.id === 'note-content-editor') RichEditor.flushPending();
        if (this._autoSaveTimer) { clearTimeout(this._autoSaveTimer); this._autoSaveTimer = null; }
        if (!this.hasUnsavedChanges) return { ok: true };
    ...
    saveCurrentNote(silent = false) {
        ...
        this.saveNotes();
        // A newly-enabled routine should start producing without
        // waiting for the next scheduler poll.
        if (this.currentTemplate === 'prompt' && typeof RoutineEngine !== 'undefined') {
        ...
        this._renderEditorDates(this.notes.find(n => n.id === this.currentNoteId));
        if (!silent) {
            this.updateSaveStatus('saved');
    ...
    /**
     * Created / edited stamps in the editor's meta strip. Relative text
     * inline ("Edited 3 hours ago"), exact timestamps in the tooltip.
     * Hidden for a draft with no record yet — there is nothing to say.
     */
    _renderEditorDates(note) {
        const el = document.getElementById('note-dates');
        if (!el) return;
        const sep = el.previousElementSibling;
        if (!note || (!note.createdAt && !note.modifiedAt)) {
            el.hidden = true;
            el.textContent = '';
        ...
        const created = note.createdAt || note.modifiedAt;
        const edited = note.modifiedAt || note.createdAt;
        const sameStamp = new Date(created).getTime() === new Date(edited).getTime();
        el.textContent = sameStamp
            ? `Created ${UIUtils.formatDate(created)}`
            : `Edited ${UIUtils.formatDate(edited)} \u00b7 Created ${UIUtils.formatDate(created)}`;
        el.title = `Created ${UIUtils.formatDateTime(created)}\nLast edited ${UIUtils.formatDateTime(edited)}`;
        el.hidden = false;
    ...
    async deleteCurrentNote() {
        ...
        if (confirmed) {
            ...
            this.saveNotes({ [this.currentNoteId]: new Date().toISOString() });

            // The note is gone — skip any outgoing-save flush.
            this.hasUnsavedChanges = false;
            if (this._autoSaveTimer) { clearTimeout(this._autoSaveTimer); this._autoSaveTimer = null; }
            if (typeof RichEditor !== 'undefined' && RichEditor.autoSaveTimeout) { clearTimeout(RichEditor.autoSaveTimeout); RichEditor.autoSaveTimeout = null; }
            this._editorOrigin = null;
            this._clearSelection();
    ...
    _updateWikiMenu() {
        ...
        const candidates = this.notes
            .filter(n => n.id !== this.currentNoteId && !['prompt', 'feed'].includes(NoteTemplates.resolve(n)))
    ...
    _relatedNotes(note, limit = 5) {
        ...
        return this.notes
            .filter(n => n.id !== note.id && !['prompt', 'feed'].includes(NoteTemplates.resolve(n)))
            .map(n => {
    ...
    getFilteredNotes() {
        ...
        // Routines live on the Routines page, and feed posts (their
        // machine-generated outputs) on the Home feed — in the general
        // buckets both would only bury real notes; only an explicit user
        // pin surfaces them here. Saved (non-scheduled) prompt notes are
        // ordinary notes and DO show since 2026-07-31.
        if (this.currentFilter !== 'pinned') {
            filtered = filtered.filter(note =>
                NoteTemplates.resolve(note) !== 'feed' && !NotePrompts.isRoutine(note));
        }
    ...
    // (Goals / goal) renders here so the user can step back.
...
// One builder for the CURRENT NOTE block, used by the page provider (the
// currently-open note, so the assistant can answer "make this clearer"
// without the user pasting anything) and the 'notes' record resolver (a
// conversation attached to a note, continued away from the Notes app) —
// one builder so "this note" cannot drift between the two paths.
NotesApp.noteContextBlock = function (noteId, opts = {}) {
    if (!noteId) return null;
    if (!Array.isArray(NotesApp.notes) || NotesApp.notes.length === 0) {
        NotesApp.loadNotes();
    }
    const note = (NotesApp.notes || []).find(n => n && n.id === noteId);
    if (!note) return null;

    // Markdown, not stripped text — the SAME read get_note gives
    // (AgentTools.noteHtmlToMd). This block is what "make this clearer"
    // edits are written from, so a read that drops the headings/lists/bold
    // guarantees the update_note rewrite drops them too. Raw-strip only as
    // a fallback if AgentTools is somehow absent.
    let plain;
    try {
        plain = AgentTools.noteHtmlToMd(note.content);
    } catch {
        plain = String(note.content || '')
    ...
    const lead = opts.attached
        ? 'This conversation is attached to the note below (the user may not have it open right now)'
        : 'The user is currently viewing or editing the note below in the Notes sub-app';
    return {
        recordKey: 'notes:' + note.id,
        recordLabel: note.title || '(untitled note)',
        title: 'CURRENT NOTE',
        body: `${lead}. The note is available as context, not a hard constraint:

- When their question or instruction refers to "this note", "the note", "what I wrote", etc., or is clearly about its content, work with the text below. To save edits, call the update_note tool with id: ${note.id} — the content below is markdown, and an edit must keep its headings, lists, bold and links intact, changing only what was asked.
...
if (typeof AgentContext !== 'undefined') {
    AgentContext.register('notes', () => NotesApp.noteContextBlock(NotesApp.currentNoteId));

    // Record resolver — rebuilds the CURRENT NOTE block from a conversation's
    // 'notes:<id>' attachment when the chat is continued away from Notes.
    AgentContext.registerRecord('notes', (id) => NotesApp.noteContextBlock(id, { attached: true }));
}
