...
const NotesApp = {
    ...
    getFilteredNotes() {
        ...
        // Prompt notes live on the Prompts page, and feed posts (their
        // machine-generated outputs) on the Home feed — in the general
        // buckets both would only bury real notes. Only an explicit user
        // pin surfaces them here.
        if (this.currentFilter !== 'pinned') {
            filtered = filtered.filter(note => !['prompt', 'feed'].includes(NoteTemplates.resolve(note)));
        }
