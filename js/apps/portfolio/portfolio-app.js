...
const PortfolioApp = {
    ...
    setupEventListeners() {
        ...
        // "New strategy" is a conversation, not a form — same intake the
        // empty state offers.
        bind('portfolio-strategy-new-btn', 'click', () => {
            if (typeof AgentUI === 'undefined') {
                UIUtils.showToast('The assistant is needed to build a strategy', 'warning');
                return;
            }
            AgentUI.askWithPrompt(PortfolioStrategyUI.INTERVIEW_PROMPT, { newChat: true });
        });
     ...
     * permanent header button: New account, Strategy, Snapshots (history
    ...
    openMoreMenu(anchor) {
        ...
        const items = [
            ...
            { action: 'strategy', label: 'Strategy', icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="4"/><circle cx="12" cy="12" r="1"/></svg>' },
        ...
        menu.querySelectorAll('.portfolio-more-menu-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                else if (action === 'strategy') this.openStrategy();
    ...
    /**
     * The account scope menu behind the header picker: "All Accounts" plus
     * one row per account, the current scope check-marked. Same anchored-
     * popover contract as openMoreMenu (outside click, Escape, re-click to
     * toggle off); it scrolls internally rather than growing without bound,
     * so any number of accounts stays usable.
     */
    openAccountMenu(anchor, activeAccountId = null) {
        const existing = document.getElementById('portfolio-account-menu');
        if (existing) {
            existing.remove();
            anchor.setAttribute('aria-expanded', 'false');
            if (existing._anchor === anchor) return; // toggle-off
        ...
        const check = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.25" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>';
        const rows = [
            { id: 'all', name: 'All Accounts' },
            ...this.getAccounts().map(a => ({ id: a.id, name: a.name })),
        ];
        const currentId = activeAccountId || 'all';

        const menu = document.createElement('div');
        menu.id = 'portfolio-account-menu';
        menu.className = 'portfolio-more-menu portfolio-account-menu';
        menu.setAttribute('role', 'menu');
        menu._anchor = anchor;
        menu.innerHTML = rows.map(r => `
            <button type="button" class="portfolio-more-menu-item portfolio-account-menu-item${r.id === currentId ? ' is-current' : ''}"
                    role="menuitemradio" aria-checked="${r.id === currentId}" data-account="${AppManager.escapeHtml(r.id)}">
                <span class="portfolio-account-menu-check">${r.id === currentId ? check : ''}</span>
                <span>${AppManager.escapeHtml(r.name)}</span>
            </button>
        `).join('');
        ...
        // Below the anchor, left-aligned with it, nudged back on screen if a
        // long account name pushes the menu past the right edge.
        const rect = anchor.getBoundingClientRect();
        const menuRect = menu.getBoundingClientRect();
        menu.style.top = `${rect.bottom + window.scrollY + 6}px`;
        const maxLeft = window.innerWidth - menuRect.width - 8;
        menu.style.left = `${Math.max(8, Math.min(rect.left + window.scrollX, maxLeft))}px`;
        ...
        menu.querySelectorAll('[data-account]').forEach(item => {
            item.addEventListener('click', () => {
                const target = item.dataset.account;
                close();
                if (target === 'all') {
                    if (this.viewingAccountId) this.closeAccountDetail();
                } else if (target !== this.viewingAccountId) {
                    this.openAccountDetail(target);
                }
            });
        });
    },
    ...
    render() {
        ...
        this.loadData();
        PortfolioUI.renderAccountPicker('portfolio-account-picker');
        ...
        if (typeof PortfolioStrategyUI !== 'undefined') PortfolioStrategyUI.renderCard('portfolio-strategy-card');
        ...
        // Portfolio-wide linked notes ('overview' pseudo-item).
    ...
    /**
     * Open the Strategy view.
     */
    openStrategy(strategyId = null) {
        this.viewingStrategyId = strategyId;
        ...
        document.getElementById('portfolio-strategy-view').classList.add('active');
        Breadcrumb.render('portfolio-strategy-breadcrumb', [
            { label: 'Portfolio', action: () => this.closeStrategy() },
            { label: 'Strategy' }
        ]);
    ...
    closeStrategy() {
        document.getElementById('portfolio-strategy-view').classList.remove('active');
    ...
    renderStrategyView() {
        this.loadData();
        PortfolioStrategyUI.renderView(this.viewingStrategyId);
    },
    ...
    openAccountDetail(accountId) {
        ...
        // Just "Portfolio", as a way back — the account is already named by
        // the scope picker beside it and by the page heading below, and three
        // copies of "Padma - Fidelity" in one header is two too many.
    ...
    renderAccountDetail() {
        ...
        PortfolioUI.renderAccountPicker('portfolio-account-picker-detail', account.id);
...
if (typeof AgentContext !== 'undefined') {
    AgentContext.register('portfolio', () => {
        ...
        // The Strategy page has no editor by design — the assistant IS the
        // editor — so the pill here is the main way in, and its prompts
        // differ depending on whether there is a plan to work on yet.
        if (viewActive('portfolio-strategy-view')) {
            const strategy = typeof PortfolioStrategy !== 'undefined'
                ? (PortfolioStrategy.getById(PortfolioApp.viewingStrategyId) || PortfolioStrategy.getDefault())
                : null;
            if (!strategy) {
                return {
                    recordKey: 'portfolio:strategy',
                    recordLabel: 'Strategy',
                    title: 'INVESTMENT STRATEGY',
                    body: 'The user is on the Strategy page with no strategy saved yet. If they want one, call start_strategy_interview and work through its agenda one question at a time — do not ask them to describe a strategy cold.',
                    suggestedPrompts: [
                        'Help me build an investment strategy',
                        'What should a strategy actually cover?',
                        'Look at what I hold and suggest a starting plan'
                    ]
                };
            }
            const draft = (strategy.status || 'active') === 'draft';
            return {
                recordKey: 'portfolio:strategy:' + strategy.id,
                recordLabel: strategy.name,
                title: 'INVESTMENT STRATEGY',
                body: `The user is viewing their investment strategy "${strategy.name}"${draft ? ' (unfinished)' : ''}. When they say "this strategy" or "my plan", they mean this one. get_strategy reads it; check_strategy scores their real holdings against it; save_strategy records changes they agree to.`,
                suggestedPrompts: draft
                    ? ['Finish setting up this strategy', 'What do you still need to know?']
                    : [
                        'Am I still following this?',
                        'Stress-test this strategy',
                        'How would I get back on plan?'
                    ]
            };
        }
