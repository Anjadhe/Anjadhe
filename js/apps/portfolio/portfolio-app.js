...
const PortfolioApp = {
    currentAccountFilter: 'all',    // 'all', an account id, 'tickers' or 'strategy' — the nav scope
    ...
    sort: {
        ...
        tickers: { col: 'value', dir: 'desc' },       // the Tickers page (PortfolioTickers.sort)
        indicators: { col: 'ticker', dir: 'asc' },    // its Indicators view (columns are `ind:<verdict>`)
    ...
    init() {
        ...
        // Linked brokerages (Connect/Plaid): learns this Mac's id so only
        // the linking Mac pulls. Never a view — the sync starts from render.
        if (typeof PortfolioBrokerage !== 'undefined') PortfolioBrokerage.init();
        // Sharing with contacts (docs/PORTFOLIO_SHARING.md): listens for
        // snapshots from contacts and pushes this Mac's shares. Flag-gated
        // inside; a no-op otherwise.
        if (typeof PortfolioSharing !== 'undefined') PortfolioSharing.init();
    ...
    loadData() {
        ...
        // Shares given to contacts (docs/PORTFOLIO_SHARING.md) — {id,
        // accountId, contactId, contactLabel, scope, generation, lastPushedAt,
        // lastPushedHash, delivered, lastAckAt, revokedAt, pushMachineId},
        // record-merged + tombstoned like the other arrays. Accounts
        // RECEIVED from a contact live in `accounts` with a `shared` block.
        this.shares = Array.isArray(data?.shares) ? data.shares : [];
    ...
    /** The user's OWN accounts — what totals, the Tickers page, tools and
     *  the nav's Accounts section read. Accounts a contact shares in
     *  (`shared` block) are deliberately not here: they never count in
     *  net worth and are reached through getSharedAccounts(). */
    getAccounts() {
        return this.accounts.filter(a => !a.shared);
    },
    /** Accounts another nenva user shares with this one (read-only copies). */
    getSharedAccounts() {
        return this.accounts.filter(a => a && a.shared);
    },
    getTransactions() {
    ...
    /* ---------- AI writing (Settings › Portfolio) ----------
     *
     * Two things in this app are written by a model: a business profile per
     * ticker (PortfolioProfile, once a day) and the daily market brief
     * (PortfolioBrief). ONE switch governs both, because to the user they are
     * the same thing — "the AI writing about my money".
     *
     * Its own synced key, NOT a field on the `portfolio` blob: that blob is
     * record-merged (main.js RECORD_MERGED_KEYS) around id-keyed arrays and
     * only real user edits may stamp it, so a scalar preference has no home
     * there. `discover-settings` (News) is the precedent for a per-app
     * preference key. Absent means ON: the feature shipped before the switch
     * did, and nobody should have to find a setting to get back what they
     * already had.
     */
    AI_SETTINGS_KEY: 'portfolio-settings',

    aiWritingEnabled() {
        try {
            const s = StorageManager.get(this.AI_SETTINGS_KEY);
            return !s || s.aiWriting !== false;
        } catch { return true; }
    },

    setAiWritingEnabled(on) {
        const s = StorageManager.get(this.AI_SETTINGS_KEY) || {};
        s.aiWriting = !!on;
        StorageManager.set(this.AI_SETTINGS_KEY, s);
    },

    /**
     * Settings › Portfolio › "Current headlines from CNBC" (PortfolioHeadlines).
     * Same key, its own field, and — unlike aiWriting — ABSENT MEANS OFF: it
     * reads a third-party site every ten minutes and spends model calls on
     * it, so it is something the user turns on, not something they find.
     */
    headlinesEnabled() {
        try { return StorageManager.get(this.AI_SETTINGS_KEY)?.headlines === true; }
        catch { return false; }
    },

    setHeadlinesEnabled(on) {
        const s = StorageManager.get(this.AI_SETTINGS_KEY) || {};
        s.headlines = !!on;
        StorageManager.set(this.AI_SETTINGS_KEY, s);
    },
    ...
    saveData() {
        ...
        this.loadData();
        // A change to an account shared with a contact travels within
        // seconds (debounced; a push that finds nothing changed is a no-op).
        if (typeof PortfolioSharing !== 'undefined') PortfolioSharing.onDataSaved();
    },
    ...
    setupEventListeners() {
        ...
        bind('portfolio-account-share-btn', 'click', () => {
            if (typeof PortfolioSharing !== 'undefined' && this.viewingAccountId) PortfolioSharing.showShareModal(this.viewingAccountId);
        });
        bind('portfolio-account-sync-btn', 'click', () => {
            if (typeof PortfolioBrokerage === 'undefined' || !this.viewingAccountId) return;
            const a = this.accounts.find(x => x.id === this.viewingAccountId);
            if (a?.brokerage?.needsLogin) PortfolioBrokerage.relink(a.brokerage.itemId);
            else PortfolioBrokerage.sync([this.viewingAccountId], { manual: true });
        });
        bind('portfolio-account-drift-btn', 'click', () => {
            if (typeof PortfolioBrokerage !== 'undefined' && this.viewingAccountId) PortfolioBrokerage.showDrift(this.viewingAccountId);
        });
    ...
    render() {
        ...
        this.loadData();
        // Today's business profiles for the holdings, written quietly one
        // at a time (once a day; PortfolioProfile decides whether it may).
        if (typeof PortfolioProfile !== 'undefined') PortfolioProfile.scheduleSweep();
        // Linked brokerage accounts older than a day are pulled quietly
        // (docs/PORTFOLIO.md "Brokerage linking": on open, TTL-gated, no
        // background poll).
        if (typeof PortfolioBrokerage !== 'undefined') PortfolioBrokerage.ensureFresh();
        ...
        // The Tickers page replaces the pane the same way (PortfolioTickers —
        // holdings + watchlist consolidated on one sortable table).
        if (scope === 'tickers') {
            PortfolioUI.updateHideValuesBtn(this.hideValues);
            PortfolioUI.renderNav(scope);
            PortfolioUI.renderScopeHeader(null);
            this.renderPricesAsOfFor(this.computeHoldings());
            if (typeof PortfolioTickers !== 'undefined') PortfolioTickers.render();
            this.autoRefreshPrices();
            return;
        }
        if (typeof PortfolioTickers !== 'undefined') PortfolioTickers.hide();
        const scoped = scope !== 'all';
        // Own accounts and shared-in ones are both scopes; only the former
        // are in getAccounts().
        const account = scoped ? this.accounts.find(a => a.id === scope) : null;
        ...
        // inherits).
        // A shared-in account is someone else's money: no plan of ours
        // applies to it, so the line is hidden there.
        PortfolioUI.renderStrategyLine('portfolio-strategy-card', !account?.shared, scoped ? scope : null);
        ...
        // Headlines on the holdings in scope; the blank slate has none.
        let newsScope = true;
        if (this.currentView === 'transactions') {
            // One account reads its own rows (a shared-in account's rows
            // are not in getTransactions, which is the user's own money).
            const txns = (scoped ? this.transactions.filter(t => t.accountId === scope) : this.getTransactions())
        ...
        if (typeof PortfolioNews !== 'undefined') {
            if (newsScope) PortfolioNews.renderForScope(scoped ? scope : null);
            else { const n = document.getElementById('portfolio-news'); if (n) n.innerHTML = ''; }
        }

        // Today's market-and-portfolio brief: All Accounts only (an account
        // has no market of its own), and only when there is a book to brief.
        if (typeof PortfolioBrief !== 'undefined') {
            if (!scoped && newsScope) PortfolioBrief.renderForOverview();
            else { const b = document.getElementById('portfolio-brief'); if (b) b.innerHTML = ''; }
        }
        // CNBC's current headlines: All Accounts only, like the brief.
        if (typeof PortfolioHeadlines !== 'undefined') {
            if (!scoped && newsScope) PortfolioHeadlines.renderForOverview();
            else { const h = document.getElementById('portfolio-headlines'); if (h) h.innerHTML = ''; }
        }
    ...
    setScope(scope) {
        ...
        // A chat about the All Accounts brief does not follow to an account.
        if (typeof PortfolioBrief !== 'undefined') PortfolioBrief.chatting = false;
        this.viewingAccountId = (scope === 'all' || scope === 'strategy' || scope === 'tickers') ? null : scope;
        this.render();
    },
    ...
    async autoRefreshPrices() {
        ...
        // refreshPrices ends in render(), which lands back here. A fetch
        // that leaves something stale (network down, a symbol nobody can
        // price) must not turn that into a repaint loop, so an automatic
        // refresh runs at most once a minute; the header's manual refresh
        // is not gated.
        const now = Date.now();
        if (hasStale && !this.isRefreshing && now - (this._lastAutoRefreshAt || 0) > 60000) {
    ...
    getUniqueTickers() {
        ...
        // Positions in accounts shared with the user are quoted here too —
        // the tickers are theirs to see; the prices are this Mac's own.
        for (const a of this.getSharedAccounts()) {
            for (const h of this.computeHoldings(a.id)) {
                if (h.totalShares > 0 && !tickers.includes(h.ticker)) tickers.push(h.ticker);
            }
        }
        return tickers;
    },
    ...
    computeHoldings(accountId = null) {
        // One account: its own rows (a shared account's rows are not in
        // getTransactions, which is scoped to the user's own accounts).
        let txns = accountId
    ...
    async deleteCurrentAccount() {
        const account = this.accounts.find(a => a.id === this.viewingAccountId);
        if (!account) return;
        // A shared-in account is the sharer's; removing it here only removes
        // the copy (they can share it again).
        if (account.shared) {
            const ok = await UIUtils.confirm('Remove shared account',
                `Remove "${account.name}", shared by ${account.shared.contactLabel || 'a contact'}, from your Portfolio? They keep their account; they can share it again later.`, '', { confirmText: 'Remove' });
            if (!ok) return;
            if (typeof PortfolioSharing !== 'undefined') PortfolioSharing.removeShared(account.shared.shareId);
            this.closeAccountDetail();
            return;
        }

        const txnCount = this.transactions.filter(t => t.accountId === account.id).length;
        const linkedNote = account.brokerage
            ? ` This also disconnects ${account.brokerage.institution || 'the brokerage'} link if no other account uses it.` : '';
        const confirmed = await UIUtils.confirm(
            'Delete Account',
            `Delete "${account.name}" and its ${txnCount} transaction(s)? This cannot be undone.${linkedNote}`
        );

        if (confirmed) {
            if (typeof PortfolioBrokerage !== 'undefined') PortfolioBrokerage.onAccountDeleted(account);
    ...
    openTransactionEditor(transactionId = null) {
        if (transactionId) {
            ...
            // A cash event from a linked brokerage (dividend, deposit, fee)
            // is the broker's ledger line, not a trade — the editor has no
            // shape for it and cash itself comes from the broker's balance.
            if (this.editingTransaction?.type === 'cash') {
                this.editingTransaction = null;
                UIUtils.showToast('Cash events from a linked brokerage are read-only here', 'info');
                return;
            }
            // Rows of an account shared IN are the sharer's — read-only here.
            const owner = this.editingTransaction && this.accounts.find(a => a.id === this.editingTransaction.accountId);
            if (owner?.shared) {
                this.editingTransaction = null;
                UIUtils.showToast(`This account is shared by ${owner.shared.contactLabel || 'a contact'} and is read-only`, 'info');
                return;
            }
        } else {
            this.editingTransaction = null;
            const scopeAcc = this.viewingAccountId && this.accounts.find(a => a.id === this.viewingAccountId);
            if (scopeAcc?.shared) {
                UIUtils.showToast(`This account is shared by ${scopeAcc.shared.contactLabel || 'a contact'} and is read-only`, 'info');
                return;
            }
        }
    ...
    setSort(scope, col) {
        ...
        if (s.col === col) {
            ...
            s.col = col;
            // A column may name the direction its FIRST click means (the
            // Indicators view sorts best-first, which is why it exists).
            s.dir = this.TEXT_SORT_COLS.includes(col) ? 'asc' : (this.defaultSortDir(scope, col) || 'desc');
        }
    ...
    /** The direction a column's first click means, when its owner has one. */
    defaultSortDir(scope, col) {
        if (scope === 'indicators' && typeof PortfolioTickers !== 'undefined') return PortfolioTickers.defaultDir(col);
        return null;
    },
    ...
    sortHoldings(holdings) {
        ...
        return [...holdings].sort((a, b) => {
            ...
            // Weight is value over a fixed denominator, so it orders exactly
            // like Value — its own column exists for the header's arrow.
            if (col === 'weight') return dir * ((a.currentValue || 0) - (b.currentValue || 0));
    ...
    closeTickerDetail() {
        this.viewingTicker = null;
        if (typeof PortfolioProfile !== 'undefined') PortfolioProfile.chatting = null;
        document.getElementById('portfolio-ticker-view').classList.remove('active');
        document.getElementById('portfolio-view').classList.add('active');
        this.render();
    },
...
if (typeof AgentContext !== 'undefined') {
    AgentContext.register('portfolio', () => {
        ...
        if (PortfolioApp.viewingTicker) {
            ...
            // Opened from the profile's own "Ask about this profile…": the
            // profile text IS the subject, so it rides along.
            const profileBlock = (typeof PortfolioProfile !== 'undefined') ? PortfolioProfile.contextBlock(t) : null;
            if (profileBlock) {
                return {
                    recordKey: 'portfolio:ticker:' + t,
                    recordLabel: label,
                    title: profileBlock.title,
                    body: `The user is viewing the detail page for ${label}.${symbolNote} ${profileBlock.body}`,
                    suggestedPrompts: profileBlock.suggestedPrompts
                };
            }
            return {
                recordKey: 'portfolio:ticker:' + t,
                ...
                body: `The user is viewing the detail page for ${label}. When they say "this stock" / "this ticker" / "this position", they mean ${label}.${symbolNote} Use get_ticker_detail or list_portfolio if you need current numbers, get_holdings_news with tickers ["${t}"] for recent headlines about it, and get_ticker_profile for today's business profile (what it sells, how sound it is, valuation, risks). When the user corrects or annotates that profile, call correct_ticker_profile — it saves the note on the ticker and rewrites the profile around it.`,
                suggestedPrompts: [
                    `How is ${label} performing?`,
                    `Is ${label} expensive or cheap right now?`,
                    `What are the biggest risks to ${label}?`
                ]
            };
        }
        ...
        // The Tickers page: every held or watched symbol on one table. The
        // subject is the whole set, so the tools are the same as the overview's.
        if (PortfolioApp.currentAccountFilter === 'tickers') {
            const indicators = typeof PortfolioTickers !== 'undefined' && PortfolioTickers.view() === 'indicators';
            return {
                recordKey: 'portfolio:tickers',
                recordLabel: 'Tickers',
                title: 'TICKERS PAGE',
                body: indicators
                    ? 'The user is viewing the Tickers page on its Indicators view: every symbol they hold (across all accounts) or follow on the watchlist, each wearing the one-word verdicts from its daily business profile — valuation, soundness, AI role, near- and long-term risk and value, news tone — one sortable column each. Those words are the profile\'s own; get_ticker_profile returns the profile behind any one of them (the reasoning, not just the word), list_portfolio the holdings and totals, get_ticker_detail one symbol\'s numbers, get_holdings_news headlines. Ground observations in tool results.'
                    : 'The user is viewing the Tickers page: every symbol they hold (across all accounts) or follow on the watchlist, on one table with shares, average cost, price, value, weight, P&L, today\'s move and the accounts holding it. Use list_portfolio for holdings and totals, get_ticker_detail for one symbol, get_holdings_news for headlines. Ground observations in tool numbers.',
                suggestedPrompts: indicators
                    ? [
                        'Which of these look cheap with low long-term risk?',
                        'Which of these does my AI model call expensive, and why?',
                        'Which holdings carry the most near-term risk?'
                    ]
                    : [
                        'Which of these moved the most today?',
                        'Which tickers do I hold in more than one account?',
                        'What on my watchlist looks cheap right now?'
                    ]
            };
        }
        ...
        if (hasData) {
            // Opened from the brief's own "Ask about this brief…": the
            // brief text IS the subject, so it rides along.
            const briefBlock = (typeof PortfolioBrief !== 'undefined') ? PortfolioBrief.contextBlock() : null;
            if (briefBlock) {
                return {
                    recordKey: 'portfolio:overview',
                    recordLabel: 'Portfolio',
                    title: briefBlock.title,
                    body: briefBlock.body,
                    suggestedPrompts: briefBlock.suggestedPrompts
                };
            }
            return {
                recordKey: 'portfolio:overview',
                ...
                body: 'The user is viewing their portfolio overview (all accounts, holdings, cash, real estate). Use list_portfolio for current holdings, totals, and cash; get_ticker_detail for a single stock; get_holdings_news for recent headlines on the holdings; get_portfolio_brief for today\'s written brief on the market and the whole portfolio (the one shown at the top of this page — pass refresh:true when they ask for a fresh one). Ground observations in tool numbers.',
                suggestedPrompts: [
    ...
    AgentContext.registerRecord('portfolio', (rest) => {
        ...
        return null;
    // A chat attached to a strategy/account carries the portfolio tools.
    }, { domains: ['portfolio'] });
}
