...
(function () {
    ...
    function moversHtml(list, dir) {
        ...
        const mono = (t) => (typeof PortfolioUI.tickerMono === 'function' ? PortfolioUI.tickerMono(t) : '');
    ...
    /** Investments · cash · real estate as one thin bar, the hero's palette. */
    function compositionHtml(summary) {
        const parts = [
            ['stocks', summary.totalValue - (summary.cash || 0) - (summary.realEstateValue || 0)],
            ['cash', summary.cash || 0],
            ['realestate', summary.realEstateValue || 0]
        ].filter(p => p[1] > 0);
        const total = parts.reduce((s, p) => s + p[1], 0);
        if (parts.length < 2 || !(total > 0)) return '';
        return `<div class="pfw-comp" aria-hidden="true">${parts.map(([k, v]) => `<span class="pfw-comp-seg is-${k}" style="flex-basis:${(v / total) * 100}%"></span>`).join('')}</div>`;
    }
    ...
    Widgets.register('portfolio', {
        ...
        load() {
            ...
            // Home is visited daily even when Portfolio isn't: the once-a-day
            // business profiles start from here too (idempotent, gated inside).
            ...
            if (broken.length) {
                body += `<div class="pfw-breaches">${broken.slice(0, MAX_BREACHES).map(r => `
                    <div class="pfw-breach">
                        <span class="pfw-breach-mark" aria-hidden="true"><svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></span>
