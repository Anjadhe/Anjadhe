 ...
 *  - VERDICTS ARE THE MODEL'S OWN WORDS, LIFTED OFF THE PAGE (2026-09-10).
 *    The profile ends in a fixed "Verdicts" section of "Label: Word" lines
 *    (valuation, soundness, AI role, near/long-term risk and value, news
 *    tone), each word chosen from the ones the prompt offers. The pure
 *    `parseVerdicts` lifts them into `entry.verdicts` at write time and
 *    strips the section from the prose; the page renders them as indicator
 *    tiles above the text (`_verdictsHtml`). They are judgments the profile
 *    already made in words, now readable at a glance — never arithmetic
 *    from the fact sheet, and never anything the prose does not also say.
 *    `FORMAT` stamps the entry; an older entry is rewritten once
 *    (`current()`), so indicators arrive without waiting for tomorrow.
...
const PortfolioProfile = {
    ...
    // Collapsed from the head row (one setting for every ticker): body and
    // actions hidden, heading + meta line kept. Per-Mac, like the fold.
    COLLAPSED_KEY: 'portfolio-profile-collapsed',
    HEADLINES: 8,
    ...
    // Bumped when the profile's SHAPE changes (2: the Verdicts section). An
    // entry written under an older format is rewritten once — see current().
    FORMAT: 2,
    ...
    /** Today's, with text, in the current format — what ensure() hands back
     *  without a run and what the sweep does not queue. */
    current(entry) { return this.isToday(entry) && !!entry.text && (entry.format || 1) >= this.FORMAT; },

    /* ---------- verdicts: the profile's judgments as indicators ---------- */

    /**
     * The indicator spec, in page order. `options` are ordered low → high
     * for scales (the tile fills a meter up to the word); `categorical`
     * renders the word as a chip instead. `tone` colors the word and meter
     * on the semantic palette (good / warn / bad), the accent for AI, ink
     * otherwise. `section` finds the prose section a tile jumps to.
     */
    VERDICTS: [
        { key: 'valuation', label: 'Valuation', ask: 'Valuation', options: ['Cheap', 'Fair', 'Expensive'],
          tone: { Cheap: 'good', Fair: 'ink', Expensive: 'bad' }, section: /^is (the stock|it) expensive/i,
          aliases: ['valuation', 'price', 'expensiveorcheap', 'expensivecheap'] },
        { key: 'soundness', label: 'Soundness', ask: 'Soundness', options: ['Fragile', 'Mixed', 'Sound', 'Strong'],
          tone: { Fragile: 'bad', Mixed: 'warn', Sound: 'good', Strong: 'good' }, section: /^how sound/i,
          aliases: ['soundness', 'businesssoundness', 'howsound', 'howsounditis', 'howsoundthebusinessis'] },
        { key: 'ai', label: 'AI role', ask: 'AI', categorical: true,
          options: ['Builder', 'Supplier', 'Beneficiary', 'Customer', 'Threatened', 'Unrelated'],
          fundOptions: ['Heavy', 'Moderate', 'Light', 'Unrelated'],
          tone: { Builder: 'accent', Supplier: 'accent', Beneficiary: 'accent', Customer: 'ink', Threatened: 'bad', Unrelated: 'ink', Heavy: 'accent', Moderate: 'accent', Light: 'ink' },
          section: /^where ai fits/i, aliases: ['ai', 'aifit', 'airole', 'whereaifits', 'aiplay', 'aiexposure'] },
        { key: 'nearRisk', label: 'Near-term risk', ask: 'Near-term risk', options: ['Low', 'Moderate', 'High'],
          tone: { Low: 'good', Moderate: 'warn', High: 'bad' }, section: /^near-term risks?/i,
          aliases: ['neartermrisk', 'neartermrisks', 'shorttermrisk', 'shorttermrisks'] },
        { key: 'longRisk', label: 'Long-term risk', ask: 'Long-term risk', options: ['Low', 'Moderate', 'High'],
          tone: { Low: 'good', Moderate: 'warn', High: 'bad' }, section: /^long-term risks?/i,
          aliases: ['longtermrisk', 'longtermrisks'] },
        { key: 'nearValue', label: 'Near-term value', ask: 'Near-term value', options: ['Limited', 'Moderate', 'Strong'],
          tone: { Limited: 'warn', Moderate: 'ink', Strong: 'good' }, section: /^near-term value/i,
          aliases: ['neartermvalue', 'shorttermvalue'] },
        { key: 'longValue', label: 'Long-term value', ask: 'Long-term value', options: ['Limited', 'Moderate', 'Strong'],
          tone: { Limited: 'warn', Moderate: 'ink', Strong: 'good' }, section: /^long-term value/i,
          aliases: ['longtermvalue'] },
        { key: 'news', label: 'News tone', ask: 'News', options: ['Negative', 'Mixed', 'Positive'], extra: ['None'],
          tone: { Negative: 'bad', Mixed: 'ink', Positive: 'good', None: 'ink' }, section: /^in the news/i,
          aliases: ['news', 'newstone', 'inthenews', 'headlines'] }
    ],
    // Words the model reaches for that mean one of the offered ones.
    VERDICT_SYNONYMS: {
        fairly: 'Fair', fairlyvalued: 'Fair', inbetween: 'Fair', reasonable: 'Fair', reasonablyvalued: 'Fair',
        overvalued: 'Expensive', pricey: 'Expensive', rich: 'Expensive', undervalued: 'Cheap', inexpensive: 'Cheap',
        medium: 'Moderate', middling: 'Moderate', modest: 'Moderate', elevated: 'High', significant: 'High', minimal: 'Low',
        robust: 'Strong', solid: 'Sound', healthy: 'Sound', weak: 'Fragile', shaky: 'Fragile',
        neutral: 'Mixed', balanced: 'Mixed', favorable: 'Positive', favourable: 'Positive', unfavorable: 'Negative', unfavourable: 'Negative',
        unknown: 'Unknown', unclear: 'Unknown', na: 'Unknown', notonthesheet: 'Unknown', cannotsay: 'Unknown', none: 'None', nonelisted: 'None', noheadlines: 'None'
    },

    /** The offered words for one spec entry (fund shape where it differs). */
    verdictOptions(spec, fund) {
        return (fund && spec.fundOptions) ? spec.fundOptions : spec.options;
    },

    /** The prompt's own description of the section, built from the spec so
     *  the parser and the ask can never disagree. */
    _verdictsSection(fund) {
        const lines = this.VERDICTS.map(v => `${v.ask}: ${this.verdictOptions(v, fund).join(' / ')}${v.extra ? ' / ' + v.extra.join(' / ') : ''} / Unknown`);
        return `Verdicts — a final section of exactly these ${lines.length} lines, one per line, each "Label: Word", choosing ONLY from the words offered and Unknown when the sheet cannot say (no sentences here, no other lines): ${lines.join('; ')}. Each word must agree with what you wrote in the section it sums up.`;
    },

    _normKey(s) { return String(s || '').toLowerCase().replace(/[^a-z]/g, ''); },

    /**
     * Lift the Verdicts section off a profile. Pure over the text
     * (Node-tested): returns { verdicts: {key: Word}, body: the prose without
     * that section, found }. Tolerant of markdown around the lines (bold,
     * bullets, dashes) and of the model's near-synonyms; a line whose word
     * is not one offered is dropped, never guessed. Without a heading the
     * tail of the text is scanned, so a model that forgot the "##" still
     * gets its verdicts read — and the matched lines still leave the prose.
     */
    parseVerdicts(text, { fund = false } = {}) {
        const src = String(text || '');
        const out = { verdicts: {}, body: src, found: false };
        if (!src.trim()) return out;
        const lines = src.split('\n');
        const headRx = /^\s*(?:#{1,6}\s*|\*\*|__)?\s*(?:\d+\.\s*)?verdicts?\s*(?:\*\*|__)?\s*:?\s*$/i;
        let start = lines.findIndex(l => headRx.test(l));
        let end = lines.length;
        if (start >= 0) {
            for (let i = start + 1; i < lines.length; i++) {
                if (/^\s*#{1,6}\s+\S/.test(lines[i])) { end = i; break; }
            }
        }
        const scanFrom = start >= 0 ? start + 1 : Math.max(0, lines.length - 24);
        const byAlias = new Map();
        for (const v of this.VERDICTS) for (const a of v.aliases) byAlias.set(a, v);
        const matched = new Set();
        for (let i = scanFrom; i < end; i++) {
            const raw = lines[i].replace(/^[\s\-*•·]+/, '').replace(/[*_`]/g, '').trim();
            // Colon, em/en dash, or a spaced hyphen — a bare hyphen is part
            // of "Near-term", not a separator.
            const m = /^([A-Za-z][A-Za-z \-]{0,28}?)\s*(?::|—|–|\s-\s)\s*(.+?)\s*$/.exec(raw);
            if (!m) continue;
            const spec = byAlias.get(this._normKey(m[1]));
            if (!spec) continue;
            const word = this._matchVerdict(spec, m[2], fund);
            if (!word) continue;
            out.verdicts[spec.key] = word;
            matched.add(i);
            out.found = true;
        }
        // Strip: the whole section when headed, else only the lines matched.
        const keep = lines.filter((l, i) => start >= 0 ? (i < start || i >= end) : !matched.has(i));
        out.body = keep.join('\n').replace(/\n{3,}/g, '\n\n').trim();
        return out;
    },

    /** The offered word a free-text value means, or null. */
    _matchVerdict(spec, value, fund) {
        const offered = [...this.verdictOptions(spec, fund), ...(spec.extra || []), 'Unknown'];
        const cleaned = String(value).replace(/\(.*?\)/g, ' ').replace(/[.;!,]+.*$/, '').trim();
        const norm = this._normKey(cleaned);
        if (!norm) return null;
        // Exact word first (case-insensitive), then a leading match, then a synonym.
        for (const o of offered) if (norm === this._normKey(o)) return o;
        for (const o of offered) if (norm.startsWith(this._normKey(o))) return o;
        const syn = this.VERDICT_SYNONYMS[norm] || this.VERDICT_SYNONYMS[norm.split(/(?=[A-Z])/)[0]];
        if (syn && offered.includes(syn)) return syn;
        // A word buried in a phrase ("looks Fair to me") — first offered word found.
        const words = cleaned.toLowerCase().split(/[^a-z]+/).filter(Boolean);
        for (const w of words) {
            const hit = offered.find(o => o.toLowerCase() === w) || (this.VERDICT_SYNONYMS[w] && offered.includes(this.VERDICT_SYNONYMS[w]) ? this.VERDICT_SYNONYMS[w] : null);
            if (hit) return hit;
        }
        return null;
    },

    /** Prose without the Verdicts section (display only; the cache keeps the whole text). */
    stripVerdicts(text) { return this.parseVerdicts(text).body; },
    ...
    /**
     * May a profile be written without a click? Asked of the model that
     * answers the Portfolio surface (model-routing.js R4).
     */
    autoAllowed() {
    ...
    /** The entry that answers this surface — the brain unless assigned. */
    _entry() {
        try { return AgentService.entryForSource?.(this.SOURCE) ?? AgentService.getDefaultEntry?.() ?? null; }
        catch { return null; }
    },

    /** Is the model that writes profiles metered? (model-routing.js R4.) */
    _metered() {
        try { return AgentService.isMeteredFor?.(this.SOURCE) ?? AgentService.isMeteredBrain?.() ?? false; }
        catch { return false; }
    },
    ...
    /** The answering entry's context size (see AgentService.entryNumCtx). */
    _numCtx() {
        try {
            const entry = this._entry();
    ...
    async ensure(ticker, { manual = false, force = false } = {}) {
        ...
        if (!force && this.current(have)) return have;
    ...
    async _generate(subject) {
        ...
        const verdicts = this.parseVerdicts(clean, { fund: sheet.fund }).verdicts;
        const entry = { day: today, at: Date.now(), model: this._modelLabel(), name: sheet.name, fund: sheet.fund, text: clean, verdicts, format: this.FORMAT, error: null, corrections: notes.length };
    ...
    scheduleSweep() {
        ...
        if (!this.settingOn()) return;
        if (!this.autoAllowed()) return;
        if (typeof AgentService !== 'undefined' && this._metered()) return;
    ...
    async sweep() {
        ...
        const missing = this.sweepTickers().filter(s => !this.current(this.cache()[s]));
        ...
        try {
            ...
            while (this._sweep.queue.length) {
                if (!this.autoAllowed() || this._metered()) break;   // the gate can change mid-sweep
                await this._waitForQuietChat();
    ...
    /** Settings › Portfolio; see PortfolioBrief.settingOn for why it is
     *  separate from autoAllowed(). */
    settingOn() {
        try { return PortfolioApp.aiWritingEnabled(); } catch { return true; }
    },
    OFF_ERROR: 'Turned off in Settings › Portfolio ("Generate stock profile using AI").',
    ...
    renderForTicker(ticker) {
        ...
        // Off: nothing written, nothing shown — a profile carries its day on
        // it and a stale one reads as current.
        if (!this.settingOn()) { el.innerHTML = ''; return; }
    ...
    _sectionHtml(subject, { entry, partial, running }) {
        ...
        // The prose shows without its Verdicts lines — they become the tiles.
        const prose = (t) => this._html(this.stripVerdicts(t));
        // Indicators: the entry's own (stamped at write time), else lifted
        // off an older text on the spot; a streaming partial shows what has
        // arrived so far (the section is the tail, so usually nothing yet).
        let verdicts = null, verdictsFund = !!entry?.fund;
        if (running && partial) verdicts = this.parseVerdicts(partial, { fund: verdictsFund }).verdicts;
        else if (entry?.text) verdicts = entry.verdicts || this.parseVerdicts(entry.text, { fund: verdictsFund }).verdicts;
        if (running) {
            meta = partial ? `Writing today’s profile with ${esc(this._modelLabel())}…` : 'Reading the numbers…';
            if (partial) body = prose(partial);
            else if (entry?.text) { body = prose(entry.text); dim = true; }
        ...
        // The head row is the collapse door (see PortfolioBrief).
        const collapsed = this._collapsed();
        return `
            <div class="portfolio-profile-head is-collapsible" data-profile-collapse role="button" tabindex="-1">
                <h4 class="portfolio-ticker-section-title">
                    <button type="button" class="portfolio-profile-chevron" aria-expanded="${collapsed ? 'false' : 'true'}" aria-label="${collapsed ? 'Show' : 'Hide'} the business profile"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg></button>
                    Business profile · ${esc(label)}
                </h4>
                <span class="portfolio-profile-meta">${meta}</span>
            </div>
            ${this._verdictsHtml(verdicts, verdictsFund, { dim: dim || (running && !!entry?.text && !partial) })}
    ...
    /**
     * The indicator tiles: one per verdict the profile gave, in spec order.
     * A scale fills a meter up to the word; a categorical verdict wears the
     * word as a chip; Unknown renders muted with an empty meter. Rendered
     * above the prose and kept when the section is collapsed — they ARE the
     * short read. Nothing when the profile gave no verdicts.
     */
    _verdictsHtml(verdicts, fund, { dim = false } = {}) {
        if (!verdicts || !Object.keys(verdicts).length) return '';
        const esc = AppManager.escapeHtml;
        const tiles = this.VERDICTS.map(spec => {
            const word = verdicts[spec.key];
            if (!word) return '';
            const options = this.verdictOptions(spec, fund);
            const unknown = word === 'Unknown';
            const tone = unknown ? 'ink' : (spec.tone[word] || 'ink');
            const idx = options.indexOf(word);
            const meter = spec.categorical || (idx < 0 && !unknown) ? '' :
                `<span class="pf-verdict-meter" aria-hidden="true">${options.map((o, i) => `<span class="pf-verdict-seg${!unknown && i <= idx ? ' is-on' : ''}"></span>`).join('')}</span>`;
            const title = unknown ? `${spec.label}: the fact sheet could not say` : `${spec.label}: ${word} (${options.join(' · ')}${spec.extra ? ' · ' + spec.extra.join(' · ') : ''}) — open the section`;
            return `<button type="button" class="pf-verdict${unknown ? ' is-unknown' : ''}${spec.categorical ? ' is-chip' : ''}" data-tone="${tone}" data-verdict="${spec.key}" title="${esc(title)}">
                <span class="pf-verdict-label">${esc(spec.label)}</span>
                <span class="pf-verdict-word">${esc(unknown ? 'Not on the sheet' : word)}</span>
                ${meter}
            </button>`;
        }).join('');
        return tiles ? `<div class="pf-verdicts${dim ? ' is-stale' : ''}" role="list" aria-label="Profile verdicts">${tiles}</div>` : '';
    },

    _bind(el, subject, ticker) {
        // A tile opens the prose at the section it sums up.
        el.querySelectorAll('[data-verdict]').forEach(btn => btn.addEventListener('click', () => {
            const spec = this.VERDICTS.find(v => v.key === btn.dataset.verdict);
            if (this._collapsed()) this._setCollapsed(false);
            if (!this._expanded()) this._setExpanded(true);
            this.renderForTicker(ticker);
            const body = document.querySelector('#portfolio-ticker-profile .portfolio-profile-body');
            const heading = spec && body ? [...body.querySelectorAll('h1,h2,h3,h4')].find(h => spec.section.test(h.textContent.trim())) : null;
            (heading || body)?.scrollIntoView({ block: 'start', behavior: 'smooth' });
        }));
        el.querySelector('[data-profile-collapse]')?.addEventListener('click', () => {
