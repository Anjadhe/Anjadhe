...
const PortfolioBrief = {
    ...
    // Collapsed from the head row: the body and its actions are hidden, the
    // heading and meta line stay (still says when it was written). Per-Mac,
    // like the fold — a reading preference, not data.
    COLLAPSED_KEY: 'portfolio-brief-collapsed',
    ...
    /**
     * Settings › Portfolio › "Generate stock profile using AI". Separate from
     * autoAllowed() on purpose: that one gates AUTOMATIC runs and a manual
     * click goes through it, while this gates EVERY run including the user's
     * own and the assistant's tool — "don't generate" means don't.
     */
    settingOn() {
    ...
    // Every question below is asked of the model that answers THIS
    // surface, not of the brain (model-routing.js R4).
    autoAllowed() {
    ...
    renderForOverview() {
        ...
        // Off: show nothing at all. A brief is dated, and yesterday's read as
        // today's is worse than none — which is why the switch hides as well
        // as stops.
    ...
    _sectionHtml({ entry, partial, running }) {
        ...
        // The head row is the collapse door: a chevron beside the title,
        // the whole row clickable. Collapsed hides body + actions and keeps
        // the meta line, so a folded brief still says when it was written.
        const collapsed = this._collapsed();
        return `
            <div class="portfolio-profile-head is-collapsible" data-brief-collapse role="button" tabindex="-1">
                <h4 class="portfolio-ticker-section-title">
                    <button type="button" class="portfolio-profile-chevron" aria-expanded="${collapsed ? 'false' : 'true'}" aria-label="${collapsed ? 'Show' : 'Hide'} today’s brief"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg></button>
                    Today’s brief · market and portfolio
                </h4>
    ...
    _bind(el) {
        el.querySelector('[data-brief-collapse]')?.addEventListener('click', () => {
