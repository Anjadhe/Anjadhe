...
const PortfolioStrategy = {
    ...
    save(patch = {}) {
        ...
        // A renamed strategy carries its review routine (and any routine
        // quoting the old name) along — same recipe as goal renames.
        if (changed.includes('name') && !isNew && typeof ReviewRoutines !== 'undefined') {
            const prefix = (typeof PortfolioStrategyPage !== 'undefined')
                ? PortfolioStrategyPage.REVIEW_PREFIX : 'Strategy Review: ';
            ReviewRoutines.syncRename(prefix, oldName, strategy.name);
        }
        return strategy;
    },

    _makeDefault(list, strategy) {
        // Stamp every record whose flag flips — record-level sync resolves
        // each strategy by updatedAt, and an unstamped isDefault change
        // would lose to the other Mac's copy (two "overall" strategies).
        const now = new Date().toISOString();
        for (const s of list) {
            const v = (s === strategy);
            if (s.isDefault !== v) { s.isDefault = v; s.updatedAt = now; }
        }
    },
    ...
    remove(id) {
        ...
        for (const account of (PortfolioApp.accounts || [])) {
            if (account.strategyId === id) { account.strategyId = null; account.updatedAt = now; }
        }
        if (wasDefault && list.length) { list[0].isDefault = true; list[0].updatedAt = now; }
        PortfolioApp.strategies = list;
        PortfolioApp.saveData();
        return true;
    },
