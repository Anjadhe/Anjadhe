...
(function registerPortfolioTools() {
    ...
    const VOCAB = /\b(portfolios?|stocks?|tickers?|shares?|holdings?|invest\w*|net\s?worth|dividends?|strateg\w+|allocations?|rebalanc\w+|diversif\w+|asset\s?mix|risk\s?tolerance|accounts?|transactions?|trades?|brokerages?|401k|403b|ira|hsa|robinhood|fidelity|e-?trade|schwab|vanguard|plaid)\b/;
    ...
    if (typeof PortfolioBrokerage !== 'undefined' && PortfolioBrokerage.available()) reg({
        ...
        if (!linked.length) return { linked: [], hint: 'No brokerage is linked. In Portfolio, click "+ Link brokerage" in the account list to connect one through Anjadhe Cloud.' };
    ...
    reg({
        ...
        return {
            ...
            note: (res && res.error) ? `Today's brief could not be written (${res.error}); this is the one from ${entry.day}.` : `Written ${PortfolioBrief.isToday(entry) ? 'today' : 'on ' + entry.day}; every number in it came from the app's own figures and that day's quotes. It describes and never prescribes; keep that stance when relaying it.`,
            brief: entry.text
        };
    }, { readOnly: true });
