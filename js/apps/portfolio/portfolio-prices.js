...
const PriceFetcher = {
    ...
    async fetchPrices(tickers, priceCache = {}) {
        ...
        // A symbol nobody could price (delisted, a broker's private fund, a
        // sandbox ticker) gets a dated MISSING marker, not silence: with no
        // entry it stayed "stale" forever, and render → autoRefresh →
        // refetch → render spun the page (2026-09-09, the first linked
        // brokerage). Readers already treat a null price as unpriced; the
        // marker ages out on the normal TTL, so a listing that appears
        // later is picked up.
        remaining.forEach(ticker => {
            updated[ticker.toUpperCase()] = { price: null, change: 0, changePercent: 0, updatedAt: now, missing: true };
        });
    ...
    async fetchCompanyInfo(ticker) {
        try {
            ...
            return {
                ...
                type: quoteType.quoteType || null, // EQUITY / ETF / MUTUALFUND — PortfolioNews skips funds in collated feeds
