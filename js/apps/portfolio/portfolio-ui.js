...
const PortfolioUI = {
    ...
    renderNav(scope) {
        ...
        const record = (kind, id, name, value) => `
            <button type="button" class="portfolio-nav-item" data-record="${kind}" data-id="${esc(id)}">
                <span class="portfolio-nav-name">${esc(name)}</span>
                <span class="portfolio-nav-value">${this.hv(this.formatMoney(value))}</span>
            </button>`;
        ...
        nav.innerHTML = `
            ...
            ${properties.map(p => record('property', p.id, p.name, p.currentValue || 0)).join('')}
            ...
            ${liabilities.map(l => record('liability', l.id, l.name, l.balance || 0)).join('')}
            ...
            ${row('strategy', 'Strategy', null, scope === 'strategy')}
        `;
    ...
    mastheadHtml(summary) {
        ...
        const dayPct = summary.totalDayBase > 0 ? ` (${this.formatPercent(summary.totalDayChangePercent)})` : '';
        const perf = `
            <span class="${this.hvPlClass(summary.totalDayChange)}">${this.hv(this.formatPL(summary.totalDayChange))}${dayPct} today</span>
            <span class="portfolio-masthead-sep">&middot;</span>
            <span class="${this.hvPlClass(summary.totalPL)}">${this.hv(this.formatPL(summary.totalPL))} (${this.formatPercent(summary.totalPLPercent)}) all time</span>`;
        ...
        if (classes.length > 1 || hasDebt) {
            const bar = classes.length > 1 ? `
                <div class="portfolio-masthead-bar" aria-hidden="true">
                    ${classes.map(c => `<span class="portfolio-masthead-seg is-${c.key}" style="flex-basis:${(c.value / assets) * 100}%"></span>`).join('')}
                </div>` : '';
            const legend = classes.map((c, i) => `
                <span class="portfolio-masthead-term">
                    ...
                    <span class="portfolio-masthead-term-value">${this.hv(this.formatMoney(c.value))}</span>
                </span>`);
            if (hasDebt) legend.push(`
                <span class="portfolio-masthead-term is-debt">
                    ...
                    <span class="portfolio-masthead-term-value">${this.hv(this.formatMoney(summary.liabilitiesTotal))}</span>
                </span>`);
            composition = `${bar}<div class="portfolio-masthead-legend">${legend.join('')}</div>`;
        }

        return `
            ${hasDebt ? '<div class="portfolio-masthead-eyebrow">Net worth</div>' : ''}
            <div class="portfolio-masthead-value ${hasDebt && headline < 0 ? 'pl-negative' : ''}">${this.hv(this.formatMoney(headline))}</div>
            <div class="portfolio-masthead-stats">${perf}</div>
    ...
    renderLiabilitiesSection(liabilities) {
        ...
        const html = `
            <div class="portfolio-liabilities-section">
                <h4 class="portfolio-ticker-section-title">Liabilities</h4>
                <table class="portfolio-table">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ${sorted.map(l => `
                            <tr class="portfolio-row portfolio-liability-row" data-liability-id="${l.id}">
                                <td class="portfolio-ticker">${AppManager.escapeHtml(l.name)}</td>
    ...
    renderWatchlistSection() {
                    ...
                    <tbody>
                        ${items.map(w => {
                            ...
                            <tr class="portfolio-row portfolio-watchlist-row" data-ticker="${esc(w.ticker)}">
                                <td class="portfolio-ticker">${esc(w.ticker)}</td>
        ...
        container.insertAdjacentHTML('beforeend', `
            <div class="portfolio-watchlist-section">
                <div class="portfolio-watchlist-head">
