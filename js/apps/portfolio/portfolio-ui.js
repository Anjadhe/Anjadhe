...
const PortfolioUI = {
    ...
    renderScopeHeader(account) {
        ...
        if (header) {
            ...
            const st = (account && typeof PortfolioBrokerage !== 'undefined') ? PortfolioBrokerage.statusLine(account) : null;
            if (linkedEl) {
                linkedEl.hidden = !st;
                linkedEl.textContent = st ? st.text : '';
    ...
    renderTickerDetail(ticker, holding, byAccount, companyInfo, priceData, tickerHistory, hideValues) {
        ...
        if (optMeta) {
            ...
            html += '<div class="portfolio-company-card">';
            html += `<div class="portfolio-company-head">${this.tickerMono(ticker)}<div><h3 class="portfolio-company-name">${AppManager.escapeHtml(companyInfo.name)}</h3><div class="portfolio-company-sym">${AppManager.escapeHtml(ticker)}</div></div></div>`;
