...
const PortfolioUI = {
    ...
    /**
     * Top-level account tabs — "All Accounts" (the dashboard) plus one tab
     * per account. Rendered on the main view AND the account view (each has
     * its own strip container) so switching accounts never loses the tabs.
     */
    renderTopTabs(containerId, activeAccountId = null) {
        const container = document.getElementById(containerId);
        if (!container) return;
        const accounts = PortfolioApp.getAccounts();
        // A tab strip with a single "All Accounts" entry is noise.
        ...
        container.innerHTML = [
            `<button class="portfolio-tab ${activeAccountId ? '' : 'active'}" data-account-tab="all" type="button">All Accounts</button>`,
            ...accounts.map(a =>
                `<button class="portfolio-tab ${a.id === activeAccountId ? 'active' : ''}" data-account-tab="${a.id}" type="button">${AppManager.escapeHtml(a.name)}</button>`)
        ].join('');
        ...
        container.querySelectorAll('[data-account-tab]').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (target === 'all') {
                    if (PortfolioApp.viewingAccountId) PortfolioApp.closeAccountDetail();
                } else if (target !== PortfolioApp.viewingAccountId) {
                    PortfolioApp.openAccountDetail(target);
                }
            });
        });
    },
