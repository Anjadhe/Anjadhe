...
const PortfolioNews = {
    ...
    // An entry with no rows is re-asked sooner: an empty list is as likely
    // a failed fetch as a quiet ticker, and 30 minutes of nothing is what
    // the user sees when it was the former.
    EMPTY_TTL_MS: 5 * 60 * 1000,
    _fresh(entry) {
        if (!entry) return false;
        return (Date.now() - entry.at) < (entry.items?.length ? this.TTL_MS : this.EMPTY_TTL_MS);
    },

    // The last fetch's failure message (null after a clean one). Shown in
    // place of an empty section, and returned to the assistant's tool, so
    // "no headlines" and "could not fetch" never read the same. Nothing is
    // fetched from this Mac around a Connect failure (docs/DISCOVER.md).
    _lastError: null,
    _failureMessage(r) {
        if (r && typeof r.error === 'string' && r.code === 'connect') return r.error;
        if (r && r.route === 'connect') return 'Anjadhe Connect could not fetch headlines right now.';
        return 'The news feed could not be reached right now.';
    },
    ...
    async _ensure(tickers) {
        ...
        const run = (async () => {
            ...
            for (const r of results) {
                if (!r || r.error || !Array.isArray(r.topics)) { this._lastError = this._failureMessage(r); continue; }
                // A topic the server could not fetch (`error`) is a miss,
                // not an empty result: null keeps yesterday's rows below.
                for (const t of r.topics) {
                    if (t.error) this._lastError = this._failureMessage(r);
                    byTopic.set(String(t.topic || '').trim().toLowerCase(), t.error ? null : (t.items || []));
                }
            }
            const now = Date.now();
            stale.forEach((ticker, i) => {
                const items = byTopic.get(topics[i].trim().toLowerCase());
                if (!items) return; // that chunk or topic failed: keep the old entry
                const rows = [];
        ...
        el.querySelectorAll('[data-news-i]').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                if (news) {
                    ...
                    AppManager.openExternal(it.url);
                }
            });
        });
    },
    ...
    async _render(el, tickers, { limit, chips, back, heading = 'News' }) {
        ...
        const paint = (loading) => {
            ...
            // Nothing cached and the fetch failed: say so, in the section's
            // place — a vanished section reads as "no news", which is not
            // what happened. Genuinely no headlines still renders nothing.
            if (!rows.length && !loading && !this._lastError) { el.innerHTML = ''; return; }
            const empty = loading ? 'Looking for headlines&hellip;' : AppManager.escapeHtml(this._lastError || '');
            el.innerHTML = `
                <h4 class="portfolio-ticker-section-title">${heading}</h4>
                ${rows.length ? this._rowsHtml(rows, { chips }) : `<p class="portfolio-news-empty">${empty}</p>`}`;
            this._bind(el, rows, back);
        };
        paint(true);
        try { await this._ensure(tickers); } catch { this._lastError = this._failureMessage(null); }
        paint(false);
    },
    ...
    async headlines({ tickers = null, accountId = null, limit = 20 } = {}) {
        ...
        try { await this._ensure(subjects); } catch { this._lastError = this._failureMessage(null); }
        return { tickers: subjects, items: this._collate(subjects, limit), fetchError: this._lastError };
    },
