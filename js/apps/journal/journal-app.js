...
const JournalApp = {
    ...
    // Individually collapsible panes — the Notes recipe (2026-08-09),
    // carried here 2026-08-11. Preference is per-Mac (localStorage, the
    // side-nav precedent): pane layout depends on the monitor, so it must
    // not ride the sync journal.
    PANES: {
        nav: { cls: 'journal-nav-collapsed', btn: 'journal-toggle-nav-btn', store: 'journal-nav-collapsed', label: 'filters' },
        list: { cls: 'journal-list-collapsed', btn: 'journal-toggle-list-btn', store: 'journal-list-collapsed', label: 'timeline' }
    },

    setPaneCollapsed(pane, collapsed, opts = {}) {
        const def = this.PANES[pane];
        if (!def) return;
        document.getElementById('journal-view')?.classList.toggle(def.cls, !!collapsed);
        if (!opts.transient) {
    ...
    _paneCollapsed(pane) {
        const def = this.PANES[pane];
        return !!def && !!document.getElementById('journal-view')?.classList.contains(def.cls);
    },

    /** Re-assert the per-Mac preference on entry to the app. */
    ...
    //
    // The Inbox / Email AI / Notes control (Notes got it 2026-08-10): the
    // entry pane is docked beside the timeline by default on a big monitor
    // (off by default on a laptop-sized display — UIUtils.readerPaneDefault,
    // 2026-09-02); turning it off
    // gives the timeline the full width, and opening an entry then swaps
    // to a full-width entry with a way back above it. Per-Mac localStorage
    // like the pane collapse state. Under the 981px stacked layout the
    // preference is moot (the CSS is scoped to the wide frame) and the
    // toggle hides.

    READER_KEY: 'journal-reader-pane',
    ...
    toggleReaderPane() {
        ...
        // The FYI guard, carried over: hiding the pane while an entry is
        // open would answer "no pane" with a full-width entry — drop back
    ...
    /** Display name of the list the current filters show — what the
     *  full-width entry's back chip promises to return to. Narrowest
     *  active filter names the list (a tag beats a mood beats the date
     *  range), same spirit as the FYI back row naming its folder. */
    _listLabel() {
        if (this.tagFilter) return `#${this.tagFilter}`;
        if (this.moodFilter) {
            const info = (typeof JournalUI !== 'undefined' && JournalUI.getMoodInfo)
                ? JournalUI.getMoodInfo(this.moodFilter) : null;
            return info?.label || this.moodFilter;
        }
        if (this.filterBy === 'week') return 'This Week';
        if (this.filterBy === 'month') return 'This Month';
        return 'All entries';
    },

    /** Reflect the preference and the open-entry state on the view + toggle.
     *  `journal-reader-off` widens the timeline; `journal-reading` (pane
     *  off AND an entry session live — a brand-new draft counts) swaps to
     *  the entry. */
    _syncReaderState() {
        const view = document.getElementById('journal-view');
        if (!view) return;
        const off = !this._readerPref();
        view.classList.toggle('journal-reader-off', off);
        view.classList.toggle('journal-reading', off && !!this._paneSessionActive);
        const btn = document.getElementById('journal-reader-toggle');
        if (btn) {
            btn.classList.toggle('is-active', off);
            btn.setAttribute('aria-pressed', String(off));
            btn.title = `${off ? 'Show' : 'Hide'} reading pane`;
        }
        const back = document.getElementById('journal-reader-back');
        if (back) back.innerHTML = `&#8592; ${UIUtils.escapeHtml(this._listLabel())}`;
    },

    /** One door for every sidebar pick (date filter, mood, tag). With the
     *  reading pane off, picking a list while an entry has the full width
     *  means "show me that list" — the entry closes; with the pane on,
     *  the timeline is already beside the entry and the selection stays.
     *  (The Notes setFilter rule.) */
    _afterFilterPick() {
        if (!this._readerPref() && this._paneSessionActive) {
            this.flushSave();
            this._clearSelection();   // renders
            return;
        }
        this.render();
    },
    ...
    init() {
        ...
        // Entering the app restores the user's own pane preferences.
    ...
    setupEventListeners() {
        ...
        // Pane collapse toggles + reading pane + the full-width entry's
        // way back (its label promises the list, so it always lands there).
        replaceBtn('journal-toggle-nav-btn', () => this.setPaneCollapsed('nav', !this._paneCollapsed('nav')));
        replaceBtn('journal-toggle-list-btn', () => this.setPaneCollapsed('list', !this._paneCollapsed('list')));
        replaceBtn('journal-reader-toggle', () => this.toggleReaderPane());
        replaceBtn('journal-reader-back', () => {
            this.flushSave();
            this._clearSelection();
        });
    ...
    updateSaveStatus(status, { quiet = false } = {}) {
        // The shared SaveStatus shape; legacy vocabulary 'unsaved' → dirty.
        const state = status === 'unsaved' ? 'dirty' : status;
        if (typeof SaveStatus !== 'undefined') SaveStatus.mark('journal-save-status', state, '', { quiet });
        if (status === 'saved') this.hasUnsavedChanges = false;
    },

    /**
     * SaveStatus registration for the live pane session — lets navigation,
     * window hide and the unload guard flush the entry from anywhere.
     */
    _registerSaveStatus() {
        if (typeof SaveStatus === 'undefined') return;
        SaveStatus.register('journal', {
            isDirty: () => this._paneSessionActive && (this.hasUnsavedChanges
                || (RichEditor.editor?.id === 'journal-content-input' && !!RichEditor.autoSaveTimeout)),
            flush: () => this.flushSave(),
            label: () => {
                const d = document.getElementById('journal-date-display')?.textContent?.trim();
                return d ? `Journal entry for ${d}` : 'This journal entry';
            },
            reopen: () => {
                AppManager.openApp('journal');
                if (this.currentEntryId) this.openEditor(this.currentEntryId);
            }
        });
    },

    /**
     * Auto-save current entry: mark dirty, (re)arm ONE cancellable timer.
     */
    autoSave() {
        if (!(this.currentEntryId || RichEditor.getText().trim().length > 0)) return;
        this.updateSaveStatus('unsaved');
    ...
    /**
     * Write now (see NotesApp.flushSave — same contract, same reason).
     */
    flushSave() {
        if (!this._paneSessionActive) return { ok: true };
        if (typeof RichEditor !== 'undefined' && RichEditor.editor?.id === 'journal-content-input') RichEditor.flushPending();
        if (this._autoSaveTimer) { clearTimeout(this._autoSaveTimer); this._autoSaveTimer = null; }
    ...
    _loadEntryIntoPane(entryId = null) {
        ...
        const bodyEl = document.getElementById('journal-content-input');
    ...
    _teardownEntrySession() {
        ...
        if (this._autoSaveTimer) { clearTimeout(this._autoSaveTimer); this._autoSaveTimer = null; }
        if (typeof SaveStatus !== 'undefined') SaveStatus.unregister('journal');
        RichEditor.destroy();
    ...
    saveCurrentEntry(silent = true) {
        ...
        if (this.currentEntryId) {
            this.updateEntry(this.currentEntryId, { date, content, mood, tags });
        } else if (!empty) {
            // A draft becomes a real entry on its first non-empty save.
            const newEntry = this.createEntry({ date, content, mood, tags });
