...
const JournalUI = {
    ...
    render(entries) {
        ...
        this.renderSidebar(JournalApp.entries);
        this.renderPaneWelcome();

        if (!listContainer) return;
        if (entries.length === 0) {
            // Clear, don't just hide: the rows that no longer match stayed in
            // the DOM behind the empty state, so anything reading the list
            // (tests, the next attachEventListeners, a11y tooling) still saw
            // the pre-filter set.
    ...
    /**
     * First-run welcome in the entry pane (the News-app recipe): shown only
     * while the journal holds NO entries at all; swapped back to the plain
     * "Select an entry" placeholder once the first entry exists.
     */
    renderPaneWelcome() {
        const host = document.getElementById('journal-entry-empty');
        if (!host) return;
        const none = !(JournalApp.entries || []).length;
        // The welcome IS the page while there are no entries: the view
        // flag lets the reader-off layout (the laptop default) show the
        // pane instead of an empty timeline (journal.css).
        document.getElementById('journal-view')?.classList.toggle('journal-welcome', none);
        if (!none) {
            if (host.dataset.welcome) {
                delete host.dataset.welcome;
                host.innerHTML = `<p class="journal-entry-empty-title">Select an entry</p>
                    <p class="journal-entry-empty-hint">&hellip;or start today&rsquo;s with + New Entry.</p>`;
            }
            return;
        }
        ...
        host.innerHTML = UIUtils.appWelcome({
            title: 'A private daily record',
            lede: 'Write the day down — what happened, how it felt — and it stays with you, on your Mac.',
            cta: '<button id="journal-welcome-new" class="primary-btn" type="button">+ New Entry</button>',
            rows: [
                ['A page for the day',
                 'A full editor with a mood on every entry and tags for the threads that run through your life — filter the timeline by date, mood, or tag.'],
                ['Look back with the assistant',
                 'Ask &ldquo;what was on my mind in June?&rdquo; — the assistant can read back through your journal, and can write an entry for you when you tell it one.'],
                ['Yours alone',
                 'No journaling service, no cloud account — entries are stored in the app on your Mac.']
            ]
        });
        host.querySelector('#journal-welcome-new')?.addEventListener('click', () =>
