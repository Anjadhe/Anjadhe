...
const FocusUI = {
    ...
    /**
     * No breadcrumb TRAIL on Plan: the left nav names the selection and is
     * always up (clicking an area/group is the way back from any detail,
     * including the embedded task editor), and the goal detail's in-pane
     * eyebrow links back to its area — a chrome trail would just repeat
     * what the page already says (same rule as the Tasks tab).
     *
     * The app title itself does stay, though. Hiding it left Actions as the
     * only page in the app with an unnamed header.
     */
    renderBreadcrumb() {
        const title = document.querySelector('#focus-view > .app-header-bar > .app-view-title');
        if (title) title.style.display = '';
        Breadcrumb.render('focus-breadcrumb', [{ label: 'Actions' }]);
    },
    ...
    _renderUnassignedGoals() {
        ...
        const unassigned = GoalsApp.goals || []
    ...
    _attachGoalsTasksListeners(container, focusId) {
        ...
        // list's new-task context stays unambiguous. The trailing
        // no-goal-id card is the "Other Tasks" bucket.
     ...
     * open in-pane, × deletes the task, "+ New Task" links to the goal
     * and its focus area.
     */
