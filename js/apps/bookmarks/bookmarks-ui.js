...
const BookmarksUI = {
        ...
        return `
            <div class="bookmark-card" data-id="${bookmark.id}">
                <div class="bookmark-card-header">
                    ...
                    <button class="bookmark-info-btn" data-info="${bookmark.id}" title="Details" aria-label="Bookmark details">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 16v-5"/><path d="M12 8h.01"/></svg>
                    </button>
                </div>
    ...
    // A bookmark behaves like a link: clicking the card opens the page in
    // their default browser. The info button (and cards with no URL) opens
    // the detail view — description, tags, edit, delete.
    attachCardListeners(app) {
        document.querySelectorAll('.bookmark-card, .bookmarks-list-item').forEach(card => {
            card.addEventListener('click', (e) => {
                if (e.target.closest('.bookmark-info-btn')) {
                    app.openViewer(card.dataset.id);
                    return;
                }
                const bookmark = app.bookmarks.find(b => b.id === card.dataset.id);
                if (bookmark?.url) app.openInBrowser(bookmark.url);
                else app.openViewer(card.dataset.id);
            });
        });
    },
