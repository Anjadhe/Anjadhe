...
const BookmarksApp = {
    ...
    // Open a bookmark in the in-app Browse sub-app with an iOS-style
    // "Back to Bookmarks" strip — returns to the bookmark's detail page
    // when one was open, otherwise the list.
    openInBrowser(url) {
        if (!url) return;
        if (typeof AppManager !== 'undefined' && AppManager.openInBrowse) {
            const viewerId = this.currentBookmarkId;
            AppManager.openInBrowse(url, {
                label: 'Back to Bookmarks',
                onBack: () => {
                    AppManager.openApp('bookmarks');
                    if (viewerId) setTimeout(() => this.openViewer(viewerId), 60);
                }
            });
            return;
        }
        // Fallback for a partial environment: plain Browse handoff.
