...
const BookmarksApp = {
    ...
    // Open a bookmark in the Mac's default browser, where the user is
    // already signed in to the sites they saved.
    openInBrowser(url) {
        if (!url) return;
        if (typeof AppManager !== 'undefined') AppManager.openExternal(url);
        else window.electronAuth?.openExternal?.(url);
    },
...
// fetching the page (read_url handles full-page extraction).
if (typeof AgentContext !== 'undefined') {
    AgentContext.register('bookmarks', () => {
        ...
        return {
            ...
            body: `The user is viewing or editing the bookmark below. Opening it sends the page to their default browser.
