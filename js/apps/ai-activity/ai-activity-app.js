...
const AIActivityApp = {
    ...
    render() {
        ...
        if (!view.dataset.bound) {
            ...
            const openFromRow = (e) => {
                // Stop lives on the row but is not the row: it ends the work
                // behind it (the routine, the task, the chat turn).
                const stopBtn = e.target.closest('[data-stop]');
                if (stopBtn) {
                    e.stopPropagation();
                    const ok = AIActivity.stop(stopBtn.dataset.stop);
                    if (!ok && typeof UIUtils !== 'undefined') UIUtils.showToast('Nothing left to stop — it already ended', 'info');
                    else if (typeof UIUtils !== 'undefined') UIUtils.showToast('Stopping…', 'info');
                    return;
                }
    ...
    _renderStatus() {
        ...
        if (loading.length) {
            ...
            const labels = [...new Set(requests.map(r => this._title(r)))];
            const running = requests.filter(r => r.status !== 'queued');
            const queued = requests.length - running.length;
            // Say what's actually running vs. waiting — "3 tasks are running"
            // over three queued rows reads as a contradiction.
            if (!running.length) {
                headline = requests.length === 1
                    ? `${labels[0]} is waiting to run`
                    : `${requests.length} AI tasks are waiting to run`;
                sub = 'Queued behind other AI work — background jobs also hold back briefly after a chat so they don’t get in your way.';
            } else {
                headline = requests.length === 1
                    ? `${labels[0]} is running`
                    : (queued
                        ? `${running.length} AI task${running.length === 1 ? '' : 's'} running, ${queued} waiting`
        ...
        if (models === null) {
            ...
            const where = entry.engine === 'openai' ? 'the OpenAI API'
                ...
                : entry.engine === 'anjadhe' ? 'Anjadhe Cloud'
                : 'your server';
    ...
    _renderRecent() {
        ...
        this._syncList(el, AIActivity.recent, false);
    },

    // Row nodes must SURVIVE a re-render. This page repaints every second
    // (ticker) and on every activity event — near-continuous while the
    // email drain runs — and a wholesale innerHTML swap replaces the row
    // between the user's mousedown and mouseup. The click then fires on
    // the nearest surviving common ancestor — the LIST CONTAINER — where
    // closest('[data-uid]') finds no row, so the detail modal "never
    // opened" (reported 2026-08-30). Keeping the row element stable is
    // enough: patching its CONTENTS mid-click still lands the click on
    // the row itself. Full rebuild only when the row set changes.
    _syncList(el, items, isActive) {
        const rows = [...el.children].filter(r => r.dataset && r.dataset.uid !== undefined);
        const same = rows.length === items.length && items.length > 0
            && rows.every((r, i) => r.dataset.uid === String(items[i].uid || ''));
        if (!same) {
            el.innerHTML = items.map(item => this._renderRow(item, isActive)).join('');
            return;
        }
        const tpl = document.createElement('template');
        items.forEach((item, i) => {
            tpl.innerHTML = this._renderRow(item, isActive).trim();
    ...
    _renderRow(item, isActive) {
        ...
        // The mark that matters on this page: this request left the Mac.
        // Local rows stay unmarked so the mark means something.
        const left = item.kind === 'request' && item.local === false
            ? `<span class="aiact-chip aiact-chip--left" title="This request left this Mac">&#8599; ${this._esc(this._destinationShort(item))}</span>` : '';
        const errLine = (item.status === 'failed' || item.status === 'lost') && item.error
            ? `<div class="aiact-error">${this._esc(String(item.error).slice(0, 160))}</div>` : '';
        // A row that names its owner can be stopped from here — the
        // routine, task or chat turn behind it (AIActivity.stop).
        const stop = isActive && item.runId
            ? `<button class="aiact-stop" type="button" data-stop="${this._esc(item.runId)}" title="Stop this run">Stop</button>` : '';
        ...
        return `
            <div class="aiact-row is-clickable" data-uid="${this._esc(item.uid || '')}" title="Show details">
                ...
                <div class="aiact-main">
                    <div class="aiact-label">${this._esc(this._title(item))} ${chip} ${left}</div>
                    ${item.desc ? `<div class="aiact-desc">${this._esc(item.desc)}</div>` : ''}
                    ${errLine}
                </div>
                <div class="aiact-meta">${metaBits.join(' · ')}${stop}</div>
            </div>
        `;
    },

    // "Routine · Daily News Summary" — the tag's label plus what the caller
    // said the work is about. The label alone named every routine "Routine".
    _title(item) {
        return item.subject ? `${item.label} · ${item.subject}` : item.label;
    },

    _destinationShort(item) {
        return item.engine === 'anjadhe' ? 'Anjadhe Cloud'
            : item.engine === 'openai' ? 'OpenAI'
            : item.engine === 'anthropic' ? 'Anthropic'
            : 'Your server';
    },
    ...
    _openDetail(item) {
        if (typeof Modal === 'undefined' || !Modal.create) { console.warn('[aiactivity] Modal component missing — cannot open detail'); return; }
        try {
            this._openDetailInner(item);
        } catch (e) {
            // A silent throw here reads as "the modal never opens" — say so.
            console.error('[aiactivity] detail modal failed:', e);
        }
    },

    _openDetailInner(item) {
        ...
        const statusText = {
            ...
            failed: 'Failed',
            stopped: 'Stopped',
            lost: 'Did not finish — the app was closed while this ran',
            blocked: 'Skipped — this data stays on this Mac (Settings › AI Assistant › Cloud Privacy)'
        }[item.status] || item.status;

        const rows = [];
        const add = (label, value) => { if (value) rows.push({ label, value }); };

        add('What', `${this._title(item)}${item.desc ? ' — ' + item.desc : ''}`);
        add('Subject', item.subject || null);
        ...
        add('Runs on', item.kind === 'engine' ? 'This Mac (llama.cpp)'
            ...
            : item.engine === 'anjadhe' ? 'Anjadhe Cloud (api.anjadhe.com)'
            : 'Your server');
        ...
        if (item.status === 'lost') {
            // The end is unknown; the last save that still saw it running
            // is a floor on how long it went.
            add('Ran for', item.lastSeenAt ? `at least ${this._elapsed(item.lastSeenAt - item.startedAt)}` : 'unknown');
        } else {
        ...
        // call produced a log entry (calls through LLMLogger do;
        // task-step calls don't create one).
