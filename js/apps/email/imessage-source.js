...
const IMessageSource = {
    ...
    INITIAL_LOOKBACK_DAYS: 30,               // first run: a month back, never the whole history (was 3 until 2026-09-10 — "only five items" read as a bug). Applies only while the cursor is 0; a grown window needs the switch off and on.
    ...
    init() {
        ...
        // The assistant's context line reads a cached iMessage status
        // (send_text, 2026-09-10); warm it so the first turn already knows.
    ...
    async tick(reason = '') {
        ...
        try {
            ...
            // the phone for a month of already-seen texts.
