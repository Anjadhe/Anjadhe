...
(function () {
    ...
    Widgets.register('email-insights', {
        ...
        load() {
            ...
            // Same bargain with the Trips card: a reservation riding a live
            // trip is already on home a few rows away, listed with its
            // whole itinerary. Say it once.
            const onTrip = (typeof EmailTrips !== 'undefined')
                ? EmailTrips.coveredIds(analyses, UIUtils.todayISO())
                : new Set();
            ...
            const unread = Object.entries(analyses)
                ...
                .filter(([id, a]) => a && !a.readAt && !a.rolledUp && !onTrip.has(id)
                    && !EmailApp.representedByTask(id, a, tasked))
