...
const EmailApp = {
    ...
    async loadData() {
        ...
        // Analyses that COMPLETED but produced nothing to show (irrelevant
        // mail, suppressed, disabled type, unparseable output). Without this
        // tombstone map, only insight-producing analyses are remembered — and
        // any event that makes old messages look new again (account
        // reconnect, a rebuilt local cache, historyId expiry) would re-spend
        // an LLM call per already-judged email. messageId → { at, why }.
        this.analyzedNoInsight = data?.analyzedNoInsight || {};
    ...
    saveData() {
        ...
        StorageManager.set('email', {
            ...
            analyzedNoInsight: this.analyzedNoInsight,
    ...
    setupEventListeners() {
        ...
        // Viewer back/reply/forward
        this._bindBtn('email-viewer-back-btn', () => this.closeViewer());
    ...
    async cleanupAccountData(email) {
        ...
        if (Array.isArray(this.emails)) {
            ...
            this.analyzedNoInsight = analyzedNoInsight;
    ...
    async drainAnalysisQueue() {
        ...
        try {
            ...
            while (done < this.ANALYSIS_BATCH_SIZE && this.pendingAnalysisIds.length) {
                ...
                if (!email || this.priorityAnalyses[id] || this.analyzedNoInsight[id]) continue;  // gone or already done
                done++;
                ...
                if (completed) {
                    // Surface the new insight card right away if the user is
                    // sitting on the home feed.
...
Respond ONLY with a JSON object mapping each number to a bundle key, e.g. {"1":"${defs[0]?.key || 'none'}","2":"none"}.`
    ...
    // Show only the inline viewer section within the email layout.
    _showViewerSection() {
    ...
    render() {
        ...
        // When scoped, the breadcrumb names the account so the shorter list
        // is never a mystery; "Email" clears the scope.
        Breadcrumb.render('email-breadcrumb', this.currentAccount
            ? [{ label: 'Email', action: () => this.setAccountScope(null) }, { label: this.currentAccount }]
            : [{ label: 'Email' }]);
