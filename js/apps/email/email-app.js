...
--- END DOCUMENT ---`
    ...
    async _extractReservation(email, analysis, opts = {}) {
        const body = this._plainBody(email).slice(0, this.RESERVATION_BODY_CHARS);
...
- Airport codes are better than city names for from/to when the email gives them.`
    ...
    async analyzeSingleEmail(email) {
        ...
        try {
            ...
            const bodyContent = this._plainBody(email).slice(0, 3000);
...
--- END EMAIL ---`
    ...
    async _extractTransactionsLLM(email) {
        ...
        const bodyContent = this._plainBody(email).slice(0, 4000);
