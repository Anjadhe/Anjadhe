...
--- END DOCUMENT ---`
    ...
    async _extractReservation(email, analysis, opts = {}) {
        const body = (email.bodyText || email.snippet || '').slice(0, this.RESERVATION_BODY_CHARS);
...
- "receipt": money that has ALREADY MOVED — a purchase receipt, "payment received", a card charge that went through, a refund. Nothing left to pay.
...
--- END EMAIL ---`
            ...
            if (result?.error && (result.errorCode === 'rate' || result.errorCode === 'busy')) {
                this._throttledForMs = Math.max(2000,
            ...
            if (result?.message?.content) {
                ...
                // requires analysis.reservation to exist.
                if (type === 'reservation') {
                    try {
                        const reservation = await this._extractReservation(email, analysis);
                        if (reservation) analysis.reservation = reservation;
                    } catch (e) {
