...
const EmailApp = {
    ...
    notifyPriorityEmails(emails) {
        ...
        if (emails.length === 1) {
            ...
            Notify.show(`New insight from ${EmailUI.extractName(e.from)}`, body, { kind: 'email' });
            return;
        }
        ...
        Notify.show(`${emails.length} new email insights`, rows.join('\n'), { kind: 'email' });
    },
...
--- END DOCUMENT ---`
    ...
    async _extractAttendEvent(email, analysis, opts = {}) {
        ...
        const result = await LLMLogger.call('email-event', {
            model: AgentService.model,
            format: 'json',
            ...
            const result = await LLMLogger.call('email', {
                ...
                messages: [
                    ...
                    },
                    {
                        ...
                        role: 'user',
                        content: EmailInsightPrompt.user({ from: email.from, to: email.to, subject: email.subject, date: email.date, body: bodyContent })
                    }
                ],
                stream: false
            });
