...
const EmailApp = {
    ...
    async analyzeSingleEmail(email) {
        ...
        try {
            ...
            for (const t of (scored.types.length ? scored.types : ['general'])) {
...
If you cannot determine a field, use null for that field. Always try to extract what you can.`
    ...
    // How long a message has to stay open before it counts as read. Marking
    // on the click itself made the Unread list yank the row out from under
    // the cursor the instant you selected it, and a mis-click or a skim past
    // a message silently emptied the list. The row also stays put while the
    // message is open (see getFilteredEmails) — it drops out when you move on.
    READ_DWELL_MS: 1500,
...
Respond ONLY with a JSON object mapping each number to a bundle key, e.g. {"1":"${defs[0]?.key || 'none'}","2":"none"}.`
    ...
    _renderBreadcrumb() {
        ...
        const inDetail = this.currentView === 'insight-detail' ||
            (this.currentView === 'email-detail' && !this._splitActive());
