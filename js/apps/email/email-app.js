...
const EmailApp = {
    ...
    async _initBody() {
        ...
        // Deep link from the home feed's insight card: land on that
        // insight's detail page (same consume-once pattern as above).
        if (this._openToInsightDetail) {
            const id = this._openToInsightDetail;
            this._openToInsightDetail = null;
            if (this.priorityAnalyses[id]) {
                this.currentView = 'insight-detail';
                this.currentInsightId = id;
                this.markAnalysisRead(id, true);
            }
        }
        this.render();
        this.startSmartSync();
    ...
    // click — the inbox reads like a two-pane mail client, and there is no
    // back button to get out of a message (selecting another row, or the
    // breadcrumb, is the way around). The header toggle hides the pane for a
    // full-width list; like the labels sidebar it's a per-machine display
    // preference, so it doesn't ride the sync journal.
    ...
    toggleReaderPane() {
        ...
        // Hiding the pane while a message is open would strand the user in a
        // full-width viewer with no back button — drop to the list instead.
        ...
        try {
            ...
            while (done < this.ANALYSIS_BATCH_SIZE && this.pendingAnalysisIds.length) {
                ...
                if (completed) {
                    ...
                    // Only a stored insight refreshes the feed and (for new
                    // mail flagged at queue time) earns the notification —
                    // so a notification always has a record behind it.
                    if (this.priorityAnalyses[id]) {
                        if (typeof PromptFeed !== 'undefined') PromptFeed.notifyInsightsChanged();
...
Respond ONLY with a JSON object mapping each number to a bundle key, e.g. {"1":"${defs[0]?.key || 'none'}","2":"none"}.`
    ...
    bundlesActive() {
        return (this.currentView === 'emails' || this.currentView === 'email-detail') &&
    ...
    // Full-page view of one insight — reached from the Insights list rows
    // and the home feed's insight cards. Opening it reads the insight (like
    // opening a message); the detail page has a Mark-unread to undo.
    showInsightDetail(emailId) {
        if (!this.priorityAnalyses[emailId]) { this.showInsights(); return; }
        this.currentInsightId = emailId;
        this.currentView = 'insight-detail';
        this.markAnalysisRead(emailId, true);
        this.render();
    },
     ...
     * names the account scope, the drilled-into bundle, and (only when the
     * message is full width, so the list isn't beside it) the open message.
     */
    _renderBreadcrumb() {
        const trim = (s) => (s.length > 64 ? s.slice(0, 61).trimEnd() + '…' : s);
        const fromInsights = this.currentView === 'email-detail' &&
            (this._viewerReturnView || 'emails') !== 'emails';

        const crumbs = [{ label: 'Email', action: () => this._goToList() }];
        ...
        if (this.currentView === 'insights' || this.currentView === 'insight-detail' || fromInsights) {
            crumbs.push({
                label: 'AI Insights',
                action: () => { this.currentEmailId = null; this.showInsights(); },
            });
            if (this.currentView === 'insight-detail') {
                const insightEmail = this.emailById(this.currentInsightId);
                crumbs.push({ label: trim(insightEmail?.subject || 'Insight') });
            }
        } else if (this.bundlesActive() && this.currentBundle) {
        ...
        if (this.currentView === 'email-detail' && !this._splitActive()) {
            crumbs.push({ label: trim(this.emailById(this.currentEmailId)?.subject || '(no subject)') });
        }

        // The trailing crumb is where we already are.
        delete crumbs[crumbs.length - 1].action;
    ...
    _goToList({ account = null, bundle = null } = {}) {
        this.currentEmailId = null;
        this._viewerReturnView = null;
        this.currentView = 'emails';
    ...
    render() {
        ...
        if (headerActions) {
            headerActions.querySelectorAll('button, span, .email-search-wrap, .email-read-filter').forEach(el => {
        ...
        } else if (this.currentView === 'insight-detail') {
            if (listSection) listSection.style.display = 'none';
            if (insightsSection) insightsSection.style.display = '';
            if (viewerSection) viewerSection.style.display = 'none';
            EmailUI.renderInsightDetail(this, 'email-insights-inline-content');
