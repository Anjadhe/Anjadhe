...
const EmailApp = {
    ...
    bundlesOff: true,        // header bundles toggle; OFF by default since 2026-09-20, so the stored flag is the opt-IN
    ...
    async _loadDataBody() {
        ...
        // Bundles OFF by default (2026-09-20, by request). An absent flag is
        // the new default; an explicit `false` is a user who turned them on.
        this.bundlesOff = data?.bundlesOff !== false;
        // ...and every existing install has a persisted `false` that came
        // from the OLD default rather than from anyone choosing it — the blob
        // is written on every save, so "never touched the toggle" and "turned
        // it on" look identical on disk. One stamped pass moves those to off
        // (the seedDefaultHiddenOnce mould: the stamp is kept by saveData, so
        // turning bundles back on afterwards sticks).
        if (data && data.bundlesDefaultOffOnce !== true) this.bundlesOff = true;
        // Stamped unconditionally, fresh installs included. Stamping only
        // when `data` existed wrote `false` on a first run, and the pass
        // then fired AGAIN on the next launch — undoing bundles for anyone
        // who turned them on during that first session.
        this.bundlesDefaultOffOnce = true;
    ...
    saveData() {
        ...
        StorageManager.set('email', {
            ...
            bundlesDefaultOffOnce: this.bundlesDefaultOffOnce === true,
    ...
    /**
     * A source turned off: its records are gone, so their verdicts go
     * too (the orphan prune's rule, run at once rather than at next
     * load). Tasks the insights created are the user's and stay.
     */
    ...
    // --- Reading pane ---
    //
    // The viewer is docked beside the list at ALL times on a big monitor,
    // not just after a click — the inbox reads like a two-pane mail client,
    // (On a laptop-sized display the pane starts OFF — UIUtils.
    // readerPaneDefault, 2026-09-02 — and the header toggle brings it back.) and with the list
    ...
    _readerPanePref() {
        ...
        try {
            const v = localStorage.getItem('email-reader-pane');
            if (v === '0') return false;
            if (v === '1') return true;
        } catch { /* private mode */ }
    ...
    async _pruneOutgoingInsights() {
        ...
        if (outgoing.length) {
            ...
            for (const id of outgoing) {
                const a = this.priorityAnalyses[id];
                // A head built on the user's own email: its incoming
                // members stand on their own again.
                for (const m of (a?.matter?.members || [])) {
                    const member = this.priorityAnalyses[m.messageId];
                    if (!member || gone.has(m.messageId)) continue;
                    member.rolledUp = false;
                    delete member.matterId;
                    delete member.matter;
                    touched.add(m.messageId);
                }
                delete this.priorityAnalyses[id];
                this.analyzedNoInsight[id] = { at, why: 'outgoing' };
            }
            // A surviving head may list a pruned email in its timeline.
            for (const [hid, a] of Object.entries(this.priorityAnalyses)) {
                if (!a?.matter?.members?.length) continue;
                const kept = a.matter.members.filter(m => !gone.has(m.messageId));
                if (kept.length === a.matter.members.length) continue;
                a.matter.members = kept;
                a.matter.count = kept.length + 1;
                if (!kept.length) { delete a.matter; delete a.matterId; }
                touched.add(hid);
            }
...
If the insight you extract from THIS email is essentially the same KIND of notification as any of the above — a recurring or low-value message the user evidently doesn't want — set "suppress": true. If it is a meaningfully different or more important matter, set "suppress": false. Never set "suppress": true when actionRequired is true — the user always wants insights that need action from them.` : '';
    ...
    markAnalysisRead(emailId, read = true) {
        ...
        this._syncEmailRead(emailId, read);
        AppManager.updateStats();
    },
    ...
    /**
     * The insight derived from these messages follows their read state.
     * Called by every email-side read path; persists once.
     *
     * A rolled-up matter member is never put BACK to unread: its analysis
     * is a row nowhere, so unread it would count as "needs you" forever
     * with nothing to open (see the rollup in _rollUpInto).
     */
    _syncInsightRead(messageIds, read) {
        if (this._readSyncing) return;
        const ids = (Array.isArray(messageIds) ? messageIds : [messageIds]).filter(Boolean);
        const touched = [];
        for (const id of ids) {
            const a = this.priorityAnalyses[id];
            ...
            if (read) {
                ...
                if (!a.readAt || a.rolledUp) continue;
                delete a.readAt;
            }
            touched.push(id);
        }
        if (!touched.length) return;
        this._persistAnalyses(touched);
        if (typeof AppManager !== 'undefined') AppManager.updateStats();
    },

    /**
     * The mirror: the message an insight came from follows the insight.
     * Records from another source (texts) have no read state of their own
     * and no Gmail to tell — they are the insight.
     */
    _syncEmailRead(messageId, read) {
        if (this._readSyncing) return;
        if (this._sourceFor(messageId)) return;
        const email = this.emailById(messageId);
        if (!email || !!email.isRead === !!read) return;
        this._readSyncing = true;
        try { this.setEmailRead(messageId, read); } finally { this._readSyncing = false; }
        this.saveDataSoon();
        if (AppManager.currentApp === 'email') this.render();
    },
    ...
    // Attend-event hint lexicon — the words invitation-shaped mail uses.
    // A cost gate only (the TRADE_CONFIRM_RX mould), never the judge:
    // _validateAttendEvent decides what survives.
    EVENT_HINT_RX: /\b(rsvp|invit(?:ed|ation)|save the date|open house|back[- ]to[- ]school|parent[- ]teacher|orientation|recital|rehearsal|field trip|ceremony|graduation|potluck|fundraiser|book fair|picture day|spirit (?:week|day)|school (?:event|night)|pta|ptsa|join us)\b/i,

    /**
     * Should the sweep spend a model call on the attend-event pass?
     * Two doors, both closed on a metered brain (BYOK: ambient calls are
     * the user's money; Re-analyze is the on-demand door):
     * - 'general' mail whose from/subject/snippet hit the event lexicon —
     *   the school-newsletter case: the EMAIL is a newsletter, the EVENT
     *   inside it is an appointment (the Groupon boundary, same mould).
     * - 'appointment' mail the first pass left actionRequired:false —
     *   attending IS the action, so an FYI verdict on an appointment is
     *   the boundary itself, not a settled answer.
     */
    _shouldExtractAttendEvent(email, type, analysis) {
        ...
        return this.EVENT_HINT_RX.test(
            `${email.from || ''}\n${email.subject || ''}\n${email.snippet || ''}`);
    },
...
If you cannot determine a field, use null for that field. Always try to extract what you can.`
    ...
    showTransactionConfirmModal(transactions, email, onCreated) {
        const P = this._portfolio();
        if (!P) {
            UIUtils.showToast('The Portfolio app is not available.', 'error');
            return;
        }
        const accounts = P.accounts();
        if (accounts.length === 0) {
                    ...
                    onClick: () => {
                        ...
                        for (let i = 0; i < transactions.length; i++) {
                            ...
                            if (optTypeEl) {
                                ...
                                ticker = P.buildOccSymbol(ticker, expiration, optTypeEl.value, strike);
                            }
                        ...
                        // One call: cash effect per transaction (options ×100),
                        // save, price refresh — the package owns that recipe.
                        const created = pending.length ? P.addTransactions(pending) : 0;
                        if (created > 0) {
    ...
    openMessageFrom(messageId, origin = null) {
        if (!messageId) return;
        // A source record has no Inbox page to land on — the insight's own
        // detail (Email AI) IS the message: it shows the conversation and
        // carries the door into Messages.app.
        if (this._sourceFor(messageId)) {
            if (typeof FyiPage !== 'undefined' && FyiPage.openTo) FyiPage.openTo(messageId);
            return;
        }
    ...
    setEmailRead(messageId, read = true) {
        ...
        this._persistEmail(email);
        this._syncInsightRead(messageId, read);
        if (email.account) {
            const call = read
    ...
    markEmailsRead(messageIds) {
        ...
        this._syncInsightRead(targets.map(e => e.messageId), true);
        this.saveDataSoon();
        this.render();
    ...
    markEmailsUnread(messageIds) {
        ...
        this._persistEmails(targets);
        this._syncInsightRead(targets.map(e => e.messageId), false);
        this.saveDataSoon();
        this.render();
