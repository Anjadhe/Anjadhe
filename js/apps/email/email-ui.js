...
const EmailUI = {
    ...
    renderEmailRow(email, app) {
        ...
        // Which-account chip — only when the list is actually mixed (2+
        // accounts, unscoped view). Local part only; full address on hover.
        const accountChip = this.renderAccountChip(email.account, app);
    ...
    renderAccountChip(accountEmail, app) {
        if (!accountEmail || app.currentAccount || app.getAccounts().length < 2) return '';
        const localPart = accountEmail.split('@')[0] || accountEmail;
        return `<span class="email-row-account" title="${UIUtils.escapeHtml(accountEmail)}">${UIUtils.escapeHtml(localPart)}</span>`;
    },
        ...
        container.innerHTML = `
            ...
            <article class="insight-detail-page">
                <header class="insight-detail-head">
                    ...
                    <h2 class="insight-detail-title">${esc(email.subject || '(no subject)')}</h2>
                    <div class="insight-detail-sub">${esc(this.extractName(email.from))} &middot; ${esc(timeAgo)}</div>
                </header>
                ${analysis.summary ? `<p class="insight-detail-lede">${esc(analysis.summary)}</p>` : ''}
                ${actions.length ? `
                        ...
                        ${actions.map(item => {
                            ...
                            return `<div class="insight-action-row${taskId ? ' insight-action-linked' : ''}"${taskId ? ` data-task-id="${taskId}" role="button" title="Open this task"` : ''}>
                                ...
                                <div class="insight-action-content">
                                    <span class="insight-action-text">${esc(item.text)}</span>
        ...
        container.innerHTML = drafts.map(d => {
            ...
            return `
                <div class="email-row email-draft-row" data-draft-id="${d.draftId}" data-account="${UIUtils.escapeHtml(d.account)}">
                    ...
                    ${this.renderAccountChip(d.account, app)}
                    <div class="email-row-date">${date}</div>
                </div>
            `;
        }).join('');
