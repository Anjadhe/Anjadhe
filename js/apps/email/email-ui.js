...
const EmailUI = {
        ...
        return `
            <div class="email-bundle" data-bundle="${bundleKey}">
                <div class="email-bundle-card email-bundle-trigger ${unread > 0 ? 'has-unread' : ''}"
                    ...
                    <!-- Same text-chip hover actions as email rows, parked in
                         the card's top-right over the date. -->
    ...
    bindListDelegates(app, container) {
        ...
        container.addEventListener('click', (e) => {
            ...
            const act = e.target.closest('.email-bundle-act');
            if (act) {
                e.stopPropagation();
                const key = act.closest('.email-bundle')?.dataset.bundle;
                if (!key) return;
                const ids = this.bundleMessageIds(key, a);
                if (act.dataset.act === 'read') a.markEmailsRead(ids);
                else if (act.dataset.act === 'archive') a.archiveEmails(ids);
                return;
            }
