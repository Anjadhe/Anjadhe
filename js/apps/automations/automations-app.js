...
const AutomationsApp = {
    ...
    async _renderGrants() {
        ...
        this._count('automations-grants-count', grants.length);
        const label = (g) => {
        ...
        box.innerHTML = grants.map(g => {
            ...
            <div class="automations-row" data-grant="${this._esc(g.id)}">
                ...
                <span class="automations-row-actions">
                    <button class="automations-btn" data-act="revoke" title="Revoke — the assistant asks again from then on">Revoke</button>
                </span>
            </div>`;
        }).join('');
        box.onclick = async (e) => {
    ...
    _renderWaiting() {
        ...
        this._count('automations-waiting-count', waiting.length);
        // This section IS the call to action, so its primary button stays
        // visible; everywhere else row actions wait for hover.
        box.innerHTML = waiting.map(t => `
            <div class="automations-row" data-task="${this._esc(t.id)}">
                ...
                <span class="automations-row-actions is-open">
                    <button class="automations-btn automations-btn--primary" data-act="${t.status === 'paused' ? 'resume' : 'approve'}">${t.status === 'paused' ? 'Resume' : 'Review &amp; run'}</button>
                    <button class="automations-btn" data-act="cancel">Cancel</button>
                </span>
            </div>`).join('');
        box.onclick = async (e) => {
    ...
    _renderArmed() {
        ...
        this._count('automations-armed-count', list.length);
        if (!list.length) {
            box.innerHTML = '<p class="automations-empty">Nothing armed yet. Ask the assistant &mdash; &ldquo;every weekday at 7, review my portfolio&rdquo; &mdash; and it offers to arm it. Armed tasks run by themselves while Anjadhe is open; anything needing permission pauses and notifies you.</p>';
            return;
        }
            ...
            <div class="automations-row${a.enabled ? '' : ' is-disabled'}" data-auto="${this._esc(a.id)}">
                <div class="automations-row-main">
                    ...
                    <div class="automations-row-sub">
                        ${a.enabled ? '' : 'off &middot; '}${this._esc(AutomationService.triggerLabel(a))}
                ...
                <span class="automations-row-actions">
                    <button class="automations-btn" data-act="run">Run now</button>
                    <button class="automations-btn" data-act="toggle">${a.enabled ? 'Turn off' : 'Turn on'}</button>
                    <button class="automations-btn automations-btn--danger" data-act="remove">Remove</button>
                </span>
            </div>`).join('');
        box.onclick = async (e) => {
            const btn = e.target.closest('button[data-act]');
    ...
    _renderRecent() {
        ...
        box.innerHTML = recent.map(t => {
            const running = ['planning', 'running', 'verifying'].includes(t.status);
            const mark = t.status === 'done' ? '✓' : running ? '…' : '✗';
            const markKind = t.status === 'done' ? 'ok' : running ? 'run' : 'bad';
            ...
            // The mark is its own fixed column so every goal starts at the
            // same x — a glyph inside the title made the list ragged.
            return `
            <div class="automations-row" data-recent="${this._esc(t.id)}">
                <span class="automations-mark automations-mark--${markKind}" aria-hidden="true">${mark}</span>
                <div class="automations-row-main">
                    <div class="automations-row-title">${this._esc(t.goal)}</div>
                    <div class="automations-row-sub">
