...
const NewsApp = {
    ...
    _heroBandHtml(hero, subs, latest, loc) {
        ...
        const meta = [hero.source, hero.publishedAt ? NewsFeed._ago(hero.publishedAt) : '']
            .filter(Boolean).map(x => UIUtils.escapeHtml(x)).join(' &middot; ');
        const subsHtml = subs.length ? `
            <div class="news-hero-subs">
                ...
                <button class="news-hero-sub${this._isRead && this._isRead(it.title) ? ' is-read' : ''}" type="button" data-news-g="${l.gi}" data-news-i="${l.ri}">
                    ...
                    <span class="news-latest-meta">${[it.source, it.publishedAt ? NewsFeed._ago(it.publishedAt) : '']
    ...
    _rowHtml(it, gi, ri, lead, withTopic) {
        const meta = [it.source, it.publishedAt ? NewsFeed._ago(it.publishedAt) : '']
            .filter(Boolean).map(x => UIUtils.escapeHtml(x)).join(' &middot; ');
        ...
        <div class="news-row-wrap${lead ? ' news-lead-wrap' : ''}${read ? ' is-read' : ''}">
            <button class="news-row${lead ? ' news-lead' : ''}" type="button" data-news-g="${gi}" data-news-i="${ri}">
                ${withTopic && it.topic ? `<span class="news-latest-kicker">${UIUtils.escapeHtml(it.topic)}</span>` : ''}
    ...
    _searchHtml() {
        ...
        html += hits.map(h => this._rowHtml(h.it, h.gi, h.ri, false, true)).join('');
        if (saved.length) {
