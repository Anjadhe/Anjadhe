...
const NewsApp = {
    ...
    _mode: 'list',             // 'list' (card grid) | 'topic' | 'saved' | 'reader'
    ...
    init() {
        ...
        view.addEventListener('click', (e) => {
            ...
            if (savedRow && !savedRow.dataset.newsAction) {
     ...
     */
    _sidebarHtml() {
        if (typeof DiscoverPane === 'undefined') return '';
    ...
    _listHtml() {
        ...
        if (!s.enabled || !DiscoverPane._webOn) {
            return `<div class="news-empty">News needs the Discover pane turned on (Settings &rsaquo; Appearance) and web access enabled (Settings &rsaquo; AI Assistant &rsaquo; Web Search).</div>`;
        }
        if (!s.interests.length) {
            return `<div class="news-empty">Pick some topics to follow.</div>
        ...
        return digest + band + `<div class="news-sections">${sections}</div>` + foot;
    },
    ...
    _topicHtml() {
        ...
        if (gi === -1) {
            ...
            const followed = DiscoverPane.settings().interests.includes(this._topic);
    ...
    _savedHtml() {
        ...
        if (typeof DiscoverPane !== 'undefined') {
            const cache = DiscoverPane.cache();
    ...
    _savedRowHtml(it, i) {
        const ago = (ts) => (ts && typeof DiscoverPane !== 'undefined') ? DiscoverPane._ago(ts) : '';
        // Topic rides the meta line: in Saved it's a label for where the story
        // came from, not the emphasis "For you" reasons get.
        const meta = [it.topic, it.source, ago(it.publishedAt), it.savedAt ? 'saved ' + ago(it.savedAt) : '']
            .filter(Boolean).map(x => UIUtils.escapeHtml(x)).join(' &middot; ');
        return `
        <div class="news-row-wrap">
            <button class="news-row" type="button" data-news-saved-i="${i}">
                <span class="news-row-title">${UIUtils.escapeHtml(it.title || '')}</span>
                <span class="news-row-meta">${meta}</span>
            </button>
    ...
    _readerHtml() {
        ...
        const backTo = this._returnMode === 'saved' ? 'Saved'
