    ...
    async _loadRelated(item, key, seedResults) {
        try {
            ...
            if (this._current === item) {
                this._summaryState.relatedLoading = false;
                this._summaryState.related = related;
                this.render();
            }
        } catch {
            // Garnish, like the events card: the summary already stands.
            if (this._current === item && this._summaryState) {
                this._summaryState.relatedLoading = false;
                this._renderIfReader(item);
            }
        }
...
Return JSON exactly like: {"related":[{"i":0,"kind":"coverage"}]}. Use only the numbers above; never write a title or a link.` }
    ...
    // The browser door for a related row — the same Browse hop the article's
    // own "Open article" button takes, so the way back is the same too.
    _openRelatedExternal(idx) {
        const r = this._relatedAt(idx);
        if (!r || !/^https?:/i.test(r.url)) return;
        if (typeof AppManager !== 'undefined' && AppManager.openInBrowse) {
            AppManager.openInBrowse(r.url, {
                label: 'Back to News',
    ...
    // The Hacker News thread for the open story: the same Browse door the
    // article uses, minus reader mode (a comment thread is not an article).
    _openDiscussion() {
        const url = this._current?.discussionUrl || '';
        if (!/^https?:/i.test(url)) return;
        if (typeof AppManager !== 'undefined' && AppManager.openInBrowse) {
            AppManager.openInBrowse(url, { label: 'Back to News', onBack: () => AppManager.openApp('news') });
        } else if (window.electronAuth?.openExternal) {
            window.electronAuth.openExternal(url);
        }
    },
