 ...
 * choice (nenva Connect relay with server-side topic caching, or direct
 ...
 *   - `discover-settings` (synced): { interests: [], location, sources }.
 *     `sources` (2026-09-10) is the user's pick of where headlines come
 *     from — ids from main's js/main/news-sources.js registry; absent
 *     means ['google'], which is what every pre-existing blob reads as.
...
const NewsFeed = {
    ...
    /**
     * Where headlines come from (2026-09-10). Every followed topic is
     * fetched from EACH picked source and the rows merge per topic. The
     * ALLOWLIST is main's js/main/news-sources.js registry (Connect's
     * lib/news.js is its twin); this is the label list the Topics page
     * draws and the byline names, and its ids must stay in step with
     * that registry. Google News alone is the default — a user who never
     * opens the Sources card sees exactly what they saw before.
     */
    SOURCES: [
        { id: 'google', label: 'Google News', desc: 'Thousands of publishers, searched by topic. On by default.' },
        { id: 'bing', label: 'Bing News', desc: 'A second news index, searched by topic.' },
        { id: 'hn', label: 'Hacker News', desc: 'Stories from the last two days that match your topics, ranked by its readers. Best for technology, science and startups.' }
    ],
    ...
    // The renderer's copy of the allowlist gate: unknown ids drop,
    // duplicates fold, registry order, empty means the default.
    _normalizeSources(list) {
        const ids = this.SOURCES.map(x => x.id);
        const want = new Set((Array.isArray(list) ? list : [])
            .map(x => String(x || '').trim().toLowerCase()).filter(x => ids.includes(x)));
        const out = ids.filter(id => want.has(id));
        return out.length ? out : this.DEFAULT_SOURCES.slice();
    },
    ...
    async ensureFresh() {
        ...
        // A source picked (or dropped) since the last fetch is stale the
        // same way: the cache was built from a different set of places.
        const sourcesChanged = c.generatedAt
            && this.settings().sources.join(',') !== this._normalizeSources(c.sources).join(',');
        if (!missing && !sourcesChanged && c.generatedAt && (Date.now() - c.generatedAt) < this.TTL_MS) return;
        await this._refresh();
    },
    ...
    settings() {
        ...
        return {
            ...
            sources: this._normalizeSources(s.sources)
        };
    },
    ...
    cache() {
        ...
        return {
            ...
            // Which sources the rows were fetched from; absent in caches
            // from before sources existed (they were Google News).
    ...
    /**
     * A feed's pubDate is a story's LATEST publish, not its first: a live
     * blog re-published all day reads "40m ago" on a page that went up the
     * day before (cnbc.com/2026/09/20/stock-market-today-live-updates.html,
     * first 09-20 22:01Z, pubDate 09-21 20:26Z). When there is evidence the
     * story appeared earlier, the age is labelled "updated …" — never
     * re-dated, since the feed time is still the true last update.
     *
     * Evidence is a date in the article's own URL (only when the publish is
     * more than a day and a half past it, so no timezone reading can make a
     * same-day story look updated) or the page's own published time, which
     * the reader reads from the article head (`notePagePublished`). Google's
     * links are news.google.com redirects with no date, so for those only
     * the reader's evidence applies.
     */
    _pagePublished: new Map(),   // row url -> the page's own first-published ms (this session)

    notePagePublished(url, when) {
        const t = typeof when === 'number' ? when : Date.parse(when || '');
        if (!url || !Number.isFinite(t)) return;
        if (this._pagePublished.size > 500) this._pagePublished.clear();
        this._pagePublished.set(String(url), t);
    },

    _isUpdate(it, pagePublished) {
        const pub = typeof it?.publishedAt === 'number' ? it.publishedAt : Date.parse(it?.publishedAt || '');
        if (!Number.isFinite(pub)) return false;
        const HOUR = 3600000;
        const page = pagePublished != null ? Date.parse(pagePublished) || Number(pagePublished)
            : this._pagePublished.get(String(it.url || ''));
        if (Number.isFinite(page) && pub - page > HOUR) return true;
        for (const u of [it.url, it.openUrl]) {
            const m = String(u || '').match(/\/(20\d{2})[/-](\d{2})[/-](\d{2})(?=[/-]|$)/);
            if (!m) continue;
            const day = Date.UTC(+m[1], +m[2] - 1, +m[3]);
            if (Number.isFinite(day) && pub - day > 38 * HOUR) return true;
        }
        return false;
    },

    /** The age a row shows: "40m ago", or "updated 40m ago" for a story that first ran earlier. */
    _ageLabel(it, pagePublished) {
        const pub = typeof it?.publishedAt === 'number' ? it.publishedAt : Date.parse(it?.publishedAt || '');
        if (!Number.isFinite(pub)) return '';
        const ago = this._ago(pub);
        return this._isUpdate(it, pagePublished) ? 'updated ' + ago : ago;
    },
    ...
    async _fetchNews(topics, sources) {
        ...
        const results = await Promise.all(
            chunks.map(c => window.electronSearch.news(c, { sources }).catch(() => null)));
    ...
    _failureMessage(res) {
        ...
        if (res && res.route === 'connect') return 'nenva Connect could not fetch some headlines just now.';
    ...
    async _refresh() {
        if (this._busy) return;
        const { interests, location, sources } = this.settings();
        ...
        try {
            ...
            const res = await this._fetchNews(topics, sources);
            ...
            for (const t of res.topics) {
                ...
                for (const it of (Array.isArray(t.items) ? t.items : [])) {
                    ...
                    rows.push({
                        ...
                        publishedAt: pub,
                        // Which source answered; rows from a build that
                        // predates sources carry none and are Google's.
                        via: this._normalizeSources([it.via])[0],
                        ...(/^https?:\/\//i.test(it.discussionUrl || '')
                            ? { discussionUrl: String(it.discussionUrl).slice(0, 300) } : {}),
            ...
            // capped PER SOURCE — newest first, so with two sources on a
            // topic the older-dated Hacker News rows are not crowded out
            // by Google's twenty): the News app's topic groups expand into
            // these. The round-robin below consumes the rows, so snapshot
            // now.
            const topicLists = [];
            for (const [topic, rows] of perTopic) {
                ...
                for (const r of rows) {
                    if ((perVia.get(r.via) || 0) >= this.TOPIC_MAX) continue;
                    if (kept.some(k => this._sameStory(k.title, r.title))) continue;
                    perVia.set(r.via, (perVia.get(r.via) || 0) + 1);
                    kept.push(r);
                }
...
Catch the user up, topic by topic. For each topic with something notable, write exactly one line: the topic name, a colon, then one or two short sentences summarizing that topic's headlines. Keep every sentence inside its topic — never combine different topics in one sentence, and skip a topic entirely rather than stretching to include it. Every statement must come strictly from a headline above. Plain text: one line per topic, no bullet points, no markdown, no preamble.` }
    ...
    /**
     * The one writer of the followed list, location and sources. Merges
     * over the stored blob so any field the topics page doesn't own
     * survives, and says whether anything actually changed (the caller
     * decides whether that earns a refresh).
     */
    saveSettings({ interests, location, sources }) {
        ...
        const next = {
            ...
            sources: Array.isArray(sources) ? this._normalizeSources(sources) : s.sources
        };
        const changed = JSON.stringify(next) !== JSON.stringify({ interests: s.interests, location: s.location, sources: s.sources });
        if (changed) {
