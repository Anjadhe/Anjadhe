...
Catch the user up, topic by topic. For each topic with something notable, write exactly one line: the topic name, a colon, then one or two short sentences summarizing that topic's headlines. Keep every sentence inside its topic — never combine different topics in one sentence, and skip a topic entirely rather than stretching to include it. Every statement must come strictly from a headline above. Plain text: one line per topic, no bullet points, no markdown, no preamble.` }
    ...
    openPrefs() {
        ...
        const group = (label, chipsHtml, id = '') =>
            `<div class="discover-group"${id ? ` id="${id}"` : ''}>
                <div class="discover-prefs-label discover-group-label">${UIUtils.escapeHtml(label)}</div>
                <div class="discover-chip-grid">${chipsHtml}</div>
            </div>`;
        ...
        const modal = Modal.create({
            ...
            content: `
                <p class="discover-prefs-desc">Follow up to ${this.TOPIC_LIMIT} topics; they fill the News page with current headlines. Any subject works: a country, a team, a company. The list below is ideas, not a limit.</p>
                <div class="discover-prefs-label">Topics <span class="discover-count" id="discover-topic-count"></span></div>
                <input type="text" id="discover-topic-filter" class="discover-topic-filter" placeholder="Search topics&hellip;" spellcheck="false" aria-label="Search topics">
                <div id="discover-catalog">
                    ${group('Your topics', customs.map(t => chip(t, true, true)).join(''), 'discover-custom-group')}
                    ${this.TOPIC_CATALOG.map(g =>
                        group(g.group, g.topics.map(t => chip(t, selected.has(t.toLowerCase()), false)).join(''))).join('')}
                    <p class="discover-prefs-hint" id="discover-filter-empty" hidden>No topic by that name here. Add it below and it becomes one.</p>
                </div>
            ...
            buttons: [{
                ...
                onClick: () => {
                    const catalog = modal.body.querySelector('#discover-catalog');
                    const interests = [...catalog.querySelectorAll('.discover-chip.selected')]
        ...
        const customGroup = modal.body.querySelector('#discover-custom-group');
        const selCount = () => catalog.querySelectorAll('.discover-chip.selected').length;
        const updateCount = () => {
            const n = selCount();
            countEl.textContent = n ? `${n} of ${this.TOPIC_LIMIT} selected` : 'none selected';
        };
        // "Your topics" only shows once there is one — an empty labeled
        // group reads as a rendering fault.
        const updateCustomGroup = () => {
            customGroup.hidden = !customGroup.querySelector('.discover-chip');
        };
        // The follow limit is ENFORCED HERE, out loud — the fetch used to
        // cap silently and a topic past the cap looked broken forever.
        const overLimit = () => {
            if (selCount() < this.TOPIC_LIMIT) return false;
            UIUtils.showToast(`Up to ${this.TOPIC_LIMIT} topics. Unfollow one first.`, 'info');
            return true;
        };
        updateCount();
        updateCustomGroup();
        ...
        // The filter narrows the catalog by name; groups with nothing left
        // fold away. It never touches selection state.
        const filter = modal.body.querySelector('#discover-topic-filter');
        filter?.addEventListener('input', () => {
            const q = filter.value.trim().toLowerCase();
            let any = false;
            catalog.querySelectorAll('.discover-chip').forEach(b => {
                const hit = !q || b.dataset.topic.toLowerCase().includes(q);
                b.hidden = !hit;
                if (hit) any = true;
            });
            catalog.querySelectorAll('.discover-group').forEach(g => {
                g.hidden = !g.querySelector('.discover-chip:not([hidden])');
            });
            const empty = modal.body.querySelector('#discover-filter-empty');
        ...
        const addCustom = () => {
            ...
            if (!t) return;
            const existing = [...catalog.querySelectorAll('.discover-chip')]
            ...
            if (existing) {
                ...
                if (overLimit()) return;
                customGroup.querySelector('.discover-chip-grid')
                    .insertAdjacentHTML('beforeend', chip(t, true, true));
            }
            input.value = '';
            input.focus();
            updateCount();
