...
(function registerWellnessTools() {
    ...
    // Activity verbs and distance words matter as much as the medical nouns:
    // "I went for a walk last night for 2 miles" carries no workout/exercise
    // word, and without the group the model has no log_wellness to call
    // (2026-08-31 miss — the tuned model logged nothing, big models flailed).
    // Bare "run"/"runs" over-matches routine/shell talk, but the group is 5
    // small schemas and over-matching is cheap; under-matching is a dead end.
    AgentTools.registerDomain(SOURCE,
        /\b(wellness|health|blood\s?pressure|bp|systolic|diastolic|pulses?|heart\s?rate|bpm|weight|weigh(ed|ing)?|glucose|blood\s?sugar|spo2|oxygen|temperatures?|fever|sleep|slept|hydration|water|meals?|ate|eating|medications?|meds?|vitamins?|supplements?|symptoms?|headaches?|workouts?|exercis\w+|breath\w+|steps|mood|walk(s|ed|ing)?|jogg?\w*|hik(e|es|ed|ing)|runs?|ran|running|swim\w*|swam|bik(e|es|ed|ing)|cycl(e|es|ed|ing)|yoga|gym|cardio|stretch\w*|miles?|kms?|kilometers?|calories?)\b/);
    ...
    AgentTools.register({ type: 'function', function: {
        name: 'log_wellness',
        description: 'Log a health entry in the Wellness app. kind decides which fields apply: bp (systolic, diastolic, pulse) · pulse = resting heart rate (value) · glucose (value, context) · spo2 (value %) · temperature (value) · weight (value) · activity (activityType, duration min, distance, avgBpm, maxBpm) · steps (value) · meal (mealType, description) · water (amount) · sleep (hours, quality 1-5) · mood (mood/energy/stress 1-5) · medication (name, dose) · symptom (name, severity 1-5) · note (notes). EVERY kind also takes `notes` — put every specific the user gave that has no field of its own there (the exercises in a strength workout, how a symptom developed, what changed): details the user said out loud must NEVER be dropped. Fill a field ONLY with a value the user actually stated — NEVER estimate or infer a number (a 2-mile walk does NOT get a guessed duration; leave duration empty). Log meals/activities BEFORE a bp/glucose reading so the reading gets timing context. FOLLOW-UPS: when the user adds to or corrects something already logged (a start time, a missed detail), call update_wellness on that entry — logging again would create a duplicate.',
        parameters: { type: 'object', properties: {
            ...
            distance: { type: 'number', description: 'Activity distance, in the user\'s distance unit (mi or km — pass the number as the user said it)' },
