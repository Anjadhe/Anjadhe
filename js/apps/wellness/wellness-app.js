...
const WellnessApp = {
     ...
     * the not-a-doctor rule outright (W4). Voice comes from the row's
     * select (routines carry voiceId; assistant's own voice = null).
     */
    REVIEWS: {
        weekly: {
            title: 'Wellness Review',
            interval: 'weekly', time: '09:00',
            voiceSel: 'wellness-voice-weekly',
    ...
    armReview(which) {
        ...
        const voiceSel = document.getElementById(spec.voiceSel);
        NotePrompts.create({
            title: spec.title,
            body: spec.body,
            config: {
                ...
                web: false,
                useContext: true,
                voiceId: (voiceSel && voiceSel.value) || null,
