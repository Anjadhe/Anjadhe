...
const ReaderApp = {
    ...
    init() {
        ...
        if (!this._reader) {
            this._reader = DocReader.create({
                ...
                docMeta: (id) => (this._listing.docs || []).find(d => d.id === id) || null
            });
        }
        this._wire();
        ...
        view.innerHTML = `
            <div class="app-header-bar">
                ...
                <div class="app-header-actions">
                    <button id="reader-import-btn" class="primary-btn library-head-btn" type="button" title="Copy files into your documents folder"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 16V4"/><path d="m7 9 5-5 5 5"/><path d="M4 16v3a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-3"/></svg>Import files&hellip;</button>
                    <button id="reader-open-folder-btn" class="secondary-btn library-head-btn" type="button" title="Open the documents folder in Finder"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>Open folder</button>
                ...
                <div class="actions-layout library-layout">
                    ...
                    <div class="actions-main library-pane">
                        <div id="reader-search-row" class="library-search">
                            <span class="library-search-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg></span>
                            <input id="reader-search-input" class="search-input" type="search" placeholder="Search your documents by what they say…" autocomplete="off">
                        </div>
    ...
    renderNav() {
        ...
        const item = (label, count, attrs, isActive, extraClass = '', lead = '') => `
            <button type="button" class="actions-nav-item library-nav-item${isActive ? ' is-active' : ''}${extraClass}" ${attrs}>
                ${lead}<span class="actions-nav-label">${label}</span>
                ${count ? `<span class="actions-nav-count">${count}</span>` : ''}
            </button>`;
        const glyph = (paths) => `<span class="library-nav-glyph" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">${paths}</svg></span>`;
        const dot = (tag) => `<span class="library-nav-dot" style="--tag-hue:${DocReader.tagHue(tag)}" aria-hidden="true"></span>`;
        let html = '<div class="actions-nav-section">'
            + item('All documents', docs.length, 'data-nav="all"', active('all'), '',
                   glyph('<path d="M4 6a2 2 0 0 1 2-2h8l6 6v10a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2z"/><path d="M14 4v6h6"/>'))
            + item('Untagged', untagged, 'data-nav="untagged"', active('untagged'), '',
                   glyph('<path d="M20 12 12 20l-8-8V4h8z"/><circle cx="8.5" cy="8.5" r="1.2"/><path d="M4 4l16 16" opacity=".55"/>'))
            + '</div>';
            ...
            for (const n of nodes) {
                ...
                html += `<div class="library-tag-node${n.pending ? ' is-pending' : ''}">`
                    + item(esc(n.name), n.count,
                        ...
                        active('tag', n.tag), ' library-tag-item', dot(n.tag))
    ...
    _renderList(body) {
        ...
        const sel = this._sel;
        const eyebrow = sel.kind === 'tag' ? 'Tag' : sel.kind === 'untagged' ? 'Documents' : 'Library';
        const hueStyle = sel.kind === 'tag' ? ` style="--tag-hue:${DocReader.tagHue(sel.tag)}"` : '';
        let html = `<div class="library-list-card"${hueStyle}>
            <div class="library-list-head">
                <div class="library-list-headtext">
                    <span class="library-list-eyebrow">${sel.kind === 'tag' ? '<span class="library-nav-dot" aria-hidden="true"></span>' : ''}${eyebrow}</span>
                    <h3 class="library-list-title">${esc(this._selectionLabel())}</h3>
                </div>
                <span class="library-list-meta">
                    <span class="library-count-pill">${docs.length} document${docs.length === 1 ? '' : 's'}</span>
                    ${sel.kind === 'tag' ? `<button type="button" class="library-pill" data-tag-rename><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M4 20h4l10.5-10.5a2.1 2.1 0 0 0-3-3L5 17z"/><path d="m13 7 4 4"/></svg>Rename tag</button>` : ''}
                </span>
            </div>
            ${this._mixHtml(docs)}`;
        if (!docs.length) {
            ...
            html += `<div class="library-doc-rows">${docs.map(d => DocReader.rowHtml(d, { tags: tags.get(d.id) || [] })).join('')}</div>`;
        }
        html += '</div>' + this._inventoryLine();
        body.innerHTML = html;
    ...
    /**
     * What the selection is made of, by KIND — a composition bar with a
     * legend, in each kind's hue (colour as data; the Spending month bar's
     * idea). Pure arithmetic over the listing; nothing when there is one
     * kind only (a bar of one colour says nothing the tile doesn't).
     */
    _mixHtml(docs) {
        ...
        for (const d of docs) {
            const k = DocReader.kindOf(d.relpath);
            const cur = counts.get(k.key) || { ...k, n: 0 };
            cur.n++;
            counts.set(k.key, cur);
        }
        const kinds = [...counts.values()].sort((a, b) => b.n - a.n || a.label.localeCompare(b.label));
        if (kinds.length < 2) return '';
        const total = docs.length;
        return `<div class="library-mix" aria-label="Documents by kind">
            <div class="library-mix-bar">${kinds.map(k => `<span class="library-mix-seg" style="--doc-hue:${k.hue};width:${(k.n / total * 100).toFixed(2)}%" title="${esc(k.label)} · ${k.n}"></span>`).join('')}</div>
            <div class="library-mix-legend">${kinds.map(k => `<span class="library-mix-key" style="--doc-hue:${k.hue}"><span class="library-mix-swatch" aria-hidden="true"></span>${esc(k.label)} <b>${k.n}</b></span>`).join('')}</div>
        </div>`;
    },
    ...
    async _renderResults(body) {
        ...
        const relpathOf = new Map((this._listing.docs || []).map(d => [d.id, d.relpath]));
        let html = `<div class="library-results-head">
                <span class="library-list-eyebrow">Search</span>
                <span class="library-results-count">${byDoc.size} document${byDoc.size === 1 ? '' : 's'}${scoped ? ` under ${esc(this._selectionLabel())}` : ''}</span>
            </div>`;
        html += res.keywordOnly
        ...
        for (const [docId, g] of byDoc) {
            ...
            const relpath = relpathOf.get(docId) || g.title || '';
            html += `
            <div class="library-result-doc" data-kind="${DocReader.kindOf(relpath).key}" style="--doc-hue:${DocReader.kindOf(relpath).hue}">
                <button type="button" class="library-result-dochead" data-lib-doc="${esc(docId)}">
                    ${DocReader.tileHtml(relpath)}
                    <span class="library-result-doctitle">${esc(g.title)}</span>
                    ${t.length ? `<span class="library-result-tags">${DocReader._chipsHtml(docId, t, { removable: false })}</span>` : ''}
