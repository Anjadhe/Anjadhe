...
const ReaderApp = {
    ...
    _renderHome(body) {
        ...
        if (!docs.length) {
            body.innerHTML = `
                <p class="library-intro">Drop documents here and read them with AI at hand — reports, essays, papers, statements, anything worth asking questions of. Everything is indexed on this Mac, searchable by meaning, and the assistant can quote and ground on it.</p>
                <p class="library-intro">Use <strong>Import files&hellip;</strong> above, drag files onto this page, or drop them into the folder in Finder (.md, .txt, .pdf, .docx, .html). Documents you give a Writing Voice appear here too.</p>`;
            return;
        }
