 ...
 *
 * Reads a budgeted SAMPLE of a voice's material and asks the user's
 * chosen brain for (a) a compact style guide and (b) exemplar passages
 * quoted verbatim. USER-TRIGGERED only, from the Writing Voices page —
 * the button is the consent, and this service owns no scheduler
 ...
 *
 * SOURCES (2026-08-30 restructure): an ordinary voice's material is its
 * Library folder's documents, as before. The SELF voice (VoiceStore
 * `self: true`) samples from whatever its `sources` map enables —
 * documents, Notes, Journal, and the user's SENT emails. The study budget
 * splits evenly across the enabled sources that actually have content, and
 * each source gets the same newest-plus-spread pick the documents always
 * had.
 *
...
const VoiceService = {
    ...
    async _study(collection) {
        ...
        if (!texts.length) {
            return { error: (voice && voice.self)
                ? 'Nothing to study yet — the sources you selected are empty. Add documents, write some notes or journal entries, or connect email.'
                : 'Nothing indexed here yet — import some of your writing first.' };
        }
    ...
    /** The voice's Library folder — the only source ordinary voices have. */
...
JSON object:`;
        ...
        if (!parsed) {
            ...
            if (response?.finish_reason === 'length') {
                return { error: 'The model\'s reply was cut off at its output limit before the study finished — nothing was changed. Try again, set Study depth to Light (Settings › Reader & Writing Voices), or use a model with a larger output window.' };
            }
