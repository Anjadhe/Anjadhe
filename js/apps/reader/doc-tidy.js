...
const DocTidy = {
    ...
    /**
     * May the tidy start on its own when a document opens? Asked of the
     * model that answers the Documents surface, not of the brain
     * (model-routing.js R4).
     */
    autoAllowed() {
        if (!this.available()) return false;
        if (typeof CloudPrivacy === 'undefined') return true;
        return !CloudPrivacy.leavesFor(this.SOURCE) || CloudPrivacy.allowsFor('files', this.SOURCE);
    },
    ...
    /** Can the model that reads documents see? (async: inputs load once) */
    async visionAvailable() {
        ...
        try {
            const entry = AgentService.entryForSource?.(this.VISION_SOURCE) ?? AgentService.getDefaultEntry?.();
            return !!AgentService.supportsVision(entry || null);
        } catch { return false; }
    },
