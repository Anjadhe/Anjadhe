 ...
 *
 * MY VOICE (2026-08-30 restructure): the app's default purpose is learning
 * the USER's voice. One voice per user carries `self: true` plus a
 * `sources` map ({documents, notes, journal, emails} booleans) naming what
 * its study reads — uploaded documents ride the voice's Library folder as
 * before; notes/journal/sent-emails are sampled live from their own stores
 * at study time (nothing is copied into the folder). Other voices stay
 * document-folder-only. A pre-restructure voice literally named "My voice"
 * is adopted as self by `adoptSelfOnce` (its corpus was documents, so it
 * keeps {documents: true}).
 *
...
const VoiceStore = {
    ...
    /** Normalized sources for a self voice; other voices are documents-only. */
    sourcesOf(page) {
        if (!page || !page.self) return { documents: true, notes: false, journal: false, emails: false };
        const s = (page.sources && typeof page.sources === 'object') ? page.sources : {};
        const out = {};
        for (const k of this.SOURCE_KEYS) out[k] = s[k] === true;
        // A self voice with every source off studies nothing — treat the
        // stored shape as "documents at least", the pre-restructure corpus.
     ...
     * create; its corpus was uploaded documents). Runs from LibraryApp.init
    ...
    adoptSelfOnce() {
        ...
        const mine = data.voicePages.find(p =>
            String(p.name || p.collection || '').trim().toLowerCase() === 'my voice');
        if (!mine) return;
        mine.self = true;
     ...
     * that will hold uploaded documents), with the user's source picks.
     */
    createSelf(sources, collection) {
        const existing = this.selfVoice();
        if (existing) return { voice: existing, existed: true };
        // A (renamed) voice may already sit on this folder — claim it as
        // self rather than returning it unclaimed, which would leave the
        // enable card up forever over a taken folder.
        const onFolder = this.pageFor(collection);
        if (onFolder) {
            const data = this._load();
            const page = data.voicePages.find(p => p.id === onFolder.id);
            page.self = true;
            const clean = {};
            ...
            this._save(data);
            return { voice: page, existed: true };
        }
        return this.create('My voice', collection, { self: true, sources });
    },

    /** Change which sources a self voice studies. Takes effect next study. */
    ...
    create(name, collection, opts = {}) {
        ...
        if (opts.self) {
            page.self = true;
            const src = {};
            for (const k of this.SOURCE_KEYS) src[k] = !!(opts.sources && opts.sources[k]);
            if (!this.SOURCE_KEYS.some(k => src[k])) src.documents = true;
            page.sources = src;
        }
