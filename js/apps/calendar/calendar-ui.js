...
const CalendarUI = {
    ...
    updateConnectVisibility() {
        ...
        // No accounts: a full first-run welcome, or — once the user is
        // already using the calendar over local tasks or the Apple mirror —
        // a slim connect hint. A permanent pitch page over a calendar
        // someone chose to use without Google would be furniture.
        if (prompt) {
        ...
        if (inUse) {
            prompt.innerHTML = `
                <div class="calendar-connect-slim">
                    <span>Showing your scheduled tasks. Connect Google to bring calendar events in too.</span>
                    <button id="calendar-connect-prompt-btn" class="secondary-btn" type="button">Open Settings</button>
                </div>`;
            return;
        }
        prompt.innerHTML = UIUtils.appWelcome({
            title: 'One calendar, all of it',
            lede: 'Google Calendar, your Mac&rsquo;s iCloud calendars, and the tasks you schedule here &mdash; one grid. Tasks will show below as you schedule them; connect Google to bring your events in.',
            cta: '<button id="calendar-connect-prompt-btn" class="primary-btn" type="button">Connect Google Calendar</button>',
            note: 'Apple calendars can join from Settings &rsaquo; Accounts &rsaquo; Apple apps.',
            rows: [
                ['Every account, one grid',
                 'Connect one or more Google accounts and scope the grid to any of them; recurring events, edits and deletes all round-trip to Google.'],
                ['Tasks on the same days',
                 'Anything you schedule in Tasks shows on the calendar too &mdash; the grid below works without any account at all.'],
                ['Ask about any event',
                 'Open an event and hand it to the assistant &mdash; the conversation already knows which event you mean.']
            ]
        });
    },
