...
const WorkroomEngine = {
    ...
    async transcript(room, events) {
        ...
        for (const event of events) {
            if (event.type === 'message' && event.from === 'you') {
                ...
                lines.push(line);
            } else if (event.type === 'message') lines.push(`You (Anjadhe): ${event.text}`);
    ...
    masterSystem() {
        return `You are Anjadhe, the user's personal AI, in a group chat with the user. They can hand you any digital task. You can bring specialists into the chat with the delegate tool.
How to work:
...
${Specialists.roster() || '(nobody is available right now)'}`;
    ...
    async run(room, run) {
        ...
        try {
            ...
            if (typeof Notify !== 'undefined' && !(typeof WorkroomsApp !== 'undefined' && WorkroomsApp.viewing(id))) Notify.show('Anjadhe replied', room.title, { kind: 'task' });
        } catch (error) {
