/* Workrooms — a chat where nenva can bring in specialists (2026-09-21).
 *
 * The page is a messaging app: a list of conversations on the left, ONE
 * thread on the right. You and nenva talk; when the job needs it an agent
...
const WorkroomsApp = {
    ...
    eventHtml(room, event, previous) {
        ...
        const sourcesHtml = () => (event.sources || []).slice(0, 4).map(url => { try { return `<a href="${esc(url)}">${esc(new URL(url).hostname.replace(/^www\./, ''))}</a>`; } catch { return ''; } }).filter(Boolean).join('');
        /* A specialist's report is evidence for Anjadhe, who answers the user
         * from it — so it sits folded under one line and opens on a click. */
        if (event.type === 'task_end') {
            const sources = sourcesHtml(), from = event.agent;
            const tag = event.status === 'blocked' ? '<span class="wr-tag">could not finish</span>' : event.status === 'partial' ? '<span class="wr-tag is-quiet">partial</span>' : '';
            const peek = String(event.text || '').replace(/\[([^\]]*)\]\([^)]*\)/g, '$1').replace(/[#*_`>|]+/g, '').replace(/\s+/g, ' ').trim().slice(0, 160);
            return `<div class="wr-msg is-report" data-seq="${event.seq}" data-from="${esc(from)}">${this.avatar(from)}<div class="wr-bubble${event.status === 'blocked' ? ' is-blocked' : ''}">
                <details class="wr-report"><summary><span class="wr-report-head"><span class="wr-report-name">${esc(this.label(from))}’s report</span>${tag}</span><span class="wr-report-peek">${esc(peek)}</span></summary>
                ${event.note ? `<p class="wr-report-note">${esc(event.note)}</p>` : ''}${this.prose(event.text)}${sources ? `<div class="wr-sources">${sources}</div>` : ''}</details>
                ${event.status === 'blocked' && from === 'browser' ? '<div class="wr-replies"><button data-wr="agent" data-agent="browser">Show the browser</button></div>' : ''}<time>${esc(this.time(event.at))}</time></div></div>`;
        }
        if (event.type === 'message') {
            const from = event.from;
            const follows = previous?.type === 'message' && previous.from === from;
            const sources = sourcesHtml();
            return `<div class="wr-msg${follows ? ' is-follow' : ''}${from === 'master' ? ' is-master' : ''}" data-seq="${event.seq}" data-from="${esc(from)}">${this.avatar(from)}<div class="wr-bubble">
                <header><button class="wr-name" ${from === 'master' ? 'disabled' : `data-wr="agent" data-agent="${esc(from)}"`}>${esc(this.label(from))}</button></header>
                ${this.prose(event.text)}${sources ? `<div class="wr-sources">${sources}</div>` : ''}<time>${esc(this.time(event.at))}</time></div></div>`;
        }
        if (event.type === 'join') return line(`${esc(this.label(event.agent))} joined`);
        if (event.type === 'task') return `<details class="wr-line wr-brief" data-seq="${event.seq}"><summary><span>nenva asked ${esc(this.label(event.agent))}</span></summary><p>${esc(event.text)}</p></details><div class="wr-ticker" data-task="${esc(event.id)}">${this.tickerHtml(room, event.id)}</div>`;
    ...
    renderStatus(room) {
        ...
        if (sub) sub.textContent = ['You', 'nenva', ...(room.members || []).map(id => this.label(id))].join(', ');
        if (stop) {
    ...
    renderPanel(room) {
        ...
        if (this._panel === 'info') {
            ...
            panel.innerHTML = `<header><h2>Group info</h2>${close}</header>
                <h3>In this chat</h3><ul class="wr-members"><li>${'<span class="wr-avatar" aria-hidden="true">Y</span>'}<span>You</span></li><li>${this.avatar('master')}<span>nenva</span></li>
                ...
                ${(room.members || []).length ? '' : '<p class="wr-hint">nenva brings in agents as the work needs them.</p>'}
        ...
        const blocks = tasks.map(task => {
            ...
            const state = end ? (end.status === 'done' ? 'Finished' : end.status === 'partial' ? 'Finished in part' : 'Could not finish') : room.status === 'running' ? 'Working' : 'Stopped';
            ...
            const refresh = async () => {
                ...
                try {
                    ...
                    status.textContent = !state.supported ? 'Available on macOS and Linux for now.' : !state.available ? 'Install Google Chrome first.'
                        : state.stale ? 'Connected, but the extension loaded in Chrome is older than this app. Open chrome://extensions and reload the nenva extension.'
...
if (FEATURES.isEnabled('workrooms')) {
    ...
    tile.dataset.desc = 'Hand nenva a job and it brings in the agents it needs.';
    ...
    AgentTools.register({ type: 'function', function: {
        ...
        if (WorkroomsApp.locked()) return { error: 'Unlock nenva before creating a workroom.' };
