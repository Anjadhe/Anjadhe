...
const ActionsReview = {
    ...
    STEP_TITLES: ['Wins', 'Projects check', 'Overdue sweep', 'No-date backlog', 'Week ahead'],
    ...
    _noDateItems() {
        return ScheduleApp.scheduleItems.filter(i =>
    ...
    _renderWins() {
        ...
        return `
            <p class="review-lede">You finished <strong>${all.length}</strong> thing${all.length === 1 ? '' : 's'} this week${w.goals.length ? `, including ${w.goals.length} project${w.goals.length === 1 ? '' : 's'}` : ''}. Take the credit.</p>
            <div class="review-list">
                ${shown.map(x => `<div class="review-win-row">${x.goal ? '<span class="review-win-goal">Project</span>' : '&#10003;'} ${UIUtils.escapeHtml(x.t)}</div>`).join('')}
    ...
    _renderGoalsCheck() {
        ...
        if (stalled.length === 0) {
            return `<p class="review-lede">All ${open} open project${open === 1 ? '' : 's'} saw activity in the last two weeks. Nothing to untangle.</p>`;
        }
        return `
            <p class="review-lede">${stalled.length} project${stalled.length === 1 ? ' has' : 's have'} had no action in two weeks. Still worth your time?</p>
            <div class="review-list">
                ${stalled.map(g => `
    ...
    _tallyParts() {
        ...
        if (t.goalsResolved) parts.push(`${t.goalsResolved} stalled project${t.goalsResolved === 1 ? '' : 's'} resolved`);
    ...
    _goalVerdict(act, goalId) {
        ...
        if (act === 'keep') {
            ...
            // No stuck status any more (a goal is completed or it is not) —
            // stuck is a conversation: hand the goal to the assistant.
            goal.modifiedAt = new Date().toISOString();
            if (typeof AgentUI !== 'undefined' && AgentUI.askWithPrompt) {
                AgentUI.askWithPrompt(
                    `I am stuck on my project "${goal.title}". Ask me what is blocking it, then help me find the smallest next step and schedule it.`,
                    { newChat: true });
            }
        } else {
            return;
