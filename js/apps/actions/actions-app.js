...
const ActionsApp = {
    ...
    _normalizeSel(sel) {
        ...
        if (group !== 'unassigned' && group !== null
            && !(typeof group === 'string' && group.startsWith('g:'))) {
            group = null;
        }
    ...
    _renderTaskAskRow() {
        ...
        row.innerHTML = item ? this.taskAskPillsHtml(item.title) : '';
        row.querySelectorAll('[data-ask]').forEach(b =>
            b.addEventListener('click', () => AgentUI.askWithPrompt(b.dataset.ask, { newChat: true })));
    },
    ...
    _renderRangeView(pred = null) {
        ...
        if (container) {
            ...
            if (days.length === 0) {
                html = `<div class="actions-empty">Nothing scheduled for ${this._selLabel().toLowerCase()}. Add an action above, or open the Goals tab to line up your goals.</div>`;
    ...
    _renderGroupView() {
        ...
        if (container) {
            ...
            if (openCount === 0) {
                html += `<div class="actions-empty">No open tasks for the ${UIUtils.escapeHtml(name || 'Ungrouped')} goals. Open a goal on the Goals tab to add some.</div>`;
            }
            html += this._doneSectionHtml(done);
            container.innerHTML = html;
        }
    ...
    _renderSelLine(summaryParts) {
        ...
        // Right-aligned action: open the group on the Goals tab when one is
        // in scope; otherwise the weekly-review link on the plain Today view.
        let trailing = '';
        if (this._isGroupSel()) {
            trailing = '<button class="actions-review-nudge" id="actions-open-goals" title="Open this group on the Goals tab">Open Goals</button>';
            ...
            html += `
                <div class="actions-empty">
                    ${doneCount > 0
                        ...
                        : 'Nothing scheduled for today. Add an action above, or open the Goals tab to line up your goals.'}
                </div>`;
        }
...
Respond ONLY with a JSON object mapping each task number to its verdict, e.g. {"1":{"goal":"G2"},"2":{"goal":"none","date":"${today}"}}.`
    ...
    _bindOnce() {
        ...
        // Hub strip: Tasks is this view; Goals opens the Goals page.
        document.querySelectorAll('#actions-view .actions-hub-btn').forEach(btn => {
    ...
    /**
     * Wire an Actions hub strip embedded in another rung's header (the
     * Goals page) — same markup as the Actions view's own strip, with that
     * rung's button carrying is-active. Clone-replace so repeated init
     * passes don't stack listeners.
     */
    wireHubNav(viewId) {
