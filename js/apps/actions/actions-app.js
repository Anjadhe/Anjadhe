...
const ActionsApp = {
    ...
    render() {
        ...
        // A selected focus area (or group) may have been deleted or synced away.
        if (this._sel.focus && this._sel.focus !== 'unassigned') {
            const gone = this._isGroupSel()
                ? this._groupAreas(this._selGroupName()).length === 0
                : !this._focusArea(this._sel.focus);
            if (gone) this._sel = this._normalizeSel({ time: this._sel.time, focus: null });
        }
    ...
    _focusLabel() {
        ...
        if (this._isGroupSel()) return this._selGroupName() || 'Ungrouped';
    ...
    _focusPredicate() {
        ...
        // A group scope is the union of its areas' subtrees.
        const subtree = this._isGroupSel()
            ? this._groupSubtreeIds(this._selGroupName())
            : ScheduleApp.getFocusSubtreeIds(f, this._focusAreas());
        return (i) => {
    ...
    _focusAreas() {
        return (StorageManager.get('focus')?.focusItems) || [];
    },
    ...
    // --- Group scope ('group:<name>' in _sel.focus; '' = Ungrouped) ---

    _isGroupSel() {
        return typeof this._sel.focus === 'string' && this._sel.focus.startsWith('group:');
    },

    _selGroupName() {
        return this._isGroupSel() ? this._sel.focus.slice('group:'.length) : null;
    },

    /** The group key an area belongs to (trimmed name, '' = Ungrouped). */
    _groupOf(area) {
        return (typeof area.group === 'string' && area.group.trim()) || '';
    },

    _groupAreas(name) {
        return this._focusAreas().filter(a => this._groupOf(a) === name);
    },

    _groupSubtreeIds(name) {
        const all = this._focusAreas();
        const ids = new Set();
        for (const a of this._groupAreas(name)) {
            for (const id of ScheduleApp.getFocusSubtreeIds(a.id, all)) ids.add(id);
        }
        return ids;
    },
    ...
    _syncViewChrome() {
        ...
        if (!inReview) Breadcrumb.render('actions-breadcrumb', [{ label: 'Actions' }]);
    ...
    _renderNav() {
        ...
        if (areas.length > 0) {
            ...
            for (const g of order) {
                // Group labels are filters too: clicking one scopes the list
                // to every area in the group (union of their subtrees).
                if (showLabels) html += `
                    <button type="button" class="actions-nav-group${isSel('focus', 'group:' + g) ? ' is-active' : ''}"
                            data-nav-group="${UIUtils.escapeHtml(g)}" title="Show tasks in every ${UIUtils.escapeHtml(g || 'Ungrouped')} area">
                        ${UIUtils.escapeHtml(g || 'Ungrouped')}
                    </button>`;
    ...
    _renderAreaView() {
        ...
        const { taskFocus, taskGoals } = ScheduleApp.buildTaskLinkIndex();
        const linked = ScheduleApp.scheduleItems.filter(item => {
            const set = taskFocus.get(item.id);
            if (!set) return false;
            for (const fid of set) if (subtree.has(fid)) return true;
            return false;
    ...
    // --- Focus-group view (a nav group label, at All time) ---

    // Open tasks across every area in the group, sectioned per area so the
    // view reads as a mini overview of the group. A task linked into several
    // of the group's areas lands under the first one (nav order).
    _renderGroupView() {
        const name = this._selGroupName();
        const areas = this._groupAreas(name);
        const all = this._focusAreas();
        ...
        const sections = areas.map(a => ({ area: a, subtree: ScheduleApp.getFocusSubtreeIds(a.id, all), items: [] }));
        const done = [];
        let openCount = 0;
        for (const item of ScheduleApp.scheduleItems) {
            if (!item.title) continue;
            const set = taskFocus.get(item.id);
            if (!set) continue;
            const sec = sections.find(s => { for (const fid of set) if (s.subtree.has(fid)) return true; return false; });
            if (!sec) continue;
        ...
        if (container) {
            ...
            for (const sec of sections) {
                ...
                sec.items.sort(byUrgency);
                html += `
                    <div class="actions-section">
                        <button class="actions-section-header actions-goal-heading" data-open-area="${sec.area.id}" title="Show only ${UIUtils.escapeHtml(sec.area.title)}">
                            <span class="actions-nav-dot" style="background:${sec.area.color || '#4A90A4'}"></span>
                            ${UIUtils.escapeHtml(sec.area.title)} <span class="actions-section-count">${sec.items.length}</span>
                        </button>
                        ${sec.items.map(row).join('')}
                    </div>`;
            }
            if (openCount === 0) {
                html += `<div class="actions-empty">No open tasks in the ${UIUtils.escapeHtml(name || 'Ungrouped')} areas. Drag a task onto one of them in the sidebar to file it.</div>`;
            }
            html += this._doneSectionHtml(done);
            container.innerHTML = html;
        }
    ...
    _renderSelLine(summaryParts) {
        ...
        if (sel.focus) {
            // A group pill carries no single color — leave the dot off.
            let dot = '';
            if (sel.focus === 'unassigned') {
                dot = `<span class="actions-nav-dot" style="background:transparent;box-shadow:inset 0 0 0 1.5px var(--color-text-tertiary)"></span>`;
            } else if (!this._isGroupSel()) {
                const area = this._focusArea(sel.focus);
                dot = `<span class="actions-nav-dot" style="background:${(area && area.color) || '#4A90A4'}"></span>`;
            }
...
Respond ONLY with a JSON object mapping each task number to its verdict, e.g. {"1":{"goal":"G2"},"2":{"goal":"none","date":"${today}"}}.`
    ...
    _pendingSuggestionItems() {
        return ScheduleApp.scheduleItems
    ...
    _bindOnce() {
        ...
        if (nav) {
            nav.addEventListener('click', (e) => {
                ...
                if (group) { this._toggleFocus('group:' + group.dataset.navGroup); return; }
        ...
        document.getElementById('actions-date-line')?.addEventListener('click', (e) => {
            ...
            if (e.target.closest('#actions-open-plan') && this._sel.focus && this._sel.focus !== 'unassigned') {
                // Group scope has no single area to select — just open Plan.
                const areaId = this._isGroupSel() ? null : this._sel.focus;
                AppManager.openApp('focus');
                if (areaId) setTimeout(() => FocusApp.selectNode('area', areaId), 0);
            }
        });
        ...
        if (container) {
            ...
            container.addEventListener('click', (e) => {
                ...
                // Area section headings in the group view narrow to that area.
                const areaHeading = e.target.closest('[data-open-area]');
                if (areaHeading) {
                    this.select({ time: this._sel.time, focus: areaHeading.dataset.openArea });
                    return;
                }
