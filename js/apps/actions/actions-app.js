...
const ActionsApp = {
    ...
    render() {
        ...
        if (this._sel.time === 'all') {
            ...
            else if (this._sel.group === 'src:email') this._renderTagView(pred, '<div class="actions-empty">No tasks from email yet — Insights files action items here as it reads your mail and texts.</div>');
...
Task id: ${item.id}${item.description ? `\n\nNotes:\n${item.description}` : ''}${emailBlock}`,
    ...
    _groupPredicate() {
        ...
        // every task the email pipeline created (sourceEmailId, stamped by
        // syncActionItemsToSchedule and the insight "Add task" door).
        if (f === 'src:email') return (i) => !!i.sourceEmailId;
    ...
    _renderNav() {
        ...
        // Other filters — provenance slices. First (and so far only) one:
        // tasks the email pipeline created (sourceEmailId). The section
        // appears once any exist and stays while any exist, completed
        // included — the _allTaskTags stability rule; the count itself is
        // open tasks, quiet inventory like every non-Today number.
        if (ScheduleApp.scheduleItems.some(i => i.sourceEmailId)) {
    ...
    _navCounts() {
        ...
        for (const item of ScheduleApp.scheduleItems) {
            ...
            // Same rule for provenance — an email-born task with no goal
            // link must still count.
            if (item.sourceEmailId) counts.fromEmail++;
