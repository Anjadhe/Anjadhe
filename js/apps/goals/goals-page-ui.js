...
const GoalsPageUI = {
    ...
    /**
     * The table: one row per project — title (+ draft chip), the group as
     * its own quiet column when the table spans groups (2026-09-11: a dot
     * and the name in secondary ink, a filter door like the chip it
     * replaced), progress track + n/m, next open step, last update, target. Rows are
    ...
    _tableHtml(goals, facts, { showGroup, completed = false, today }) {
        let html = `<div class="goals-table${completed ? ' is-completed' : ''}${showGroup ? ' has-group' : ''}" role="table">
            <div class="goals-tr goals-th" role="row">
                <span>Project</span>${showGroup ? '<span>Group</span>' : ''}<span>Progress</span><span>${completed ? 'Completed' : 'Next step'}</span><span>Last update</span><span>${completed ? '' : 'Target'}</span>
            </div>`;
        for (const g of goals) {
            ...
            html += `<button type="button" class="goals-tr${completed ? ' completed' : ''}${f.quiet && !completed ? ' is-quiet' : ''}" role="row" data-open-goal="${g.id}"${this._hueStyle(grp)}>
                ...
                ${showGroup ? (grp !== GoalsApp.UNGROUPED
                    ? `<span class="goals-td-group" data-filter-group="${this._esc(grp)}" title="Only ${this._esc(grp)}">${this.groupHue(grp) ? '<span class="goals-hue-dot" aria-hidden="true"></span>' : ''}<span class="goals-td-group-name">${this._esc(grp)}</span></span>`
                    : '<span class="goals-td-group"><span class="goals-td-empty">&mdash;</span></span>') : ''}
    ...
    _moreHint(goal, goalId) {
        const bits = [];
        try {
            const routine = GoalInterview.findReview(goal.title, goalId);
    ...
    _attachGoalReviewListeners(container) {
        container.querySelectorAll('[data-goal-review-start]').forEach(btn => {
            btn.addEventListener('click', () => {
                GoalInterview.startWeeklyReview(btn.dataset.goalReviewStart, btn.dataset.goalReviewId || null);
    ...
    _goalReviewHtml(goal) {
        ...
        const routine = GoalInterview.findReview(goal.title, goal.id);
        if (routine) {
            return `
                <div class="ai-review-foot goals-review-foot">
                    nenva reviews this project ${esc(NotePrompts.scheduleLabel(NotePrompts.config(routine)))} and posts its read to the Updates above and the Home feed.
        ...
        return `
            <div class="ai-review-foot goals-review-foot">
                ...
                <button type="button" class="quiet-link-btn" data-goal-review-start="${esc(goal.title)}" data-goal-review-id="${esc(goal.id)}">Review it weekly</button>
            </div>`;
    },
