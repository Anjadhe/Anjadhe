...
const GoalsPageUI = {
    ...
    _attnBriefing(goals) {
        ...
        const noStep = goals.filter(g => {
            if (g.status === 'completed' || g.status === 'draft') return false;
            if (datedIds.has(g.id)) return false;
            const c = LinkManager.getTaskCountForGoal(g.id);
