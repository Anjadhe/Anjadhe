...
const GoalsPageUI = {
    ...
    renderNav() {
        ...
        let html = `<div class="gnav-new-row">
            <button type="button" class="actions-nav-item gnav-new${newActive ? ' is-active' : ''}"
                    data-new-goal-home title="Start a new project">
                <span class="actions-nav-label">+ New project</span>
            </button>
        </div>`;
        ...
        for (const [g, goals] of groupGoals) {
            ...
            html += shown.map(goal => {
                ...
                return `<button type="button" class="actions-nav-item gnav-goal${goal.status === 'completed' ? ' completed' : ''}${selGoal && selGoal.id === goal.id ? ' is-active' : ''}"
                    ...
                    ${count.total ? `<span class="gnav-progress" style="--gp:${(count.completed / count.total).toFixed(3)}" aria-hidden="true"></span><span class="actions-nav-count">${count.completed}/${count.total}</span>` : ''}
                </button>`;
            }).join('');
        }
        nav.innerHTML = html;
    ...
    _newGoalAskRow(group) {
        ...
        return `<div class="ask-prompt-row goals-ask-row">
            ...
            <button type="button" class="quiet-link-btn goals-manual-add" data-new-goal${scope}>+ New project by hand</button>
        </div>`;
    },
    ...
    renderNewGoalHome(pane) {
        ...
        pane.innerHTML = `<div class="goals-overview goals-blank">
            <div class="goals-detail-eyebrow"><span>New project</span></div>
    ...
    renderGroupOverview(pane) {
        ...
        const label = group === GoalsApp.UNGROUPED ? 'Other projects' : group;
        ...
        let html = `<div class="goals-overview">
            ...
            <div class="goals-overview-sub">${goals.length} project${goals.length === 1 ? '' : 's'} &mdash; rename or move projects to reshape the group</div>`;

        if (goals.length === 0) {
            html += '<div class="goals-empty-line">No projects in this group yet.</div>';
        } else {
    ...
    _renderGoalCard(goal) {
        ...
        html += `<div class="goals-card-header" data-goal-id="${goal.id}">
                    ...
                    <span class="goals-card-nav" title="Open project">&#8594;</span>
                 </div>`;
        if (expanded) {
            if (tasks.length > 0) {
        ...
        container.querySelectorAll('[data-goal-ask]').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const prompts = {
                    ...
                    status: `How is my project "${title}" going? Ground your read in its linked tasks - what got done recently, what is scheduled next, and how long it has been since movement - then tell me plainly.`
                };
    ...
    renderGoalDetail(goalId, pane) {
        ...
        const html = `<div class="goal-editor-main goals-goal-embed">
            <div class="goals-detail-crumbs">
                ${group ? `<span class="breadcrumb-link" data-crumb-group="${this._esc(group)}">${this._esc(group)}</span><span class="breadcrumb-separator">&#8250;</span>` : ''}<span class="breadcrumb-current">Project</span>
            ...
            <div class="ask-prompt-row goals-detail-ask">${this._goalAskButtons(goal)}<button type="button" class="ask-prompt-btn ask-prompt-open" data-ask-open-goal>Ask about this project…</button></div>
