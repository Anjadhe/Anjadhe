/**
 * Goal Interview (the record is labeled "project" since 2026-08-30 —
 * labels and prose only; the object name, tool names and storage keys
 * still say goal, the Actions-is-Tasks rename law)
 *
 * A project the user arrived at by TALKING to the assistant, not by filling
 * in a form — the same shape as the Portfolio Strategy interview, because
 * the problem is the same: users generally do not know what a workable
 * project needs to contain. Asking "what's the goal?" gets a wish ("get
 * fit"); the agenda below is what turns it into an outcome with a finish
 * line, a date, and a task timeline on the calendar.
 *
...
const GoalInterview = {
    ...
    // project orphans its routine, and the page simply offers to start
    // again. The pre-rename prefix is still recognized (findReview) and
    // healed forward (_migrateReviewTitles), so nobody's weekly reviews
    // were orphaned by the Goals → Projects label rename.
    GOAL_REVIEW_PREFIX: 'Project Review: ',
    LEGACY_REVIEW_PREFIX: 'Goal Review: ',
    ...
    INTERVIEW: [
        {
            ...
            why: 'A project is something you can finish. Naming the outcome — not the activity — is what separates a project from a wish.',
        ...
        },
        {
            ...
            why: 'The why is what keeps a project alive in week six. It is also what a review quotes back when progress stalls.',
        ...
        },
        {
            ...
            why: 'Projects are shown grouped — Work, Health, Family — so related outcomes sit together. Reuse an existing group when one fits.',
        ...
        },
        {
            ...
            why: 'A project only moves when it is on the calendar. Three to six tasks with dates spread toward the target date turn the plan into a timeline.',
        ...
        },
        {
            id: 'review',
            required: false,
            question: 'Want a weekly AI review of this project?',
            why: 'A project nobody looks at quietly stops being a project. The review reads the linked tasks each week and posts an honest read to your Home feed.',
    ...
    save(patch = {}) {
        ...
        // A renamed project carries its review routine (and any routine
        // quoting the old title) along — see ReviewRoutines.syncRename.
        // Legacy-prefixed titles are healed first so the convention match
        // sees the current prefix.
        if (renamedFrom) this._migrateReviewTitles();
    ...
    /**
     * The project's review routine, under the current OR the pre-rename
     * title prefix — the one lookup every surface should use, so a routine
     * a not-yet-migrated Mac wrote is still found rather than duplicated.
     */
    findReview(goalTitle, goalId) {
        if (typeof ReviewRoutines === 'undefined') return null;
        // A1: the id is the link. A routine that carries one is found after
        // any rename on either side; the title prefixes below are the
        // fallback for every routine written before the field existed.
        if (goalId) {
            const byRef = ReviewRoutines.findFor('goal', goalId);
            if (byRef) return byRef;
        }
        if (!goalTitle) return null;
        return ReviewRoutines.find(this.GOAL_REVIEW_PREFIX + goalTitle)
            || ReviewRoutines.find(this.LEGACY_REVIEW_PREFIX + goalTitle);
    },

    /**
     * Create the "Project Review: <title>" routine — weekly, personal
     * context, no web (the project's ground truth is the user's own data).
     * The one recipe both the project page and the save_goal tool use, so
     * UI-created and agent-created routines never drift.
     */
    startWeeklyReview(goalTitle, goalId) {
        if (typeof ReviewRoutines === 'undefined' || !goalTitle) return null;
        this._migrateReviewTitles();
        const existing = this.findReview(goalTitle, goalId);
        if (existing) {
            // An existing review written before references: give it the id
            // now, so the next rename cannot orphan it.
            if (goalId && typeof NotePrompts !== 'undefined'
                && !NotePrompts.config(existing).about.length) {
                NotePrompts.update(existing.id, { config: { ...NotePrompts.config(existing), about: [{ type: 'goal', id: goalId }] } });
            }
            return existing;
        }
        return ReviewRoutines.start({
            title: this.GOAL_REVIEW_PREFIX + goalTitle, body: this._reviewBody(goalTitle),
            interval: 'weekly', time: '09:00', web: false, useContext: true,
            about: goalId ? [{ type: 'goal', id: goalId }] : []
        });
    },

    _reviewBody(goalTitle) {
        return `Review my project "${goalTitle}". Call list_goals and look at the tasks linked to it and its updates: what got done since the last review, what is scheduled next, how long it has been since anything moved, and how long since the last update (daysSinceUpdate). ` +
            'Check the project\'s ageDays first: a project only days old is JUST STARTING, never "stalled" — for a new project, judge whether the first steps are scheduled and say it is under way. ' +
            'For an established project, write a short honest read: is it moving, stalled, or done in all but name? Name any risk to the target date. If it is stalled, name the smallest next step worth scheduling; if the log has been quiet for two weeks or more, say so. ' +
            'If nothing changed since the last review, say so in two sentences and stop. This review is also posted to the project\'s Updates log.';
    },

    /**
     * Mirror a Project Review's feed post into the project's Updates log
     * (source 'ai', kind 'review'), keyed by the post id so a second
     * sighting of the same post — the other Mac, a re-render — never
     * duplicates it. Called by PromptFeed after every successful post; a
     * routine that is not a review, or a review whose project is gone,
     * mirrors nothing.
     */
    mirrorReviewPost(prompt, note) {
        if (!prompt || !note || note.feed?.error || typeof UpdateStore === 'undefined' || typeof ReviewRoutines === 'undefined') return null;
        const title = String(prompt.title || '').trim();
        const prefix = [this.GOAL_REVIEW_PREFIX, this.LEGACY_REVIEW_PREFIX].find(p => title.startsWith(p));
        if (!prefix) return null;
        const goalTitle = title.slice(prefix.length).trim().toLowerCase();
        const goal = ((StorageManager.get('goals')?.goals) || []).find(g => (g.title || '').trim().toLowerCase() === goalTitle);
        if (!goal) return null;
        const body = ReviewRoutines.excerpt(note, 700);
        if (!body) return null;
        const res = UpdateStore.add({ key: `goal:${goal.id}`, body, source: 'ai', kind: 'review', postId: note.id });
        return res.update || null;
    },

    // Templates startWeeklyReview wrote in the past — the 2026-07-31
    // pre-ageDays original and the pre-rename "goal"-worded version
    // (2026-08-30) — kept verbatim so the healer below can recognize an
    // unmodified machine-authored body.
    _legacyReviewBodies(goalTitle) {
        return [
            // 2026-08-30 → 2026-09-10: the pre-Updates project-worded body.
            `Review my project "${goalTitle}". Call list_goals and look at the tasks linked to it: what got done since the last review, what is scheduled next, and how long it has been since anything moved. ` +
                'Check the project\'s ageDays first: a project only days old is JUST STARTING, never "stalled" — for a new project, judge whether the first steps are scheduled and say it is under way. ' +
                'For an established project, write a short honest read: is it moving, stalled, or done in all but name? If it is stalled, name the smallest next step worth scheduling. ' +
                'If nothing changed since the last review, say so in two sentences and stop.',
            `Review my goal "${goalTitle}". Call list_goals and look at the tasks linked to it: what got done since the last review, what is scheduled next, and how long it has been since anything moved. ` +
                ...
                'If nothing changed since the last review, say so in two sentences and stop.',
            `Review my goal "${goalTitle}". Look at the tasks linked to it: what got done since the last review, what is scheduled next, and how long it has been since anything moved. ` +
    ...
    /**
     * 2026-08-30 label rename (Goals → Projects): retitle machine-authored
     * "Goal Review: X" routines to "Project Review: X". The prefix is the
     * recipe's own writing, never the user's prose, and the note keeps its
     * id so past feed reviews stay in its series. Idempotent; skipped when
     * a routine already holds the new title (two-Mac race).
     */
    _migrateReviewTitles() {
        if (typeof NotePrompts === 'undefined' || typeof ReviewRoutines === 'undefined') return;
        for (const n of NotePrompts.list()) {
            if (!NotePrompts.isRoutine(n)) continue;
            const title = (n.title || '').trim();
            if (!title.startsWith(this.LEGACY_REVIEW_PREFIX)) continue;
            const newTitle = this.GOAL_REVIEW_PREFIX + title.slice(this.LEGACY_REVIEW_PREFIX.length);
            if (ReviewRoutines.find(newTitle)) continue;
            NotePrompts.update(n.id, { title: newTitle });
        }
    },

    /**
     * Refresh existing "Project Review: <title>" routines whose body is
     * still an old machine-written template — a body the user edited is
     * never touched. Also heals pre-rename "Goal Review: " titles forward.
     * Called once per session from GoalsPage.init.
     */
    refreshReviewBodies() {
        ...
        for (const n of NotePrompts.list()) {
            ...
            const body = norm(NotePrompts.bodyText(n));
            if (this._legacyReviewBodies(goalTitle).some(t => norm(t) === body)) {
