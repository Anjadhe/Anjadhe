...
const GoalsPage = {
    ...
    setGoalField(goalId, field, value, { render = true, syncRename = true } = {}) {
        ...
        // Quiet writes (typing) repaint only the rail so the row's name
        // follows the title without rebuilding the sheet under the caret.
        if (render) this.render(); else GoalsPageUI.renderNav();
    },
    ...
    setGoalGroup(goalId, group, opts = {}) {
        ...
        if (opts.render === false) { if (changed) GoalsPageUI.renderNav(); return; }
        this.render();
...
- When the user's question is about "this project", "the project", or asks to update / refine it, work with the data below. To modify it, call save_goal (it merges) or update_goal with id: "${goal.id}".
