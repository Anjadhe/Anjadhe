...
GoalsPage.goalContextBlock = function (goalId, opts = {}) {
    ...
    const lead = opts.attached
        ? 'This conversation is attached to the goal below (the user may not have it open right now)'
        : 'The user is viewing or editing the goal below';
    return {
        recordKey: 'goals:' + goal.id,
        recordLabel: goal.title || '(untitled goal)',
        title: 'CURRENT GOAL',
        body: `${lead}. The goal is available as context, not a constraint:
