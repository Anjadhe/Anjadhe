    ...
    async suggest({ goalId }, opts = {}) {
        ...
        try {
            ...
            if (suggestions.length === 0) {
                bodyEl.innerHTML = '<div class="goal-ai-wait">No suggestions came back for this project. A short description of what &ldquo;done&rdquo; looks like usually helps.</div>';
                return;
            }
            this._renderSuggestions(bodyEl, modal, suggestions, { goalId }, opts);
        } catch (err) {
            const bodyEl = document.getElementById('goal-ai-body');
...
Break the user's project into concrete next tasks. Respond ONLY with a JSON object:
...
- "date" is OPTIONAL: include it only when the project's target date makes a schedule obvious; otherwise omit it.
- Dates must form a realistic timeline: work takes days or weeks, so spread them out between today and the target date, each task later than the one before. NEVER give every task the same date.
- Never repeat the user's existing tasks.`
        ...
        // Floor-model defense: small models often stamp every task with the
        // same date (usually the target date) — a "schedule" that says
        // nothing. Drop the dates entirely; undated plan steps are the
        // feature's honest default.
        const dated = out.filter(t => t.date);
        if (dated.length > 1 && new Set(dated.map(t => t.date)).size === 1) {
            for (const t of out) t.date = null;
        }
        return out;
    },
