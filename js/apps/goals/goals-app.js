...
const GoalsApp = {
    ...
    /**
     * Rename a group — i.e. rewrite the `group` field on every goal that
     * carries it (a group IS that label; there is no group record). `to`
     * blank dissolves the group: its goals stay, ungrouped. Renaming onto
     * an existing group's name merges into it. One save; every touched
     * goal stamps modifiedAt (the record merge keys on it). Returns the
     * number of goals moved.
     */
    renameGroup(from, to) {
        const src = (from || '').trim();
        const dst = (to || '').trim();
        if (!src || src === this.UNGROUPED || src === dst) return 0;
        const next = dst === this.UNGROUPED ? '' : dst;
        const now = new Date().toISOString();
        let n = 0;
        for (const g of this.goals) {
            if (this.groupOf(g) !== src) continue;
            g.group = next;
            g.modifiedAt = now;
            n++;
        }
