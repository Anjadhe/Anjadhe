...
import { startLanRelay, isLocalConnection } from './lan-relay.mjs';
...
// The exported gate: loopback socket, no proxy headers. A Tailscale Funnel
// visitor arrives from 127.0.0.1 via tailscaled's local proxy and must NOT
// count as local (it could squat the host slot otherwise).
const req = (addr, headers = {}) => ({ socket: { remoteAddress: addr }, headers });
check('isLocalConnection accepts a bare loopback socket',
  isLocalConnection(req('127.0.0.1')) && isLocalConnection(req('::1', { host: 'x' })));
check('isLocalConnection rejects a non-loopback socket',
  !isLocalConnection(req('192.168.1.20')) && !isLocalConnection(req('')) && !isLocalConnection(null));
check('isLocalConnection rejects a Funnel-forwarded loopback socket',
  !isLocalConnection(req('127.0.0.1', { 'tailscale-funnel-request': '?1' }))
  && !isLocalConnection(req('127.0.0.1', { 'x-forwarded-for': '203.0.113.9' }))
  && !isLocalConnection(req('::1', { forwarded: 'for=203.0.113.9' })));
