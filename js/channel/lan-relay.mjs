 ...
 * (nenva Connect `/v1/relay`) and the standalone dev relay
 ...
 *     Noise handshake would still fail against an impostor). Loopback
 *     means the SOCKET is loopback AND nothing forwarded it: a Tailscale
 *     Funnel / Serve proxy (js/main/tailscale.js, 2026-09-08) hands a
 *     visitor from the internet to this port from 127.0.0.1, marked by
 *     `Tailscale-Funnel-Request` / `X-Forwarded-For` headers — those
 *     are remote, whatever the socket says.
...
// Headers a reverse proxy stamps on a forwarded request. A local proxy
// (Tailscale Funnel/Serve, an ssh -R, nginx) connects from loopback on the
// visitor's behalf; the visitor is not local.
const FORWARDED_HEADERS = ['x-forwarded-for', 'tailscale-funnel-request', 'forwarded', 'x-real-ip'];

/** True only for a socket that is loopback AND was not forwarded by a proxy. */
export function isLocalConnection(req) {
  const addr = (req && req.socket && req.socket.remoteAddress) || '';
  if (!LOOPBACK_RX.test(addr)) return false;
  const headers = (req && req.headers) || {};
  return !FORWARDED_HEADERS.some((h) => h in headers);
}
...
export function startLanRelay(port = 0) {
  ...
  wss.on('connection', (ws, req) => {
    ...
    ws.isLoopback = isLocalConnection(req);
