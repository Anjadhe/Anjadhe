 ...
 */
export function createPairingOffer(hostIdentity, { relayUrl, routingId, lanUrls }) {
  ...
  // The Mac's own LAN relay addresses, if it runs one — the phone tries
  // these before the hosted relay (direct beats relayed when reachable).
  const lan = Array.isArray(lanUrls)
    ? lanUrls.filter((u) => typeof u === 'string' && /^wss?:\/\//.test(u)).slice(0, 4)
    : [];
  return {
    offer: {
      v: 1,
      relayUrl,
      ...(lan.length ? { lanUrls: lan } : {}),
      routingId: rid,
...
export function acceptPairingOffer(offer, phoneIdentity) {
  ...
  return {
    ...
    pairing: {
      ...
      ...(Array.isArray(offer.lanUrls) && offer.lanUrls.length
        ? { lanUrls: offer.lanUrls } : {}),
