/**
 * End-to-end test for the nenva channel.
 *
...
// a payload too large for one relay frame is chunked and reassembled —
// 1.2M chars of plaintext seals to ~2.4M hex chars, i.e. 4 chunk frames
const bigText = 'x'.repeat(1_200_000);
client.send({ type: 'ping', text: bigText });
await wait(500);
check('large phone request survives chunking intact',
  hostInbox.length === 7 && hostInbox[6].text === bigText);
check('large Mac reply survives chunking intact',
  clientInbox.length === 8 && clientInbox[7].type === 'pong'
  && clientInbox[7].text === bigText);
