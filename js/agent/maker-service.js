 ...
 *
 * Deliberately SIMPLE: Maker writes a self-contained web artifact —
 * arbitrary HTML/CSS/JS files in one folder, rendered in a sandboxed
 * <webview>. Generating HTML directly plays to a local model's strengths,
 * so this path is the one that runs well on a small local model.
 *
...
const MakerService = {
    ...
    // so the engine serves Maker from the same loaded model (no second model
    // load), and — critically — so the model has room to emit a whole HTML file
    // in one write_file. Without an explicit num_ctx, a small default context
    ...
    // into the Maker log. Resolves to the same final
    // { message: { content, tool_calls } } shape either way.
    _chat(params, onChunk) {
        // Maker follows the selected model — when that's a server/cloud
        // entry, merge its routing so main resolves the entry's API key
        // (same fix as LLMLogger._attachEntryRouting).
        if (!params.engine && !params.providerOverride && !params.entryId && !params.baseUrl
            && typeof AgentService !== 'undefined' && AgentService.remoteEntryRouting) {
    ...
    async _webSearchAvailable() {
        try {
            ...
            return !!(s && s.enabled && s.providers && s.provider && s.providers[s.provider]?.hasKey);
        } catch { return false; }
    },
        ...
        try {
            ...
            while (iterations < this.maxIterations) {
                ...
                const response = await this._chat({
                    ...
                    // write_file carries whole HTML files — set num_ctx (the engine's
