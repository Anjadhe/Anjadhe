...
const WriteLedger = {
    ...
    EXTERNAL_TOOLS: new Set([
        'send_email', 'send_text', 'trash_email', 'mark_email_read', 'mark_email_unread',
    ...
    _saveAll(scopes) {
        ...
        try {
            window.electronStore.set(`app_${this.STORE_KEY}`, scopes.slice(0, this.MAX_SCOPES));
            // Bypassing StorageManager.set means bypassing its cache drop;
            // main's broadcast arrives a tick later, and _all() may read first.
            if (typeof StorageManager !== 'undefined' && StorageManager.invalidate) StorageManager.invalidate(this.STORE_KEY);
        }
