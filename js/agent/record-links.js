...
const RecordLinks = {
    ...
    TYPES: {
        ...
        note: {
            ...
            open(id) { RecordLinks._into('notes', () => typeof NotesApp !== 'undefined' && NotesApp.openEditor?.(id, { focus: true })); }
        },
        goal: {
            label: 'goal',
        ...
        // bookmark: registered by its package (js/apps/bookmarks/bookmarks-tools.js)
        ...
        email: {
            label: 'email', hint: 'a message — use its id',
        ...
        insight: {
            label: 'email insight', hint: 'an email insight — use its emailId',
            exists: (id) => (typeof EmailApp !== 'undefined' && EmailApp._dataLoaded)
    ...
    /**
     * Add a linkable type from an app package (docs/PLATFORM.md "App
     * packages"): { label, exists(id) → true|false|null, open(id), hint? }.
     * The type joins the grammar the system prompt teaches (promptTypes),
     * so the model can link the package's records the moment it loads.
     * Navigation only, never a mutation — the HelpActions rule.
     */
    register(type, def) {
        if (!/^[a-z][a-z0-9-]{0,30}$/.test(String(type || '')) || !def || typeof def.open !== 'function') return false;
        this.TYPES[type] = {
            label: def.label || type,
            hint: def.hint,
            exists: typeof def.exists === 'function' ? def.exists : () => null,
            open: def.open
        };
        return true;
    },

    unregister(type) { delete this.TYPES[type]; },

    /** The "Types: …" clause of the prompt's link grammar, from the live table. */
    promptTypes() {
        return Object.keys(this.TYPES)
            .map(t => this.TYPES[t].hint ? `${t} (${this.TYPES[t].hint})` : t)
            .join(', ');
    },
