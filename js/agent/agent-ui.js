...
const AgentUI = {
    ...
    // opens in the in-app Browse tab with an iOS-style "Back to Chat"
    // strip (AppManager.openInBrowse). Without this, target-less anchors
    // navigate the app's BrowserWindow (blanking out the app) and
    ...
    // filter covers both, and onBack restores whichever surface the click
    // came from (full view, or the docked panel over its host app).
    _setupLinkHandler() {
        document.addEventListener('click', (e) => {
            ...
            if (typeof AppManager !== 'undefined' && AppManager.openInBrowse) {
                const agentViewActive = document.getElementById('agent-view')?.classList.contains('active');
                const fromApp = AppManager.currentApp;
                const panelWasOpen = this.isOpen;
                AppManager.openInBrowse(href, {
                    label: 'Back to Chat',
                    onBack: () => {
                        if (agentViewActive) { AppManager.openApp('agent'); return; }
                        if (fromApp && AppManager.apps && AppManager.apps[fromApp]) {
                            AppManager.openApp(fromApp);
                        } else {
                            AppManager.showDashboard();
                        }
                        if (panelWasOpen) setTimeout(() => AgentUI.open(), 60);
                    }
                });
    ...
    /**
     * Paint the model chips (assistant page, docked panel, home, titlebar).
    ...
    _appendMessage(container, role, content, responseMs, metadata, thinking) {
        ...
        if (role === 'user' && metadata && Array.isArray(metadata.attachments) && metadata.attachments.length) {
            ...
            row.innerHTML = metadata.attachments.map(a => a.kind === 'image' && a.dataUrl
                ...
                <span class="agent-msg-attachment" title="${this.escapeHtml(a.name)}">
                    ...
                    <span class="agent-msg-attachment-meta">${a.kind === 'image' ? 'Image' : a.kind === 'pdf' ? `PDF${a.pages ? ` &middot; ${a.pages}p` : ''}${a.ocr ? ' &middot; OCR' : ''}` : this._formatFileSize(a.size)}</span>
                </span>`).join('');
    ...
    async attachFiles(fileList) {
        ...
        for (const file of files) {
            ...
            if (this._isImage(file)) {
                att = await this._readImageAttachment(file);
     ...
     */
    async _readImageAttachment(file) {
        const avail = AgentService.visionAvailability();
        if (!avail.ok) {
    ...
    _openRecordFromKey(recordKey) {
        ...
        switch (app) {
            case 'browse': {
                // rest is the page URL (which itself contains colons). If the
                // active tab is already on it, just switch views; otherwise
                // reopen the page in a new tab.
                const current = (typeof BrowseApp !== 'undefined') ? BrowseApp.lastExtract?.url : null;
                if (current === rest) AppManager.openApp('browse', false);
                else AppManager.openInBrowse(rest);
                return;
            }
            case 'email':
    ...
    onTaskUpdate(task) {
        ...
        card.innerHTML = `
            <div class="agent-task-head">
                <span class="agent-task-eyebrow">Task &middot; ${TITLE[task.status] || esc(task.status)}</span>
            ...
            ${task.note ? `<div class="agent-task-statusline">${esc(task.note)}</div>` : ''}
    ...
    confirmToolCall(toolName, args, summary, convId, opts = {}) {
        ...
        return new Promise((resolve) => {
            ...
            const ask = { toolName, args, summary, convId, resolve, settled: false, onceOnly: !!(opts && opts.onceOnly) };
     ...
     */
    _paintInlineSlot() {
        const ask = this._inlineAskCurrent;
        const offer = (!ask
            && this._continueOffer
            && this._continueOffer.convId === AgentService.activeConversationId)
            ? this._continueOffer : null;
        for (const el of this._inlineSlots()) {
            el.innerHTML = '';
    ...
    // more tool calls. Cleared by any send or by switching conversations.
    _continueOffer: null,

    offerContinue(convId, text) {
        if (!convId) return;
        this._continueOffer = { convId, text: text || '' };
        this._paintInlineSlot();
    },

    clearContinueOffer() {
        if (!this._continueOffer) return;
        this._continueOffer = null;
        this._paintInlineSlot();
    },
    ...
    _buildContinueCard(offer) {
        ...
        dismiss.onclick = () => this.clearContinueOffer();
        ...
        go.onclick = () => {
            ...
            this.clearContinueOffer();
            if (AgentService.isConversationStreaming(convId)) return;
            if (AgentService.activeConversationId === convId) {
                this._dispatchMessage('Continue', []);
            } else {
                AgentService.sendMessage('Continue', null, { convId });
            }
        };
