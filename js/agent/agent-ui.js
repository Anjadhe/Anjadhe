...
const AgentUI = {
        ...
        if (role === 'user' && metadata && Array.isArray(metadata.attachments) && metadata.attachments.length) {
            ...
            row.innerHTML = metadata.attachments.map(a => a.kind === 'image' && a.dataUrl
                ...
                <span class="agent-msg-attachment" title="${this.escapeHtml(a.name)}">
                    ...
                    <span class="agent-msg-attachment-meta">${a.kind === 'image' ? 'Image' : a.kind === 'pdf' ? `PDF${a.pages ? ` &middot; ${a.pages}p` : ''}` : this._formatFileSize(a.size)}</span>
                </span>`).join('');
    ...
    _attachmentMeta(a) {
        const m = { name: a.name, size: a.size, kind: a.kind, pages: a.pages, truncated: a.truncated };
        ...
        ['agent-attachments', 'agent-app-attachments', 'dash-agent-attachments'].forEach((id) => {
            ...
            strip.innerHTML = atts.map((a, i) => `
                <span class="agent-attachment-chip${a.kind === 'image' ? ' is-image' : ''}" title="${this.escapeHtml(a.name)}${a.truncated ? ' — long file; the first part will be shared' : ''}">
                    ...
                    <span class="agent-attachment-meta">${a.kind === 'image' ? 'Image' : a.kind === 'pdf' ? `PDF &middot; ${a.pages} page${a.pages === 1 ? '' : 's'}` : this._formatFileSize(a.size)}${a.truncated ? ' &middot; trimmed' : ''}</span>
    ...
    _openRecordFromKey(recordKey) {
        ...
        switch (app) {
            ...
            case 'librarydoc':
                ...
                setTimeout(() => {
                    if (typeof ReaderApp !== 'undefined') ReaderApp._reader?.open(rest, { backLabel: 'Reader' });
                }, 0);
                return;
            default:
