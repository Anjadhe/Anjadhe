...
const AgentUI = {
    ...
    _buildRecordPills(records, undoScope) {
        const KIND = {
            ...
            wellness: 'Wellness', prompts: 'Prompt'
        };
        const row = document.createElement('div');
    ...
    _describeToolAction(toolName, args) {
        ...
        switch (toolName) {
            ...
            case 'create_scheduled_prompt':
            case 'update_scheduled_prompt': {
                const verb = toolName === 'create_scheduled_prompt' ? 'Schedule' : 'Change the routine';
                // Partial updates may omit the schedule — only describe it
                // when given, so we never show a default the user didn't pick.
                const sched = !args.interval ? ''
                    : (typeof NotePrompts !== 'undefined' && NotePrompts.scheduleLabel)
                        ? NotePrompts.scheduleLabel({ interval: args.interval, time: args.time })
                        : args.interval;
                ...
                const label = args.title ? `“<strong>${a(args.title)}</strong>”`
                    : (toolName === 'create_scheduled_prompt' ? 'a recurring prompt' : '');
                return `${verb}${label ? ` ${label}` : ''}` +
                       (sched ? ` to run <strong>${a(sched)}</strong>` : '') +
                       (toolName === 'create_scheduled_prompt' ? ' in the background on your local model' : '') +
                       `${args.web ? ' with web search' : ''}.` +
                       (bodyShort ? `<br><span class="agent-confirm-note">“${a(bodyShort)}”</span>` : '') +
                       `<br><span class="agent-confirm-note">Results post to the Feed on the Home page.</span>`;
            }
