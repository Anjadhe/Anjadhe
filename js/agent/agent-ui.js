...
const AgentUI = {
        ...
        try {
            ...
            if (isServer) {
                ...
                if (!engineUp) {
                    ...
                    // a background job (on ≤16 GB machines everything runs
     ...
     * chat's own reply — model loads, email insights, background prompts,
    ...
    updateChatbotChip() {
        const convId = AgentService.activeConversationId;
        const active = convId ? AgentService.getConversationComposerMode(convId) : 'agent';
        ...
        const home = blank ? AgentService.getConversationComposerMode(blank.id) : 'agent';

        const taskAvailable = typeof TaskService !== 'undefined'
            && (typeof FEATURES === 'undefined' || FEATURES.isEnabled('taskmode'));
        const NEXT = taskAvailable
            ? { agent: 'chatbot', chatbot: 'task', task: 'agent' }
            : { agent: 'chatbot', chatbot: 'agent', task: 'agent' };
        const LABEL = { agent: 'agent mode', chatbot: 'chatbot mode', task: 'task mode' };
        const TITLE = {
            agent: 'Agent mode (the normal assistant). Click to switch modes — chatbot (raw model, latency diagnostic)' + (taskAvailable ? ' or task (plan-and-run for long jobs)' : '') + '.',
            chatbot: 'Chatbot mode — no system prompt, no personal context, no tools; your question goes straight to the model, like llama-server’s own chat page. For diagnosing slowness. Click to switch modes.',
            task: 'Task mode — your message becomes a long-running task: planned into steps, run with tools under safeguards, verified, and reported back. Click to switch back to agent mode.'
        };

        const paint = (el, mode, isHome) => {
            if (!el) return;
            el.textContent = LABEL[mode] || LABEL.agent;
            el.classList.toggle('agent-context-chip--off', mode === 'agent');
            el.classList.toggle('agent-context-chip--task', mode === 'task');
            el.title = TITLE[mode] || TITLE.agent;
            if (!el.dataset.chatbotClickWired) {
                ...
                el.addEventListener('click', (e) => {
                    ...
                    const current = AgentService.getConversationComposerMode(targetId);
                    AgentService.setConversationComposerMode(targetId, NEXT[current] || 'agent');
                    this.updateChatbotChip();
                });
            }
        };
    ...
    _ensureStreamingMessage(container) {
        ...
        if (!msg) {
            ...
            // splitting one message around the card.
            container.querySelectorAll('.agent-task-card').forEach(c => container.appendChild(c));
    ...
    async _dispatchMessage(text, attachments) {
        ...
        // Task lane: the message IS the goal — no chat turn runs. The plan
        // card (with its Run plan consent button) is the response.
        const modeConvId = AgentService.activeConversationId;
        if (AgentService.getConversationComposerMode?.(modeConvId) === 'task') {
            if (attachments.length) {
                UIUtils.showToast('Attachments don’t carry into task mode — switch to agent mode to send files', 'error');
            }
            this.addMessage('user', text);
            const res = await AgentService.startComposerTask(text, modeConvId);
    ...
    _openableFromResult(toolName, result) {
        ...
        switch (toolName) {
            ...
            case 'update_scheduled_prompt':
                return result.id ? openRec('prompts', result.id, 'Open prompt') : null;
            default: return null;
        }
    },
    ...
    _openRecord(app, id) {
        ...
        const editors = {
            ...
            // Goals live in Plan now — the standalone Goals pages are retired.
            goals: () => typeof FocusApp !== 'undefined' && FocusApp.selectNode?.('goal', id),
    ...
    onTaskUpdate(task) {
        ...
        // Pin the live card to the transcript bottom on every update — a
        ...
        card.innerHTML = `
            ...
            ${total ? `<ol class="agent-task-rail">${stepsHtml}</ol>` : ''}
    ...
    _describeToolAction(toolName, args) {
        ...
        switch (toolName) {
            ...
            case 'delete_scheduled_prompt': {
                ...
                return `Delete the background prompt <strong>${a(target?.title || 'Untitled prompt')}</strong> and stop its runs. Past feed posts stay.`;
            }
            case 'create_scheduled_prompt':
            case 'update_scheduled_prompt': {
                const verb = toolName === 'create_scheduled_prompt' ? 'Schedule' : 'Change the background prompt';
