...
const AgentUI = {
    ...
    /**
     * Open the model picker modal. Lists installed local models with the
    ...
    setupProfilePanelListeners() {
        ...
        bind('agent-wiki-back-btn', () => { this._wikiPageId = null; this.renderProfilePanel(); });

        // Live-filter the page list as the user types; Escape clears.
        ...
        if (searchEl) {
            ...
            const apply = () => {
                ...
                const listEl = document.getElementById('agent-wiki-list');
                if (listEl && !this._wikiPageId) this._renderWikiList(listEl);
            };
    ...
    renderProfilePanel() {
        ...
        const backBtn = document.getElementById('agent-wiki-back-btn');
        const addBtn = document.getElementById('agent-profile-add-btn');
        const subEl = document.getElementById('agent-wiki-sub');
        if (!listEl || !pageEl) return;

        const inPage = !!this._wikiPageId;
        listEl.hidden = inPage;
        pageEl.hidden = !inPage;
        if (backBtn) backBtn.hidden = !inPage;
        if (addBtn) addBtn.hidden = inPage;
        const searchWrap = document.getElementById('agent-wiki-search-wrap');
        if (searchWrap) searchWrap.hidden = inPage;
        if (subEl) {
            subEl.textContent = inPage
                ? 'Edits are saved as you go and won’t be overwritten by the assistant.'
                : 'Memory pages built from your chats. Open a page to edit it — your edits are kept and won’t be overwritten.';
        }

        if (inPage) this._renderWikiPage(pageEl);
        else this._renderWikiList(listEl);
    },
    ...
    _renderWikiList(wrap) {
        let pages = MemoryManager.listSections();

        // Search matches title, summary, and page body so a half-remembered
        // phrase is enough to find the page it lives on.
        ...
        if (q && !pages.length) {
            wrap.innerHTML = `<div class="agent-wiki-no-match">No pages match &ldquo;${this.escapeHtml(this._wikiQuery.trim())}&rdquo;</div>`;
            return;
        }

        wrap.innerHTML = pages.map(s => {
            const badges = [
                s.core ? '<span class="agent-profile-badge" title="Always included in the assistant\'s context">always shown</span>' : '',
                s.userEdited ? '<span class="agent-profile-badge" title="You edited this — the assistant keeps your wording">edited by you</span>' : ''
            ].join('');
            const summary = (s.summary || '').trim()
                || ((s.body || '').trim() ? MemoryManager._deriveSummary(s.body) : '');
            const summaryHtml = summary
                ? `<div class="agent-wiki-row-summary">${this.escapeHtml(summary)}</div>`
                : `<div class="agent-wiki-row-summary agent-wiki-row-empty">${this.escapeHtml(this._wikiHintFor(s.key))}</div>`;
            return `
                <button class="agent-wiki-row" data-id="${this.escapeHtml(s.id)}" type="button">
                    <div class="agent-wiki-row-head">
                        <span class="agent-wiki-row-title">${this.escapeHtml(s.title)}</span>
                        ${badges}
                    </div>
                    ${summaryHtml}
                    <div class="agent-wiki-row-meta">${(s.body || '').trim() ? 'Updated ' + this.escapeHtml(this._relativeTime(s.updatedAt)) : 'Nothing here yet'}</div>
                </button>`;
        }).join('');

        wrap.querySelectorAll('.agent-wiki-row').forEach(el => {
            el.addEventListener('click', () => this._openWikiPage(el.dataset.id));
        });
    },
        ...
        wrap.innerHTML = `
            <div class="agent-profile-section" data-id="${this.escapeHtml(s.id)}">
                <div class="agent-profile-section-head">
                    <input class="agent-profile-section-title" type="text" value="${this.escapeHtml(s.title)}" aria-label="Page title">
                    ${badge}
                    <button class="agent-profile-section-del" type="button" title="Delete page">&#x2715;</button>
                </div>
                <input class="agent-wiki-summary-input" type="text" value="${this.escapeHtml(s.summary || '')}" placeholder="One-line summary shown in the assistant's index" aria-label="Page summary">
                <textarea class="agent-profile-section-body" rows="6" placeholder="${this.escapeHtml(this._wikiHintFor(s.key))}">${this.escapeHtml(s.body || '')}</textarea>
            </div>`;

        const el = wrap.querySelector('.agent-profile-section');
        const id = s.id;
        const titleEl = el.querySelector('.agent-profile-section-title');
        const summaryEl = el.querySelector('.agent-wiki-summary-input');
    ...
    _addProfileSection() {
        ...
        const titleEl = document.querySelector('#agent-wiki-page .agent-profile-section-title');
