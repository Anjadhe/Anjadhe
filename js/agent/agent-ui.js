...
const AgentUI = {
    ...
    _recordTypeLabel(recordKey) {
        ...
        const sub = parts[1];
        if (app === 'portfolio') {
            return sub === 'account' ? 'Account' : sub === 'strategy' ? 'Strategy' : 'Ticker';
        }
        const map = {
    ...
    GENERAL_SUGGESTIONS: [
        ...
        { text: 'Catch me up on the news',
          when: () => ((StorageManager.get('discover-settings')?.interests) || []).length > 0 },
    ...
    _openableFromResult(toolName, result) {
        ...
        switch (toolName) {
            ...
            case 'save_decision': {
                ...
                const RK = {
                    task: `schedule:${id}`, goal: `goals:${id}`, note: `notes:${id}`,
                    routine: `prompts:${id}`, strategy: `portfolio:strategy:${id}`,
                    account: `portfolio:account:${id}`
                };
                return RK[type] ? { label: `Open ${type}`, open: () => this._openRecordFromKey(RK[type]) } : null;
            }
            default: return null;
        }
    },
    ...
    _openRecordFromKey(recordKey) {
        ...
        switch (app) {
            ...
            case 'portfolio': {
                // rest: overview | history | ticker:<T> | account:<id> | strategy:<id>
                AppManager.openApp('portfolio', false);
                const [sub, id] = rest.split(':');
                if (typeof PortfolioApp === 'undefined' || !id) return;
                if (sub === 'ticker') setTimeout(() => PortfolioApp.openTickerDetail?.(id), 0);
                else if (sub === 'account') setTimeout(() => PortfolioApp.openAccountDetail?.(id), 0);
                else if (sub === 'strategy') setTimeout(() => {
            ...
            case 'artifact':
                AppManager.openApp('maker', false);
                setTimeout(() => { if (typeof MakerApp !== 'undefined') MakerApp._selectArtifact?.(rest); }, 0);
                return;
            case 'userapp':
            ...
            default:
                ...
                // wellness — _openRecord handles these (and degrades to just
                // opening the app for ids it doesn't know how to edit).
    ...
    _buildRecordPills(records, undoScope) {
        const KIND = {
            ...
            wellness: 'Wellness', prompts: 'Routine'
        };
        const row = document.createElement('div');
    ...
    _buildDecisionsNote(dec) {
        ...
        const RECORD_KEYS = {
            task: id => `schedule:${id}`, goal: id => `goals:${id}`,
            note: id => `notes:${id}`, routine: id => `prompts:${id}`,
            strategy: id => `portfolio:strategy:${id}`, account: id => `portfolio:account:${id}`
        };
        note.onclick = () => {
            if (!firstKey) return;
            const sep = firstKey.indexOf(':');
            const toKey = RECORD_KEYS[firstKey.slice(0, sep)];
            if (!toKey) return;
            if (this.mode !== 'app' && this.isOpen) this.close();
            this._openRecordFromKey(toKey(firstKey.slice(sep + 1)));
        };
        return note;
    },
    ...
    // --- Build progress card ---
    // The create_artifact/edit_artifact tools dispatch to
    // the Maker engine, which streams {type, message} progress
    ...
    onBuildProgress(convId, kind, e) {
        ...
        if (e.type === 'status') {
            ...
            card.querySelector('.agent-warming-spinner')?.remove();
            card.querySelector('.agent-build-title').textContent = 'Artifact built';
            settleActive();
            activityEl.textContent = '';
            statusEl.textContent = (e.summary || '').slice(0, 200);
            stopTimer();
            const id = e.artifactId;
            if (id) {
                const btn = document.createElement('button');
                btn.className = 'agent-build-open';
                btn.textContent = 'Open in Maker';
                btn.onclick = () => {
    ...
    _describeToolAction(toolName, args) {
        ...
        switch (toolName) {
            ...
            case 'delete_wellness': {
                // Name the entry so "delete a wellness entry" reads as the
                // specific reading/workout going, not a raw id.
                let e = null;
                try {
                    if (typeof WellnessApp !== 'undefined') {
                ...
                return `Delete the wellness entry <strong>${a(label || args.id || 'Unknown')}</strong>` +
                       (e?.time ? ` from ${a(String(e.time).replace('T', ' '))}` : '') + '.';
            }
            ...
            case 'edit_artifact': {
                const target = toolName === 'edit_artifact' ? ` <strong>${a(args.artifactId || '')}</strong>` : '';
