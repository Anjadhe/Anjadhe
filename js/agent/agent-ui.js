...
const AgentUI = {
    ...
    updateModelChip() {
        ...
        const paint = (el) => {
            ...
            if (entry) {
                ...
                el.textContent = name === this._engineLabel(entry.engine)
                    ? name
                    : `${name} · ${this._engineLabel(entry.engine)}`;
                el.classList.remove('agent-context-chip--off');
                el.title = `Default model: ${name} on ${this._engineLabel(entry.engine)}. Click to switch.`;
            } else {
    ...
    async _toggleModelMenu(chipEl) {
        ...
        for (const entry of entries) {
            ...
            // Engine tag — skipped when the name already says it (an Anjadhe
            // Cloud entry labeled "Anjadhe Cloud").
