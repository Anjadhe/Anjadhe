...
const Specialists = {
    ...
    label(id) { return id === 'master' ? 'Anjadhe' : id === 'you' ? 'You' : this._defs.get(id)?.label || id; },
    ...
    system(def, { playbooks = '' } = {}) {
        const now = new Date();
        return `You are ${def.label}, a specialist working for Anjadhe, the user's personal AI. You were brought into a group chat for ONE job, written below by Anjadhe.
${def.prompt}
...
Specialists.register({ id: 'calendar', label: 'Calendar', needs: { apps: ['calendar'] },
    ...
    when: 'reads the user\'s calendar: what is on, when they are free, what overlaps.',
    prompt: 'Settle the date range and time zone first. Treat all-day events separately from timed ones. An empty result only means free if every relevant calendar was covered. Name the events and exact times you are relying on.' });
Specialists.register({ id: 'tasks', label: 'Tasks', needs: { apps: ['actions', 'schedule'] },
    tools: ['list_schedule', 'list_goals'],
    when: 'reads the user\'s tasks and projects: what is due, overdue, blocked or next.',
