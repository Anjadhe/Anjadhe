 ...
 *   1. The same step on an unchanged page, three times, is not progress.
    ...
    createGuard() {
        return {
            ...
            observe(name, args, result) {
                ...
                if (name !== 'browser_act' && name !== 'browser_look') return null;
                const key = JSON.stringify([name, args, result.url, result.position, result.controls, result.text, result.stepError || result.error || null]);
                this.repeats = key === this.last ? this.repeats + 1 : 0;
                this.last = key;
                if (this.repeats >= 2) return { reason: 'no_progress', message: 'The same step kept leaving the page unchanged, so I stopped instead of repeating it.' };
                return null;
            }
        };
    },
