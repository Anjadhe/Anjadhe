...
const MacTools = {
    ...
    _explain(res) {
        if (res.error === 'no-accessibility') {
            ...
            return { error: 'Anjadhe needs Accessibility permission to read and operate other apps. macOS just opened the request: the user turns Anjadhe on in System Settings › Privacy & Security › Accessibility, then asks again. (When Anjadhe runs from a terminal during development, the terminal app is the one to turn on.) Tell the user this and stop.' };
        }
    ...
    async look(args = {}, ctx = {}) {
        ...
        if (!window.electronMac) return { error: 'Using other apps needs the Anjadhe desktop app on a Mac.' };
        ...
        if (canSee && res.image && res.image.dataUrl) {
            ...
            if (res.imageError === 'no-screen-recording') {
                ...
                out.imageError = 'No screenshot: Anjadhe needs Screen Recording permission (System Settings › Privacy & Security › Screen & System Audio Recording). macOS just asked; until the user allows it, work from the element list.';
            } else {
    ...
    async act(args = {}, ctx = {}) {
        ...
        // setTimeout, not rAF: Anjadhe's own window is usually behind the app.
