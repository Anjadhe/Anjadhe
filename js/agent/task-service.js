...
const TaskService = {
    ...
    // Sized for fan-out tasks ("find N things, look up a field for each"):
    // the ten-lead research run burned 6 rounds/step on one search per round
    // and died at 30 total calls with two steps unstarted (finding #37).
    MAX_STEPS: 6,
    ...
    async _runStep(taskId, index, retryIssue) {
        ...
        for (let iter = 0; iter < this.MAX_STEP_ITERATIONS; iter++) {
            ...
            if (!calls.length) {
                ...
                return { ok: !failed, note: body.slice(0, 200), result: body.slice(0, 2000) };
            }
