...
const TaskService = {
    ...
    FOREACH_CHUNK: 5,
    CHUNK_ITERATIONS: 8,
    // I10: chunk passes in flight at once on a parallel-capable brain.
    // Wall-clock overlap only — pacing, budgets and pause semantics are
    // untouched; see _foreachConcurrency for who qualifies (and who
     ...
     */
    init() {
        // §v3.8: a second window is not an app restart.
        if (typeof window !== 'undefined' && window.electronBackground && !window.electronBackground.state()?.owner) return;
        const tasks = this._all();
        let changed = false;
        let resumable = 0;
    ...
    async _resumeUnattendedRuns() {
        const background = typeof window !== 'undefined' && window.electronBackground?.state();
        if (background && (!background.owner || background.paused)) return;
        ...
        for (const t of this._all().filter(t => t && t.resumeOnLaunch && t.status === 'paused')) {
            if (!Array.isArray(t.plan) || !t.plan.length) {
                ...
                this._notifySystem('A routine run did not survive a restart', this._routineLine(t), 'routine');
            }
        }
        const pending = this._all().filter(t => t && t.resumeOnLaunch && t.status === 'paused');
        if (!pending.length) return;
        const goal = this._routineLine(pending[0]);

        if (this._all().some(t => ['planning', 'running', 'verifying'].includes(t?.status))) {
            this._notifySystem('A routine run is waiting', `${goal}\nIt resumes when the current task finishes.`, 'routine');
            return;
        }
        if (typeof AgentService === 'undefined' || !this._model()) {
            this._notifySystem('A routine run is paused', `${goal}\nNo model is selected, so it could not resume.`, 'routine');
            return;
        }
        ...
        this._notifySystem('Routine run resumed after a restart', this._routineLine(t), 'routine');
        this._run(t.id);
    },
    ...
    // nenva Cloud's free tier allows 30 requests/minute and 2 concurrent,
    ...
    // AI Activity identity for a task-mode call: the run's name (routine
    // title / goal) and the handle its Stop button uses (TaskService.cancel).
    // Task mode runs ONE task at a time, so the active set names it.
    _activityFields() {
        if (this._activeRuns.size !== 1) return {};
        const id = [...this._activeRuns][0];
        const t = this.get(id);
        return {
            activitySubject: t ? this._routineLine(t).slice(0, 120) : null,
            activityRunId: 'task:' + id
        };
    },

    // Task mode is the 'routines' surface (js/agent/model-routing.js): the
    // app working while the user is away. It may be pointed at an entry
    // other than the brain, and every question below — where the call goes,
    // whether it is metered, how wide a step may fan out — follows THAT
    // entry (R4), not the brain's.
    SURFACE_SOURCE: 'task',

    _surfaceEntry() {
        try { return AgentService.entryForSource?.(this.SURFACE_SOURCE) ?? AgentService.getDefaultEntry?.() ?? null; }
        catch { return null; }
    },

    async _chat(params) {
        // Routing, in the same order LLMLogger uses: an assigned surface
        // wins, otherwise the default entry's remote routing — a server or
        // cloud brain needs its entry's API key resolved in main, or
        // task-mode calls fail with "Invalid API key" while chat works.
        const surface = (typeof AgentService !== 'undefined' && AgentService.surfaceRouting)
            ? AgentService.surfaceRouting(this.SURFACE_SOURCE) : null;
        let surfaceCtx = 0;
        let routing;
        if (surface) {
            const { numCtx, ...rest } = surface;
            routing = rest;
            surfaceCtx = numCtx > 0 ? numCtx : 0;
        } else {
            routing = (typeof AgentService !== 'undefined' && AgentService.remoteEntryRouting)
                ? AgentService.remoteEntryRouting() : {};
        }
        const metered = (typeof AgentService !== 'undefined')
            && !!(AgentService.isMeteredFor?.(this.SURFACE_SOURCE) ?? AgentService.isMeteredBrain?.());
        for (let attempt = 0; ; attempt++) {
            if (metered) {
                // Slot RESERVATION, not read-then-sleep: with concurrent
                // chunk passes (I10) two callers reading the same
                // _lastChatAt would both sleep out the same wait and then
                // fire together. Claiming the slot synchronously keeps
                // calls TASK_CALL_SPACING_MS apart however many units are
                // in flight.
                const slot = Math.max(Date.now(), this._lastChatAt + this.TASK_CALL_SPACING_MS);
                this._lastChatAt = slot;
                const wait = slot - Date.now();
                if (wait > 0) await new Promise(r => setTimeout(r, wait));
            } else {
                this._lastChatAt = Date.now();
            }
            const call = { model: this._model(), think: false, ...routing, ...this._activityFields(), ...params };
            // Merge, never replace: the caller's options carry temperature
            // and output caps; a surface only has an opinion about context.
            if (surfaceCtx) call.options = { ...(call.options || {}), num_ctx: surfaceCtx };
            const resp = await window.electronLLM.chat(call);
    ...
    _impliedGroups(goalText) {
        ...
        if (/\b(texts?|texting|imessages?|sms)\b/i.test(goal)) implied.push('messages');
        ...
        // App packages own their vocabulary (AgentTools.registerDomain) —
        // the same matchers chat uses, so this table can no longer drift
        // from _domainsForMessage for a packaged app (portfolio was here).
        if (typeof AgentTools !== 'undefined' && AgentTools.dynamicDomainsFor) {
            for (const g of AgentTools.dynamicDomainsFor(goal)) {
                if (!g.startsWith('userapp:')) implied.push(g);
            }
        }
        if (/\b(calendar|meetings?|appointments?)\b/i.test(goal)) implied.push('calendar');
        // journal: covered by dynamicDomainsFor (the package's registerDomain).
        if (/\b(projects?|goals?|focus areas?)\b/i.test(goal)) implied.push('goals');
    ...
    async start(goal, conversationId, opts = {}) {
        ...
        try {
            if (this._goalReferencesMaterial(text)) {
                ...
                if (g.ready === false && String(g.missing || '').trim()) {
                    ...
                    if (task.unattended) this._notifySystem('Task could not start', `${this._routineLine(task)}\nMissing: ${missing}`, task.routineId ? 'routine' : 'task');
        ...
        if (task.unattended) {
            ...
            this._notifySystem('Automated task started', `${this._routineLine(task)}\nPlanned ${steps.length} step${steps.length === 1 ? '' : 's'}.`, task.routineId ? 'routine' : 'task');
            this._run(task.id);
        } else {
    ...
    _goalReferencesMaterial(goal) {
        // I1 boundary (docs/TASK_ENGINE.md): the readiness gate covers
        // CHAT-provided material only. A concrete filesystem path is
        // materialized by reference — the fs step that reads it is its
        // existence check, and the gate model, shown only chat context,
        // can answer nothing but "absent" for on-disk material (that
        // exact misfire failed model-task-pipeline on every brain).
        // Strip "the file <path>" units, then any bare paths, before
        // testing — "the attached list" in the same goal still gates.
        const t = String(goal || '')
            .replace(/\b(?:the|this|that|these|those)\s+(?:files?|documents?)\s+(?:at\s+|in\s+)?["']?~?\/[^\s"']+/gi, '')
            .replace(/["']?~?\/[^\s"']+/g, '');
    ...
    /**
     * PLAN ONLY — no task record, no execution, no writes (U2,
     * docs/ROUTINES_UX.md). What a routine's "Try it" shows for a
     * `runMode:'task'` routine, because the honest preview of a run that
     * ACTS cannot be the run itself.
     *
     * Returns `{ steps: [{ step, tools, kind }] }` or `{ error }`. It
     * deliberately borrows the real planner (`_planChat` with the real
     * `_availableGroups`), so what the user reads is what the arming run
     * would set out to do — not a second model's description of it.
     */
    async dryRun(goal) {
        const text = String(goal || '').trim();
        if (!text) return { error: 'Nothing to plan' };
        const groups = this._availableGroups();
        const coreLine = 'Steps may also rely on always-available core tools (search, web, notes, schedule list).';
        const resp = await this._planChat(text, groups, coreLine, null);
        if (!resp || resp.error) return { error: (resp && resp.error) || 'Could not plan this run' };
        // `_planChat` answers with the raw chat response; `_parsePlanSteps`
        // is what turns it into steps, and going around it is how the first
        // cut of this reported "the planner returned no steps" over a
        // perfectly good plan.
        const steps = this._parsePlanSteps(resp, groups) || [];
        if (!steps.length) return { error: 'The planner returned no steps' };
        return {
            steps: steps.slice(0, this.MAX_STEPS).map(s => ({
                step: String((s && (s.step || s.description)) || '').slice(0, 300),
                tools: Array.isArray(s && s.tools) ? s.tools : [],
                kind: (s && s.kind) === 'foreach' ? 'foreach' : 'single'
            })).filter(s => s.step)
        };
    },
    ...
    /**
     * The first body line of every unattended-run notice: the routine's
     * name when the run belongs to one, then the goal — so a phone reading
     * "Routine finished" knows WHICH routine without opening the app.
     * Titles never carry the app name (2026-09-03): the sender is known.
     */
    _routineLine(task) {
        const goal = String(task?.goal || '').replace(/\s+/g, ' ').trim().slice(0, 160);
        let name = '';
        try {
            if (task?.routineId && typeof NotePrompts !== 'undefined') {
                name = String(NotePrompts.list().find(n => n.id === task.routineId)?.title || '').trim();
            }
        } catch { /* the goal alone still names the run */ }
        if (name && goal && name !== goal) return `${name}: ${goal}`;
        return name || goal;
    },

    /**
     * System notification for unattended moments — best-effort, silent-fail.
     * `kind` ('task' | 'routine') names the notice on the Telegram side.
     */
    _notifySystem(title, body, kind = 'task') {
        const b = String(body || '').slice(0, 500);
        if (typeof Notify !== 'undefined') { Notify.show(title, b, { kind }); return; }
        try { new Notification(title, { body: b, silent: false }); }
    ...
    async _runForeach(taskId, index, retryIssue) {
        ...
        // I10: read-only fan-outs on a parallel-capable brain run chunk
        // passes in WAVES of `conc`; everything else keeps conc = 1 and this
        // loop degenerates to the old strictly-sequential one. Merging (the
        // covered set, rows, persistence) stays with this caller — a pass
        // returns its own results and touches no shared aggregate.
        const conc = this._foreachConcurrency(tools);
        for (let c = 0; c < todo.length; c += this.FOREACH_CHUNK * conc) {
            ...
            for (let w = 0; w < conc; w++) {
                const chunk = todo.slice(c + w * this.FOREACH_CHUNK, c + (w + 1) * this.FOREACH_CHUNK);
                if (chunk.length) wave.push(this._runChunkPass(taskId, index, task, step, tools, chunk, retryIssue));
            }
            const passes = await Promise.all(wave);
            // Merge every finished sibling BEFORE honoring an interruption —
            // covered rows must land in the results map so Resume skips them
            // (I7's incremental-persistence promise holds wave-wide).
            const interrupted = passes.find(p => p.interrupted);
    ...
    /** I10: how many chunk passes may be in flight at once for this step.
     *  Two conditions, both required:
     *  - The brain's ENGINE serves real concurrent requests on the user's
     *    own key ('openai'/'anthropic'). Local is one llama-server whose
     *    queue makes overlap a no-op; nenva Cloud's free tier allows 2
     *    concurrent and is quota-bound (the 2.5s pacing is the binding
     *    limit, so overlap buys nothing worth the burst risk); a custom
     *    server's capacity is unknowable — all stay at 1. Same
     *    _surfaceEntry seam as the metered check above, so the two can't
     *    disagree about which model a task is on.
     *  - EVERY tool in the step's set is read-only (the chat loop's
     *    parallel-batch rule, `AgentService._isReadOnlyTool`). A
     *    write-bearing chunk must see its siblings' writes in
     *    _priorWritesNote before it runs, and concurrent siblings cannot —
     *    sequential is what keeps I8's reconcile-first promise airtight.
     */
    _foreachConcurrency(tools) {
        try {
            const entry = this._surfaceEntry();
            if (!entry || (entry.engine !== 'openai' && entry.engine !== 'anthropic')) return 1;
            const readOnly = (tools || []).every(d => AgentService._isReadOnlyTool?.(d.function?.name));
            return readOnly ? this.FOREACH_CONCURRENCY : 1;
        } catch { return 1; }
    },

    /** One chunk, start to settled: the first pass, the count check, the
     *  informed retry (I9-gated), and classification of what's left.
     *  Self-contained so waves can run passes concurrently (I10) — it reads
     *  no shared aggregate and returns {rows, unverified, missed} for the
     *  caller to merge, or {interrupted} passthrough. */
    async _runChunkPass(taskId, index, task, step, tools, chunk, retryIssue) {
        ...
        return { rows: got, unverified, missed };
    },
    ...
    async _runUnit(taskId, index, messages, tools, opts = {}) {
        ...
        for (let iter = 0; iter < roundCap; iter++) {
            ...
            for (const tc of calls) {
                ...
                const perm = replayed ? { decision: 'allow' } : await AgentService._resolvePermission(name, args, this.get(taskId)?.conversationId);
                if (perm.decision === 'deny') {
                    ...
                    this._notifySystem('Task needs your approval', `${this._routineLine(task)}\nWants to run ${name}${why}. Open the task to continue.`, task.routineId ? 'routine' : 'task');
                    return { interrupted: true };
                ...
                if (!result) {
                    ...
                    const toolCtx = { untrusted: !!(t?.untrustedInput), ambient: !!(t?.routineId), convId: t?.conversationId, unattended: !!(t?.unattended), source: this.SURFACE_SOURCE };
                    try {
        ...
        for (let iter = 0; iter < this.MAX_VERIFY_ITERATIONS; iter++) {
            ...
            for (const tc of calls) {
                ...
                const result = broken
                    ...
                    : await AgentTools.execute(name, args || {}, { untrusted: !!task.untrustedInput, ambient: !!task.routineId, convId: task.conversationId, unattended: !!task.unattended, source: this.SURFACE_SOURCE });
    ...
    async _report(taskId, verdicts, goalVerdict) {
        ...
        if (task.unattended) {
            const changed = this._ledgerSummary(task);
            const body = [
                this._routineLine(task),
                settleNote,
                changed ? `Changed: ${changed}` : '',
                ok ? '' : 'Open Routines to see what happened.',
            ].filter(Boolean).join('\n');
            this._notifySystem(ok ? 'Routine finished' : 'Routine did not finish', body, 'routine');
        }
