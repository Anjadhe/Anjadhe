...
const TaskService = {
    ...
    async _chat(params) {
        ...
        for (let attempt = 0; ; attempt++) {
            ...
            const resp = await window.electronLLM.chat({ model: this._model(), think: false, ...routing, ...this._activityFields(), ...params });
     ...
     *    getDefaultEntry seam as isMeteredBrain, so the two can't disagree
     *    about which brain a task is on.
    ...
    _foreachConcurrency(tools) {
        try {
            const entry = (typeof AgentService !== 'undefined') && AgentService.getDefaultEntry?.();
        ...
        for (let iter = 0; iter < roundCap; iter++) {
            ...
            for (const tc of calls) {
                ...
                if (!result) {
                    ...
                    const toolCtx = { untrusted: !!(t?.untrustedInput), ambient: !!(t?.routineId), convId: t?.conversationId, unattended: !!(t?.unattended) };
                    try {
        ...
        for (let iter = 0; iter < this.MAX_VERIFY_ITERATIONS; iter++) {
            ...
            for (const tc of calls) {
                ...
                const result = broken
                    ...
                    : await AgentTools.execute(name, args || {}, { untrusted: !!task.untrustedInput, ambient: !!task.routineId, convId: task.conversationId, unattended: !!task.unattended });
