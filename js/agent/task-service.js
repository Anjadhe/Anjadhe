    ...
    async _resumeUnattendedRuns() {
        ...
        for (const t of this._all().filter(t => t && t.resumeOnLaunch && t.status === 'paused')) {
            if (!Array.isArray(t.plan) || !t.plan.length) {
                ...
                this._notifySystem('A routine run did not survive a restart', t.goal, 'routine');
            }
        }
        ...
        if (this._all().some(t => ['planning', 'running', 'verifying'].includes(t?.status))) {
            this._notifySystem('A routine run is waiting', `${goal} — it resumes when the current task finishes.`, 'routine');
            return;
        }
        if (typeof AgentService === 'undefined' || !this._model()) {
            this._notifySystem('A routine run is paused', `${goal} — no model is selected, so it could not resume.`, 'routine');
            return;
        }
        ...
        this._notifySystem('Anjadhe resumed a routine run', t.goal, 'routine');
        this._run(t.id);
    },
    ...
    async _report(taskId, verdicts, goalVerdict) {
        ...
        if (task.unattended) {
            this._notifySystem(ok ? 'Anjadhe finished a routine' : 'A routine did not finish',
                `${task.goal.slice(0, 100)}${ok ? '' : ' — open Routines to see what happened'}`, 'routine');
        }
