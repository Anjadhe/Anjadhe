...
const TaskService = {
    ...
    async _runForeach(taskId, index, retryIssue) {
        ...
        // Prior UNKNOWN rows are re-attempted on resume/retry — they're
        // placeholders, not coverage.
        const rows = (prior && Array.isArray(prior.rows)) ? prior.rows.filter(r => !r.unknown) : [];
        const covered = new Set(rows.map(r => this._itemKey(r.item)));
        ...
        if (missed.length) {
            ...
            this._setResult(taskId, index, { rows });
            this._setStep(taskId, index, { note: `${resolved}/${items.length} items (${missed.length} unknown)` });
        }
        return {
            ok,
            note: ok ? `covered all ${items.length} items`
                : `covered ${rows.length - missed.length} of ${items.length} items — unknown: ${missed.slice(0, 5).join('; ')}${missed.length > 5 ? '…' : ''}`,
     ...
     *  a row for an item that wasn't asked is dropped. */
    ...
    async _report(taskId, verdicts, goalVerdict) {
        ...
        const recipeEligible = ok && verdictsClean && !(task.replans > 0)
