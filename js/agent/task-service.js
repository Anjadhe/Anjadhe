...
const TaskService = {
    ...
    /**
     * Deterministic capability routing: when the GOAL itself names browser
     * work, every step gets the browser server's tools regardless of what
     * the planner wrote per step — a small model routinely mislabels which
     * step needs which group, and a browser task without browser tools can
     * only fail.
     */
    _impliedGroups(goalText) {
        const implied = [];
        if (typeof MCPTools !== 'undefined' && MCPTools.browserServers && MCPTools.browserServers.size
            && /(browser|browse|website|web ?page|screenshot|navigate|url|www\.|https?:\/\/|\.com\b|\.org\b)/i.test(String(goalText || ''))) {
