...
const TaskService = {
    ...
    async start(goal, conversationId, opts = {}) {
        ...
        const planWithRetry = async () => {
            for (let attempt = 0; ; attempt++) {
                const resp = await this._planChat(text, groups, coreLine);
                if (!resp?.error) return resp;
                const engineDown = /not running|no server|still loading|loading the model|timed out/i.test(resp.error);
        ...
        try {
            ...
            steps = (parsed.steps || [])
                ...
                .map(s => {
                    ...
                    // C8.4 post-condition: kept only when well-formed and
                    // actually checkable (a check without an arg is noise).
                    const c = s.check;
    ...
    _planChat(text, groups, coreLine) {
        return this._jsonChat({
            messages: [
                { role: 'system', content:
                    ...
                    'Reply ONLY with JSON: {"steps":[{"step":"imperative description","tools":["group",...],"check":{"kind":"none","arg":""}}]}\n' +
            ...
            // The enum makes hallucinated group names unemittable — the
            // grammar only accepts groups that exist on this machine. Same
            // for check kinds (C8.4): the plan carries machine-checkable
            // post-conditions, and the grammar keeps them well-formed.
            type: 'object',
            properties: {
                steps: {
    ...
    _notifySystem(title, body) {
        ...
        catch { /* notifications unavailable — the Tasks page still shows it */ }
    },
            ...
            for (const tc of calls) {
                ...
                if (!result) {
                    ...
                    result = (typeof WriteLedger !== 'undefined' && scopeId)
                        ? await WriteLedger.captureToolRun(scopeId, () => AgentTools.execute(name, args || {}))
                        : await AgentTools.execute(name, args || {});
                ...
                this._recordToolCall(taskId, index, name, args, result);
    ...
    async _verify(taskId) {
        ...
        // Every step settled mechanically → no model pass at all.
        if (hard.every(v => v !== null)) return hard;
        ...
        try {
            ...
            return task.plan.map((s, i) => hard[i] || ({ ok: true, issue: '' }));
        }
    },
    ...
    async _report(taskId, verdicts) {
        ...
        const recipeEligible = ok && verdictsClean
            && (task.toolLog || []).some(e => e && e.ok && e.args !== undefined)
            && !task.toolLogOverflow;
        if (!recipeEligible) {
            const tasks = this._all();
            const t = tasks.find(x => x && x.id === taskId);
            if (t) { delete t.toolLog; delete t.toolLogOverflow; this._saveAll(tasks); }
        }
        ...
        // C8.5: an unattended run reports through a notification — the Tasks
        // page holds the full result and the ledger holds what it changed.
        if (task.unattended) {
            this._notifySystem(ok ? 'Anjadhe finished an automated task' : 'An automated task did not finish',
                `${task.goal.slice(0, 100)}${ok ? '' : ' — open Tasks to see what happened'}`);
        }
