...
const RoutineEngine = {
    ...
    load() {
        ...
        this.state = {
            ...
            // U3 (docs/ROUTINES_UX.md): runs that settled with nothing to
            // say. A quiet run posts no edition, so without this ledger the
            // detail could not tell "quiet" from "never ran" — and that
            // distinction is the whole reason silence is safe to ship.
            quiet: (d && d.quiet && typeof d.quiet === 'object') ? d.quiet : {}
        };
        return this.state;
    },
    ...
    statusFor(routineId) {
        ...
        return {
            ...
            lastQuietAt: (this.quietRuns(routineId)[0] || {}).at || null,
    ...
    // Newest-first, capped. The cap is enforced in main.js's promptFeed
    // merge as well — every renderer write of this key passes through that
    // union, so a cap applied only here would be resurrected by its own
    // save (the SEEN_MAX lesson).
    QUIET_MAX: 20,

    /** A digest run settled with nothing to report. Not an error, not a
     *  gap: `runs` is already stamped by the run itself. */
    stampQuiet(routineId, reason) {
        const q = this.state.quiet || (this.state.quiet = {});
        const list = Array.isArray(q[routineId]) ? q[routineId] : [];
        list.unshift({ at: new Date().toISOString(), reason: String(reason || '').slice(0, 200) });
        q[routineId] = list.slice(0, this.QUIET_MAX);
        this.save();
    },

    /** Quiet runs for a routine, newest first. */
    quietRuns(routineId) {
        const list = (this.state.quiet || {})[routineId];
        return Array.isArray(list) ? list : [];
    },

    /** How many runs in a row said nothing — quiet stamps newer than the
     *  routine's newest posted edition. Arithmetic over two ledgers, so it
     *  cannot disagree with what the detail lists. `sinceISO` is that
     *  edition's time (null when the routine has never posted). */
    quietStreak(routineId, sinceISO) {
        const floor = sinceISO ? (Date.parse(sinceISO) || 0) : 0;
        return this.quietRuns(routineId).filter(q => (Date.parse(q && q.at) || 0) > floor).length;
    },
    ...
    enqueue(routineId, fired, mode) {
        ...
        // A digest describes NOW: when a new schedule slot fires, an older
        // slot's still-queued item (waiting out its backoff ladder, or
        // carried across a restart) is superseded — two briefs for one gap
        // is noise, and the stale one would post the new one's data anyway.
        // Task-mode items are exempt (an action run is work, not content),
        // and mail/file identities are never superseded — each thing is its
        // own run (T2).
        if (mode !== 'task' && /^(slot|span):/.test(identity)) {
            for (let i = q.length - 1; i >= 0; i--) {
                const x = q[i];
                if (x && x.routineId === routineId && x.mode === 'digest'
                    && /^(slot|span):/.test(x.identity || '')) q.splice(i, 1);
            }
        }
        q.push({
    ...
    /** Backoff for a carded digest retry: 15m, 30m, then hourly. Keyed on
     *  total attempts — the first RETRY_MAX are tick-spaced and never land
     *  here, so the first carded step is 15 minutes. */
    _redoDelayMs(attempts) {
        const quick = (typeof PromptFeed !== 'undefined' && PromptFeed.RETRY_MAX) || 2;
        const step = Math.max(0, attempts - (quick + 1));
        return Math.min(60, 15 * Math.pow(2, step)) * 60 * 1000;
    },

    /** A brain just became available (model configured or switched):
     *  collapse every backoff so waiting retries run on the tick the caller
     *  fires next, not an hour from now. */
    retryNow() {
        let changed = false;
        for (const item of this.queue()) {
            if (item && item.nextRetryAt) { delete item.nextRetryAt; changed = true; }
        }
     ...
     */
    // T8: only the main-designated owner may start automatic work. Pausing
    // stops the next item, without aborting a request already in flight.
    canSchedule() {
        if (typeof window === 'undefined' || !window.electronBackground) return true;
        const state = window.electronBackground.state();
        return !!state?.owner && !state.paused;
    },

    async drain() {
        if (!this.canSchedule() || this._draining) return;
        this._draining = true;
        this._busy = true;
        try {
            ...
            // this one forever.
            for (const item of [...this.queue()]) {
                if (!this.canSchedule()) break;
                if (!this.queue().includes(item)) continue;
                // A carded digest waits out its backoff-ladder step (set
                // below); retryNow() collapses the wait when a brain
                // becomes available.
                ...
                try {
                    out = await PromptFeed.runRoutine(prompt, {
                        ...
                        attempts: item.attempts || 0, carded: !!item.carded
                    });
                } catch (e) {
                ...
                // completion) stays queued rather than settling: tick-spaced
                // for the first PromptFeed.RETRY_MAX attempts, then — once
                // the error card has posted (out.card) — on the backoff
                // ladder, so a dead server is not hammered every tick for
                // hours while the run still completes when it returns.
                // PromptFeed.REDO_MAX bounds the whole climb. The identity
                // is deliberately NOT stamped yet: the fire is consumed when
                // the run settles, not while it retries (T7).
                if (out && out.retry) {
                    item.attempts = (item.attempts || 0) + 1;
                    if (out.card) item.carded = true;
                    if (item.carded) item.nextRetryAt = Date.now() + this._redoDelayMs(item.attempts);
                    this.saveLocal();
                    continue;
                }
    ...
    async tick() {
        if (!this.canSchedule() || this._ticking || this._draining) return;
        this._ticking = true;
        try {
            ...
            if (!routines.length) return await this.drain();
