 ...
 * mid-conversation — the user can open a different note,
...
const AgentContext = {
     ...
     */
    registerRecord(prefix, resolverFn, opts = {}) {
        if (typeof resolverFn !== 'function') return;
        this._recordResolvers.set(prefix, resolverFn);
        // The tool domains a chat attached to one of these records should
        // carry (AgentService.RECORD_DOMAINS for the built-ins; a package
        // declares its own here so the shell needs no row for it).
        if (opts.domains) this._recordDomains.set(prefix, Array.isArray(opts.domains) ? opts.domains.slice() : [String(opts.domains)]);
    },

    _recordDomains: new Map(),
    recordDomains(prefix) { return this._recordDomains.get(prefix) || null; },
