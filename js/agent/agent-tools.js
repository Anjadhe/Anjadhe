...
const AgentTools = {
    ...
    definitions: [
        ...
        { type: 'function', function: {
            name: 'get_email',
            description: 'Get full contents of one email by id (from list_emails). Also lists any ATTACHMENTS — their contents are not included, use read_email_attachment for those.',
            parameters: { type: 'object', properties: {
                id: { type: 'string' }
            }, required: ['id'] }
        }},
    ...
    _domainsForMessage(text) {
        ...
        if (/\b(librar(y|ies)|essays?|blog\s?posts?|newsletters?|manuscripts?|corpus)\b/.test(s)
    ...
    handlers: {
        ...
        get_note(args, ctx) {
            ...
            return AgentTools._withDecisions(result, mounts, ctx);
        },
...
// Library tools follow their flag the same way (docs/LIBRARY.md L1): with
// the feature off there is no Library page, no importer, and no index —
// tools over a surface the user can't see would only confuse the model.
if (typeof FEATURES === 'undefined' || !FEATURES.isEnabled('library')) {
    AgentTools.definitions = AgentTools.definitions.filter(d => d.function && !['search_library', 'read_library_doc', 'draft_in_style'].includes(d.function.name));
    delete AgentTools.handlers.search_library;
    delete AgentTools.handlers.read_library_doc;
    delete AgentTools.handlers.draft_in_style;
    delete AgentTools._toolGroups.search_library;
    delete AgentTools._toolGroups.read_library_doc;
    delete AgentTools._toolGroups.draft_in_style;
}
