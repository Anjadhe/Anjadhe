...
const AgentTools = {
    ...
    definitions: [
        ...
        { type: 'function', function: {
            ...
            parameters: { type: 'object', properties: {
                ...
                date_from: { type: 'string', description: 'Only shift tasks dated on/after this ("today" | "tomorrow" | YYYY-MM-DD)' },
                date_to: { type: 'string', description: 'Only shift tasks dated on/before this ("today" | "tomorrow" | YYYY-MM-DD)' },
    ...
    _toolGroups: {
        ...
        // journal
        list_journal: 'journal', get_journal_entry: 'journal', create_journal_entry: 'journal',
        // wellness (health log)
        ...
        get_news: 'news', get_holdings_news: 'portfolio',
    ...
    _shiftPlan(args = {}) {
        ...
        if (from === undefined || to === undefined) {
            return { error: 'date_from/date_to must be "today", "tomorrow", or YYYY-MM-DD.' };
        }
        if (from || to) {
    ...
    handlers: {
        ...
        list_journal(args = {}) {
            ...
            if (from) entries = entries.filter(e => AgentTools._journalDay(e) >= from);
            if (to) entries = entries.filter(e => AgentTools._journalDay(e) <= to);
        ...
        get_journal_entry(args, ctx) {
            ...
            const result = {
                id: entry.id,
                date: AgentTools._journalDay(entry),
                mood: entry.mood,
         ...
         */
        async get_holdings_news(args = {}) {
            if (typeof PortfolioNews === 'undefined' || typeof PortfolioApp === 'undefined') return { error: 'Portfolio news is not available' };
            let webOn = false;
            ...
            let accountId = null;
            if (args.accountName) {
                const want = String(args.accountName).trim().toLowerCase();
                const acct = PortfolioApp.getAccounts().find(a => String(a.name || '').toLowerCase() === want)
                    || PortfolioApp.getAccounts().find(a => String(a.name || '').toLowerCase().includes(want));
