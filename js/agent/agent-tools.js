...
const AgentTools = {
    ...
    definitions: [
        ...
        { type: 'function', function: {
            ...
            description: 'Update a calendar event (Google or Apple). Only passed fields change. For a recurring Apple event only this occurrence changes.',
            parameters: { type: 'object', properties: {
                id: { type: 'string' },
                summary: { type: 'string' },
        ...
        { type: 'function', function: {
            ...
            description: 'Delete a calendar event (Google or Apple). search ≥3 chars and unique, or pass id. For recurring: mode="single" (default) or "all".',
            parameters: { type: 'object', properties: {
                search: { type: 'string' },
                id: { type: 'string' },
...
Set relevant to false when the email does not contain what the task asks for. Use null for any field you cannot determine.` },
        ...
        async update_calendar_event(args) {
            ...
            if (event.source === 'apple') {
                const payload = { externalId: event.appleExternalId, occurrenceStart: event.appleOccurrenceStart, span: 'this' };
                ...
                AgentTools.refreshApp('calendar');
                return { success: true, updated: { id: event.id, changes: eventData, source: 'apple', scope: event.recurringEventId ? 'this occurrence only' : undefined } };
            }
        ...
        async delete_calendar_event(args) {
            ...
            if (target.source === 'apple') {
                const res = await AppleImport.writeEvent('delete', {
                    externalId: target.appleExternalId, occurrenceStart: target.appleOccurrenceStart, span: mode === 'all' ? 'all' : 'this'
                });
