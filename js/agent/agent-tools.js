...
const AgentTools = {
    ...
    definitions: [
        ...
        { type: 'function', function: {
            ...
            description: 'Mark an email action-item analysis read/unread. The message it came from follows (its Gmail read state too) — they are one thing.',
            parameters: { type: 'object', properties: {
                emailId: { type: 'string' },
                read: { type: 'boolean', description: 'Default: true' }
            }, required: ['emailId'] }
        ...
        // TEXT MESSAGES (iMessage / SMS through this Mac's Messages app,
        // 2026-09-10). One tool; consent is asked per send.
        { type: 'function', function: {
            name: 'send_text',
            description: 'Send a text message (iMessage, or SMS when the recipient has no iMessage) through the Messages app on this Mac. Recipient: a phone number, an iMessage email, "me" (the user\'s own number), or the name of someone they recently texted. This is NOT email — for email use send_email.',
            parameters: { type: 'object', properties: {
        ...
        // by the Documents package (js/apps/reader/library-tools.js); the
        // writing-voice kit (draft_in_style) by js/agent/voice-tools.js.
    ...
    _domainsForMessage(text) {
        ...
        // Texts (2026-09-10): "text Priya", "send an iMessage", "sms". A bare
        // "message" / "send" ships BOTH channels so the model can ask which
        // — "send a message to myself" used to ship neither (no email word)
        // and the model announced it had no send action at all.
        if (/\b(texts?|texting|texted|imessages?|sms)\b/.test(s)) out.add('messages');
        if (/\b(messages?|msg|send|sending)\b/.test(s)) { out.add('messages'); out.add('email'); }
        ...
        if (/\b(tasks?|todos?|to-dos?|remind\w*|dues?|deadlines?|overdue)\b/.test(s) || /\bschedul\w+/.test(s)
            ...
            || /\b(what'?s?|hows?|how's|what\s+does)\s+(today|tomorrow|monday|tuesday|wednesday|thursday|friday|saturday|sunday|my\s+\w+day)\b/.test(s)) {
            out.add('schedule');
            // A task carries an Updates log (js/agent/update-tools.js) —
            // the tools ride the task nouns, not only the word "update".
            out.add('updates');
        }
        ...
        if (/\b(projects?|goals?|focus\s+areas?|priorit\w+)\b/.test(s)) {
            ...
            out.add('schedule');
            // Project reviews and check-ins post their read to the project's
            // Updates log — the goals prose names post_update.
            out.add('updates');
        }
        ...
        // library vocabulary: registered by the Documents package (registerDomain)
     ...
     * runtime via nenva SDK `registerTool` (see docs/PLATFORM.md). The
     ...
     * means the user saved their own handle (Notifications) or turned on
     ...
     * handle saved in Settings › Accounts › iMessage; a phone number or
    ...
    async resolveTextRecipient(raw) {
        ...
        if (!status || !status.available) {
            return { error: 'iMessage is not set up on this Mac. The user can set it up in Settings › Accounts › iMessage — save their own number under Notifications, or turn on Insights from your texts. Offer email instead.' };
        }
        if (AgentTools.SELF_HANDLE_RX.test(s)) {
            if (!status.handle) return { error: 'The user has not saved their own number: Settings › Accounts › iMessage › Notifications holds it. Ask for the number, or offer email.' };
            return { handle: status.handle, name: 'you' };
        }
        if (/^(\+?[0-9][0-9 ()-]{5,}|[^\s@]+@[^\s@]+\.[^\s@]+)$/.test(s)) return { handle: s, name: '' };
        const recs = (typeof IMessageSource !== 'undefined' && IMessageSource.records) ? IMessageSource.records() : [];
        const needle = s.toLowerCase();
        const seen = new Map();
        for (const r of recs) {
            const c = r && r.chat;
            if (!c || c.isGroup || !c.handle) continue;
            const name = String(c.name || '').trim();
            if (name && name.toLowerCase().includes(needle) && !seen.has(c.handle)) seen.set(c.handle, name);
        }
        if (seen.size === 1) { const [handle, name] = [...seen.entries()][0]; return { handle, name }; }
        if (seen.size > 1) return { error: `Several recent conversations match "${s}": ${[...seen.values()].join(', ')}. Ask which one, or use the number.` };
...
Set relevant to false when the email does not contain what the task asks for. Use null for any field you cannot determine.` },
        ...
        async mark_email_read(args) {
            ...
            // The insight derived from this message follows its read state
            // (EmailApp._syncInsightRead) — this handler writes the local
            // fields itself instead of going through setEmailRead.
            EmailApp._syncInsightRead(args.id, read);
            EmailApp.saveData();
         ...
         * handle saved in Settings › Accounts › iMessage; a name resolves
