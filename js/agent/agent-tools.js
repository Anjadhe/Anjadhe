...
const AgentTools = {
    ...
    definitions: [
        ...
        { type: 'function', function: {
            ...
            description: 'Add a stock buy/sell to a portfolio account. Type "holding" records shares the user already owns (pricePerShare = their average cost) without affecting cash — use it for initial/existing positions.',
            parameters: { type: 'object', properties: {
    ...
    handlers: {
        ...
        search_all(args) {
            ...
            // Wellness — text-bearing fields only (numbers aren't searchable text).
            // Not profile-filtered: the health log is deliberately profile-agnostic.
            const wellness = StorageManager.get('wellness')?.entries || [];
            wellness.forEach(w => {
                const text = [w.name, w.description, w.notes, w.activityType, w.mealType].filter(Boolean).join(' ').toLowerCase();
                if (text.includes(q)) {
                    results.push({ app: 'wellness', id: w.id, kind: w.kind, time: w.time,
                        title: `${w.kind}: ${(w.name || w.description || w.notes || w.activityType || '').slice(0, 60)}` });
                }
            });
        ...
        create_scheduled_prompt(args) {
            ...
            if (title) {
                ...
                if (typeof ProfileManager !== 'undefined' && ProfileManager.filterByActiveProfile) {
                    existing = ProfileManager.filterByActiveProfile(existing);
                }
