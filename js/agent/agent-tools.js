...
const AgentTools = {
    ...
    definitions: [
        ...
        { type: 'function', function: {
            ...
            description: 'Create a routine: it runs automatically on a recurring schedule and each run posts its result to the Home feed. Use when the user wants something regularly ("every morning…", "weekly digest of…"). The prompt must be a complete standalone instruction with every stated preference baked in.',
            parameters: { type: 'object', properties: {
                ...
                interval: { type: 'string', enum: ['hourly', '6h', 'daily', 'weekdays', 'weekly'], description: 'weekdays = Mon–Fri only (market days, workdays)' },
                time: { type: 'string', description: 'HH:MM 24h run time for daily/weekdays/weekly ("every morning" → "08:00", "every evening" → "18:00"). Omit for hourly/6h.' },
        ...
        { type: 'function', function: {
            ...
            description: 'List the user\'s routines — scheduled prompts that run in the background (the Routines page; their runs post to the Home feed): id, title, prompt snippet, schedule (interval + time), last run. Long prompt text is snipped — call get_note with the prompt\'s id when the answer depends on the full wording. Call this FIRST for any question about a recurring prompt\'s schedule, timing, or behavior — e.g. "why did my market review run at the wrong time".',
            parameters: { type: 'object', properties: {} }
        }},
        { type: 'function', function: {
            name: 'update_scheduled_prompt',
            description: 'Change a routine\'s title, prompt text, or schedule. Resolve id with list_scheduled_prompts first. Omitted fields stay unchanged.',
            parameters: { type: 'object', properties: {
                id: { type: 'string' },
                title: { type: 'string' },
        ...
        { type: 'function', function: {
            ...
            description: 'Delete a routine and stop its runs (past feed posts stay). Resolve id with list_scheduled_prompts first.',
            parameters: { type: 'object', properties: {
                id: { type: 'string' }
            }, required: ['id'] }
        }},
        ...
        { type: 'function', function: {
            ...
            description: 'Arm a recurring UNATTENDED task: when the trigger fires (a schedule, a matching new email, or a new file in a folder), the task starts by itself on this Mac — no one needs to be watching. The user must approve the arming; each armed run still pauses and notifies them when a step needs permission. Use when the user asks for something to happen automatically/recurringly ("every morning…", "whenever an invoice email arrives…"). Manage/disable on the Tasks page.',
            parameters: { type: 'object', properties: {
                goal: { type: 'string', description: 'The complete task to run each time, restated fully' },
                trigger: { type: 'object', description: 'One of: {"type":"time","interval":"hourly|daily|weekly","time":"HH:MM"} · {"type":"email","from":"...","subject":"..."} (substring match, at least one) · {"type":"file","folder":"~/...","pattern":"*.pdf"}' }
        ...
        list_goals(args) {
            ...
            return {
                ...
                goals: goals.map(g => ({
                    ...
                    tasks: LinkManager.getTasksForGoal(g.id).filter(t => {
...
Set relevant to false when the email does not contain what the task asks for. Use null for any field you cannot determine.` },
        ...
        create_scheduled_prompt(args) {
            ...
            const title = (args.title || '').trim();
            // Dedup by title against existing routines (active profile),
        ...
        update_scheduled_prompt(args) {
            ...
            if (!NotePrompts.list().some(n => n.id === id)) {
                return { error: 'Routine not found — call list_scheduled_prompts for ids' };
            }
        ...
        delete_scheduled_prompt(args) {
            ...
            if (!note) return { error: 'Routine not found — call list_scheduled_prompts for ids' };
        ...
        create_automation(args) {
            if (typeof AutomationService === 'undefined') return { error: 'Automations not available in this build.' };
            const res = AutomationService.create({ goal: args?.goal, trigger: args?.trigger });
            if (res.error) return res;
            return {
                ok: true,
                automationId: res.automation.id,
                trigger: AutomationService.triggerLabel(res.automation),
                note: 'Armed on this Mac. It runs while Anjadhe is open; steps needing permission pause and notify. The user can disable or remove it on the Tasks page.'
            };
        }
    },
};
...
// Task mode (C4) ships behind its own flag the same way. Recipes (C8.3) and
// automations (C8.5) are born from tasks, so they follow the same flag.
if (typeof FEATURES === 'undefined' || !FEATURES.isEnabled('taskmode')) {
    AgentTools.definitions = AgentTools.definitions.filter(d => d.function && !['start_task', 'run_recipe', 'create_automation'].includes(d.function.name));
    delete AgentTools.handlers.start_task;
    delete AgentTools.handlers.run_recipe;
    delete AgentTools.handlers.create_automation;
}
