...
const PermissionManager = {
     ...
     */
    ASK_TOOLS: new Set([
        'send_email', 'send_text', 'trash_email',
        'create_calendar_event', 'update_calendar_event',
        // C10: arming a routine commits recurring UNATTENDED runs — a model
        // turn, or (runMode 'task') a full task that may WRITE. The dialog
        // showing the prompt and the trigger IS that auto-run consent, and
        // it is shown for EVERY routine: after the Automations merge the
        // category no longer tells you whether a run can change anything,
        // and the alternative — having the model classify its own goal as
        // read-only — is a model verdict gating a safety property.
        'create_routine', 'update_routine',
        // A decision becomes a STANDING instruction the model re-reads on
        // every future look at that record — Ram's call (2026-08-06): each
        // save asks, with the dialog quoting the record and the text.
        'save_decision',
        // One call rewrites the dates on a whole goal's task list (36+
        // records) — reversible, but far past what a silent in-app write
        // should cover. The dialog is also the PREVIEW: it renders the same
        // computed plan the handler applies (count, shift, first/last
        // dates), so approving it is approving exact dates — the
        // delete_goal count-the-cascade precedent.
    ...
    resolve(toolName, args) {
        ...
        // 5. Static policy — plus what app packages registered (AgentTools.
        //    register adds `ask` tools to ASK_TOOLS; a user-app tool flagged
        //    destructive used to run silently unless its name began with
        //    delete_, closed 2026-08-29).
        const dyn = (typeof AgentTools !== 'undefined' && AgentTools._dynamicTools?.[toolName]) || null;
        if (this.ASK_TOOLS.has(toolName) || /^delete_/.test(toolName) || (dyn && (dyn.ask || dyn.destructive))) {
            return { decision: 'ask' };
        }
    ...
    _grantVerdict(key, args) {
        ...
        // bound, not a quota.
        if (key === 'send_text' && Array.isArray(g.exclusions) && g.exclusions.includes('other_people')) {
            const to = String(args?.to || '').trim();
            const own = String((typeof AgentTools !== 'undefined' && AgentTools._imessageStatus?.handle) || '');
            const digits = (h) => h.toLowerCase().replace(/[\s()-]/g, '');
            const isSelf = (typeof AgentTools !== 'undefined' && AgentTools.SELF_HANDLE_RX?.test(to))
                || (own && digits(to) === digits(own));
            if (!isSelf) {
                return {
                    decision: 'ask',
                    note: `Your standing permission covers texts to your own number; "${to}" is someone else. Approving sends this one; the permission itself stays own-number-only (adjust in Settings → AI Assistant).`
                };
            }
        }
     ...
     *
     * Reads EmailApp.emails, NOT `StorageManager.get('email').emails` — that
     * field stopped existing when messages moved to the SQLite table, so this
     * answered "unknown sender" for every address in the user's mailbox.
     * Synchronous by contract (the decision path is), so an unloaded mailbox
     * answers false: erring toward asking is the safe direction here.
     */
    _isKnownRecipient(addr) {
        try {
            const emails = (typeof EmailApp !== 'undefined' && Array.isArray(EmailApp.emails))
                ? EmailApp.emails : [];
    ...
    async grantAlways(toolName) {
        ...
        // Same shape for texts (2026-09-10): a standing send_text covers the
        // user's OWN number only — a text to another person asks every time.
        if (toolName === 'send_text') grant.exclusions = ['other_people'];
