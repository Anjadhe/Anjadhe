...
const CLIBridge = {
    ...
    async _cmdModelSet({ ref, makeDefault }) {
        await AgentService.ensureModelList();
        const list = AgentService.getModelList();
        if (!list.length) throw new Error('No models are set up yet. Add one in nenva: Settings → AI Assistant → Models.');
        const conv = this._terminalConv();
...
if (typeof window !== 'undefined') {
    ...
    // Behind the `terminal` feature flag (2026-09-09): with it off, main's
    // server never starts (AppManager.init skips electronCLI.ensure()), so
    // no request can arrive — and the bridge stays unwired to match.
    if (typeof FEATURES === 'undefined' || FEATURES.isEnabled('terminal')) CLIBridge.init();
}
