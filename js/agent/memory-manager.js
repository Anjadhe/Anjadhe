...
const MemoryManager = {
     ...
     * (identity + behavior rules); everything else appears only as an index
     * line the model can follow up on with the recall_memory tool. Pages with
     * empty bodies are omitted entirely.
     */
    pagesForBriefing(profile) {
        const pages = this.listSections(profile).filter(s => (s.body || '').trim());
        return {
            core: pages.filter(s => s.core).map(s => ({ title: s.title, body: s.body.trim() })),
            index: pages.filter(s => !s.core).map(s => ({
                title: s.title,
