...
const AgentService = {
    ...
    // waiting on one (the email-insight backlog, due scheduled prompts,
    ...
    _kickBackgroundAI() {
        try { if (typeof EmailApp !== 'undefined') EmailApp.drainAnalysisQueue(); } catch { /* best-effort */ }
        try { if (typeof EmailApp !== 'undefined') EmailApp.classifyBundlesWithAI(); } catch { /* best-effort */ }
        try { if (typeof PromptFeed !== 'undefined') PromptFeed.tick(); } catch { /* best-effort */ }
    },
    ...
    async ensureVisionInfo(force = false) {
        ...
        const info = { catalogVision: {}, installedVision: null, cloudPrefixes: null };
        try {
        ...
        const clean = (Array.isArray(list) ? list : [])
            ...
            .map(e => {
                const engine = (e.engine === 'llamacpp' || e.engine === 'server'
                    || e.engine === 'openai' || e.engine === 'anthropic') ? e.engine : 'llamacpp';
                const out = {
    ...
    async ensureModelList() {
        ...
        if (Array.isArray(settings.modelList) && (settings.modelListVersion || 1) >= 2) {
            ...
            // background prompts) probes the local engine first and logs
    ...
    // ────────── Composer mode: agent / chatbot / task ──────────
    // One per-conversation selector behind the composer chip. 'agent' is the
    // normal assistant, 'chatbot' the latency diagnostic (raw model), and
    // 'task' routes the message straight into TaskService — an explicit lane
    // for long-horizon work, instead of hoping the model calls start_task.

    getConversationComposerMode(convId) {
        const conv = convId ? this.conversations.find(c => c.id === convId) : null;
        if (conv && conv.taskMode === true) return 'task';
        return this.getConversationChatbotMode(convId) ? 'chatbot' : 'agent';
    },

    setConversationComposerMode(convId, mode) {
        const conv = this.conversations.find(c => c.id === convId);
        if (!conv) return;
        delete conv.taskMode;
        delete conv.chatbotMode;
        delete conv.bareMode;
        if (mode === 'task') conv.taskMode = true;
        else if (mode === 'chatbot') conv.chatbotMode = true;
        this._saveConversations();
    },

    /**
     * The task-mode send: persist the user's message in the conversation
     * (it's the transcript context the report lands in), then hand the text
     * to TaskService as the goal. No chat turn runs at all — the plan card
     * is the response.
     */
    async startComposerTask(text, convId) {
        const conv = this.conversations.find(c => c.id === convId);
        if (!conv) return { error: 'Conversation not found' };
        if (typeof TaskService === 'undefined'
            || (typeof FEATURES !== 'undefined' && !FEATURES.isEnabled('taskmode'))) {
            return { error: 'Task mode is not available in this build.' };
        }
        conv.messages.push({ role: 'user', content: text });
        this._syncActiveConversation(convId, conv);
        this._persistConversation(conv);
        return await TaskService.start(text, convId);
    },
     ...
     * exactly what happened to the stock-market background prompt on
...
4. The newest user message ends with an auto-attached CURRENT CONTEXT block (date/time), possibly followed by a CONVERSATION GOAL line (the running aim of this chat). The app attaches these — the user did not type them. Use them for dates and direction; never echo them back.`,
     ...
     * shell, MCP and background prompts now live in _domainGuidance and ship
    ...
    _capabilityAddendum() {
        ...
        const extras = [
            'build the user\'s own mini-apps and artifacts',
            'set up recurring background prompts that run on their own and post to the Home feed'
        ];
...
Calendar accounts connected: ${calendarAccounts.length === 0 ? 'none' : calendarAccounts.map(a => a.email).join(', ')}.`;
    ...
    async residentModels() {
        try {
            // Remote entries (server / cloud API) have no local residency.
    ...
    async handleMemoryPressure(level) {
        ...
        try {
            ...
            if (llm?.provider === 'remote' || llm?.provider === 'custom') return 0;
    ...
    async _onIdleUnload() {
        ...
        // Cloud (remote) and OpenAI-compatible (custom) providers have no
        // local weights for us to free.
        try {
            const llm = await window.electronLLM?.getSettings?.();
    ...
    async sendMessage(userMessage, onChunk, opts = {}) {
        // Headless/background runs (e.g. background prompts via runHeadless)
        ...
        try {
            ...
            // Read-only runs (headless background prompts) may READ the user's
            ...
            while (iterations < this.maxToolIterations) {
                ...
                if (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
                    ...
                    for (const i of writeIndices) {
                        ...
                        results[i] = ledgerOn
                            ? await WriteLedger.captureToolRun(turnScopeId, () => AgentTools.execute(name, parsed[i].args))
                            : await AgentTools.execute(name, parsed[i].args);
                ...
                // callers (background prompts) attribute the run correctly.
                ...
                lastResponse = { type: 'text', content, thinking: thinking || undefined, sources: srcs || undefined, records: recs || undefined, undoScope: undoScope || undefined, model: response.model || activeModel };
                break;
            }
     ...
     *
     * Used by background prompts that opt into "Use my context": the
    ...
    async runHeadless(text, options = {}) {
        ...
        // prompt. Callers pass provenance (which background prompt, its cadence)
        ...
        temp.extraContext =
            ...
            (src.title ? `the saved background prompt "${src.title}"` : 'a saved prompt') +
...
Rules:
- File each fact into the most fitting page. Merge it with what's already there; drop duplicates; if a new fact contradicts an older one, keep the newer.
