            ...
            while (iterations < turnCaps.maxToolIterations) {
                ...
                if (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
                    ...
                    if (readIndices.length > 0) {
                        ...
                        const readPromises = runIndices.map(i => AgentTools.execute(parsed[i].name, parsed[i].args, { untrusted: untrustedTurn, convId: targetConvId, unattended: !!opts.unattended }));
                    ...
                    for (const i of writeIndices) {
                        ...
                        results[i] = ledgerOn
                            ? await WriteLedger.captureToolRun(turnScopeId, () => AgentTools.execute(name, parsed[i].args, { untrusted: untrustedTurn, convId: targetConvId, unattended: !!opts.unattended }))
                            : await AgentTools.execute(name, parsed[i].args, { untrusted: untrustedTurn, convId: targetConvId, unattended: !!opts.unattended });
