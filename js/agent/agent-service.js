...
const AgentService = {
    ...
    async _resolvePermission(name, args, convId) {
        ...
        if (name === 'web_search' && this._webSearchUnset === true) {
            ...
            return {
                decision: 'ask',
                grantKey: 'web_search',
                note: 'First web search — approving turns web search ON, through Anjadhe Connect (built in, no account, 300 searches a month included). Only the query shown here leaves this Mac; the service keeps a usage count, not what you searched. Turn it off anytime in Settings › AI Assistant › Web Search.'
            };
        }
        return PermissionManager.resolve(name, args);
    },
    ...
    async _resolveEgressPermission(name, args) {
        ...
        if (name === 'read_url') {
            ...
            note = this._webSearchUnset === true
                ? 'First web search — approving turns web search ON, through Anjadhe Connect (built in, no account, 300 searches a month included). Only the query shown here leaves this Mac; the service keeps a usage count, not what you searched. Turn it off anytime in Settings › AI Assistant › Web Search.'
    ...
    async _confirmWrite(name, args, perm, convId) {
        ...
        // Anjadhe forward so the question is seen; the step re-activates
    ...
    async _syncBrainToEntry(entry) {
        ...
        if (entry.engine === 'llamacpp') {
            ...
            // (email insights, filing) follow the brain too. Anjadhe
     ...
     */
    _stableSystemPrompt: `You are the user's personal assistant in Anjadhe. Help with anything — knowledge, advice, writing, math, code, chat — and with their data (focus, projects, tasks, notes, journal, portfolio, bookmarks, wellness/health log, Gmail, Calendar) via tools. Not restricted to productivity topics.
