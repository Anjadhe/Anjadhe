...
- Task: concrete action. create_schedule_item; pass goalTitle only if linked.`,
    ...
    async _ensureWebSearchState(force = false) {
        ...
        try {
            ...
            this._webSearchReady = !!s?.provider;
    ...
    _WEB_SEARCH_OFF_BLOCK: 'WEB SEARCH: not enabled. You have no web access right now — the user has not turned on web search (it lives in Settings › AI Assistant › Web Search, and in the Setup Assistant checklist). Never claim you can look things up online or list web search among your abilities. If a question needs current information, say web search is off, point at where to enable it once, and answer from knowledge.',
...
Calendar accounts connected: ${calendarAccounts.length === 0 ? 'none' : calendarAccounts.map(a => a.email).join(', ')}.`;
    ...
    async handleMemoryPressure(level) {
        ...
        return await this.unloadAllResident(level >= 4 ? 'memory-critical' : 'memory-pressure');
    },
