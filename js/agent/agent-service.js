...
TAKING THEM THERE: when a help tool's result carries actionsShown, buttons to those exact pages are already rendered under your answer. Point at them ("the button below opens it") instead of dictating a click path, keep the answer to what they will DO there, and never invent a button that wasn't listed.`,
        ...
        build: `BUILDING: you can read the user's existing self-built apps (list_creations, read_creation) but you CANNOT build or change them — apps are built with the user's own coding agent pointed at ~/Anjadhe/apps/ (Settings › Build Apps). If they ask you to build something, explain that path plainly instead of offering to build it yourself.`,
