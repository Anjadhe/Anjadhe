...
const AgentService = {
    ...
    openVoiceConversation(voice) {
        ...
        const conv = (active && (active.messages || []).length === 0 && !active.recordKey)
            ...
            : this.createConversation();
        const domains = new Set(Array.isArray(conv.scopedDomains) ? conv.scopedDomains : []);
        if (!domains.has('library')) {
            domains.add('library');
            conv.scopedDomains = [...domains];
        }
        conv.extraContext = `This conversation was opened from the Writing Voices app for the writing voice "${name}". For ANY request to write, draft, or rewrite something, call draft_in_style FIRST with voice: "${name}" and imitate the exemplars it returns — they are real writing in that voice. End the draft naming the groundedIn documents.`;
        // The drawer must never open silent — the user clicked a button
        // whose promise is "we write this way now", and an empty composer
        // reads as nothing having happened. A synthetic assistant message,
        // no model call; it also anchors the task for the model as plain
        // history.
        conv.messages = conv.messages || [];
        if (!conv.messages.length) {
            conv.messages.push({
                role: 'assistant',
                content: `You're drafting in the writing voice **${name}**. Tell me what to write — a post, an email, a note, a speech — plus any details, and I'll write it the way ${name === 'My voice' ? 'you' : `"${name}"`} would.`
            });
        }
        this._saveConversations();
        return conv;
...
TAKING THEM THERE: when a help tool's result carries actionsShown, buttons to those exact pages are already rendered under your answer. Point at them ("the button below opens it") instead of dictating a click path, keep the answer to what they will DO there, and never invent a button that wasn't listed.`,
    ...
    _domainGuidanceFor(domains) {
        ...
        if (set.has('library') && on('library')) out.push(this._domainGuidance.library);
            ...
            while (iterations < this.maxToolIterations) {
                ...
                if (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
                    ...
                    if (readIndices.length > 0) {
                        ...
                        const readPromises = runIndices.map(i => AgentTools.execute(parsed[i].name, parsed[i].args, { untrusted: untrustedTurn }));
                    ...
                    for (const i of writeIndices) {
                        ...
                        results[i] = ledgerOn
                            ? await WriteLedger.captureToolRun(turnScopeId, () => AgentTools.execute(name, parsed[i].args, { untrusted: untrustedTurn }))
                            : await AgentTools.execute(name, parsed[i].args, { untrusted: untrustedTurn });
