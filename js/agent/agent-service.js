        ...
        try {
            if (typeof MemoryManager !== 'undefined') {
                const activeProfile = (typeof ProfileManager !== 'undefined')
                    ...
                    : undefined;
                const { core, index } = MemoryManager.pagesForBriefing(activeProfile);
                if (core.length) {
                ...
                if (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
                    ...
                    for (let i = 0; i < toolResults.length; i++) {
                        ...
                        if (!(/^mcp_/.test(tr.tool) && /(snapshot|screenshot|console|network|tabs?|find|continue_output)/.test(tr.tool))) {
                            ...
                            if ((toolCallCounts.get(key) || 0) === this.perToolHardBreak - 2) {
