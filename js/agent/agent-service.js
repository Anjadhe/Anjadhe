...
Calendar accounts connected: ${calendarAccounts.length === 0 ? 'none' : calendarAccounts.map(a => a.email).join(', ')}.`;
    ...
    async sendMessage(userMessage, onChunk, opts = {}) {
        ...
        try {
            ...
            // 'fyi' (Email AI) joined 2026-08-05 with its "Ask about this
            // insight" door: everything that page shows derives from
            // incoming mail, so a chat opened over it reads attacker-
            // supplied text exactly like one opened over the Inbox.
            const UNTRUSTED_CONTEXT_APPS = new Set(['browse', 'email', 'fyi']);
            ...
            // set this flag for any run whose input is attacker-supplied.
            if (UNTRUSTED_CONTEXT_APPS.has(currentApp) || this.untrustedInput || opts.untrustedInput) {
            ...
            while (iterations < this.maxToolIterations) {
                ...
                if (
                    ...
                    console.warn(`[agent] running-task claim before approval ("${content.slice(0, 80)}") — retrying with nudge`);
                    if (typeof AgentUI !== 'undefined' && AgentUI.onRetryPass) {
                ...
                lastResponse = { type: 'text', content, thinking: thinking || undefined, sources: srcs || undefined, records: recs || undefined, actions: acts || undefined, memory: memMeta || undefined, undoScope: undoScope || undefined, model: response.model || activeModel };
                break;
            }
            ...
            if (!lastResponse) {
                ...
                try {
                    ...
                    if (content) {
                        ...
                        lastResponse = { type: 'text', content, sources: synthSrcs || undefined, records: synthRecs || undefined, actions: synthActs || undefined, model: synthMeta.model };
...
Each memory: {"type":"preference"|"fact"|"context"|"correction","title":"short label","body":"neutral-wording memory","quote":"short EXACT phrase copied verbatim from the transcript that states this","entity":"person/place/topic it is about, or omit"}
...
- "quote" must be copied character-for-character from the transcript — it is how the memory is verified. A memory whose quote isn't in the transcript is discarded.
...
JSON array:`;
        ...
        // actually appears in the transcript (whitespace-normalized,
        // case-insensitive). This is the validation a 12B extractor can't
        // argue with — a paraphrase it invented has no quote to point at,
        // and the quote doubles as provenance on the memory page.
        ...
        for (const c of candidates.slice(0, 5)) {
            ...
            if (!quote || quote.length < 4 || !transcriptNorm.includes(norm(quote))) {
                unquoted++;
                continue;
            }
...
JSON array:`;
        ...
        // model produced nothing usable, so the items get another chance.
        if (updated > 0) {
