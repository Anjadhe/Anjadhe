...
const DecisionStore = {
    ...
    TYPES: ['task', 'goal', 'note', 'routine', 'strategy', 'account'],
    TITLE_MAX: 80,
    BODY_MAX: 1200,
    ...
    resolveKey(type, { id, name } = {}) {
        ...
        const fromBlob = (blobKey, arrayKey, label) => {
            const list = (StorageManager.get(blobKey) || {})[arrayKey] || [];
            const rec = list.find(r => r && String(r.id) === wantId);
            return rec
                ? { key: `${t}:${rec.id}`, recordTitle: rec.title || label }
                : { error: `No ${label} with id "${wantId}".` };
        };

        switch (t) {
            case 'task': return wantId ? fromBlob('schedule', 'scheduleItems', 'task')
                : { error: 'task requires an id (from list_schedule).' };
            case 'goal': return wantId ? fromBlob('goals', 'goals', 'goal')
                : { error: 'goal requires an id (from list_goals).' };
            case 'note': return wantId ? fromBlob('notes', 'notes', 'note')
                : { error: 'note requires an id (from list_notes).' };
            case 'routine': return wantId ? fromBlob('notes', 'notes', 'routine')
                : { error: 'routine requires an id (from list_routines).' };
            case 'strategy': {
                if (typeof PortfolioStrategy === 'undefined') return { error: 'Portfolio is not available.' };
                const s = PortfolioStrategy.find(wantId || name || '');
                return s ? { key: `strategy:${s.id}`, recordTitle: s.name || 'strategy' }
                    : { error: `No strategy matching "${wantId || name}".` };
            }
            case 'account': {
                if (typeof PortfolioApp === 'undefined') return { error: 'Portfolio is not available.' };
                PortfolioApp.loadData();
                const accounts = PortfolioApp.getAccounts() || [];
                const a = accounts.find(x => x && String(x.id) === wantId)
                    || (wantName ? accounts.find(x => (x.name || '').toLowerCase() === wantName) : null)
                    || (wantName ? accounts.find(x => (x.name || '').toLowerCase().includes(wantName)) : null);
                return a ? { key: `account:${a.id}`, recordTitle: a.name || 'account' }
                    : { error: `No account matching "${wantId || name}".` };
            }
        }
    ...
    pruneOrphans() {
        this.init();
        const ids = (blobKey, arrayKey) => {
            const blob = StorageManager.get(blobKey);
            const list = blob ? blob[arrayKey] : null;
            return Array.isArray(list) ? new Set(list.map(r => r && String(r.id)).filter(Boolean)) : null;
        };
        const notes = ids('notes', 'notes');
        const hosts = {
            task: ids('schedule', 'scheduleItems'),
            goal: ids('goals', 'goals'),
            note: notes,
            routine: notes,   // a routine IS a prompt note
            strategy: ids('portfolio', 'strategies'),
            account: ids('portfolio', 'accounts')
        };
