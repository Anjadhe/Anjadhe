...
**Updates — the log.** Every project and every task carries an **Updates** section: post what happened ("sent the deck, waiting on Dana"), and the assistant posts there too — its read when you ask "How is this going?" or press **Ask Anjadhe to check in**, a risk it noticed, and every weekly review. Each entry says who posted it and when; the page shows how long it has been since the last one. Updates are history for you and the assistant to read back, not instructions — a standing instruction is a Decision.
...
A **routine** is a prompt that runs on a schedule (hourly, every 6h, daily, weekdays, or weekly, optionally at a set time like 8:00 AM) in the background on your local model and posts each result to the feed on the home page. (That is a routine that writes answers; a routine that **takes actions** keeps each run's log on its own page under Run history instead — the feed carries what your AI wrote, not what it did.) Manage them on the **Routines** page (reached with Cmd+K, or **Manage routines** on the home feed). You can also just ask the assistant in chat — "every morning, give me a list of Staff+ engineering jobs" — and it sets up the routine for you (you approve it first). Not sure what to automate? The **"Help me set up a routine"** button on the Routines page starts a short guided chat: the assistant explains what routines can do, asks one question at a time (what it should do, when it runs, whether it writes answers or takes actions), and builds the routine with you. Per-routine options: use personal context, allow web search, or plain offline generation. Anjadhe ships with starter routines already scheduled — Daily Briefing, News Digest, Daily Motivation, Weekly Reflection — edit, reschedule, or delete them freely. A prompt you run by hand instead is a **saved prompt**: it lives in the Notes app (New note → Saved prompt template) with a panel to run it in the AI Assistant, or as a web search in your own browser — schedule it there and it becomes a routine and moves to the Routines page. Open a feed post and **Discuss with Assistant** starts a chat with the result already in context. The feed groups posts by the routine that wrote them: one block per routine, showing the newest result with a line or two of what it says, and older results folded behind **Show N earlier**. Click a block to read the full post. **Unread / All** in the feed header switches between what still needs you and the whole history. Email insights are not in the feed; they appear on the home page above it. Anjadhe keeps the last 10 results per routine — pin one in the Notes app to keep it for good. A feed post and its routine link to each other: the post's meta line opens the routine on the Routines page, and the routine's page shows its recent runs. Links inside feed posts (and chat, notes, journal, bookmarks) open in your own web browser, where you are already signed in.`
        ...
        'ai-models': {
            ...
            description: 'The four homes for the model, switching the default model, adding an OpenAI/Anthropic key or Anjadhe Cloud, pointing one part of the app at a different model, when a cloud model makes sense.',
...
4. **Anjadhe Cloud (the default for a new install).** An open-weight model hosted by Anjadhe — no account, no key, a free monthly allowance of AI requests. What runs on it goes to Anjadhe's server (api.anjadhe.com), which forwards it to a zero-data-retention inference provider, without your identity; the service keeps usage counts, never what you asked, and its source code is public so that can be checked.

Anjadhe Cloud is where a new install starts; every other off-Mac option is an explicit choice, and nothing ever falls back to a provider you didn't add.
...
- **Work sent to a model off this Mac runs beside your chat** instead of waiting in line behind it, because it isn't competing for this Mac's memory. That's the one way to make Anjadhe do two things at once.
...
- **Sharing (experimental)** — share one account read-only with another Anjadhe user. Turn on **Sharing** under **Settings → Developer → Experimental features**, pair with the other person by invite code under **Settings → Accounts → Contacts**, then use **Share** on the account. Their Mac shows it under **Shared with you**, left out of their totals; **Revoke** removes it. Everything travels end to end encrypted between the two Macs through Anjadhe's relay, which sees only ciphertext and keeps no record of who shares with whom. For now both Macs have to be open and online for a share or an update to arrive, and a shared copy that hears nothing for 30 days removes itself.
...
Connect or disconnect Google (Gmail, Calendar) per account. Each Mac authorizes independently. **Email preferences** (insights, bundles, sender rules — shared by every account) has its own row here. With the experimental Sharing feature on, a **Contacts** card pairs you with another Anjadhe user by invite code for Portfolio sharing.
...
## Appearance
Choose a theme and which apps appear in navigation. On macOS, **Keep running in the menu bar** lets routines continue after you close the main window. The menu bar has Open Anjadhe, View updates, Pause scheduled routines and Quit. **Launch at login** is available in the installed app. Your Mac can sleep normally; routines catch up after waking. Quit stops background work.
...
- **Experimental features** — per-Mac switches for features that are off by default: **Terminal access** (the \`anjadhe\` command) and **Sharing** (share a Portfolio account with another Anjadhe user). Flipping one reloads the app.
