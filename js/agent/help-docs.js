...
1. Open **Actions** (press Cmd+K and type its name), then open the **Plan** tab.
...
The nav beside the list filters the same system along two dimensions that **combine**: **Dates** — Today, Tomorrow, This Week, This Month, Later (beyond this month, plus the undated backlog), All time — and **Focus areas**, including a "No focus area" row for tasks filed under nothing. Pick one of each (say Today + Health) and the list narrows to both. Pills above the list always show the active filters; the × on a pill clears just that one. Every view ends with a collapsed "N done" section of completed tasks — click it to expand.
...
Click down the hierarchy; the left nav (All groups → group → area) and the links inside each page navigate back up. "Stuck" (no progress / need help) counts show in red on Plan cards.
...
Each quoted task above is literally what you'd type into quick-add — the dates, times, and repeats are parsed from the words. Note the shape: a couple of groups, a handful of areas, one or two live goals per area, and repeating tasks carrying the routines. Areas change monthly, goals weekly, actions hourly — if the tree is bigger than that, it's doing overwhelm's job for it.`
        ...
        'the-assistant': {
            title: 'The assistant',
            description: 'What the AI assistant can do, agent vs. chatbot mode, memory, sources, long tasks, background prompts and the feed.',
...
**Automations — working while you are away.** Ask for something on a schedule ("every weekday at 7, check my portfolio against the news") or when something happens (an email from a certain sender arrives, a file changes) and the assistant offers to arm it as an automation. Automations run in the background on this Mac, notify you when they start and finish, and pause to ask if they hit something needing approval — waiting until you answer rather than guessing. The **Automations** page (Cmd+K, type "automations") lists what is armed, what ran, and what is waiting on you. It also lists **Standing permissions** — grants that let the assistant act without asking, each with optional limits (uses per day, an end date, exceptions such as "known email recipients only") and one-tap revoke. When a limit runs out, the assistant asks instead of acting.
...
**Three modes: agent, chatbot, task.** A chip on the message box shows the mode; clicking it cycles. **Agent** (default) is the full assistant: personal context, memory, tools. **Chatbot** sends your words straight to the model — no system prompt, no personal data, no tools; useful for quick generic questions and for checking whether slowness comes from context or the model. **Task** turns your message into a long-running job: the assistant plans the steps, you approve the plan, and it works through them with tools and reports back. The mode is per-chat.
...
## Background prompts & the feed

A prompt (the **Prompts** page, reached with Cmd+K, or **Manage prompts** on the home feed) is a reusable instruction — "summarize AI news". Run it manually, or schedule it (daily, weekly…, optionally at a set time) to run in the background on your local model — a **background prompt**. You can also just ask the assistant in chat — "every morning, give me a list of Staff+ engineering jobs" — and it sets up the background prompt for you (you approve it first); daily and weekly schedules can run at a specific time like 8:00 AM. Background runs post to the feed on the home page; open a post and **Discuss with Assistant** starts a chat with the result already in context. Per-prompt options: use personal context, allow web search, or plain offline generation. Anjadhe ships with starter prompts already scheduled — Daily Briefing, News Digest, Daily Motivation, Weekly Reflection — edit, reschedule, or delete them freely on the Prompts page. The feed groups posts by the prompt that wrote them: one block per prompt, showing the newest result with a line or two of what it says, and older results folded behind **Show N earlier**. Click a block to read the full post. **Unread / All** in the feed header switches between what still needs you and the whole history, and the sliders button opens **Feed preferences** to choose whether prompt results and email insights appear at all. Email insights are not in the feed; they appear on the home page above it. Anjadhe keeps the last 10 results per prompt — pin one in the Notes app to keep it for good. A feed post and its prompt link to each other: the post's meta line opens the prompt on the Prompts page, and the prompt's page shows its recent runs. Links inside feed posts (and chat, notes, journal, bookmarks) open in Anjadhe's own browser with a "Back" strip to return where you were.`
        },
...
Anjadhe runs AI in the background on your behalf: email insights and bundling after a mail sync, background prompts, memory tidy-ups after a chat, goal-progress updates, and model warm-ups (loading the model so your first message answers fast). Loading a model into memory is itself the heaviest moment for the GPU, even before any request runs. All of it shows on this page, so a busy GPU always has a visible reason.
...
Web search is an explicit opt-in: first-run setup asks, and the **Enable web search** switch at the top of **Settings → AI Assistant → Web Search** turns it on or off anytime. When it is off, nothing in Anjadhe sends queries to the web — the assistant answers from the model alone, and News and scheduled prompts work from local data only. When enabled, the built-in option is **Anjadhe Connect** (api.anjadhe.com), a small relay Anjadhe runs, which forwards the query to a search provider and returns the results. 300 searches a month are included — no account, no key to paste.
...
**News** (Cmd+K, then "news") is a front page of current headlines for your topics: a lead story, a Latest rail, then a section per topic — "All N stories →" drills into a topic, "← All news" comes back. Headlines come straight from news feeds; no AI writes them. Stories you have already opened dim.

Clicking a headline opens the **reader**: the article is fetched and summarized on your Mac, with **Open article** for the original page and **Ask about this** to discuss it with the assistant. When the AI spots upcoming dates in an article (an event, a deadline), it offers **Add to Schedule** — checked by you before you rely on it.
...
The assistant can read these headlines too — ask it "what's the news?" and it summarises your followed topics rather than searching the web. The **Morning News** background prompt does the same thing at 7am and posts the digest to your home feed (edit or reschedule it under **Manage prompts**).
...
- **Bundles** — categorical mail (newsletters, promotions, receipts) is grouped into a handful of piles, each a card showing the pile's name, when its newest mail arrived, and who has been writing lately. Click a card to see only that mail; the breadcrumb at the top names where you are, and clicking "Email" takes you back. Inside a bundle, a row of buttons above the list acts on the whole pile: mark all read, mark all unread, archive all. Personal mail is never sorted into a topic bundle: it sits in "Unbundled", the first card.
- **View toggles** — two icon buttons in the header. The envelope filters the list to unread mail only. The stack turns bundling off, for one flat list of everything in date order. Both remember their setting on this Mac. Opening a message marks it read after a second and a half, so a quick look does not clear it, and the row you are reading stays in place while it is open.
- **Reading pane** — the inbox is two panes: the list on the left, the message on the right. Click a row to read it, click another to move on. Drag the divider to resize (double-click it to reset), and use the panel button in the header to hide the pane when you want the full-width list. The page itself never scrolls: the sidebar, list, message, and insights each scroll on their own. The hamburger beside the Email title folds the Labels and Accounts sidebar away.
...
- **Quick capture** — type into the dashed box at the top of the list and press Enter to start a note.
- **Search** — full-text across titles and bodies as you type; arrow keys walk the results, Enter opens, Cmd/Ctrl+F jumps to the search box.
...
A strategy is the plan behind the holdings: what the money is for, when you need it, how big a drop you can sit through, a target mix, and the limits you want to hold yourself to. It lives with the assistant, not in a settings page. The Portfolio view shows a single quiet line with your strategy's name and objective; clicking it starts the conversation.
...
Once saved, the app measures your real holdings against the plan whenever the assistant checks it: which sleeves drifted outside their band, which guardrails are broken, and roughly what it would take to get back. Those numbers are computed by the app, not written by the AI. Adding a transaction that puts you off plan says so at the time, and the Market Close Review (below) names any drift in the evening.
...
Three background prompts review the portfolio on weekdays and post to the Home feed: a pre-market review around 7:30 AM (overnight headlines that touch your holdings, what to watch), a midday check around 12:30 PM (only positions moving 2% or more), and a market-close review around 4:30 PM (the day's result, cash, and whether you are still on plan). They run on your own model with read-only tools, skip weekends, and stay quiet when there is nothing to say. Edit times or turn any of them off in the Prompts app.`
        },
...
- **Ctrl+Tab** — switch between recently opened apps, like browser tabs. Tap once to bounce back to the app you were just in; hold Ctrl and keep tapping Tab (Shift+Tab reverses) to walk the list, release Ctrl to switch. The clock icon in the titlebar opens the same strip with the mouse — click an app to switch.
