...
**Name it.** You can give the assistant a name: first-run setup asks, and the **Name** field at the top of **Settings → AI** works anytime. The name replaces the "AI Assistant" label across the app, and the assistant knows it and answers to it. Clear the field to go back to the generic label; the name syncs to your other Macs.
...
Every AI feature — chat, email insights, action suggestions, builds — runs on one model: the **default entry** in **Settings → AI**. It can live in three places:
...
Web search is an explicit opt-in: first-run setup asks, and **Settings → AI Assistant → Web Search** turns it on or off anytime. Without it the assistant answers from the model alone. When enabled, the built-in option is **Anjadhe Connect** (api.anjadhe.com), a small relay Anjadhe runs, which forwards the query to a search provider and returns the results. 300 searches a month are included — no account, no key to paste.
...
The **Tavily** and **Brave Search** cards (Settings → AI → Web Search) send searches straight from this Mac to that provider — no Anjadhe relay involved. Create a free account (Tavily includes 1,000 searches a month, no credit card), paste the key on the card, **Save Key**, then **Test**. Keys are stored encrypted on this Mac and never sync — paste the key on each Mac you use.

## Check what left this machine

A Sources row under web-assisted answers lists what was searched and every page opened, and **Settings → AI → Web Search Logs** records every query that left this Mac — whichever provider handled it.`
        },
...
- **Models** — the model list with a default-model radio. Download local models (built-in llama.cpp engine), point at your own OpenAI-compatible server, or **+ Add model** with an OpenAI/Anthropic API key. Per-model Manage panel for keys and options.
- **Web Search** — Anjadhe Connect card (built in — plan, usage, Test) plus Tavily / Brave key cards for direct-to-provider searches.
...
## Appearance
Dark mode (follows the OS or manual), Customize home apps — show/hide/reorder launcher tiles — and the **Discover pane** toggle: an optional right-hand links pane on the home page with stories for your chosen topics. Off by default; it also needs web search enabled (Settings → AI → Web Search) to appear.
