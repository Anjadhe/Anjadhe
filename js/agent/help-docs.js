...
The **Tasks** app is the front door — it opens to Today, a short finishable list merging today's tasks, action items pulled from email, and your calendar. Tomorrow, This Week, This Month, Later, and each goal group are one click away in its nav.
...
Open **Settings → AI Assistant** to download a model (Gemma is a good default; local models are offered on Macs with 32 GB of memory or more). Everything runs on this Mac by default. On a smaller Mac — or for more power — use **+ Add model**: Anjadhe Cloud (hosted by Anjadhe, free monthly allowance, no key), your own OpenAI-compatible server, or an OpenAI/Anthropic model with your own API key. Always an explicit choice, never a fallback.
...
Every feature page has a small **?** in its header that opens that page's guide in place.`
        ...
        'your-day': {
            title: 'Your day — Tasks & Goals',
            description: 'The Tasks app: Today list, quick-add in plain language, repeating tasks, overdue, weekly review, the Email AI app (every insight found in mail, incl. bookings), and the Goals page behind the Plan button (groups, goals, the goal interview).',
...
**Mention a record with @.** Type @ in any assistant message box to pull up your tasks, goals, notes, routines, strategies and accounts; keep typing to narrow, or make the first word a type ("@goal marathon"). Picking one inserts its name and attaches the conversation to that record — the assistant sees its current state and its decisions from then on. A banner above the chat names the attachment, with an ✕ to detach.
...
1. **This Mac (default).** An open-weight model like Gemma or Qwen via the built-in llama.cpp engine. Free, offline, nothing leaves the machine. Offered on Macs with 32 GB of memory or more — that's what first-run setup installs there; below 32 GB no local model is offered (small ones didn't work well enough) and setup suggests Anjadhe Cloud instead.
...
1. **Settings → AI Assistant** → **+ Add model**. The page lists the homes
   ...
   Cloud** first on a Mac under 32 GB.
...
Local models are offered only on Macs with 32 GB of memory or more — smaller Macs were offered small models for a while, and they didn't work well enough to keep. On an 8–16 GB Mac, Anjadhe Cloud is the no-setup option, with a free monthly allowance — when it runs out, requests pause until the 1st. With your own key, be clear-eyed: what you run on that model goes to that provider under your account and their data terms, and usage is billed by the provider to you — Anjadhe adds nothing on top.`
        },
...
The × on any story means show fewer like this — ranking learns from it, and hidden stories can be restored from the Topics dialog. Stories boosted toward your interests carry a "For you" label with the reason. Clicks, hides, and ranking all stay on this Mac; the model never chooses or writes the headlines you see.`
        ...
        'connected-accounts': {
            title: 'Connected accounts — Email, Calendar & Apple apps',
            description: 'Connecting Gmail and Google Calendar, email bundles/insights/action items, the calendar lens, and importing Apple Reminders, Notes and Calendar.',
...
- **Panes** — three icon buttons in the header, each a small map of the page with the pane it hides or shows shaded: the filters sidebar, the note list, and the reading pane. With the reading pane off the list takes the full width, and opening a note swaps to the full-width note with a back chip naming the list you came from ("← Pinned", "← work"). Each choice is remembered on this Mac, and the dividers between panes drag to resize.
...
A **Market Review** routine reviews the portfolio each weekday and posts to the Home feed shortly after the US market closes, at the matching time in your own timezone (4:30 PM Eastern is 1:30 PM in California). It covers the day's result, your best and worst positions, cash, whether you are still on plan, and any headlines that touch your holdings. It runs on your own model with read-only tools, skips weekends, and stays quiet when there is nothing to say. Edit the time or turn it off in the Routines app.`
        ...
        'how-anjadhe-works': {
            ...
            description: 'Where data lives, multi-Mac sync, keyboard shortcuts, Maker, Writing Voices, building apps with a coding agent.',
...
**Reader** — the user's imported documents (reports, essays, papers, statements), indexed on this Mac and searchable by meaning. Import files, drag-drop, or Finder-drop into the folder (~/Anjadhe/library/; .md, .txt, .pdf incl. scans, .docx, .html). Searching shows passages grouped by document; opening one shows the full text with matches marked, plus Open original / Show in Finder / Ask about this document (the assistant then knows what's open). Deleting a document moves the file to the macOS Trash. Semantic search needs a one-time ~330 MB model download in **Settings → Reader & Writing Voices** (also index status, Rescan/Re-index, the folder location); until then search is keyword-only. Indexing always runs locally — only passages retrieved for a question travel with that chat turn.

**Writing Voices** — writing styles learned from documents. The app opens on the list of voices; sample voices (Mark Twain, Abraham Lincoln — public-domain works, bundled) import with one click, or create one by naming it ("My voice", "Mark Twain") — it gets its own folder under ~/Anjadhe/library/ — then add documents on its page (Add documents… or drag-drop; posts, essays, books, speeches; .md, .txt, .pdf, .docx, .html) and press **Study**: the model reads a spread sample of the voice's documents (more documents = a wider sample; **Study depth** at Settings › Reader & Writing Voices sets how much one study reads) and writes a short, editable style guide plus verbatim passages from those documents. Edits to the guide stick — re-studies never overwrite them — and passages can be pinned. **Draft in this voice…** opens a chat already primed for that voice (it greets the user and asks what to write; the draft names the documents it drew on); asking any chat to write "in X's voice" works too. A digest ROUTINE can carry a voice for its posts (the Voice picker on the routine's form, or voice: "<name>" on create_routine) — default is the assistant's own voice, and a routine keeps running in it if its voice is deleted. Deleting a voice keeps its documents. No fine-tuning, no hidden style profile: the guide is a page the user can read and change. Every document is also visible in the Reader app — the two are lenses over one folder.
...
- **Your Models** — the model list with a default-model radio. Download local models (built-in llama.cpp engine), point at your own OpenAI-compatible server, add Anjadhe Cloud (free allowance, no key), or **+ Add model** with an OpenAI/Anthropic API key. Per-model Manage panel for keys and options; models that can view images carry a "Reads images" badge.
