...
A prompt (the **Prompts** page in the sidebar, or **Manage prompts** on the home feed) is a reusable instruction — "summarize AI news". Run it manually, or schedule it (daily, weekly…, optionally at a set time) to run in the background on your local model — a **background prompt**. You can also just ask the assistant in chat — "every morning, give me a list of Staff+ engineering jobs" — and it sets up the background prompt for you (you approve it first); daily and weekly schedules can run at a specific time like 8:00 AM. Background runs post to the feed on the home page; open a post and **Discuss with Assistant** starts a chat with the result already in context. Per-prompt options: use personal context, allow web search, or plain offline generation. Anjadhe ships with starter prompts already scheduled — Daily Briefing, News Digest, Daily Motivation, Weekly Reflection — edit, reschedule, or delete them freely on the Prompts page. The feed also shows unread **email insights** as cards, and the sliders button in the feed header opens **Feed preferences**: choose whether prompt results and email insights appear, and whether read insights stay hidden. A feed post and its prompt link to each other: the post's meta line opens the prompt on the Prompts page, and the prompt's page shows its recent runs. Links inside feed posts (and chat, notes, journal, bookmarks) open in Anjadhe's own browser with a "Back" strip to return where you were.`
        },
...
Open **AI Activity** from the System group in the sidebar, or click the pulse icon in the titlebar — it pulses with an amber dot whenever AI work is running. The page shows what is using the AI engine on this Mac, as plain activities rather than logs:
...
Web search is an explicit opt-in: first-run setup asks, and the **Enable web search** switch at the top of **Settings → AI Assistant → Web Search** turns it on or off anytime. When it is off, nothing in Anjadhe sends queries to the web — the assistant answers from the model alone, and News, Discover, and scheduled prompts work from local data only. When enabled, the built-in option is **Anjadhe Connect** (api.anjadhe.com), a small relay Anjadhe runs, which forwards the query to a search provider and returns the results. 300 searches a month are included — no account, no key to paste.
...
A Sources row under web-assisted answers lists what was searched and every page opened, and **Settings → AI Assistant → Web Search Logs** records every query that left this Mac — whichever provider handled it.`
        ...
        'news': {
            title: 'News & the Discover pane',
            description: 'Following topics: the News front page, article reader, Catch me up digest, dates to Schedule, the Discover pane on Home, topics and location, "show fewer like this".',
            content: `## Turn it on

News and the Discover pane share one switch and one topic list. Turn on the **Discover pane** in **Settings → Appearance** and enable web search (**Settings → AI Assistant → Web Search**), then pick topics. Topics live in the **Discover topics** dialog — the **Topics** button on the News page, or the small tune icon on the pane. Pick from the suggestions or add your own ("San Francisco city updates"). An optional **Location** adds local news, and puts today's weather at the top of the Discover pane.

## The News page

**News** in the sidebar is a front page of current headlines for your topics: a lead story, a Latest rail, then a section per topic — "All N stories →" drills into a topic, "← All news" comes back. Headlines come straight from news feeds; no AI writes them. Stories you have already opened dim.
...
**Catch me up** (on the News page and the Discover pane) writes a short AI digest of the current headlines, on your Mac. It needs an AI model set up.

## Make it yours

The × on any story means show fewer like this — ranking learns from it, and hidden stories can be restored from the Discover topics dialog. Stories boosted toward your interests carry a "For you" label with the reason. Clicks, hides, and ranking all stay on this Mac; the model never chooses or writes the headlines you see.`
        },
...
- **Accounts** — the tab strip under the header holds All Accounts (the dashboard) plus one tab per account, so you can switch straight to an account without going back through the dashboard.
- **Holdings** — lots of the same symbol fold into one row you can open; the row shows the combined position.
- **Options** — long calls and puts are tracked next to stocks. In the transaction editor, switch from Stock to Option and fill in the underlying, call or put, strike, and expiration; quantity means contracts and price means premium, and the 100x multiplier is applied for you. Positions read as "AAPL $250 Call 12/18/26" with a chip that turns amber near expiry and red once expired. Short and written options are not supported.
...
**Maker** — describe a document or small interactive page ("a mortgage calculator", "a research brief on X with sources") and the assistant writes a self-contained artifact you can open any time. Ask in chat ("build me a…") or open Maker from the launcher. Builds stream progress and run on your own model.
...
Prefer a terminal? Turn on **Build Apps** in **Settings**. Anjadhe creates ~/Anjadhe/apps/ — one subfolder per app, plus CLAUDE.md / AGENTS.md holding the full contract (manifest format, the Anjadhe SDK, worked examples) and .anjadhe-schemas.json describing built-in data shapes. Start your coding agent in that folder and describe the app; the agent picks up the contract on its own. Anjadhe watches the folder — changes reload live, and errors are written to .errors.log inside the app's folder so the agent can read and fix them.`
        ...
        'settings': {
            ...
            description: 'Map of the Settings app: AI Assistant, Accounts, Appearance, Browser, Data & Storage, Privacy & Security, Build Apps, Advanced.',
...
## Appearance
Dark mode (follows the OS or manual), Customize home apps — show/hide/reorder launcher tiles — and the **Discover pane** toggle: an optional right-hand links pane on the home page with stories for your chosen topics. Off by default; it also needs web search enabled (Settings → AI Assistant → Web Search) to appear. The News page shares this switch and its topic list.
...
Turn on the ~/Anjadhe/apps/ folder for coding-agent-built apps. Each installed app is listed with a **Reset Data** button that clears that app's saved data alone (the app stays installed; the reset syncs to other Macs).
