...
## Appearance
Theme (Light, Dark, or System to follow your Mac) and **More apps** — every app as a card with an enable switch (also the "More apps" row at the bottom of the sidebar). An app turned off keeps its data and can still be found by typing its name in Cmd+K; some apps (Portfolio, Wellness, Web Browser, AI Activity, Help, About) start off and are enabled here. Packaged apps (Wellness, Pomodoro, Portfolio, Bookmarks, News) also have an **Uninstall** action: an uninstalled app is not loaded at all (no page, home card, search results or assistant tools), its data stays on the Mac, and it comes back with **Install** from the same page after a reload.
