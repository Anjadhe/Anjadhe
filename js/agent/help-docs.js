...
A new install starts on **nenva Cloud**: an open-weight model hosted by nenva, with a free monthly allowance and no account or key. What the AI works on goes to nenva's server and on to its inference provider under a no-retention agreement; your data itself stays on this Mac. To keep the AI on this Mac too, open **Settings → AI Assistant** → **+ Add model** and download a local model (offered on Macs with 8 GB or more, sized to the machine), then make it the default. The same page adds your own OpenAI-compatible server or an OpenAI/Anthropic model with your own API key. Nothing ever falls back to a destination you didn't pick.
...
Every feature page has a small **?** in its header that opens that page's guide in place.`
        ...
        'your-day': {
            ...
            description: 'The Tasks app: Today list, quick-add in plain language, repeating tasks, overdue, weekly review, the Insights app (every insight found in mail and texts, incl. bookings), and the Projects page behind the Plan button (groups, projects, the project interview).',
            actions: ['tasks', 'goals', 'email-insights'],
            content: `**Tasks** is what to do now — it opens to Today. **Insights** (its own app) is everything the assistant found in your mail and texts. Planning lives one door away: the **Plan** button on the Tasks page opens **Projects** — every active project in one table with its progress, next step, last update and target, each broken into tasks, each with an Updates log you and the assistant both post to. (Projects — formerly Goals — was a third tab until 2026-08-03; planning is a different altitude from doing, so it is a door rather than a peer.)
...
**No date, no time.** A task does not need either. In the task's page, hover the Date, Start or End row and click the × beside the value to clear it; the row menu's "Set date → No date" does the same for the date. An undated task lives in the Later view's "No date" backlog until you give it a day. A repeating task always keeps its start date (that is what its occurrences are computed from), so its date cannot be cleared.
...
## Insights — what your mail and texts said (the app was labelled "Email AI" until 2026-09-10)

**Every** insight found in mail lands here, grouped by kind — and, when **Settings → Accounts → iMessage → Insights from your texts** is on, the same folders hold what your texts said (each conversation is read once it goes quiet, appears with an "iMessage" tag and an iMessage entry under Accounts, and opens to the conversation itself with an "Open in Messages" door). Grouped by kind: a flight confirmation, a renewal date, a statement closing, a package arriving, a bill due Friday. Anything that needs doing *also* becomes a **task** on the Tasks tab, with a badge back to the email — and the insight here links forward to that task. The two answer different questions: Tasks is what you have to do, this is what your mail said. (Before 2026-08-03 an insight left this page the moment it became a task, which made it a page of leftovers; it was called "FYI" then.)
...
A project is something you can finish, with what "done" looks like, a target date, a **Group** label, and linked tasks. A project is either completed or it isn't — progress lives in its tasks (the n/m count on every project). The page opens on **every active project in one table**: progress, the next open step, when it was last updated, and the target date (red once it has passed, amber within a week). A **Needs attention** list above it names what wants you — a target passed or due this week, a project with no open next step, one that has gone quiet for two weeks — and completed projects fold beneath. The pills under the title filter the table by group; each group keeps its own colour on chips and progress bars. Open a project for its sheet: **Mark complete** in the head (it asks what to do with any open tasks), its tasks, its **Updates**, then the details and a folded **More** (weekly review, decisions, notes and bookmarks, delete).
...
**Groups are just labels.** A group exists because a project carries its name. Edit the Group field on a project (it suggests existing names). To rename a group, pick it in the filter row and click its title (or Rename); to dissolve one, Remove group on that page — its projects stay, ungrouped. Feeling stuck is a conversation, not a label — the "I'm stuck" button hands the project to the assistant.

**Updates — the log.** Every project and every task carries an **Updates** section: post what happened ("sent the deck, waiting on Dana"), and the assistant posts there too — its read when you ask "How is this going?" or press **Ask nenva to check in**, a risk it noticed, and every weekly review. Each entry says who posted it and when; the page shows how long it has been since the last one. Updates are history for you and the assistant to read back, not instructions — a standing instruction is a Decision.

**Weekly AI reviews.** "Review it weekly" (under More on a project) schedules a routine that reads the project's linked tasks and updates each week and posts a short honest read to the Home feed and to the project's Updates — moving, stalled, or done in all but name. On any project, **Suggest tasks** asks the assistant to propose next steps; nothing is added until confirmed.
...
The assistant is one chat that can: answer about your life (tasks, projects, notes, journal, email insights, calendar), do things for you (create tasks and notes, file action items, search the web, read pages, build documents and apps, and — if enabled in **Settings → AI Assistant** — work with files), and answer anything general.

**Private by design.** The model runs on this Mac by default, on a server you own, on OpenAI or Anthropic if you added your own API key, or on nenva Cloud if you chose it. Conversations and personal data go only to the brain you picked. Pick and manage models in **Settings → AI Assistant**.
...
**Attachments.** The + button on the message box (or drag and drop) attaches text files, CSVs, PDFs — and images, if the model can read them. Vision-capable models carry a "Reads images" badge on their card in **Settings → AI Assistant** (most OpenAI and Anthropic models qualify; local Gemma models need their vision file downloaded). On a model that can't view images, nenva says so at attach time.

**Seeing and using the app.** Ask the assistant to look at the screen ("what do you see?") or to do something in the app itself ("open Tasks and search for dentist", "click Save"). It takes a screenshot of nenva's own window, numbers every button and field on it, and works one step at a time: click, type, press a key, or scroll, looking again after each step. With a model that can read images it sees the screenshot itself; with any other model it reads the screen as text (the page's words and a list of its buttons and fields). Every action asks you first, like other actions that change things — pick "for the rest of this session" to let a longer job run. It only works inside nenva's window (not other Mac apps), never in background routines, and it can never click its own chat or answer its own permission prompts. After it has looked at an email or a web page in a chat, every action asks again, whatever you allowed before.

**Using your other apps (experimental).** Turn on **Settings → Developer → Experimental features → Use other apps** and the assistant can work in your other apps the way you would: open your own browser (with the accounts you are already signed in to), go to a website and click through it, fill in a form, open Finder, Numbers or Mail and do things there. It reads each app's buttons and fields through macOS Accessibility, looks at the window if the model can read images, and does one step at a time, looking again after each. macOS asks you once to allow nenva under **Privacy & Security → Accessibility** (and **Screen & System Audio Recording** for screenshots). It asks before working on a website you haven't approved before (choose "for the rest of this session" or "always" to approve that site), and it asks every time before anything sensitive — buying, placing or confirming an order, paying, sending, deleting or signing in. Ordinary clicks, typing and scrolling don't ask. It never types passwords, codes or card numbers — it stops and asks you to do those parts — and it never touches Keychain, password managers, System Settings or terminal apps. A bar with a **Stop** button sits at the bottom of the screen while it works. It does not run in background routines or from Telegram. If you also added the **Browser (Playwright)** tool, that one opens a separate browser window that is signed in to nothing; anything that involves your accounts (a cart, an order, checking out, signing in, or simply "open the browser") uses your own browser instead.
...
**From the terminal.** Terminal access is an experimental feature: turn it on under **Settings → Developer → Experimental features**, then open **Settings → AI Assistant → Terminal Access**, switch it on and click **Install command** to get the \`anjadhe\` command. Bare \`anjadhe\` opens an interactive session; \`anjadhe ask "question"\` is a one-shot; \`anjadhe task "goal"\` runs task mode. Same brain, same permissions — approvals appear right in the terminal — and the transcript lands in a "Terminal" conversation that stays on this Mac. Only this Mac can connect.

## Routines & the feed

A **routine** is a prompt that runs on a schedule (hourly, every 6h, daily, weekdays, or weekly, optionally at a set time like 8:00 AM) in the background on your local model and posts each result to the feed on the home page. (That is a routine that writes answers; a routine that **takes actions** keeps each run's log on its own page under Run history instead — the feed carries what your AI wrote, not what it did.) Manage them on the **Routines** page (reached with Cmd+K, or **Manage routines** on the home feed). You can also just ask the assistant in chat — "every morning, give me a list of Staff+ engineering jobs" — and it sets up the routine for you (you approve it first). Not sure what to automate? The **"Help me set up a routine"** button on the Routines page starts a short guided chat: the assistant explains what routines can do, asks one question at a time (what it should do, when it runs, whether it writes answers or takes actions), and builds the routine with you. Per-routine options: use personal context, allow web search, or plain offline generation. nenva ships with starter routines already scheduled — Daily Briefing, News Digest, Daily Motivation, Weekly Reflection — edit, reschedule, or delete them freely. A prompt you run by hand instead is a **saved prompt**: it lives in the Notes app (New note → Saved prompt template) with a panel to run it in the AI Assistant, or as a web search in your own browser — schedule it there and it becomes a routine and moves to the Routines page. Open a feed post and **Discuss with Assistant** starts a chat with the result already in context. The feed groups posts by the routine that wrote them: one block per routine, showing the newest result with a line or two of what it says, and older results folded behind **Show N earlier**. Click a block to read the full post. **Unread / All** in the feed header switches between what still needs you and the whole history. Email insights are not in the feed; they appear on the home page above it. nenva keeps the last 10 results per routine — pin one in the Notes app to keep it for good. A feed post and its routine link to each other: the post's meta line opens the routine on the Routines page, and the routine's page shows its recent runs. Links inside feed posts (and chat, notes, journal, bookmarks) open in your own web browser, where you are already signed in.`
        },
...
A Sources row under web-assisted answers lists what was searched and every page opened, and **Settings → AI Assistant → Web Search Logs** records every query that left this Mac — whichever provider handled it.`
        ...
        'news': {
            title: 'News',
            description: 'Following topics: the News timeline, article reader, reading any article by its link (and the Chrome extension), Catch me up digest, dates to Schedule, topics and location, "show fewer like this".',
            actions: ['news', 'web-search'],
            content: `## Turn it on

News needs web search enabled (**Settings → AI Assistant → Web Search**). Then pick topics with the **Topics** button on the News page. Pick from the suggestions or add your own ("San Francisco city updates"). An optional **Location** adds local news. The **Sources** card on the same page chooses where headlines come from: Google News is on by default, and you can add Bing News or Hacker News — every topic is fetched from each source you turn on, and a story from a source other than Google News says so in its byline. With more than one source on, the left rail grows a **Sources** list: click one to see only its stories (on the front page, inside a topic, and in search), and **All sources** to go back. Headlines refresh when you open News, not in the background.
...
Clicking a headline opens the **reader**: the article is fetched and summarized on your Mac, with **Open article** for the original page and **Ask about this** to discuss it with the assistant. That chat starts with the article's text itself in context, and when web search is on the assistant also searches for background or newer developments instead of relying on what it remembers. When the AI spots upcoming dates in an article (an event, a deadline), it offers **Add to Schedule** — checked by you before you rely on it. Under the summary, **Related coverage** lists a few real links to where the story continues — another outlet on the same story, background, what followed — found by one web search on the headline; the titles and links are the publishers' own, and the AI only chooses which are worth your time. A row opens here in the reader; the arrow beside it opens the page in your own browser. It needs web search on, and it is skipped when it finds nothing.

**Any article by its link.** Paste a link into the News search box and press Enter (or click **Read this link**) and the reader opens on it: the page is fetched and summarized on your Mac, and when it will not load — a paywall, a bot wall, a consent wall — coverage of the same story from other sources is searched and summarized instead, labeled as such. The **nenva Reader** Chrome extension (in the app's source under extension/chrome, loaded unpacked for now) is the same door from the browser: one toolbar button, Alt+Shift+R, or right-click → "Read this page in nenva" hands the tab's address and title to the app; it reads nothing on the page and sends nothing anywhere else.
...
The × on any story means show fewer like this — ranking learns from it, and hidden stories can be restored from the Topics page (the Topics button, or Edit topics in the rail). Stories boosted toward your interests carry a "For you" label with the reason. Clicks, hides, and ranking all stay on this Mac; the model never chooses or writes the headlines you see.`
        },
...
- **Google round-trips** — new events, edits, deletes and recurring events (the form's Repeat row; an instance asks whether the change is for this event or the whole series) all write back to Google, and the same edits on an Apple Calendar event write to Apple Calendar on this Mac (see below).
...
- **Apple Notes → Notes** — notes come across with their text, tagged \`apple-notes\` plus the name of their Apple Notes folder (a note moved to another folder picks up the new folder's tag on the next import, as long as you have not changed its tags here). Locked notes are skipped. Turn this on for ONE Mac only; imported notes reach your other Macs through nenva's own sync.
- **Apple Calendar ↔ Calendar** — events from this Mac's iCloud and local calendars appear in the Calendar with their own "Apple Calendar · this Mac" row in the accounts rail, refreshed every minute while Calendar is open. New events can go into any of those calendars (pick it in the form's Account row), and editing or deleting an Apple event writes to Apple Calendar on this Mac, with the same this-event / all-events choice for a series. Calendars that refuse edits (subscribed, holidays, shared read-only) show their events but no Edit or Delete. The assistant can create, update and delete Apple events too: say which calendar, or "apple" when there is one. Google-account calendars in the Mac's Calendar app are deliberately left out — nenva syncs Google Calendar itself.

All three are one-way: nothing is ever written back to the Apple apps. Reminders and Notes import when the app opens, plus an **Import now** button on each row; the Calendar mirror also refreshes on its own (every minute while Calendar is open, every five minutes otherwise), so an event added in Calendar.app shows up without a relaunch. A note or task you edit in nenva keeps your edit — later changes on the Apple side no longer touch it. **Turning a switch off removes what it imported** (everything stays in the Apple app; turning it back on brings it all back).
...
**Settings → Accounts → iMessage** has two cards. The first sends this Mac's reminders and alerts to your own iMessage — a text from you to you, through the Messages app on this Mac, so it reaches your iPhone and Apple Watch with Apple's own end-to-end encryption and no bot or server in between. Enter the phone number or email you use for iMessage, press **Send a test** (the first send asks macOS for permission to let nenva control Messages), then turn the switch on. Messages on this Mac must be signed in to iMessage.

**Insights from your texts** (the second card) reads the texts that arrive in Messages on this Mac the way Insights reads your mail: each conversation is read once it goes quiet, and appointment reminders, deliveries, bills, codes and plans made over text land in the Insights app's folders (with an "iMessage" tag and their own entry under Accounts); anything that needs doing becomes a task, with a "Text from" banner on the task that opens the insight. It needs **Full Disk Access**, and you have to give it by hand — macOS does not put nenva in that list on its own, so do not look for a switch that is not there yet. Open System Settings → Privacy & Security → Full Disk Access, click the **+** button under the list, pick nenva from your Applications folder (it is still called Anjadhe there if this Mac updated from it), and make sure its switch is on. Then quit nenva, open it again, and press **Check now** on the card. The first read covers the **last 30 days** of texts, grouped by conversation (one conversation counts as one item, and a chat with only your own texts is skipped), so the count is smaller than your Messages list; from then on new texts are read as they arrive. That permission covers the whole Messages database, which is why the switch is off until you turn it on. Your texts go only to the model you chose; with a cloud brain they stay on this Mac unless **Settings → AI Assistant → Cloud Privacy** allows Messages. Turn it on for ONE Mac. Turning it off forgets the conversations it read; tasks stay. The assistant still does not answer texts — use Telegram or the nenva phone app to chat with it from your phone.

**The assistant can send texts.** Once either card is set up, ask it in chat: "text Priya that I'm running late", "send myself a reminder by iMessage", "text +1 555 0100 the address". It sends through the Messages app on this Mac, and it shows you the recipient and the exact wording first — every text to another person asks for your OK, and a standing permission (Settings → AI Assistant → Permissions) can only ever cover texts to your own number. It knows your own number from the Notifications card, and a person's name from conversations Insights from your texts has read; otherwise give it the number. If you just say "send a message", it asks whether you mean a text or an email rather than guessing.`
        },
...
- **Your notes are files.** Every note is also kept as a plain Markdown file in the nenva folder of your home directory (\`~/nenva/Notes/\`, images beside it in \`assets/\`), and every journal entry in \`~/nenva/Journal/\`. Open them in Finder or any editor, including Obsidian: edits you make to the files show up in nenva, and edits here are written back. A file you rename keeps its name. The switch and an Open Folder button are in **Settings → Storage & Backup → Notes and Journal as Files** (on by default, per Mac).
...
Bookmarks is its own app (in the sidebar, and Cmd+K finds it by name): save links with a title and tags, browse them as a grid or a list. Clicking a bookmark opens the page in your own web browser; the i button on a card opens its details instead. The assistant can save bookmarks and pull them into research.
...
- **Current headlines from CNBC** — off by default; turn it on under **Settings → Portfolio → Current headlines from CNBC**. All Accounts (under the brief) and each ticker page (under the profile) then show a short summary of what CNBC is running now, written by your AI model from CNBC's own headlines and rewritten every 10 minutes while the section is on screen, or with **Refresh now**. A ticker page only counts a CNBC story that names the symbol or the company. Needs web access.
- **Turning the AI writing off** — **Settings → Portfolio → Generate stock profile using AI** is one switch over both the business profiles and Today's brief. Off, neither is written and neither is shown (one written on an earlier day could read as current); prices, holdings, strategy and headlines are unaffected.
...
- **Sharing (experimental)** — share one account read-only with another nenva user. Turn on **Sharing** under **Settings → Developer → Experimental features**, pair with the other person by invite code under **Settings → Accounts → Contacts**, then use **Share** on the account. Their Mac shows it under **Shared with you**, left out of their totals; **Revoke** removes it. Everything travels end to end encrypted between the two Macs through nenva's relay, which sees only ciphertext and keeps no record of who shares with whom. For now both Macs have to be open and online for a share or an update to arrive, and a shared copy that hears nothing for 30 days removes itself.
...
A **Market Review** routine reviews the portfolio each weekday and posts to the Home feed shortly after the US market closes, at the matching time in your own timezone (4:30 PM Eastern is 1:30 PM in California). It covers the day's result, your best and worst positions, cash, whether you are still on plan, and any headlines that touch your holdings. It runs on your own model with read-only tools, skips weekends, and stays quiet when there is nothing to say. Edit the time or turn it off in the Routines app.`
        ...
        'how-anjadhe-works': {
            title: 'How nenva works — privacy, sync, and building your own',
...
nenva is private by default. No remote database, no account. Data is stored on this Mac in the standard macOS Application Support area. AI runs where you choose — this Mac (default), a server you own, OpenAI/Anthropic with your own key, or nenva Cloud if you chose it. Backups and storage location live in **Settings → Storage & Backup**. Transparency logs of every AI call and web search are in **Settings → AI Assistant → Logs** — machine-local, never synced.
...
nenva is free during the closed alpha. The plan after it is a **one-time license** with a year of updates, then paid upgrades if you want later years; there is no subscription for the app and no account. The price is not set yet. **Everyone who installs during the alpha keeps the app free for good.** Hosted extras (web search through nenva Connect, nenva Cloud models) keep a free monthly allowance; only usage beyond it would ever be paid, and that stays optional.

## Claim your alpha license

Open **Settings → License** and press **Claim alpha license**. Enter an email address to issue it to. That address is the only thing sent to nenva, and it exists so the same key can be issued to you again: on another Mac, or after a reinstall, claim again with the same address and you get the same key back.
...
**Settings → License → Remove from this Mac** forgets the key on this Mac only. It is not revoked and nothing is sent; enter it again any time.`
        ...
        'settings': {
            ...
            description: 'Map of the Settings app: AI Assistant, Accounts, Appearance, Storage & Backup, Privacy & Security, Developer.',
...
- **Your Models** — the model list with a default-model radio, grouped by where each model runs (On this Mac, Your server, nenva Cloud, API providers), with a line under each heading saying where your data goes. Download local models (built-in llama.cpp engine), point at your own OpenAI-compatible server, add nenva Cloud (free allowance, no key), or **+ Add model** with an OpenAI/Anthropic API key. Per-model Manage panel for keys and options; models that can view images carry a "Reads images" badge. With two or more models, **Which model answers what** appears at the bottom: Email and texts, News, Documents, Portfolio, Routines and tasks, and Small jobs. Chat is not listed — it runs on the model marked Default above. Rows left on "Follows the default model" move with that choice, and this Mac holds one local model at a time, so a second local model is offered only in place of the first.
- **Web Search** — the **Enable web search** master switch, the nenva Connect card (built in — plan, usage, Test), plus Tavily / Brave key cards for direct-to-provider searches.
- **Terminal Access** — turn on the local CLI bridge and install the \`anjadhe\` command for using the assistant from a terminal (appears once the experimental feature is enabled under Developer).
...
Connect or disconnect Google (Gmail, Calendar) per account. Each Mac authorizes independently. **Email preferences** (insights, bundles, sender rules — shared by every account) has its own row here. With the experimental Sharing feature on, a **Contacts** card pairs you with another nenva user by invite code for Portfolio sharing.

## Appearance
Theme (Light, Dark, or System to follow your Mac) and **More apps** — every app as a card with an enable switch (also the "More apps" row at the bottom of the sidebar). An app turned off keeps its data and can still be found by typing its name in Cmd+K; some apps (Portfolio, Wellness, AI Activity, Help, About) start off and are enabled here. Packaged apps (Wellness, Pomodoro, Portfolio, Bookmarks, News) also have an **Uninstall** action: an uninstalled app is not loaded at all (no page, home card, search results or assistant tools), its data stays on the Mac, and it comes back with **Install** from the same page after a reload.
...
## Privacy & Security
- **Lock nenva** — require Touch ID to open the app.
...
## Appearance
Choose a theme and which apps appear in navigation. On macOS, **Keep running in the menu bar** lets routines continue after you close the main window. The menu bar has Open nenva, View updates, Pause scheduled routines and Quit. **Launch at login** is available in the installed app. Your Mac can sleep normally; routines catch up after waking. Quit stops background work.

## Developer
- **Build apps** — turn on the ~/nenva/apps/ folder for coding-agent-built apps. Each installed app is listed with a **Reset Data** button that clears that app's saved data alone (the app stays installed; the reset syncs to other Macs).
- **Experimental features** — per-Mac switches for features that are off by default: **Terminal access** (the \`anjadhe\` command) and **Sharing** (share a Portfolio account with another nenva user). Flipping one reloads the app.
