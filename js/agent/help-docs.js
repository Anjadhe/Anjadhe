...
**Memory.** The assistant keeps a small wiki of memory pages about you — who you are, how to help you, plus pages it starts on its own as new topics come up (a person, a project, a hobby). It files what it learns from chats into the right page and reads a page back when a conversation needs it. Click **memory** in the chat header to browse the pages, edit any of them, or add your own — your edits are kept and never overwritten.
...
A prompt note (**Notes → + New Prompt**) is a reusable instruction — "summarize AI news". Run it manually, or schedule it (daily, weekly…, optionally at a set time) to run in the background on your local model — a **background prompt**. You can also just ask the assistant in chat — "every morning, give me a list of Staff+ engineering jobs" — and it sets up the background prompt for you (you approve it first); daily and weekly schedules can run at a specific time like 8:00 AM. Background runs post to the feed on the home page; open a post and **Discuss with Assistant** starts a chat with the result already in context. Per-prompt options: use personal context, allow web search, or plain offline generation. A feed post and its prompt link to each other: the post's meta line opens the prompt note, and the prompt note shows its recent runs as compact cards. Links inside feed posts (and chat, notes, journal, bookmarks) open in Anjadhe's own browser with a "Back" strip to return where you were.`
        },
