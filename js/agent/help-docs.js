...
A strategy is the plan behind the holdings: what the money is for, when you need it, how big a drop you can sit through, a target mix, and the limits you want to hold yourself to. Open it from the **⋯ menu → Strategy**.
...
Once saved, the app measures your real holdings against it: which sleeves drifted outside their band, which guardrails are broken, what is not covered by the plan, and roughly what it would take to get back. Those numbers are computed by the app, not written by the AI. A card on the main Portfolio view shows the status at a glance, and adding a transaction that puts you off plan says so at the time.

Accounts follow the overall strategy unless you give one its own. That helps when, say, a Roth should be judged differently from a taxable account. Changes always go through the assistant, so the plan stays something you discussed rather than something you quietly edited; the Strategy page keeps a log of how it has changed.`
        },
