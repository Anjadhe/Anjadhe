...
**Writing Voices** — writing styles learned from documents, and the home of the user's document corpus. The app opens on the list of voices; create one by naming it ("My voice", "Barack Obama") — it gets its own folder under ~/Anjadhe/library/ — then add documents on its page (Add documents… or drag-drop; posts, essays, books, speeches; .md, .txt, .pdf, .docx, .html) and press **Study**: the model writes a short, editable style guide plus verbatim passages from those documents. Edits to the guide stick — re-studies never overwrite them — and passages can be pinned. **Draft in this voice…** opens a chat already primed for that voice (it greets the user and asks what to write; the draft names the documents it drew on); asking any chat to write "in X's voice" works too. Deleting a voice keeps its documents. No fine-tuning, no hidden style profile: the guide is a page the user can read and change.

Searching in the app finds passages across ALL documents (grouped by document; clicking one opens the document with matches marked, plus Open original / Show in Finder). Files imported outside any voice, or dropped into the folder in Finder, appear under "Other documents" — searchable grounding material. Deleting a document (Remove on its row, or Delete in the reader) moves the file to the macOS Trash — recoverable, never a hard delete. Semantic (by-meaning) search needs a one-time ~330 MB model download in **Settings → Writing Voices** (which also holds index status, Rescan/Re-index, and the folder location); until then search is keyword-only. Indexing always runs locally whatever chat brain is selected — only the passages retrieved for a question travel with that chat turn.
