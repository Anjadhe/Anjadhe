...
The × on any story means show fewer like this — ranking learns from it, and hidden stories can be restored from the Topics dialog. Stories boosted toward your interests carry a "For you" label with the reason. Clicks, hides, and ranking all stay on this Mac; the model never chooses or writes the headlines you see.`
        ...
        'connected-accounts': {
            title: 'Connected accounts — Email, Calendar, Apple apps, Telegram & iMessage',
            description: 'Connecting Gmail and Google Calendar, email bundles/insights/action items, the calendar lens, importing Apple Reminders, Notes and Calendar, chatting with the assistant from Telegram, and getting this Mac\'s notifications by iMessage.',
...
**Settings → Accounts → Telegram** lets you chat with this Mac's assistant from the Telegram app on any device. You bring your own bot: create one with Telegram's @BotFather (send it \`/newbot\`), paste the token it gives you, then link exactly one chat by sending the pairing code shown on the card to your bot. Messages from that chat — and only that chat — run on this Mac's assistant with your own AI model, and the reply comes back in Telegram. Message text is never written to this Mac's logs.

A separate switch forwards this Mac's notifications — reminders and alerts — to the linked chat, each under an "Anjadhe · Reminder" (or Task, Routine, Email) header so it never reads like a reply. Disconnecting removes the token and the linked chat from this Mac.
...
A **Market Review** routine reviews the portfolio each weekday and posts to the Home feed shortly after the US market closes, at the matching time in your own timezone (4:30 PM Eastern is 1:30 PM in California). It covers the day's result, your best and worst positions, cash, whether you are still on plan, and any headlines that touch your holdings. It runs on your own model with read-only tools, skips weekends, and stays quiet when there is nothing to say. Edit the time or turn it off in the Routines app.`
        ...
        'how-anjadhe-works': {
            ...
            description: 'Where data lives, multi-Mac sync, keyboard shortcuts, Writing Voices, building apps with a coding agent.',
...
**Writing Voices** — learns how the user writes. The app leads with **My voice**: turn it on and pick what it learns from — documents you add, your Notes, your Journal, and your **sent emails** (only the parts you wrote: quoted replies, forwarded messages and signatures are stripped before anything is studied; each source is a switch you can change any time, and nothing is read until you press Study). **Other voices** live behind the door at the bottom of the page, on their own page: sample voices (Mark Twain, Abraham Lincoln — public-domain works, bundled) import with one click, or create one by naming it — it gets its own folder under ~/Anjadhe/library/ — then add that author's documents on its page (Add documents… or drag-drop; posts, essays, books, speeches; .md, .txt, .pdf, .docx, .html). Press **Study**: the model reads a spread sample of the voice's material (more material = a wider sample; **Study depth** at Settings › Reader & Writing Voices sets how much one study reads) and writes a short, editable style guide plus verbatim passages from that material. Edits to the guide stick — re-studies never overwrite them — and passages can be pinned. **Draft in this voice…** opens a chat already primed for that voice (it greets the user and asks what to write; the draft names the documents it drew on); asking any chat to write "in X's voice" works too. A digest ROUTINE can carry a voice for its posts (the Voice picker on the routine's form, or voice: "<name>" on create_routine) — default is the assistant's own voice, and a routine keeps running in it if its voice is deleted. Deleting a voice keeps its documents. No fine-tuning, no hidden style profile: the guide is a page the user can read and change. Every document is also visible in the Reader app — the two are lenses over one folder.
...
Prefer a terminal? Turn on **Build Apps** in **Settings**. Anjadhe creates ~/Anjadhe/apps/ — one subfolder per app, plus CLAUDE.md / AGENTS.md holding the full contract (manifest format, the Anjadhe SDK, worked examples) and .anjadhe-schemas.json describing built-in data shapes. Start your coding agent in that folder and describe the app; the agent picks up the contract on its own. Anjadhe watches the folder — changes reload live, and errors are written to .errors.log inside the app's folder so the agent can read and fix them.`
        ...
        'license': {
            ...
            description: 'Pricing (one-time license, about $79, a year of updates, no account), the alpha license everyone who installed during the alpha keeps for good, how to claim or enter a key, what a license does and does not do.',
            actions: ['license', 'setup-checklist'],
            content: `## What Anjadhe costs

Anjadhe is free during the closed alpha. The plan after it is a **one-time license of about $79** with a year of updates, then paid upgrades if you want later years; there is no subscription for the app and no account. **Everyone who installs during the alpha keeps the app free for good.** Hosted extras (web search through Anjadhe Connect, Anjadhe Cloud models) keep a free monthly allowance; only usage beyond it would ever be paid, and that stays optional.
...
## Appearance
Theme (Light, Dark, or System to follow your Mac) and **More apps** — every app as a card with an enable switch (also the "More apps" row at the bottom of the sidebar). An app turned off keeps its data and can still be found by typing its name in Cmd+K; some reference apps (Wellness, Web Browser, AI Activity, Help, About) start off and are enabled here. Packaged apps (Wellness, Pomodoro, Portfolio, Bookmarks, News) also have an **Uninstall** action: an uninstalled app is not loaded at all (no page, home card, search results or assistant tools), its data stays on the Mac, and it comes back with **Install** from the same page after a reload.
