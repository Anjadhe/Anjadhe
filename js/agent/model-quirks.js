...
const ModelQuirks = {
    ...
    /**
     * A tool call written as TEXT. Local models are trained on a textual
     * tool-call syntax (Qwen/Hermes `<tool_call>…</tool_call>`, Qwen3-Coder
     * `<function=name><parameter=…>`), which the engine parses into
     * structured tool_calls only when tools were offered. On a tools-free
     * turn (the synthesis pass at the tool limit) the model can still reach
     * for one, and the markup came back as the "answer" — a routine posted
     * a bare `<tool_call><function=read_url>…` to the feed. Nothing ran;
     * the markup is never something to show.
     *
     * Returns the content with every such block removed (an unterminated
     * block runs to the end) and `leaked` saying whether any was found.
     */
    stripLeakedToolCalls(content) {
        const text = String(content || '');
        if (!/<\/?tool_call>|<function=|<\|tool_call/i.test(text)) return { text, leaked: false };
        const out = text
...
if (typeof module !== 'undefined' && module.exports) module.exports = ModelQuirks;
