 ...
 *    regenerates once a day at most. Metered brains (own key / nenva
...
const PromptSuggestions = {
    ...
    // Every question below is asked of the model that answers THIS work,
    // not of the brain (js/agent/model-routing.js R4).
    _entry() {
    ...
    _allows(cls) {
        try {
            ...
            if (!CloudPrivacy.leavesFor(this.SOURCE)) return true;
            return CloudPrivacy.allowsFor(cls, this.SOURCE);
        } catch { return false; }
    },
    ...
    _ttl() {
        try { return (AgentService.isMeteredFor?.(this.SOURCE) ?? AgentService.isMeteredBrain?.()) ? this.METERED_TTL_MS : this.TTL_MS; }
    ...
    /**
     * Off by default since 2026-09-20 (Ram: "not really helpful"). The flag
     * gates the AI half ONLY — with it off nothing here runs, `current()`
     * is empty, and both surfaces fall back to AgentUI's static pool, which
     * is exactly what they showed before this was built on 2026-09-10.
     *
     * Self-gated rather than gated at the two call sites, so a stale cache
     * written while the flag was on cannot keep feeding the pills after it
     * is turned off.
     */
    enabled() {
        return typeof FEATURES === 'undefined' || FEATURES.isEnabled('aisuggestions');
    },
    ...
    _systemPrompt() {
        return `You write the "Try asking…" suggestions inside nenva, a private personal-assistant app on the user's Mac. The user sees them as buttons under the chat box and clicks one to send it.
...
- Return ONLY JSON: {"prompts": ["…", "…"]}`;
    ...
    _inputs() {
        ...
        // Whether the SHEET leaves this Mac — the model that writes the
        // suggestions is the one that reads it (R4), which is not always
        // the brain named above.
        try { out.brainLeaves = typeof CloudPrivacy !== 'undefined' && CloudPrivacy.leavesFor(this.SOURCE); } catch { out.brainLeaves = false; }
        ...
        try {
            ...
            out.recentChats = (AgentService.conversations || [])
                .filter(c => c && !c.private && c.title && c.title !== 'New chat' && Date.parse(c.updatedAt || 0) >= weekAgo && (c.messages || []).length)
