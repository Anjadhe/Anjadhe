...
const Breadcrumb = {
    ...
    appLabels: {
        ...
        calendar: 'Calendar', portfolio: 'Portfolio', wellness: 'Wellness',
...
const AppManager = {
    ...
    async init() {
        ...
        // Start the background-prompt scheduler (runs globally; catches up
        ...
        // Home's right-hand Discover pane — AI-curated links for the user's
        // chosen topics. No-ops until topics are picked and a model exists.
        if (typeof DiscoverPane !== 'undefined') DiscoverPane.init();
    ...
    showSetup() {
        ...
        // tiers per-RAM defaults (16→qwen3.5:9b, 32→gemma4:12b, 48→qwen3.6:27b,
        // 64→qwen3.6:35b-a3b, 96→gpt-oss:120b). Caps first-run downloads at
        ...
        const showStep2c = (totalMemGB) => {
            ...
            if (intro && totalMemGB) {
                intro.textContent = `This Mac has ${totalMemGB} GB of memory, which is too little to run the AI on its own. Two ways to add a brain right here: run the model on another computer you own — a desktop or a home server — and connect it, or bring your own OpenAI or Anthropic API key, if that's a service you trust with what you send it. Anjadhe sends your data only where you point it. Or continue without AI — everything else works.`;
            }
        };
        const showStep2b = () => {
        ...
        const goToStep1 = () => {
            ...
            document.getElementById('setup-step-web').style.display = 'none';
        ...
        const goToStep2 = async () => {
            ...
            document.getElementById('setup-step-ind-web').classList.remove('active');
        ...
        const goToStepWeb = () => {
            document.getElementById('setup-step-1').style.display = 'none';
            document.getElementById('setup-step-2').style.display = 'none';
            document.getElementById('setup-step-web').style.display = '';
            document.getElementById('setup-step-3').style.display = 'none';
            document.getElementById('setup-step-name').style.display = 'none';
            const steps = document.getElementById('setup-steps');
            if (steps) steps.style.visibility = 'visible';
            document.getElementById('setup-step-ind-1').classList.remove('active');
            document.getElementById('setup-step-ind-web').classList.add('active');
            document.getElementById('setup-step-ind-2').classList.remove('active');
            document.getElementById('setup-step-ind-name').classList.remove('active');
        ...
        // Web access → Your data (step "3. Storage"). Local by default;
        // multi-Mac sync (the encrypted iCloud journal) is the opt-in here.
        const goToStep3 = () => {
            document.getElementById('setup-step-1').style.display = 'none';
            document.getElementById('setup-step-2').style.display = 'none';
            document.getElementById('setup-step-web').style.display = 'none';
            ...
            // iCloud Drive means an existing Anjadhe setup — surface it and
            // make joining the prominent choice. Fire-and-forget; the step
            // works unchanged if the lookup fails or finds nothing.
            (async () => {
                try {
                    ...
                    found.textContent = `Found Anjadhe from another Mac in your iCloud Drive: ${names}. Enable sync to pull that data in, or start fresh.`;
                    found.style.display = '';
                    // Swap prominence: joining becomes the primary action.
                    const keepBtn = document.getElementById('setup-keep-local-btn');
                    const syncBtn = document.getElementById('setup-enable-sync-btn');
                    syncBtn.classList.replace('secondary-btn', 'primary-btn');
                    keepBtn.classList.replace('primary-btn', 'secondary-btn');
                    keepBtn.textContent = 'Start fresh — just this Mac';
        ...
        document.getElementById('setup-enable-sync-btn').addEventListener('click', async () => {
            const btn = document.getElementById('setup-enable-sync-btn');
            btn.disabled = true;
            btn.textContent = 'Turning on sync...';
            await applySetupBackupChoice();
            try {
                const result = await window.electronSync?.setEnabled?.(true);
                if (result?.locked) {
                    // Peers use a passphrase-protected key — merging waits
                    // until it's entered. Say so before the reload eats it.
                    UIUtils.showToast('Sync is on — enter your sync passphrase in Settings › Storage & Backup to pull in your data', 'info');
                    await new Promise(r => setTimeout(r, 2200));
                }
            } catch (e) {
                console.warn('[setup] enable sync failed:', e);
            }
            goToStepName();
        });
        ...
        document.getElementById('setup-skip-model-btn').addEventListener('click', goToStepWeb);
        ...
        if (setupSkipAiBtn) {
            setupSkipAiBtn.addEventListener('click', goToStepWeb);
        }

        // ── Alternate brains: server / BYOK (collapsed under the local
        // recommendation on 2b; expanded on the low-RAM panel 2c). Reuses
        // the Settings add-model steps so validation, key storage, and
        // entry creation stay one code path. Adding a model counts as the
        // assistant set up — the wizard moves on to the web-access step.
        const altToggle = document.getElementById('setup-alt-toggle');
        if (altToggle) {
            altToggle.addEventListener('click', () => {
                const altBody = document.getElementById('setup-alt-body');
        ...
        document.querySelectorAll('.setup-alt-open').forEach((btn) => {
            btn.addEventListener('click', () => {
                ...
                // The step's Back link leads to the Settings engine chooser —
                // wrong place mid-wizard.
                body.querySelector('.settings-add-model-back')?.remove();
            });
        });

        // ── Step web: web-access opt-in ──────────────────────────────────
        // Enable stores the explicit provider choice; the Connect key itself
        // still mints lazily on the first search. Skip stores nothing — web
        // search stays off until enabled in Settings.
        document.getElementById('setup-enable-web-btn').addEventListener('click', async () => {
            try { await window.electronSearch?.setProvider?.('anjadhe'); }
            catch (e) { console.warn('[setup] enable web search failed:', e); }
            goToStep3();
        });
        document.getElementById('setup-skip-web-btn').addEventListener('click', async () => {
            // Skip means OFF — including when the user backtracked after
            // clicking Enable. Clearing the choice keeps the two buttons
            // truthful on a re-visit (fresh installs hold no search keys,
            // so no provider falls back in).
            try { await window.electronSearch?.setProvider?.(null); }
            catch (e) { console.warn('[setup] clearing web search failed:', e); }
            goToStep3();
        });
    },
    ...
    // ret: { label: 'Back to Feed', onBack: () => … } — optional.
    _browseReturn: null,
    ...
    openInBrowse(url, ret) {
        ...
        setTimeout(() => {
            try {
                if (BrowseApp._createTab) BrowseApp._createTab({ url, activate: true });
    ...
    openApp(appName, updateHash = true) {
        // The standalone Tasks and Goals pages were absorbed by the Actions
        // hub (Tasks tab / Plan's goal detail). Old hashes and stragglers
        // land on the hub instead of the retired list pages. ScheduleApp and
        // GoalsApp live on as the data layer + full-page editors only.
        if (appName === 'schedule') return this.openApp('actions', updateHash);
        if (appName === 'goals') return this.openApp('focus', updateHash);
    ...
    showDashboard(updateHash = true) {
        ...
        if (dashboard) {
            ...
            if (typeof DiscoverPane !== 'undefined') DiscoverPane.render();
            this.updateStats();
            this.applyHiddenApps();
            this.renderFavoriteApps();
    ...
    setupDashboardTabs() {
        ...
        // Manual minimize toggle — the user's persisted choice, independent
        // of the width-gated auto-collapse (body.nav-minimized applies on
        // every page at every width; see core.css).
        const applyNavMin = (min) => {
            document.body.classList.toggle('nav-minimized', !!min);
            const btn = document.getElementById('dash-nav-min-btn');
            if (btn) {
                btn.title = min ? 'Expand sidebar' : 'Minimize sidebar';
                btn.setAttribute('aria-label', btn.title);
            }
        };
        applyNavMin(StorageManager.get('nav-minimized') === true);
        document.getElementById('dash-nav-min-btn')?.addEventListener('click', () => {
            const min = !document.body.classList.contains('nav-minimized');
            StorageManager.set('nav-minimized', min);
            applyNavMin(min);
            // Minimizing happens with the pointer on the rail, i.e. mid-peek —
            // drop the peek so the rail visibly shrinks right away instead of
            // waiting for the mouse to leave. (No re-peek until the next
            // mouseenter, so the animation reads as one clean collapse.)
            if (min) nav?.classList.remove('is-peek');
        });
    },
