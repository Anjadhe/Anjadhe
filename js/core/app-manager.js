...
const Breadcrumb = {
    ...
    appLabels: {
        actions: 'Actions',
        goals: 'Goals', fyi: 'Email AI', schedule: 'Tasks', notes: 'Notes',
        bookmarks: 'Bookmarks', journal: 'Journal', email: 'Inbox',
...
const AppManager = {
    ...
    showSetup() {
        ...
        const DOWNLOAD_TIPS = [
            ...
            { label: 'Routines', text: 'Ask for a news digest every weekday at 7am, or a weekly look at your goals — Anjadhe runs it on its own and posts the answer on your home page.' },
            { label: 'One shortcut for everything', text: '⌘K searches across your tasks, notes, journal, goals and bookmarks, and opens any app by name.' },
            { label: 'Goals, in conversation', text: 'Tell the assistant what you want to achieve; it works through it with you, breaks it into tasks, and checks in every week.' },
        ...
        document.querySelectorAll('.dash-apps-section .dash-apps-group').forEach(group => {
            ...
            const tiles = Array.from(group.querySelectorAll('.dash-app-tile[data-app]'))
                .filter(t => {
                    ...
                    // offering a checkbox for it would promise a control
                    // that governs nothing.
                    if (t.dataset.subappOf) return false;
            ...
            tiles.forEach(t => {
                ...
                row.innerHTML =
                    ...
                    ` aria-label="Show this app in launchers">`;
     ...
     * a way back: the left slot holds Home whenever you are not already
     * there, and nothing when you are.
     *
    ...
    updateSidebarActive(appName) {
        ...
        const homeBtn = document.getElementById('titlebar-home-btn');
        if (homeBtn) homeBtn.hidden = atHome;
    },
    ...
    setupDashboardTabs() {
        ...
        // "home" to mean there. The left slot repeats Home in words while
        // you are inside an app (updateSidebarActive hides it at home) —
        ...
        document.getElementById('titlebar-home-btn')?.addEventListener('click', () => this.showDashboard());
     ...
     * a task run or a Maker build was in the middle of.
     *
        ...
        const buttons = [
            {
                text: 'Reload anyway',
                className: 'secondary-btn',
                onClick: () => { modal.close(); window.location.reload(); }
            }
        ];
        if (syncOn) {
            buttons.push({
