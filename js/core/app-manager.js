...
const Breadcrumb = {
    ...
    appLabels: {
        actions: 'Tasks',
        goals: 'Projects', fyi: 'Insights', schedule: 'Tasks', notes: 'Notes',
        email: 'Inbox',
        calendar: 'Calendar',
...
const AppManager = {
    ...
    async init() {
        ...
        if (typeof FEATURES !== 'undefined') {
            ...
            // Mac<->Mac sharing (docs/PORTFOLIO_SHARING.md): the same
            // bargain — the peer channel starts only from here, flag on.
            if (FEATURES.isEnabled('sharing') && window.electronPeers?.ensure) {
                window.electronPeers.ensure();
            }
            // Terminal access (the `anjadhe` CLI): same bargain — main's
            // loopback server starts only when asked from here, and only
            // with the `terminal` flag on (2026-09-09).
            if (FEATURES.isEnabled('terminal') && window.electronCLI?.ensure) {
                window.electronCLI.ensure();
            } else if (window.electronCLI?.suspend) {
                // Flag off: take down a server an earlier reload started,
                // keeping the user's setting for when the flag returns.
                window.electronCLI.suspend();
            }
        }
        ...
        // Dynamic views must exist before routing restores their hash. The
        // execution runtime still initializes through its normal lifecycle.
        if (typeof WorkroomsApp !== 'undefined' && WorkroomsApp.active()) WorkroomsApp.mount();
        ...
        if (typeof WorkroomsApp !== 'undefined' && WorkroomsApp.active()) WorkroomsApp.init();
        ...
        // Noticing P0 (docs/NOTICING.md), flag-gated and off by default: the
        // pass over settled journal entries. Started here for the D5 reason
        // like every other engine — and nothing it produces is rendered, by
        // design, until a week of real moments has been read.
        if (typeof Noticing !== 'undefined' && typeof FEATURES !== 'undefined'
            && FEATURES.isEnabled('noticing')) {
            Noticing.init();
        }

        // The hourly "Try asking…" writer (js/agent/prompt-suggestions.js).
        // Started here for the same reason: its liveness must not depend
        // on Home or the Assistant being visited.
        if (typeof PromptSuggestions !== 'undefined') {
            PromptSuggestions.init();
            // The repaint hook goes in now, not on the first home paint: a
            // run that finishes before Home is ever drawn must still land.
            if (typeof AgentUI !== 'undefined') AgentUI._watchSuggestionChanges?.();
        }

        // A deep link that arrived before this renderer was ready (a cold
        // launch from the Chrome extension) waits in main until now.
     ...
     */
    /**
     * The welcome's two beats (2026-09-20, by request): "Welcome." lands,
     * then the name — one serif line, the deck's title-slide device. Driven from here and not from a keyframe, because a
     * hidden first frame with `forwards` hangs an element's visibility on
     * the document timeline — which STOPS while the window is occluded,
     * and the title would never arrive (the animation rule in core.css,
     * and the bug that once rendered this whole wizard invisible).
     *
     * The stylesheet keeps both visible; this only holds the title back for
     * a beat. So no JS, a thrown timer or a stalled compositor all leave
     * the row readable, and a reduced-motion Mac skips the wait entirely.
     */
    _playWelcomeIntro() {
        const title = document.querySelector('#setup-step-1 .setup-hero-name');
        if (!title) return;
        try {
            if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
        } catch { return; }
        title.classList.add('is-waiting');
        setTimeout(() => title.classList.remove('is-waiting'), 600);
    },

    showSetup() {
        this._clearInitialLoader();
        this._playWelcomeIntro();
        ...
        // ── The brain: nenva Cloud by default (2026-09-18) ──────────────
        // The model step left the wizard by request: every new install
        // starts on nenva Cloud, and a model on this Mac, a server or a
        // key is a Settings › AI Assistant choice (docs/POSITIONING.md
        // "Decided (2026-09-18)"). The operator's catalog decides WHICH
        // nenva Cloud model (its first entry — the lineup is managed data
        // on Connect); an unreachable catalog falls back to the launch
        // model, the same fallback Settings' add step uses. Fetched now so
        // Finish never waits on the network; nothing reaches the service
        // until the model is first used (the Connect key mints then).
        const cloudModelsPromise = (async () => {
            try { return await window.electronLLM?.anjadheModels?.(); } catch { return null; }
        })();
        const addDefaultBrain = async () => {
            try {
                ...
                const first = res && Array.isArray(res.models) && res.models[0];
                const model = (first && first.id) || 'anjadhe-cloud';
                const label = (first && first.label) || 'nenva Cloud';
                const entry = AgentService.addEntry({ engine: 'anjadhe', model, label });
                // saveModelList's brain write-through is fire-and-forget,
                // and finishSetup reloads right after this — await it so
                // provider-routed work (email insights, routines) boots on
                // the brain rather than the 'local' default.
                if (entry) await AgentService._syncBrainToEntry(entry);
                if (typeof AnalyticsManager !== 'undefined') {
                    AnalyticsManager.record('model.added', { engine: 'anjadhe', source: 'default' });
                }
            } catch (err) {
                console.warn('[setup] default nenva Cloud entry failed:', err);
            }
        };
        ...
        // chip never paints at its old spot for a frame) and deferred (in
        // case the row to land in is not rendered yet).
        const placeBack = () => {
        ...
        // Welcome → Your data (step "Storage"). Reveal the step indicator
        // here — it's hidden on the story screen so the first impression is
        // the pitch, not wizard chrome.
        const goToStep3 = () => {
            ...
            document.getElementById('setup-step-ind-2').classList.remove('done');
            document.getElementById('setup-step-ind-name').classList.remove('done');
            setBack(goToStep1);
            ...
            // iCloud Drive means an existing nenva setup — say so, and
            ...
            (async () => {
                try {
                    ...
                    found.textContent = `Found nenva from another Mac in your iCloud Drive: ${names}. To pull that data in, turn on sync in Settings › Storage & Backup after setup.`;
                    found.style.display = '';
        ...
        const goToStepName = () => {
            ...
            document.getElementById('setup-step-ind-2').classList.add('done');
            document.getElementById('setup-step-ind-name').classList.remove('done');
            setBack(goToStep3);
    ...
    _wireUserAppsRow() {
        ...
        row.addEventListener('click', (e) => {
            ...
            if ((e.metaKey || e.ctrlKey) && window.electronWindow?.openNew) {
                ...
                this.openAppFromLauncher(tile.dataset.app);
            }
        });
    },
    ...
    openApp(appName, updateHash = true) {
        ...
        if (appView) {
            ...
            if (typeof AnalyticsManager !== 'undefined') {
                // A USER app's id is a name its author typed (~/Anjadhe/apps),
                // so sending it verbatim would put the user's own words in an
                // analytics payload. The opt-in card promises none of what
                // they wrote leaves; the fold is what makes that true. We
                // still learn that a user app was opened.
                const isUserApp = Object.values(this._userAppsByDir || {}).includes(appName);
                AnalyticsManager.record('app.opened', { app: isUserApp ? 'userapp' : appName });
            }
        }
    },
    ...
    DEFAULT_HIDDEN_APPS: ['aiactivity', 'help', 'about'],
    ...
    openAppVisibilityModal(origin = 'dashboard') {
        ...
        list.onclick = (e) => {
            ...
            ref = Modal.create({
                ...
                content: `<p>${esc(label)} will be removed from nenva: its page, home card, search results and assistant tools go away, and apps that work with it carry on without it.</p>` +
    ...
    _settingsCategoryForApp: {
        agent: 'ai',
        calendar: 'accounts',
        // Portfolio has its own panel (2026-09-22). Spending still
        // configures one thing, the Linked institutions card on Accounts.
        portfolio: 'portfolio',
        spending: 'accounts',
        // Sharing's door from Portfolio (docs/PORTFOLIO_SHARING.md).
        'portfolio-sharing': 'accounts',
    },
    ...
    // the eye finds it. Both money apps share the Linked institutions card.
    _settingsAnchorForApp: {
        spending: { head: 'settings-brokerages-head', card: 'settings-brokerages-group' },
        'portfolio-sharing': { head: 'settings-contacts-head', card: 'settings-contacts-group' },
    },
     ...
     */
    setupMenuActions() {
        if (window.electronMenu.onDeepLink) {
            window.electronMenu.onDeepLink(() => this._consumeDeepLink());
        }
    ...
    /**
     * anjadhe:// deep links. Main holds the parsed link; this asks for it and
     * routes it. Called from the live ping and once at the end of init, and
     * main clears the link on the first ask, so a link opens exactly once.
     */
    async _consumeDeepLink() {
        if (!window.electronMenu?.consumeDeepLink) return;
        let link = null;
        try { link = await window.electronMenu.consumeDeepLink(); } catch { return; }
        if (link) this.handleDeepLink(link);
    },

    /**
     * @param {{action:'read', url:string, title?:string}} link — already
     *   validated by js/main/deep-link.js (http(s) only).
     */
    handleDeepLink(link) {
        if (!link || link.action !== 'read') return;
        // News is the reader: fetched on this Mac, summarized by the brain,
        // and when the page will not load, coverage from other sources
        // instead. An uninstalled News package means no reader — say so
        // rather than silently handing the link back to the browser it came
        // from, which is not what the extension promised.
        if (typeof NewsApp !== 'undefined' && typeof NewsApp.openUrl === 'function') {
            NewsApp.openUrl(link.url, { title: link.title || '' });
            return;
        }
        if (typeof UIUtils !== 'undefined' && UIUtils.showToast) {
            UIUtils.showToast('Install News (More apps) to read links sent from your browser.', 'info');
        }
    },
    ...
    async requestReload() {
        ...
        const content = `
            ...
            ${syncOn ? '<p class="settings-hint">Refreshing is also how nenva picks up changes from your other Macs. You can do just that without stopping anything.</p>' : ''}`;
    ...
    updateStats() {
        ...
        if (this.currentApp !== null && this.currentApp !== 'updates') return;
    ...
    /**
     * Is it time to nudge about journalling? ONE rule, two surfaces: the
     * classic home's strip below and the Simple home's shortcuts row
     * (SimpleExperience.render) — a second copy of "more than two days"
     * would drift, and the two homes would disagree about the same fact.
     *
     * Not pure: with no entries at all it STAMPS a first-seen date, so a
     * clean install doesn't immediately claim you haven't journaled in a
     * while. Both callers want that stamp.
     */
    journalNudgeDue() {
        // Journal is a package (js/apps/journal/): with it uninstalled the
        // nudge would point at a door that no longer exists.
        if (!this.apps.journal) return false;
        ...
        return (new Date() - new Date(anchor)) / (1000 * 60 * 60 * 24) > 2;
    },
    ...
    isAppLocked(appName) {
        ...
        // History is another view of the assistant's saved conversations.
        if (appName === 'conversations' || appName === 'workrooms') appName = 'agent';
        ...
        const checkAndNotify = () => {
            ...
            for (const item of items) {
                ...
                if (item.reminderDaysBefore?.length && item.scheduledDate && item.lastCompletedDate !== today && !oneTimeAbandoned) {
                    ...
                    if (item.reminderDaysBefore.includes(daysUntilDue)) {
                        ...
                        if (currentTime === '09:00' && !notifiedSet.has(reminderKey)) {
                            ...
                            if (tr) when += ` at ${tr}`;
                            Notify.show(item.title, reminderBody(item, when), { kind: 'reminder' });
                        }
                    }
                }
                ...
                if (notifyBefore > 0 && nowMinutes === notifyAt && !notifiedSet.has(item.id + '_early')) {
                    ...
                    Notify.show(item.title, reminderBody(item, `Starting in ${notifyBefore} min, today at ${timeRange(item)}`), { kind: 'reminder' });
                }
                ...
                if (item.startTime === currentTime && !notifiedSet.has(item.id)) {
                    ...
                    Notify.show(item.title, reminderBody(item, `Starting now, today at ${timeRange(item)}`), { kind: 'reminder' });
                }
            }
        };
