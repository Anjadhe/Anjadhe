...
const AppManager = {
    ...
    showSetup() {
        ...
        // tiers per-RAM defaults (32→gemma4:12b, 48→qwen3.6:27b,
        // 64→qwen3.6:35b-a3b, 96→gpt-oss:120b — nothing below 32: small
        // local models didn't work well, so sub-32 GB Macs get the low-RAM
        // step). Caps first-run downloads at
        ...
        const showStep2c = (totalMemGB) => {
            ...
            if (intro && totalMemGB) {
                intro.textContent = `A large language model is your assistant's brain, and running one well on the Mac itself takes 32 GB of memory or more — this Mac has ${totalMemGB} GB. For a Mac this size, Anjadhe suggests Anjadhe Cloud:`;
            }
        };
        const showStep2b = () => {
    ...
    updateSidebarActive(appName) {
        ...
        if (homeIcon) homeIcon.hidden = !toHome;
        if (arrowIcon) arrowIcon.hidden = toHome;
