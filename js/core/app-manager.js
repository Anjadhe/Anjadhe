...
const AppManager = {
    ...
    showSetup() {
        ...
        document.getElementById('setup-skip-web-btn').addEventListener('click', async () => {
            ...
            // clicking Enable. The explicit disable + cleared choice keep
            // the two buttons truthful on a re-visit (fresh installs hold
            // no search keys, so no provider falls back in).
            try {
                await window.electronSearch?.setEnabled?.(false);
                await window.electronSearch?.setProvider?.(null);
