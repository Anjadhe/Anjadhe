...
const NetworkLogger = {
    ...
    /**
     * Patch the global fetch too. Electron's fetch rides undici, NOT the
     * http/https modules, so every fetch()-based caller — Anjadhe Connect
     * (keys/news/usage), direct Google News RSS, Open-Meteo weather — was
     * invisible to this log until this wrapper.
     */
    _patchFetch() {
        const orig = globalThis.fetch;
        if (typeof orig !== 'function' || orig.__anjadheNetLog) return;
        const self = this;
        const wrapped = async function (input, init) {
            ...
            try {
                ...
                self._recordFetch(url, method, start, res.status, res.headers, null);
                return res;
            } catch (e) {
                self._recordFetch(url, method, start, null, null, (e && e.message) || 'fetch failed');
                throw e;
            }
        };
        wrapped.__anjadheNetLog = true;
        globalThis.fetch = wrapped;
    },

    // Same metadata-only rules as every other capture layer: no query
    // strings, no headers, no bodies. Duration runs to headers-received
    // (undici streams the body after fetch() resolves).
    _recordFetch(url, method, start, status, headers, error) {
        try {
            const u = new URL(url);
            if (u.protocol !== 'http:' && u.protocol !== 'https:') return;
            const len = headers && headers.get ? Number(headers.get('content-length')) : NaN;
            this._add(this._newEntry({
                ...
                resBytes: Number.isFinite(len) ? len : null,
                error: error || null
            }));
    ...
    _classify(host) {
        ...
        if (h === 'api.anjadhe.com') return 'Anjadhe Connect';
        if (h.includes('news.google.com')) return 'News feeds';
        if (h.includes('open-meteo.com')) return 'Weather';
