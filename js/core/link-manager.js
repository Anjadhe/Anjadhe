 ...
 * Bookmarks, and any app package that registers (registerApp). (Focus areas were retired 2026-07-31 — goals
...
const LinkManager = {
    ...
    // App packages that take links register here (docs/PLATFORM.md "App
    // packages"); the built-in cases below are the legacy of listing each
    // app by name. def: { label, plural, getItemMeta(id), getAppItems(),
    // renderMeta(item) → text, open(id) }.
    _apps: {},
    // Optional: createLabel + createNew(ctx, origin) put a "+ New …" button
    // on the linked-items section (Notes/Tasks have theirs built in).
    registerApp(app, def) {
        if (!app || !def || typeof def.getItemMeta !== 'function' || typeof def.getAppItems !== 'function') return false;
        this._apps[app] = def;
        return true;
    },
    unregisterApp(app) { delete this._apps[app]; },
    /** Built-in link targets plus every registered package. */
    linkableApps() { return ['goals', 'schedule', 'notes', ...Object.keys(this._apps)]; },
    ...
    getItemMeta(app, itemId) {
        switch (app) {
            ...
            default:
                return this._apps[app] ? this._apps[app].getItemMeta(itemId) : null;
        }
    },
    ...
    getAppItems(app) {
        ...
        switch (app) {
            ...
            default:
                return this._apps[app] ? (this._apps[app].getAppItems() || []) : [];
        }
    }
};
