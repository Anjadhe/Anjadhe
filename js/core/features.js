...
const FEATURE_DEFAULTS = Object.freeze({
    ...
    maker: false,
    // Off by default (2026-08-07): the Library — imported original content +
    // the local semantic index + search (docs/LIBRARY.md L1). Ships hidden
    // while the index and the Voice layer (L2) mature; turning it on adds
    // the app, downloads the ~330 MB local embedding model on first index,
    // and ships the search_library/read_library_doc assistant tools (cut in
    // agent-tools.js when off, same pattern as maker).
    library: false,
