...
const Notify = {
     ...
     * { kind: 'reminder'|'task'|'routine'|'email'|'message' } — names what the
     ...
     * reading like an assistant reply, or like a stray text from your own
     * number (the macOS banner needs no such header: the app icon already
     * says who is talking). No app name anywhere in it — the bot and the
     * handle already say who is sending. Give `body` a real summary: the
    ...
    show(title, body, opts = {}) {
        const b = String(body || '');
        // A focus session holds what is not time-critical (email insights,
        // routine results) and releases it as one summary when the focus
        // phase ends — js/core/focus-mode.js. Reminders always pass.
        if (typeof FocusMode !== 'undefined' && FocusMode.hold(opts.kind || '', title, b, opts)) return;
        try {
