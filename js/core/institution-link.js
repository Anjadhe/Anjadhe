 ...
 * links a financial institution through nenva Connect (Portfolio for
...
const InstitutionLink = {
    ...
    failMessage(r, fallback) {
        const code = r?.code;
        if (code === 'disabled') return 'Account linking is not enabled on nenva Connect yet.';
        if (code === 'capacity') return 'nenva Connect is at its account-linking capacity right now. Try again later.';
        ...
        if (code === 'connect') return 'nenva Connect could not be reached right now.';
