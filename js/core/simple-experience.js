...
const SimpleExperience = {
    ...
    init() {
        ...
        document.addEventListener('click', e => {
            ...
            if (simple === 'updates') AppManager.openApp('updates');
            ...
            if (simple === 'settings') AppManager.openApp('settings');
            ...
            if (simple === 'launch' && this.shortcuts().some(app => app.id === id)) AppManager.openApp(id);
            if (simple === 'journal' && AppManager.apps.journal) AppManager.openApp('journal');
            if (simple === 'agenda') AppManager.openApp(id === 'calendar' ? 'calendar' : 'actions');
    ...
    routineUpdates(limit = 3) {
        ...
        return series
            // "From Anjadhe" announcements are not something a routine did.
    ...
    showHistory() {
        AppManager.openApp('conversations');
    },
