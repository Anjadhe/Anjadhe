...
const Noticing = {
    ...
    // The LLMLogger tag every pass carries — also how CloudPrivacy and the
    // metered gate find the model that answers this work (model-routing.js).
    SOURCE: 'noticing',
    ...
    blockedReason() {
        ...
        // Asked of the model that answers THIS pass, not of the brain
        // (model-routing.js R4) — the assistant surface may be pointed
        // somewhere other than where chat runs.
        if (AgentService.isMeteredFor?.(this.SOURCE) ?? AgentService.isMeteredBrain?.()) return 'metered brain';
        ...
        if (typeof CloudPrivacy !== 'undefined' && !CloudPrivacy.allowsFor('journal', this.SOURCE)) {
            return 'journal may not leave this Mac with the model that would read it';
        }
        return null;
    },
