 ...
 * run. A page reload throws all of it away mid-flight, and
...
const BackgroundWork = {
    ...
    _sources: [
        ...
        // An open private chat with something in it. It is stored nowhere
        // (js/agent/private-chat.js P1), so a reload or a closed window
        // ends it for good — say so before Cmd+R does.
        () => {
            if (typeof AgentService === 'undefined' || !AgentService.isPrivateConversation) return null;
            const id = AgentService.activeConversationId;
            if (!AgentService.isPrivateConversation(id)) return null;
            const conv = AgentService.conversations.find(c => c.id === id);
            if (!conv || !(conv.messages || []).length) return null;
            return { id: 'private-chat', label: 'Your private chat, which is not saved anywhere', resumable: false };
        },
        ...
        () => {
            ...
            // Name it: "A routine is running" left the user guessing which
            // of thirty (2026-09-04).
            const title = PromptFeed._current && PromptFeed._current.title;
            const what = title ? `The routine “${title}” is running` : 'A routine is running';
            return {
                id: 'routine-run',
                label: queued ? `${what} (${queued} queued)` : what,
                resumable: false,
            };
        },

        // A voice study reading the user's Library (one model call, started
        // by the user's own click on the Library page).
        // Daily business profiles for the portfolio's tickers. Resumable:
        // each finished profile is saved as it lands, and the sweep restarts
        // on the next Portfolio or home visit.
        () => {
            if (typeof PortfolioProfile === 'undefined') return null;
            const s = PortfolioProfile.sweeping();
            if (!s) return null;
            return {
                id: 'portfolio-profiles',
                label: s.left > 1 ? `Writing today’s stock profiles (${s.left} left)` : 'Writing today’s stock profile',
                resumable: true,
            };
        },
        // Today's market-and-portfolio brief. NOT resumable, for the same
        // reason routine-run above is not: nothing re-runs it after a reload.
        // It is asked for again only when the user next lands on Portfolio or
        // home, and the call in flight — one long generation over the whole
        // book — is thrown away. Marked resumable until 2026-09-22, which put
        // it in neither losable() nor pausable(): Cmd+R and closing the window
        // both went through in silence while it was writing. Unlike the
        // profile sweep beside it there is no partial progress to keep, so
        // "would stop" is the true sentence, not "would pause".
        () => {
            if (typeof PortfolioBrief === 'undefined' || !PortfolioBrief.writing()) return null;
            return { id: 'portfolio-brief', label: 'Writing today’s portfolio brief', resumable: false };
        },
        // CNBC's current-headlines summary. Not resumable, by the brief's
        // test: nothing re-runs it without the user — the ten-minute clock
        // only ticks while the section is on screen.
        () => {
            if (typeof PortfolioHeadlines === 'undefined') return null;
            const w = PortfolioHeadlines.writing();
            return w ? { id: 'portfolio-headlines', label: `Summarizing current headlines (${w.current})`, resumable: false } : null;
        },
        // The hourly "Try asking…" writer (one short call). Resumable: a
        // run cut off by a reload is simply due again on the next check.
        () => {
            if (typeof PromptSuggestions === 'undefined' || !PromptSuggestions.writing()) return null;
            return { id: 'prompt-suggestions', label: 'Writing suggested prompts', resumable: true };
        },
        // A linked-brokerage pull (Connect/Plaid). Resumable: every row is
        // written by sourceRef, so a cut-off run just re-pulls on the next
        // Portfolio open.
        () => {
            if (typeof PortfolioBrokerage === 'undefined') return null;
            const s = PortfolioBrokerage.syncing();
            if (!s) return null;
            return { id: 'portfolio-brokerage', label: s.total > 1 ? `Syncing your brokerage accounts (${s.done} of ${s.total})` : 'Syncing your brokerage account', resumable: true };
        },
        // A linked bank/card pull (Spending). Resumable: the cursor is saved
        // only after a complete run, so a cut-off run asks for the same
        // pages again on the next Spending open.
        () => {
            if (typeof SpendingSync === 'undefined') return null;
            const s = SpendingSync.syncing();
            if (!s) return null;
            return { id: 'spending-sync', label: s.total > 1 ? `Syncing your bank accounts (${s.done} of ${s.total})` : 'Syncing your bank account', resumable: true };
        },
        () => {
            if (typeof VoiceService === 'undefined' || !VoiceService.anyRunning()) return null;
            return { id: 'voice-study', label: 'Studying your writing voice', resumable: false };
        },
        ...
        // Workrooms run in this renderer. A reload pauses them in main (the
        // lease dies with the window) and Continue picks the chat back up,
        // so it survives — but it is the user's own job, so it is named.
        () => {
            const n = typeof WorkroomEngine !== 'undefined' ? WorkroomEngine.runs.size : 0;
            if (!n) return null;
            return { id: 'workrooms', label: n === 1 ? 'A workroom is working (it will pause; Continue resumes it)' : `${n} workrooms are working (they will pause; Continue resumes them)`, resumable: true, warn: true };
        },
    ],
