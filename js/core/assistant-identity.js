...
const AssistantIdentity = {
    ...
    applyToDom() {
        ...
        // Home composer: "Ask nenva anything…" becomes "Ask <Name> anything…".
        const dashInput = document.getElementById('dash-agent-input');
        if (dashInput) dashInput.placeholder = `Ask ${name || 'nenva'} anything…`;
        const dashHero = document.querySelector('.dash-agent-hero');
        if (dashHero) dashHero.setAttribute('aria-label', `Ask ${name || 'nenva'}`);
