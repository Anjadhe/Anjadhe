...
const GlobalSearch = {
    ...
    allApps() {
        ...
        for (const tile of tiles) {
            ...
            // A feature-gated-off app (one behind an experimental flag) is
    ...
    data(query, limit = 20) {
        ...
        // journal rows: registered by its package (GlobalSearch.registerSource)
        ...
        // App packages index their own records (registerSource).
        for (const app in this._sources) {
            try { this._sources[app].index?.((id, title, body, extra) => push(app, id, title, body, extra), get); }
            catch (e) { console.warn(`[search] source ${app} failed:`, e); }
        }
    ...
    open(hit) {
        if (!hit) return;
        // ⌘K is a launcher: picking an app here is a jump, so it leaves no
        // Back step. Record hits below are the opposite — they open something
        // you asked for BY NAME inside an app, and the way back is earned.
        if (hit.kind === 'app') { AppManager.openAppFromLauncher(hit.id); return; }
        if (hit.kind === 'setting') {
        ...
        const src = this._sources[hit.app];
        if (src) {
            if (src.open) src.open(hit); else AppManager.openApp(hit.app);
            return;
        }
        switch (hit.app) {
            default:
    ...
    appLabel(app) {
        const map = {
            goals: 'Project', notes: 'Note', schedule: 'Task',
            // bookmarks: registered by its package
        };
        return map[app] || this._sources[app]?.label || app;
    },

    _sources: {},

    /**
     * Let an app package index its own records — the seam that keeps this
     * file from listing every app by name (the built-in loops above are the
     * legacy of that). `index(push, get)` runs on every query: call
     * `push(id, title, body, { sub, meta })` per candidate (the scorer drops
     * misses); `get(storeKey, field)` reads an array off a blob. `open(hit)`
     * receives a chosen result; absent, the app itself opens. `label` is
     * the badge word on the row. Also feeds the assistant's search_all.
     */
    registerSource(app, { label, index, open } = {}) {
        if (!app || typeof index !== 'function') return false;
        this._sources[app] = { label, index, open };
        return true;
    },

    unregisterSource(app) { delete this._sources[app]; }
};
