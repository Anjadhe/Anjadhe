...
const Widgets = {
     ...
     *   load()  → null | { body, count, footer, tone, title }  (may be async)
     *           tone: 'warn' | 'good' | 'alert' — optional STATE of the
     *           card's data, rendered as a tinted edge (components.css).
     *           Semantic only, never app identity: what the data means,
     *           computed from the data, or omitted.
     *           title: optional override of the registered heading, for a
     *           card whose name IS its data ("Trip · Seattle"). Derived
     *           from the data, never model-written.
     *   onAction(action, data, el)   optional, for data-w-action buttons
     *   onOpen()  optional — owns the header/overflow door. The Trips card
     *           opens its trip view rather than just the app. Falls back
     *           to opening `app` when absent; navigation only, never a
     *           mutation (the HelpActions rule).
     */
    register(id, def) {
    ...
    unregister(id) {
        const i = this._defs.findIndex(d => d.id === id);
        if (i >= 0) this._defs.splice(i, 1);
    },

    /**
     * Data-only widget result → the { body, … } contract. The shape an app
     * package returns when it must not (sandboxed user app) or would rather
     * not (any app) write markup:
     *   { rows: [{ sub, subQuiet, text, flag, actions: [{action, id, label, title}] }],
     *     more: n | { n, label }, count, footer, tone, title }
     * Every string lands through rows()/more(), i.e. escaped; `tone` is
     * checked against TONES. Returns null for an empty result, so the
     * widget rule ("nothing to say → not rendered") holds by construction.
     */
    fromData(res) {
        if (!res || typeof res !== 'object') return null;
        const rows = Array.isArray(res.rows) ? res.rows.filter(r => r && typeof r === 'object') : [];
        if (!rows.length && !res.body) return null;
        const more = res.more == null ? '' : (typeof res.more === 'object'
            ? this.more(Number(res.more.n) || 0, String(res.more.label || 'more'))
            : this.more(Number(res.more) || 0));
        const out = { body: (res.body && typeof res.body === 'string' ? res.body : this.rows(rows)) + more };
        if (Number.isFinite(res.count)) out.count = res.count;
        if (typeof res.footer === 'string') out.footer = UIUtils.escapeHtml(res.footer);
        if (typeof res.title === 'string') out.title = res.title;
        if (this.TONES?.includes?.(res.tone) || ['warn', 'good', 'alert'].includes(res.tone)) out.tone = res.tone;
        return out;
    },
    ...
    refresh() {
        ...
        this._refreshTimer = setTimeout(() => {
            if (!this._host?.closest('.view')?.classList.contains('active')) return;
            this.render();
        }, 80);
    },
     ...
     */
    TONES: ['warn', 'good', 'alert'],

    _card(def, res, order = 0) {
        const esc = UIUtils.escapeHtml.bind(UIUtils);
        // A fixed vocabulary, not a color channel — a widget cannot invent
        // a hue, only name a state the CSS knows how to say.
        const tone = this.TONES.includes(res.tone) ? ` data-tone="${res.tone}"` : '';
        const title = (typeof res.title === 'string' && res.title) ? res.title : def.title;
        const count = (res.count !== undefined && res.count !== null)
            ? `<span class="widget-count">${esc(String(res.count))}</span>` : '';
        // The app's own registry icon beside the title (2026-09-09): a card
        // says which app is speaking at a glance. Ink, like every icon in
        // the chrome — the tone edge stays the only color on the card frame.
        let icon = '';
        if (def.app && typeof GlobalSearch !== 'undefined' && typeof GlobalSearch.allApps === 'function') {
            try { icon = (GlobalSearch.allApps().find(a => a.id === def.app) || {}).icon || ''; } catch { icon = ''; }
        }
        const iconHtml = icon ? `<span class="widget-app-icon" data-app="${esc(def.app)}" aria-hidden="true">${icon}</span>` : '';
        const head = def.app
            ? `<button class="widget-head" type="button" data-w-open
                       aria-label="Open ${esc(title)}">
                 ${iconHtml}
                 <h3 class="widget-title">${esc(title)}</h3>
                 ${count}
            ...
            : `<div class="widget-head"><h3 class="widget-title">${esc(title)}</h3>${count}</div>`;
        return `
        <section class="widget-card" data-widget="${esc(def.id)}" data-kind="${esc(def.kind)}"${tone} style="order:${order}">
            ${head}
    ...
    rows(items) {
        ...
        return `<ul class="widget-rows">` + items.map(it => {
            ...
            return `<li class="widget-row">
                <span class="widget-row-when${it.subQuiet ? ' is-quiet' : ''}">${it.sub ? esc(it.sub) : ''}</span>
    ...
    _wire(host) {
        ...
        host.addEventListener('click', (e) => {
            ...
            if (e.target.closest('[data-w-open]')) {
                ...
                else if (def.app) AppManager.openApp(def.app);
                return;
            }
