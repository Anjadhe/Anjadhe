 ...
 * send mail / run shell.
 *
 * The fix: run each code app in a sandboxed <iframe> with
 * `sandbox="allow-scripts allow-forms"` and NO `allow-same-origin`. That gives
 * the frame a unique OPAQUE origin with
...
function _guestMain() {
    ...
    var usesSnap = {};       // api name -> [method names] the host will broker (manifest `uses`)
    var pendingCalls = {};   // callId -> { resolve, reject } for guest→host requests
    var callSeq = 0;
    function request(type, fields) {
        var callId = 'g' + (++callSeq);
        return new Promise(function (resolve, reject) {
            pendingCalls[callId] = { resolve: resolve, reject: reject };
            var msg = Object.assign({ type: type, callId: callId }, fields || {});
            post(msg);
            setTimeout(function () {
                if (pendingCalls[callId]) { delete pendingCalls[callId]; reject(new Error(type + ' timed out')); }
            }, 30000);
        });
    }
    var widgetLoader = null; // () => data-shape result | null (see Widgets.fromData)
    var widgetAction = null; // (action, { id }) => void
    var appObj = null;
    ...
    function makeSurface() {
        return {
            ...
            storage: makeStorage(),
            navigate: function (name) { post({ type: 'navigate', name: String(name) }); },
            registerTool: function (definition, handler, opts) {
                // OpenAI-style: { type:'function', function:{ name, … } } — same
                // shape AgentTools.register requires on the host side.
                // opts.ask: the assistant confirms with the user before every run.
                ...
                post({ type: 'register-tool', definition: definition, ask: !!(opts && opts.ask) });
            },
            // A card on home. spec: { kind, title, order }; load() returns
            // { rows: [{ sub, text, flag, actions: [{action, id, label}] }],
            //   more, count, footer, tone, title } or null for "nothing to
            // say" (then no card). The HOST paints it — this frame never
            // hands markup across. onAction(action, { id }) for row buttons.
            registerWidget: function (spec, load, onAction) {
                if (typeof load !== 'function') { logError('registerWidget(spec, load) needs a load function'); return; }
                widgetLoader = load;
                widgetAction = typeof onAction === 'function' ? onAction : null;
                var s = spec || {};
                post({ type: 'register-widget', spec: { kind: s.kind === 'glance' ? 'glance' : 'attention', title: String(s.title || ''), order: Number(s.order) } });
            },
            // Another app's exposed API (docs/PLATFORM.md "App packages"):
            // declare it in the manifest — `"uses": ["schedule"]` — then
            // anjadhe.use('schedule') is an object whose methods return
            // PROMISES (each call crosses into the host). null when that app
            // is not installed; throws when it was not declared.
            use: function (name) {
                if ((MANIFEST.uses || []).indexOf(name) === -1) {
                    throw new Error('Declare uses:["' + name + '"] in manifest.json to call that app\'s API');
                }
                var methods = usesSnap[name];
                if (!methods) return null;
                var api = {};
                methods.forEach(function (m) {
                    api[m] = function () {
                        var args = Array.prototype.slice.call(arguments);
                        return request('use-call', { api: name, method: m, args: JSON.parse(JSON.stringify(args)) });
                    };
                });
                return api;
            },
    ...
    window.addEventListener('message', function (e) {
        ...
        } else if (d.type === 'use-result') {
            var pc = pendingCalls[d.callId];
            if (pc) {
                delete pendingCalls[d.callId];
                if (d.error) pc.reject(new Error(String(d.error))); else pc.resolve(d.result);
            }
        } else if (d.type === 'widget-load') {
            Promise.resolve().then(function () { return widgetLoader ? widgetLoader() : null; }).then(
                function (result) { post({ type: 'widget-result', callId: d.callId, result: result == null ? null : JSON.parse(JSON.stringify(result)) }); },
                function (err) { post({ type: 'widget-result', callId: d.callId, error: (err && err.message) || String(err) }); }
            );
        } else if (d.type === 'widget-action') {
            if (widgetAction) { try { widgetAction(String(d.action || ''), { id: d.id == null ? '' : String(d.id) }); } catch (e) { logError('widget action: ' + ((e && e.message) || e)); } }
...
const UserAppSandbox = {
    ...
    mountCodeApp(manifest, entry, opts = {}) {
        ...
        // allow-forms is needed for the <form> submit EVENT to fire at all
        // (without it Chromium aborts submission before app handlers run —
        // every generated form's Add button is dead). Actual navigation-style
        // submissions stay blocked by the guest CSP's form-action 'none'.
    ...
    _buildGuestHtml(manifest, entry) {
        ...
        // No webfont link. The host self-hosts Nunito/Inter (css/fonts.css), but
        // this guest is an OPAQUE-ORIGIN srcdoc frame — it cannot resolve the
        // host's relative stylesheet, and a font loaded by the parent document
        // is not shared with it. The old fix was a fonts.googleapis.com <link>,
        // which meant every user-app render made its own request to Google.
        // The design-system stack in `--font-serif` falls through to
        // -apple-system, which is what a user app renders in now. Inlining the
        // faces as data: URIs would be ~500KB of base64 per frame — don't.
        ...
        return '<!doctype html><html' + themeAttr + '><head><meta charset="utf-8">'
            ...
            + '<div id="' + manifest.id + '-view" class="view app-view active'
            + (manifest.layout === 'full' ? ' app-view-full' : '') + '"></div>'
    ...
    _onMessage(e) {
        ...
        switch (d.type) {
            case 'ready':
                ...
                break;
            case 'register-tool':
                this._registerGuestTool(record, d.definition, d.ask);
                break;
            case 'register-widget':
                this._registerGuestWidget(record, d.spec);
                break;
            case 'use-call':
                this._useCall(record, d);
                break;
            case 'widget-result': {
    ...
    /**
     * Push the app's CURRENT host-side storage blob into a running guest
     * (it keeps a synchronous snapshot) and re-render. Used after the host
     * changes the blob out from under the app — e.g. Settings › Build Apps
     * › Reset Data.
     */
    pushStorage(id) {
        const r = this._records[id];
        if (!r) return;
        const storage = StorageManager.get(`userapp-${id}`) || {};
        this._post(id, { type: 'storage-changed', storage });
    },
    ...
    _sendInit(record) {
        ...
        // Exposed cross-app APIs the manifest declared (Anjadhe.use): the
        // guest gets method NAMES only and calls back over use-call.
        const uses = {};
        for (const name of (manifest.uses || [])) {
            const api = (typeof Anjadhe !== 'undefined') ? Anjadhe.use(name) : null;
            if (api) uses[name] = Object.keys(api).filter(k => typeof api[k] === 'function' && !k.startsWith('_'));
        }
        this._post(manifest.id, { type: 'init', storage, reads, uses, keywords: record.keywords });
    },
    ...
    _storageWrite(id, key, value, isDelete) {
        ...
        if (typeof Anjadhe !== 'undefined' && Anjadhe._announce) Anjadhe._announce(ns);
    },

    /**
     * A guest calling another app's exposed API. The manifest's `uses` is
     * the allowlist (enforced HERE, host-side — the guest's own check is a
     * courtesy), only public functions are callable, and the result is
     * JSON-cloned on the way back so no live object crosses the boundary.
     */
    async _useCall(record, d) {
        const reply = (fields) => this._post(record.id, Object.assign({ type: 'use-result', callId: d.callId }, fields));
        try {
            const name = String(d.api || '');
            if (!(record.manifest.uses || []).includes(name)) throw new Error(`"${name}" is not declared in uses`);
            const api = (typeof Anjadhe !== 'undefined') ? Anjadhe.use(name) : null;
            if (!api) throw new Error(`"${name}" is not installed`);
            const method = String(d.method || '');
            if (!method || method.startsWith('_') || typeof api[method] !== 'function') throw new Error(`"${name}.${method}" is not available`);
            const args = Array.isArray(d.args) ? d.args : [];
            const result = await api[method](...args);
            reply({ result: result === undefined ? null : JSON.parse(JSON.stringify(result)) });
        } catch (e) {
    ...
    /**
     * The guest's card on home. load() is a round-trip into the frame that
     * returns DATA (never markup); Widgets.fromData paints it host-side
     * with every string escaped. Row buttons go back into the frame.
     */
    _registerGuestWidget(record, spec) {
        if (typeof Anjadhe === 'undefined' || !Anjadhe.registerWidgetFor) return;
        const id = record.id;
        Anjadhe.registerWidgetFor(id, {
            kind: spec?.kind, title: spec?.title || record.manifest?.name || id, order: spec?.order,
            load: () => this._request(id, 'widget-load', {}, 10000).catch(() => null),
            onAction: (action, data) => this._post(id, { type: 'widget-action', action, id: data?.id })
        });
        if (typeof Widgets !== 'undefined') Widgets.refresh();
    },

    /** One request/response round trip into the guest (tools, widget loads). */
    _request(id, type, fields, timeoutMs) {
        const r = this._records[id];
        ...
        return new Promise((resolve, reject) => {
            ...
            this._post(id, Object.assign({ type, callId }, fields || {}));
        });
    },

    _registerGuestTool(record, definition, ask) {
        ...
        AgentTools.register(
            ...
            { source: id, keywords: record.keywords, ask: !!ask }
        );
    },

    _callGuestTool(id, name, args) {
        return this._request(id, 'tool-call', { name, args: args || {} }, 30000);
    },
