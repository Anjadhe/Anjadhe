...
document.addEventListener('DOMContentLoaded', () => {
    ...
    AppManager.init();
    AgentUI.init();
    // One delegated click handler for inline record links in AI-written
    // content ([title](anjadhe://task/<id>) chips) — every surface that
    // renders model markdown gets them for free. See js/agent/record-links.js.
    RecordLinks.init();
    // @-mention autosuggest in the assistant composers — attaches the
    // conversation to a record (see js/components/record-mention.js).
    RecordMention.init();
    UpdaterUI.init();
    WhatsNew.init();
});
