...
const BundledApps = {
    ...
    async _loadOne(id) {
        ...
        if (manifest.view) {
            ...
            // AppManager ran FEATURES.applyToDocument before packages loaded,
            // so a data-feature inside a package's own view.html (Portfolio's
            // settings gear rides the brokerage flag) needs its own pass.
            if (typeof FEATURES !== 'undefined' && FEATURES.applyToDocument) {
                FEATURES.applyToDocument(document.getElementById(`${id}-view`));
            }
        }
