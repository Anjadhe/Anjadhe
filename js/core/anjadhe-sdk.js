...
const Anjadhe = {
    ...
    // A user app's write repaints home (its widget) the way ScheduleApp.
    // saveData does — the same event, so Widgets needs no second listener.
    _announce(key) {
        try { document.dispatchEvent(new CustomEvent('anjadhe:data-changed', { detail: { key } })); } catch (_) {}
    },

    /**
     * A user app's card on home. `def` = { kind, title, order, load,
     * onAction } as Widgets.register takes, except that load() may return
     * the DATA shape (Widgets.fromData: rows/more/count/footer/tone/title)
     * instead of markup — the only shape a sandboxed app can return, and
     * the one the docs teach. Returns the widget id.
     */
    registerWidgetFor(appId, def) {
        if (typeof Widgets === 'undefined' || !def || typeof def.load !== 'function') return null;
        const id = `userapp-${appId}`;
        Widgets.register(id, {
            kind: def.kind === 'glance' ? 'glance' : 'attention',
            title: String(def.title || appId),
            app: appId,
            order: Number.isFinite(def.order) ? def.order : 50,
            load: async () => {
                const res = await def.load();
                return (res && typeof res === 'object' && typeof res.body === 'string' && !res.rows)
                    ? res : Widgets.fromData(res);
            },
            onAction: typeof def.onAction === 'function' ? (action, data) => def.onAction(action, data) : undefined
        });
        return id;
    },
    ...
    // Both bundled packages and (later, over the bridge) user apps go
    // through this; a manifest `uses: ["schedule"]` is the documented
    // dependency, the mirror of `reads`.
    _apis: {},
    ...
    storageFor(appId) {
        ...
        return {
            ...
            set(key, value) {
                const blob = StorageManager.get(ns) || {};
                blob[key] = value;
                const r = StorageManager.set(ns, blob);
                Anjadhe._announce(ns);
                return r;
            },
            delete(key) {
                const blob = StorageManager.get(ns) || {};
