 ...
 *   - linkPopover: Cmd+Shift+K and the toolbar Link button open an inline
 *     popover instead of the native prompt() (plain Cmd+K belongs to the
 *     command palette app-wide)
 *
...
const RichEditor = {
    ...
    init(editorId, toolbarClass, onAutoSave = null, options = {}) {
        ...
        this.pasteHandler = (e) => {
            ...
            // A raw image on the clipboard (screenshot, copied image file)
            // pastes inline, Notion-style. HTML wins when it carries real
            // content beyond an image render — Excel/Numbers put both a
            // snapshot image AND the table on the clipboard, and there the
            // table is the intent.
            const imgFile = this._clipboardImageFile(cd);
            if (imgFile && !this._htmlBeyondImages(html)) {
                this._insertClipboardImage(imgFile);
                return;
            }
            ...
            if (text) {
                // Text copied out of a .md file arrives as markdown source.
                // Render it, so `# Title` pastes as a heading, not as a
                // paragraph starting with a hash.
                if (this._looksLikeMarkdown(text)) {
                    const md = this.sanitizeHtml(this._markdownToHtml(text));
                    if (md) { document.execCommand('insertHTML', false, md); return; }
                }
                const lines = text.split(/\r?\n/);
                // A single line pastes inline. Several lines become their own
                // paragraphs: joined with <br> they all landed inside whatever
                // block the caret was in, so a markdown file pasted into a
                // heading came out as one enormous heading.
                const safe = lines.length === 1
                    ? this.escapeHtml(lines[0])
    ...
    /** Block-level markdown on at least one line, or inline emphasis/code. */
    _looksLikeMarkdown(text) {
        if (!text || text.indexOf('\n') === -1 && !/^#{1,6} /.test(text)) {
            return /^#{1,6} /.test(text || '');
        }
        const block = /^(#{1,6} |[-*+] |\d+[.)] |> |```|(-{3,}|\*{3,})\s*$)/m;
        const inline = /(\*\*|__)[^*_\n]+\1|`[^`\n]+`|\[[^\]\n]+\]\(https?:[^)\s]+\)/;
        return block.test(text) || inline.test(text);
    },

    /**
     * A small markdown renderer for paste: headings, lists (one level),
     * quotes, fenced code, rules, paragraphs, and inline bold / italic /
     * strikethrough / code / links. Output goes through sanitizeHtml.
     */
    _markdownToHtml(text) {
        ...
        const flushList = () => {
            if (list) { out.push(`<${list.tag}>${list.items.map(t => `<li>${inline(t)}</li>`).join('')}</${list.tag}>`); list = null; }
        };
        while (i < lines.length) {
            ...
            if ((m = line.match(/^(#{1,6}) +(.*?)\s*#*\s*$/))) {
                ...
                flushPara(); flushList();
                const q = [m[1]];
                while (i + 1 < lines.length && /^> ?/.test(lines[i + 1])) q.push(lines[++i].replace(/^> ?/, ''));
    ...
    /* ---- image paste (2026-08-10) ---- */

    /** First image FILE on the clipboard, or null. */
    _clipboardImageFile(cd) {
        if (!cd) return null;
    ...
    /** Does the clipboard HTML say anything an image alone doesn't? */
    _htmlBeyondImages(html) {
    ...
    /** Encode the image and insert it at the caret the paste landed on.
     *  Encoding is async, so the caret is captured first and restored —
     *  focus can move while a large screenshot re-encodes. */
    async _insertClipboardImage(file) {
        const editor = this.editor;
        const sel = window.getSelection();
        const range = (sel && sel.rangeCount > 0
            && editor.contains(sel.getRangeAt(0).commonAncestorContainer))
            ? sel.getRangeAt(0).cloneRange() : null;

        let dataUrl = null;
        try {
            dataUrl = await this._encodePastedImage(file);
        } catch (err) {
            console.error('Image paste failed:', err);
        }
        if (!dataUrl) {
            if (typeof UIUtils !== 'undefined') UIUtils.showToast('Could not paste that image', 'error');
            return;
        }
        // The editor can be torn down (note switched) mid-encode.
        if (this.editor !== editor || !editor.isConnected) return;
        ...
        // execCommand fires `input`, so autosave sees the image like any
        // other edit.
        document.execCommand('insertHTML', false, `<img src="${dataUrl}" alt="">`);
    },

    /** Downscale + re-encode so a Retina screenshot doesn't land megabytes
     *  in a synced content blob: longest edge capped at 1600px, WebP —
     *  photos and text screenshots both compress well and transparency
     *  survives (Chromium always encodes it). Animated GIFs under 2MB are
     *  kept whole — a canvas pass would freeze the animation. */
    async _encodePastedImage(file) {
        const MAX_EDGE = 1600;
        if (/^image\/gif$/i.test(file.type) && file.size <= 2 * 1024 * 1024) {
    ...
    /**
     * "Normal text": every heading / quote / code block the selection touches
     * becomes a plain paragraph, and inline bold/italic/etc. inside the
     * selection is stripped. Done on the DOM rather than via
     * `formatBlock p` because Chromium's formatBlock is unreliable across a
     * multi-line selection (and does nothing about inline styling), which is
     * exactly the state a pasted markdown heading leaves a note in.
     */
    _toNormalText() {
        const sel = window.getSelection();
        ...
        // Strip inline formatting first while the selection is intact
        // (execCommand keeps this part on the undo stack).
        if (!sel.isCollapsed) document.execCommand('removeFormat', false, null);
        ...
        const blocks = Array.from(this.editor.querySelectorAll('h1,h2,h3,h4,h5,h6,blockquote,pre'))
            .filter(el => range.intersectsNode(el));
        // Innermost first, so a heading inside a blockquote unwraps cleanly.
        blocks.reverse();
        let changed = false;
        for (const el of blocks) {
            const tag = el.tagName.toLowerCase();
            if (tag === 'blockquote') {
                // Unwrap: its child blocks (or inline content) stand on their own.
                const hasBlockChild = Array.from(el.children).some(c => /^(P|DIV|H[1-6]|UL|OL|PRE|BLOCKQUOTE)$/.test(c.tagName));
                if (hasBlockChild) {
                ...
                // One paragraph per line; the <code> wrapper goes.
        ...
        try {
            ...
            sel.removeAllRanges();
            sel.addRange(r);
        } catch (err) { /* endpoints were inside a replaced <pre>; leave the caret where it landed */ }
    ...
    /**
     * Make a freshly-created, empty code block typeable.
     *
     * `<pre></pre>` renders nothing, so a caret inside it canonicalizes into
     * the block above and the first character typed lands in THAT paragraph
     * ("intro" + "x" → "introx", verified in Electron 40). A <br> gives the
     * block a real line to hold the caret — but inside <pre> a <br> is a
     * newline that would stay visible as a blank first line, so it is dropped
     * again as soon as there is genuine content to hold the caret instead.
     */
    _primeCodeBlock(pre) {
        pre.textContent = '';
        ...
        const onFirstInput = () => {
            this.editor.removeEventListener('input', onFirstInput);
            const br = Array.from(pre.childNodes).find(n => n.nodeName === 'BR');
            if (br && (pre.textContent || '').length > 0) br.remove();
        };
        this.editor.addEventListener('input', onFirstInput);
    },
    ...
    _handleEmptyAreaClick(e) {
        ...
        const isEmptyPara = (el) => isEl(el)
            && /^(P|DIV|H[1-6]|BLOCKQUOTE)$/.test(el.nodeName)
            && el.textContent.trim() === '';
        // Blocks the browser can land a caret in on its own. A gap bordered
        // by one of these is NOT a dead zone: default click handling places
        // the caret in the nearest line, and a click must never invent a
        // line — that's Enter's job (2026-08-09: every click in the margins
        // between paragraphs/bullet lists was appending a fresh <p>). Only
        // a gap walled by caret-proof blocks on both sides (code, divider,
        // image, table) still gets a paragraph inserted — the case this
        // handler exists for.
        const caretEnterable = (n) => {
            if (!n) return false;
            if (n.nodeType === Node.TEXT_NODE) return true;
            if (!isEl(n)) return false;
            if (n.classList.contains('agent-codeblock')) return false;
            return /^(P|DIV|H[1-6]|BLOCKQUOTE|UL|OL)$/.test(n.nodeName);
        };
        ...
        if (isEmptyPara(below)) {
            ...
            target = above;
        } else if (!caretEnterable(below) && !caretEnterable(above)) {
            target = document.createElement('p'); // caret-proof gap — insert a line
            target.appendChild(document.createElement('br'));
            this.editor.insertBefore(target, below); // below == null → append at end
        } else {
            return;   // the browser places the caret; nothing is created
        }
        e.preventDefault();
    ...
    _handleEnterInCode(e) {
        ...
        e.preventDefault();
        // insertLineBreak — NOT insertText with '\n'. Chromium's typing command
        // turns a newline in insertText into a paragraph separator, which splits
        // the <pre> in two: the guard above would prevent Enter's default and
        // then perform exactly the split it exists to prevent (verified in
        // Electron 40 — `<pre><code>a</code></pre>` became two blocks). Both
        // commands keep undo working and fire the input event for auto-save.
    ...
    handleKeyboardShortcuts(e) {
        ...
        // the OS / browser — and plain Cmd+K to the command palette, which
        // owns that key app-wide.
        ...
        if (k === '0') { e.preventDefault(); this._toNormalText(); return; }
    ...
    /**
     * Fire the pending auto-save callback NOW, if one is inside the 1 s
     * window. Owners call this on their flush path (note switch, leaving
     * the app, close) — before it existed, a keystroke followed within a
     * second by any of those was silently discarded: the owner's dirty
     * flag is set by this very callback, so it read "clean" and destroy()
     * cleared the timer (2026-09-10).
     */
    flushPending() {
        if (!this.autoSaveTimeout) return false;
    ...
    _setupSelectionToolbar() {
        ...
        el.innerHTML = `
            ...
            <button type="button" data-action="normal" title="Normal text (Cmd+Shift+0)">T</button>
            ...
            <button type="button" data-command="insertUnorderedList" title="Bullet list (Cmd+Shift+8)">&bull;</button>
            <button type="button" data-command="insertOrderedList" title="Numbered list (Cmd+Shift+7)">1.</button>
            <span class="re-divider"></span>
            <button type="button" data-command="createLink" title="Link (Cmd+Shift+K)">&#128279;</button>
        ...
        el.addEventListener('mousedown', (e) => {
            ...
            const normalBtn = e.target.closest('button[data-action="normal"]');
            if (normalBtn) {
                e.preventDefault();
                this._toNormalText();
    ...
    _setupSlashMenu() {
        ...
        this._slashItems = [
            { label: 'Text',          hint: 'Normal paragraph',     run: () => this._toNormalText() },
            ...
            // Enter inside a code block stays inside it, so the block needs a
            // normal paragraph after it or a note ending in code has no line to
            // continue on (same reason the table below adds one).
            { label: 'Code',          hint: 'Code block',           run: () => {
                document.execCommand('formatBlock', false, 'pre');
                const pre = this._closestPre(window.getSelection().anchorNode);
                if (pre) this._primeCodeBlock(pre);
    ...
    _selectSlashItem() {
        ...
        // command, so the command operates on a clean empty block. The <br>
        // matters: a block emptied to `<p></p>` renders nothing, so a caret in
        // it canonicalizes upstream into the PREVIOUS block and every command
        // reformats that one instead — "/code" on a fresh line under a
        // paragraph pulled the paragraph into the code block, "/heading 1"
        // turned it into a heading.
        this._slashTriggerNode.textContent = '';
        this._slashTriggerNode.appendChild(document.createElement('br'));
        // Restore the cursor inside the now-empty block, before the <br>.
        const range = document.createRange();
        range.setStart(this._slashTriggerNode, 0);
        range.collapse(true);
        const sel = window.getSelection();
        sel.removeAllRanges();
        sel.addRange(range);
