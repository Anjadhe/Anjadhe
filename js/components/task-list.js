 ...
 *     Open, Rename, Set date, Delete — Delete confirms first, then the
 *     toast still offers Undo
...
const TaskListUI = {
    ...
    renderList(tasks, opts = {}) {
        ...
        const row = (t) => {
            ...
            return `<div class="task-row${resolved ? ' task-row--completed' : ''}" data-task-id="${t.itemId}" title="Open task">
                ...
                <span class="task-row-title" data-edit-title="${t.itemId}">${this._esc(t.title)}</span>
    ...
    attach(root, opts = {}) {
        ...
        root.querySelectorAll('.task-row').forEach(row => {
            row.addEventListener('click', (e) => {
                if (e.target.closest('.task-row-check, .task-row-date, .task-row-menu')) return;
    ...
    /**
     * Same proxy trick for the native TIME picker — anchored to whatever
     * element invoked it (the Actions page's "anytime" gutter placeholder).
     * Commits startTime; clearing the input clears the time.
     */
