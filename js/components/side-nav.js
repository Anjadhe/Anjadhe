...
const SideNav = {
    ...
    toggleCollapsed() {
        this._transient = null; // a deliberate toggle is the user's call from here on
    ...
    /**
     * Fold the rail for a focus session WITHOUT touching the stored
     * preference (FocusMode). `false` restores whatever the user had.
     */
    setTransientCollapsed(on) {
        if (on) {
            if (this._transient) return;
            this._transient = { was: this.isCollapsed() };
            if (!this._transient.was) {
                document.body.classList.add('sidenav-collapsed');
                this._paintCollapseBtn();
                this.render();
            }
    ...
    render() {
        ...
        list.querySelectorAll('.sidenav-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                // A rail click is a jump, not a push — no Back entry (see
                // AppManager.openAppFromLauncher). It still puts up the
                // unlock overlay itself for a locked app.
                AppManager.openAppFromLauncher(app);
            });
        });
    },
