...
const LinkPicker = {
     ...
     * App display labels
     */
    _builtinLabels: { goals: 'Project', schedule: 'Task', notes: 'Note' },
    _builtinPlural: { goals: 'Projects', schedule: 'Tasks', notes: 'Notes' },
    ...
    renderItemContent(app, item) {
        switch (app) {
            ...
            default: {
                const meta = LinkManager._apps[targetApp]?.renderMeta ? String(LinkManager._apps[targetApp].renderMeta(item) || '') : '';
                return `
                    <span class="link-picker-item-title">${UIUtils.escapeHtml(item.title)}</span>
