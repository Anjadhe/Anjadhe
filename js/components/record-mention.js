...
const RecordMention = {
    ...
    _index() {
        const out = [];
        const get = (k, f) => {
            const d = StorageManager.get(k);
            return Array.isArray(d?.[f]) ? d[f] : [];
        };
        for (const g of get('goals', 'goals')) {
            if (g.status === 'completed') continue;
            out.push({ key: 'goals:' + g.id, type: 'Goal', title: g.title || '(untitled)',
                sub: g.group || g.targetDate || '', body: g.description || '',
                recency: g.modifiedAt || g.createdAt || '' });
        }
        for (const s of get('schedule', 'scheduleItems')) {
            const live = (typeof AgentTools !== 'undefined' && AgentTools._isLiveTask)
                ? AgentTools._isLiveTask(s)
                : !s.lastCompletedDate;
            if (!live) continue;
            out.push({ key: 'schedule:' + s.id, type: 'Task', title: s.title || '(untitled)',
                sub: s.scheduledDate || '', body: s.description || '',
                recency: s.modifiedAt || s.createdAt || s.scheduledDate || '' });
        }
        const pf = StorageManager.get('portfolio') || {};
        for (const s of (Array.isArray(pf.strategies) ? pf.strategies : [])) {
            out.push({ key: 'portfolio:strategy:' + s.id, type: 'Strategy',
                title: s.name || '(untitled)', sub: s.objective || '',
                body: `${s.objective || ''} ${s.thesis || ''}`,
                recency: s.modifiedAt || s.updatedAt || s.createdAt || '' });
        }
        for (const a of (Array.isArray(pf.accounts) ? pf.accounts : [])) {
            out.push({ key: 'portfolio:account:' + a.id, type: 'Account',
                title: a.name || '(untitled)', sub: a.type || '',
                body: `${a.type || ''} ${a.institution || ''}`,
                recency: a.updatedAt || a.createdAt || '' });
        }
        // Routines and plain notes are both notes under the hood — the
        // routine's recordKey is 'prompts:' (its own resolver), and feed
        // posts (machine-written routine output) are not mentionable.
        for (const n of get('notes', 'notes')) {
            const tpl = (typeof NoteTemplates !== 'undefined' && NoteTemplates.resolve)
                ? NoteTemplates.resolve(n) : n.template;
            if (tpl === 'feed') continue;
            if (tpl === 'prompt') {
                out.push({ key: 'prompts:' + n.id, type: 'Routine',
                    title: n.title || '(untitled)', sub: '', body: '',
                    recency: n.modifiedAt || n.createdAt || '' });
            } else {
                out.push({ key: 'notes:' + n.id, type: 'Note',
                    title: n.title || '(untitled)', sub: '',
                    body: (typeof AgentTools !== 'undefined' && AgentTools._noteText)
                        ? AgentTools._noteText(n).slice(0, 400) : '',
                    recency: n.modifiedAt || n.createdAt || '' });
            }
        }
        return out;
    ...
    TYPE_WORDS: {
        goal: 'Goal', goals: 'Goal', task: 'Task', tasks: 'Task',
        note: 'Note', notes: 'Note', routine: 'Routine', routines: 'Routine',
    ...
    _render(anchor) {
        ...
        const plural = this._typeFilter === 'Strategy' ? 'strategies'
            : this._typeFilter ? this._typeFilter.toLowerCase() + 's' : '';
        ...
        const hint = !this._typeFilter && new Set(this._items.map(it => it.type)).size > 1
            ? `<div class="record-mention-hint">Narrow by type: @goal&ensp;@task&ensp;@note&ensp;@routine&ensp;@strategy&ensp;@account — then your search</div>` : '';
