...
const NavResizer = {
     ...
     */
    // responsiveDefault (opt-in): with no stored width, do NOT write the px
    // default at attach — leave the css var unset so the stylesheet's own
    // fallback (which may be responsive, e.g. a clamp()) governs until the
    // user actually drags. measureSel names the pane to read the REAL
    // rendered width from when a drag starts with no stored value, so the
    // first drag doesn't jump from the visual width to defaultW.
    attach({ layoutSel, resizerId, cssVar, storageKey, defaultW, min = 140, max = 360,
             responsiveDefault = false, measureSel = null }) {
        ...
        const current = () => {
            ...
            if (measureSel) {
                const rect = document.querySelector(measureSel)?.getBoundingClientRect();
        ...
        // Restore the width saved on this machine. With nothing saved, a
        // responsiveDefault attach leaves the var unset (the stylesheet's
        // fallback rules); everyone else pins the px default as before.
        let saved = null;
        try {
        ...
        else if (!responsiveDefault) apply(defaultW);
