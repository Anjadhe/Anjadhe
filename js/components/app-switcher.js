...
const AppSwitcher = {
    ...
    init() {
        ...
        // The button moves with the window (the titlebar insets follow the
        // content band), so an open panel follows it.
        window.addEventListener('resize', () => {
            if (this._open) this._place(document.getElementById('app-switcher'));
        });
    },
    ...
    // Drop the panel from the button: left edges aligned, 6px under it,
    // clamped so it never runs off either side of the window. Measured
    // after unhiding, since a hidden panel has no width.
    _place(panel) {
        const btn = document.getElementById('titlebar-apps-btn');
        if (!panel || !btn) return;
    ...
    _render(panel) {
        ...
        + `<button class="app-switcher-customize" type="button">More apps&hellip;</button>`;

        panel.querySelectorAll('.app-switcher-tile').forEach(tile => {
            tile.addEventListener('click', () => {
                this.close();
                // A jump, so it records no Back step (see
                // AppManager.openAppFromLauncher). Handles the App Lock
                // overlay itself when the target is locked.
                AppManager.openAppFromLauncher(tile.dataset.app);
            });
        });
