...
const chromeBrowser = new (require('./js/main/browser/chrome-runtime').ChromeRuntime)({
    ...
    onChange: ({ manual = false } = {}) => {
        const state = chromeBrowser.state(), working = state.connected && state.control === 'agent';
        if (chromeWasWorking && !working || manual && workroomBrowser.holder) {
            pauseBrowserWork(state.connected ? 'You have control of Chrome. Come back here and Continue when ready.' : 'Chrome disconnected. Reconnect the browser, then Continue from its current page.');
        }
        chromeWasWorking = working;
...
function releaseChromeControl() {
    // An app-driven pause/completion already updated the room. Its control
    // acknowledgment must not pause a different room a second time.
    chromeWasWorking = false;
