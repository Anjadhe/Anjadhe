...
ipcMain.handle('discover-news', async (event, { topics, sources } = {}) => {
    ...
    // holdings themselves — reach the news services from Anjadhe's server,
    ...
    if (provider === 'anjadhe') {
        ...
        catch { return { error: 'Anjadhe Connect could not fetch headlines right now.', code: 'connect' }; }
    }
...
function handleChannelView(message) {
    ...
    if (BrowserWindow.getAllWindows().length === 0) {
        return { type: 'view-error', view, reqId, error: 'Anjadhe is not open on your Mac.' };
    }
...
function handleChannelChat(message) {
    ...
    if (BrowserWindow.getAllWindows().length === 0) {
        return { type: 'chat-error', text: 'Anjadhe is not open on your Mac.' };
    }
...
ipcMain.handle('license-open-file', async (event) => {
    ...
    const result = await dialog.showOpenDialog(parent, {
        ...
        buttonLabel: 'Open',
        filters: [{ name: 'Anjadhe license', extensions: ['anjadhe-license', 'txt', 'json'] }, { name: 'All files', extensions: ['*'] }]
    });
...
ipcMain.handle('license-claim-alpha', async (event, email) => {
    ...
    try {
        ...
        if (!res.ok || !data?.license) {
            return { error: data?.error || `Anjadhe Connect answered HTTP ${res.status}.`, code: data?.code || null };
        }
        const status = LicenseStore.save(data.license, addr);
        if (status.error) return status;
        return { ...status, created: data.created === true };
    } catch (e) {
        return { error: `Could not reach Anjadhe Connect (${e.message}). Check the connection and try again.` };
    }
});
