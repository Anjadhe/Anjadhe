...
const SYNC_EXCLUDE_KEYS = new Set([
    ...
    'app_ai-activity',
    // Discover pane cache: generated by this Mac's model from live web
...
function openaiStreamRequest(cfg, messages, tools, maxTokens, sender, streamId, think) {
...
function isLocalLLMJob(params) {
    ...
    if (engine === 'openai' || engine === 'anthropic' || engine === 'server') return false;
    const provider = settingsStore.get('llmProvider', 'auto');
    if (provider === 'custom' || provider === 'openai' || provider === 'anthropic') return false;
...
ipcMain.handle('llm-chat-stream', withLLMWake(withLLMScheduler('interactive', async (event, params) => {
    ...
    if (!override && (params.engine === 'openai' || params.engine === 'anthropic')) {
        ...
        try {
            return params.engine === 'anthropic'
                ...
                : await openaiStreamRequest(cfg, params.messages, params.tools, params.maxTokens, sender, streamId, params.think);
        } catch (e) {
    ...
    if (!override && (params.engine === 'llamacpp' || params.engine === 'server')) {
        try {
            if (params.engine === 'server') {
                ...
                return await openaiStreamRequest(cfg, params.messages, params.tools, params.maxTokens, sender, streamId, params.think);
            }
            const cfg = await llamaCppChatConfig(params);
            const result = await openaiStreamRequest(cfg, params.messages, params.tools, params.maxTokens, sender, streamId, params.think);
    ...
    if (provider === 'custom') {
        return await openaiStreamRequest(getCustomConfig(), params.messages, params.tools, params.maxTokens, sender, streamId, params.think);
    }
    ...
    if (customCfg.baseUrl) {
        try {
            const result = await openaiStreamRequest(customCfg, params.messages, params.tools, params.maxTokens, sender, streamId, params.think);
            if (!result?.error) return result;
...
ipcMain.handle('agent-process-start', async (event, { command, cwd } = {}) => {
    ...
    console.log(`[agent-proc] started ${id} (pid ${child.pid}): ${cmd.slice(0, 120)}`);
...
const NEWS_MAX_ITEMS_PER_TOPIC = 10;
...
// ── Discover weather (docs/DISCOVER.md, deferred from D1) ───────────────
//
// Today's conditions for the pane's typed location, from Open-Meteo —
// keyless and account-free. The only things that leave the machine are the
// city-level location text (geocoding) and its coordinates (forecast),
// exactly the exposure the doc's privacy model states. Deterministic data;
// no model involved. Same anonymous-shaped fetch as direct news: no
// cookies, minimal static User-Agent, hardcoded hosts.

async function _weatherFetch(url) {
    const res = await fetch(url, {
        headers: { 'Accept': 'application/json', 'User-Agent': 'Anjadhe/1.0' },
        signal: AbortSignal.timeout(15000)
    });
...
ipcMain.handle('discover-weather', async (event, { location, unit } = {}) => {
    // Same consent gates as discover-news: this only ever runs for users
    // who opted into web access, and only with their typed location.
    if (!isWebSearchEnabled()) return { error: 'Web search is turned off' };
    if (!getActiveSearchProvider()) return { error: 'Web access not configured' };
    const loc = String(location || '').trim();
    if (!loc || loc.length > 80) return { error: 'location required' };
    try {
        const geo = await _weatherFetch('https://geocoding-api.open-meteo.com/v1/search?' +
            new URLSearchParams({ name: loc, count: '1', language: 'en', format: 'json' }));
        const g = geo?.results?.[0];
        if (!g || !Number.isFinite(g.latitude)) return { error: 'location not found' };
        const tempUnit = unit === 'fahrenheit' ? 'fahrenheit' : 'celsius';
        const fc = await _weatherFetch('https://api.open-meteo.com/v1/forecast?' + new URLSearchParams({
        ...
        return {
            place: String(g.name || loc).slice(0, 60),
            unit: tempUnit,
            tempNow: Number(fc?.current?.temperature_2m),
            codeNow: Number(fc?.current?.weather_code),
            hi: Number(fc?.daily?.temperature_2m_max?.[0]),
            lo: Number(fc?.daily?.temperature_2m_min?.[0]),
            precipProb: Number(fc?.daily?.precipitation_probability_max?.[0]),
            codeToday: Number(fc?.daily?.weather_code?.[0])
        };
    } catch {
    ...
    _connectMintInflight = (async () => {
        try {
            const res = await fetch(`${CONNECT_API_URL}/v1/keys`, {
                ...
                body: JSON.stringify({ installId: getConnectInstallId() }),
                signal: AbortSignal.timeout(15000)
            });
