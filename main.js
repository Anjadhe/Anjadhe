/**
 * nenva - Electron Main Process
 */

const { app, BrowserWindow, Menu, Tray, nativeImage, ipcMain, dialog, systemPreferences, powerMonitor, powerSaveBlocker, Notification, ShareMenu, utilityProcess, protocol } = require('electron');
...
// Chrome starts a tiny Native Messaging child, not a second application.
// Keep stdout exclusively for framed messages and never open user stores here.
const browserHostArg = process.argv.indexOf('--anjadhe-browser-host');
if (browserHostArg !== -1) {
    const configPath = process.argv[browserHostArg + 1];
    if (!configPath) { process.exit(1); }
    const hostData = path.join(path.dirname(configPath), 'host-process');
    fs.mkdirSync(hostData, { recursive: true, mode: 0o700 });
    app.setPath('userData', hostData);
    app.whenReady().then(() => app.dock?.hide());
    require('./js/main/browser/native-host').run(configPath, process.argv[browserHostArg + 2]);
    return;
}
...
// answers disagree — the loudest bot signal there is. The removed Browse
// app used to pin Chrome/124 while the engine had moved on to 144: verified
// 2026-08-02, zillow.com served a PerimeterX 403 on the pinned UA and
// loaded normally on the derived one.
//
// So the UA is DERIVED from the running engine. What still reads it is the
// news fetcher (`_GNEWS_UA`) — a plain server-side fetch, not a browser —
// and nothing here needs touching on an Electron upgrade. `Chrome/<major>.0.0.0` is the reduced
...
if (DATA_ROOT) {
    ...
    // The product is called nenva, but its data folder keeps the old name:
    // userData follows productName, so without this pin a renamed build would
    // open an empty ~/Library/Application Support/nenva instead of the user's.
    app.setPath('userData', path.join(app.getPath('appData'), 'Anjadhe'));
}

// The user-content folder, ~/nenva (was ~/nenva; js/main/content-root.js
// moves an old one once, leaving a link). Decided here, before any store,
// watcher or file handle can touch the old path.
const ContentRoot = require('./js/main/content-root');
const CONTENT_ROOT = ContentRoot.resolveRoot();

// The INTERNAL app name stays "Anjadhe" (the visible one is productName,
// "nenva"). Chromium names the Keychain item that protects every secret
// "<app name> Safe Storage", so this keeps "Anjadhe Safe Storage" and every
// saved key readable. Menus that would print app.name name nenva explicitly
// (APP_DISPLAY_NAME). Must run before app ready.
const APP_DISPLAY_NAME = 'nenva';
app.setName('Anjadhe');
...
// ── Updater ──
// Checks GitHub releases (the public nenva repo) for a newer version via
...
const UpdaterManager = {
    _wired: false,
    // Interval between background re-checks. nenva is left open for days,
    ...
    // The newest release the license gate declined to fetch, if any —
    // {version, reason, updatesUntil}; rehydrated by state() like the pill.
    _blocked: null,
    // The download in flight, if any — {version, percent, transferred,
    // total}; rehydrated by state() so a window opened mid-download shows
    // the progress strip instead of nothing. Cleared on downloaded/error.
    _downloading: null,
    ...
    start() {
        ...
        if (!this._wired) {
            ...
            // Download is decided per release, not automatic: the license
            // gate (LicenseStore.updateAllowed) reads the release date from
            // the feed and either starts the download or tells the renderer
            // why not. autoInstallOnAppQuit still handles a finished download.
            autoUpdater.autoDownload = false;
            autoUpdater.autoInstallOnAppQuit = true;

            autoUpdater.on('update-available', (info) => {
                console.log('[updater] update available:', info?.version);
                const gate = LicenseStore.updateAllowed(info?.releaseDate || null);
                if (!gate.allowed) {
                    // Paid year over, or an unlicensed trial that ended. The
                    // build is not fetched; the pill names the reason and
                    // opens Settings › License. Nothing else changes.
                    console.log(`[updater] ${info?.version} not offered: ${gate.reason}`);
                    this._blocked = { version: info?.version || null, reason: gate.reason, updatesUntil: gate.updatesUntil || null };
                    broadcastToAllWindows('updater:blocked', this._blocked);
                    return;
                }
                this._blocked = null;
                this._downloading = { version: info?.version || null, percent: 0, transferred: 0, total: 0 };
                broadcastToAllWindows('updater:available', { version: info?.version });
                autoUpdater.downloadUpdate().catch((e) => {
                    console.warn('[updater] download failed:', e?.message || e);
                    this._downloadFailed(e);
                });
            });
            autoUpdater.on('download-progress', (p) => {
                const prog = {
                    version: this._downloading?.version || null,
                    percent: Math.round(p?.percent || 0),
                    transferred: p?.transferred,
                    total: p?.total
                };
                this._downloading = prog;
                broadcastToAllWindows('updater:progress', prog);
            });
            ...
            autoUpdater.on('error', (err) => {
                ...
                // Only a download in flight has a strip to take down; a
                // failed metadata check stays a log line, as before.
                if (this._downloading) this._downloadFailed(err);
            });
    ...
    // A download that will not finish: clear the in-flight record and tell
    // the renderer, so the progress strip does not sit at 43% forever. The
    // periodic re-check tries again later.
    _downloadFailed(err) {
        const version = this._downloading?.version || null;
        this._downloading = null;
        broadcastToAllWindows('updater:download-error', { version, message: String(err?.message || err || '').slice(0, 200) });
    },

    // Snapshot for a freshly-opened window to rehydrate its pill or strip.
    state() {
        return { downloadedVersion: this._downloadedVersion, blocked: this._blocked || null, downloading: this._downloading || null };
    },
...
// Background preferences are machine-local, independent of the UI feature flag.
const BackgroundMode = require('./js/main/background-mode');
const backgroundMode = new BackgroundMode({ app, ipcMain, Tray, Menu, nativeImage,
    store: settingsStore, platform: process.platform, isolated: !!DATA_ROOT,
    openWindow: () => createWindow() });
// Squirrel closes windows before app's before-quit during update installation.
require('electron').autoUpdater.on('before-quit-for-update', () => { backgroundMode.quitting = true; });

// Workrooms are local execution state, never a synced multi-Mac lock.
// An append-only event log per room (js/main/workroom-store.js); every
// append is PUSHED to the windows as a delta, nothing polls.
let workroomStore;
function broadcastWorkrooms(delta) {
    for (const win of backgroundMode.windows.values()) if (!win.isDestroyed()) win.webContents.send('workrooms-event', delta || {});
}
function getWorkroomStore() {
    if (!workroomStore) workroomStore = new (require('./js/main/workroom-store'))(path.join(app.getPath('userData'), 'workrooms'),
        { legacyFile: path.join(app.getPath('userData'), 'workrooms.json'), onEvent: broadcastWorkrooms });
    return workroomStore;
}
/** The user (or a disconnect) took the browser: stop the room that had it. */
function pauseBrowserWork(reason) {
    const holder = workroomBrowser.holder;
    if (holder && getWorkroomStore().leases.has(holder)) getWorkroomStore().command('pause', { id: holder, reason });
    workroomBrowser.holder = null;
}
const chromeBrowser = new (require('./js/main/browser/chrome-runtime').ChromeRuntime)({
    userData: app.getPath('userData'), source: path.join(__dirname, 'browser-extension'),
    operations: path.join(__dirname, 'js/main/browser/page-operations.js'), settings: settingsStore,
    hostCommand: config => [process.execPath, ...(!app.isPackaged ? [app.getAppPath()] : []), '--anjadhe-browser-host', config],
    /* A room stops for a TAKEOVER or a lost connection — never for every drop
     * out of agent control. Chrome hands control back for incidental reasons
     * too (the tab closed, the extension reconnected), and pausing on those
     * told the user they were driving when they had only looked. Whatever the
     * reason, the agent's next step takes the wheel again and says what it
     * finds ("the browser is on a blank page"). */
    onChange: ({ reason = null } = {}) => {
        if (reason === 'disconnect') pauseBrowserWork('Chrome disconnected. Reconnect the browser, then Continue from its current page.');
        else if (reason === 'takeover') pauseBrowserWork('You have control of Chrome. Come back here and Continue when ready.');
        broadcastWorkrooms({ browser: true });
    }
});
const workroomBrowser = new (require('./js/main/workroom-browser'))({ store: getWorkroomStore, settings: settingsStore, chrome: chromeBrowser });
function releaseChromeControl() {
    // The extension answers this with reason 'app', which pauses nothing.
    return chromeBrowser.takeover();
}
// Reconnect a previously selected profile without launching Chrome or a task.
chromeBrowser.prepare().catch(() => {});
app.on('will-quit', () => chromeBrowser.close());
ipcMain.handle('agent-browser', async (event, action, input = {}) => {
    if (!backgroundMode.windows.has(event.sender.id) || event.senderFrame !== event.sender.mainFrame) throw new Error('App window required');
    if (action === 'state') return workroomBrowser.state();
    if (action === 'chrome-state') return chromeBrowser.state();
    if (action === 'chrome-prepare') return chromeBrowser.prepare();
    if (action === 'chrome-launch') return chromeBrowser.launch({ setup: !!input.setup });
    if (action === 'chrome-folder') { await chromeBrowser.prepare(); require('electron').shell.showItemInFolder(path.join(chromeBrowser.extension, 'manifest.json')); return { ok: true }; }
    if (action === 'ensure') return chromeBrowser.ensure();
    if (action === 'view') return workroomBrowser.view(input.roomId);
    if (action === 'input') return workroomBrowser.input(input.roomId, input.event);
    if (action === 'trusted-sites') return workroomBrowser.trustedSites();
    if (action === 'revoke-site') return workroomBrowser.revoke(input.origin);
    if (action === 'execute') return workroomBrowser.execute(event.sender.id, input);
    if (action === 'release') {
        const held = workroomBrowser.holder === input.run?.id;
        workroomBrowser.release(input.run);
        if (held) await releaseChromeControl().catch(() => {});
        return { ok: true };
    }
    if (action === 'takeover') {
        const reason = input.here ? 'You are driving the browser. Choose Continue when you are done.' : 'You have control of Chrome. Come back here and Continue when ready.';
        pauseBrowserWork(reason);
        // Driving from the chat stops THAT chat's team even between browser
        // jobs, or its next delegate would take the wheel back mid-keystroke.
        if (input.roomId && getWorkroomStore().leases.has(input.roomId)) getWorkroomStore().command('pause', { id: input.roomId, reason });
        await chromeBrowser.takeover();
        return { ok: true };
    }
    if (action === 'show') return chromeBrowser.launch();
    throw new Error('Unknown browser command');
});
ipcMain.handle('workrooms', async (event, action, input) => {
    if (!backgroundMode.windows.has(event.sender.id) || event.senderFrame !== event.sender.mainFrame) throw new Error('App window required');
    const store = getWorkroomStore();
    if (action === 'list') return store.snapshot();
    if (action === 'events') return store.events(input?.id, Number(input?.after) || 0);
    if (action === 'attachment') return store.attachment(input?.id, input?.fileId);
    if (action === 'pause') input = { id: input?.id }; // System pause reasons originate in main only.
    if (['claim', 'commit'].includes(action) && !backgroundMode.state(event.sender.id).owner) throw new Error('Background owner required');
    const heldBrowser = workroomBrowser.holder === input?.id;
    const result = store.command(action, input, event.sender.id);
    // A room that stopped (paused, finished, deleted) lets go of the browser.
    if (heldBrowser && !store.leases.has(input?.id)) { workroomBrowser.holder = null; await releaseChromeControl().catch(() => {}); }
    if (action === 'delete') workroomBrowser.forget(input?.id);
    return result;
});
...
// The renderer asks for the server once per launch, only with the
// `terminal` feature flag on (the mobilesync 'channel-ensure' pattern).
ipcMain.handle('cli-ensure', () => CLIServer.ensure());
ipcMain.handle('cli-suspend', () => CLIServer.suspend());
...
// The renderer caches parsed blobs (js/core/storage-manager.js): a sync
// read of `app_notes` marshals ~7 MB across sendSync each time, and home
// used to do that fourteen times per paint. Every write to the kv store —
// a window's own set, the iCloud merge, a phone change, a startup
// migration — names its KEY to every open window so the cached copy is
// dropped. Key names only, never values; null means "everything".
function notifyStoreKeyChanged(key) {
    try { broadcastToAllWindows('store-key-changed', key); } catch { /* before any window */ }
}
...
const dataStore = {
    ...
    /** True when `value` serializes to exactly the stored text (see store-set-sync). */
    isUnchanged(key, value) {
        const row = dataDb.prepare('SELECT value FROM kv WHERE key = ?').get(key);
        if (!row) return false;
        try { return row.value === JSON.stringify(value); } catch { return false; }
    },
    set(key, value) {
...
function restoreFromBackup(backupPath) {
    try {
        ...
        try {
            ...
            // Every merge watermark describes a decision made against the DB
            // we just replaced — drop them so the next merge re-reads the
            // other Macs' journals in full.
            clearMergeWatermarks();
...
//
// Phones run the same migrations from js/adapter/native-bridge.js so a
...
const SYNC_EXCLUDE_KEYS = new Set([
    ...
    'app_email',
    // iMessage insight source: this Mac's Messages database is the source
    // of truth and the records carry message text — read here, stays
    // here. Its analyses live in the machine-local email_analyses table.
    'app_imessage-insights',
...
const RECORD_MERGED_KEYS = new Set([
    ...
    'app_agent-decisions',
    // Project/task Updates (2026-09-10, docs/GOALS.md "Updates"): the dated
    // log the user and the assistant post on a project or task. Written
    // from chat and detail pages on any Mac, mirrored from review posts —
    // the decisions shape exactly. updatedAt stamps + tombstones.
    'app_updates',
    // Spending (2026-09-09, docs/SPENDING.md): linked bank/card accounts,
    // their transactions (keyed by Plaid's transaction id) and the user's
    // per-merchant category overrides. The linking Mac writes every sync;
    // the other Mac edits categories — record union + tombstones.
    'app_spending',
    // Noticing moments (2026-09-12, docs/NOTICING.md N10): what the app
    // noticed in the user's own writing, and — from P1 — whether they told
    // it to stop. A dismissal that does not hold on the other Mac is the
    // most damaging bug this feature can have, so the records carry
    // updatedAt stamps and every removal tombstones.
    'app_noticing',
    ...
    // stamps + tombstones.
    'app_library',
    // Assistant conversations (2026-09-25, docs/MOBILE_NATIVE.md "M5" — one
    // conversation list across Macs and the phone). Whole-key LWW let the
    // Mac that saved last erase the other side's chats, and the phone could
    // not write a turn at all without losing it on the next sync. Each
    // conversation is its own record (AgentService stamps updatedAt in its
    // one save funnel and tombstones removed ones there too); two copies of
    // the SAME conversation also union their messages (_mergeConversation).
    'app_agent-conversations'
]);
const RECORD_MERGE_ARRAYS = {
    app_portfolio: ['accounts', 'transactions', 'properties', 'liabilities', 'strategies', 'watchlist', 'shares'],
    ...
    // Scalar-free: updates + tombstones only.
    'app_updates': ['updates'],
    // Scalar-free too. links carry the per-institution sync cursor +
    // lastSyncAt (stamped on every pull), accounts the bank's accounts and
    // balances, transactions the bank's rows (Plaid's id is the record id),
    // rules the user's merchant → category overrides (stamped on edit).
    'app_spending': ['links', 'accounts', 'transactions', 'rules'],
    // Scalar-free: moments + tombstones. A moment's id IS its identity
    // (source record + content hash), so two Macs noticing the same entry
    // write the same record and the union folds them into one.
    'app_noticing': ['moments'],
    // Also scalar-free by design. docTags (2026-09-02): the Documents
    // app's tags, keyed by document id, tombstoned on delete.
    'app_library': ['voicePages', 'exemplars', 'docTags'],
    'app_agent-conversations': ['conversations']
};

// Two copies of ONE conversation (the phone answered while the Mac was
// away, or two Macs both continued it): the newer copy wins every field, and
// the older copy's messages the winner does not have are KEPT, appended in
// their own order — losing a turn is the failure this exists to prevent.
// A message is identified by role + content + its occurrence among equal
// ones (no stored ids: two sides must agree on identity for messages written
// before this existed). `removedMessages` is the conversation's own
// tombstone list — editLastUserMessage records what it cut there — so a
// stale copy cannot resurrect a deliberately removed turn. Deterministic in
// argument order, like every merge here: the winner is chosen first.
function _messageKeys(messages) {
    const seen = new Map();
...
function _mergeConversation(winner, loser) {
    if (!loser || !Array.isArray(loser.messages) || !Array.isArray(winner.messages)) return winner;
    const removed = new Set([...(winner.removedMessages || []), ...(loser.removedMessages || [])]);
    const wKeys = _messageKeys(winner.messages);
    const have = new Set(wKeys);
    const lKeys = _messageKeys(loser.messages);
    // The removal list only stops the OLDER copy's extras coming back; the
    // winner's own messages are its latest word (a turn cut and then re-sent
    // with the same text has the same key, and must stay).
    const extra = loser.messages.filter((m, i) => !have.has(lKeys[i]) && !removed.has(lKeys[i]));
    const removedList = [...removed].slice(-200);
    if (!extra.length && removedList.length === (winner.removedMessages || []).length) return winner;
    return {
        ...winner,
        messages: winner.messages.concat(extra),
        ...(removedList.length ? { removedMessages: removedList } : {}),
    };
}
...
function mergeRecordBlobs(key, a, b) {
    ...
    if (key === 'app_promptFeed') {
        ...
        // U3 quiet ledger (docs/ROUTINES_UX.md): per-routine lists of runs
        // that settled with nothing to say. Both Macs' schedulers write it
        // all day, exactly like `runs` — under the `{...b, ...a}` rule above
        // one side's list would regularly shadow the other's, which reads on
        // the detail as "this routine has not run", the one thing the ledger
        // exists to disprove. Union by stamp; nothing ever un-quiets a run,
        // so there is no tie-break. The cap is enforced here as well as in
        // the renderer, or a renderer save would resurrect its own pruning.
        const QUIET_MAX = 20;
        const quiet = {};
        for (const src of [b.quiet, a.quiet]) {
            if (!src || typeof src !== 'object') continue;
            for (const [rid, list] of Object.entries(src)) {
                if (!Array.isArray(list)) continue;
                const cur = quiet[rid] || (quiet[rid] = []);
                for (const e of list) {
                    if (!e || typeof e !== 'object' || !e.at) continue;
                    if (!cur.some(x => x.at === e.at)) cur.push({ at: e.at, reason: e.reason || '' });
                }
            }
        }
        for (const [rid, list] of Object.entries(quiet)) {
            list.sort((x, y) => (Date.parse(y.at) || 0) - (Date.parse(x.at) || 0));
            quiet[rid] = list.slice(0, QUIET_MAX);
        }
        out.quiet = quiet;
    }
    ...
    for (const name of (RECORD_MERGE_ARRAYS[key] || [])) {
        ...
        for (const r of Array.isArray(a[name]) ? a[name] : []) {
            ...
            const aWins = sa > so || (sa === so && JSON.stringify(r) >= JSON.stringify(o));
            if (key === 'app_agent-conversations') {
                map.set(r.id, aWins ? _mergeConversation(r, o) : _mergeConversation(o, r));
            } else if (aWins) map.set(r.id, r);
        }
...
// Per-peer-file merge watermarks (2026-09-20). `mergeFromOtherMachines` used
// to read, decrypt and compare EVERY journal file from every other Mac on
// every merge — and for a record-merged key that means three JSON.stringify
// passes over the whole blob (6.6 MB for notes on this developer's Mac) just
// to discover nothing changed. All of it runs synchronously on the main
// process, so while it runs every `store-get` the renderer makes is blocked:
// the app is not merely slow then, it is stuck. A file whose mtime and size
// are exactly what we merged last time cannot have anything new in it, so we
// skip it before the read.
//
// Machine-local (never synced), and deliberately easy to invalidate:
//   * stamped with the app version, so an upgrade that changes
//     SYNC_EXCLUDE_KEYS or the merge rules re-reads everything once;
//   * dropped outright by `restoreFromBackup`, which swaps the whole DB
//     under us and makes every previous merge decision meaningless.
// A file we skip is also a file we do not HEAL (the journal-drift repair
// below) — correct, because healing only has work to do when a merge does.
const MERGE_WATERMARK_KEY = 'syncMergeWatermarks';

function readMergeWatermarks() {
    try {
        const wm = settingsStore.get(MERGE_WATERMARK_KEY);
        if (!wm || typeof wm !== 'object' || wm.v !== app.getVersion()) return {};
        return (wm.files && typeof wm.files === 'object') ? wm.files : {};
    } catch { return {}; }
}

function writeMergeWatermarks(files) {
    try { settingsStore.set(MERGE_WATERMARK_KEY, { v: app.getVersion(), files }); }
    catch { /* a lost watermark costs one full merge, never correctness */ }
}

function clearMergeWatermarks() {
    try { settingsStore.delete(MERGE_WATERMARK_KEY); } catch { /* nothing to drop */ }
}
...
function mergeFromOtherMachines() {
    ...
    try {
        ...
        let mergedCount = 0;
        let skippedCount = 0;
        const wmSeen = readMergeWatermarks();
        // Rebuilt from the files we actually see, so vanished machines and
        // deleted keys prune themselves.
        const wmNext = {};
        ...
        for (const otherMachine of machines) {
            ...
            for (const file of files) {
                // Set once the file's identity is known; the `finally` below
                // carries it forward whatever exit the body takes — including
                // the record-merged branch's own `continue`.
                let wmKey = null;
                let stamp = null;
                let fileOk = true;
                try {
                    ...
                    wmKey = otherMachine + '/' + file;
                    stamp = stat.mtimeMs + ':' + stat.size;
                    if (wmSeen[wmKey] === stamp) { skippedCount++; continue; }
                    ...
                    // A file that threw keeps no watermark, so the next merge
                    // tries it again.
                    if (fileOk && stamp) wmNext[wmKey] = stamp;
                }
            }
        }

        writeMergeWatermarks(wmNext);
        // The duration is in the log because this runs synchronously on the
        // main process: however long it says is how long the renderer's store
        // reads were blocked.
        writeSyncLog(`Merge complete: ${mergedCount} changes from [${machines.join(', ')}]`
            + (skippedCount ? ` (${skippedCount} unchanged file(s) skipped)` : '')
            + ` in ${Date.now() - mergeStartedAt}ms`);
        return { merged: mergedCount, machines, skipped: skippedCount };
    } catch (err) {
...
function createWindow(hash = '') {
    const win = new BrowserWindow({
        ...
        webPreferences: {
            ...
            sandbox: false,
            // No <webview> anywhere in the app since the Web Browser app was
            // removed (2026-09-20). webviewTag stays off (Electron's default)
            // and the 'web-contents-created' handler below is the fail-closed
            // backstop if one is ever attached again.
    ...
    const hiddenLogin = backgroundMode.register(win);
    const workroomWindowId = win.webContents.id;
    const releaseWorkrooms = () => {
        if (workroomStore) {
            const ownedLease = [...workroomStore.leases.values()].some(lease => lease.sender === workroomWindowId);
            workroomStore.release(workroomWindowId);
            if (ownedLease) { workroomBrowser.holder = null; releaseChromeControl().catch(() => {}); }
            broadcastWorkrooms({});
        }
    };
    win.on('closed', releaseWorkrooms);
    win.webContents.on('render-process-gone', releaseWorkrooms);
    // Hash routing may emit loading events without replacing the renderer.
    // Only a real main-frame navigation invalidates its execution claims.
    win.webContents.on('did-start-navigation', details => {
        if (details.isMainFrame && !details.isSameDocument) releaseWorkrooms();
    });
    ...
    win.webContents.on('will-prevent-unload', (event) => {
        ...
        else backgroundMode.quitting = false;
    });
    ...
    const template = [
        ...
        },
        {
            ...
            submenu: [
                ...
                },
                {
                    label: 'New Window at…',
                    submenu: [
                        ['Notes', 'notes'], ['Agent', 'agent'], ['Schedule', 'schedule'],
                        ['Projects', 'goals'], ['Focus', 'focus'],
                        ['Journal', 'journal'], ['Email', 'email'], ['Bookmarks', 'bookmarks']
                    ].map(([label, hash]) => ({
                        label,
                        click: () => createWindow('#' + hash)
                    }))
                },
                { type: 'separator' },
                { role: 'close' }
        ...
        },
        {
            ...
            submenu: [
                ...
                // reply or a task run mid-flight — and Cmd+R
...
// ---------------------------------------------------------------------------
// User-built apps (docs/PLATFORM.md). Read-only listing of ~/nenva/apps —
...
function getUserAppsDir() {
    ...
    // tree too — real ~/nenva folders must never be opened (the law).
    return path.join(CONTENT_ROOT, 'apps');
}
...
// ---------------------------------------------------------------------------
// Content files (2026-08-26) — Notes and Journal as Markdown in ~/nenva/
// (docs/CONTENT_FILES.md). Phase 1+2: the store blob stays canonical and the
// sync transport; `ContentFiles` in the renderer projects every record to a
// file and imports outside edits back. This side is a thin, contained fs
// layer: two folders (Notes/, Journal/), each with an assets/ subfolder,
// flat filenames only, and a debounced watcher. The Mac is the ONLY writer
// of this folder that the app knows about; a phone never touches it.
// ---------------------------------------------------------------------------
const CONTENT_FILE_KINDS = { notes: 'Notes', journal: 'Journal' };
function getContentFilesRoot() {
    if (process.env.ANJADHE_CONTENT_DIR) return process.env.ANJADHE_CONTENT_DIR;
    return CONTENT_ROOT;
}
function getContentFilesDir(kind) {
    const sub = CONTENT_FILE_KINDS[kind];
    if (!sub) throw new Error(`unknown content kind "${kind}"`);
    return path.join(getContentFilesRoot(), sub);
}
// A file name is one path segment (assets are `assets/<segment>`), no dot
// prefix, no traversal. Everything else is refused before touching disk.
const CONTENT_FILE_NAME_RX = /^[^/\\\0]{1,200}$/;
function _contentFilePath(kind, name, asset = false) {
    const n = String(name || '');
    if (!CONTENT_FILE_NAME_RX.test(n) || n.startsWith('.') || n === '..') throw new Error(`bad file name "${n}"`);
    const dir = asset ? path.join(getContentFilesDir(kind), 'assets') : getContentFilesDir(kind);
    const full = path.join(dir, n);
    if (!full.startsWith(dir + path.sep)) throw new Error('path escapes folder');
    return full;
}
function contentFilesEnabled() {
    const v = settingsStore.get('contentFilesEnabled');
    return v === undefined ? true : !!v;
}

ipcMain.handle('content-files-get-enabled', () => ({ enabled: contentFilesEnabled(), root: getContentFilesRoot() }));
ipcMain.handle('content-files-set-enabled', (event, enabled) => {
    settingsStore.set('contentFilesEnabled', !!enabled);
    if (enabled) startContentFilesWatcher(); else stopContentFilesWatcher();
    return { enabled: contentFilesEnabled() };
});
ipcMain.handle('content-files-list', (event, kind) => {
    try {
        const dir = getContentFilesDir(kind);
        if (!fs.existsSync(dir)) return { exists: false, files: [] };
        const files = [];
        for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
            if (!ent.isFile() || ent.name.startsWith('.') || !/\.md$/i.test(ent.name)) continue;
            try {
                const st = fs.statSync(path.join(dir, ent.name));
                files.push({ name: ent.name, mtimeMs: st.mtimeMs, size: st.size });
            } catch { /* vanished mid-list */ }
        }
        return { exists: true, files };
    } catch (e) { return { error: e.message }; }
});
ipcMain.handle('content-files-read', (event, { kind, name } = {}) => {
    try {
        const full = _contentFilePath(kind, name);
        const st = fs.statSync(full);
        if (st.size > 5 * 1024 * 1024) return { error: 'file too large' };
        return { text: fs.readFileSync(full, 'utf8'), mtimeMs: st.mtimeMs };
    } catch (e) { return { error: e.message }; }
});
// Write is atomic (tmp + rename) so a watcher on the other side — Obsidian,
// iCloud — never reads a half-written note. Returns the resulting mtime so
// the renderer can tell its own write apart from a later outside edit.
ipcMain.handle('content-files-write', (event, { kind, name, text } = {}) => {
    try {
        const full = _contentFilePath(kind, name);
        ...
        _contentFilesSelfWrote(kind, name);
        return { mtimeMs: fs.statSync(full).mtimeMs };
    } catch (e) { return { error: e.message }; }
});
ipcMain.handle('content-files-rename', (event, { kind, from, to } = {}) => {
    try {
        const a = _contentFilePath(kind, from), b = _contentFilePath(kind, to);
        if (!fs.existsSync(a)) return { error: 'source missing' };
        if (fs.existsSync(b)) return { error: 'target exists' };
        fs.renameSync(a, b);
        _contentFilesSelfWrote(kind, from); _contentFilesSelfWrote(kind, to);
        return { mtimeMs: fs.statSync(b).mtimeMs };
    } catch (e) { return { error: e.message }; }
});
ipcMain.handle('content-files-remove', (event, { kind, name } = {}) => {
    try {
        const full = _contentFilePath(kind, name);
        if (fs.existsSync(full)) { _contentFilesSelfWrote(kind, name); fs.unlinkSync(full); }
        return { ok: true };
    } catch (e) { return { error: e.message }; }
});
ipcMain.handle('content-files-write-asset', (event, { kind, name, dataUrl } = {}) => {
    try {
        const full = _contentFilePath(kind, name, true);
        const m = /^data:([^;,]+);base64,(.*)$/s.exec(String(dataUrl || ''));
        if (!m) return { error: 'not a base64 data URL' };
        const buf = Buffer.from(m[2], 'base64');
        if (buf.length > 30 * 1024 * 1024) return { error: 'asset too large' };
        fs.mkdirSync(path.dirname(full), { recursive: true });
        if (!fs.existsSync(full) || fs.statSync(full).size !== buf.length) fs.writeFileSync(full, buf);
        return { ok: true, size: buf.length };
    } catch (e) { return { error: e.message }; }
});
ipcMain.handle('content-files-read-asset', (event, { kind, name } = {}) => {
    try {
        const full = _contentFilePath(kind, name, true);
        const st = fs.statSync(full);
        if (st.size > 30 * 1024 * 1024) return { error: 'asset too large' };
        const ext = path.extname(full).toLowerCase();
        const mime = { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.webp': 'image/webp', '.gif': 'image/gif', '.mp4': 'video/mp4', '.mov': 'video/quicktime', '.webm': 'video/webm' }[ext];
        if (!mime) return { error: 'unsupported asset type' };
        return { dataUrl: `data:${mime};base64,${fs.readFileSync(full).toString('base64')}` };
    } catch (e) { return { error: e.message }; }
});
ipcMain.handle('content-files-reveal', (event, kind) => {
    try {
        const dir = kind ? getContentFilesDir(kind) : getContentFilesRoot();
...
// Watcher: one per kind folder, debounced, events name the files that
// changed. Our own writes are remembered for a moment so the echo of a
// projection never reads as an outside edit (the renderer double-checks by
// content hash anyway — this is only to keep the event stream quiet).
const contentFilesWatchers = {};
const _contentSelfWrites = new Map();
function _contentFilesSelfWrote(kind, name) {
    _contentSelfWrites.set(`${kind}/${name}`, Date.now());
}
function startContentFilesWatcher() {
    if (!contentFilesEnabled()) return;
    for (const kind of Object.keys(CONTENT_FILE_KINDS)) {
        if (contentFilesWatchers[kind]) continue;
        const dir = getContentFilesDir(kind);
        try { fs.mkdirSync(dir, { recursive: true }); } catch { /* read-only home? the renderer reports */ }
        if (!fs.existsSync(dir)) continue;
        let pending = new Set();
        let timer = null;
        try {
            contentFilesWatchers[kind] = fs.watch(dir, { recursive: false }, (eventType, filename) => {
                if (!filename) return;
                const base = String(filename);
                if (base.startsWith('.') || /\.tmp-\d+$/.test(base) || !/\.md$/i.test(base)) return;
                const self = _contentSelfWrites.get(`${kind}/${base}`);
                if (self && Date.now() - self < 2500) return;
                pending.add(base);
                clearTimeout(timer);
                timer = setTimeout(() => {
                    const names = [...pending];
                    pending = new Set();
                    for (const win of BrowserWindow.getAllWindows()) {
                        win.webContents.send('content-files-changed', { kind, names });
                    }
                }, 700);
            });
        } catch (e) {
            console.error(`[content-files] watcher for ${kind} failed:`, e.message);
        }
    }
}
function stopContentFilesWatcher() {
    for (const kind of Object.keys(contentFilesWatchers)) {
        try { contentFilesWatchers[kind].close(); } catch { /* already closed */ }
        delete contentFilesWatchers[kind];
    }
}
...
// ~/nenva/apps/.anjadhe-schemas.json by refreshUserAppsDocs().
// ---------------------------------------------------------------------------
...
ipcMain.on('store-set-sync', (event, key, value) => {
    ...
    // A write whose content is byte-identical to what is stored is a no-op:
    // it must not re-stamp `modifiedAt` (a fresher stamp on an unchanged blob
    // is how a Mac that merely LOOKED at data outranks another Mac's real
    // edits in the LWW merge), must not journal, and must not nudge paired
    // phones. EmailApp.saveData re-saves its two settings keys on every mail
    // pass; before this check each pass pushed `data-changed` to the phone
    // and the phone synced in a loop for as long as mail was being analyzed.
    if (dataStore.isUnchanged(key, value)) { event.returnValue = true; return; }
...
ipcMain.handle('store-set', (event, key, value) => {
    value = mergedForWrite(key, value);
    if (dataStore.isUnchanged(key, value)) return true; // no-op write (see store-set-sync)
    dataStore.set(key, value);
    writeChangeJournal(key, value);
    return true;
});
...
ipcMain.handle('auth-prompt-touch-id', async () => {
    try {
        await systemPreferences.promptTouchID('unlock nenva');
        return { success: true };
    } catch (error) {
        return { success: false, error: error.message };
    }
...
ipcMain.handle('open-external', (event, url) => {
    // Only ever hand http/https (and, since 2026-08-25, mailto — the
    // "Contact the founder" links) to the OS. shell.openExternal with an
    // arbitrary scheme (file:, smb:, custom URI handlers) is a known
    // Electron escape — match the browse/email handlers' validation.
    const s = String(url || '').trim();
    if (!/^(https?:\/\/|mailto:[^\s/]+@)/i.test(s)) return { ok: false, error: 'unsupported scheme' };
    const { shell } = require('electron');
...
// CNBC's current headlines for Portfolio's "Current headlines" section
// (js/main/cnbc-headlines.js — section RSS feeds, plus CNBC's own site
// search for a symbol on a ticker page). Web access is the consent gate,
// like every other read of the web; the renderer's Settings › Portfolio
// switch decides whether anything asks at all.
const CnbcHeadlines = require('./js/main/cnbc-headlines');
ipcMain.handle('cnbc-headlines', async (event, opts = {}) => {
    if (!isWebSearchEnabled()) return { error: 'Web access is turned off (Settings › AI Assistant › Web Search).', code: 'web-off' };
    const names = (Array.isArray(opts?.names) ? opts.names : [])
        .map(n => String(n || '').trim().slice(0, 60)).filter(Boolean).slice(0, 3);
    try { return await CnbcHeadlines.fetchHeadlines({ symbol: opts?.symbol || null, names }); }
    catch (e) { return { error: 'CNBC could not be reached right now.' }; }
});
...
// (another Mac's nenva, via a "Your server" model entry) can use this
...
// so a fresh install grows ~/nenva/library only when the user opens it.
...
// The reader's AI-tidied rendering (display only; see LibraryStore.tidyGet).
ipcMain.handle('library-tidy-get', (event, { docId } = {}) => {
    try { return LibraryStore.tidyGet(docId) || { none: true }; } catch (e) { return { error: e.message }; }
});
ipcMain.handle('library-tidy-set', (event, { docId, text, coveredChars, model, method, pages, pagesDone } = {}) => {
    try { return LibraryStore.tidySet(docId, { text, coveredChars, model, method, pages, pagesDone }); } catch (e) { return { error: e.message }; }
});

// One page of a document as a JPEG data URL, for a vision-capable brain
// (js/main/page-render.js). 0-based page; images are page 0 of 1.
ipcMain.handle('library-page', async (event, { docId, page, maxEdge } = {}) => {
    try {
        const abs = LibraryStore.docAbsPath(docId);
        if (!abs) return { error: 'no such document' };
        const { renderPage, DEFAULT_MAX_EDGE } = require('./js/main/page-render.js');
        return await renderPage(abs, Math.max(0, parseInt(page, 10) || 0), Math.min(2000, Math.max(400, parseInt(maxEdge, 10) || DEFAULT_MAX_EDGE)));
    } catch (e) { return { error: e.message }; }
});
...
// One connection per request for the OpenAI-compatible helpers — NO
// keep-alive pooling. Node 22 (Electron 40) turns keep-alive on for the
// default agent, and llama-server closes its socket after a STREAMED reply;
// the next request the pool hands that socket to dies with ECONNRESET
// ("socket hang up") in milliseconds. On a 16 GB Mac the scheduler runs
// local jobs one at a time, so the email insight queued behind a chat
// turn ran the instant the stream ended and failed every time (2026-08-30,
// reproduced with a bare Node script: stream → 200, next call → ECONNRESET,
// the call after that → 200). A fresh TCP handshake on localhost costs
// nothing; a silently failed insight sweep costs the feature.
const LLM_HTTP_AGENT = new http.Agent({ keepAlive: false });
const LLM_HTTPS_AGENT = new https.Agent({ keepAlive: false });
...
function openaiRequest(cfg, messages, tools, maxTokens, format, think) {
    return new Promise((resolve) => {
        ...
        // which an escape-hatch schema needs. Servers that don't
        ...
        const req = lib.request({
            ...
            method: 'POST',
            headers,
            agent: lib === https ? LLM_HTTPS_AGENT : LLM_HTTP_AGENT
        }, (res) => {
            let data = '';
            ...
            res.on('end', () => {
                if (res.statusCode !== 200) {
                    ...
                    try {
                        ...
                        // nenva Connect throttle responses carry a machine-
...
// nenva Cloud ('anjadhe' engine): hosted open-weight inference on nenva
...
// Client-side pacing for nenva Cloud. Connect brakes every install at
...
// One chat/stream call against nenva Cloud with the same key self-heal as
...
async function anjadheChatCall(params, run) {
    const unreachable = (e) => ({
        error: `nenva Cloud is unreachable (${e.message}). Check your connection, or pick another model in Settings → AI Assistant.`
    });
    let cfg;
    ...
    // digests, run history) say nenva Cloud, not the weights du jour.
...
function toAnthropicPayload(messages, format) {
    ...
    for (const m of (messages || [])) {
        ...
        // Roles must alternate here, and a user turn can legitimately follow
        // tool results — a routine's synthesis turn ("research time is up,
        // write the post") lands exactly there when its round budget runs
        // out right after a tool call. Fold it into that user turn as one
        // more text block rather than emitting two user turns in a row,
        // which is the same thing the multimodal branch above does.
        const text = typeof m.content === 'string' ? m.content : JSON.stringify(m.content);
        const prevTurn = out[out.length - 1];
        if (prevTurn && prevTurn.role === 'user' && Array.isArray(prevTurn.content)) {
            prevTurn.content.push({ type: 'text', text });
        } else if (prevTurn && prevTurn.role === 'user' && typeof prevTurn.content === 'string') {
            prevTurn.content += '\n\n' + text;
        } else {
...
const SEARCH_PROVIDERS = {
    // nenva Connect (api.anjadhe.com): the built-in, no-signup option.
    ...
    // Searches route through nenva's server; the service stores usage
...
// Idle windows, including the retained menu bar window, allow system sleep.
// Only an in-flight model request takes a temporary sleep assertion below.

// --- Keep the Mac awake while an LLM request is in flight ---
// A long generation (a task turn can stream for many minutes
...
function isLocalLLMJob(params) {
    ...
    const provider = settingsStore.get('llmProvider', 'local');
    if (provider === 'custom' || provider === 'openai' || provider === 'anthropic' || provider === 'anjadhe') return false;
    return true; // 'local' (or anything unknown — the handlers run those on this Mac)
}
...
function clampBackgroundPrompt(params) {
    ...
    // L6 (docs/CLOUD_PRIVACY.md): calls and results are inseparable. The
    // old reverse walk dropped recent results while retaining their calls,
    // which made long background research invalid at the next API request.
    // A tool exchange also owns subsequent user turns (captured images,
    // synthesis instructions) until the next assistant turn.
    const groups = [];
    let exchange = null, queryGroup = null, latestUserGroup = null;
    for (let i = 0; i < msgs.length; i++) {
        const m = msgs[i];
        if (!m || m.role === 'system') continue;
        if (exchange && (m.role === 'tool' || m.role === 'user')) {
            exchange.indices.push(i);
            exchange.size += size(m);
            if (m.role === 'user') latestUserGroup = exchange;
        } else {
            const group = { indices: [i], size: size(m) };
            groups.push(group);
            if (m.role === 'user') queryGroup = latestUserGroup = group;
            const hasCalls = m.role === 'assistant' && (m.tool_calls?.length
                || m._anthropicContent?.some(block => block.type === 'tool_use'));
            exchange = hasCalls ? group : null;
        }
    }
    // Keep the active query as well as the newest evidence. Tool results
    // alone are invalid for Qwen's template and lose the task on every model.
    // A screenshot/synthesis user turn inside an exchange cannot replace the
    // initiating query. Also keep the latest user-bearing exchange: it may
    // contain a correction queued by the user during tool work.
    const protectedGroups = new Set([queryGroup, latestUserGroup, groups.at(-1)]);
    const removed = new Set();
    for (const group of groups) {
        if (total <= CLOUD_BG_PROMPT_CHARS) break;
        if (protectedGroups.has(group)) continue;
        for (const i of group.indices) removed.add(i);
        total -= group.size;
    }
    // Keep the latest exchange even when it alone exceeds the budget. Only
    // shorten ordinary string content, never IDs/arguments or raw Anthropic
    // blocks (the API requires their thinking signatures unchanged).
    // Copies keep subsequent iterations/retries from inheriting this trim.
    const marker = '\n[trimmed before sending off this Mac]';
    const keep = msgs.filter((_, i) => !removed.has(i));
    // Reduce observations before shortening the query or follow-up. A large
    // seat map must not consume the words specifying the venue/date/format.
    const trimOrder = keep.map((m, i) => i).sort((a, b) => Number(keep[a]?.role === 'user') - Number(keep[b]?.role === 'user'));
    for (const i of trimOrder) {
        const m = keep[i];
        if (total <= CLOUD_BG_PROMPT_CHARS || !m || m.role === 'system'
            || typeof m.content !== 'string'
            || (m.role === 'assistant' && m._anthropicContent?.length)) continue;
        const reduction = Math.min(total - CLOUD_BG_PROMPT_CHARS, Math.max(0, m.content.length - marker.length));
        if (!reduction) continue;
        total -= reduction;
        keep[i] = { ...m, content: m.content.slice(0, m.content.length - reduction - marker.length) + marker };
    }
...
const withLLMScheduler = (defaultClass, fn) => async (event, params) => {
    ...
    let preview = null;
    // A private assistant chat shows no preview (js/agent/private-chat.js P4).
    const privateChat = !!(params && params.privateChat);
    for (let i = msgs.length - 1; !privateChat && i >= 0; i--) {
    ...
    emitAIActivity({
        ...
        // What the work is ABOUT, in the caller's words (a routine's title,
        // a task's goal) — the tag alone said "Routine" for every routine,
        // which is how a 100-minute run went unnamed (2026-09-04). And the
        // handle a Stop button needs: 'conv:<id>' for a chat/headless turn,
        // 'task:<id>' for a task-mode run.
        subject: (params && typeof params.activitySubject === 'string') ? params.activitySubject.slice(0, 120) : null,
        runId: (params && typeof params.activityRunId === 'string') ? params.activityRunId : null,
        ts: Date.now()
    });
...
ipcMain.handle('llm-chat-stream', withLLMWake(withLLMScheduler('interactive', async (event, params) => {
    ...
    // force one side for a single call (no caller does today);
    ...
    const provider = (override === 'local' || override === 'custom')
        ? override
        : settingsStore.get('llmProvider', 'local');
    ...
    if (!override && params.engine === 'anjadhe') {
        try {
            ...
            return { error: e.message || 'nenva Cloud failed' };
        }
    }
    ...
    // Provider-routed nenva Cloud (callers that don't know about entries).
    if (provider === 'anjadhe') {
    ...
    // Everything else — 'local', or a legacy/unknown value — runs on THIS
    // Mac, and nowhere else. There is no fallback to a server or a cloud:
    // the brain is whichever entry the user picked (Settings → AI
    // Assistant), and traffic only ever goes where they pointed it. The
    // old 'auto' mode (local first, then the configured server) was
    // removed 2026-08-30 — it sent a failed local call to a server the
    // user had not chosen for it, and hid the local error behind "both
    // failed". The engine's own error now comes back verbatim.
    try {
...
// (no caller does today); everything else follows the global setting.
ipcMain.handle('llm-chat', withLLMWake(withLLMScheduler('background', async (event, params) => {
    ...
    const provider = (override === 'local' || override === 'custom')
        ? override
        : settingsStore.get('llmProvider', 'local'); // 'local' | 'custom' | 'openai' | 'anthropic' | 'anjadhe'
    ...
    async function runLlmChat() {
        ...
        // Provider-routed nenva Cloud — same shape as the streaming handler.
        if (provider === 'anjadhe') {
            return await anjadheChatCall({ model: settingsStore.get('cloudBrainModel', '') || ANJADHE_LLM_MODEL, jobClass: params.jobClass }, (cfg) =>
                openaiRequest(cfg, params.messages, params.tools, params.maxTokens, params.format, params.think));
        }
        ...
        // Everything else — 'local', or a legacy/unknown value — runs on
        // THIS Mac, and nowhere else (same rule as the streaming handler:
        // no server or cloud fallback, the engine's own error verbatim).
        try {
            const cfg = await llamaCppChatConfig(params);
            ...
            // runs on this Mac).
            if (localResult && !localResult.error) localResult.provider = 'local';
            if (localResult && localResult.error) console.error('[llm-chat] Local engine error:', localResult.error);
            return localResult;
        } catch (e) {
            console.error('[llm-chat] Local engine error:', e.message);
            return { error: e.message || 'Failed to connect to local backend' };
        }
    }
})));
...
ipcMain.handle('llm-get-settings', () => {
    return {
        provider: settingsStore.get('llmProvider', 'local'),
...
// insights, action filing — follow the brain without knowing
...
ipcMain.handle('llm-set-provider', (event, provider) => {
    ...
    // normalizes to local — there is no 'auto' (removed 2026-08-30): the
    // user chooses where the AI goes, and nothing falls through to a
    // destination they did not pick.
    const valid = (provider === 'local' || provider === 'custom'
        || provider === 'openai' || provider === 'anthropic' || provider === 'anjadhe') ? provider : 'local';
...
// Assistant screen tools (docs/COWORK_AGENT.md C13): capture this window and
// send it validated input steps. Renderer half: js/agent/screen-tools.js.
require('./js/main/screen-control').register({ ipcMain, BrowserWindow });
// Mac control (docs/COWORK_AGENT.md C14): the assistant operates OTHER apps
// through a native helper. Inert until the renderer's mac tools call it,
// which they register only behind the `maccontrol` flag.
require('./js/main/mac-control').register({ app, ipcMain, BrowserWindow, screen: () => require('electron').screen });
...
ipcMain.handle('search-get-status', () => {
    ...
    return {
        ...
        provider,
        providers,
        // Never chosen: no stored toggle AND no provider ever picked. The
        // assistant ships web_search in this state and the FIRST call's
        // standard approval dialog is the opt-in (the setup wizard's web
        // step was removed 2026-08-31); explicit off stays off.
        unset: settingsStore.get('webSearchEnabled', null) === null && provider === null
    };
});
...
// Master toggle (Settings › Web Search; also flipped by the assistant's
// first-search opt-in — the setup wizard's web step is gone, 2026-08-31).
// Stored as an explicit boolean from here on — isWebSearchEnabled only
// derives from provider presence for installs that predate the toggle.
...
// ~/nenva scope, persisted "always" grants, session grants, and one-shot
...
const AGENT_FS_WRITE_CAP = 5 * 1024 * 1024;  // per-file write cap
...
// Directories the assistant may touch without a grant. The content root
// (~/nenva) is the app's own workspace (apps, exports); the env overrides
// matter for isolated testing.
function _agentDefaultScopes() {
    const list = [CONTENT_ROOT];
...
// nenva's own secrets the way the fs tools are already forbidden from doing.
...
function _agentCheckAccess(cls, target, consumeOnce = false) {
    if (cls !== 'shell') {
        ...
        // deny prefixes, so a link that points into nenva's own storage is
        ...
        for (const deny of _agentDenyPrefixes()) {
            if (_agentPathInside(target, deny) || _agentPathInside(real, _agentRealPath(deny))) {
                return { decision: 'deny', reason: `${target} is inside nenva's own data/sync storage, which the assistant may never touch` };
            }
        }
        // The REAL path must land in a scope — a symlink out of ~/nenva no
        // longer counts as in-scope (scopes are realpath'd too, so a legitimately
        // symlinked ~/nenva still matches).
        ...
        // Hard-deny any shell command that references nenva's own storage —
        ...
        if (denied) return { decision: 'deny', reason: `${denied} is inside nenva's own data/sync storage, which the assistant may never touch` };
...
async function extractPdfText(bytes, name) {
    try {
        ...
        // The globals pdf.js evaluates at import (DOMMatrix) must exist
        // first — the packaged app ships without @napi-rs/canvas, pdf.js's
        // own Node polyfill source (see js/main/pdfjs-node-shims.js).
        require('./js/main/pdfjs-node-shims.js').installPdfjsShims();
        const pdfjs = await import('pdfjs-dist/legacy/build/pdf.mjs');
        // pdf.js TRANSFERS the array it is given (the caller's buffer is
        // detached, byteLength 0, when this returns). Hand it a copy so the
        // caller can still write the same bytes to a temp file for OCR.
...
// explicitly picked/dropped, so no path-permission check applies. A
// scanned PDF (no text layer) takes the same Vision OCR fallback as fs_read
// and read_url — through a temp file, because the Vision helper reads from
// disk. Before 2026-09-02 this path skipped OCR, so a scan attached in chat
// was refused as "no extractable text" even though the agent could read
// the very same file from disk.
ipcMain.handle('agent-pdf-extract', async (event, { data, name } = {}) => {
    ...
    if (extracted.error || !_pdfTextIsEmpty(extracted.text, extracted.pagesRead)) return extracted;
    const tmp = path.join(os.tmpdir(), `anjadhe-attach-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.pdf`);
    try {
        fs.writeFileSync(tmp, bytes);
        const withOcr = await _extractPdfWithOcrFallback(tmp, bytes);
        return withOcr.error ? extracted : { ...withOcr, name: extracted.name };
    } catch {
        return extracted;
    } finally {
...
// Undo may only touch paths outside nenva's own storage (same deny
...
// deny-prefix guarded so nothing can be steered at nenva's own storage.
...
// commands), and no references to nenva's own storage (same deny scan the
...
ipcMain.handle('agent-run-applescript', async (event, { script } = {}) => {
    ...
    if (denied) return { error: `${denied} is inside nenva's own data/sync storage, which the assistant may never touch` };
    const { execFile } = require('child_process');
    return new Promise((resolve) => {
        execFile('osascript', ['-e', src], { timeout: 60000, maxBuffer: 1024 * 1024 }, (err, stdout, stderr) => {
            ...
            if (err) {
                ...
                if (/-1743|not authorized to send Apple events/i.test(msg)) {
                    msg += '\nmacOS blocked nenva from controlling that app. Tell the user: open System Settings → Privacy & Security → Automation, find nenva, and enable the app you want controlled — then ask me again.';
                }
...
function _runRemindersHelper(command, input) {
    const { execFile } = require('child_process');
    return _remindersHelperBinary().then(bin => new Promise((resolve) => {
        ...
        const child = execFile(bin, [command], { timeout: 60000, maxBuffer: 8 * 1024 * 1024 }, (err, stdout) => {
            if (err) {
            ...
            try {
                ...
                if (parsed.error === 'access-denied') {
                    ...
                    resolve({ error: 'access-denied', message: `macOS blocked access to ${pane}. Open System Settings → Privacy & Security → ${pane} and enable nenva.` });
                } else resolve(parsed);
            } catch (e) {
                resolve({ error: `Helper returned unparseable output: ${String(stdout).slice(0, 200)}` });
            }
        });
        // Write commands take their payload on stdin (event text is not
        // argv material); read commands get an empty stdin and never block.
        try { child.stdin.end(input ? JSON.stringify(input) : ''); } catch (e) { /* helper exited first */ }
...
// Calendar write-through (2026-09-09): create / update / delete an event in
// this Mac's iCloud or local calendars through EventKit. The renderer
// re-reads the mirror right after, so there is no local edit state.
const APPLE_EVENT_OPS = { create: 'event-create', update: 'event-update', delete: 'event-delete' };
ipcMain.handle('apple-events-write', async (_e, op, payload) => {
    const cmd = APPLE_EVENT_OPS[op];
    if (!cmd) return { error: `unknown calendar write "${op}"` };
    return _runRemindersHelper(cmd, (payload && typeof payload === 'object') ? payload : {});
});
    ...
    return new Promise((resolve) => {
        execFile('osascript', ['-l', 'JavaScript', '-e', src],
            ...
            if (err) {
                ...
                if (/-1743|not authorized to send Apple events/i.test(msg)) {
                    resolve({ error: 'access-denied', message: 'macOS blocked nenva from reading Notes. Open System Settings → Privacy & Security → Automation, find nenva, and enable Notes.' });
                } else {
...
ipcMain.handle('agent-process-start', async (event, { command, cwd } = {}) => {
    ...
    return { processId: id, pid: child.pid, note: 'Started in the background. Poll with process_status (it returns output since your last check); stop with process_stop. It dies when nenva quits.' };
});
...
// fire several web_search calls at once — as can task-mode research
...
ipcMain.handle('web-search', async (event, { query, maxResults } = {}) => {
    ...
    // assistant, Discover, scheduled prompts — can send a query.
...
//
// Current headlines for the News app's topics. Routing follows the user's
// web-access provider choice: Connect users go through /v1/news (their IP
// and fetch timing never reach the news services; the server's per-topic
// cache serves everyone following that topic), BYOK users fetch the
// sources directly from this machine. Direct fetches are deliberately
// anonymous-shaped: Node fetch has no cookie jar, the User-Agent is a
// minimal static string (not Electron's), and nothing but the topic rides
// in the URL.
//
// SOURCES (2026-09-10): the user picks where headlines come from — Google
// News (the default), Bing News, Hacker News — on the Topics page, and
// every topic is fetched from each picked source. The registry, parsers
// and per-source fetch live in js/main/news-sources.js (Connect's
// lib/news.js is its twin); `sources` rides the IPC and the /v1/news
// body, and each item comes back stamped `via` with its source id.

const NewsSources = require('./js/main/news-sources');
const NEWS_MAX_TOPICS = 8;
// 20 per SOURCE per topic, matching Connect's cap: the News app's
// drill-in and headline search both read the full per-topic lists, and
// 10 rows starved them.
const NEWS_MAX_ITEMS_PER_TOPIC = NewsSources.MAX_ITEMS;

// Per topic, every picked source in parallel. A source that fails leaves
// the others' rows standing; the topic is `unavailable` only when EVERY
// source failed — the same shape Connect returns.
async function _newsDirect(topics, sources) {
    const out = await Promise.all(topics.map(async (topic) => {
        const per = await Promise.all(sources.map(async (id) => {
            try { return { items: await NewsSources.fetchTopicFrom(id, topic) }; }
            catch { return { error: true }; }
        }));
        const ok = per.filter(p => !p.error);
        if (!ok.length) return { topic, items: [], error: 'unavailable' };
        return { topic, items: NewsSources.mergeSourceItems(ok.map(p => p.items)) };
    }));
    return { topics: out, route: 'direct' };
}

async function _newsConnect(topics, sources) {
    await migrateConnectInstallId();
    let apiKey = getSearchApiKey('anjadhe');
    if (!apiKey) apiKey = await ensureConnectKey();
    const perTopicCap = NEWS_MAX_ITEMS_PER_TOPIC * sources.length;
    const call = async (key) => {
        const res = await fetch(`${CONNECT_API_URL}/v1/news`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key}` },
            body: JSON.stringify({ topics, sources }),
        ...
        // A Connect build from before sources existed answers Google-only
        // rows with no `via`; those read as Google, which they are.
        data.topics = data.topics.slice(0, NEWS_MAX_TOPICS).map(t => ({
            topic: String(t?.topic || '').slice(0, 80),
            // Kept so the surfaces can tell "nothing found" from "the
            // server could not fetch this one" and say which.
            ...(t?.error ? { error: 'unavailable' } : {}),
            items: (Array.isArray(t?.items) ? t.items : [])
                ...
                .map(it => ({
                    ...
                    sourceUrl: (typeof it?.sourceUrl === 'string' && /^https?:\/\//i.test(it.sourceUrl))
                        ? it.sourceUrl.slice(0, 300) : '',
                    publishedAt: typeof it?.publishedAt === 'string' ? it.publishedAt.slice(0, 40) : null,
                    via: NewsSources.SOURCE_IDS.includes(it?.via) ? it.via : 'google',
                    ...((typeof it?.discussionUrl === 'string' && /^https?:\/\//i.test(it.discussionUrl))
                        ? { discussionUrl: it.discussionUrl.slice(0, 300) } : {}),
                    ...(Number.isInteger(it?.points) && it.points >= 0 ? { points: it.points } : {})
                }))
        }));
        return data;
    };
...
ipcMain.handle('discover-news', async (event, { topics, sources } = {}) => {
    ...
    // The allowlist gate: unknown ids drop, nothing picked means Google
    // News — so Portfolio's ticker headlines and the market brief, which
    // never pass sources, keep reading the news index they were built on.
    const picked = NewsSources.normalizeSources(sources);
    // The provider choice IS the routing decision (docs/DISCOVER.md): a
    // Connect user's topics — followed interests, and in Portfolio the
    // holdings themselves — reach the news services from nenva's server,
    // never from this Mac's address. So when Connect fails there is NO
    // direct fetch from here: the reply says so and the surfaces show it.
    // (A silent direct fallback existed until 2026-09-03 and was widened
    // that morning to per-topic failures; both went the same day — "goes
    // only where you point it" does not survive a silent reroute.) Per-
    // topic failures inside a 200 travel through as `error` on the topic.
    if (provider === 'anjadhe') {
        try { return await _newsConnect(list, picked); }
        catch { return { error: 'nenva Connect could not fetch headlines right now.', code: 'connect' }; }
    }
    return _newsDirect(list, picked);
});


// ── nenva Connect (api.anjadhe.com) ───────────────────────────────────
...
async function _searchAnjadheAuto(query, maxResults) {
    ...
    try {
        ...
        return { error: `nenva Connect unavailable (${e.message}). You can add your own search key in Settings instead.`, provider: 'anjadhe' };
    }
    let out = await _searchAnjadhe(query, maxResults, apiKey);
    if (out && out.authFailed) {
        setSearchApiKey('anjadhe', '');
        try { apiKey = await ensureConnectKey(); }
        catch (e) { return { error: `nenva Connect unavailable (${e.message}). You can add your own search key in Settings instead.`, provider: 'anjadhe' }; }
...
async function _searchAnjadhe(query, maxResults, apiKey) {
    try {
        ...
        if (!res.ok) {
            const msg = data?.error || `nenva Connect error (${res.status})`;
...
// THE id for this Mac (2026-09-17). Analytics used to carry a second UUID of
// its own so usage could never be joined to Connect billing; keeping the two
// apart cost two checkboxes, two strings and a raw-vs-hash explanation on a
// card that is only about counters, so they were collapsed into this one.
//
// Two forms, one identity: `id` is the raw UUID, which is what goes on the
// wire (the analytics and feedback ingests hash on arrival); `idHash` is its
// SHA-256, the ONLY form the service stores and therefore the only one worth
// showing a user — every Connect table, and now analytics_daily, keys on it.
// Knowing the raw id is not enough to do anything: /v1/keys/migrate is
// Bearer-authed, so the key proves ownership, not the id.
//
// Minting is deliberate about ONE case: a Mac holding a Connect key but no
// UUID is a legacy install waiting for migrateConnectInstallId() to rename it
// server-side, and generating one here would strand its tier and usage. That
// Mac gets null until the migration lands.
ipcMain.handle('connect-install-id', () => {
    try {
        let id = settingsStore.get('connectInstallId');
        if (!id) {
            if (getSearchApiKey('anjadhe')) return { id: null, idHash: null };
            id = crypto.randomUUID();
            settingsStore.set('connectInstallId', id);
        }
        return { id, idHash: crypto.createHash('sha256').update(id).digest('hex') };
    } catch {
        return { id: null, idHash: null };
    }
});
...
// ── Connect: one authenticated call ──────────────────────────────────────
// Bearer-authed request to Connect with the same stale-key self-heal the
// search / news / usage paths each inlined: on 401 discard the stored key,
// re-mint once, retry once. Returns {status, data} — a non-2xx is NOT
// thrown, because Connect's house error shape ({error, code}) is what the
// caller acts on (quota, login_required, capacity…). Bodies are bounded
// before parsing (a hostile relay reply must not become an unbounded parse
// in the main process).
async function connectCall(path, { method = 'GET', body = null, timeoutMs = 20000, maxBytes = 4 * 1024 * 1024 } = {}) {
    await migrateConnectInstallId();
    let apiKey = getSearchApiKey('anjadhe') || await ensureConnectKey();
    const once = async (key) => {
        const res = await fetch(`${CONNECT_API_URL}${path}`, {
            method,
            headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${key}` },
        ...
        if (text.length > maxBytes) throw new Error('Connect response too large');
        let data = null;
        try { data = JSON.parse(text); } catch { /* handled by caller via status */ }
        return { status: res.status, data };
    };
    let r = await once(apiKey);
    if (r.status === 401) {
        setSearchApiKey('anjadhe', '');
        apiKey = await ensureConnectKey();
        r = await once(apiKey);
...
// ── Brokerage linking (Portfolio ↔ Connect ↔ Plaid) ─────────────────────
// The app never holds a Plaid credential: Connect keeps the sealed access
// token, this Mac keeps only the item id on the Portfolio account record.
// Link: ask Connect for a Hosted Link URL, open it in the DEFAULT BROWSER
// (the user signs in on the brokerage's own page; the deep-link scheme is
// packaged-only, so the renderer polls `brokerage-link-status` instead of
// waiting for a callback). Holdings and transactions are proxied through
// Connect and land in the renderer, which writes them into the synced
// portfolio blob like any user edit. Docs: docs/PORTFOLIO.md "Brokerage
// linking".
function brokerageResult(r) {
    if (r.status >= 200 && r.status < 300 && r.data) return r.data;
    const err = r.data?.error || `HTTP ${r.status}`;
    return { error: err, code: r.data?.code || (r.status === 401 ? 'auth' : 'http'), status: r.status };
}
// `products` names what the link may pull — ['investments'] for a
// brokerage, ['transactions', 'investments'] for a bank or card (Spending
// first; the brokerage half rides along as an optional product). Connect
// asks Plaid for exactly that list, so a brokerage-only link never bills
// Transactions. Omitted on a relink: the item keeps its products.
ipcMain.handle('brokerage-link-start', async (event, { itemId, products } = {}) => {
    try {
        const body = itemId ? { itemId } : {};
        if (Array.isArray(products) && products.length) body.products = products.map(String).slice(0, 4);
        const r = brokerageResult(await connectCall('/v1/brokerage/link', { method: 'POST', body }));
        if (r.error) return r;
        if (!/^https:\/\/(secure\.plaid\.com|[a-z0-9.-]*plaid\.com)\//i.test(String(r.url || ''))) {
            return { error: 'Connect returned an unexpected link address', code: 'bad_url' };
        }
        // ANJADHE_BROKERAGE_NO_BROWSER=1 hands the URL back instead of
        // opening it — for driving the flow against a mock Connect in tests.
        if (process.env.ANJADHE_BROKERAGE_NO_BROWSER) return { linkToken: r.linkToken, expiresAt: r.expiresAt || null, url: r.url };
        const { shell } = require('electron');
        await shell.openExternal(r.url);
        return { linkToken: r.linkToken, expiresAt: r.expiresAt || null };
    } catch (e) {
        return { error: e.message, code: 'connect' };
    }
});
ipcMain.handle('brokerage-link-status', async (event, { linkToken } = {}) => {
    if (!/^[A-Za-z0-9_-]{8,200}$/.test(String(linkToken || ''))) return { error: 'linkToken malformed', code: 'bad_request' };
    try { return brokerageResult(await connectCall(`/v1/brokerage/link/${linkToken}`)); }
    catch (e) { return { error: e.message, code: 'connect' }; }
});
ipcMain.handle('brokerage-items', async () => {
    try { return brokerageResult(await connectCall('/v1/brokerage/items')); }
    catch (e) { return { error: e.message, code: 'connect' }; }
});
ipcMain.handle('brokerage-holdings', async (event, { itemId } = {}) => {
    try { return brokerageResult(await connectCall('/v1/brokerage/holdings', { method: 'POST', body: { itemId }, timeoutMs: 40000 })); }
    catch (e) { return { error: e.message, code: 'connect' }; }
});
ipcMain.handle('brokerage-transactions', async (event, { itemId, startDate, endDate } = {}) => {
    try {
        return brokerageResult(await connectCall('/v1/brokerage/transactions', {
            method: 'POST', body: { itemId, startDate, endDate }, timeoutMs: 60000
        }));
    } catch (e) { return { error: e.message, code: 'connect' }; }
});
// Bank/card side (the Spending app): the item's accounts with balances,
// and Plaid's cursor-based transaction sync. The CURSOR is the app's to
// keep (on the Spending account record) — Connect stores nothing between
// calls, so the same page can be asked for again after a cut-off run.
ipcMain.handle('brokerage-accounts', async (event, { itemId } = {}) => {
    try { return brokerageResult(await connectCall('/v1/brokerage/accounts', { method: 'POST', body: { itemId }, timeoutMs: 40000 })); }
    catch (e) { return { error: e.message, code: 'connect' }; }
});
ipcMain.handle('brokerage-transactions-sync', async (event, { itemId, cursor } = {}) => {
    try {
        const body = { itemId };
        if (typeof cursor === 'string' && cursor) body.cursor = cursor.slice(0, 1024);
        return brokerageResult(await connectCall('/v1/brokerage/transactions/sync', { method: 'POST', body, timeoutMs: 60000 }));
    } catch (e) { return { error: e.message, code: 'connect' }; }
});
ipcMain.handle('brokerage-unlink', async (event, { itemId } = {}) => {
    try { return brokerageResult(await connectCall('/v1/brokerage/unlink', { method: 'POST', body: { itemId } })); }
    catch (e) { return { error: e.message, code: 'connect' }; }
});

// The nenva Cloud model catalog for the Settings picker: keyless (the
...
ipcMain.handle('anjadhe-llm-models', async () => {
    ...
    const fallback = { models: [{ id: ANJADHE_LLM_MODEL, label: 'nenva Cloud' }], fallback: true, host: CONNECT_API_URL };
    try {
...
<title>${oauthHtmlEscape(heading)} &mdash; nenva</title>
<style>
        ...
        const server = http.createServer(async (req, res) => {
            ...
            if (url.pathname === '/callback') {
                ...
                if (!returnedState || returnedState !== oauthState) {
                    ...
                    res.end(oauthResultPage('Authentication blocked', 'The response could not be verified. Please close this window and try connecting again from nenva.'));
                    server.close();
                    resolve({ success: false, error: 'OAuth state mismatch — this callback was not initiated by nenva' });
                    return;
                }
                handled = true;
                ...
                if (error) {
                    ...
                    res.end(oauthResultPage('Sign-in cancelled', 'No changes were made. You can close this window and return to nenva.'));
                    server.close();
                ...
                if (code) {
                    try {
                        ...
                        if (tokenData.error) {
                            res.writeHead(200, { 'Content-Type': 'text/html' });
                            res.end(oauthResultPage('Sign-in failed', 'Something went wrong during sign-in. You can close this window and try again from nenva.'));
                            server.close();
                        ...
                        if (!email) {
                            res.writeHead(200, { 'Content-Type': 'text/html' });
                            res.end(oauthResultPage('Could not read your account', 'Google did not return your email address. You can close this window and try again from nenva.'));
                            server.close();
                        ...
                        if (!saved) {
                            res.writeHead(200, { 'Content-Type': 'text/html' });
                            res.end(oauthResultPage('Could not save credentials', 'This Mac&rsquo;s keychain is unavailable, so the connection was not stored. You can close this window and try again from nenva.'));
                            server.close();
                        ...
                        res.writeHead(200, { 'Content-Type': 'text/html' });
                        res.end(oauthResultPage('Connected', `<strong>${oauthHtmlEscape(email)}</strong> is now linked to nenva. You can close this window and return to the app.`, true));
                        server.close();
                        resolve({ success: true, email });
                    } catch (err) {
                        res.writeHead(200, { 'Content-Type': 'text/html' });
                        res.end(oauthResultPage('Authentication error', 'Something went wrong. You can close this window and try again from nenva.'));
                        server.close();
...
//
// Inline images (2026-08-26): a picture pasted into the body arrives as an
// image part with a Content-ID and often NO filename, so the old
// filename-only rule dropped it and the agent read a mail that was mostly
// a screenshot as empty. Image parts with an attachmentId are kept even
// when nameless (synthesised name), and `inline`/`contentId` say which
// ones the body references (cid:) — get_email attaches those to a vision
// model automatically.
function extractAttachmentsMeta(payload) {
    ...
    (function walk(part) {
        ...
        if (part.body?.attachmentId && (part.filename || isImage)) {
            n++;
            const disp = getHeader(part.headers, 'Content-Disposition');
            const cid = getHeader(part.headers, 'Content-ID').replace(/^<|>$/g, '');
            const inline = /^\s*inline/i.test(disp) || (!!cid && !/^\s*attachment/i.test(disp));
            const ext = isImage ? ('.' + (mime.split('/')[1] || 'img').replace(/[^a-z0-9]/gi, '').toLowerCase()) : '';
            attachments.push({
                filename: part.filename || `inline-image-${n}${ext}`,
                mimeType: mime,
...
ipcMain.handle('agent-email-attachment-text', async (event, { account, messageId, attachmentId, filename, mimeType } = {}) => {
    try {
        ...
        // Images: same contract as agent-fs-read — never text, never base64
        // in the tool JSON. The agent loop lifts `images` into a vision turn
        // when the model can see, and says so when it can't.
        const imageMime = AGENT_FS_IMAGE_EXTS[ext]
            || (/^image\/(png|jpe?g|webp|gif)$/i.test(String(mimeType || '')) ? String(mimeType).toLowerCase().replace('jpg', 'jpeg') : null);
        if (imageMime) {
            if (buf.length > 8 * 1024 * 1024) return { error: `Image too large to attach (${Math.round(buf.length / 1048576)}MB — max 8MB)` };
            return {
                name,
                kind: 'image',
                images: [{ dataUrl: `data:${imageMime};base64,${buf.toString('base64')}` }],
                note: 'Image attachment. If it is attached below, analyze it directly.'
            };
        }
        ...
        if (buf.subarray(0, 8192).includes(0)) {
            return { error: `Cannot read "${name}" as text — it is a binary format. PDF, xlsx, docx, text and image (png/jpeg/webp/gif) attachments are readable.` };
        }
            ...
            if (url.pathname === '/callback') {
                ...
                if (code) {
                    try {
                        ...
                        res.writeHead(200, { 'Content-Type': 'text/html' });
                        res.end(oauthResultPage('Connected', `The calendar for <strong>${oauthHtmlEscape(email)}</strong> is now linked to nenva. You can close this window and return to the app.`, true));
                        server.close();
                        resolve({ success: true, email });
                    } catch (err) {
                        res.writeHead(200, { 'Content-Type': 'text/html' });
            ...
            if (url.pathname === '/callback') {
                ...
                if (code) {
                    try {
                        ...
                        res.writeHead(200, { 'Content-Type': 'text/html' });
                        res.end(oauthResultPage('Connected', `<strong>${oauthHtmlEscape(email)}</strong> is now linked to nenva with ${services.length} service${services.length === 1 ? '' : 's'} (${oauthHtmlEscape(services.join(', '))}). You can close this window and return to the app.`, true));
                        server.close();
...
// The event's call. Google Meet rides hangoutLink; Zoom/Teams add-ons and
// Meet alike list conferenceData.entryPoints: the 'video' entry is the link,
// 'phone' entries are dial-ins (number + PIN), 'more' is Google's page of
// further numbers. http(s) only for anything that becomes a link.
function calendarMeeting(ev) {
    const cd = ev.conferenceData || {};
    const points = Array.isArray(cd.entryPoints) ? cd.entryPoints : [];
    const video = points.find(p => p?.entryPointType === 'video' && p.uri);
    const url = video?.uri || ev.hangoutLink || '';
    if (!/^https?:\/\//i.test(url)) return null;
    const more = points.find(p => p?.entryPointType === 'more' && /^https?:\/\//i.test(p.uri || ''));
    return {
        url,
        label: cd.conferenceSolution?.name || (ev.hangoutLink ? 'Google Meet' : ''),
        dialIn: points
            .filter(p => p?.entryPointType === 'phone' && p.uri)
            .map(p => ({
                label: p.label || String(p.uri).replace(/^tel:/i, ''),
                uri: p.uri,
                pin: p.pin || p.accessCode || p.passcode || p.password || '',
            })),
        moreUrl: more?.uri || '',
        notes: cd.notes || '',
        meetingCode: video?.meetingCode || cd.conferenceId || '',
    };
}

function calendarPerson(p) {
    if (!p) return null;
    return { email: p.email || '', displayName: p.displayName || '', self: !!p.self };
}

// Where a working-location event says the user is working.
function calendarWorkingLocation(ev) {
    const w = ev.workingLocationProperties;
    if (!w) return '';
    if (w.type === 'homeOffice' || w.homeOffice) return 'Home';
    if (w.officeLocation) return w.officeLocation.label || w.officeLocation.buildingId || 'Office';
    if (w.customLocation) return w.customLocation.label || 'Elsewhere';
    return '';
}

// Bump when mapCalendarEvent starts carrying a new field: a stored sync
// token from an older mapping forces one full re-fetch, since incremental
// sync only returns events that changed and the rest would never gain it.
// 2 = meeting links, 3 = every readable field (2026-09-22).
const CALENDAR_MAP_VERSION = 3;
...
function mapCalendarEvent(ev, calendarId) {
    return {
        ...
        // Everything else Google says about the event (2026-09-22) — the
        // app should never show less than Google Calendar does.
        organizer: calendarPerson(ev.organizer),
        creator: calendarPerson(ev.creator),
        showAs: ev.transparency === 'transparent' ? 'free' : '',
        visibility: ev.visibility && ev.visibility !== 'default' ? ev.visibility : '',
        eventType: ev.eventType && ev.eventType !== 'default' ? ev.eventType : '',
        workingLocation: calendarWorkingLocation(ev),
        reminders: ev.reminders ? {
            useDefault: !!ev.reminders.useDefault,
            overrides: (ev.reminders.overrides || []).map(o => ({ method: o.method || 'popup', minutes: o.minutes }))
        } : null,
        attachments: (ev.attachments || [])
            .filter(a => /^https?:\/\//i.test(a.fileUrl || ''))
            .map(a => ({ title: a.title || '', url: a.fileUrl, mimeType: a.mimeType || '' })),
        sourceLink: ev.source && /^https?:\/\//i.test(ev.source.url || '')
            ? { title: ev.source.title || '', url: ev.source.url } : null,
        ...
        for (const calendarId of calendarIds) {
            ...
            if (res.nextSyncToken) {
                allTokens[tokenKey] = {
                    ...
                    v: mode === 'full' ? CALENDAR_MAP_VERSION : saved?.v
                };
                tokensDirty = true;
            }
...
// The production relay — hosted on nenva Connect since 2026-07-28 (the
...
// --- embedded LAN relay (2026-08-31) --------------------------------------
// The Mac runs its own relay (js/channel/lan-relay.mjs) bound to the local
// network, so a phone on the same Wi-Fi syncs DIRECTLY with the Mac instead
// of round-tripping through the hosted relay. Same protocol, same Noise
// channel on top — the phone tries the LAN addresses first and falls back
// to the hosted relay (the ladder lives in js/adapter/mobile-sync.js). The
// Mac's own host endpoint reaches it over loopback; the relay accepts the
// `host` role from loopback only.
const LAN_RELAY_PORT = Number(process.env.ANJADHE_LAN_RELAY_PORT) || 18436;
let lanRelay = null;
let lanRelayPort = 0;

// The LAN addresses a phone can dial right now. Recomputed on every call —
// the Mac's IP drifts with networks, and these are advertised both in the
// pairing QR and in-band on every sync-plan so the phone always holds the
// current ones.
// --- Tailscale Funnel: the relay reachable from anywhere (2026-09-08) ----
// With Tailscale on this Mac, `tailscale funnel` gives the embedded relay a
// public wss address on the Mac's MagicDNS name (TLS ends here; the phone
// needs nothing installed). js/main/tailscale.js owns the CLI; this is the
// cached answer, advertised FIRST in the url list so the 4-url cap in the
// offer / phone never drops it. Refreshed at channel start, on every
// Settings toggle and every 10 min — the CLI is spawnSync, never on the
// per-reply path.
const Tailscale = require('./js/main/tailscale');
let funnelUrl = null;
function refreshFunnelUrl() {
    try {
        const st = Tailscale.status(lanRelayPort);
        funnelUrl = (st && st.url) || null;
        return st;
    } catch (err) {
        funnelUrl = null;
        return { installed: false, url: null, error: (err && err.message) || String(err) };
    }
}

function getLanRelayUrls() {
    if (!lanRelayPort) return [];
    const urls = funnelUrl ? [funnelUrl] : [];
    try {
        const nets = require('os').networkInterfaces();
        for (const name of Object.keys(nets)) {
            for (const ni of nets[name] || []) {
                // Link-local (169.254.x) is the USB cable's private link or a
                // self-assigned address, not a network: a phone that stored it
                // "connected directly" only while plugged in, and every real
                // LAN/relay rung was skipped because that rung had won.
                if (ni.family === 'IPv4' && !ni.internal && !ni.address.startsWith('169.254.')) {
                    urls.push(`ws://${ni.address}:${lanRelayPort}`);
                }
            }
        }
    } catch { /* no interfaces — no LAN path */ }
    return urls.slice(0, 4);
}

// Fixed port first (a stable port keeps a phone's stored LAN address valid
// across Mac restarts — the no-internet-at-home case); ephemeral fallback
// if something squats it. A failure here degrades to relay-only, never
// blocks the channel.
async function startLanRelay() {
    if (lanRelay) return;
    try {
        const modUrl = require('url').pathToFileURL(
            path.join(__dirname, 'js', 'channel', 'lan-relay.mjs'),
        ).href;
        const { startLanRelay: start } = await import(modUrl);
        try {
            lanRelay = start(LAN_RELAY_PORT);
            lanRelayPort = await lanRelay.ready;
        } catch (err) {
            console.warn(`[channel] LAN relay port ${LAN_RELAY_PORT} unavailable (${err.code || err.message}) — using an ephemeral port`);
            lanRelay = start(0);
            lanRelayPort = await lanRelay.ready;
        }
        console.log(`[channel] LAN relay listening on :${lanRelayPort}`);
    } catch (err) {
        lanRelay = null;
        lanRelayPort = 0;
        console.warn('[channel] LAN relay not started:', (err && err.message) || err);
    }
}
...
// `dirty` (2026-09-25): keys the phone changed ITSELF since the Mac last took
// them. For a record-merged key that is a conflict whole-key timestamps
// cannot see — the Mac's copy is newer (it saved something else), yet the
// phone holds records the Mac has never seen (a turn answered while the Mac
// was away). Sending the Mac's copy down would erase them; instead the Mac
// asks for the phone's copy, merges it (handleSyncValues), and the merged
// value goes down on the next round. Other keys keep whole-key LWW. A phone
// build from before this sends no `dirty` and behaves exactly as it did.
function handleSyncManifest(phoneManifest, dirty) {
    ...
    const phoneDirty = new Set(Array.isArray(dirty) ? dirty.filter(k => typeof k === 'string').slice(0, 500) : []);
    ...
    for (const key of Object.keys(macSet)) {
        ...
        if (!theirs) {
            ...
            send[key] = mine;
        } else if (phoneDirty.has(key) && RECORD_MERGED_KEYS.has(key) && theirs !== mine.modifiedAt) {
            want.push(key);
...
function handleSyncValues(phoneValues) {
    ...
    for (const key of Object.keys(phoneValues || {})) {
        ...
        if (!incoming || !incoming.modifiedAt || SYNC_EXCLUDE_KEYS.has(key)) continue;
        const mine = macSet[key];
        // A record-merged value is MERGED, never compared: an older phone
        // copy can still carry records the Mac lacks (see handleSyncManifest
        // `dirty`), and the per-record stamps and tombstones decide the rest.
        const merges = RECORD_MERGED_KEYS.has(key) && !incoming.deleted;
        if (!merges && mine && new Date(mine.modifiedAt) >= new Date(incoming.modifiedAt)) continue;
...
// Advertise the Mac's current LAN relay addresses on every sync reply, so a
// phone that connected via the hosted relay still learns where to dial
// directly next time (IPs drift; the pairing QR's copy goes stale).
function withLanUrls(reply) {
    const urls = getLanRelayUrls();
    if (urls.length) reply.lanUrls = urls;
    return reply;
}
...
async function dispatchChannelRequest(message) {
    ...
    if (message.type === 'sync') {
        return withLanUrls(handleChannelSync(message.changes));
    }
    if (message.type === 'sync-manifest') {
        return withLanUrls(handleSyncManifest(message.manifest, message.dirty));
    }
    if (message.type === 'sync-values') {
        return handleSyncValues(message.values);
    }
    if (message.type === 'chat-send') {
        return handleChannelChat(message);
    }
    if (message.type === 'view-get') {
        return handleChannelView(message);
    }
    if (message.type === 'chat-answer') {
        return handleChannelAnswer(message);
    }
    return { ok: false, error: 'unsupported request type: ' + message.type };
}

// --- Mac-served views for the phone (2026-08-31) ---------------------------
// Lane 2 of the mobile restructure (docs/MOBILE_NATIVE.md): data that
// deliberately does NOT sync as a blob — email insights (the mailbox lives in
// SQLite, Gmail is source of truth), the News cache (machine-local), computed
// portfolio numbers (quotes are machine-local) — served to the phone as
// read-only digests on request. The renderer computes them (that's where
// EmailApp / NewsFeed / PortfolioApp live); main only correlates request to
// response. Unlike chat, a view is quick to build, so the channel request
// stays open and the answer IS the reply.
// The Portfolio and News families (2026-09-21) are each app's own scopes —
// the phone draws the desktop page, so it asks for the desktop's pages. The
// `-action` pair are the few writes a phone may make; like `home-answer`,
// the renderer decides what it will honour, never this list.
const MOBILE_VIEWS = new Set([
    'home', 'home-answer', 'insights', 'insight',
    'news', 'news-saved', 'news-article', 'news-action',
    'portfolio', 'portfolio-tickers', 'portfolio-ticker',
    'portfolio-strategy', 'portfolio-news', 'portfolio-action',
    'tasks', 'task', 'tasks-action',
    'chat-asks',
]);
const VIEW_TIMEOUT_MS = 30 * 1000;
const pendingViewReqs = new Map(); // reqId -> { resolve, timer, view }
let viewReqCounter = 0;

function handleChannelView(message) {
    const view = typeof message.view === 'string' ? message.view : '';
    const reqId = typeof message.reqId === 'string' ? message.reqId.slice(0, 64) : '';
    // Small, plain parameters for parameterised views (an insight's emailId,
    // a read flag). Anything non-object or oversized is dropped, never trusted.
    let params = null;
    if (message.params && typeof message.params === 'object' && !Array.isArray(message.params)) {
        try { if (JSON.stringify(message.params).length <= 2048) params = message.params; } catch { params = null; }
    }
    // Answered HERE, not by the renderer: the Connect key lives in main.
    if (view === 'cloud-access') return handleCloudAccess(reqId);
    if (!MOBILE_VIEWS.has(view)) return { type: 'view-error', view, reqId, error: 'unknown view' };
    if (BrowserWindow.getAllWindows().length === 0) {
        return { type: 'view-error', view, reqId, error: 'nenva is not open on your Mac.' };
    }
    const internalId = 'v' + (++viewReqCounter) + '_' + Date.now();
    return new Promise((resolve) => {
        const timer = setTimeout(() => {
            pendingViewReqs.delete(internalId);
            resolve({ type: 'view-error', view, reqId, error: 'Your Mac did not answer in time.' });
        }, VIEW_TIMEOUT_MS);
        pendingViewReqs.set(internalId, { resolve, timer, view, reqId });
        broadcastToAllWindows('mobile-view-request', { view, reqId: internalId, params });
    });
}

// --- The phone's own AI when the Mac is away (2026-09-25) -------------------
// docs/MOBILE_NATIVE.md "M5". The phone answers by itself only on a model the
// user CHOSE for it (the synced `phone-ai` key, set in Settings on either
// device; absent = off, POSITIONING.md "the phone's second model"). For
// nenva Cloud it needs this Mac's Connect key: ONE key per person, so a
// phone is never a second free allowance. It is handed over only on the
// Noise channel (a paired phone is the only possible asker), only while
// that choice is nenva Cloud, and never logged. The phone keeps it in its
// Keychain and asks again when Connect refuses it.
async function handleCloudAccess(reqId) {
    const view = 'cloud-access';
    let pref = null;
    try { pref = dataStore.get('app_phone-ai'); } catch { pref = null; }
    const fb = pref && typeof pref === 'object' ? pref.fallback : null;
    // The second use of the same key (M5 phase 2): this Mac's News goes
    // through nenva Cloud, so a phone building News while the Mac is away
    // must go there too — never straight to Google from the phone.
    const newsViaCloud = isWebSearchEnabled() && getActiveSearchProvider() === 'anjadhe';
    if ((!fb || fb.engine !== 'anjadhe') && !newsViaCloud) {
        return { type: 'view-error', view, reqId, error: 'No model is chosen for your phone.' };
    }
    try {
        await migrateConnectInstallId();
        const apiKey = await ensureConnectKey();
        if (!apiKey) throw new Error('no key');
        console.log('[channel] cloud-access: key handed to the paired phone');
        return { type: 'view-data', view, reqId, data: { apiKey, baseUrl: CONNECT_API_URL } };
    } catch (e) {
        return { type: 'view-error', view, reqId, error: 'nenva Cloud is unavailable right now (' + (e.message || 'unreachable') + ').' };
    }
}

ipcMain.handle('mobile-view-result', (_event, res) => {
    if (!res || typeof res.reqId !== 'string') return { ok: false };
    const p = pendingViewReqs.get(res.reqId);
    if (!p) return { ok: false }; // timed out or duplicate — already answered
    clearTimeout(p.timer);
    pendingViewReqs.delete(res.reqId);
    p.resolve(res.error
        ? { type: 'view-error', view: p.view, reqId: p.reqId, error: String(res.error) }
        : { type: 'view-data', view: p.view, reqId: p.reqId, data: res.data ?? null });
    return { ok: true };
});

// --- phone chat (2026-08-31) -----------------------------------------------
// The phone app as a remote channel to the assistant — the Telegram shape on
// our own encrypted channel. The text is the USER's own (Noise authenticates
// the paired phone), the run happens in the renderer (MobileChannel →
// AgentService.sendMessage, model calls live there), and the reply comes back
// as a `chat-reply` push via broadcastToPeers. The immediate response is only
// an ack: a model run takes longer than a request should stay open, and the
// reply also lands in the synced `app_agent-conversations` blob, so a phone
// whose socket dropped mid-run still gets the answer on its next sync.
// Message text is never logged here (the renderer's LLMLogger ledgers the
// model call — that is the disclosure surface).
function handleChannelChat(message) {
    const text = typeof message.text === 'string' ? message.text.trim() : '';
    if (!text) return { type: 'chat-error', text: 'Empty message.' };
    if (text.length > 8000) return { type: 'chat-error', text: 'Message too long.' };
    if (BrowserWindow.getAllWindows().length === 0) {
        return { type: 'chat-error', text: 'nenva is not open on your Mac.' };
    }
    broadcastToAllWindows('mobile-chat-message', {
        text,
        convId: typeof message.convId === 'string' ? message.convId.slice(0, 80) : null,
        fresh: message.fresh === true,
    });
    console.log('[channel] chat-send forwarded to renderer (' + text.length + ' chars)');
    return { type: 'chat-ack' };
}

// The renderer's MobileChannel answers here once the assistant run finishes;
// push the result to whichever paired phone is connected. If none is (the
// phone backgrounded mid-run), the synced conversation still carries the
// reply — the push is for immediacy, sync is the durability.
ipcMain.handle('mobile-chat-result', (_event, res) => {
    if (!res || typeof res !== 'object') return { ok: false };
    const msg = {
        type: res.ok ? 'chat-reply' : 'chat-error',
        convId: typeof res.convId === 'string' ? res.convId : null,
        text: typeof res.text === 'string' ? res.text : '',
    };
    const reached = desktopChannel ? desktopChannel.broadcastToPeers(msg) : 0;
    console.log('[channel] chat result pushed to ' + reached + ' peer(s) (' + msg.text.length + ' chars, ok=' + !!res.ok + ')');
    return { ok: true, reached };
});

// Approvals on the phone (2026-09-25, js/agent/mobile-channel.js): the
// renderer's ask goes to every connected phone as `chat-ask`; the phone's
// answer comes back as a `chat-answer` request and is handed to the renderer,
// which alone decides whether it matches a live ask. A settled ask is pushed
// as `chat-ask-done` so a second phone drops its card. Only the ask's own
// words travel — never logged here.
ipcMain.on('mobile-chat-ask', (_event, ask) => {
    if (!ask || typeof ask !== 'object' || typeof ask.askId !== 'string' || !desktopChannel) return;
    desktopChannel.broadcastToPeers({
        type: 'chat-ask',
        ask: {
            askId: ask.askId.slice(0, 80),
            convId: typeof ask.convId === 'string' ? ask.convId.slice(0, 80) : null,
            tool: String(ask.tool || '').slice(0, 80),
            text: String(ask.text || '').slice(0, 2000),
            note: String(ask.note || '').slice(0, 600),
            onceOnly: ask.onceOnly === true,
            at: typeof ask.at === 'string' ? ask.at.slice(0, 40) : null,
        },
    });
});
ipcMain.on('mobile-chat-ask-done', (_event, done) => {
    if (!done || typeof done.askId !== 'string' || !desktopChannel) return;
    desktopChannel.broadcastToPeers({ type: 'chat-ask-done', askId: done.askId.slice(0, 80), approved: done.approved === true });
});
function handleChannelAnswer(message) {
    const askId = typeof message.askId === 'string' ? message.askId.slice(0, 80) : '';
    if (!askId) return { type: 'chat-answer-ack', ok: false };
    if (BrowserWindow.getAllWindows().length === 0) return { type: 'chat-answer-ack', askId, ok: false };
    broadcastToAllWindows('mobile-chat-answer', {
        askId,
        approved: message.approved === true,
        scope: ['once', 'session', 'always'].includes(message.scope) ? message.scope : 'once',
    });
    return { type: 'chat-answer-ack', askId, ok: true };
}

// A slice of the answer while it is still being written (2026-09-21). The
// renderer coalesces these to a few a second (MobileChannel._streamTo); this
// only reseals and forwards them. Deltas are PURE UX — the finished answer
// still arrives as a `chat-reply` and still rides the synced conversation
// blob, so a phone that misses every delta loses nothing. The message text is
// never logged here, exactly as for a reply.
ipcMain.on('mobile-chat-delta', (_event, delta) => {
    if (!delta || typeof delta !== 'object' || !desktopChannel) return;
    const text = typeof delta.text === 'string' ? delta.text : '';
    if (!text) return;
    desktopChannel.broadcastToPeers({
        type: 'chat-delta',
        convId: typeof delta.convId === 'string' ? delta.convId.slice(0, 80) : null,
        seq: Number.isFinite(delta.seq) ? delta.seq : 0,
        // The same ceiling a whole reply gets; a delta is a slice of one.
...
async function initDesktopChannel() {
    ...
    try {
        ...
        await startLanRelay(); // best-effort — relay-only if it can't bind
        if (lanRelayPort) {
            refreshFunnelUrl();
            if (funnelUrl) console.log('[channel] Tailscale Funnel advertises the relay at', funnelUrl);
            setInterval(refreshFunnelUrl, 10 * 60 * 1000).unref();
        }
        desktopChannel = createDesktopChannel({
            ...
            lanRelayUrl: lanRelayPort ? `ws://127.0.0.1:${lanRelayPort}` : null,
            getLanUrls: getLanRelayUrls,
        ...
        console.log('[channel] hosting on relay', getChannelRelayUrl(),
            lanRelayPort ? `+ LAN :${lanRelayPort}` : '(no LAN relay)');
    } catch (err) {
...
// --- Mac<->Mac contacts: the sharing room (docs/PORTFOLIO_SHARING.md) ------
// The peer channel (js/channel/peer-channel.mjs) is the desktop channel's
// sibling for OTHER PEOPLE's Macs: same identity, its own relay room,
// contacts paired by invite code. Renderer-driven like the phone channel —
// main connects only when electronPeers.ensure() is called, which
// AppManager does only with the `sharing` flag on. Inbound messages are
// opaque here; they go to every window as `peers-message` and the app
// (PortfolioSharing) decides what they mean.
...
async function initPeerChannel() {
    ...
    try {
        const modUrl = require('url').pathToFileURL(
            path.join(__dirname, 'js', 'channel', 'peer-channel.mjs'),
        ).href;
        const { createPeerChannel } = await import(modUrl);
        peerChannel = createPeerChannel({
            storage: {
                get: (key) => settingsStore.get(key, null),
                set: (key, value) => settingsStore.set(key, value),
            },
            relayUrl: getChannelRelayUrl(),
            onMessage: (contactId, message) => broadcastToAllWindows('peers-message', { contactId, message }),
            onChanged: () => broadcastToAllWindows('peers-changed', {}),
        });
        await peerChannel.start();
        console.log('[peers] hosting the sharing room on', getChannelRelayUrl());
    } catch (err) {
        console.warn('[peers] not started:', (err && err.message) || err);
    }
}
function peersInfo() {
    if (!peerChannel) return { available: false, contacts: [], invites: [] };
    return {
        available: true,
        connected: peerChannel.isConnected(),
        ...peerChannel.getPublicInfo(),
        contacts: peerChannel.listContacts(),
        invites: peerChannel.listInvites(),
    };
}
ipcMain.handle('peers-ensure', () => { initPeerChannel(); return { ok: true }; });
ipcMain.handle('peers-info', () => peersInfo());
ipcMain.handle('peers-create-invite', (_event, label) => {
    if (!peerChannel) return { error: 'The sharing channel is not running.' };
    try { return { ok: true, invite: peerChannel.createInvite(String(label || '')) }; }
    catch (err) { return { error: (err && err.message) || String(err) }; }
});
ipcMain.handle('peers-cancel-invite', (_event, id) => { if (peerChannel) peerChannel.cancelInvite(String(id || '')); return { ok: true }; });
ipcMain.handle('peers-accept-invite', async (_event, code, label) => {
    if (!peerChannel) return { error: 'The sharing channel is not running.' };
    try { return { ok: true, contact: await peerChannel.acceptInvite(String(code || ''), String(label || '')) }; }
    catch (err) { return { error: (err && err.message) || String(err) }; }
});
ipcMain.handle('peers-rename-contact', (_event, id, label) => ({ ok: !!(peerChannel && peerChannel.renameContact(String(id || ''), String(label || ''))) }));
ipcMain.handle('peers-remove-contact', (_event, id) => { if (peerChannel) peerChannel.removeContact(String(id || '')); return { ok: true }; });
ipcMain.handle('peers-send', (_event, contactId, message) => {
    if (!peerChannel) return { ok: false, delivered: false, error: 'The sharing channel is not running.' };
    if (!message || typeof message !== 'object') return { ok: false, delivered: false, error: 'bad message' };
    return peerChannel.send(String(contactId || ''), message);
});

// Settings › Paired Devices › Reach from anywhere. Status is read-only;
// `set` runs `tailscale funnel` for the relay port (on) or clears that one
// port (off), then re-advertises.
ipcMain.handle('channel-remote-access-status', () => ({ relayPort: lanRelayPort, ...refreshFunnelUrl() }));
ipcMain.handle('channel-remote-access-set', (_event, on) => {
    if (!lanRelayPort) return { error: 'The Mac relay is not running' };
    let res;
    try { res = on ? Tailscale.enable(lanRelayPort) : Tailscale.disable(lanRelayPort); }
    catch (err) { res = { error: (err && err.message) || String(err) }; }
    const st = refreshFunnelUrl();
    return { relayPort: lanRelayPort, ...st, ...(res && res.error ? { error: res.error } : {}) };
});

// --- Telegram bridge (2026-08-31) ------------------------------------------
// Telegram as a remote channel to the assistant: the user's own bot token
// (encrypted, machine-local — one Mac polls per bot, so the token not syncing
// IS the single-Mac rule), a long-poll loop that dials out like everything
// else, and a one-time-code link to exactly one chat. Incoming text goes to
// the renderer (telegram-message), which runs AgentService.runHeadless and
// answers over telegram-send. See js/main/telegram-bridge.js.
...
ipcMain.handle('telegram-status', () => telegramBridge.status());
ipcMain.handle('telegram-set-token', (_e, token) => telegramBridge.configure(token));
ipcMain.handle('telegram-set-enabled', (_e, on) => { telegramBridge.setEnabled(!!on); return telegramBridge.status(); });
ipcMain.handle('telegram-begin-link', () => telegramBridge.beginLink());
ipcMain.handle('telegram-cancel-link', () => { telegramBridge.cancelLink(); return { ok: true }; });
ipcMain.handle('telegram-unlink', () => { telegramBridge.unlink(); return telegramBridge.status(); });
ipcMain.handle('telegram-disconnect', () => { telegramBridge.disconnect(); return telegramBridge.status(); });
ipcMain.handle('telegram-send', (_e, text) => telegramBridge.sendToLinked(text));
ipcMain.handle('telegram-set-notify', (_e, on) => { telegramBridge.setNotify(!!on); return telegramBridge.status(); });
ipcMain.handle('telegram-notify', (_e, title, body, kind) => telegramBridge.notifyToLinked(title, body, kind));
ipcMain.handle('telegram-typing', () => { telegramBridge.sendTyping(); return { ok: true }; });

// --- iMessage notification bridge (2026-09-01) -----------------------------
// Outbound only: this Mac's notifications, sent to the user's OWN iMessage
// handle by scripting Messages.app (Apple end-to-end, no bot, no server of
// ours). Receiving is designed and parked — see docs/IMESSAGE_CHANNEL.md
// and js/main/imessage-bridge.js. The script runner is the same osascript
// the Apple Notes import uses; text and handle travel as argv, never
// spliced into the source.
const { createIMessageBridge } = require('./js/main/imessage-bridge');
const imessageBridge = createIMessageBridge({
    settingsStore,
    runScript: (script, args) => new Promise((resolve) => {
        const { execFile } = require('child_process');
        execFile('osascript', ['-e', script, ...args], { timeout: 30000, maxBuffer: 256 * 1024 }, (err, _stdout, stderr) => {
            if (err) resolve({ error: String(stderr || err.message) });
            else resolve({ ok: true });
        });
    }),
});
ipcMain.handle('imessage-status', () => imessageBridge.status());
ipcMain.handle('imessage-set-notify', (_e, on) => { imessageBridge.setNotify(!!on); return imessageBridge.status(); });
ipcMain.handle('imessage-set-handle', (_e, handle) => imessageBridge.setHandle(handle));
ipcMain.handle('imessage-test', () => imessageBridge.sendTest());
ipcMain.handle('imessage-notify', (_e, title, body, kind) => imessageBridge.notifyToLinked(title, body, kind));
// The assistant's send_text (2026-09-10): any handle; consent was asked
// in the renderer (PermissionManager), main checks the shape and sends.
ipcMain.handle('imessage-send', (_e, handle, text) => imessageBridge.sendTo(handle, text));

// --- iMessage as an insight SOURCE (2026-09-10) ---------------------------
// Reads NEW texts from this Mac's Messages database (read-only, per call,
// Full Disk Access) so the renderer's IMessageSource can fold them into
// conversations for the same insight engine that reads mail. Message text
// crosses to the renderer and is never logged here. The per-Mac switch is
// the renderer's (Settings › Accounts › iMessage); main only answers when
// asked. Chat over iMessage stays parked — docs/IMESSAGE_CHANNEL.md.
const { createIMessageReader } = require('./js/main/imessage-reader');
const imessageReader = createIMessageReader({ Database });
ipcMain.handle('imessage-probe', () => imessageReader.probe());
ipcMain.handle('imessage-read', (_e, opts) => imessageReader.readSince(opts || {}));
// The insight detail's "Open in Messages" door: imessage://<handle> lands
// on that conversation; with no single handle (a group) Messages just
// opens. The handle is validated to the bridge's own shape so no other
// scheme or path can ride this.
ipcMain.handle('imessage-open-chat', (_e, handle) => {
    const { shell } = require('electron');
    const h = String(handle || '').trim();
    if (h && !/^(\+?[0-9]{5,20}|[^\s@/?#]+@[^\s@/?#]+\.[^\s@/?#]+)$/.test(h)) return { ok: false, error: 'unsupported handle' };
    shell.openExternal(h ? `imessage://${encodeURIComponent(h)}` : 'imessage://');
    return { ok: true };
});
...
async function startupBootstrap() {
    ...
    // Telegram bridge: reads its token through Secrets, so it may only start
    // here (post-ready) or later. No-op unless configured AND enabled.
    try { telegramBridge.start(); }
    catch (err) { console.warn('[telegram] start failed:', err.message); }
    // The app license (js/main/license-store.js): a per-Mac signed file under
    // userData, verified offline. Policy (alpha cutover, trial length) rides
    // remote-config `licensing`; RemoteConfig.get() is null-safe before load.
    try {
        LicenseStore.init({
            filePath: path.join(app.getPath('userData'), 'license.json'),
            settings: settingsStore,
            getPolicy: () => (RemoteConfig.get() || {}).licensing || {}
        });
    } catch (err) { console.warn('[license] init failed:', err.message); }
}

// ── App license IPC ─────────────────────────────────────────────────────
// Everything about the license stays in main: the file, the public key and
// the one network call (the alpha claim) that sends the email the user
// typed to Connect and brings the signed key back. The renderer only ever
// sees status objects. See docs/BUSINESS_MODEL.md "License".
ipcMain.handle('license-get', () => LicenseStore.status());

ipcMain.handle('license-set', (event, keyText) => {
    if (typeof keyText !== 'string' || keyText.length > 4000) return { error: 'That is not a license key.' };
    return LicenseStore.save(keyText);
});

ipcMain.handle('license-clear', () => LicenseStore.clear());

// Pick a license file (the key as plain text, or the {license} JSON the
// admin export writes) — same dialog mould as select-folder.
ipcMain.handle('license-open-file', async (event) => {
    const parent = BrowserWindow.fromWebContents(event.sender) || getActiveWindow();
    const result = await dialog.showOpenDialog(parent, {
        properties: ['openFile'],
        title: 'Open license file',
        buttonLabel: 'Open',
        filters: [{ name: 'nenva license', extensions: ['anjadhe-license', 'txt', 'json'] }, { name: 'All files', extensions: ['*'] }]
    });
    if (result.canceled || !result.filePaths[0]) return { canceled: true };
    try {
        const text = fs.readFileSync(result.filePaths[0], 'utf8').slice(0, 4000);
        return LicenseStore.save(text);
    } catch (e) {
        return { error: `Could not read that file: ${e.message}` };
    }
});

// Claim the free-for-good alpha license. The ONLY thing sent is the address
// the user typed (plus the app version, for the operator's list); Connect
// answers with the signed key, which is saved here and never leaves again.
// Re-claiming with the same address returns the same key — this is also
// how a re-install recovers it.
ipcMain.handle('license-claim-alpha', async (event, email) => {
    const addr = String(email || '').trim().toLowerCase();
    if (addr.length > 200 || !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(addr)) {
        return { error: 'That does not look like an email address.' };
    }
    try {
        const res = await fetch(`${CONNECT_API_URL}/v1/license/alpha`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email: addr, appVersion: app.getVersion() }),
            signal: AbortSignal.timeout(15000)
        });
        const data = await res.json().catch(() => null);
        if (!res.ok || !data?.license) {
            return { error: data?.error || `nenva Connect answered HTTP ${res.status}.`, code: data?.code || null };
        }
        const status = LicenseStore.save(data.license, addr);
        if (status.error) return status;
        return { ...status, created: data.created === true };
    } catch (e) {
        return { error: `Could not reach nenva Connect (${e.message}). Check the connection and try again.` };
    }
});

// Whether the alpha claim is open, from the server that would honour it.
// Best-effort: offline, the card falls back to the local policy.
ipcMain.handle('license-remote-status', async () => {
    try {
        const res = await fetch(`${CONNECT_API_URL}/v1/license/status`, { signal: AbortSignal.timeout(8000) });
        if (!res.ok) return null;
        const d = await res.json();
        return { enabled: d.enabled === true, alphaOpen: d.alphaOpen === true, alphaClosesAt: d.alphaClosesAt || null };
    } catch {
        return null;
    }
});
...
app.whenReady().then(async () => {
    ...
    RemoteConfig.load().then(() => {
        ...
        // start() no-ops until ~/nenva/library exists, so fresh installs
        // pay nothing.
        try {
            EmbedManager.spec = embeddingModelSource();
            LibraryStore.start();
    ...
    // old low-RAM wizard stored) fall back to local; the dormant cloud API key
    // is deleted outright — there is no UI that could use it anymore.
    // 'auto' (local first, then the configured server) was removed
    // 2026-08-30 and lands on local too — the next brain pick re-syncs the
    // real value through AgentService._syncBrainToEntry.
    try {
        const _prov = settingsStore.get('llmProvider', 'local');
        if (_prov === 'remote' || _prov === 'anthropic' || _prov === 'auto') {
            settingsStore.set('llmProvider', 'local');
            console.log(`[migrate] llmProvider '${_prov}' → 'local' (no auto/cloud fallback path)`);
        }
    ...
    powerMonitor.on('resume', () => {
        ...
        backgroundMode.broadcast();
    });

    // Free the local model when the Mac itself is low on memory (macOS only).
    startMemoryPressureMonitor();
    // Embed servers orphaned by a session that died without will-quit sat on
    // 18444..18448 for days (~0.5 GB each). Sweep them at startup rather than
    // only on the first embed() call, which a session may never make.
    if (!EmbedManager.disabled) EmbedManager.reapOrphans().catch(() => {});
});
...
// Lock down any <webview> the renderer attaches. Nothing attaches one any
// more (the Web Browser app was removed 2026-09-20 and `webviewTag` is off),
// so this is the fail-closed backstop: whatever ever attaches one gets no
// preload, no Node, a sandbox, and an http(s) src or about:blank.
app.on('web-contents-created', (_event, contents) => {
    contents.on('will-attach-webview', (_e, webPreferences, params) => {
        ...
        if (params && typeof params.src === 'string') {
            if (!/^(https?:\/\/|about:blank)/i.test(params.src)) {
...
app.on('activate', () => backgroundMode.open());

// ---------------------------------------------------------------------------
// anjadhe:// deep links (2026-09-05). The Chrome companion extension hands
// the page it is on to the app as `anjadhe://read?url=…&title=…`; macOS
// delivers it as 'open-url' (to the running instance, or at launch). The
// grammar and the http(s)-only rule live in js/main/deep-link.js. Delivery
// is hold-then-consume: the parsed link waits in `pendingDeepLink` until the
// renderer asks for it (`deep-link-consume`), which it does both on a ping
// and at the end of AppManager.init — so a link that arrives before the
// window is ready is not lost, and one that arrives while it is ready opens
// exactly once. The scheme is registered by the packaged app only: in a
// source checkout the binary is Electron's own, whose Info.plist knows no
// such scheme, and macOS would ignore the registration anyway.
// ---------------------------------------------------------------------------
const DeepLink = require('./js/main/deep-link');
let pendingDeepLink = null;

function deliverDeepLink(raw) {
    const link = DeepLink.parseDeepLink(raw);
    if (!link) {
        console.warn('[deep-link] ignored:', String(raw || '').slice(0, 120));
        return;
    }
    pendingDeepLink = link;
    const win = getActiveWindow();
    if (!win) {
        if (app.isReady()) createWindow();   // the renderer consumes it on init
        return;
    }
    if (win.isMinimized()) win.restore();
    win.show();
    win.focus();
    try { win.webContents.send('deep-link'); } catch { /* window mid-teardown; init will consume */ }
}

app.on('open-url', (event, url) => {
    event.preventDefault();
    if (app.isReady()) deliverDeepLink(url);
    else app.whenReady().then(() => deliverDeepLink(url));
});

// Windows/Linux hand the link to a fresh process on argv; the app is not
// single-instance there, so this covers only the cold-start case honestly.
if (process.platform !== 'darwin') {
    const argvLink = DeepLink.deepLinkFromArgv(process.argv);
    if (argvLink) app.whenReady().then(() => deliverDeepLink(argvLink));
}

app.whenReady().then(() => {
    if (!app.isPackaged) return;
    try { app.setAsDefaultProtocolClient(DeepLink.SCHEME); }
    catch (e) { console.warn('[deep-link] protocol registration failed:', e.message); }
});
