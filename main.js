...
async function _newsConnect(topics) {
    ...
    const call = async (key) => {
        ...
        data.topics = data.topics.slice(0, NEWS_MAX_TOPICS).map(t => ({
            ...
            // Kept so the direct backfill below can tell "nothing found"
            // from "the server could not fetch this one".
...
ipcMain.handle('discover-news', async (event, { topics } = {}) => {
    ...
    if (provider === 'anjadhe') {
        let viaConnect = null;
        try { viaConnect = await _newsConnect(list); }
        catch { /* relay down — direct keeps the pane alive */ }
        if (viaConnect) return _newsBackfillDirect(viaConnect);
    }
    return _newsDirect(list);
});

// Connect answers per TOPIC: a topic whose upstream fetch failed on the
// server comes back as `{items: [], error: 'unavailable'}` inside an HTTP
// 200 — a well-formed reply, so the relay-down catch above never sees it.
// On 2026-09-03 every topic came back that way (Google News refusing the
// server's requests while the same feed answered from a Mac), and the
// Portfolio and News surfaces went blank because the empty lists were
// taken as truth. Failed topics are refetched here from this Mac; a topic
// that Connect genuinely found nothing for (no error) is left alone.
async function _newsBackfillDirect(reply) {
    const failed = reply.topics.filter(t => t.error).map(t => t.topic);
    if (!failed.length) return reply;
    const direct = await _newsDirect(failed);
    const byTopic = new Map(direct.topics.map(t => [t.topic.trim().toLowerCase(), t]));
    reply.topics = reply.topics.map(t => t.error ? (byTopic.get(t.topic.trim().toLowerCase()) || t) : t);
    reply.route = failed.length === reply.topics.length ? 'direct' : 'connect+direct';
    return reply;
}
