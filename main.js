...
function getLanRelayUrls() {
    ...
    try {
        ...
        for (const name of Object.keys(nets)) {
            for (const ni of nets[name] || []) {
                if (ni.family === 'IPv4' && !ni.internal) urls.push(`ws://${ni.address}:${lanRelayPort}`);
            }
        }
...
const MOBILE_VIEWS = new Set(['insights', 'news', 'portfolio']);
...
function handleChannelView(message) {
    ...
    return new Promise((resolve) => {
        ...
        broadcastToAllWindows('mobile-view-request', { view, reqId: internalId });
    });
}
