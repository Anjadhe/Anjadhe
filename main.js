...
const SEARCH_PROVIDERS = {
    ...
    // The key is minted automatically from this machine's machineId
...
// The built-in search path. The key is minted once per machine from the
// machineId (the same id the sync journal uses) and stored like any other
// provider key. Mint is lazy — first search, or the Settings card's
// usage fetch — so no network call happens before the feature is used.
    ...
    _connectMintInflight = (async () => {
        try {
            const res = await fetch(`${CONNECT_API_URL}/v1/keys`, {
                ...
                body: JSON.stringify({ installId: machineId }),
...
ipcMain.handle('search-connect-usage', async (event, { provision } = {}) => {
    ...
    try {
        ...
        if (res.status === 401) {
            ...
            return await retry.json().catch(() => ({ error: 'Bad response' }));
        }
