 ...
 * view) → Inbox (a message + its insight panel) → Tasks (complete one,
 * confetti) → Goals → Routines → Portfolio
...
/* ---- choreography ----------------------------------------------------- */
await S('opening slide', () => d.slide('This is nenva',
...
// nenva Cloud take). With no model the send would error; skip cleanly.
...
await S('to Inbox', async () => {
    ...
    await d.cap('Your mail, with the AI\u2019s read under every message.', 2000);
});
...
await S('goal weekly review', async () => {
    await d.cap('A review is scheduled on this goal. Every week, nenva reads the real progress and writes an honest read.');
    try {
...
await S('feed: the goal review', () => showFeedPost('^Goal Review:',
    'And here it is, back on the home feed. nenva read the plan and wrote this on its own.', 3600));
...
await S('open Writing Voices', async () => {
    ...
    await d.cap('It can even write like you. Give nenva documents you wrote, and it studies how you sound.', 2800);
});
...
await S('voice everywhere', async () => {
    await d.cap('From here on, anything nenva writes for you, a draft, a post, even a routine’s digest, can come out in this voice.', 3200);
});
...
await S('closing slide', () => d.slide('Private by default',
    'The AI runs on your Mac by default, or nenva Cloud, a server you own, or a provider you trust with your own key.', 4200, CLOSING));
