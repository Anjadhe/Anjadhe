 ...
 *
 * The model reads the real clock, so a shot taken at night opens with
 * "It's 10:22 PM, let's be realistic". Run it under a morning timezone
 * (TZ=Europe/Berlin from a US evening) when the landing page needs a
 * morning answer; the seed and the app both honour TZ, so the data agrees.
 *
...
(async () => {
    ...
    await page.evaluate(() => {
        ...
        document.querySelectorAll('button, div, span').forEach((el) => {
            if (el.childElementCount <= 2 && /Set up more of nenva/.test(el.textContent || '')) el.remove();
        });
