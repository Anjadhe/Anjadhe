...
// 1. Local writes update the mirror AND forward to native.
{
  ...
  // The phone's changed-keys list (anjadhe:channel:dirty) is persisted
  // alongside — a different key; these checks are about the value's key.
  const own = () => posted.filter(p => p.key === 'schedule');
  check('set forwards a persist to native', own().length === 1 && own()[0].type === 'persist');
  check('persist carries value + modifiedAt', !!own()[0].entry.value && !!own()[0].entry.modifiedAt);
  s.electronStore.delete('schedule');
  check('delete tombstones in mirror', s.electronStore.get('schedule') === null);
  check('delete forwards a tombstone to native', own()[1].entry.deleted === true);
}
...
//    native (loop prevention) but stays exportable for the next sync upload.
{
  ...
  check('applyLocalWrite does NOT re-post to native', posted.filter(p => p.key === 'notes').length === 0);
...
// 9. The changed-keys list (2026-09-25): local writes mark a key, the Mac's
//    ack of that exact write clears it, a later write keeps it, and a Mac
//    copy replacing ours settles it. Channel keys never enter it.
{
  const { s } = make();
  s.bridge.applyLocalWrite('agent-conversations', { conversations: [] }, '2026-09-25T10:00:00.000Z');
  s.electronStore.set('schedule', { scheduleItems: [] });
  s.electronStore.set('anjadhe:channel:x', 1);
  check('local writes are dirty', JSON.stringify(s.anjadheStore.exportDirty().sort()) === JSON.stringify(['agent-conversations', 'schedule']));
  s.anjadheStore.clearDirty({ 'agent-conversations': '2026-09-25T10:00:00.000Z' });
  check('an acked write clears', !s.anjadheStore.exportDirty().includes('agent-conversations'));
  s.bridge.applyLocalWrite('notes', { notes: [] }, '2026-09-25T10:00:01.000Z');
  s.bridge.applyLocalWrite('notes', { notes: [1] }, '2026-09-25T10:00:02.000Z');
  s.anjadheStore.clearDirty({ notes: '2026-09-25T10:00:01.000Z' });
  check('a newer write survives an older ack', s.anjadheStore.exportDirty().includes('notes'));
  s.anjadheStore.applyRemote('notes', { notes: [] }, '2026-09-25T11:00:00.000Z');
  check('a Mac copy replacing ours settles it', !s.anjadheStore.exportDirty().includes('notes'));
  s.bridge.reset('anjadhe:channel:');
  check('reset clears the list', s.anjadheStore.exportDirty().length === 0);
}
