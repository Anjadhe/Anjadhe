...
if (story === 'email') {
    ...
    await S('caption: widget', () => d.cap("nenva already read this morning's mail. This widget shows only what actually needs you.", 2200));
...
if (story === 'routine') {
    ...
    // injected mail would also fire triage, and that
    ...
    await d.E(`
        ...
        // Session-local stub: the free tier allows 2 concurrent requests,
        // and a trickling triage call holding a slot is what killed
    ...
    await S('open Routines', async () => {
        ...
        await d.cap('These are the things nenva does on its own. Some run on a schedule. Some run when a certain email arrives.', 3000);
    });
...
if (story === 'voice') {
    ...
    await S('open the voice', async () => {
        ...
        await sleep(1400);
        await d.cap('nenva studied them. What it learned is a style guide you can read, and edit.');
