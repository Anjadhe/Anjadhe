...
export class Demo {
     ...
     *  (a server-based pick must survive the recorder); the nenva Cloud
    ...
    /** Wait for ambient analysis to finish. */
    async settleEmail(maxMs = 180000) {
        const t0 = Date.now();
        while (Date.now() - t0 < maxMs) {
            const s = await this.E(`
                await EmailApp.loadData();
                return { a: EmailApp.isAnalyzing || false, p: (EmailApp.pendingAnalysisIds || []).length };`);
                ...
                async slide(title, sub, holdMs, opts) {
                    ...
                    if (!s) {
                        ...
                        s.innerHTML = '<img class="__ds-logo" src="build/icon.png" alt="" hidden>'
                            + '<div class="__ds-eyebrow">nenva</div><h1></h1><p></p>'
                            + '<div class="__ds-track"><div class="__ds-bar"></div></div>'
                            + '<div class="__ds-cta"><span>Download nenva at</span><strong></strong></div>';
