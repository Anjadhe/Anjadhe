 ...
 *   npm run build:icons
 *
 * The mark (2026-09-22, the nenva rename) is the SPROUT N: a rounded white
 * lowercase n with a leaf growing from its shoulder (see glyph() below). It
 * replaced, the same day, a bold serif "n" that Google's brand review
 * rejected as not identifying the brand. Before those (2026-09-11, by request: "consistent with the
 * new font design, remove the blue") the same tile carried a serif "A" set
 ...
 *   extension/chrome/icons/, browser-extension/icons/   icon-{16,32,48,128}.png
 *                                                    for the two Chrome extensions
 *   js/main/assets/trayTemplate{,@2x}.png          the menu-bar mark, black on
 *                                                    transparent (a template)
...
const DEFS = `<defs>`
    ...
    + `</defs>`;
// The sprout n (2026-09-22): a rounded white n with one leaf growing from
// its right shoulder, a hairline vein cut into the leaf in the tile's ink.
// Drawn, not typeset: Google's brand review rejected a bare letter as not
// identifying the brand; the leaf is what makes it ours. Geometry is on the
// 824px macOS tile centred at (512,512); `scale` grows it around the centre
// for the full-bleed frames. `size` keeps the call sites' shape; unused.
const VEIN = '#1e1e1d';
function glyph(size, scale = 1) {
    const mark = `<path d="M392 740 V520 a130 130 0 0 1 260 0 V740" fill="none" stroke="${GLYPH}" stroke-width="84" stroke-linecap="round"/>`
        + `<path d="M600 408 C620 300 720 260 790 280 C780 360 700 420 600 408 Z" fill="${GLYPH}"/>`
        + `<path d="M606 404 C660 360 710 320 770 292" fill="none" stroke="${VEIN}" stroke-width="10" stroke-linecap="round"/>`;
    // The leaf pulls the bounding box right; 40px left centres it by eye.
    return `<g transform="translate(512 512) scale(${scale}) translate(-552 -512)">${mark}</g>`;
}
...
const MAC = svg(tile(100, 100, 824, 186) + glyph(760));
const IOS = svg(tile(0, 0, S, 0) + glyph(760, 1024 / 824));
...
const LAUNCH = svg(tile(0, 0, S, 231) + glyph(760, 1024 / 824));
// The menu-bar icon (background mode's tray) is a macOS TEMPLATE image: the
// bare mark in black on transparent, no tile, cropped to the mark so it fills
// the 18pt slot — macOS reads only the alpha and paints it for a light or dark
// menu bar. No vein: at 18px it is a speck, and in a template it would have to
// be a hole. It replaced a code-drawn "A" the rebrand missed (2026-09-23).
const TRAY = `<svg xmlns="http://www.w3.org/2000/svg" width="${S}" height="${S}" viewBox="318 246 520 520">`
    + `<path d="M392 740 V520 a130 130 0 0 1 260 0 V740" fill="none" stroke="#000" stroke-width="84" stroke-linecap="round"/>`
    + `<path d="M600 408 C620 300 720 260 790 280 C780 360 700 420 600 408 Z" fill="#000"/></svg>\n`;
...
const mac = render('mac', MAC), ios = render('ios', IOS), launch = render('launch', LAUNCH), tray = render('tray', TRAY);
...
// Chrome extensions (the Reader and the workroom Browser Preview): the same
// rounded tile at the four sizes Chrome asks for, so the toolbar shows the
// app's mark rather than a stale letter or Chrome's generic puzzle piece.
for (const dir of ['extension/chrome/icons', 'browser-extension/icons']) {
    fs.mkdirSync(path.join(ROOT, dir), { recursive: true });
    for (const s of [16, 32, 48, 128]) resize(launch, s, path.join(ROOT, dir, `icon-${s}.png`));
}
// Menu bar: 18pt at 1x and 2x; the Template suffix is what tells Electron
// (and macOS) to treat it as a template image.
const trayDir = path.join(ROOT, 'js/main/assets');
fs.mkdirSync(trayDir, { recursive: true });
resize(tray, 18, path.join(trayDir, 'trayTemplate.png'));
resize(tray, 36, path.join(trayDir, 'trayTemplate@2x.png'));
fs.rmSync(tmp, { recursive: true, force: true });
console.log('Icons rebuilt: build/icon.{svg,png,icns,-120.png}, iOS AppIcon + launch-logo, both Chrome extensions, menu-bar template.');
