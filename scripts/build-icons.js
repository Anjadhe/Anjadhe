 ...
 *   npm run build:icons
 *
 * The mark (2026-09-10, by request: "less boring but trustable"; it
 * replaced the plain white "A" of 2026-09-02, which replaced the
 * turbaned-face drawing) is a SHELTER A: the two legs of a bold
 * rounded-stroke white "A" meeting at a roof-like apex, with no crossbar —
 * where the bar would be sits ONE indigo dot (the kit's accent, #5B76F0 on
 * the dark tile), your data held inside, on a deep charcoal-navy tile with
 * a soft vertical gradient and a faint sheen at the top. Still nothing
 * cultural or figurative, still one glyph, still quiet in a Dock; the dot
 * is the one point of colour and the reason it stops reading as a font
 * sample. The geometry lives
...
const S = 1024;
// White glyph, indigo dot, charcoal-navy tile (top → bottom of the gradient).
const GLYPH = '#ffffff', DOT = '#5B76F0', TILE_TOP = '#3b4150', TILE_BOTTOM = '#22252f';
...
const DEFS = `<defs>`
    ...
    + `<radialGradient id="glow" cx="0.5" cy="0.5" r="0.5"><stop offset="0" stop-color="${DOT}" stop-opacity="0.42"/><stop offset="1" stop-color="${DOT}" stop-opacity="0"/></radialGradient>`
    + `</defs>`;
function glyph(sw, scale = 1) {
    const p = (x, y) => [512 + (x - 512) * scale, 512 + (y - 512) * scale - 8 * scale];
    const [ax, ay] = p(512, 296), [lx, ly] = p(322, 722), [rx, ry] = p(702, 722);
    const [dx, dy] = p(512, 592);
    const d = `M${lx} ${ly} L${ax} ${ay} L${rx} ${ry}`;
    return `<circle cx="${dx}" cy="${dy}" r="${(150 * scale).toFixed(1)}" fill="url(#glow)"/>`
        + `<path d="${d}" fill="none" stroke="${GLYPH}" stroke-width="${(sw * scale).toFixed(1)}"`
        + ' stroke-linecap="round" stroke-linejoin="round"/>'
        + `<circle cx="${dx}" cy="${dy}" r="${(50 * scale).toFixed(1)}" fill="${DOT}"/>`;
}
...
const MAC = svg(tile(100, 100, 824, 186) + glyph(88));
const IOS = svg(tile(0, 0, S, 0) + glyph(88, 1024 / 824));
...
const LAUNCH = svg(tile(0, 0, S, 231) + glyph(88, 1024 / 824));
