...
async function stage(page) {
    await page.evaluate(() => {
        ...
        // The throwaway root has no quotes, so the Portfolio widget's
        // day-change / P&L band reads "-$54,370 -100.00%": a data gap, not
        // the product. Drop the band; the value itself is fine.
        document.querySelectorAll('.pfw-changes').forEach((el) => { el.style.display = 'none'; });
        // The greeting follows the clock; a shot taken after work says
        // "Good evening" on a landing page whose story opens at Morning.
        const g = document.getElementById('dash-greeting');
        if (g) g.textContent = 'Good morning';
        // First-run "your assistant can have a name" nudge.
...
(async () => {
    ...
    // Home, scrolled to the Feed: the routines' dated editions (seeded by
    // --feed) with the last widget row still in view above them.
    await page.evaluate(() => {
        // The first-run welcome post is not a routine's work.
        document.querySelectorAll('.feed-series').forEach((el) => {
            if (/welcome to your feed/i.test(el.textContent || '')) el.remove();
        });
        // The feed is the subject: park the widget grid so the editions
        // sit right under the composer (the page is too short to scroll
        // them up otherwise).
        const w = document.getElementById('dash-widgets');
        if (w) w.style.display = 'none';
        const m = document.querySelector('.dash-main');
        if (m) m.scrollTop = 0;
    });
    await page.waitForTimeout(600);
    await shot('home-routines');
    await page.evaluate(() => { const w = document.getElementById('dash-widgets'); if (w) w.style.display = ''; });
    ...
    // if any background work is registered; this is a throwaway root.
