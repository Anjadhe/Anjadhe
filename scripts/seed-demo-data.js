...
// --feed (with --baked) pre-bakes a few routine EDITIONS in the Home feed
// (feed-template notes, exactly what PromptFeed._postToFeed writes) and
// stamps their runs so no routine fires live during a capture. Used by
// capture-screenshots.js; the demo recordings deliberately leave it off
// so the catch-up runs happen on camera.
...
function email(m) {
    ...
    // definition) — seeded so the Inbox's Sent view has something to show.
...
const mockEmails = [
    ...
    // The one outbound message, and so the only row in the Sent view.
    email({
 ...
 */
const DEMO_FILES = path.join(ROOT, 'nenva Demo');
...
// BAKED --feed: a Morning News routine plus editions of the three
// routines, written in the app's own feed-note shape. Content is authored
// (the screenshot is of the surface, not the model); titles never restate
// the series name so PromptFeed._summaryFor lifts the first line.
const feedRuns = {};
if (BAKED && FEED) {
    const morningNews = promptNote(
        'Morning News',
        'Give me the morning news on my topics: local AI models, personal finance, the Pacific Northwest. Use get_news; date every headline and link every source. Then two lines on what matters for me today.',
        { interval: 'daily', time: '07:00', web: true, useContext: true }, -60);
    notes.push(morningNews);
    const feedNote = (prompt, html, dayOffset, hour, read) => ({
        id: id('note'), title: prompt.title, content: html, tags: [],
        template: 'feed',
        feed: { promptId: prompt.id, model: 'Qwen3.6 35B · Mac Studio', error: null, ...(read ? { readAt: iso(dayOffset, hour + 1) } : {}) },
        pinned: false, createdAt: iso(dayOffset, hour), modifiedAt: iso(dayOffset, hour),
    });
    const strategyReview = notes.find((n) => n.title === 'Strategy Review: Balanced Growth');
    const goalReview = notes.find((n) => n.title === 'Goal Review: Run a 10K in under an hour');
    notes.push(
        feedNote(morningNews,
            '<p>The rate hold keeps your bond sleeve\'s drift small, and Sunday light-rail service helps the 10K route you planned along the lake. Three headlines on your topics:</p>' +
            '<ul><li><strong>Open-weight models</strong> · A new 32B release claims tool-calling parity with last year\'s frontier models; early reports say it runs on 32 GB Macs. <em>The Verge, today</em></li>' +
            '<li><strong>Personal finance</strong> · The Fed held rates steady again; two-year yields slipped after the statement. <em>Reuters, today</em></li>' +
            '<li><strong>Seattle</strong> · Light rail\'s Lynnwood extension adds Sunday service starting this weekend. <em>Seattle Times, yesterday</em></li></ul>' +
            '<p>Nothing here changes today\'s plan.</p>',
            0, 7, false),
        feedNote(morningNews,
            '<p>A quiet news day on your topics. The one item worth a minute: Apple\'s macOS point release fixes the Wi-Fi drop several people in your Seattle thread were hitting. <em>MacRumors, yesterday</em></p>',
            -1, 7, true),
        feedNote(morningNews,
            '<p>Two model releases and a bond auction, none of them urgent. Nothing changes today\'s plan.</p>',
            -2, 7, true),
        feedNote(strategyReview,
            '<p>Balanced Growth still fits. Bonds sit at 18% against the 25% target, the only sleeve outside its band; equities and cash are inside. Nothing in yesterday\'s market moves changes that, so the drift is the one thing that deserves attention this week, and about $4,200 into bonds would close it.</p>',
            0, 8, false),
        feedNote(goalReview,
            '<p>Under way and moving. Two of the three runs planned last week happened (5K on Tuesday, 6K on Saturday at a 6:10 pace); the tempo run slipped. Next scheduled: intervals Thursday. Smallest step worth adding: put the Sunday long run on the calendar before the week fills up.</p>',
            -3, 9, false),
    );
    feedRuns[morningNews.id] = iso(0, 7);
    if (strategyReview) feedRuns[strategyReview.id] = iso(0, 8);
    if (goalReview) feedRuns[goalReview.id] = iso(-3, 9);
}
...
const voicePage = {
    ...
    // The user's own voice (Settings › Writing voice): self-flagged, with
    // the documents source on — the folder above is what it studied.
    self: true,
    sources: { documents: true, notes: false, journal: false, emails: false },
    body: [
...
db.transaction(() => {
    ...
    set('promptFeed', { items: [], runs: feedRuns });
    set('actionsSettings', { aiFiling: true });
    // Tags on the demo documents (the Documents app organizes by tag; ids
    // are the index's own: doc_ + sha1(relpath)[:16]).
    const docTagRec = (file, tags) => {
        const rel = path.join(VOICE_COLLECTION, file);
        const id = 'doc_' + require('crypto').createHash('sha1').update(rel).digest('hex').slice(0, 16);
        return { id, relpath: rel, tags, createdAt: iso(-10, 9), updatedAt: iso(-2, 9) };
    };
    const docTags = [
        docTagRec('shipping-the-fleet-dashboard.md', ['Work/Writing', 'Writing/Posts']),
        docTagRec('running-notes-summer.md', ['Personal/Running', 'Writing/Posts']),
        docTagRec('note-to-the-team.md', ['Work/Writing'])
    ];
    set('library', { voicePages: [voicePage], exemplars: voiceExemplars, docTags, tombstones: {} });
