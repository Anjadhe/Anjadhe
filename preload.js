...
contextBridge.exposeInMainWorld('electronSearch', {
    ...
    // Discover pane headlines (docs/DISCOVER.md D1) — routed by provider
    // choice in main (Connect relay vs direct RSS).
    news: (topics) => ipcRenderer.invoke('discover-news', { topics }),
    // Today's conditions for the pane's typed location (Open-Meteo, direct).
    weather: (location, unit) => ipcRenderer.invoke('discover-weather', { location, unit })
});
