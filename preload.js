...
const storeApi = {
    ...
    // Fires with the key after ANY write to the kv store (this window's,
    // another window's, the sync merge, the phone); null = everything.
    onKeyChanged: (callback) => ipcRenderer.on('store-key-changed', (event, key) => callback(key))
};
...
contextBridge.exposeInMainWorld('electronLibrary', {
    ...
    tidyGet: (docId) => ipcRenderer.invoke('library-tidy-get', { docId }),
    tidySet: (docId, rec) => ipcRenderer.invoke('library-tidy-set', { docId, ...(rec || {}) }),
    renderPage: (docId, page, maxEdge) => ipcRenderer.invoke('library-page', { docId, page, maxEdge }),
...
contextBridge.exposeInMainWorld('electronAppleImport', {
    ...
    // op: 'create' | 'update' | 'delete'; payload is the helper's stdin JSON.
    writeEvent: (op, payload) => ipcRenderer.invoke('apple-events-write', op, payload),
...
contextBridge.exposeInMainWorld('electronChannel', {
    ...
    remoteAccessStatus: () => ipcRenderer.invoke('channel-remote-access-status'),
    setRemoteAccess: (on) => ipcRenderer.invoke('channel-remote-access-set', on),
    onPaired: (callback) => ipcRenderer.on('channel-paired', (event, info) => callback(info))
});

// Mac<->Mac contacts and the sharing room (docs/PORTFOLIO_SHARING.md).
// Messages are opaque JSON both ways; PortfolioSharing owns their meaning.
contextBridge.exposeInMainWorld('electronPeers', {
    ensure: () => ipcRenderer.invoke('peers-ensure'),
    info: () => ipcRenderer.invoke('peers-info'),
    createInvite: (label) => ipcRenderer.invoke('peers-create-invite', label),
    cancelInvite: (id) => ipcRenderer.invoke('peers-cancel-invite', id),
    acceptInvite: (code, label) => ipcRenderer.invoke('peers-accept-invite', code, label),
    renameContact: (id, label) => ipcRenderer.invoke('peers-rename-contact', id, label),
    removeContact: (id) => ipcRenderer.invoke('peers-remove-contact', id),
    send: (contactId, message) => ipcRenderer.invoke('peers-send', contactId, message),
    onMessage: (callback) => ipcRenderer.on('peers-message', (event, msg) => callback(msg)),
    onChanged: (callback) => ipcRenderer.on('peers-changed', () => callback())
});
...
contextBridge.exposeInMainWorld('electronMobileChat', {
    ...
    // Partial answer text while the run is still going (2026-09-21). `send`
    // and not `invoke`: a delta is fire-and-forget several times a second and
    // has no reply worth waiting for.
    sendDelta: (delta) => ipcRenderer.send('mobile-chat-delta', delta),
    // Approvals on the phone (2026-09-25): an ask goes out, its answer comes
    // back, and a settled ask is announced so the phone drops its card.
    sendAsk: (ask) => ipcRenderer.send('mobile-chat-ask', ask),
    sendAskDone: (done) => ipcRenderer.send('mobile-chat-ask-done', done),
    onAnswer: (callback) => ipcRenderer.on('mobile-chat-answer', (event, a) => callback(a))
});
...
// iMessage: the notification lane (outbound — js/main/imessage-bridge.js)
// and, since 2026-09-10, the insight SOURCE (read-only over this Mac's
// Messages database — js/main/imessage-reader.js; the renderer's
// IMessageSource owns the switch and the cursor).
contextBridge.exposeInMainWorld('electronIMessage', {
    ...
    send: (handle, text) => ipcRenderer.invoke('imessage-send', handle, text),
    probe: () => ipcRenderer.invoke('imessage-probe'),
    read: (opts) => ipcRenderer.invoke('imessage-read', opts),
    openChat: (handle) => ipcRenderer.invoke('imessage-open-chat', handle),
});
...
contextBridge.exposeInMainWorld('electronNet', {
    ...
    // CNBC's current headlines (+ its stories on `symbol`); see js/main/cnbc-headlines.js.
    fetchCnbcHeadlines: (opts) => ipcRenderer.invoke('cnbc-headlines', opts || {}),
...
contextBridge.exposeInMainWorld('electronMenu', {
    ...
    // anjadhe:// deep links (the Chrome extension). Main pings, the renderer
    // consumes — see the deep-link block at the bottom of main.js.
    onDeepLink: (callback) => {
...
contextBridge.exposeInMainWorld('electronUpdater', {
    ...
    // A download that started (onAvailable) and will not finish — the
    // progress strip comes down; the periodic re-check tries again later.
    onDownloadError: (cb) => ipcRenderer.on('updater:download-error', (_, info) => cb(info)),
...
contextBridge.exposeInMainWorld('electronLLM', {
    ...
    // nenva Cloud catalog for the Settings picker (keyless; falls back to
...
// Assistant screen tools (docs/COWORK_AGENT.md C13) — capture this window,
// send one validated input step. Main re-validates every step.
contextBridge.exposeInMainWorld('electronScreen', {
    capture: () => ipcRenderer.invoke('screen-capture'),
    input: (step) => ipcRenderer.invoke('screen-input', step)
});

// Mac control (docs/COWORK_AGENT.md C14) — other apps through the native
// helper. main validates every step and refuses the blocked apps.
contextBridge.exposeInMainWorld('electronMac', {
    status: () => ipcRenderer.invoke('mac-status'),
    requestPermission: (what) => ipcRenderer.invoke('mac-request-permission', what),
    snapshot: (opts) => ipcRenderer.invoke('mac-snapshot', opts),
    act: (step) => ipcRenderer.invoke('mac-act', step),
    elementAt: (pt) => ipcRenderer.invoke('mac-element-at', pt),
    hideBar: () => ipcRenderer.invoke('mac-hide-bar'),
    focusSelf: () => ipcRenderer.invoke('mac-focus-self'),
    onStop: (cb) => { ipcRenderer.on('mac-stop', () => cb()); }
});
...
contextBridge.exposeInMainWorld('electronCLI', {
    ensure: () => ipcRenderer.invoke('cli-ensure'),
    suspend: () => ipcRenderer.invoke('cli-suspend'),
...
contextBridge.exposeInMainWorld('electronSearch', {
    ...
    // nenva Connect plan + usage; provision=true mints this machine's key.
    connectUsage: (provision) => ipcRenderer.invoke('search-connect-usage', { provision }),
    // THE id for this Mac: { id, idHash }. Local read, no network. `id` is
    // what goes on the wire, `idHash` is the only form nenva stores and the
    // only one shown to the user. Null on a legacy install mid-migration.
    installId: () => ipcRenderer.invoke('connect-install-id'),
    ...
    // `opts.sources` (2026-09-10): which news sources answer — ids from
    // js/main/news-sources.js; absent means Google News.
    news: (topics, opts) => ipcRenderer.invoke('discover-news', { topics, sources: opts && opts.sources })
});

// Brokerage linking through nenva Connect (Plaid). The Mac holds no
// Plaid credential; every call is proxied by Connect — see main.js.
contextBridge.exposeInMainWorld('electronBrokerage', {
    // Opens Plaid Hosted Link in the default browser; resolves {linkToken}.
    // itemId re-authorises an existing link (update mode).
    // products: ['investments'] (brokerage) or ['transactions', 'investments']
    // (bank or card, for Spending); omitted on a relink.
    linkStart: (itemId, products) => ipcRenderer.invoke('brokerage-link-start', { itemId, products }),
    linkStatus: (linkToken) => ipcRenderer.invoke('brokerage-link-status', { linkToken }),
    items: () => ipcRenderer.invoke('brokerage-items'),
    holdings: (itemId) => ipcRenderer.invoke('brokerage-holdings', { itemId }),
    transactions: (itemId, startDate, endDate) => ipcRenderer.invoke('brokerage-transactions', { itemId, startDate, endDate }),
    // Bank/card side (Spending): accounts with balances; cursor-paged
    // transaction sync (added / modified / removed + nextCursor).
    accounts: (itemId) => ipcRenderer.invoke('brokerage-accounts', { itemId }),
    transactionsSync: (itemId, cursor) => ipcRenderer.invoke('brokerage-transactions-sync', { itemId, cursor }),
    unlink: (itemId) => ipcRenderer.invoke('brokerage-unlink', { itemId })
});
...
// Narrow app-lifecycle bridge; main is authoritative for scheduler ownership.
contextBridge.exposeInMainWorld('electronBackground', {
    state: () => ipcRenderer.sendSync('background-state'),
    set: patch => ipcRenderer.invoke('background-set', patch),
    status: status => ipcRenderer.send('background-status', status),
    onChange: callback => ipcRenderer.on('background-changed', (_event, state) => callback(state)),
    onOpen: callback => ipcRenderer.on('background-open', (_event, route) => callback(route))
});

contextBridge.exposeInMainWorld('electronWorkrooms', {
    list: () => ipcRenderer.invoke('workrooms', 'list'),
    events: (id, after) => ipcRenderer.invoke('workrooms', 'events', { id, after }),
    attachment: (id, fileId) => ipcRenderer.invoke('workrooms', 'attachment', { id, fileId }),
    command: (action, input) => ipcRenderer.invoke('workrooms', action, input),
    // Deltas: { room?, event?, deleted?, browser? }. Nothing polls.
    onEvent: callback => ipcRenderer.on('workrooms-event', (_event, delta) => callback(delta || {}))
});

contextBridge.exposeInMainWorld('electronAgentBrowser', {
    command: (action, input) => ipcRenderer.invoke('agent-browser', action, input)
});
