...
const LlamaCppManager = {
    ...
    // Network sharing (Settings › AI Assistant › llama.cpp › Share on local
    // network): when enabled the server binds 0.0.0.0 so other devices on the
    // LAN can use this Mac's model as an OpenAI-compatible endpoint. The key
    // then comes from shareKey — persistent across restarts, so a MacBook's
    // saved provider config keeps working. Both are set by main.js from the
    // machine-local settings store before any spawn.
    shareNetwork: false,
    shareKey: null,
    _quitting: false,
    ...
    /**
     * The model's vision adapter (mmproj GGUF) on disk, or null. Recorded in
     * the map by pullModel when the catalog entry carries one — its presence
     * is what makes a loaded model accept image input.
     */
    _mmprojPath(modelName) {
        const entry = this._readMap()[modelName];
        if (!entry || !entry.mmproj) return null;
        const p = path.join(this.modelsDir, entry.mmproj);
        return fs.existsSync(p) ? p : null;
    },

    _spawn(binaryPath, port, ggufPath, alias, ctx, mmprojPath) {
        ...
        //
        // Network sharing swaps both knobs together: bind 0.0.0.0 AND use the
        // persistent shareKey (per-spawn keys would break the other Mac's
        // saved config on every model reload). Never bind wide without a key
        // — a missing shareKey fails closed to localhost.
        const sharing = this.shareNetwork && !!this.shareKey;
        this.apiKey = sharing ? this.shareKey : crypto.randomBytes(24).toString('hex');
        const args = [
            '-m', ggufPath,
            '--host', sharing ? '0.0.0.0' : '127.0.0.1',
            '--port', String(port),
            '-c', String(ctx),
        ...
        // Vision: load the model's multimodal projector so /v1 chat accepts
        // image_url content parts. Only passed when the sidecar is actually
        // on disk — a missing/failed mmproj degrades to text-only chat.
        // KV-shift prefix reuse is incompatible with multimodal in
        // llama-server, so --cache-reuse only rides on text-only spawns — a
        // rejected flag combo would fail the whole model load.
        if (mmprojPath) {
            args.push('--mmproj', mmprojPath);
        } else {
            // KV-shift partial prefix reuse when the tool list drifts between
            // turns (probe-verified on b10015: a mid-prefix byte change
            // re-prefills only ~200 tokens instead of everything after it;
            // works with q8_0 KV).
        ...
        child.on('exit', (code) => {
            ...
            // Only clean up when the exiting process is still the current
            // one (or stop() already nulled the reference). A restart's old
            // process can exit AFTER the new spawn took over — nulling state
            // then would orphan the new server: _waitForReady sees
            // process===null, reports "crashed during load", and the live
            // process keeps the port with a key nobody holds.
            if (this.process !== child && this.process !== null) return;
            this._notifyActivity('stopped', alias);
            this.process = null;
            this.isReady = false;
            this.loadedModel = null;
            this.loadedMmproj = null;
    ...
    async ensureModel(modelName, numCtx) {
        ...
        // mmproj is part of the process identity like (model, ctx): after a
        // vision re-download adds the adapter, the next ensureModel restarts
        // the server with --mmproj instead of no-oping on the text-only one.
        if (this.isReady && this.process && this.loadedModel === modelName && this.loadedCtx === ctx
            && (this.loadedMmproj || null) === this._mmprojPath(modelName)) {
            return true;
        }
    ...
    async _loadModel(modelName, ctx) {
        ...
        const mmprojPath = this._mmprojPath(modelName);
        console.log(`[llamacpp] Starting llama-server: ${modelName} (ctx ${ctx}${mmprojPath ? ', vision' : ''})`);
        this._notifyActivity('loading', modelName);
        this.process = this._spawn(binaryPath, this.port, ggufPath, modelName, ctx, mmprojPath);
        const ready = await this._waitForReady(this.port);
        if (ready) {
            this.isReady = true;
            this.loadedModel = modelName;
            this.loadedCtx = ctx;
            this.loadedMmproj = mmprojPath;
    ...
    async listModels() {
        ...
        for (const [name, entry] of Object.entries(map)) {
            ...
            if (entry.mmproj) seenFiles.add(entry.mmproj);
            // vision = the adapter is on disk NOW (a catalog flag alone isn't
            // enough — pre-vision downloads need a re-download to get it).
            models.push({ name, size: fs.statSync(p).size, vision: !!this._mmprojPath(name) });
        }
        try {
            for (const f of fs.readdirSync(this.modelsDir)) {
                // Vision adapters aren't chat models — never list them as
                // drop-ins. Upstream repos name them both mmproj-*.gguf and
                // *-mmproj.gguf, so match the token anywhere.
                if (!f.endsWith('.gguf') || seenFiles.has(f) || /mmproj/i.test(f)) continue;
    ...
    async pullModel(modelName, gguf, onProgress) {
        ...
        if (!fs.existsSync(finalPath)) {
            await this._downloadVerified(gguf.url, finalPath, gguf.sha256, `Model "${modelName}"`, onProgress, 'Downloading');
        }
        // Register the weights BEFORE the optional vision sidecar — a failed
        // mmproj download must not strand multi-GB weights uninstalled. This
        // also backfills mmproj for models downloaded before the catalog
        // carried one: re-running the download skips the existing GGUF and
        // fetches only the sidecar.
        const map = this._readMap();
        map[modelName] = { file: gguf.file };
        this._writeMap(map);
        let mmprojError = null;
        const mmproj = gguf.mmproj && gguf.mmproj.url && gguf.mmproj.file ? gguf.mmproj : null;
        if (mmproj) {
            const mmPath = path.join(this.modelsDir, mmproj.file);
            try {
                if (!fs.existsSync(mmPath)) {
                    await this._downloadVerified(mmproj.url, mmPath, mmproj.sha256, `Vision adapter for "${modelName}"`, onProgress, 'Downloading vision support');
                }
                const m2 = this._readMap();
                m2[modelName] = { file: gguf.file, mmproj: mmproj.file };
                this._writeMap(m2);
            } catch (e) {
                // Vision is an extra, not a gate: the model still chats as
                // text-only. supportsVision sees the missing mmproj and the
                // UI offers a re-download.
                mmprojError = e.message || 'Vision adapter download failed';
                console.warn(`[llamacpp] mmproj download failed for ${modelName}: ${mmprojError}`);
            }
        }
        if (onProgress) onProgress({ status: 'Done', percent: 100, completed: null, total: null });
        return mmprojError ? { success: true, mmprojError } : { success: true };
    },

    /**
     * Download url → finalPath with retry/resume + integrity check. The
     * .part file only becomes the real file after the digest passes — a bad
     * download must never land in models/ (llama.cpp would parse it).
     */
    async _downloadVerified(url, finalPath, sha256, label, onProgress, statusLabel) {
        const partPath = finalPath + '.part';
        const MAX_ATTEMPTS = 6;
        let lastErr;
        for (let n = 1; n <= MAX_ATTEMPTS; n++) {
            try {
                await this._downloadWithResume(url, partPath, onProgress, statusLabel);
                if (onProgress) onProgress({ status: 'Verifying', percent: null, completed: null, total: null });
                try {
                    await this._verifyDigest(partPath, sha256, label);
                } catch (ve) {
    ...
    _downloadWithResume(url, partPath, onProgress, statusLabel = 'Downloading') {
    ...
    /** Delete a downloaded model (its vision adapter and map entry too). */
    async deleteModel(modelName) {
        ...
        if (entry && entry.mmproj) {
            try { fs.unlinkSync(path.join(this.modelsDir, entry.mmproj)); } catch {}
        }
    ...
    async installFromUrl(url, onProgress, expectedSha256) {
        ...
        try {
            ...
            // Upgrade path: a running llama-server holds the old binary. Stop
            // it (quitting=false so respawns stay allowed) — the next
            // ensureModel boots the new build with the same model.
            if (this.process) this.stop(false);
            ...
            // Drop the cache first or an upgrade would "verify" the old build.
