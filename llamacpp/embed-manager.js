...
const EmbedManager = {
    ...
    async _start(ggufPath) {
        ...
        // Ports held by embed servers we don't own (orphans from sessions that
        // died without will-quit — force quit, crash). Used to step aside to a
        // nearby port instead of killing; that left one ~0.5 GB orphan per dead
        // session parked on 18444..18448, five of them for days. An orphan is
        // useless to us (every spawn mints a fresh api key), so reclaim the
        // whole range the way the chat manager reclaims 18434, and only fall
        // over if something that is NOT a llama-server holds the home port.
    ...
    /**
     * Kill stale llama-servers on our home port and the fallover range.
     * Reuses the chat manager's reaper, which only touches listeners that
     * answer /health like a llama-server and leaves anything else alone.
     */
    async reapOrphans() {
        for (let p = 18444; p <= 18448; p++) {
            // eslint-disable-next-line no-await-in-loop
            if ((await this._checkHealth(p)) === 'free') continue;
            // eslint-disable-next-line no-await-in-loop
            await LlamaCppManager._reclaimPort.call(this, p);
        }
    },
