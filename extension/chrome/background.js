'use strict';
/**
 * nenva Reader — the Chrome companion.
 *
 * One job: hand the page you are on (or a link you right-clicked) to the
 * nenva app on this Mac as an `anjadhe://read?url=…&title=…` link. The
...
// app: Chrome asks "Open nenva?" the first time (tick "Always allow") and
...
async function sendToApp(tabId, url, title) {
    if (!isWebUrl(url)) {
        await flashBadge(tabId, '!', 'Only web pages (http/https) can be read in nenva.');
        return;
    }
    try {
        await chrome.tabs.update(tabId, { url: buildReadLink(url, title) });
        await flashBadge(tabId, '→', 'Sent to nenva');
    } catch (e) {
        await flashBadge(tabId, '!', `Could not open nenva: ${e && e.message ? e.message : e}`);
    }
}
...
async function flashBadge(tabId, text, title) {
    try {
        ...
        setTimeout(() => {
            ...
            chrome.action.setTitle({ tabId, title: 'Read in nenva' }).catch(() => {});
        }, 2500);
...
chrome.runtime.onInstalled.addListener(() => {
    chrome.contextMenus.removeAll(() => {
        chrome.contextMenus.create({
            ...
            title: 'Read this page in nenva',
            contexts: ['page']
        });
        chrome.contextMenus.create({
            id: 'anjadhe-read-link',
            title: 'Read this link in nenva',
            contexts: ['link']
        });
    });
});
