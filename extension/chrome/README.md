# nenva Reader — Chrome companion extension

A one-button companion to the nenva desktop app. Click it on any article
and the page is handed to nenva on this Mac, where the News reader
...
- Only the current tab's **address and title**, only when you click, only
  to the nenva app on this machine, as an `anjadhe://read?url=…&title=…`
...
1. Install nenva (a packaged build — the `anjadhe://` scheme is registered
...
3. Pin **nenva Reader** to the toolbar.
4. On an article, click the button (or press **Alt+Shift+R**, or right-click
   → **Read this page in nenva** / **Read this link in nenva** on a link).
   Chrome asks "Open nenva?" the first time — tick *Always allow* for
