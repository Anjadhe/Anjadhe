...
**Personal AI that lives with your data, private by default.**

Cloud-based AI chat tools are great — until the question involves personal
or confidential data you don't want to share. nenva flips it around: your
life stays in nenva's own apps on your Mac — email, calendar, tasks,
...
Learn more at [anjadhe.ai](https://anjadhe.ai).
...
- **A canvas you can extend.** The built-in apps are the starter set —
  point any coding agent (Claude Code, Codex, …) at `~/Anjadhe/apps/`
  and build your own. New apps get storage, encrypted backup, sync, and
  the assistant's attention automatically.
...
- No account, no sign-up, no nenva database. Connect Gmail and mail
...
nenva is a macOS app (Apple Silicon or Intel).

- **Any Mac with 8 GB of memory or more can run the AI locally.** Setup
  downloads a free model sized to your machine and everything happens on
  it: on an 8 or 16 GB Mac that is nenva's own small Qwen model, tuned
  for the assistant's tools — light and fast, best at everyday tasks;
  from 32 GB up, larger open-weight models.
- **Want a stronger brain on a small Mac?** nenva Cloud — an open-weight
  model we host, free monthly allowance, no account — or your own
  OpenAI-compatible server, or your own OpenAI or Anthropic key. Each is
  an explicit choice, never a fallback.

**[Download the latest release](https://anjadhe.ai/download)**
