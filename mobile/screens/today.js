...
(function () {
  ...
  function render(host) {
    ...
    // The launcher door — the Apps grid is a pushed screen since 2026-08-31
    // (its function-bar slot went to the Assistant); this mirrors the
    // desktop titlebar's grid-icon app switcher.
    var appsBtn = App.el('<button class="head-action" type="button" aria-label="All apps"></button>');
    appsBtn.innerHTML = App.icons.grid;
    appsBtn.addEventListener('click', function () { App.open('apps'); });
    ...
    // --- assistant composer (the desktop home recipe: masthead, then the
    // composer, then what needs you) — a door, not a second send path: it
    // opens the Assistant root with the focus carried in. ---
    var ask = App.el('<section class="section" style="--i:' + (counter.i++) + '">'
      + '<button class="home-ask" type="button">'
      + App.icons.sparkle + '<span>Ask your assistant…</span>'
      + '</button></section>');
    ask.querySelector('.home-ask').addEventListener('click', function () {
      var scr = App.screens.assistant;
      if (scr && scr.openCompose) scr.openCompose(); else App.root('assistant');
    });
  ...
  App.registerScreen('today', { label: 'Home', render: render });
})();
