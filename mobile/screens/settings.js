...
(function () {
  ...
  function render(host) {
    ...
    // The assistant channel — honest copy about where the brain runs.
    var asec = App.el('<section class="section"></section>');
    asec.style.setProperty('--i', 2);
    asec.appendChild(App.el('<div class="section-label">Assistant</div>'));
    var card = App.el('<div class="card"></div>');
    card.innerHTML = '<div class="pair-status-sub" style="padding:12px 14px">'
      + 'The assistant runs on your Mac — your data and your model stay '
      + 'there, and this phone reaches it over the same encrypted '
      + 'connection as sync. On your home network the phone talks straight '
      + 'to your Mac; away from home, an encrypted relay forwards data it '
      + 'cannot read. Actions that need approval are confirmed on the Mac.'
      + '</div>';
  ...
  function buildPaired(section) {
    // Name the transport-ladder rung when the channel is up (mobile-sync.js):
    // read at render — every sync that applies changes re-renders the screen,
    // so this stays current without holding a state subscription.
    var sub = 'Your notes, tasks and journal sync both ways.';
    try {
      var t = window.AnjadheSync && window.AnjadheSync.getTransport
        ? window.AnjadheSync.getTransport() : null;
      if (t === 'direct') sub = 'Connected directly to your Mac on this network.';
      else if (t === 'relay') sub = 'Connected through the encrypted relay.';
    } catch (e) { /* preview host — no channel layer */ }
    section.appendChild(statusCard(true, 'Paired with your Mac', sub));
