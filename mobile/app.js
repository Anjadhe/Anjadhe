...
const App = {
  screens: {},
  // Function-bar navigation: the bottom bar holds VERBS (Home · Assistant ·
  // ＋ · Search), not apps, so it never grows. Roots have no back bar; opening
  // an app or record pushes a screen with a sticky top bar while the function
  // bar stays put. `apps` is the launcher grid order; the Apps grid itself is
  // a pushed screen since 2026-08-31 (doors: the Home grid icon and the ＋
  // sheet) — its slot in the bar went to the Assistant, the product spine.
  roots: ['today', 'assistant', 'search'],
  apps: ['tasks', 'goals', 'notes', 'journal', 'calendar', 'wellness',
    'fyi', 'news', 'portfolio', 'prompts', 'feed', 'bookmarks'],
  current: null,
  ...
  backTitle() {
    ...
    return (r && this.screens[r] && this.screens[r].label) || 'Home';
  },
  ...
  // (Home · Assistant · ＋ · Search), not apps, so it never outgrows its slots.
  initNavbar() {
    const bar = document.getElementById('navbar');
    if (!bar) return;
    bar.innerHTML = '';
    const items = [
      { id: 'today',     label: 'Home',      icon: this.icons.today,   action: () => this.root('today') },
      { id: 'assistant', label: 'Assistant', icon: this.icons.sparkle, action: () => this.root('assistant') },
  ...
  _formatInline(text) {
    ...
    out = out.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_, label, url) => {
      // anjadhe:// record links deep-link into the phone's own screens
      // (App.openRecordLink) — the same records synced from the Mac.
      const safe = /^(https?:|mailto:|anjadhe:)/i.test(url) ? url : '#';
  ...
  // --- Mac-served views (lane 2: data that deliberately doesn't sync) ----
  // A screen calls macView('insights'|'news'|'portfolio', ttlMs) on every
  // render: it returns the cached digest immediately and, when the cache is
  // past its TTL, asks the Mac in the background and re-renders on arrival.
  // Failures are remembered briefly (no hot retry loop) and surfaced as
  // `error` so screens can say honestly why the data is old.
  _viewInFlight: {},
  _viewError: {},
  _viewTriedAt: {},
  VIEW_RETRY_MS: 30 * 1000,
  macView(view, ttlMs) {
    var key = 'anjadhe:view:' + view;
    var cached = null;
    try { cached = JSON.parse(localStorage.getItem(key)); } catch (e) { /* no cache */ }
    var fresh = cached && (Date.now() - cached.at < ttlMs);
    var recentlyTried = (Date.now() - (this._viewTriedAt[view] || 0)) < this.VIEW_RETRY_MS;
    var self = this;
    if (!fresh && !this._viewInFlight[view] && !recentlyTried
        && window.AnjadheSync && typeof window.AnjadheSync.requestView === 'function') {
      this._viewInFlight[view] = true;
      this._viewTriedAt[view] = Date.now();
      window.AnjadheSync.requestView(view).then(function (data) {
        self._viewInFlight[view] = false;
        self._viewError[view] = null;
        try { localStorage.setItem(key, JSON.stringify({ at: Date.now(), data: data })); } catch (e) { /* full */ }
        self.refresh();
      }, function (err) {
        self._viewInFlight[view] = false;
        self._viewError[view] = (err && err.message) || 'could not reach your Mac';
        self.refresh();
      });
    }
    return {
      ...
      loading: !!this._viewInFlight[view],
      error: this._viewError[view] || null,
    };
  },
  // User-initiated refresh: forget the backoff stamp so the next render asks.
  macViewRefresh(view) {
    this._viewTriedAt[view] = 0;
    try {
      var key = 'anjadhe:view:' + view;
      var cached = JSON.parse(localStorage.getItem(key));
      if (cached) { cached.at = 0; localStorage.setItem(key, JSON.stringify(cached)); }
    } catch (e) { /* no cache to expire */ }
    this.refresh();
  },
  // "Updated 5m ago" — the honesty line under every Mac-served screen.
  agoLabel(ts) {
    if (!ts) return '';
  ...
  // A tapped <a> inside model-written content (feed posts, chat replies):
  // anjadhe:// record links open the record's own screen on the phone;
  // everything else opens outside the app. Call from a delegated click
  // handler with the anchor element (e.target.closest('a')).
  handleLinkTap(a) {
    const href = (a && a.getAttribute('href')) || '';
    if (/^anjadhe:/i.test(href)) { this.openRecordLink(href); return; }
    if (href && href !== '#') this.openExternal(href);
  },

  // anjadhe://<type>/<id> → the phone screen that shows that record. Types
  // the phone has no screen for yet (goal, email, insight, …) get an honest
  // toast instead of a dead tap. Mirrors the desktop RecordLinks vocabulary.
  RECORD_LINK_APPS: {
    task: 'tasks', note: 'notes', journal: 'journal', event: 'calendar',
    bookmark: 'bookmarks', routine: 'prompts',
  },
  openRecordLink(url) {
    const m = /^anjadhe:\/\/([a-z-]+)\/(.+)$/i.exec(String(url || '').trim());
    if (!m) return;
    const appId = this.RECORD_LINK_APPS[m[1].toLowerCase()];
    if (!appId || !this.screens[appId]) {
      this.toast('Open this one on your Mac');
      return;
    }
    let id = m[2];
    try { id = decodeURIComponent(id); } catch (e) { /* keep raw */ }
    this.openDetail(appId, id);
  },
  ...
  icons: {
    ...
    sparkle: '<svg class="i" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M12 4l1.8 4.6L18.4 10l-4.6 1.8L12 16.4l-1.8-4.6L5.6 10l4.6-1.4z"/><path d="M18.5 15.5l.9 2.1 2.1.9-2.1.9-.9 2.1-.9-2.1-2.1-.9 2.1-.9z"/></svg>',
  },
};
