...
async function setHuman(reason = 'user') {
...
function connect() {
    ...
    try {
        ...
        connection.onDisconnect.addListener(() => {
            ...
            if (port === connection) { port = null; connected = false; connecting = false; setHuman(); }
        });
...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
    ...
    // The ONLY way a page hands control to the user: its pill's button.
    if (message.kind === 'take-control' && sender.tab?.id === tabId) { setHuman(); respond({ ok: true }); return; }
    ...
    if (message.kind === 'takeover') { setHuman().then(() => queue).then(() => state()).then(respond); return true; }
});
...
chrome.tabs.onRemoved.addListener(id => {
    if (id !== tabId) return;
    tabId = null; windowId = null; generation++; setHuman(); chrome.storage.session.remove('agentTab');
});
