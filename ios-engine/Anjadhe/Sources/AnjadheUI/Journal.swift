            ...
            if store.findItem("journal", "entries", id: id) == nil && !loaded {
                ...
                VStack(spacing: 0) {
                    ...
                    RichEditorView(html: $html, placeholder: "How was today?")
                        .onChange(of: html) { v in PendingWrites.shared.schedule("journal:\(id):content", after: PendingWrites.shortDelay) { store.patchItem("journal", "entries", id: id, ["content": .string(v)]) } }
                }
            }
        }
        .pushedScreen()
