...
struct SettingsView: View {
    ...
    @State private var simpleHome = SimpleExperience.isOn
    ...
    var body: some View {
        ScreenColumn {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                Toggle(isOn: $simpleHome) {
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Simple home").font(.system(size: 15, weight: .semibold)).foregroundStyle(Theme.text)
                        Text("Your assistant, what needs you today, and what your routines found. Turn it off for the full home with today's tasks and what you were writing.")
                            .font(.system(size: 13)).foregroundStyle(Theme.textTertiary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
                }
                .tint(Theme.text)
                .onChange(of: simpleHome) { on in
                    // Per device, like the theme — see SimpleExperience. No
                    // reload needed: SwiftUI rebuilds the root on its own.
                    SimpleExperience.setOn(on)
                    router.showToast(on ? "Simple home on" : "Full home on")
                }
