...
struct RichEditorView: UIViewRepresentable {
    ...
    /// SwiftUI is taking the editor away: ask the page for whatever is in
    /// flight before it goes. The editor's own `pagehide` covers the app
    /// being killed; this covers an ordinary back-navigation.
    ...
    final class Coordinator: NSObject, WKScriptMessageHandler, WKNavigationDelegate {
        ...
        /// Push out anything typed inside the debounce window. Called when
        /// the editor is dismantled, so the last characters are never the
        /// ones that got away.
        func flush() { webView?.evaluateJavaScript("window.flushContent && flushContent()") }
    ...
    static func page(_ ph: String) -> String {
        ...
        <script>
          ...
          // Posting on EVERY keystroke handed Swift the whole body each time,
          // which then rewrote and re-encoded the whole notes blob. The
          // content is debounced; a formatting command, losing focus and the
          // page going away all post immediately, so nothing typed is lost.
          var t=null;
          function post(){try{webkit.messageHandlers.rich.postMessage(ed.innerHTML);}catch(e){}}
          function postSoon(){if(t)clearTimeout(t);t=setTimeout(function(){t=null;post();},250);}
          function postNow(){if(t){clearTimeout(t);t=null;}post();}
          function cmd(c){document.execCommand(c,false,null);postNow();}
          function blk(t2){var cur=document.queryCommandValue('formatBlock');
            document.execCommand('formatBlock',false,(cur&&cur.toUpperCase()===t2)?'P':t2);postNow();}
          ed.addEventListener('input',postSoon);
          ed.addEventListener('blur',postNow);
          window.addEventListener('pagehide',postNow);
          document.addEventListener('visibilitychange',function(){if(document.hidden)postNow();});
          // Native calls this when the editor is going away.
          window.flushContent=postNow;
