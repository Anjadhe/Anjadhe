            ...
            if store.findItem("prompts", "prompts", id: id) == nil && !loaded {
                ...
                Form {
                    TextField("Routine title", text: $title, axis: .vertical).lineLimit(1...4)
                        .displayStyle(20)
                        .onChange(of: title) { v in PendingWrites.shared.schedule("prompt:\(id):title") { store.patchItem("prompts", "prompts", id: id, ["title": .string(v)]) } }
                    Section {
                        TextField("Write the prompt…", text: $body_, axis: .vertical).lineLimit(6...)
                            .onChange(of: body_) { v in PendingWrites.shared.schedule("prompt:\(id):body") { store.patchItem("prompts", "prompts", id: id, ["body": .string(v)]) } }
                    }
                    Section {
