...
public final class SyncCoordinator: NSObject, ObservableObject {
    ...
    /// The rung named for the UI: "lan" (the Mac's relay on this network),
    /// "tailscale" (the same relay through Tailscale Funnel, from anywhere),
    /// "relay" (hosted), or nil while offline.
    @Published public private(set) var via: String?
    ...
    /// A chat push from the Mac: `kind` is "chat-ack" | "chat-delta" |
    /// "chat-reply" | "chat-error". A delta is a SLICE of the answer being
    /// written, numbered by `seq` so a gap after a reconnect is detectable;
    /// the finished answer still arrives as "chat-reply", so deltas are pure
    /// UX and losing every one of them costs nothing.
    public struct ChatEvent {
        public let kind: String
        public let text: String
        public let convId: String?
        public let seq: Int?
        /// "chat-ask": the approval the Mac is asking for; "chat-ask-done":
        /// `{askId, approved}` once it is settled (any phone, or timeout).
        public var ask: JSONValue? = nil
    }
    public let chat = PassthroughSubject<ChatEvent, Never>()

    /// Store keys the Mac and this phone now AGREE on, published once a
    /// change has settled in both directions (2026-09-22).
    ///
    /// Two things reach it. A key the Mac sent DOWN arrives as a `persist`
    /// and is republished here the moment it is applied. A key this phone
    /// sent UP waits for the Mac's `sync-values-ack`, because until then the
    /// Mac would answer a view request with the rows it already had.
    ///
    /// It exists for the screens the MAC computes (`MacViews`): the synced
    /// blobs redraw themselves through `AppStore.revision`, but a Mac-served
    /// digest is a cached answer that nothing else can tell is out of date.
    public let dataSettled = PassthroughSubject<Set<String>, Never>()
    private var settledPending: Set<String> = []
    private var settledScheduled = false

    /// One publish per runloop turn — a sync lands its keys one message at a
    /// time, and a refetch per key would ask the Mac forty times for one view.
    private func noteSettled(_ key: String) {
        settledPending.insert(key)
        guard !settledScheduled else { return }
        settledScheduled = true
        DispatchQueue.main.async { [weak self] in
            guard let self = self else { return }
            self.settledScheduled = false
            let keys = self.settledPending
            self.settledPending = []
            if !keys.isEmpty { self.dataSettled.send(keys) }
        }
    }
    ...
    /// A sync applies keys one message at a time, and each used to bump the
    /// store — so landing 40 keys re-rendered every screen 40 times over.
    /// One bump per runloop turn is the same information for a fortieth of
    /// the work, and screens cannot tell the difference: they read the store
    /// when they redraw, not from the notification.
    ...
    public func forgetPairing() {
        ...
        // The nenva Cloud key was this Mac's (P4); it goes with the pairing.
    ...
    /// Answer an approval the Mac asked for (`chat-ask`). `completion(false)`
    /// when there is no live channel — the card stays and is answered later.
    public func answerAsk(_ askId: String, approved: Bool, scope: String, completion: @escaping (Bool) -> Void) {
        let js = "(function(){ try { return !!(window.AnjadheSync && AnjadheSync.answerAsk && AnjadheSync.answerAsk(\(Self.jsString(askId)), \(approved ? "true" : "false"), \(Self.jsString(scope)))); } catch (e) { return false; } })()"
        #if canImport(WebKit)
        guard let wv = webView else { completion(false); return }
        wv.evaluateJavaScript(js) { result, _ in DispatchQueue.main.async { completion((result as? Bool) ?? false) } }
        #else
        completion(false)
        #endif
    }
    ...
    ///
    /// MUST wait for the native store to finish loading (2026-09-20). The
    /// disk read moved off the main thread so launch is not blocked on it,
    /// which means this can now run first — and an EMPTY snapshot here makes
    /// the JS mirror empty, which makes the phone's manifest empty, which the
    /// Mac reads as a first sync and answers by sending everything. Anything
    /// edited on the phone and not yet uploaded would be overwritten. The JS
    /// side guards the same race with `__anjadheStoreReady`; this is its
    /// native half.
    fileprivate func hydrate() {
        store.whenHydrated { [weak self] in self?.hydrateNow() }
    }

    private func hydrateNow() {
      ...
      (function hook() {
        if (window.AnjadheSync && window.AnjadheSync.onStateChange) {
          window.AnjadheSync.onStateChange(function (s) {
            ...
            var v = null;
            try { v = window.AnjadheSync.getVia ? window.AnjadheSync.getVia() : null; } catch (e) {}
            try { window.webkit.messageHandlers.anjadhe.postMessage({ type: 'syncState', state: s, transport: t, via: v }); } catch (e) {}
          });
          if (window.AnjadheSync.onChat) {
            window.AnjadheSync.onChat(function (m) {
              var ask = m.type === 'chat-ask' ? (m.ask || null) : (m.type === 'chat-ask-done' ? { askId: m.askId || '', approved: !!m.approved } : null);
              try { window.webkit.messageHandlers.anjadhe.postMessage({ type: 'chat', kind: String(m.type || ''), text: String(m.text || ''), convId: m.convId || null, seq: typeof m.seq === 'number' ? m.seq : null, ask: ask }); } catch (e) {}
            });
          }
          if (window.AnjadheSync.onSynced) {
            window.AnjadheSync.onSynced(function (keys) {
              try { window.webkit.messageHandlers.anjadhe.postMessage({ type: 'synced', keys: keys }); } catch (e) {}
            });
          }
...
extension SyncCoordinator: WKScriptMessageHandler {
    public func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        case "syncState":
            let t = msg.transport
            let v = msg.via
            if let s = msg.state { DispatchQueue.main.async { self.state = s; self.transport = t; self.via = v } }
            print("[sync] state → \(msg.state ?? "?") (\(v ?? t ?? "no transport"))")
            logLine("state → \(msg.state ?? "?")" + ((v ?? t).map { " (\($0))" } ?? ""))
        case "pairResult":
        ...
        case "chat":
            let ev = ChatEvent(kind: msg.kind ?? "", text: msg.text ?? "", convId: msg.convId, seq: msg.seq, ask: msg.ask)
            DispatchQueue.main.async { self.chat.send(ev) }
        case "synced":
            // The Mac acked our upload — these keys are now ITS truth too.
            for key in msg.keys ?? [] where !key.hasPrefix("anjadhe:channel:") { noteSettled(key) }
        case "viewData":
