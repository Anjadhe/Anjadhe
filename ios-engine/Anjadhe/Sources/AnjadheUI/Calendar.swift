...
struct CalendarView: View {
    ...
    /// Scheduled tasks on the calendar — the desktop's "Show tasks" switch
    /// (`CalendarApp.showTasks`), on the phone.
    ///
    /// Deliberately PER-PHONE rather than the desktop's own field. Over there
    /// it lives inside the synced `calendar` blob, which also carries the
    /// whole event cache under whole-key last-writer-wins — so a phone
    /// writing that blob to flip one boolean could clobber the events the Mac
    /// had just fetched. It is a display preference either way, and the phone
    /// keeps its own like the theme and the side-nav collapse.
    @AppStorage("calendar-show-tasks") private var showTasks = true

    private var tasks: [JSONValue] { showTasks ? store.items("schedule", "scheduleItems") : [] }
    ...
    private var agenda: some View {
        ...
        return VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                ...
                Spacer()
                // The one control on this screen: events are the calendar's
                // own, tasks are another app's records shown here as a
                // courtesy — and a day of tasks can bury the appointments.
                Button { showTasks.toggle() } label: {
                    HStack(spacing: 4) {
                        Image(systemName: showTasks ? "checkmark.square.fill" : "square")
                            .font(.system(size: 13))
                        Text("Tasks").font(.system(size: 12, weight: .medium))
                    }
                    .foregroundStyle(showTasks ? Theme.text : Theme.textTertiary)
                    .padding(.horizontal, 9).padding(.vertical, 4)
                    .background(Capsule().fill(Theme.surface))
                    .overlay(Capsule().strokeBorder(Theme.border))
                }
                .buttonStyle(.plain)
                .accessibilityLabel(showTasks ? "Hide tasks" : "Show tasks")
            }
            if rowCount == 0 {
                EmptyText(showTasks ? "Nothing on this day." : "No events on this day. Tasks are hidden.")
            } else {
                CardList {
