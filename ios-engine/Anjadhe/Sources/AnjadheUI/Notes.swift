            ...
            if store.findItem("notes", "notes", id: id) == nil && !loaded {
                ...
                VStack(spacing: 0) {
                    TextField("Title", text: $title, axis: .vertical).lineLimit(1...4)
                        ...
                        .onChange(of: title) { v in PendingWrites.shared.schedule("note:\(id):title") { store.patchItem("notes", "notes", id: id, ["title": .string(v)]) } }
                    Divider().padding(.top, 10)
                    RichEditorView(html: $html, placeholder: "Write…")
                        .onChange(of: html) { v in PendingWrites.shared.schedule("note:\(id):content", after: PendingWrites.shortDelay) { store.patchItem("notes", "notes", id: id, ["content": .string(v)]) } }
                }
            }
        }
        .pushedScreen()
