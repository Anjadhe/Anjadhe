...
// The native shell: one NavigationStack per root (Home · Assistant · Apps ·
// Search), the function bar beneath (those four plus ＋), the ＋ capture
// sheet, and the toast. Records are pushed screens (`Route`) on the root they
// were opened from.
//
...
struct Shell: View {
    ...
    var body: some View {
        ...
        .animation(.easeOut(duration: 0.18), value: showBar)
    ...
    @ViewBuilder private var content: some View {
        ...
        case .home:
            NavigationStack(path: router.binding(for: .home)) {
                // Which home (2026-09-20). A preference, not a flag, and
                // read live: unlike the Mac — where `SimpleExperience.init()`
                // has no teardown, so its switch has to reload the window —
                // SwiftUI just rebuilds, so the phone's switch takes effect
                // at once. See SimpleHomeView.
                if SimpleExperience.isOn {
...
struct RouteView: View {
    ...
    var body: some View {
        ...
        case .portfolioTickers: PortfolioTickersView()
        ...
        case .newsArticle(let url): NewsReaderView(url: url)
        case .newsTopics: NewsTopicsView()
        case .newsSaved: NewsSavedView()
        }
    }
}
...
struct FunctionBar: View {
    ...
    var body: some View {
        // Five slots: four roots plus the ＋. The Apps tab (2026-09-21)
        ...
        HStack(spacing: 0) {
            ...
            item(.apps, "Apps", "square.grid.2x2")
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            VStack(alignment: .leading, spacing: 3) {
                Text(title).displayStyle(greeting ? 30 : 28)
