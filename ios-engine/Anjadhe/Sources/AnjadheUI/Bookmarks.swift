                ...
                Form {
                    Section {
                        ...
                        TextField("Link title", text: $title, axis: .vertical).lineLimit(1...4)
                            .onChange(of: title) { v in PendingWrites.shared.schedule("bookmark:\(id):title") { store.patchItem("bookmarks", "bookmarks", id: id, ["title": .string(v.trimmingCharacters(in: .whitespaces))]) } }
                    }
                    Section {
                        fieldLabel("URL")
                        TextField("https://", text: $url).autocorrectionDisabled().urlKeyboard()
                            .onChange(of: url) { v in PendingWrites.shared.schedule("bookmark:\(id):url") { store.patchItem("bookmarks", "bookmarks", id: id, ["url": .string(v.trimmingCharacters(in: .whitespaces))]) } }
                    }
                    Section {
                        fieldLabel("Description")
                        TextField("Optional note", text: $desc, axis: .vertical).lineLimit(2...4)
                            .onChange(of: desc) { v in PendingWrites.shared.schedule("bookmark:\(id):desc") { store.patchItem("bookmarks", "bookmarks", id: id, ["description": .string(v.trimmingCharacters(in: .whitespaces))]) } }
                    }
                    Section {
