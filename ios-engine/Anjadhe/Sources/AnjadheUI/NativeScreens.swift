    ...
    var body: some View {
        Form {
            Section("Status") {
                ...
                if sync.transport != nil {
                    HStack {
                        Text("Path"); Spacer()
                        Text(sync.transport == "direct" ? "Direct to your Mac" : "Via encrypted relay")
...
struct TodayView: View {
    ...
    enum Dest: Hashable { case task(String), note(String), journal(String) }
...
struct SearchView: View {
    ...
    enum SearchDest: Hashable { case task(String), note(String), journal(String), bookmark(String), prompt(String) }
        ...
        NavigationStack {
            List {
                if query.trimmingCharacters(in: .whitespaces).isEmpty {
                    Text("Search your tasks, notes, journal, bookmarks and prompts.").foregroundStyle(Theme.textTertiary)
