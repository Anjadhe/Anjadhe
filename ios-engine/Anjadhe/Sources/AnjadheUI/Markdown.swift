...
///
/// This is the ONLY renderer for prose. There used to be a `markdownText`
/// helper beside it that rendered inline styling only; three model-written
/// surfaces used it (the portfolio brief, a ticker's business profile, an
/// article summary) and put `##` headings and `-` bullets on the screen as
/// literal characters, which is what Ram reported. It is gone; collapse a long
/// body with `RecordText.preview`, never a character slice.
///
/// It takes a record's body in EITHER format (2026-09-21). Notes, journal
/// entries and routine results are stored as HTML, and feeding that to a
/// Markdown renderer put `<p>` and `<li>` on the screen. `RecordText.markdown`
/// translates markup back to Markdown when it sees any, so the fix lands
/// wherever a body is rendered rather than at one call site — see RecordText
/// for why translating beats stripping.
struct MarkdownView: View {
    let text: String

    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ForEach(Array(Self.parse(RecordText.markdown(text)).enumerated()), id: \.offset) { idx, b in
    ...
    @ViewBuilder private func blockView(_ b: Block, first: Bool) -> some View {
        ...
        case .heading(let lvl, let s):
            ...
            inline(s).displayStyle(lvl == 1 ? 22 : lvl == 2 ? 19 : 17)
