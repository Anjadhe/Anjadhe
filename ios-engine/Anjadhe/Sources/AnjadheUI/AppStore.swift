...
public final class AppStore: ObservableObject {
    ...
    ///
    /// The file is read OFF THE MAIN THREAD and the store returns straight
    /// away: decoding it measured 223 ms for 8.9 MB on a Mac, inside
    /// `AppRoot.init()`, before the first frame — a launch tax on every
    /// single start. The safety rules that makes necessary live in
    /// `DiskStore.attachAsync` and `KVStore.hydrate`; `hydrated` and
    /// `whenHydrated` are how callers wait when they genuinely must.
    public static func persistent(directory: URL? = nil) -> AppStore {
        let kv = KVStore()
        let store = AppStore(kv: kv)
        let disk = DiskStore(directory: directory)
        store.disk = disk
        disk.attachAsync(to: kv) { [weak store] in
            guard let store = store else { return }
            LaunchTrace.mark("store hydrated")
            store.hydrated = true
            store.revision += 1
            let waiters = store.hydrationWaiters
            store.hydrationWaiters = []
            for w in waiters { w() }
        }
        return store
    }
    ...
    /// Every blob write funnels through here — which is why the hydration
    /// guard lives here and nowhere else.
    ///
    /// A write before the store has loaded would be built from an EMPTY
    /// read, and `KVStore.hydrate` preserves what is already in the cache,
    /// so that truncated blob would win and then be written to disk over the
    /// real one. The splash holds the UI until `hydrated`, so this is
    /// unreachable through the interface — but "unreachable by UI timing" is
    /// not a guarantee, so a write that does arrive early completes the load
    /// first. It costs one blocking read, once, instead of losing notes.
    public func saveBlob(_ blobKey: String, _ blob: [String: JSONValue]) {
        if !hydrated { disk?.hydrateNowIfNeeded() }
