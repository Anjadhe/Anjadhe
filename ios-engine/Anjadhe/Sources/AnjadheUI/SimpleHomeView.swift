...
struct SimpleHomeView: View {
                        ...
                        ForEach(Array(updates.enumerated()), id: \.element.id) { i, u in
                            RowView(u.title,
                                    ...
                                    sub: u.failed ? "Did not finish"
                                        : feedPreviewText(u.content, id: u.id, stamp: u.stamp, max: 70),
    ...
    /// What is left of the shortcuts row (2026-09-21).
    ///
    /// The row itself is gone: a sideways-scrolling strip of app pills hid
    /// most of the apps behind a gesture nobody makes, and the launcher is
    /// the Apps tab in the function bar now. The journal NUDGE is not a
    /// shortcut though — it is the app asking for something, the same
    /// `journalNudgeDue` rule the desktop's home strip reads — so it keeps
    /// its door, alone, and only when it is due.
    @ViewBuilder private func journalNudge(_ due: Bool) -> some View {
        if due {
            Button { router.open(app: "journal") } label: {
                HStack(spacing: 6) {
                    Image(systemName: "book.closed").font(.system(size: 12))
                    Text("Log journal").font(.system(size: 14, weight: .medium))
                }
