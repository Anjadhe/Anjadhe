...
struct NewsReaderView: View {
    ...
    @ViewBuilder private func summarySection(_ a: [String: JSONValue]?, pending: Bool) -> some View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            if !summary.isEmpty {
                let short = summary.count > 1200 && !summaryOpen
                markdownText((short ? String(summary.prefix(1200)) + "…" : summary))
                    .font(.system(size: 15)).foregroundStyle(Theme.text).lineSpacing(5)
                    .fixedSize(horizontal: false, vertical: true)
