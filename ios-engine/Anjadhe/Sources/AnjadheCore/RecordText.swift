...
public enum RecordText {
    ...
    /// A collapsed preview of a body, cut on a LINE boundary (2026-09-22).
    ///
    /// `MarkdownView` reads a body as BLOCKS, so cutting it at an arbitrary
    /// character lands mid-heading or mid-bullet and the tail renders as a
    /// broken block. Cut at the last newline that fits instead and let whole
    /// blocks be what the preview shows. A cut that would throw away most of
    /// the budget (a long first paragraph with no newline in it) falls back to
    /// the character cut: half a paragraph still reads, half a heading does not.
    public static func preview(_ body: String, _ max: Int) -> String {
        if body.count <= max { return body }
        let cut = String(body.prefix(max))
        guard let brk = cut.lastIndex(of: "\n"),
              cut.distance(from: cut.startIndex, to: brk) > max / 3 else { return cut + "…" }
        // Nor may it END on a heading: that promises a section the reader
        // cannot see, which is worse than stopping a line earlier.
        var lines = cut[cut.startIndex..<brk].components(separatedBy: "\n")
        while let last = lines.last?.trimmingCharacters(in: .whitespaces), last.isEmpty || last.hasPrefix("#") {
            lines.removeLast()
        }
        guard !lines.isEmpty else { return cut + "…" }
