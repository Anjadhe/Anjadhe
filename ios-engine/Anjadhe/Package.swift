...
//                   type-checks it); runs in the iOS app (ios/App). Resources
//                   hold the launch glyph; type comes from the OS (Charter).
let package = Package(
    name: "Anjadhe",
    ...
    targets: [
        ...
        // Fixtures/ are read by path (#filePath), and shared with the Mac's
        // tests/task-list-parity-test.js — never bundled.
        .testTarget(name: "AnjadheCoreTests", dependencies: ["AnjadheCore"], exclude: ["Fixtures"]),
    ]
)
