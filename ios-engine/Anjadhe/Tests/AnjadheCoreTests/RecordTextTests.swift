...
final class RecordTextTests: XCTestCase {
    ...
    // MARK: preview — a collapsed body is cut on a block boundary

    /// The brief is headings and bullets; a character cut lands inside one and
    /// the tail renders as a broken block.
    func testPreviewCutsOnALineBoundary() {
        let body = """
            ## The market today
            ...
            A long paragraph about the portfolio that runs past the budget.
            """
        let out = RecordText.preview(body, 80)
        XCTAssertTrue(out.hasSuffix("…"), "a cut says it was cut: \(out)")
        // The budget reaches into "## Your book", which would leave a heading
        // promising a section the reader cannot see.
        XCTAssertFalse(out.contains("## Your"), "a preview never ends on a heading")
        // Whatever is kept ends a whole line, never mid-word or mid-marker.
        let kept = out.replacingOccurrences(of: "\n\n…", with: "")
        XCTAssertTrue(body.hasPrefix(kept), "the preview is a prefix of the body")
        XCTAssertEqual(kept.last, ".", "got: \(kept)")
        XCTAssertTrue(kept.hasSuffix("- Yields eased."), "got: \(kept)")
    }

    func testPreviewLeavesAShortBodyAlone() {
        XCTAssertEqual(RecordText.preview("Short enough.", 400), "Short enough.")
        XCTAssertEqual(RecordText.preview("", 400), "")
    }

    /// One long paragraph with no newline to cut at: half a paragraph still
    /// reads, so the character cut is the honest fallback.
    func testPreviewFallsBackWhenThereIsNoBreak() {
        let body = String(repeating: "word ", count: 200)
        let out = RecordText.preview(body, 100)
        XCTAssertEqual(out.count, 101, "100 characters plus the ellipsis")
        XCTAssertTrue(out.hasSuffix("…"))
    }

    /// A break so early that the preview would say almost nothing is not worth
    /// taking — the budget goes to the text, not to the boundary.
    func testPreviewIgnoresAnEarlyBreak() {
        let body = "Title\n" + String(repeating: "x", count: 500)
        let out = RecordText.preview(body, 120)
        XCTAssertTrue(out.count > 100, "an early newline must not eat the preview: \(out.count)")
    }
