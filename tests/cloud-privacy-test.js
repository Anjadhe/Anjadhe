...
assert.ok(CP.guardSource('email-bundles').blocked);
...
assert.strictEqual(CP.guardSource('email-bundles'), null, 'email on a local model is not gated');
...
assert.ok(CP.guardSource('email-bundles').blocked, 'email on a cloud model is gated under a local brain');
