...
(async () => {
    ...
    try {
        ...
        // Looking at the window is not taking it over. A stray click or key in the
        // agent's tab is swallowed: the page does not get it, the team is not paused.
        ...
        await human.locator('h1').click({ force: true }); await human.keyboard.press('a');
        await delay(300);
        assert.equal(runtime.state().control, 'agent', 'A stray click or key must not take control');
