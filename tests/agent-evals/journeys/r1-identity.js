...
const RESET = `
  (() => {
    ...
    // The engine's own startup tick may still be in flight: a demo routine
    // whose time has passed today (Daily Briefing, 08:00) fires ~8 s after
    // launch and its real run never settles here. Its _ticking flag made
    // every tick below return at once, so these journeys passed or failed
    // by the hour the gate ran (2026-09-24). Release it with _draining.
    RoutineEngine._ticking = false;
  })()
`;
