...
module.exports = [
  ...
  {
    id: 'ss-overdue-to-today',
    name: 'list_schedule rows carry date+overdue (never the query day); collapse:true lands every overdue task on today',
    kind: 'det',
    async run({ page }) {
      return await page.evaluate(async () => {
        const iso = (off) => {
        ...
        const mk = async (title, date) => {
          const r = await AgentTools.handlers.create_schedule_item({ title, scheduledDate: date, startTime: '14:00' });
          return r.item?.id || r.id;
        };
        const made = [];
        try {
          made.push(await mk('eval ss overdue launch hunt', iso(-2)));
          made.push(await mk('eval ss overdue call agent', iso(-1)));
          made.push(await mk('eval ss overdue future one', iso(3)));

          // The 2026-08-30 failure: a search hit showed only the query day
          // (today) next to a start time, and the model called a 2-day-old
          // task "already scheduled for today". Rows must carry their own
          // date + overdue, and the envelope must NOT say date for a search.
          const hit = await AgentTools.handlers.list_schedule({ search: 'eval ss overdue launch hunt' }, {});
          const row = (hit.items || [])[0] || {};
          const rowsHonest = hit.today === iso(0) && hit.date === undefined
            && row.date === iso(-2) && row.overdue === true;
          const future = await AgentTools.handlers.list_schedule({ search: 'eval ss overdue future one' }, {});
          const futureHonest = (future.items || [])[0]?.date === iso(3) && (future.items || [])[0]?.overdue === undefined;

          // The consent dialog names the landing day, not a delta.
          const dialog = AgentUI._describeToolAction('shift_schedule_items',
            { ids: made, date_to: 'yesterday', anchor_date: 'today', collapse: true });
          const dialogNames = dialog.includes('2 task') && dialog.includes(iso(0));

          const res = await AgentTools.handlers.shift_schedule_items({
            ids: made, date_to: 'yesterday', anchor_date: 'today', collapse: true
          });
          const byId = new Map((StorageManager.get('schedule')?.scheduleItems || []).map(i => [i.id, i]));
          const landed = byId.get(made[0]).scheduledDate === iso(0)
            && byId.get(made[1]).scheduledDate === iso(0)
            && byId.get(made[2]).scheduledDate === iso(3);   // outside the window: untouched
          const reported = res.success && res.count === 2 && res.landedOn === iso(0);

          return {
            pass: rowsHonest && futureHonest && dialogNames && landed && reported,
            detail: JSON.stringify({ rowsHonest, futureHonest, dialogNames, landed, reported, row, res })
          };
        } finally {
          for (const id of made) { try { await AgentTools.handlers.delete_schedule_item({ id }); } catch {} }
        }
      });
    }
  }
