...
module.exports = [
  ...
  },
  {
    ...
    name: 'library tools ship always-on (flag graduated 2026-08-08) and sit in the untrusted blocklist (V4)',
    kind: 'det',
    async run({ page }) {
      return await page.evaluate(() => {
        const present = AgentTools.definitions.some(d => d.function && d.function.name === 'search_library')
          && AgentTools.definitions.some(d => d.function && d.function.name === 'read_library_doc')
          && !!AgentTools.handlers.search_library && !!AgentTools.handlers.read_library_doc;
