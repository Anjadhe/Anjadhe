...
module.exports = [
  ...
  },
  {
    ...
    name: 'unresolved items get explicit UNKNOWN rows (no silent holes) and are re-attempted on retry',
    kind: 'det',
    async run({ page }) {
      return await page.evaluate(async () => {
        ...
        const unknownRow = rows1.find(r => r.item === 'Gamma Three');
        const holeFilled = out1.ok === false && rows1.length === 3
          && unknownRow && unknownRow.unknown === true && /unknown/.test(unknownRow.data)
          && /unknown: Gamma Three/.test(out1.note);
        ...
        const retried = out2.ok === true && rows2.length === 3
          && gamma && !gamma.unknown && gamma.data === 'found on retry';
