  ...
  },
  {
    ...
    async run({ page }) {
      return await page.evaluate(async () => {
        ...
        // I1 boundary: a concrete filesystem path is materialized by
        // reference — the fs step that reads it is its existence check,
        // so "the file <path>" never pays for the gate (which, shown
        // only chat context, could answer nothing but "absent").
        const b2 = intakeCalls;
        const r3 = await TaskService.start('Read the file /tmp/evals/statement.txt and note the closing balance', null);
        const pathSkipped = intakeCalls === b2 && !r3.error;
        if (r3.taskId) TaskService.cancel(r3.taskId);
        TaskService._chat = origChat;
        return { pass: gated && noted && skipped && pathSkipped, detail: JSON.stringify({ gated, noted, skipped, pathSkipped, intakeCalls }) };
      });
    }
  },
  {
