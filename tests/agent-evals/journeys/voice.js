/**
 * Voice laws (docs/LIBRARY.md L2, js/apps/library/voice-store.js +
...
module.exports = [
  ...
  },
  {
    ...
    name: 'draft_in_style is cut with the library flag off and untrusted-blocked always (V4)',
    kind: 'det',
    async run({ page }) {
      return await page.evaluate(() => {
        const flagOff = !FEATURES.isEnabled('library');
        const defCut = !AgentTools.definitions.some(d => d.function && d.function.name === 'draft_in_style');
        const handlerCut = !AgentTools.handlers.draft_in_style;
        const blocked = AgentService.UNTRUSTED_BLOCKED_TOOLS.has('draft_in_style');
        const pass = flagOff ? (defCut && handlerCut && blocked) : blocked;
        return { pass, detail: JSON.stringify({ flagOff, defCut, handlerCut, blocked }) };
      });
    }
  },
  {
  ...
  },
  {
    ...
    kind: 'model',
    async run({ page, docs }) {
      // The eval profile leaves the library flag off and the tool registry
      // is built at script load — enable and reload, then restore both at
      // the end so later journeys see the slate they expect.
      const setFlag = async (on) => {
        await page.evaluate((enable) => {
          const cur = new Set((localStorage.getItem('anjadheFeatures') || '').split(',').filter(Boolean));
          if (enable) cur.add('library'); else cur.delete('library');
          if (cur.size) localStorage.setItem('anjadheFeatures', [...cur].join(','));
          else localStorage.removeItem('anjadheFeatures');
        }, on);
        await page.reload();
