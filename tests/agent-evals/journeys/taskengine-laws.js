...
module.exports = [
  ...
  {
    // The other half of I9: genuine absence still fails. Mismatch is not
    // absence, but absence is not mismatch either — a model that answers
    // cleanly for two of three items has told us something real about the
    // third, and the count-check retry must still fire for it.
    id: 'i9-genuine-absence-still-fails',
    name: 'an item nothing answered for still gets the informed retry and an UNKNOWN row',
    kind: 'det',
    async run({ page }) {
      return await page.evaluate(async (seedSrc) => {
        const SEED = eval(seedSrc);
        ...
        const pass = out.ok === false && retries === 1
          && gamma && gamma.unknown === true && !gamma.unverified
          && /unknown: Gamma Three/.test(out.note);
