...
module.exports = [
  {
    ...
    async run({ page }) {
      return await page.evaluate(() => {
        ...
        // News left the default-hidden set 2026-09-11 (enabled by default
        // again, by request); Wellness and Portfolio still seed hidden.
        const hidden = ['wellness', 'portfolio'].every(id => AppManager.DEFAULT_HIDDEN_APPS.includes(id))
          && !AppManager.DEFAULT_HIDDEN_APPS.includes('news');
  ...
  },
  {
    ...
    async run({ page }) {
      return await page.evaluate(() => {
        const expected = ['agent','actions','goals','calendar','email','fyi','notes','journal','prompts','bookmarks','news','reader','wellness','portfolio','pomodoro','settings','aiactivity','help','about'];
  ...
  {
    id: 'pkg-bundled-bookmarks-launcher-and-api',
    name: 'Bookmarks loads from its package and the shell acts only through its seams',
    kind: 'det',
    async run({ page }) {
      return await page.evaluate(async () => {
        ...
        // Its door was the Web Browser's new-tab page until that app was
        // removed (2026-09-20); it is a launcher app of its own now.
        const subapp = !!tile && !tile.dataset.subappOf && GlobalSearch.launcherApps().some(a => a.id === 'bookmarks');
