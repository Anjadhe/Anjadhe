...
module.exports = [
  ...
  {
    id: 'wellness-activity-distance',
    name: 'Activity entries carry distance in the user\'s unit (2026-08-31: "2 miles" had no field, so the model invented a duration)',
    kind: 'det',
    async run({ page }) {
      return await page.evaluate(async () => {
        const logged = await AgentTools.handlers.log_wellness({
          kind: 'activity', activityType: 'Walk', distance: 2,
          time: '2026-08-30T21:00'
        });
        const stored = logged.success
          && logged.entry.distance === 2
          && logged.entry.unit === WellnessApp.unitFor('distance')
          && logged.entry.duration == null;          // never invented
        const label = WellnessApp.activityLabel(logged.entry);
        const rendered = /2\s*(mi|km)/.test(label);
        const listed = await AgentTools.handlers.list_wellness({ kind: 'activity', limit: 5 });
        const surfaced = (listed.entries || []).some(e => e.id === logged.entry.id && e.distance === 2)
          && !!listed.units.distance;
        await AgentTools.handlers.delete_wellness({ id: logged.entry.id });
        const pass = stored && rendered && surfaced;
        return { pass, detail: JSON.stringify({ stored, rendered, surfaced, label }) };
      });
    }
  },
  {
