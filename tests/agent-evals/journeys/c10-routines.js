...
module.exports = [
  ...
  },
  {
    ...
    name: 'a transient digest failure retries across ticks before an error card posts, and recovers silently',
    kind: 'det',
    async run({ page }) {
      return await page.evaluate(async (reset) => {
        eval(reset);
        ...
        try {
          ...
          // Engine path: two ticks of silence, the card on the third.
          ...
          await RoutineEngine.drain();
          const afterThird = { queued: !!queued(), cards: cards().length,
          ...
          const pass = manualPosted
            ...
            && !afterThird.queued && afterThird.cards === 2 && afterThird.errored
            && midRecovery.attempts === 1
            && !recovered.queued && recovered.cards === 3 && recovered.good && recovered.errorCleared;
          return { pass, detail: JSON.stringify({ wasStuck, manualPosted, afterFirst, afterSecond, afterThird, midRecovery, recovered }) };
        } finally {
