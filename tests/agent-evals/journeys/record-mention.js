    ...
    async run({ page }) {
      return await page.evaluate(async () => {
        ...
        try {
          ...
          const covered = key('Project').startsWith('goals:')
    ...
    async run({ page }) {
      return await page.evaluate(async () => {
        ...
        try {
          ...
          const goalSurvives = mixed.some(h => h.type === 'Project')
            && mixed.some(h => h.type === 'Task');

          // "@goal <query>" and "@goals: <query>" both still narrow to
          // projects — the relabel widened the typed vocabulary, never
          // swapped it (the label is 'Project' since 2026-08-30).
          ...
          const narrowed = spaceForm.length > 0 && colonForm.length > 0
            && spaceForm.every(h => h.type === 'Project')
            && colonForm.every(h => h.type === 'Project');
