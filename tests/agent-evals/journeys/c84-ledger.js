  ...
  },
  {
    ...
    async run({ page, docs }) {
      return await page.evaluate(async (dir) => {
        ...
        // _verify returns {verdicts, goal} since the C9 goal-level verdict;
        // an all-mechanical pass carries no goal judgment (goal: null).
        const v = await TaskService._verify('task_e_pc');
        TaskService._chat = origChat;
        const verdicts = v?.verdicts;
        const pass = !modelCalled && verdicts?.[0]?.ok === true && verdicts?.[1]?.ok === false
          && /does not exist/.test(verdicts?.[1]?.issue || '') && v?.goal === null;
        return { pass, detail: JSON.stringify({ modelCalled, v }) };
      }, docs);
    }
  }
];
