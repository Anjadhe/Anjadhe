...
(async () => {
    // ── manifests ────────────────────────────────────────────────────────
    assert.deepEqual(Specialists.list().map(def => def.id), ['mail', 'calendar', 'tasks', 'planner', 'notes', 'documents', 'research', 'browser', 'writer']);
    ...
    // Planner needs the calendar; Tasks survives without it, so a lock narrows
    // the team rather than blinding it to the half it can still read.
    globalThis.AgentTools = { handlers: { list_schedule: () => {}, list_goals: () => {}, list_calendar_events: () => {} } };
    assert.deepEqual(Specialists.list().map(def => def.id), ['tasks', 'browser', 'writer'], 'A locked Calendar drops planner but leaves Tasks');
    ...
    // A fold keeps facts, not preamble — and never half a fact.
    const calendar = JSON.stringify({ from: '2026-09-21', to: '2026-10-05', timezone: 'America/Los_Angeles',
        note: 'Times are the user\u2019s local wall-clock time (offset included). Present them as given; do not convert.',
        total: 3, returned: 3, events: [1, 2, 3].map(n => ({ id: 'ID-' + 'x'.repeat(40) + n, summary: 'Meeting ' + n, start: '2026-09-2' + n + 'T20:00:00.000Z' })) });
    const foldedCal = AgentLoop._foldContent(calendar, AgentLoop.FOLDED_CHARS);
    assert.ok(!/note/.test(foldedCal), 'Prose is dropped before facts');
    assert.match(foldedCal, /"total":3/, 'What was there is still said');
    assert.match(foldedCal, /not kept/, 'A dropped item says it was dropped');
    const keptCal = JSON.parse(foldedCal);
    assert.ok(keptCal.events.length && keptCal.events.every(event => event.summary && event.start), 'What is kept is whole — never half a fact');
    assert.match(AgentLoop._foldContent(JSON.stringify(Array.from({ length: 40 }, (_, i) => ({ n: i }))), 200), /\{"n":0\}.*more, not kept/, 'A list folds to whole items plus a count');
    // Small results are not folded just for being old: the budget is characters.
    script = [use('look', {}), use('look', {}), use('look', {}), use('look', {}), { message: { content: 'done' } }]; seen.length = 0;
    await AgentLoop.run({ system: 's', input: 'go', tools: [def('look')], execute: async () => ({ events: ['a', 'b'] }) });
    assert.ok(seen.at(-1).messages.filter(message => message.role === 'tool').every(message => /"events"/.test(message.content)),
        'A job made of small reads keeps every one of them');
    // Rule 1 is not only about pages: the same question, the same answer, three times.
    script = [use('list_calendar_events', { from: 'today' }), use('list_calendar_events', { from: 'today' }), use('list_calendar_events', { from: 'today' })];
    out = await AgentLoop.run({ system: 's', input: 'go', tools: [def('list_calendar_events')], execute: async () => ({ total: 0, events: [] }), guard: runtime.createGuard() });
    assert.equal(out.stop, 'guard'); assert.equal(out.reason.reason, 'no_progress');
    assert.match(out.reason.message, /list_calendar_events/, 'The stop names the call it stopped');
