 ...
 *   - the message wears the shared "<Kind>" header (no app name);
...
(async () => {
    ...
    if (darwin) {
        ...
        assert.deepStrictEqual(runs[0].args, ['+14255550100', 'Reminder\nStandup\n9:00 AM'],
    ...
    if (darwin) {
        ...
        assert.match(runs[runs.length - 1].args[1], /^Notification\nTest from nenva/, 'test message wears the header');
    }
    ...
    // sendTo (the assistant's send_text, 2026-09-10): any handle, shape-
    // checked and normalised, the text passed as argv; consent is not
    // this layer's job. Empty text and garbage handles never run a script.
    runs.length = 0; nextResult = { ok: true };
    r = await bridge.sendTo(' +1 (425) 555-0199 ', 'running late');
    assert.ok(r.ok, 'sendTo sends to an arbitrary valid handle');
    assert.strictEqual(r.handle, '+14255550199', 'handle normalised');
    assert.strictEqual(runs.length, 1);
    assert.deepStrictEqual(runs[0].args, ['+14255550199', 'running late'], 'handle + text travel as argv');
    r = await bridge.sendTo('not a handle', 'hi');
    assert.ok(r.error && runs.length === 1, 'garbage handle rejected before any script');
    r = await bridge.sendTo('+14255550199', '   ');
    assert.ok(r.error && runs.length === 1, 'empty text never sends');
    nextResult = { error: "Can't get participant" };
    r = await bridge.sendTo('+14255550199', 'x');
    assert.ok(r.error && !/x/.test(r.error), 'a failed send reports the class, not the text');
