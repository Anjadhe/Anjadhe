...
(async () => {
    ...
    try {
        ...
        // A specialist's report is folded to one line: Anjadhe answers from it.
        assert.match(await page.locator('.wr-msg[data-from="research"] .wr-report-name').textContent(), /Research/);
        assert.equal(await page.locator('.wr-msg[data-from="research"] .wr-sources a').isVisible(), false, 'The report starts folded');
        if (shots) await page.screenshot({ path: path.join(shots, 'wr-2a-report-folded.png') });
        await page.click('.wr-msg[data-from="research"] .wr-report > summary');
        assert.match(await page.locator('.wr-msg[data-from="research"] .wr-sources a').textContent(), /shop\.example\.test/);
        if (shots) await page.screenshot({ path: path.join(shots, 'wr-2b-report-open.png') });
        assert.equal(await page.locator('.wr-thread .wr-steps, .wr-thread pre').count(), 0, 'Tool activity is not in the conversation');
        // …it is one tap away.
        await page.click('.wr-ticker-done');
