...
// ── verdicts: the indicator section lifted off the prose ──────────────
const profileText = `## What the company is
Example makes chips.

## Is the stock expensive, cheap, or in between
Fair, on a forward P/E of 30.

## In the news
- Example beats on revenue (1d ago)

## Bottom line
A solid business at a full price.

## Verdicts
Valuation: Fair
**Soundness:** Strong
- AI: Builder
Near-term risk — Moderate
Long-term risk: Low (the balance sheet)
Near-term value: Strong.
Long-term value: reasonable
News: Positive`;
const pv = PortfolioProfile.parseVerdicts(profileText);
check('verdicts section found', pv.found === true);
check('plain, bold, bulleted and dashed lines all parse', pv.verdicts.valuation === 'Fair' && pv.verdicts.soundness === 'Strong' && pv.verdicts.ai === 'Builder' && pv.verdicts.nearRisk === 'Moderate');
check('parenthetical and trailing period ignored', pv.verdicts.longRisk === 'Low' && pv.verdicts.nearValue === 'Strong');
check('a near-synonym that is not offered for that scale is dropped, never guessed', !('longValue' in pv.verdicts));
check('news tone parses', pv.verdicts.news === 'Positive');
check('body loses the whole section and keeps the prose', !/verdicts/i.test(pv.body) && pv.body.includes('## Bottom line') && pv.body.trim().endsWith('full price.'));
check('stripVerdicts is the body', PortfolioProfile.stripVerdicts(profileText) === pv.body);
const pv2 = PortfolioProfile.parseVerdicts('## Bottom line\nFine.\n\nValuation: overvalued\nSoundness: healthy\nAI: largely unrelated to AI\nNews: none listed');
check('no heading: the tail is scanned and synonyms resolve', pv2.found && pv2.verdicts.valuation === 'Expensive' && pv2.verdicts.soundness === 'Sound' && pv2.verdicts.ai === 'Unrelated' && pv2.verdicts.news === 'None');
check('no heading: only the matched lines leave the prose', pv2.body === '## Bottom line\nFine.');
const pv3 = PortfolioProfile.parseVerdicts('## Verdicts\nValuation: not on the sheet\nAI: Heavy', { fund: true });
check('Unknown is kept as a verdict; fund AI options apply', pv3.verdicts.valuation === 'Unknown' && pv3.verdicts.ai === 'Heavy');
check('equity shape does not accept the fund AI words', !('ai' in PortfolioProfile.parseVerdicts('## Verdicts\nAI: Heavy').verdicts));
check('no verdicts → empty, body untouched', (() => { const r = PortfolioProfile.parseVerdicts('## Bottom line\nJust prose.'); return !r.found && Object.keys(r.verdicts).length === 0 && r.body === '## Bottom line\nJust prose.'; })());
check('empty text', PortfolioProfile.parseVerdicts('').found === false);
check('prompt asks for every verdict line, equity shape', (() => { const p = PortfolioProfile._systemPrompt(false); return PortfolioProfile.VERDICTS.every(v => p.includes(`${v.ask}: ${v.options.join(' / ')}`)) && p.includes('Builder / Supplier'); })());
check('prompt asks for the fund AI words', PortfolioProfile._systemPrompt(true).includes('AI: Heavy / Moderate / Light / Unrelated / Unknown'));
check('current() wants today + text + format', (() => { const today = PortfolioProfile.today(); return PortfolioProfile.current({ day: today, text: 'x', format: PortfolioProfile.FORMAT }) && !PortfolioProfile.current({ day: today, text: 'x' }) && !PortfolioProfile.current({ day: '2020-01-01', text: 'x', format: PortfolioProfile.FORMAT }); })());
