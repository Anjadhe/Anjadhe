...
// ── indicators (the profile's verdicts as sortable columns) ──────────
// The specs a real run reads off PortfolioProfile.VERDICTS; only the
// shape matters here.
const SPECS = [
    { key: 'valuation', label: 'Valuation', options: ['Cheap', 'Fair', 'Expensive'],
      tone: { Cheap: 'good', Fair: 'ink', Expensive: 'bad' } },
    { key: 'soundness', label: 'Soundness', options: ['Fragile', 'Mixed', 'Sound', 'Strong'],
      tone: { Fragile: 'bad', Mixed: 'warn', Sound: 'good', Strong: 'good' } },
    { key: 'ai', label: 'AI role', categorical: true,
      options: ['Builder', 'Supplier', 'Beneficiary', 'Customer', 'Threatened', 'Unrelated'],
      fundOptions: ['Heavy', 'Moderate', 'Light', 'Unrelated'],
      tone: { Builder: 'accent', Heavy: 'accent', Unrelated: 'ink' } },
    { key: 'nearRisk', label: 'Near-term risk', options: ['Low', 'Moderate', 'High'],
      tone: { Low: 'good', Moderate: 'warn', High: 'bad' } },
    { key: 'news', label: 'News tone', options: ['Negative', 'Mixed', 'Positive'], extra: ['None'],
      tone: { Negative: 'bad', Mixed: 'ink', Positive: 'good', None: 'ink' } }
];
const profiles = {
    NVDA: { day: '2026-09-15', fund: false, verdicts: { valuation: 'Expensive', soundness: 'Strong', ai: 'Builder', nearRisk: 'High', news: 'Positive' } },
    AAPL: { day: '2026-09-14', fund: false, verdicts: { valuation: 'Fair', soundness: 'Sound', ai: 'Beneficiary', nearRisk: 'Moderate', news: 'Mixed' } },
    MSFT: { day: '2026-09-15', fund: false, verdicts: { valuation: 'Cheap', soundness: 'Sound', ai: 'Builder', nearRisk: 'Unknown', news: 'None' } },
    VOO: { day: '2026-09-15', fund: true, verdicts: { valuation: 'Fair', ai: 'Moderate' } }
    // NEWCO and the option's underlying have none.
};
const ind = PortfolioTickers.attachIndicators(rows, { specs: SPECS, profileOf: (t) => profiles[t] || null });
const iby = Object.fromEntries(ind.map(r => [r.ticker, r]));

check('a verdict becomes a cell with its word and its rank', iby.NVDA.ind.valuation.word === 'Expensive' && iby.NVDA.ind.valuation.rank === 2 && iby.NVDA.ind.valuation.steps === 3);
check('the tone comes from the spec', iby.NVDA.ind.valuation.tone === 'bad' && iby.MSFT.ind.valuation.tone === 'good');
check('a categorical verdict is marked as one', iby.NVDA.ind.ai.categorical === true && iby.NVDA.ind.valuation.categorical === false);
check('a fund reads its own option list', iby.VOO.ind.ai.word === 'Moderate' && iby.VOO.ind.ai.rank === 1 && iby.VOO.ind.ai.steps === 4);
check('Unknown carries no rank', iby.MSFT.ind.nearRisk.unknown === true && iby.MSFT.ind.nearRisk.rank === null);
check('an extra word (news "None") is kept but unranked', iby.MSFT.ind.news.word === 'None' && iby.MSFT.ind.news.rank === null);
check('a verdict the profile never gave is simply absent', !('soundness' in iby.VOO.ind) && iby.VOO.indCount === 2);
check('a ticker with no profile is not profiled', iby.NEWCO.profiled === false && iby.NEWCO.indCount === 0 && iby.NEWCO.profileDay === null);
check('the profile day rides along', iby.AAPL.profileDay === '2026-09-14' && iby.AAPL.profiled === true);
check('attachIndicators keeps the position columns', iby.NVDA.value === 50000 && iby.NVDA.accountCount === 2);
check('attachIndicators does not mutate its input', !('ind' in rows[0]));

const si = (col, dir) => PortfolioTickers.sort(ind, { col, dir }).map(r => r.ticker);
check('valuation asc puts the cheapest first, unprofiled last',
    si('ind:valuation', 'asc').slice(0, 3).join() === ['MSFT', 'AAPL', 'VOO'].join() && si('ind:valuation', 'asc')[0] === 'MSFT', si('ind:valuation', 'asc').join());
check('valuation desc puts the most expensive first', si('ind:valuation', 'desc')[0] === 'NVDA');
// Sinking together: Unknown (MSFT), a verdict the profile never gave (VOO),
// and no profile at all (NEWCO, the option).
check('unprofiled and Unknown both sink whichever way it sorts',
    si('ind:nearRisk', 'asc').slice(-4).sort().join() === ['MSFT', 'VOO', 'NEWCO', OCC].sort().join() &&
    si('ind:nearRisk', 'desc').slice(-4).sort().join() === ['MSFT', 'VOO', 'NEWCO', OCC].sort().join(),
    si('ind:nearRisk', 'asc').join());
check('soundness desc puts the strongest first', si('ind:soundness', 'desc')[0] === 'NVDA');
check('sorting an indicator column leaves the position columns alone', PortfolioTickers.sort(ind, { col: 'value', dir: 'desc' })[0].ticker === 'NVDA');
check('profile day desc reads newest first, unprofiled last',
    si('profile', 'desc')[si('profile', 'desc').length - 1] !== 'NVDA' && si('profile', 'desc').indexOf('AAPL') > si('profile', 'desc').indexOf('NVDA'));

// First click sorts best first — the reason the view exists.
check('valuation first click is cheapest first', PortfolioTickers.defaultDir('ind:valuation', SPECS) === 'asc');
check('risk first click is lowest first', PortfolioTickers.defaultDir('ind:nearRisk', SPECS) === 'asc');
check('soundness first click is strongest first', PortfolioTickers.defaultDir('ind:soundness', SPECS) === 'desc');
check('news first click is most positive first', PortfolioTickers.defaultDir('ind:news', SPECS) === 'desc');
check('a categorical scale keeps its spec order', PortfolioTickers.defaultDir('ind:ai', SPECS) === 'asc');
check('a position column has no opinion about direction', PortfolioTickers.defaultDir('value', SPECS) === null);

const cov = PortfolioTickers.summarizeIndicators(ind, { today: '2026-09-15' });
check('coverage counts profiled, today and waiting', cov.total === 6 && cov.profiled === 4 && cov.today === 3 && cov.missing === 2);
