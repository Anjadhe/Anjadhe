...
(async () => {
    ...
    // Out of steps with something written up is PARTIAL, not a failure; the master is told so.
    chat = engineStore.command('create', { goal: 'Compare five kettles' }, owner);
    const researchSteps = Specialists.get('research').steps;
    script = [use('delegate', { agent: 'research', task: 'Compare five kettles.' }),
        ...Array.from({ length: researchSteps }, (_, i) => use('web_search', { query: `kettle ${i}` })),
        say('Acme $30 and Zed $35 checked; three not yet verified.'),
        params => { assert.match(params.messages.at(-1).content, /"partial":true/); assert.match(params.messages.at(-1).content, /used all my steps/); return say('Two checked so far.'); }];
    await WorkroomEngine.tick(); await settle(chat.id);
    const partialEnd = engineStore.events(chat.id).find(event => event.type === 'task_end');
    assert.equal(partialEnd.status, 'partial');
    assert.match(partialEnd.note, /used all my steps/);
    assert.equal(partialEnd.text, 'Acme $30 and Zed $35 checked; three not yet verified.', 'The report text is the write-up; the limit is its own note');
    ...
    // Said mid-run: the master alone reads it, once the running task reports back; the job is not restarted.
    ...
    script = [use('delegate', { agent: 'research', task: 'Find a kettle.' }),
        ...
        params => { assert.ok(!params.messages.some(message => /user just added/.test(String(message.content))), 'The specialist never hears it: it sees only the brief the master wrote'); return say('A kettle: https://shop.test/kettle'); },
        params => { assert.ok(params.messages.some(message => /user just added: Make it a red one/.test(String(message.content))), 'The master reads it after the task and decides'); return say('Here is a red one.'); }];
    await WorkroomEngine.tick(); await settle(chat.id);
