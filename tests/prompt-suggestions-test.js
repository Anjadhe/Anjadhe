...
// The flag gates the AI half (2026-09-20). Off means nothing runs and
// nothing is answered, so the surfaces fall back to the static pool even
// when a set was cached while it was on.
globalThis.FEATURES = { isEnabled: () => false };
PS._cache = { items: ['cached one', 'cached two', 'cached three'], at: Date.now(), hash: 'h' };
check('flag off: enabled() is false', PS.enabled() === false);
check('flag off: no written set is offered', PS.current().length === 0);
check('flag off: the engine reports unavailable', PS.available() === false);
globalThis.FEATURES = { isEnabled: () => true };
check('flag on: the cached set is offered again', PS.current().length === 3);
delete globalThis.FEATURES;
check('no FEATURES at all: unchanged behaviour', PS.enabled() === true);
