...
// v2 tier (2026-08-27): the stock 4B saturated v1, and a saturated eval
// cannot rank tuned candidates — these lean on precedence rules, the
// matter/suppression blocks (built in the app's exact shape), year-less
// dates and multi-date bodies. Keep v1 untouched as the regression floor.
const WEEKDAYS = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
const humanNoYear = (d) => `${WEEKDAYS[d.getDay()]}, ${MONTHS[d.getMonth()]} ${d.getDate()}`;
const matterBlockFor = (items) => `
...
const suppressionBlockFor = (items) => `
...
const V2_FIXTURES = [
    {
        id: 'v2-renewal-invoice-precedence',
        // "If the mail is really just the invoice for it, bill comes first."
        from: 'Summit Climbing Gym <billing@summitclimb.com>',
        subject: 'Your annual membership — invoice enclosed',
        body: `Hi Jordan,\n\nYour Summit membership year is up for renewal. Invoice SM-2214: $95.00, payable by ${human(inDays(16))} to keep your membership active. We do not store payment methods, so this is paid manually each year.\n\nSee you on the wall!`,
        expect: { relevant: true, type: 'bill', actionRequired: true, allowedDates: [iso(inDays(16))], amountContains: '95' },
    },
    {
        id: 'v2-bill-autopay-off-action',
        // Autopay ON: same words as an actionable bill, but nothing to do.
        from: 'Grid Utility <billing@gridutility.com>',
        subject: 'Your bill is ready',
        body: `Your monthly energy bill is ready.\n\nAmount: $210.44\nAutopay is enabled — this amount will be drafted from your checking account on ${human(inDays(12))}. No action is needed.`,
        expect: { relevant: true, type: 'bill', actionRequired: false, actionItemsEmpty: true, eventDate: iso(inDays(12)), allowedDates: [iso(inDays(12))], amountContains: '210.44' },
    },
    {
        id: 'v2-overdue-return',
        from: 'Rivertown Library <notices@rivertownlibrary.org>',
        subject: 'Overdue notice',
        body: `The following item is overdue:\n\n"The Hidden Life of Trees", due ${human(inDays(-4))}.\n\nPlease return or renew it by ${human(inDays(6))} to stop further late fees ($0.30/day, $1.20 so far).`,
        expect: { relevant: true, type: 'deadline', actionRequired: true, allowedDates: [iso(inDays(-4)), iso(inDays(6))] },
    },
    {
        id: 'v2-checkin-required',
        from: 'Alaska Airlines <no-reply@alaskaair.com>',
        subject: 'Check in now for your flight tomorrow',
        body: `Your flight departs tomorrow, ${humanNoYear(inDays(1))}.\n\nFlight 610, PDX 7:40 AM to SJC 9:55 AM. Check-in is open now — checking in online saves time at the airport. Confirmation: QRTPLM.`,
        expect: { relevant: true, type: 'reservation', actionRequired: true, allowedDates: [iso(inDays(1))] },
    },
    {
        id: 'v2-security-needs-action',
        from: 'Northgate Credit Union <security@northgatecu.org>',
        subject: 'Unusual activity on your account',
        body: `We detected unusual sign-in activity on your online banking profile.\n\nAs a precaution, please review your recent activity and reset your password. Your cards remain active while you do this.`,
        expect: { relevant: true, type: 'security', actionRequired: true, allowedDates: [] },
    },
    {
        id: 'v2-two-amounts-eventdate',
        // Statement balance vs minimum payment, period end vs due date — the
        // due date is the event that matters.
        from: 'Meridian Bank <alerts@meridianbank.com>',
        subject: 'Credit card statement — payment due',
        body: `Your Meridian Visa statement for the period ending ${human(inDays(-5))} is ready.\n\nStatement balance: $1,842.11\nMinimum payment: $35.00\nPayment due date: ${human(inDays(20))}\n\nAutopay is not set up on this card.`,
        expect: { relevant: true, type: 'bill', actionRequired: true, eventDate: iso(inDays(20)), allowedDates: [iso(inDays(-5)), iso(inDays(20))] },
    },
    {
        id: 'v2-yearless-date',
        // No year anywhere in the body — the model must resolve it forward.
        from: 'Cedar Dental Group <frontdesk@cedardental.com>',
        subject: 'Appointment reminder',
        body: `Hi Jordan,\n\nA reminder of your appointment with Dr. Okafor on ${humanNoYear(inDays(19))} at 3:15 PM.\n\nPlease reply CONFIRM, or call us to change it.`,
        expect: { relevant: true, type: 'appointment', actionRequired: true, eventDate: iso(inDays(19)), allowedDates: [iso(inDays(19))] },
    },
    {
        id: 'v2-matter-continuation',
        // Unmistakably the SAME invoice as the open matter: fold it.
        from: 'Crestline Water District <billing@crestlinewater.gov>',
        subject: 'Reminder: invoice 88231 is due soon',
        body: `This is a courtesy reminder that invoice 88231 for $84.20 is due on ${human(D.due18)}.\n\nIf you have already paid, please disregard this notice.`,
        matterBlock: matterBlockFor([`(bill, ${iso(D.due18)}, $84.20) Water and sewer invoice 88231 due — $84.20`]),
        expect: { relevant: true, type: 'bill', continuesMatter: 1, allowedDates: [iso(D.due18)], amountContains: '84.20' },
    },
    {
        id: 'v2-matter-new-item',
        // Same sender, same kind — but NEXT month's invoice: a new item.
        from: 'Crestline Water District <billing@crestlinewater.gov>',
        subject: 'Invoice 89302 — new billing period',
        body: `Your new water and sewer invoice is ready.\n\nInvoice 89302\nAmount due: $91.75\nDue date: ${human(inDays(48))}\n\nAutopay is not enabled on this account.`,
        matterBlock: matterBlockFor([`(bill, ${iso(D.due18)}, $84.20) Water and sewer invoice 88231 due — $84.20`]),
        expect: { relevant: true, type: 'bill', continuesMatter: null, allowedDates: [iso(inDays(48))], amountContains: '91.75' },
    },
    {
        id: 'v2-suppress-same-kind',
        // The dismissed kind again: suppress. (Delivered-order notices are
        // relevant by the taxonomy; the user has said they don't want them.)
        from: 'DoorDash <no-reply@doordash.com>',
        subject: 'Your order was delivered',
        body: `Your order from Thai Basil was delivered at 6:42 PM. Enjoy!\n\nRate your delivery in the app.`,
        suppressionBlock: suppressionBlockFor(['(delivery) DoorDash order from Pho Corner was delivered', '(delivery) DoorDash order from Taqueria Luz was delivered']),
        expect: { relevant: true, type: 'delivery', actionRequired: false, suppress: true, allowedDates: [] },
    },
    {
        id: 'v2-suppress-never-actionable',
        // Same sender as the dismissed noise — but THIS one needs action, and
        // the block itself says never suppress actionable mail.
        from: 'DoorDash <no-reply@doordash.com>',
        subject: 'Action needed: problem with your payment',
        body: `We couldn't charge your card for last night's order from Thai Basil ($31.60).\n\nPlease update your payment method by ${human(inDays(5))} to avoid account suspension.`,
        suppressionBlock: suppressionBlockFor(['(delivery) DoorDash order from Pho Corner was delivered', '(delivery) DoorDash order from Taqueria Luz was delivered']),
        expect: { relevant: true, type: 'bill', actionRequired: true, suppress: false, allowedDates: [iso(inDays(5))], amountContains: '31.60' },
    },
];
// v3 tier (2026-08-31): relative timeframes, named on the 16 GB reference
// Mac. The line every model from the 4B to the Studio's 27B converges on
// (measured, all at temp 0): an EXACT offset the email itself states
// ("renews in 30 days", "trial ends in 7 days") is resolved against the
// email's date — arithmetic on two quoted facts — while a FUZZY range
// ("arrive in 3-5 business days") yields no date at all. The shipped
// prompt's law is ambiguous exactly here ("a date must actually appear in
// the text" vs "never derive by adding a TYPICAL payment window"); until
// the prompt says otherwise, this tier pins the convergent reading:
// exact offsets must resolve to the RIGHT date (small-model arithmetic is
// a known weak spot), fuzzy ranges must stay null. The tuned v2's real
// regression — deriving from the fuzzy range when an injection body is
// present — is pinned by v1's injection-in-delivery.
const V3_FIXTURES = [
    {
        id: 'v3-relative-delivery-window',
        from: 'Cascade Outfitters <orders@cascadeoutfitters.com>',
        subject: 'Your order #77341 has shipped',
        body: `Good news — your order #77341 (trail shoes, $118.00) has shipped!\n\nIt should arrive in 3-5 business days. We'll email you when it's out for delivery.`,
        expect: { relevant: true, type: ['delivery', 'receipt'], actionRequired: false, actionItemsEmpty: true, allowedDates: [] },
    },
    {
        id: 'v3-relative-renewal-days',
        from: 'Ridge Gym <billing@ridgegym.com>',
        subject: 'Your membership renews in 30 days',
        body: `Hi Jordan,\n\nA heads up that your annual Ridge Gym membership ($420.00) renews automatically in 30 days.\n\nNo action is needed if you'd like to keep your membership.`,
        expect: { relevant: true, type: ['renewal', 'bill'], allowedDates: [iso(inDays(30))], amountContains: '420' },
    },
    {
        id: 'v3-relative-trial-ending',
        from: 'Streamline <no-reply@streamline.tv>',
        subject: 'Your free trial ends in 7 days',
        body: `Your Streamline free trial ends in 7 days. After that you'll be charged $9.99/month unless you cancel.\n\nManage your plan any time in Settings.`,
        expect: { relevant: true, allowedDates: [iso(inDays(7))], amountContains: '9.99' },
    },
];
for (const f of FIXTURES) f.tier = 'v1';
for (const f of V2_FIXTURES) f.tier = 'v2';
for (const f of V3_FIXTURES) f.tier = 'v3';
FIXTURES.push(...V2_FIXTURES, ...V3_FIXTURES);
...
function chat(messages) {
    const body = JSON.stringify({
        ...
        // Greedy decoding: rankings and training labels must be
        // reproducible — at the server's default temperature the 4B's
        // injection verdict flipped between runs.
        temperature: 0,
...
function score(fx, a) {
    ...
    if (e.suppress !== undefined && (a.suppress === true) !== e.suppress) {
        fails.push(`suppress: got ${a.suppress}, want ${e.suppress}`);
    }
    if (e.continuesMatter !== undefined) {
        const got = (typeof a.continuesMatter === 'number') ? a.continuesMatter : null;
        if (got !== e.continuesMatter) fails.push(`continuesMatter: got ${got}, want ${e.continuesMatter}`);
    }
    return fails;
}

// INSIGHT_ONLY=<id substring> runs a subset (fixture-level debugging);
// a partial run never overwrites the saved full-run scorecard.
const ONLY = process.env.INSIGHT_ONLY || '';

async function main() {
    const fixtures = FIXTURES.filter(fx => !ONLY || fx.id.includes(ONLY));
    console.log(`email-insight eval · model ${MODEL} · port ${PORT} · ${fixtures.length} fixtures · today ${TODAY}`);
    const rows = [];
    for (const fx of fixtures) {
        // Per-fixture system: v2 fixtures carry matter/suppression blocks in
        // the app's exact shape; v1 fixtures stand alone.
        const system = EmailInsightPrompt.system(TODAY, fx.matterBlock || '', fx.suppressionBlock || '');
        ...
        rows.push({ id: fx.id, tier: fx.tier, pass: fails.length === 0, ms, fails });
        console.log(`${fails.length ? 'FAIL' : 'PASS'} [${fx.tier}] ${fx.id} (${(ms / 1000).toFixed(1)}s)${fails.length ? ' — ' + fails.join('; ') : ''}`);
        // The raw verdict is the diagnosis — a FAIL line alone can't say
        // whether a bad date was obeyed injection or derived arithmetic.
        if (fails.length) console.log(`  raw: ${raw.replace(/\s+/g, ' ').slice(0, 500)}`);
    }

    const passed = rows.filter(r => r.pass).length;
    const totalMs = rows.reduce((s, r) => s + r.ms, 0);
    const tierLine = ['v1', 'v2', 'v3'].map(t => {
        const rs = rows.filter(r => r.tier === t);
        return `${t} ${rs.filter(r => r.pass).length}/${rs.length}`;
    }).join(' · ');
    console.log(`\nScore: ${passed}/${rows.length} (${tierLine}) · ${(totalMs / 1000).toFixed(0)}s total · ${(totalMs / rows.length / 1000).toFixed(1)}s/email`);
